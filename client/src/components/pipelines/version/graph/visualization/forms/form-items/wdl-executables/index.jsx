...
import styles from './wdl-executables.module.css';
...
class WdlExecutables extends React.Component {
  ...
  updateExecutables = () => {
    const {document: wdlDocument} = this.props;
    const {executables = []} = wdlDocument || {};
    this.setState({
      executables,
    });
  };
  ...
  renderExecutableInfo = (executable) => {
    ...
    if (info.length > 0) {
      return (
        <span className={classNames(styles.taskInfo, 'cp-text-not-important')}>
          ({info.join(', ')})
        </span>
      );
    }
  ...
  renderExecutableHeader = (executable) => {
    const {disabled, document: wdlDocument} = this.props;
    const {name, type} = getEntityNameOptions(executable, wdlDocument);
    return (
      <div className={styles.header}>
        <div className={styles.headerTitle}>
          {type && <span style={{marginRight: 5}}>{type}</span>}
          <b>{name}</b>
  ...
  renderExecutable = (executable) => {
    const {executions = [], issues = []} = executable;
    const {onSelect, disabled, document: wdlDocument} = this.props;
    ...
    return (
      <div className={styles.taskDetails}>
        {thisDocumentExecutions.length === 0 && (
          <div className={classNames('cp-text-not-important', styles.row)}>No calls specified.</div>
        )}
        {thisDocumentExecutions.length > 0 && (
        ...
        {thisDocumentExecutions.map((execution) => (
          <div key={execution.uuid} className={styles.row}>
            <a onClick={() => onSelectExecution(execution)}>
        ...
        {!disabled && !!wdlDocument && (
          <div className={styles.row}>
            <a onClick={() => onCreateCall()}>
              <PlusOutlined /> create new call
            </a>
          </div>
        )}
        {issues.length === 0 && (
          <div className={classNames('cp-text-not-important', styles.row)}>No issues found.</div>
        )}
        {issues.length > 0 && (
          <div className={styles.row}>
        ...
        {issues.length > 0 && <WdlIssues issues={issues} alert fullDescription />}
      </div>
    );
  };

  render() {
    const {className, style, document: wdlDocument} = this.props;
    if (!wdlDocument) {
      return null;
    }
    const {executables, expandedKeys = []} = this.state;
    return (
      <div className={classNames((className, styles.container))} style={style}>
        <div className={classNames(styles.header, styles.mainHeader)}>
