...
export default class LaunchPipelineForm extends localization.LocalizedReactComponent {
  ...
  static propTypes = {
    ...
    toolInstanceTypes: PropTypes.array,
    priceTypes: PropTypes.arrayOf(PropTypes.bool),
  ...
  generateConfigurationPayload = (values) => {
    let payload = {
      ...
      awsRegionId: values[EXEC_ENVIRONMENT].awsRegionId ? +values[EXEC_ENVIRONMENT].awsRegionId : undefined
    };
  ...
  get instanceTypes () {
    if (this.state.pipeline) {
      return this.props.instanceTypes;
    } else {
      return this.props.toolInstanceTypes;
    }
  }
  ...
  renderAWSRegionSelection = () => {
    ...
    return (
      <FormItem
        ...
        hasFeedback
        label="Cloud Region">
        {this.getSectionFieldDecorator(EXEC_ENVIRONMENT)('awsRegionId',
          {
            rules: [
              {
                required: this.awsRegions.length > 0 && !this.state.isDts,
            ...
            initialValue: this.getDefaultValue('awsRegionId') || this.defaultAwsRegionId
          })(
          <Select
            disabled={!!this.state.fireCloudMethodName || (this.props.readOnly && !this.props.canExecute)}
            showSearch
  ...
  correctPriceTypeValue = (value) => {
    if (value !== undefined && value !== null) {
      ...
      const [v] = this.props.priceTypes.filter(v => v === realValue);
      if (v !== undefined) {
        return `${v}`;
      } else if (this.props.priceTypes.length > 0) {
        return `${this.props.priceTypes[0]}`;
      } else {
        return undefined;
      }
    }
