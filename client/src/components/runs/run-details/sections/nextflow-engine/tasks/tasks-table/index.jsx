...
const columns = [
  ...
  },
  {
    ...
    title: 'Tag',
    sorting: true,
    filter: (props) => <TasksTableTagFilter {...props} />,
  },
  {
    key: 'status',
    size: '100px',
    ...
    filter: (props) => <TasksTableStatusesFilter {...props} />,
  },
  {
    key: 'startDate',
    title: 'Started',
...
function TasksTableColumn(props) {
  ...
  const {key, title, sorting: sortingEnabled = false, filter: ColumnFilter} = column || {};
  ...
  return (
    ...
    >
      <span>{title || key}</span>
      {ColumnFilter && onFilterChange ? (
        <ColumnFilter filter={filter} onFilterChange={onFilterChange} style={{marginLeft: 5}} />
      ) : undefined}
      {sortingEnabled && (
        <div className={styles.cellSorting}>
          <UpOutlined
            className={classNames(styles.cellSortingControl, {
  ...
  render() {
    ...
    const onChangeColumnSorting = (columnKey, descending) => {
      ...
      if (multipleSorting) {
        ...
        if (descending !== undefined) {
          onSortingChange([{column: columnKey, descending}]);
        } else {
          onSortingChange([]);
        }
      }
    ...
    const tableStyle = {
      gridTemplateColumns: columns
        .map((column) => `[${getTableColumnName(column.key)}] ${getTableColumnSize(column.size)}`)
        .join(' '),
      gridTemplateRows:
        tasks.length > 0 ? `[header] 30px repeat(${tasks.length}, 30px)` : '[header] 30px',
    };
    return (
      <div className={classNames(className, styles.tasksTableContainer)} style={style}>
        <div className={classNames(styles.tasksTableHeader, 'cp-card-background-color')}>
          {taskGroupFilter ? (
            <span>
              <span style={{fontWeight: 'bold'}}>{taskGroupFilter}</span>
              <span style={{marginLeft: 5}}>process tasks</span>
            </span>
          ) : (
          ...
          {total > 0 && (
            <span style={{marginLeft: 5}}>
              {total > totalFiltered ? `(${totalFiltered} / ${total})` : `(${totalFiltered})`}
            </span>
          )}
        ...
        {tasks.length === 0 && (
          <div className={styles.tasksInfo}>
            {error && <Alert title={error} showIcon type="warning" />}
            {pending && !error && (
