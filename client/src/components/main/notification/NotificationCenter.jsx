...
function dateSorter(notificationA, notificationB) {
  const dateA = toUtcDayjs(notificationA.createdDate);
  const dateB = toUtcDayjs(notificationB.createdDate);
  if (dateA > dateB) {
    return -1;
  } else if (dateA < dateB) {
    return 1;
  }
  return notificationA.notificationId.localeCompare(notificationB.notificationId);
}
...
export default class NotificationCenter extends React.Component {
  ...
  _notificationsBottomBound = 0;
  _notificationsOnScreen = 0;
  readUserNotifications = 0;
  ...
  get userNotifications() {
    ...
    return [...(userNotifications.value.elements || [])]
      .filter((message) => {
        return (
          !message.isRead &&
          (dateFrom ? toUtcDayjs(message.createdDate).isAfter(toUtcDayjs(dateFrom)) : true)
        );
      })
      .map(mapMessage)
      .sort(dateSorter);
  ...
  get nonBlockingNotifications() {
    return this.allNotifications.filter((n) => !n.blocking);
  }
  ...
  get userNotificationsEnabled() {
    const {userNotifications, preferences, disableEmailNotifications} = this.props;
  ...
  notificationIsVisible = (notification) => {
    const isHidden = !!this.getHiddenNotifications().find(
      (n) => n.id === notification.notificationId && n.createdDate === notification.createdDate,
    );
    const state = this.state.notificationsState.find(
      (n) => n.id === notification.notificationId && n.height !== undefined,
    );
  ...
  getPositioningInfo = (notification, index) => {
    if (this.notificationIsVisible(notification)) {
      ...
      for (let i = 0; i <= index; i++) {
        ...
        const [state] = this.state.notificationsState.filter(
          (n) => n.id === notificationItem.notificationId,
        );
        if (this.notificationIsVisible(notificationItem) && state && state.height !== undefined) {
          ...
          if (doesFit) {
            top = previouslyRenderedItem ? top + state.height + SystemNotification.margin : top;
            bottom = previouslyRenderedItem ? top : top + state.height + SystemNotification.margin;
  ...
  readMessage = async (notification, forceRead = false) => {
    ...
    const buffer = 5;
    const force =
      forceRead || DEFAULT_PAGE_SIZE - this.readUserNotifications <= MAX_NOTIFICATIONS + buffer;
    const request = new ReadMessage();
    const payload = unMapMessage(notification);
    payload.isRead = true;
    payload.readDate = dayjs.utc().format('YYYY-MM-DD HH:mm:ss.SSS');
  ...
  onHideAllClick = (event) => {
    ...
    if (userNotifications) {
      userNotifications.hideNotifications();
    }
  };
  ...
  renderSeverityIcon = (notification) => {
    switch (notification.severity) {
      case 'INFO':
        return <InfoCircleOutlined className="cp-setting-info" />;
      case 'WARNING':
        return <ExclamationCircleOutlined className="cp-setting-warning" />;
      case 'CRITICAL':
        return <CloseCircleOutlined className="cp-setting-critical" />;
      default:
        return undefined;
    }
  };
  ...
  renderControls = () => {
    const {hiddenNotifications} = this.state;
    const hiddenAmount =
      this.nonBlockingNotifications.filter(
        (n) => !hiddenNotifications.find(({id}) => id === n.notificationId),
    ...
    return (
      ...
      >
        <span>{`+${this.unreadCount - this.notificationsOnScreen} notifications`}</span>
        <a onClick={this.onHideAllClick} style={{paddingLeft: '10px'}}>
          hide
        </a>
        <a
          onClick={(e) => {
          ...
          className={classNames('cp-divider', 'left', {
  ...
  render() {
    const filterBlockingNotification = (notification) => {
      const [state] = this.getHiddenNotifications().filter(
        (n) => n.id === notification.notificationId && n.createdDate === notification.createdDate,
      );
    ...
    return (
      <div id="notification-center" style={{position: 'absolute'}}>
        {this.notificationsToShow.map((notification, index) => {
          return (
            <SystemNotification
              ...
              onClick={
                notification.notificationType === NOTIFICATION_TYPE.message
        ...
        >
          {blockingNotification ? <Markdown md={blockingNotification.body} /> : null}
        </Modal>
        {this.state.previewNotification ? (
          <Modal
            ...
            open
          >
            <PreviewNotification text={this.state.previewNotification.body} sanitize />
          </Modal>
        ) : null}
      </div>
    );
  ...
  fetchUserNotifications = (force = false) => {
    ...
    if (force) {
      return this.props.userNotifications.fetch().then(() => {
        this.readUserNotifications = 0;
      });
    }
    this.fetchNotificationsTimeout = setTimeout(() => {
      this.props.userNotifications.fetch().then(() => {
        this.readUserNotifications = 0;
      });
    }, 1500);
  };
  ...
  onFetched = (notifications) => {
    ...
    if (hiddenNotificationsInStorageStr) {
      ...
      const activeHiddenNotifications = hiddenNotifications.filter(
        (n) => activeNotifications.filter((a) => a.notificationId === n.id).length > 0,
      );
...
export {PredefinedNotifications, NOTIFICATION_TYPE};
