...
import dayjs, {ensureDayjs, fromNowUtc, toUtcDayjs} from '../../../../utils/dayjs';
...
export default class ActivitiesPanel extends localization.LocalizedReactComponent {
  ...
  findElement = (id, type) => {
    if (type === 'TOOL') {
      ...
      const findEntity = ({storages, childFolders, pipelines, configurations}) => {
        ...
        for (let i = 0; i < (childFolders || []).length; i++) {
          const result = findEntity(childFolders[i]);
          if (result) {
            return result;
          }
        }
  ...
  get issuesEvents() {
    if (this.props.issues.loaded) {
      ...
      events.sort((eA, eB) => {
        const dateA = ensureDayjs(eA.date);
        const dateB = ensureDayjs(eB.date);
        if (dateA < dateB) {
          return 1;
        } else if (dateA > dateB) {
          return -1;
  ...
  renderEvent = (event) => {
    ...
    if (event.entity) {
      ...
      switch (event.entity.type) {
        case 'PIPELINE':
          name = (
            <span>
              <ForkOutlined /> {event.entity.name}
            </span>
          );
        ...
        case 'TOOL':
          name = (
            <span>
              <ToolOutlined /> {event.entity.name}
            </span>
          );
        ...
        case 'FOLDER':
          name = (
            <span>
              <FolderOutlined /> {event.entity.name}
            </span>
          );
        ...
        case 'CONFIGURATION':
          name = (
            <span>
              <SettingOutlined /> {event.entity.name}
            </span>
          );
      ...
      if (name) {
        parent = (
          <span>
            ...
            for{' '}
            <b>
              {type} <Link to={event.entity.url}>{name}</Link>
            </b>
          </span>
        );
      }
    ...
    switch (event.type) {
      case NEW_ISSUE_EVENT:
        title = (
          <Row>
            <b>{owner}</b> created new {this.localizedString('issue')} <b>{event.object.name}</b>
            {parent} {fromNowUtc(event.date)}
            {modified}:
          </Row>
        );
        content = <IssueCommentPreview style={{padding: 0}} text={event.object.text} />;
        break;
      case NEW_COMMENT_EVENT:
        title = (
          <Row>
            <b>{owner}</b> commented {this.localizedString('issue')} <b>{event.parent.name}</b>
            {parent} {fromNowUtc(event.date)}
            {modified}:
          </Row>
        );
        content = (
          <IssueCommentPreview style={{padding: 0, fontSize: 'small'}} text={event.object.text} />
        );
        break;
      case UPDATE_ISSUE_EVENT:
        title = (
          <Row>
            <b>{owner}</b> updated an {this.localizedString('issue')} <b>{event.object.name}</b>
            {parent} {fromNowUtc(event.date)}
            {modified}:
          </Row>
        );
      ...
      case UPDATE_COMMENT_EVENT:
        title = (
          <Row>
            <b>{owner}</b> updated comment for {this.localizedString('issue')}{' '}
            <b>{event.parent.name}</b>
            {parent} {fromNowUtc(event.date)}
            {modified}:
          </Row>
        );
    ...
    return [
      <Row type="flex" key="title" className="cp-dashboard-panel-card-header" style={{padding: 5}}>
        {title}
      </Row>,
      <Row type="flex" key="content" style={{padding: 5}}>
        {content}
      </Row>,
    ];
  };
  ...
  renderContent = () => {
    ...
    if (lastVisited) {
      lastVisited = toUtcDayjs(lastVisited);
    }
    return (
      <div key="cards" style={{flex: 1, overflow: 'auto'}}>
        {this.issuesEvents.map((event, index) => {
          const isNew = !lastVisited || lastVisited.isBefore(toUtcDayjs(event.date));
          return (
            <Card
              className={classNames('cp-panel-card', {
