...
export default class ToolGroupSelector extends React.Component {
  ...
  get currentGroup() {
    return this.state.value
      ? this.props.groups.filter((r) => r.name === this.state.value)[0]
      : null;
  }
  ...
  render() {
    ...
    if (
      this.props.disabled ||
      this.props.groups.filter((r) => !this.currentGroup || r.id !== this.currentGroup.id)
        .length === 0
    ) {
      return (
        <Button
        ...
        >
          {this.currentGroup
            ...
            : this.state.value || 'Unknown group'}
        </Button>
      );
    }
    ...
    return (
      <Dropdown
        ...
        trigger={['click']}
        popupRender={() => (
          <div className={classNames(styles.navigationDropdownContainer, 'rc-dropdown-menu')}>
            <Row type="flex">
              <Input.Search
  ...
  componentDidUpdate() {
    if ((this.props.groups || []).length > 0 && !this.state.value) {
      this.updateState();
    }
  }
