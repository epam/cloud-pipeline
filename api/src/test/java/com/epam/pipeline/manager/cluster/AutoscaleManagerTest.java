...
public class AutoscaleManagerTest {
    ...
    public void setUp() throws Exception {
        ...
        autoscaleManager = new AutoscaleManager(pipelineRunManager, executorService, clusterManager, nodesManager,
                                                kubernetesManager, preferenceManager, TEST_KUBE_NAMESPACE);
        ...
        when(clusterManager.fillInstance(any(RunInstance.class)))
    ...
    public void testAutoChangeToSpot() {
        when(clusterManager.scaleUp(Mockito.eq(TEST_RUN_ID.toString()),
        ...
        verify(clusterManager).scaleUp(Mockito.eq(TEST_RUN_ID.toString()),
                                       argThat(Matchers.hasProperty("spot", Matchers.is(true))));

        autoscaleManager.runAutoscaling(); // this time it should be a on-demand request
        verify(clusterManager, times(2))
            .scaleUp(Mockito.eq(TEST_RUN_ID.toString()), argThat(
