...
class Scrollbar extends React.PureComponent {
  ...
  onMouseDown = (e) => {
    if (this.attachToScroll && this.container) {
      ...
      const {scrollTop, scrollLeft, clientWidth, clientHeight, scrollWidth, scrollHeight} =
        this.attachToScroll;
      ...
      if (visible) {
        ...
        this.setState({
          moving: {
            ...
            scroll: isVertical ? scrollTop : scrollLeft,
            ratio,
          },
        });
      }
  ...
  render() {
    const {className, direction, width, height, style} = this.props;
    const {moving} = this.state;
    return (
      <div
        ref={this.initializeContainer}
        className={classNames(styles.scrollbar, 'cp-conflict-scroller', className)}
        style={{
          ...style,
