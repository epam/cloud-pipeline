...
import UpdatePathPermissions from '../../../../../../models/dataStorage/permissions/update-path-permissions';
import FetchPathPermissions from '../../../../../../models/dataStorage/permissions/fetch-path-permissions';
...
export async function fetchStorageItemPermissions(storageId, storagePath) {
  ...
  return result.map((r) => {
    const {name, principal: isPrincipal, mask} = r;
    return {
      storageId,
...
export async function fetchStorageItemsPermissions(storageId, storagePaths = []) {
  const results = await Promise.all(
    storagePaths.map((sp) => fetchStorageItemPermissions(storageId, sp)),
  );
...
export async function saveStorageItemPermissions(storageId, permissions, initial = []) {
  ...
  const payloads = [...uniquePaths].map((up) => {
    ...
    const pathPermissions = permissions.filter(
      (p) => (p.storagePath || '/') === path && p.type === type,
    );
...
export function normalizePermissions(permissions = []) {
  const allWriteAllowed = !permissions.some(
    (permission) => !roleModel.writeAllowed({mask: permission.mask}),
  );
  const allWriteDenied = !permissions.some((permission) =>
    roleModel.writeAllowed({mask: permission.mask}),
  );
  return {
