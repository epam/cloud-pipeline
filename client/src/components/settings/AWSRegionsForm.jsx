...
import styles from './AWSRegionsForm.module.css';
...
export default class AWSRegionsForm extends React.Component {
      ...
      return (this.props.awsRegions.value || [])
        .map((r) => {
          return {
            ...
            clusterStateRegionProperties: parseSLSProperties(r.clusterStateRegionProperties),
  ...
  loadAvailableRegionIds = () => {
    if (this.state.newRegion || this.state.currentProvider) {
      this.awsRegionIds = this.props.availableCloudRegions.load(
        this.state.newRegion || this.state.currentProvider,
      );
  ...
  get availableRegionIds() {
    ...
    if (this.awsRegionIds.loaded) {
      return (this.awsRegionIds.value || []).map((r) => r).sort(defaultSorter);
    }
    return [];
  }
    ...
    const columns = [
      {
        ...
        render: (name, region) => {
          ...
          if (region.isNew) {
            return (
              <i className={styles.region}>
                {name} ({this.state.newRegion})
              </i>
            );
          }
          return (
    ...
    return (
      <Table
        ...
        rowClassName={(region) => {
          ...
          return classNames(styles.regionRow, 'cp-settings-sidebar-element', {
  ...
  updateMounts = async (regionId, originalMounts, targetMounts) => {
    if (!CloudRegionFileShareMountsFormItem.fileShareMountsAreEqual(originalMounts, targetMounts)) {
      ...
      for (let i = 0; i < newMounts.length; i++) {
        ...
        if (request.error) {
          message.error(`Error creating mount ${payload.mountRoot}: ${request.error}`, 3);
        }
      }
      for (let i = 0; i < updatedMounts.length; i++) {
        ...
        await request.send(payload);
        if (request.error) {
          message.error(`Error updating mount ${payload.mountRoot}: ${request.error}`, 3);
        }
      }
      for (let i = 0; i < deletedMounts.length; i++) {
        ...
        await request.fetch();
        if (request.error) {
          message.error(`Error deleting mount ${deletedMounts[i].mountRoot}: ${request.error}`, 3);
        }
      }
      hide();
    }
  ...
  renderProviderForm = () => {
    ...
    if (currentProvider && !currentRegionId && !newRegion) {
      return <ProviderForm provider={currentProvider} />;
    }
    return null;
  };
  ...
  render() {
    ...
    return (
      <div style={{flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column'}}>
        ...
        <Splitter style={{flex: 1, minHeight: 0}} className="cp-transparent-background">
          <Splitter.Panel defaultSize={200} min={100} style={{overflow: 'auto'}}>
...
class AWSRegionForm extends React.Component {
  ...
  static Section = ({title, layout, className, children}) => {
    return (
      <Row className={classNames(styles.sectionContainer, className)}>
        <Col
          xs={layout.labelCol.xs.span}
          sm={layout.labelCol.sm.span}
          className={classNames(styles.sectionTitle, 'cp-settings-form-item-label')}
        >
          <span>{title}</span>
        </Col>
        <Col xs={layout.wrapperCol.xs.span} sm={layout.wrapperCol.sm.span}>
          {children}
        </Col>
      </Row>
    );
    ...
    if (this.provider) {
      ...
      const [cloudRegionField] = fields.filter((f) => {
        return (
          ...
          (typeof f === 'object' && Object.hasOwn(f, 'key') && f.key === field)
        );
      });
      if (!cloudRegionField) {
        classNames.push(styles.hiddenItem);
      } else if (
        Object.hasOwn(cloudRegionField, 'visible') &&
  ...
  providerSupportsField = (field) => {
    if (this.provider) {
      ...
      if (cloudRegionField && typeof cloudRegionField === 'string') {
        ...
        cloudRegionField &&
        Object.hasOwn(cloudRegionField, 'required') &&
  ...
  onFormFieldChanged = () => {
    ...
    const checkMounts = () => {
      return !CloudRegionFileShareMountsFormItem.fileShareMountsAreEqual(
        this.props.region.fileShareMounts,
        this.props.form.getFieldValue('fileShareMounts'),
      );
    };
    const checkRunShiftPolicy = () =>
      !runShiftPoliciesEqual(
    ...
    const isContextualSettingChanged = (setting) => {
      if (this.provider && this.providerSupportsContextualSetting(setting)) {
        ...
        const initial = contextualSettingInitialValue
          ...
          : undefined;
        const current = contextualSettingValue ? contextualSettingValue[setting] : undefined;
  ...
  get permissionsModified() {
    return (
      (this.usersToAddPermission && this.usersToAddPermission.length) ||
  ...
  get permissionsDisplayList() {
    ...
    if (this.permissions && this.permissions.loaded) {
      permissions =
        this.permissions.value && this.permissions.value.permissions
  ...
  handleSubmit = (e) => {
    ...
    this.props.form
      ...
      .then(async (values) => {
        if (
          /^azure$/i.test(this.provider) &&
          !values.priceOfferId &&
          !values.enterpriseAgreements
        ) {
        ...
        if (this.cloudRegionFileShareMountsComponent?.validate) {
        ...
        if (CloudRegionFileShareMountsFormItem.validationPassed(values.fileShareMounts)) {
          ...
          const {contextualSettingValue, contextualSettingInitialValue} = this.state;
          const settings = [
            ...new Set([
          ...
          const contextualSettings = settings
            .map((setting) => {
              ...
              return {
                setting,
                value: contextualSettingValue ? contextualSettingValue[setting] : undefined,
          ...
          if (this.props.isNew) {
            if (this.props.onCreate) {
              await this.props.onCreate(values, contextualSettings);
            }
          } else {
            if (this.props.onSubmit) {
              await this.props.onSubmit(values, contextualSettings);
            }
  ...
  findGroupDataSource = () => {
    const roles =
      this.props.roles.loaded && this.state.groupSearchString
        ? (this.props.roles.value || [])
            .filter(
              (r) => r.name.toLowerCase().indexOf(this.state.groupSearchString.toLowerCase()) >= 0,
            )
  ...
  onContextualSettingsChanged = () => {
    const {contextualSettingValid} = this.state;
      ...
      <div style={{width: '100%', flex: 1, display: 'flex', flexDirection: 'column'}}>
        ...
        >
          ...
          >
            ...
            >
              <Input size="small" disabled={this.props.pending} />
            </Form.Item>
            <Form.Item
              label="Name"
              required
            ...
            >
              <Checkbox disabled={this.props.region.default}>This is default cloud region</Checkbox>
            </Form.Item>
            <Form.Item
              label="User"
            ...
            <AWSRegionForm.Section title="Mount rules:" layout={this.formItemLayout}>
              <Form.Item
              ...
              >
                <Select size="small" allowClear={false} style={{marginTop: 4}}>
            ...
            {managedIdentityAvailable ? (
              ...
              >
                <Checkbox size="small" disabled={this.props.pending} />
              </Form.Item>
            ) : null}
            {managedIdentityAvailable && authorizeByManagedIdentity ? (
              <Form.Item
                ...
                rules={[
                  {
                    required:
                      this.providerSupportsField('managedIdentity') && authorizeByManagedIdentity,
            ...
            >
              <Checkbox>Hierarchical storage namespace</Checkbox>
            </Form.Item>
            <Form.Item
            ...
            <Form.Item
              ...
              className={this.getFieldClassName(
                'sshPublicKeyPath',
                'edit-region-sshPublicKeyPath-container',
              )}
            ...
            <Form.Item
              ...
              className={this.getFieldClassName(
                'meterRegionName',
                'edit-region-meterRegionName-container',
              )}
              ...
              rules={[
                {
                  required: this.providerSupportsField('meterRegionName'),
                  message: 'Meter Region Name is required',
                },
              ]}
            >
              <Input size="small" disabled={this.props.pending} />
            ...
            <Form.Item
              ...
              rules={[
                {
                  required: this.providerSupportsField('azureApiUrl'),
                  message: 'Azure API Url is required',
                },
              ]}
            >
              <Input size="small" disabled={this.props.pending} />
            ...
            <Form.Item
              ...
              className={this.getFieldClassName(
                'priceOfferId',
                'edit-region-priceOfferId-container',
              )}
              name="priceOfferId"
            ...
            <Form.Item
              ...
              className={this.getFieldClassName(
                'enterpriseAgreements',
                'edit-region-enterpriseAgreements-container',
              )}
              name="enterpriseAgreements"
              valuePropName="checked"
              initialValue={this.props.region.enterpriseAgreements}
            >
              <Checkbox>Enterprise Agreement</Checkbox>
            </Form.Item>
            <Form.Item
            ...
            <Form.Item
              ...
              className={this.getFieldClassName(
                'applicationName',
                'edit-region-application-name-container',
              )}
            ...
            <Form.Item
              ...
              className={this.getFieldClassName(
                'tempCredentialsRole',
                'edit-region-tempCredentialsRole-container',
              )}
              ...
              rules={[
                {
                  required: this.providerSupportsField('tempCredentialsRole'),
                  message: 'Temp Credentials Role is required',
                },
              ]}
            >
              <Input size="small" disabled={this.props.pending} />
            ...
            <Form.Item
              ...
              className={this.getFieldClassName(
                'omicsServiceRole',
                'edit-region-omicsServiceRole-container',
              )}
            ...
            <Form.Item
              className={this.getFieldClassName(
                'versioningEnabled',
                'edit-region-versioningEnabled-container',
              )}
              {...this.defaultCheckBoxFormItemLayout}
            ...
            >
              <Checkbox>Versioning enabled</Checkbox>
            </Form.Item>
            <Form.Item
            ...
            <Form.Item
              ...
              rules={[
                {
                  required: this.providerSupportsField('kmsKeyArn'),
                  message: 'KMS Key Arn is required',
                },
              ]}
            >
              <Input size="small" disabled={this.props.pending} />
            ...
            <Form.Item
              ...
              className={this.getFieldClassName(
                'azurePolicy',
                'edit-region-subscription-container',
              )}
              name="azurePolicy"
              initialValue={this.props.region.azurePolicy}
            >
              <IPRangeFormItem disabled={this.props.pending} />
            </Form.Item>
            <Form.Item
              label="Subscription"
              {...this.formItemLayout}
            ...
            >
              <RunShiftPolicy disabled={this.props.pending} />
            </Form.Item>
            <Form.Item
            ...
            <Form.Item
              ...
              className={this.getFieldClassName(
                'customInstanceTypes',
                'edit-region-custom-instance-types-container',
              )}
            ...
            >
              <Checkbox>Show standalone nodes in Cluster State</Checkbox>
            </Form.Item>
            <Form.Item
        ...
        {this.props.isNew && (
          <Row className={styles.actions} type="flex" justify="end">
            <Button id="edit-region-form-cancel-button" size="small" onClick={onCancel}>
              Cancel
            </Button>
            <Button
        ...
        {!this.props.isNew && (
          <Row className={styles.actions} type="flex" justify="end">
            {this.props.region.default && (
              <Tooltip overlay="You cannot remove default AWS region. Set new default region first.">
                <Button
  ...
  rebuildContextualSettings = async () => {
    ...
    if (provider && region) {
      ...
      if (region.isNew) {
        ...
        const fetchSettingValue = (setting) =>
          fetchContextualSettingValue(setting.setting, region.id);
        const settingsValues = await Promise.all(settings.map(fetchSettingValue));
        const value = settings.reduce(
          (result, setting, idx) => ({
            ...result,
...
function AWSRegionFormHOC(props) {
  ...
  return <AWSRegionForm {...props} form={form} />;
}
...
class CloudRegionFileShareMountFormItem extends React.Component {
  ...
  static valuesAreEqual = (valueA, valueB) => {
    ...
    return (
      ...
      valueA.mountOptions === valueB.mountOptions
    );
  };
  ...
  mountRootValidationError = (value, provider) => {
    ...
    if (
      Object.hasOwn(MountRootFormat, provider) &&
      Object.hasOwn(MountRootFormat[provider], option) &&
  ...
  render() {
    return (
      <Row
        className={classNames(styles.fileShareMountRow, 'cp-divider', 'bottom', {
      ...
      >
        <Row type="flex" align="top">
          ...
          <Input
            ...
            placeholder={
              Object.hasOwn(MountRootFormat, this.props.provider)
          ...
          >
            {Object.keys(MountOptions).map((key) => (
...
class CloudRegionFileShareMountsFormItem extends React.Component {
  ...
  onChange = () => {
    if (this.props.onChange) {
      this.props.onChange(this.state.mounts);
    }
  };
  ...
  UNSAFE_componentWillReceiveProps(nextProps, nextContext) {
    if (
      !CloudRegionFileShareMountsFormItem.fileShareMountsAreEqual(
        this.state.mounts,
        nextProps.value,
      )
    ) {
  ...
  validate = () => {
    Object.keys(this.refComponents).forEach(
      (key) =>
    ...
    return (
      <Row>
        ...
        <Row type="flex" style={{minHeight: 32}} align="middle">
          <Button disabled={this.props.disabled} size="small" onClick={this.onAddFileShareMount}>
            <PlusOutlined />
            Add file share mount
          </Button>
        </Row>
      </Row>
    );
