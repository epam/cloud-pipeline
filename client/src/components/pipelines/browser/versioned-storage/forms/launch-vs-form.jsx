...
import styles from './launch-vs-form.module.css';
...
function getDefaultGroup(groups = [], adGroups = [], userRoles = []) {
  ...
  return findGroupByName(groups, 'library') || findGroupByName(groups, 'default') || groups[0];
}
...
class LaunchVSForm extends React.Component {
  ...
  handleCustomLaunch = () => {
    ...
    const {tool, tag} = this.state;
    if (onLaunchCustom) {
      onLaunchCustom(tool, tag);
    }
  };

  renderTool = (tool) => {
    const {filter: search, tool: selectedTool, tags, tagsError, tagsPending, tag} = this.state;
    ...
    return (
      <div
        key={`${tool.id}`}
        className={classNames(styles.tool, 'cp-launch-vs-tool', 'cp-panel-card', {
      ...
      >
        ...
        <div className={styles.info}>{renderMainInfo()}</div>
        {isSelected && (
          <div className={styles.versions} onClick={(e) => e.stopPropagation()}>
            {tagsPending && <LoadingOutlined />}
            {tagsError && <Alert type="error" title={tagsError} />}
            {!tagsError && !tagsPending && tags.length > 0 && (
              <Select value={tag} onChange={onChangeTag} className={styles.select}>
                {tags.map((tag) => (
  ...
  renderTools = () => {
    const {authenticatedUserInfo, dockerRegistries} = this.props;
    if (dockerRegistries.error || !dockerRegistries.loaded) {
      return null;
    }
    if (this.tools.length === 0) {
      return <Alert type="warning" title="Tools not found" />;
    }
    let personalTools = [];
    let personalGroupId;
    if (authenticatedUserInfo.loaded) {
      const {groups: adGroups = [], roles: userRoles = []} = authenticatedUserInfo.value;
    ...
    const {filter, tool: selectedTool} = this.state;
    ...
    const filterTool = (tool) =>
      ...
      (tool.group && tool.group.registry && filterRegEx.test(tool.group.registry.description));
    ...
    return (
      <div className={classNames(styles.tools, 'cp-panel', 'cp-panel-transparent')}>
        {filteredPersonalTools.map(this.renderTool)}
        {(showAllToolsSection || (selectedTool && selectedTool.group.id !== personalGroupId)) && (
          <div className={styles.divider}>
            <div className={classNames('cp-divider', 'horizontal')}>{'\u00A0'}</div>
            <div className={styles.title}>Global search</div>
            <div className={classNames('cp-divider', 'horizontal')}>{'\u00A0'}</div>
          </div>
        )}
        {!showAllToolsSection &&
        ...
        {showAllToolsSection && filteredTools.map(this.renderTool)}
      </div>
    );
  };

  render() {
    const {onClose, visible, dockerRegistries} = this.props;
    const {tool, filter} = this.state;
