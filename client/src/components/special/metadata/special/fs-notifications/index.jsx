...
import styles from './fs-notifications.module.css';
...
function notificationsEqual(a, b) {
  ...
  const aPresentations = (a || []).map(getNotificationPresentation).sort(defaultSorter).join('|');
  const bPresentations = (b || []).map(getNotificationPresentation).sort(defaultSorter).join('|');
...
class FSNotificationsDialog extends React.Component {
  ...
  get valid() {
    const {notifications, recipients = []} = this.state;
    return (
      (notifications.length === 0 || recipients.length > 0) &&
  ...
  get availableNotificationTypes() {
    ...
    return Object.values(NotificationTypes).filter(
      (o) => /^lustre$/i.test(storageType) || o === NotificationTypes.gb,
    );
  }
  ...
  render() {
    const {visible, onClose, readOnly} = this.props;
    const {notifications = [], recipients = []} = this.state;
    ...
    if (this.notificationTypesDisabledWarning) {
      NotificationTypesWrapper = ({children}) => (
        <Tooltip title={this.notificationTypesDisabledWarning} trigger="hover">
          {children}
    ...
    return (
      <Modal
        ...
        footer={
          <div className={styles.footer}>
            ...
            <Button disabled={readOnly || !this.valid} type="primary" onClick={this.onOk}>
              OK
            </Button>
          </div>
        }
      >
        <div
          className={classNames(styles.usersRolesSelect, 'cp-divider', 'bottom', {
        ...
        <div
          className={classNames(styles.usersRolesSelectError, 'cp-error', {
        ...
        <div>
          {notifications
            .map((notification) => [
              <div
                key={notification.id}
                className={classNames(styles.notification, 'cp-metadata-fs-notification', {
              ...
              >
                ...
                <NotificationTypesWrapper>
                  ...
                  >
                    {this.availableNotificationTypes.map((notificationType) => (
                      <Select.Option key={notificationType} value={notificationType}>
                ...
                >
                  {Object.values(NotificationActions || {}).map((notificationActionType) => (
                    <Select.Option key={notificationActionType} value={notificationActionType}>
              ...
              <div
                className={classNames(styles.notification, styles.notificationError, 'cp-error', {
                  [styles.visible]: !!notification.error,
                })}
                key={`${notification.id}-error`}
              >
                <ul>
                  {Object.values(notification.error || {}).map((error, index) => (
        ...
        <div className={styles.actions}>
          <Button size="small" onClick={this.onAddNotification} disabled={readOnly}>
...
class FSNotifications extends React.Component {
  ...
  render() {
    const {authenticatedUserInfo, readOnly: readOnlyRaw, info = {}, showOnlySummary} = this.props;
    const isAdmin =
    ...
    const {notifications = [], recipients = []} = this.parsedMetadata;
    let title = readOnly ? 'Notifications are not configured' : 'Configure notifications';
    ...
    return (
      <div
        ...
        onClick={
          showOnlySummary || (readOnly && notifications.length === 0)
            ? undefined
