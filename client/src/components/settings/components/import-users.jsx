...
import {Button, Checkbox, message, Modal, Upload, Dropdown, Space} from 'antd';
...
class ImportUsersButton extends React.Component {
      ...
      () => {
        ...
        const onLoad = (data) => {
          ...
          const users = lines
            ...
            .map((line) => line[0])
            .filter(Boolean);
          const attrs = (lines[0] || '').slice(2).filter(Boolean);
    ...
    return new Promise((resolve) => {
      ...
      this.checkUsers()
        .then((checkResult) => {
          ...
          const checkOnly = importResultsMode.length === 1 && importResultsMode.includes('check');
  ...
  onImport = () => {
    const {attributes, file, metadata, createGroups, createUsers, performCheck} = this.state;
    const {onImportDone} = this.props;
    this.setState(
      {
        pending: true,
      },
      () => {
        const url = importUsersUrl(createUsers, createGroups, [
        ...
        const resolve = ({error, logs}) => {
          ...
          this.setState(
            ...
            () => {
              ...
              if (performCheck) {
                this.checkUsersIntegrityClick().then(() => {
                  this.setState({
  ...
  render() {
    const {className, disabled: d, size, style, systemDictionaries} = this.props;
    const {
      attributes,
      file,
      pending,
    ...
    const dropdownMenuItems = [{key: DROPDOWN_KEYS.checkUsers, label: 'Check users integrity'}];
    return (
      <div className={classNames(className)} style={Object.assign({}, style, {display: 'inline'})}>
        <Upload
          fileList={[]}
        ...
        >
          ...
          >
            <Space.Compact>
              <Button disabled={disabled || pending} size={size}>
                Import users
              </Button>
              <Dropdown
                disabled={disabled || pending}
                ...
                trigger={['click']}
              >
                <Button disabled={disabled || pending} size={size} icon={<DownOutlined />} />
              </Dropdown>
            </Space.Compact>
          </div>
        </Upload>
        <Modal
          ...
          footer={
            <div className={styles.footer}>
              <Button onClick={this.onCancel} disabled={pending}>
                CANCEL
              </Button>
              <Button disabled={pending} type="primary" onClick={this.onImport}>
                IMPORT
              </Button>
            </div>
          }
        >
          ...
          <div className={styles.formItem}>
            <Checkbox checked={createUsers} onChange={this.onCreateUsersChanged}>
              Create users
            </Checkbox>
          </div>
          <div className={styles.formItem}>
            <Checkbox checked={createGroups} onChange={this.onCreateGroupsChanged}>
              Create groups
            </Checkbox>
          </div>
          <div className={styles.formItem}>
            <Checkbox checked={performCheck} onChange={this.onPerformCheckChanged}>
          ...
          {metadata.map((m, i) => (
            <div key={`${m}_${i}`} className={styles.formItem}>
              <Checkbox checked={m.create} onChange={this.onCreateMetadataChanged(m.name)}>
                {m.name}
              </Checkbox>
            </div>
          ))}
          ...
          {attributes.map((m, i) => (
            <div key={`${m}_${i}`} className={styles.formItem}>
              <Checkbox checked={m.create} onChange={this.onCreateAttributesChanged(m.name)}>
                {m.name}
              </Checkbox>
            </div>
          ))}
