...
function validateOS(tool = {}, mask) {
  ...
  const osMasks = mask.replace(/\./g, '\\.').replace(/\*/g, '.*').split(',').filter(Boolean);
...
export default function extractTools(groups = [], currentFilter) {
  ...
  const {my, personal, sensitive, labels, os, interactive, gpu, shared} =
  ...
  const checkBooleanProperty = (tool, property, criteria) =>
    typeof criteria === 'boolean' ? !!tool[property] === criteria : true;
