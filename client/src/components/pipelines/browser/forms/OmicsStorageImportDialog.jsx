...
import styles from './OmicsStorageImportDialog.module.css';
...
export class OmicsStorageImportDialog extends React.Component {
  ...
  renderRequiredStringField = (field) => {
    return (
      ...
      >
        <Input style={{width: '100%'}} disabled={this.props.pending} />
      </Form.Item>
    );
  };
  ...
  renderFileInput = (field) => {
    ...
    const rules =
      isSourceFile1 || (isReferencePath && this.isReferencePathRequired)
        ? [
            {
              required: true,
              validator: (rule, value) => this.validateRequiredField(rule, value, field),
    ...
    const label =
      isSourceFile1 && this.isFASTQ && form && form.getFieldValue(Fields.sourceFile1)
        ? `${FieldLabel[field]} 1`
        : FieldLabel[field];
    return (
      <Form.Item
      ...
      >
        <Input
          ...
          addonBefore={
            <div className={styles.pathType} onClick={() => this.openBucketBrowser(field)}>
              <FolderOutlined />
            </div>
          }
        />
  ...
  getFooter = () => {
    const getCancelFooter = () => {
      return (
        <Row>
          <Button id="omics-storage-dialog-cancel-button" onClick={this.props.onCancel}>
            Cancel
          </Button>
        </Row>
      );
    ...
    const footer =
      this.props.pending || this.state.restrictedAccessCheckInProgress
        ? false
        : this.props.dataStorage && !this.props.dataStorage.locked && !this.state.restrictedAccess
          ? getImportFooter()
          : getCancelFooter();
    return footer;
  };
  ...
  render() {
    ...
    const title =
      this.storageType === ServiceTypes.omicsRef
          ...
          <Form
            ...
            initialValues={{
              sourceFileType: this.isReferenceStorage
                ? FileTypes[ServiceTypes.omicsRef].REFERENCE
                : undefined,
            }}
          >
              ...
              rules={[
                {
                  ...
                  validator: (rule, value) =>
                    this.validateRequiredField(rule, value, Fields.sourceFileType),
                },
              ]}
            >
              <Select
            ...
            {this.isFASTQ &&
              form &&
              form.getFieldValue(Fields.sourceFile1) &&
