...
class FileDiffPresenter extends React.PureComponent {
  ...
  renderPresentation = () => {
    ...
    if (binary) {
      return <div className={styles.emptyContent}>Binary file</div>;
    }
    ...
    return <div className={styles.emptyContent}>No content</div>;
  };

  renderDescription = () => {
    const {file, type, selectable, selectedFiles, onSelectionChanged} = this.props;
    if (file && type) {
      return (
        <div key="description">
          {selectable && (
          ...
          >
            <FileTextOutlined />
            <span>{file}</span>
            <span className={classNames(styles.type, styles[type.toLowerCase()])}>
  ...
  render() {
    const {className, file, style, selectable} = this.props;
    const {opened} = this.state;
    return (
      <div key={file} className={className} style={style}>
        <Collapse
          className={classNames('cp-git-diff-collapse', 'git-diff-collapse', {
