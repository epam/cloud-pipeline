...
import {message, Button, Checkbox, Col, Form, Input, Row, Select, Space} from 'antd';
...
import styles from './EditEmailNotification.module.css';

const statuses = ['SUCCESS', 'FAILURE', 'RUNNING', 'STOPPED', 'PAUSING', 'PAUSED', 'RESUMING'];
...
export default class EditEmailNotification extends React.Component {
  ...
  get modified() {
    ...
    const checkPropModified = (prop, defaultValue) => {
      return (
        (this.props.template[prop] || defaultValue) !== (form.getFieldValue(prop) || defaultValue)
      );
    };
    ...
    const checkArrayPropModified = (prop, comparerFn = (a, b) => a === b) => {
      return !compareArrays(
        (this.props.template[prop] || []).map((i) => i),
        form.getFieldValue(prop),
        comparerFn,
      );
    };
    return (
      this.state.preferences.modified ||
  ...
  handleSubmit = (e) => {
    ...
    this.formRef.current
      .validateFields()
      .then(async (values) => {
        let {threshold, ...restValues} = values;
  ...
  bodyValueChanged = (code) => {
    if (this.formRef.current) {
      this.formRef.current.setFieldsValue({body: code});
    }
  };
  ...
  render() {
    ...
    const renderThresholdAndDelay =
      this.props.template.type === 'LONG_INIT' ||
  ...
  resetFormFields = (props) => {
    ...
    if (this.editor) {
      this.editor.setValue(props.template ? props.template.body || '' : '');
    }
    this.setState({
      preferencesSession: this.state.preferencesSession + 1,
    });
  ...
  componentDidUpdate(prevProps) {
    if (
      !prevProps.template ||
      !this.props.template ||
      prevProps.template.id !== this.props.template.id
    ) {
