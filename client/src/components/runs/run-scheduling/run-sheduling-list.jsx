...
class RunSchedulingList extends React.Component {
  static propTypes = {
    ...
    availableActions: PropTypes.arrayOf(
      PropTypes.oneOf([RunScheduleDialog.Actions.pause, RunScheduleDialog.Actions.resume]),
    ),
  };
  ...
  getScheduleString = (schedule, timeZone) => {
    ...
    let recurrence;
    const scheduleOrdinal = ORDINALS.find(({cronCode}) => ordinal === cronCode);
    const scheduleDay = [...DAYS, ...Object.values(COMPUTED_DAYS)].find(({key}) => key === day);
  ...
  renderList = () => {
    ...
    return <Spin spinning={!!pending}>{(rules || []).map(renderRule)}</Spin>;
  };
...
RunSchedulingList.defaultProps = {
  availableActions: [RunScheduleDialog.Actions.pause, RunScheduleDialog.Actions.resume],
};
