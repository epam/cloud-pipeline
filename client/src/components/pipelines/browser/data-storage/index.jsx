...
import {observable, makeObservable, untracked} from 'mobx';
import {
  Alert,
  Button,
  Checkbox,
...
import {METADATA_KEY as REQUEST_DAV_ACCESS_ATTRIBUTE} from '../../../special/metadata/special/request-dav-access';
...
const standardClasses = [STORAGE_CLASSES.standard, STORAGE_CLASSES.intelligentTiering];
...
const isActiveClass = (storageClass) => activeClass.includes((storageClass || '').toUpperCase());
...
class DataStorage extends React.Component {
  ...
  get fileShareMount() {
    if (this.storage.info) {
      const {fileShareMountId, type} = this.storage.info;
      if (/^nfs$/i.test(type)) {
  ...
  get userLifeCyclePermissions() {
    ...
    const isS3 = /^s3$/i.test(this.storage.info.storageType || this.storage.info.type);
    ...
    return {
      ...
      write:
        roleModel.isManager.storageAdmin(this) ||
        ((roleModel.isOwner(this.storage.info) || roleModel.isManager.archiveManager(this)) &&
          writeAllowed &&
          isS3),
    };
  }
  ...
  get showVersions() {
    ...
    return this.storage.info.type !== 'NFS' && this.props.showVersions && this.storage.isOwner;
  }
  ...
  get showArchives() {
    ...
    return (
      (this.userLifeCyclePermissions.read || this.userLifeCyclePermissions.write) && showArchives
    );
  }
  ...
  get lifeCycleRestoreInfo() {
    ...
    if (restoreInfo && restoreInfo.loaded) {
      ...
      let parentRestore;
      let currentRestores;
      if (first && first.type === 'FOLDER' && currentPath.startsWith(first.path)) {
        parentRestore = first;
  ...
  get restorableItems() {
    ...
    return (selectedItems || []).filter(
      (item) =>
        (item.labels && !isStandardClass(item.labels.StorageClass)) || item.type === 'Folder',
    );
  }
  ...
  get metadataEditable() {
    ...
    const isAdmin =
      authenticatedUserInfo && authenticatedUserInfo.loaded
  ...
  onDataStorageEdit = async (storage) => {
    ...
    const {parentFolderId, policySupported} = this.storage.info;
    const dataStorage = {
  ...
  get items() {
    ...
    const isAzure = /^az$/i.test(this.storage.info.storageType || this.storage.info.type);
    ...
    const getChildList = (item, versions, sensitive) => {
      ...
      for (const version in versions) {
        if (Object.hasOwn(versions, version)) {
          ...
          const archived = isAzure ? false : labels && !isStandardClass(labels.StorageClass);
          const versionRestored =
            restoreStatus.restoreVersions && restoreStatus.status === STATUS.SUCCEEDED;
          ...
          childList.push({
            ...
            downloadable:
              ...
              downloadable,
            editable:
              versions[version].version === item.version &&
              !versions[version].deleteMarker &&
            ...
            isVersion: true,
            archived,
            restored: latest ? fileRestored : versionRestored,
          });
        }
      }
      childList.sort((a, b) => {
        const dateA = ensureDayjs(a.changed);
        const dateB = ensureDayjs(b.changed);
        if (dateA && dateB && dateA.isAfter(dateB)) {
          return -1;
        } else if (dateA && dateB && dateA.isBefore(dateB)) {
          return 1;
        }
        return 0;
      });
    ...
    items.push(
      ...elements.map((i) => {
        ...
        const archived = isAzure ? false : i.labels && !isStandardClass(i.labels.StorageClass);
        ...
        return {
          ...
          vsi:
            !i.deleteMarker &&
            i.type.toLowerCase() === 'file' &&
            (i.path.toLowerCase().endsWith('.vsi') || i.path.toLowerCase().endsWith('.mrxs')),
          hcs:
            !i.deleteMarker &&
            i.type.toLowerCase() === 'file' &&
          ...
          ngb:
            ...
            i.type.toLowerCase() === 'file' &&
            isNgbFile(i.path),
          documentPreview: !i.deleteMarker && documentPreviewAvailable(i),
          archived,
          restored,
        };
      }),
  ...
  renameDataStorage = async (name) => {
    ...
    const {parentFolderId, description, path} = this.storage.info;
    const dataStorage = {
      id: this.props.storageId,
      parentFolderId,
      name,
  ...
  deleteStorage = async (cloud) => {
    ...
    const {name, parentFolderId} = this.storage.info;
  ...
  navigate = (id, path, options = {}) => {
    ...
    const params = [
      ...
      this.versionControlsEnabled ? `versions=${showVersions}` : null,
  ...
  navigateFull = (path) => {
    if (path && this.storage.info) {
      let {pathMask, path: bucketPath, delimiter = '/'} = this.storage.info;
  ...
  toggleGenerateDownloadUrlsModalFn = () => {
    const downloadUrlModalVisible = !this.state.downloadUrlModalVisible;
  ...
  removeItemConfirm = (event, item) => {
    ...
    if (this.showVersions) {
      if (item.isVersion) {
        ...
        const content = `Are you sure you want to delete selected version of the ${item.type.toLowerCase()} '${item.name}' from object storage?`;
        Modal.confirm({
        ...
        if (item.type.toLowerCase() === 'folder') {
          content = (
            <div>
              <Row>{content}</Row>
              <Row>All child folders and files will be removed.</Row>
            </div>
          );
        }
        Modal.confirm({
  ...
  removeSelectedItemsConfirm = (event) => {
    ...
    if (this.showVersions) {
      ...
      const removeItems = () =>
        this.removeItems(items, false, false, () => {
          this.setState({selectedItems: []});
        });
      Modal.confirm({
  ...
  actionsRenderer = (type, item) => {
    ...
    if (
      (item.isVersion ? item.editable && this.versionControlsEnabled : item.editable) &&
    ...
    if (
      ...
      (item.isVersion ? item.deletable && this.versionControlsEnabled : item.deletable) &&
    ...
    return <div className={styles.itemActionsContainer}>{actions}</div>;
  };

  fileIsSelected = (item) => {
    return !!this.state.selectedItems.find((s) => {
      if (this.isOmicsStore) {
  ...
  selectFile = (item) => () => {
    const selectedItems = this.state.selectedItems;
    const selectedItem = this.state.selectedItems.find((s) => {
      if (this.isOmicsStore) {
        return s.path === item.path && s.type === item.type;
      }
      return s.name === item.name && s.type === item.type;
  ...
  openShareItemDialog = (event) => {
    const {selectedItems = []} = this.state;
    const items = selectedItems.filter((o) => o.shareAvailable);
  ...
  openEditFileForm = async (item) => {
    ...
    const {sensitive} = this.storage.info || {};
    if (sensitive) {
  ...
  renderPreviewModal = () => {
    ...
    return <PreviewModal preview={previewModal} onClose={this.closePreviewModal} />;
  };
  ...
  checkWsiPreviewAvailability = (file) => {
    ...
    this.setState({previewPending: true}, () => {
      getPreviewConfiguration(storageId, file.path).then((configuration) =>
        this.setState({
          previewPending: false,
  ...
  getRestoredStatus = (item) => {
    const {parentRestore, currentRestores} = this.lifeCycleRestoreInfo;
    if (!item) {
      return null;
    }
    const itemRestoreStatus = (currentRestores || []).find(
      ({path = ''}) => item.name === path.split('/').filter(Boolean).pop(),
    );
    if (itemRestoreStatus) {
      return itemRestoreStatus;
    }
    return item.type === 'File' ? parentRestore : null;
  };

  get columns() {
    const {storageId} = this.props;
    const tableData = this.items;
    const hasAppsColumn = tableData.some(
      (o) => o.miew || o.vsi || o.hcs || o.ngb || o.documentPreview,
    );
    ...
    const selectionColumn = {
      key: 'selection',
      className:
        this.showVersions || hasVersions ? styles.checkboxCellVersions : styles.checkboxCell,
      render: (item) => {
        if (
          item.selectable &&
          ...
          return <Checkbox checked={this.fileIsSelected(item)} onChange={this.selectFile(item)} />;
        } else {
          return <span />;
        }
      },
          ...
          apps.push(
            ...
            >
              <Link
                ...
                to={
                  item.version
                    ? `miew?storageId=${storageId}&path=${item.path}&version=${item.version}`
                    : `miew?storageId=${storageId}&path=${item.path}`
                }
                target="_blank"
              >
    ...
    const renderTitle = (key = '', title) => (
      <span
        ...
        className={classNames({
          'cp-primary':
            (this.storage.currentSorter.field || '').toLowerCase() === key.toLowerCase(),
        })}
    ...
    const nameColumn = {
      ...
      sorter: true,
      sortOrder: this.storage.currentSorter.field === 'name' && this.storage.currentSorter.order,
      className: styles.nameCell,
      render: (text, item) => {
        const search = this.storage.currentFilter[FILTER_FIELDS.name];
        const highlightedText =
          this.storage.filtersApplied && search ? highlightText(text, search) : text;
        const name = (
          <span>
    ...
    const sizeColumn = {
      ...
      sorter: true,
      sortOrder: this.storage.currentSorter.field === 'size' && this.storage.currentSorter.order,
    ...
    const changedColumn = {
      ...
      sorter: true,
      sortOrder: this.storage.currentSorter.field === 'changed' && this.storage.currentSorter.order,
    ...
    const labelsColumn = {
      ...
      render: (labels) => <LabelsRenderer labelClassName={styles.label} labels={labels} />,
      onCell: (item) => ({onClick: () => this.didSelectDataStorageItem(item)}),
    };
    const actionsColumn = {
      key: 'actions',
      ...
      title: (
        <div style={{display: 'flex', justifyContent: 'flex-end'}}>
          {this.storage.resultsFiltered ? (
            <Button size="small" type="primary" onClick={() => this.storage.resetFilter(false)}>
              Reset filters
            </Button>
          ) : null}
        </div>
      ),
      render: (type, item) => untracked(() => this.actionsRenderer(type, item)),
    };
  ...
  get bulkDownloadEnabled() {
    ...
    const downloadableSelectedItemsLength = this.state.selectedItems.filter(
      (item) => item.downloadable,
    ).length;
    return selectedItemsLength > 0 && selectedItemsLength === downloadableSelectedItemsLength;
  }

  get removeAllSelectedItemsEnabled() {
    const selectedItemsLength = this.state.selectedItems.length;
    const editableSelectedItemsLength = this.state.selectedItems.filter(
      (item) => item.editable,
    ).length;
    return selectedItemsLength > 0 && selectedItemsLength === editableSelectedItemsLength;
  }

  get selectAllAvailable() {
    return (
      !!this.storage.pageElements &&
  ...
  get userCanChangeStorageItemsPermissions() {
    ...
    const {pathPermissionsEnabled, mask} = info;
  ...
  get currentFolderWriteAllowed() {
    ...
    const {pathPermissionsEnabled} = info;
  ...
  get selectedFileWriteAllowed() {
    ...
    const {pathPermissionsEnabled} = info;
    if (pathPermissionsEnabled) {
      return this.isAdmin || roleModel.isOwner(info) || roleModel.writeAllowed(selectedFile);
    }
    return this.storage.writeAllowed;
  }
  ...
  showFilesVersionsChanged = (showVersions) => {
    this.navigate(this.props.storageId, this.props.path, {
      showVersions,
    });
  };
  ...
  get isFileSelectedEmpty() {
    ...
    const selectedFile = this.items.find((file) => file.name === this.state.selectedFile.name);
  ...
  renderEditAction = () => {
    const convertToVersionedStorageActionAvailable =
      this.storage.infoLoaded &&
    ...
    return (
      <Button id="edit-storage-button" size="small" onClick={() => this.openEditDialog()}>
        <SettingOutlined style={{lineHeight: 'inherit', verticalAlign: 'middle'}} />
      </Button>
    );
  };
  ...
  renderSelectionActionsButton = () => {
    const {preferences, storageId} = this.props;
    const {selectedItems = []} = this.state;
    const itemsAvailableForShare = selectedItems.filter((o) => o.shareAvailable);
    ...
    const downloadOmicsAction = {
      ...
      title: 'Download',
      icon: <DownloadOutlined />,
      available: this.isOmicsStore && omicsItemsForDownload.length > 0 && this.isSequenceStorage,
    };
    const restoreAction = {
      key: Keys.restore,
    ...
    const getShareActionTitle = () => {
      if (itemsAvailableForShare.length === 1) {
        return (
          <span className="cp-ellipsis-text" style={{maxWidth: 300}}>
    ...
    return (
      <Dropdown
        ...
        key="create actions"
      >
        <Button id="selection-actions" size="small">
          {title}
          <DownOutlined />
        </Button>
      </Dropdown>
  ...
  renderShareCurrentFolderButton = () => {
    ...
    return (
      <Button id="share-selected-folder" size="small" onClick={this.openShareCurrentFolderDialog}>
  ...
  renderContent = () => {
    ...
    const title = () => {
      return (
        <Row className={styles.storageActions} type="flex" justify="space-between">
          <div>
            {(!this.isOmicsStore || this.isSequenceStorage) && (
              <Button
                id="select-all-button"
                size="small"
                onClick={() => this.selectAll(undefined)}
                disabled={!this.selectAllAvailable}
              >
                Select page
              </Button>
            )}
            {(!this.isOmicsStore || this.isSequenceStorage) && this.renderSelectionActionsButton()}
          </div>
          <div style={{paddingRight: 8}}>
            {this.renderShareCurrentFolderButton()}
            {this.currentFolderWriteAllowed && !this.isOmicsStore && (
              <Dropdown
                placement="bottomRight"
                trigger={['hover']}
                menu={{
                ...
                key="create actions"
              >
                <Button type="primary" id="create-button" size="small">
            ...
            {this.currentFolderWriteAllowed && this.isSequenceStorage && (
              <UploadOmicsButton
                storageInfo={this.storage.info}
                region={this.regionName}
                onRefresh={() => this.refreshList()}
            ...
            {this.currentFolderWriteAllowed && this.isOmicsStore && this.isOmicsFolder && (
              <Button
                id="import-button"
                size="small"
                type="primary"
  ...
  renderPresentationConfiguration = () => {
    ...
    const doAction = (action) => {
      ...
      switch (action.key) {
        ...
        case 'jobs':
          ...
          break;
        case 'archive':
          this.showArchivedFilesChanged(!this.showArchives);
          break;
        case 'version':
          this.showFilesVersionsChanged(!this.showVersions);
          break;
        default:
          break;
      }
    ...
    const menuItems = actions.map((action) => ({
      ...
      label: (
        <div style={{display: 'flex', alignItems: 'center'}}>
          {action.icon}
          {action.title}
          {action.checked && <CheckOutlined style={{marginLeft: 'auto'}} />}
        </div>
      ),
    }));
    const onMenuClick = ({key}) => {
    ...
    return (
      <Dropdown
        ...
        key="create actions"
      >
        <Button id="presentation-actions" size="small">
          <AppstoreFilled />
        </Button>
      </Dropdown>
    );
  ...
  render() {
    ...
    const restoreClassChangeDisclaimer = (item, operation) => {
      ...
      const activeRestore = item.archived ? this.getRestoredStatus(item) : null;
      let disclaimer;
      ...
      return disclaimer ? disclaimer.filter(Boolean).join('') : '';
    };
    return (
      <div
        style={{
      ...
      >
        <Row type="flex" justify="space-between" align="middle">
          <Col className={styles.itemHeader}>
            <Breadcrumbs
              ...
              iconClassName={classNames(styles.editableControl, {
                [styles.readonly]: locked,
              })}
              lock={locked}
              lockClassName={classNames(styles.editableControl, {
                [styles.readonly]: locked,
              })}
              sensitive={sensitive}
              displayTextEditableField={
                <span>
                  {name}
                  <AWSRegionTag
                    className={classNames(styles.storageRegion)}
                    displayName
          ...
          <Col>
            <Row type="flex" justify="end" align="middle" className={styles.currentFolderActions}>
              <RestrictedImagesInfo toolsToMount={this.toolsToMount} status={mountStatus} />
              ...
              <StorageSharedLinkButton storageId={this.props.storageId} />
              {this.renderEditAction()}
              <Button
                id="refresh-storage-button"
                size="small"
                onClick={() => this.refreshList(true)}
                disabled={this.storage.pagePending}
              >
                Refresh
              </Button>
            </Row>
        ...
        >
          ...
          {(this.showMetadata || this.showJobs) && (
            <Metadata
              ...
              metadataRenderFn={this.state.previewAvailable ? this.metadataRenderFn : undefined}
              entityName={this.state.selectedFile ? this.state.selectedFile.name : name}
              entityId={
                this.state.selectedFile ? this.state.selectedFile.path : this.props.storageId
              }
              entityParentId={this.state.selectedFile ? this.props.storageId : undefined}
              entityVersion={this.state.selectedFile ? this.state.selectedFile.version : undefined}
              entityClass={this.state.selectedFile ? 'DATA_STORAGE_ITEM' : 'DATA_STORAGE'}
              openEditFileForm={
        ...
        <Modal
          ...
          footer={
            <Button type="primary" onClick={() => this.closeDownloadUrlModal(true)}>
              OK
            </Button>
          }
        >
        ...
        <EditItemForm
          ...
          disclaimerFn={() => {
            const text = restoreClassChangeDisclaimer(this.state.renameItem, 'rename');
            return text ? <Alert title={text} type="info" /> : null;
          }}
        />
        <SharedItemInfo
          ...
          footer={
            <div className="cp-modal-footer-actions cp-modal-footer-actions--split">
              <div className="cp-modal-footer-actions-group">
                <Button id="delete-bucket-item-modal-cancel-button" onClick={this.closeDeleteModal}>
                  Cancel
                </Button>
              </div>
              <div className="cp-modal-footer-actions-group cp-modal-footer-actions-group--end">
                <Button
                  ...
                  danger
                  onClick={() =>
                    this.removeItems(this.state.itemsToDelete, false, false, () => {
                ...
                <Button
                  ...
                  danger
                  onClick={() =>
                    this.removeItems(this.state.itemsToDelete, true, false, () => {
                      this.closeDeleteModal();
                      this.setState({selectedItems: []});
                      this.afterDataStorageEdit();
                    })
        ...
        >
          {(this.state.itemsToDelete || []).map((item) => {
            return <Row key={item.name}>{item.name}</Row>;
          })}
        </Modal>
