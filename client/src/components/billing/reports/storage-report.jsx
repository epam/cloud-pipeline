...
import {BarChart, BillingTable, Summary, DetailsChart} from './charts';
import {costTickFormatter, numberFormatter, ResizableContainer} from './utilities';
...
function injection(stores, props) {
  ...
  const aggregate = /^object$/i.test(type) ? parseStorageAggregate(aggregateQ) : undefined;
...
class StorageReports extends React.Component {
  ...
  onSelectLayer = ({key}) => {
    ...
    const {storageAggregate, storageAggregateNavigation} = filters;
    if (typeof storageAggregateNavigation === 'function') {
      storageAggregateNavigation(!key || storageAggregate === key ? StorageAggregate.default : key);
    }
  };
  ...
  get tiersData() {
    const {filters = {}, tiersRequest} = this.props;
    let data = {};
    ...
    const datasets = filter.map((key) => {
      return {
        ...
        showDataLabel: (value, datasetValues, options) => {
          const {dataset, getPxSize = () => 0} = options || {};
    ...
    const totalDataset = (datasets || []).reduce(
      (acc, current) => {
  ...
  get summaryDatasets() {
    ...
    const storageClass = total ? 'TOTAL' : getStorageClassByAggregate(storageAggregate);
  ...
  get detailsDatasets() {
    const {type, filters = {}} = this.props;
    const {storageAggregate, metrics} = filters;
    if (!/^object$/i.test(type)) {
  ...
  render() {
    ...
    const {period, range, region: cloudRegionId, storageAggregate, metrics} = filters;
    ...
    const valueFormatter = isVolumeMetrics ? numberFormatter : costTickFormatter;
    const topDescription = isVolumeMetrics ? 'GB' : undefined;
    const showTableDetails = /^object$/i.test(type);
    const selectedIndex = tiersData.aggregates.indexOf(storageAggregate);
    const exportProperties = this.getExportProperties;
    const extraTooltipForItem = this.extraTooltipForItemCallback;
    const layout = this.layout;
    const summaryTableDataExtractors = this.summaryTableDataExtractors;
    const summaryDatasets = this.summaryDatasets;
    const detailsDatasets = this.detailsDatasets;
    return (
      <Discounts.Consumer>
        {(o, storageDiscounts) => (
          <Export.Consumer
            exportConfiguration={{
              ...
              filters: {
                ...
                cloudRegionId:
                  cloudRegionId && cloudRegionId.length > 0 ? cloudRegionId : undefined,
              },
              properties: exportProperties,
            }}
          >
            <Layout layout={layout.Layout} gridStyles={layout.GridStyles}>
              <div key={layout.Panels.summary}>
                <Layout.Panel
                  style={{
                    display: 'flex',
              ...
              {/^object$/i.test(type) && tiersRequest ? (
                <div key={layout.Panels.storageLayers}>
                  <Layout.Panel>
                      ...
                      {({height}) => (
                        <div>
                          ...
                          <DetailsChart
                            ...
                            data={tiersData}
                            title={['Object storage layers', topDescription]
                              .filter(Boolean)
                              .join(', ')}
                            style={{height: height - costsUsageSelectorHeight}}
                            valueFormatter={valueFormatter}
                            highlightTickFn={(_, tick) => tick._index === selectedIndex}
              ...
              <div key={layout.Panels.storages}>
                <Layout.Panel>
                  <ResizableContainer style={{width: '100%', height: '100%'}}>
                    {({height}) => (
                      <div>
                        ...
                        <BarChart
                          ...
                          valueFormatter={valueFormatter}
                          highlightTickFn={(s) =>
                            `${(s?.groupingInfo || {}).is_deleted}` === 'true'
                          }
                          highlightTickStyle={{
                            fontColor: reportThemes.errorColor,
                          }}
                          extraTooltipForItem={extraTooltipForItem}
                        />
                      </div>
                    )}
                  </ResizableContainer>
              ...
              <div key={layout.Panels.storagesTable}>
                <Layout.Panel>
                  <ResizableContainer style={{width: '100%', height: '100%'}}>
                    {({height}) => (
                      <StorageTable
...
export default withRouter(
  inject('reportThemes')(inject(injection)(BillingNavigation.attach(observer(StorageReports)))),
);
