...
| **`cluster.monitoring.elastic.intervals.number`** | A number of intervals in monitoring period by which metrics should be received. Any requested monitoring period will be divided on this number, and for each received interval metrics will be taken separately.<br>Related to metrics monitoring within `ElasticSearch` |
| **`cluster.monitoring.elastic.minimal.interval`** | Defines minimum interval (in seconds) between metrics taking. This interval duration will be used for metrics taking only in case when monitoring period divided on `cluster.monitoring.gcp.intervals.number` is smaller than this interval.<br>Related to metrics monitoring within `ElasticSearch` |
| **`cluster.monitoring.gcp.intervals.number`** | A number of intervals in monitoring period by which metrics should be received. Any requested monitoring period will be divided on this number, and for each received interval metrics will be taken separately.<br>Related to metrics monitoring within `Google Cloud Monitoring` |
| **`cluster.monitoring.gcp.minimal.interval`** | Defines minimum interval (in seconds) between metrics taking. This interval duration will be used for metrics taking only in case when monitoring period divided on `cluster.monitoring.gcp.intervals.number` is smaller than this interval.<br>Related to metrics monitoring within `Google Cloud Monitoring` |
...
| Setting name | Description |
|---|---|
| **`gcp.billing.account.id`** | GCP billing account ID. If it is set, GCP billing prices associated with the specific billing accoount will be retrieved (not default GCP prices) |
| **`gcp.sku.mapping`** |  |
| **`gcp.regions.list`** |  |
| **`gcp.logging.log.name`** | Logname for Google Cloud Logging |
| **`gcp.logging.sink.label.key`** | Key of the label that defines from where the log comes.<br>It is necessary for the Sink connector that redirects the log to `BigQuery` |
| **`gcp.logging.sink.label.value`** | Value of the label that defines from where the log comes.<br>It is necessary for the Sink connector that redirects the log to `BigQuery` |
...
| Setting name | Description |
|---|---|
| **`misc.ai.preferences`** | Allows to configure GUI chatbot visualization and other settings |
...
| **`system.access.code.monitor.enable`** | [_Refers to `pipe` CLI code authorization_]<br>Enables monitoring that automatically removes old authorization codes. |
| **`system.access.code.monitor.delay`** | [_Refers to `pipe` CLI code authorization_]<br>Delay for monitoring that automatically removes old authorization codes. |
| **`system.access.allowed.code.challenge.methods`** | [_Refers to `pipe` CLI code authorization_]<br>Sets authorization code challenge method. |
| **`system.access.code.length`** | [_Refers to `pipe` CLI code authorization_]<br>Sets authorization code length. |
| **`system.access.code.ttl.minutes`** | [_Refers to `pipe` CLI code authorization_]<br>Sets the period during which the authorization code is valid. |
