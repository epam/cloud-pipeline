...
import styles from './pause-confirmation.module.css';
...
class ConfirmPauseContainer extends React.Component {
  ...
  check = async (options = {}) => {
    let resolved = false;
    const {id, run, title} = options;
    if (!run && !title) {
      getRun(id).then((runInfo) => {
        if (!resolved) {
  ...
  onCancel = () => {
    const {resolver = (decision) => {}} = this.state;
  ...
  renderTitle = () => {
    ...
    return (
      <span>
        ...
        <RunDisplayName run={run} style={{marginLeft: 5}} />?
      </span>
    );
  };

  render() {
    const {runId, visible} = this.state;
    return (
...
function ConfirmPauseModal(props) {
  const {title, onConfirm, onCancel, visible} = props;
  ...
  return (
    ...
    >
      ...
      <div className={styles.footer}>
        <Button className={styles.action} id="cancel-pause-run" onClick={onCancel}>
          CANCEL
        </Button>
        <Button
...
const ConfirmPause = LayersCheckProvider.inject(
  CommitCheckProvider.inject(observer(ConfirmPauseModal)),
);
