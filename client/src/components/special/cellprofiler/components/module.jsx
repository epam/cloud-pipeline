...
function Circle({className, pending, empty}) {
  return (
    <svg
      className={classNames(className, styles.cpModuleExecutionIndicator, {
...
function CellProfilerModuleHeaderRenderer(props) {
  const {cpModule, movable = true, removable = true, hasErrors = false} = props;
  if (!cpModule) {
    return null;
  }
  /**
  ...
  const outputImage = analysis ? analysis.getOutputImageForModule(cpModule) : undefined;
  ...
  const renderIcon = () => {
    ...
    if (cpModule.pending || cpModule.done) {
      ...
      return <Circle className="cp-text-not-important" empty />;
    }
  };
  const prevent = (e) => {
    if (e) {
  ...
  return (
    ...
    >
      <b style={{marginRight: 'auto'}}>
        ...
        {hasOutputImage && (
          <PictureOutlined
            className={classNames({'cp-text-not-important': !selected, 'cp-primary': selected})}
            style={{cursor: 'pointer', marginLeft: 5, fontWeight: 'normal'}}
            onClick={onSelectOutput}
          />
        )}
      </b>
      {!cpModule.hidden && movable && (
        <Button className={styles.action} size="small" disabled={cpModule.isFirst} onClick={moveUp}>
          <UpOutlined />
        </Button>
      )}
      {!cpModule.hidden && movable && (
      ...
      {!cpModule.hidden && removable && (
        <Button className={styles.action} size="small" danger onClick={remove}>
          <DeleteOutlined />
        </Button>
      )}
    </div>
  ...
  return (
    <div>
      {params.map((parameter) => (
        <CellProfilerParameter key={parameter.parameter.id} parameterValue={parameter} />
      ))}
    </div>
  );
}
...
export {CellProfilerModule, CellProfilerModuleHeader};
