...
| `-dt` / `--date-to` | List pipeline runs started before specified date |
...
Options `-df` (`--date-from`) and `-dt` (`--date-to`) allow to output runs started after/before specified datetime.  
...
>>>>> | `-nc` / `--cores` | Number cores that a cluster shall contain |
...
**NODE_NAME** - calculation node name (ID). You can know it by the `view-runs` command with the `-nd` option or via the GUI.
