...
function createRunContinuationConfirmationDialog() {
  ...
  class _RunContinuationConfirmation extends React.Component {
    ...
    onRunCustom = () => {
      ...
      if (run && routing) {
        const {pipelineId, version, id} = run;
            ...
            <div className="ant-confirm-body-wrapper">
              <div className="ant-confirm-body">
                ...
                <span className="ant-confirm-title">
                  <div style={{display: 'flex', alignItems: 'center'}}>
                    <span>{'Continue '}</span>
                    <RunName run={run} editable={false} style={{fontWeight: 'bold'}}>
                      run #{run.id}
                    </RunName>
                    <span>?</span>
                  </div>
              ...
              >
                ...
                >
                  ...
                  <Button size="large" onClick={this.onConfirm} type="primary">
                    Continue
                  </Button>
                </div>
              </div>
...
const {openConfirmation, RunContinuationConfirmation} = createRunContinuationConfirmationDialog();
...
export function runSupportsContinue(run) {
  ...
  const {version, pipelineId, status = '', pipelineRunParameters = []} = run;
  const isRemovedPipeline = !!version && !pipelineId;
  const cpSupportContinueParameter = pipelineRunParameters.find(
    (p) => p.name === CP_SUPPORT_CONTINUE,
  );
  ...
  return (
    ...
    ['FAILURE', 'STOPPED', 'SUCCESS'].includes(status.toUpperCase()) &&
    cpSupportContinue &&
