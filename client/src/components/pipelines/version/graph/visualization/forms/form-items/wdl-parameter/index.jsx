...
import styles from './wdl-parameter.module.css';
...
function parameterValueEditable(parameter, wdlDocument) {
  return !parameter || !parameter.isOutput || parameter.document === wdlDocument;
}
...
class WdlParameter extends React.Component {
  ...
  renderProperty = (options) => {
    const {parameter, disabled} = this.props;
    if (!parameter) {
      return null;
    }
    const {
    ...
    return [
      title ? (
        <span key={`${property}-title`} className={styles.wdlParameterPartTitle}>
          {title}:
        </span>
      ) : null,
      <Component
  ...
  renderRemoveButton = () => {
    const {disabled, editable, parameter} = this.props;
    if (disabled || !editable || !parameter) {
      return null;
    }
    const removeParameter = () => {
      ...
      if (owner && typeof owner.removeParameters === 'function') {
        ...
        owner.removeParameters([parameterToRemove]);
      }
    };
    return (
      <div className={styles.wdlParameterDeleteButton} onClick={removeParameter}>
  ...
  renderIssues = () => {
    ...
    const {issues = []} = parameter;
    return <WdlIssues issues={issues} />;
  };

  render() {
    const {className, style, parameter} = this.props;
    if (!parameter) {
      return null;
    }
    ...
    return (
      <div className={className} style={style}>
        ...
        <div className={styles.wdlParameterRow}>{value}</div>
        {issues}
      </div>
    );
  }
