...
function NavigateToMlFlowActionRenderer({run, preferences}) {
  ...
  if (mlflowEndpoint) {
    ...
    return <ExportOutlined style={{fontSize: 'large', cursor: 'pointer'}} onClick={onClick} />;
  }
  return null;
}
...
export const mlflowTab = {
  ...
  title: 'MLflow',
  noPadding: true,
  action: ({run}) => <NavigateToMlFlowAction run={run} />,
  render: ({run}) => (
    <InitializeWrapper run={run}>
...
export const logsTab = {
  ...
  render: ({run, runTasks, pending, loaded}) => (
    <RunLogsSection run={run} runTasks={runTasks} pending={pending} loaded={loaded} />
  ),
};
