...
export default class MetadataEntityPreview extends React.Component {
  ...
  get description() {
    ...
    if (
      this.props.metadataEntity &&
      this.props.metadataEntity.loaded &&
  ...
  renderItems = () => {
    ...
    if (this.props.metadataEntity.error) {
      return (
        <div className={styles.contentPreview}>
          <span className={'cp-search-preview-error'}>{this.props.metadataEntity.error}</span>
        </div>
      );
    }
    if (!this.rowData) {
    ...
    for (const key in this.rowData) {
      if (Object.hasOwn(this.rowData, key)) {
        items.push(
          <tr key={key}>
