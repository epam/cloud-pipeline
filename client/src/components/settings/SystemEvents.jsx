...
import {Alert, Button, Checkbox, message, Row, Modal, Table} from 'antd';
import {
  CloseCircleOutlined,
  DeleteOutlined,
  EditOutlined,
...
export default class SystemEvents extends Component {
  ...
  renderSeverityIcon = (notification) => {
    switch (notification.severity) {
      case 'INFO':
        return <InfoCircleOutlined style={{fontSize: 'larger'}} className="cp-setting-info" />;
      case 'WARNING':
        return (
          <ExclamationCircleOutlined style={{fontSize: 'larger'}} className="cp-setting-warning" />
        );
      case 'CRITICAL':
        return <CloseCircleOutlined style={{fontSize: 'larger'}} className="cp-setting-critical" />;
      default:
        return undefined;
    }
  };
  ...
  render() {
    ...
    const {notifications} = this.props;
    const data = (notifications.loaded ? (notifications.value || []).map((n) => n) : []).sort(
      (a, b) => {
        if (a.title > b.title) {
          return 1;
        ...
        <Table
          ...
          expandable={{
            ...
            expandedRowRender: (notification) => (
              <p className={`notification-${notification.notificationId}-body`}>
                {notification.body}
              </p>
            ),
          }}
