...
import styles from './open-tool-info.module.css';
...
class OpenToolInfo extends React.Component {
  ...
  get filePath() {
    const {file, storage, tool} = this.props;
    const {platform = ''} = tool || {};
    ...
    if (isWindowsPlatform) {
      ...
      fullPath =
        mountPoint && mountPoint.length > 0
          ? `${mountPoint}/${file}`
    ...
    return (
      <div className={styles.code}>
        <div className={styles.part} style={{flex: 1}}>
          <pre ref={initializePathElement}>{this.filePath}</pre>
        </div>
        <div className={classNames(styles.part, styles.button)} onClick={copy}>
          <CopyOutlined />
        </div>
      </div>
    );
  ...
  renderAppName = () => {
    const {tool} = this.props;
    return <span>{tool.image}</span>;
  };

  renderAppLink = (linkText) => {
    const {activeJob, activeJobsFetching} = this.props;
    return (
      <span>
        {activeJobsFetching && <LoadingOutlined />}
        {!activeJobsFetching && !!activeJob && (
          <ToolJobLink job={activeJob} toolName={this.appName} linkText={linkText} />
        )}
        {!activeJobsFetching && !activeJob && (
          <Button size="small" type="primary" onClick={this.onLaunchClick} style={{marginLeft: 5}}>
            Launch
          </Button>
        )}
      </span>
  ...
  renderContent = () => {
    ...
    const preprocessed = this.template.replace(
      /\{([^}:]+)(?::([^}]*))?}/g,
      (_, type, attr = '') =>
        `<placeholder data-type="${type}" data-attribute="${attr}"></placeholder>`,
    );
    ...
    const components = {
      ...
      placeholder: ({node}) => {
        ...
        const attribute = node.properties?.dataAttribute ?? '';
        if (type && Object.hasOwn(renderers, type)) {
          return renderers[type](attribute);
        }
        return null;
      },
    };
  ...
  render() {
    const {tool, file, storage} = this.props;
