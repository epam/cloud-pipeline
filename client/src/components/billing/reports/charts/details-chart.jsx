...
function DetailsChart({
  ...
  const {aggregates} = rawData;
  const chartData = {
    labels: rawData.labels,
    datasets: rawData.datasets.map((dataset, index) => {
      const baseColors = Array(dataset.data.length).fill(reportThemes.current);
      const backgroundColors = Array(dataset.data.length)
        .fill(dataset.isOldVersions ? 'transparent' : reportThemes.current)
        .map((color, index) =>
          highlightedLabel === index || dataset.isOldVersions ? color : fadeout(color, 0.75),
        );
      return {
        showFlag: false,
        borderWidth: 2,
  ...
  const options = {
    ...
    tooltips: {
      ...
      callbacks: {
        labelColor: function (tooltipItem, chart) {
          ...
          const {datasets = []} = configData;
          const {isOldVersions} = datasets[tooltipItem.datasetIndex] || {};
          return {
    ...
    plugins: {
      ...
      [HighlightAxisPlugin.id]: {
        ...
        backgroundColor: fadeout(reportThemes.lightBlue, 0.9),
      },
