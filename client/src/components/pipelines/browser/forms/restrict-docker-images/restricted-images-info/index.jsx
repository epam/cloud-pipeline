...
import DockerImageDetails from '../../../../../cluster/hot-node-pool/docker-image-details.tsx';
import styles from './restricted-images-info.module.css';
...
function RestrictedImagesInfo({toolsToMount, status}) {
  const renderContent = () => {
    ...
    return (
      <div className={styles.popoverContainer}>
        ...
        {(toolsToMount || []).map((tool) => (
          <span key={tool.id} className={styles.toolRow}>
            <span className={styles.toolName}>
              <DockerImageDetails docker={getDockerImage(tool)} />
            </span>
  ...
  if ((!toolsToMount || toolsToMount.length === 0) && (!status || status === MountStatus.active)) {
    return null;
  }
  return (
    <div className={styles.container}>
      <Popover content={renderContent()} classNames={{root: styles.overlay}}>
...
RestrictedImagesInfo.propTypes = {
  toolsToMount: PropTypes.arrayOf(
    PropTypes.shape({
      id: PropTypes.number,
