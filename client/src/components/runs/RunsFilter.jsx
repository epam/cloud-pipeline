...
class RunsFilter extends React.Component {
  ...
  autocomplete = (text, position) => {
    ...
    if (!composerResult.error && this.keywords.length > 0) {
      ...
      if (element && element.isProperty && !this.props.keywords.pending) {
        const filter = this.keywords.filter(
          (k) => !k.regex && k.fieldName.toLowerCase().startsWith(element.text.toLowerCase()),
        );
        return {
          element,
          filter,
  ...
  onEnter = (text) => {
    if (this.state.autocomplete && this.state.autocomplete.hovered !== null) {
      ...
      const replaced =
        text.substring(0, this.state.autocomplete.element.starts) +
  ...
  renderSavedFilters = () => {
    ...
    const columns = [
      ...
      },
      {
        ...
        render: (filter) => (
          <Button danger onClick={onDelete(filter)} size="small">
            <DeleteOutlined />
          </Button>
        ),
      },
  ...
  render() {
    ...
    return (
      ...
      >
        ...
        <Row>
          <RunTable disableFilters search searchFilters={this.state.searchFilter} />
        </Row>
        <SaveFilterForm
          pending={false}
