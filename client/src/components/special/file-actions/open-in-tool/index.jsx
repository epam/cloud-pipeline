...
import getToolLaunchingOptions from '../../../pipelines/launch/utilities/get-tool-launching-options';
import styles from './open-in-tool.module.css';
...
class OpenInToolAction extends React.Component {
  ...
  get activeToolTemplate() {
    ...
    const tool = this.openInFileTools.find((tool) => tool.toolId === activeTool.id);
  ...
  renderToolInfo = () => {
    const {activeJob, activeJobsFetching, activeTool} = this.state;
    ...
    if (!activeTool) {
      content = (
        <div className={classNames(styles.toolTabContainer)}>
          ...
          >
            <span className={styles.tabHeading} style={{marginLeft: 5}}>
              Select tool to open:
            </span>
            <CloseOutlined
      ...
      content = (
        <div
          ...
          style={this.filteredFileTools.length > 1 ? {} : {paddingTop: 5}}
        >
          {this.filteredFileTools.length > 1 && (
            ...
            >
              <div style={{display: 'inline-flex'}}>
                <div onClick={this.clearToolSelection} className={styles.tabHeadingBtn}>
                  <CaretLeftOutlined />
                </div>
  ...
  render() {
    const {className, style, titleStyle, file} = this.props;
