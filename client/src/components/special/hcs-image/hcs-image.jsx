...
import styles from './hcs-image.module.css';
...
class HcsImage extends React.Component {
  ...
  componentDidMount() {
    ...
    this.hcsMergedImageSource.attachViewerState(this.hcsViewerState);
    this.hcsMergedImageSource.addOmeTiffGenerationListener(this.loadImage.bind(this));
  ...
  componentWillUnmount() {
    ...
    this.hcsMergedImageSource.destroy();
    this.hcsMergedImageSource = undefined;
  ...
  get selectedSequences() {
    ...
    const selectedSequencesIds = [...new Set(selectedSequenceTimePoints.map((o) => o.sequence))];
  ...
  get selectedZCoordinate() {
    ...
    if (field) {
      const {selectedZCoordinates = []} = this.state;
      const available = selectedZCoordinates.filter((z) => Number(z) < field.depth).sort(ascSorter);
  ...
  get selectedWells() {
    ...
    if (sequence) {
      ...
      const wellIsSelected = (aWell) => selectedWells.some((o) => o.id === aWell.id);
      const {wells = []} = sequence;
      return wells.filter(wellIsSelected).sort((a, b) => a.x - b.x || a.y - b.y);
    }
    return [];
  }
  ...
  get selectedWellFields() {
    ...
    if (well) {
      ...
      const fieldIsSelected = (aField) => selectedFields.some((o) => aField.id === o.id);
      return (well.images || []).filter(fieldIsSelected).sort((a, b) => a.x - b.x || a.y - b.y);
    }
    return [];
  }
  ...
  get showBatchJobInfo() {
    const {batchJobId, batchAnalysis} = this.state;
    return batchAnalysis && batchJobId;
  }

  prepare = () => {
    const {storage, storageId, path} = this.props;
    if (this.hcsInfo) {
      this.hcsInfo.destroy();
      this.hcsInfo = undefined;
      this.hcsVideoSource.setHcsFile();
      this.hcsMergedImageSource.setHcsFile();
    }
    if ((storageId || storage) && path) {
      if (this.hcsAnalysis) {
        this.hcsAnalysis.setSource(storage ? storage.id : storageId, path);
      }
      this.setState(
        {
          sequencePending: false,
        ...
        () => {
          HCSInfo.fetch({storageInfo: storage, storageId, path})
            .then((info) => {
              const {sequences = [], width, height, viewSettings} = info;
              const [first] = sequences;
              const {sequence, timeSeries = []} = first || {};
              this.setState(
                {
                  pending: false,
                  error: undefined,
                ...
                () => {
                  this.hcsInfo = info;
                  this.hcsInfo.addURLsRegeneratedListener(this.loadImage);
                  const hcsFilePayload = {
                    storageId: storage ? storage.id : storageId,
                    path,
                    pathMask: info && info.objectStorage ? info.objectStorage.pathMask : undefined,
                  };
                  this.hcsVideoSource.setHcsFile(hcsFilePayload);
                  this.hcsMergedImageSource.setHcsFile(hcsFilePayload);
    ...
    this.setState(
      ...
      () => {
        ...
        if (timePointId === currentTimePointId && currentSequenceId === newSequenceId) {
          ...
          this.changeSequence();
        } else if (this.hcsImageViewer && !this.hcsMergedImageSource.omeTiffAvailable) {
        ...
        this.hcsMergedImageSource.setSequenceTimePoints(newSequenceId, timePointId);
      },
    );
  };
    ...
    this.setState(
      ...
      () => {
        ...
        if (
          newZ !== currentZ &&
          this.hcsImageViewer &&
          !this.hcsMergedImageSource.omeTiffAvailable
        ) {
          this.hcsImageViewer.setGlobalZPosition(Number(newZ));
        }
        this.hcsVideoSource.setZPlanes(this.selectedZCoordinate);
        this.hcsMergedImageSource.setZPlanes(selection.slice(), mergeZPlanes);
        this.loadImageForAnalysis();
      },
    );
  };
  ...
  changeWellImages = (images = []) => {
    this.setState(
      ...
      () => {
        ...
        if (sequence && well && fields.length > 0 && this.hcsMergedImageSource) {
          this.hcsMergedImageSource.setWell(
            well,
            fields.map((f) => f.id),
          );
        }
  ...
  loadImage = () => {
    ...
    const well = this.selectedWell;
    const fields = this.selectedWellFields;
    if (this.hcsMergedImageSource.omeTiffAvailable && this.hcsImageViewer) {
      this.hcsMergedImageSource.urlsManager
        .setActiveURL(
          this.hcsMergedImageSource.omeTiffPath,
          this.hcsMergedImageSource.omeTiffOffsetsPath,
        )
        .then(() => {
          this.hcsMergedImageSource.urlsManager.reportReadAccess();
          return Promise.resolve();
        })
        .then(() => {
          this.hcsImageViewer
            .setData(
              this.hcsMergedImageSource.urlsManager.omeTiffURL,
            ...
            .then(() => {
              if (this.hcsImageViewer) {
                const imagePayload = {
                  imageTimePosition: 0,
                  imageZPosition: 0,
                  mesh: this.showEntireWell ? getWellMesh(well, fields) : undefined,
                };
    ...
    if (sequence && well && fields.length > 0) {
      ...
      if (this.hcsVideoSource) {
        this.hcsVideoSource.setWellView(this.showEntireWell, id, well, multipleZCoordinates);
      }
      if (this.hcsImageViewer) {
          ...
          .then(() => {
            if (this.hcsImageViewer) {
              const imagePayload = {
                ...
                mesh: this.showEntireWell ? getWellMesh(well, this.selectedWellFields) : undefined,
              };
              this.hcsImageViewer.setImage(imagePayload);
              this.loadImageForAnalysis();
            }
  ...
  loadImageForAnalysis = () => {
    if (this.hcsInfo && this.hcsAnalysis && this.hcsImageViewer) {
      ...
      const imageSelected = (anImage) =>
        selectedFields.some((aField) => aField.fieldID === anImage.fieldID);
      this.selectedSequences.forEach((sequence) => {
        const timePoints = selectedSequenceTimePoints
          .filter((o) => o.sequence === sequence.id)
          .map((o) => Number(o.timePoint));
        const wells = sequence.wells.filter((aWell) =>
          selectedWells.some((w) => w.id === aWell.id),
        );
        wells.forEach((aWell) => {
          const {x: row, y: column} = aWell;
      ...
      const sequence = this.selectedSequence;
      const fields = this.selectedWellFields;
      if (sequence && sequence.sourceDirectory && fields.length > 0) {
        this.hcsAnalysis.updatePhysicalSize(fields[0].physicalSize, fields[0].unit);
  ...
  renderDetailsActions = (className = styles.detailsActions, handleClick = true) => {
    const {children, detailsButtonTitle = 'Show details'} = this.props;
    if (children) {
      return (
        <Button
          size="small"
  ...
  toggleWellView = () => {
    if (this.showEntireWell) {
      this.changeWellImages(this.selectedWellFields.slice(0, 1));
    } else if (this.wellViewAvailable && this.selectedWell) {
      const {images = []} = this.selectedWell;
      this.changeWellImages(images.slice());
    }
  };

  showDetailsPanel = () => {
    const {children, detailsTitle = 'Details'} = this.props;
    return (
      <Panel title={detailsTitle} className={styles.detailsPanel} onClose={this.hideDetails}>
        {children}
      </Panel>
    );
  };
  ...
  renderConfigurationActions = () => {
    const {error, showConfiguration, showAnalysis} = this.state;
    if (
      error ||
      !this.hcsViewerState ||
    ...
    const analysisAvailable =
      this.hcsAnalysis && this.hcsAnalysis.available && this.viewSettings.analysis;
    const selectedImage = this.selectedWellFields[0];
    if (!showConfiguration) {
      return (
        <div className={styles.configurationActions}>
          {this.wellViewAvailable && (
            <Button className={styles.action} size="small" onClick={this.toggleWellView}>
          ...
          this.hcsViewerState.volumetricViewerAvailable ? (
            <HCS3DButton size="small" className={styles.action} viewer={this.hcsImageViewer} />
          ) : null}
          ...
          <Button className={styles.action} size="small" onClick={this.showConfiguration}>
    ...
    return (
      <Panel title="Settings" className={styles.configuration} onClose={this.hideConfiguration}>
        <HcsImageControls />
        <ObjectsOutline />
        <div className={styles.additionalConfigurationControls}>
          {this.wellViewAvailable && (
            <div className={styles.action}>
              ...
              >
                <Radio.Button value="enabled" className={styles.wellViewButton}>
                  Well view
                </Radio.Button>
                <Radio.Button value="disabled" className={styles.wellViewButton}>
                  Field view
                </Radio.Button>
              </Radio.Group>
            </div>
  ...
  renderHcsAnalysisResults = () => {
    const {showAnalysis, showAnalysisResults} = this.state;
    if (
      showAnalysis &&
      showAnalysisResults &&
      this.hcsAnalysis &&
      ...
      return (
        <div className={styles.hcsImageAnalysisResults}>
          <div className={classNames(styles.hcsImageAnalysisResultsBackground, 'cp-panel')}>
            {'\u00A0'}
          </div>
          <AnalysisOutput
  ...
  renderHcsImageAnalysis = () => {
    const {showAnalysis, showAnalysisResults, batchAnalysis, batchJobId} = this.state;
    if (showAnalysis) {
  ...
  renderViewer() {
    const {error, pending: hcsImagePending, sequencePending, showDetails} = this.state;
    const pending =
      hcsImagePending ||
      sequencePending ||
      (!this.hcsImageViewer && !this.hcsVideoSource.videoMode) ||
      (this.hcsVideoSource.videoMode && this.hcsVideoSource.videoPending) ||
      this.hcsSourceState.pending ||
      this.hcsViewerState.pending ||
      this.hcsMergedImageSource.omeTiffPending;
    if (this.showBatchJobInfo) {
      return null;
    }
    return (
      <div className={classNames(styles.hcsImageRenderer, 'cp-dark-background')}>
        {pending && <LoadingView className={styles.loadingView} />}
        {error && (
          <div className={styles.alertContainer}>
            {this.renderDetailsActions(styles.hiddenDetailsActions, false)}
            <Alert className={styles.alert} type="error" title={error} />
          </div>
        )}
        {!error && this.renderConfigurationActions()}
        {showDetails ? this.showDetailsPanel() : this.renderDetailsActions()}
        <div
          className={classNames(styles.hcsImage, {
        ...
        {this.hcsVideoSource.videoMode && (
          <HcsVideoPlayer videoSource={this.hcsVideoSource} className={styles.hcsImage} />
        )}
  ...
  renderSelectors() {
    ...
    if (
      ...
      if (viewSettings.well) {
        parts.push(
          <HcsCellSelector
            ...
            gridMode="CROSS"
            width={HcsCellSelector.widthCorrection(selectedWell.width, selectedWell.images)}
            height={HcsCellSelector.heightCorrection(selectedWell.height, selectedWell.images)}
            scaleToROI
      ...
      return (
        <div
          className={classNames(styles.hcsImageControls, 'cp-content-panel')}
  ...
  renderBatchInfo = () => {
    ...
    const {batchJobId} = this.state;
    return <CellProfilerJobResults className={styles.hcsBatchJobInfo} jobId={batchJobId} />;
  };
  ...
  render() {
    ...
    return (
      <Provider
        ...
        hcsMergedImageSource={this.hcsMergedImageSource}
      >
        <div className={classNames(className, styles.hcsImageContainer)} style={style}>
