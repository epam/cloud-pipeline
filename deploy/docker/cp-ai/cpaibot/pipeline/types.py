...
class Pipeline(BaseModel):
    ...
    def url(self) -> str | None:
        return settings.get_application_entity_url(f'#/{self.id}')
    ...
    def get_version_url(self, version: str) -> str | None:
        return settings.get_application_entity_url(f'#/{self.id}/{version}')
...
class DockerImage(Tool):
    ...
    @property
    def url(self) -> str | None:
        return settings.get_application_entity_url(f'#/tool/{self.id}')

    def to_markdown(self) -> str:
        url = self.url
        title = f'**[{self.image}]({url})**' if url is not None else f'**{self.image}**'
        return f'{title}: {self.shortDescription}' if self.shortDescription is not None else title

    def to_json(self) -> dict:
        return self.model_dump(mode="json", exclude={"group", "registry"})
...
class InstanceType(BaseModel):
    ...
    def to_markdown(self) -> str:
        return f"{self.name} ({self.vcpu} CPU, {self.memory} GB, {self.gpu} GPU)"
