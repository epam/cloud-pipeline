...
import {Radio, InputNumber, DatePicker, Input} from 'antd';
...
class InputFilter extends React.Component {
  ...
  onApplyFilter = () => {
    ...
    this.storage.changeFilterField(FILTER_FIELDS[filterKey], this.state.value);
  ...
  render() {
    const {label, labelStyle, placeholder} = this.props;
    return (
...
class SizeFilter extends React.Component {
  ...
  render() {
    ...
    return (
      <div className={styles.filterWrapper} onKeyDown={this.onKeyDown}>
        <div className={styles.inputContainer}>
        ...
        <FilterFooter onOk={this.onApplyFilter} onClear={this.onClearFilter} />
      </div>
    );
  }
}
...
class DateFilter extends React.Component {
  ...
  onChangeRadio = (event) => {
    this.setState({dateFilterType: event.target.value}, () => {
      ...
      const from =
        typeof predefined?.dateAfter === 'function' ? predefined.dateAfter(currentDate) : undefined;
      const to =
        typeof predefined?.dateBefore === 'function'
  ...
  onChangeFrom = (date) => {
    ...
    this.setState({[FILTER_FIELDS.dateAfter]: d ? d.startOf('day') : undefined});
  };

  onChangeTo = (date) => {
    const d = ensureDayjs(date);
    this.setState({[FILTER_FIELDS.dateBefore]: d ? d.endOf('day') : undefined});
  };
      ...
      <div style={{display: 'flex', flexDirection: 'column'}}>
        <div className={styles.datePickerContainer}>
          ...
          <DatePicker
            ...
            value={ensureDayjs(this.state[FILTER_FIELDS.dateAfter])}
        ...
        <div className={styles.datePickerContainer}>
          ...
          <DatePicker
            ...
            onOpenChange={this.onOpenChange}
            value={ensureDayjs(this.state[FILTER_FIELDS.dateBefore])}
          />
        </div>
      </div>
    );
      ...
      >
        ...
        >
          {this.predefinedDateFilters.map((filter) => (
            <Radio key={filter.key} value={filter.key}>
              {filter.title}
            </Radio>
          ))}
...
function FilterFooter({onOk, onClear, okDisabled}) {
  ...
  return (
    <div className={styles.filterWrapperControls}>
      <a disabled={okDisabled} className={okDisabled ? 'cp-disabled' : null} onClick={handleOk}>
        OK
      </a>
