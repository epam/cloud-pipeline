...
@inject((_, props) => {
  ...
  const {runId = propRunId} = params;
  return {
    runId: Number.isNaN(Number(runId)) ? undefined : Number(runId),
    preLoadedRun,
  };
})
@inject('preferences', 'dtsList', 'multiZoneManager', 'dockerRegistries', 'uiNavigation')
@runPipelineActions
@observer
class RunDetails extends React.Component {
  ...
  onRunChanged = () => {
    ...
    const {runId, preLoadedRun, preferences, dockerRegistries, uiNavigation} = this.props;
    ...
    if (runId) {
      this.setState(
        ...
        async () => {
          ...
          try {
            await Promise.all([uiNavigation.fetch(), preferences.fetchIfNeededOrWait()]);
            const {
              stop,
              fetch: reFetch,
              data,
  ...
  renderContent = (currentTab) => {
    ...
    if (error && !pending) {
      return <Alert title={error} showIcon type="error" />;
    }
  ...
  render() {
    const {className, style, runId, currentMode, modes, onChangeMode} = this.props;
    const {run, loaded, runTasks, runTasksLoaded, tabs} = this.state;
    const tab = this.getActiveTab();
    return (
      <div className={classNames(className, styles.runDetails)} style={style}>
        <RunHeader
          className={classNames(
            styles.runHeader,
            styles.runDetailsSection,
