...
import styles from './run-capabilities.module.css';
...
const CAPABILITIES_OS_FILTERS = {
  ...
  [RUN_CAPABILITIES.dcv]: ['centos 7*', 'rocky*', 'ubuntu 18.04', 'ubuntu 20.04'],
};
...
function parseOSMask(mask) {
  ...
  const regExpValue = mask.trim().replace(/\./g, '\\.').replace(/\*/g, '.*');
  return new RegExp(`^${regExpValue}$`, 'i');
}

function getAllPlatformCapabilities(preferences, platformInfo = {}) {
  const {platform, os, provider, region, reservationConfig} = platformInfo;
  const capabilities =
    platform && Object.hasOwn(PLATFORM_SPECIFIC_CAPABILITIES, platform)
  ...
  const cloudMatches = (capabilityCloud) => {
    ...
    return (
      ...
      (!capabilityRegionId || !region || capabilityRegionId === region.id)
    );
  };
  const filterByCloudProvider = (capability) =>
    !capability.cloud || capability.cloud.length === 0 || capability.cloud.some(cloudMatches);
  const mapCapability = (capability, parent) => {
    const {capabilities = [], ...capabilityInfo} = capability;
    const enabledByOS = filterByOS(capability);
    const enabledByCloudProvider = filterByCloudProvider(capability);
    const enabledByReservationConfig = isEnabledByReservationConfig(capability, reservationConfig);
    return {
      ...capabilityInfo,
...
function getAllRequiredCapabilities(preferences) {
  return getAllPlatformCapabilities(preferences).filter(
...
function plainList(capabilities = []) {
  ...
  return (capabilities || []).map(getPlain).reduce((r, c) => [...r, ...c], []);
}

function getPlatformSpecificCapabilities(preferences, platformInfo = {}) {
  return plainList(
    getAllPlatformCapabilities(preferences, platformInfo).filter(
...
export function isEnabledByReservationConfig(capability, reservationConfig) {
  ...
  const {kube_assign_policy: _kubeAssignPolicy = {}, kubeAssignPolicy = _kubeAssignPolicy} =
    reservationConfig || {};
...
class RunCapabilities extends React.Component {
  ...
  setInitialRequiredCapabilities = () => {
    ...
    this.props.preferences
      .fetchIfNeededOrWait()
      .then(() => fetchToolDefaultParameters(this.props.dockerImage, this.props.dockerRegistries))
      .then((parameters) => {
  ...
  get allCapabilities() {
    const {platform, provider, region, preferences} = this.props;
  ...
  get filteredValues() {
    const {platform, provider, region, preferences, values} = this.props;
    const {os, reservationConfig} = this.state;
    const capabilities = getPlatformSpecificCapabilities(preferences, {
      platform,
      os,
      provider,
      region,
    ...
    return (values || []).filter((value) => capabilities.find((o) => o.value === value));
  }

  get hasErrors() {
    const {values, preferences, mode, showError = true} = this.props;
    return (
      checkRequiredCapabilitiesErrors(values, preferences) &&
  ...
  fetchDockerImageOS() {
    const {dockerImageOS, dockerImage, dockerRegistries} = this.props;
    if (dockerImageOS) {
      this.setState(
        {
          os: dockerImageOS,
      ...
      fetchToolOS(dockerImage, dockerRegistries).then((os) => {
        if (this._mounted) {
  ...
  onSelectionChanged = (values) => {
    const {platform, provider, region, preferences, onChange} = this.props;
    const {os, reservationConfig} = this.state;
    const capabilities = getPlatformSpecificCapabilities(preferences, {
      platform,
      os,
  ...
  getCapabilityByValue = (value) => {
    return plainList(this.allCapabilities).find((capability) => capability.value === value);
  };
  ...
  renderRequiredCapabilities = () => {
    ...
    return [
      this.filteredValues.filter(this.capabilityIsRequired).map((value) => (
        <div
          key={value}
  ...
  toggleValue = (value) => {
    ...
    let newSelection = (this.filteredValues || []).filter((existing) => existing !== value);
    if (isSelected) {
      // add capability
    ...
    if (isRequired && isSelected) {
      ...
      const siblings = (current.capabilities || []).map((capability) => capability.value);
      // this is "parent" capability
      ...
      if (!parent.multiple) {
        const siblings = (parent.capabilities || []).map((capability) => capability.value);
        newSelection = newSelection.filter(
          (existing) => existing === value || !siblings.includes(existing),
        );
      }
      ...
      const siblings = (parent.capabilities || []).map((capability) => capability.value);
      const initialSelection = !!initialRequiredCapabilities.find(
        (initial) => initial === parent.value,
      );
      if (
        initialSelection &&
    ...
    return (
      <div className={className}>
        ...
        >
          <div
            tabIndex={0}
            className={classNames(styles.runCapabilitiesInput, 'cp-run-capabilities-input', {
              disabled,
          ...
          >
            ...
            {plainList(this.allCapabilities)
              .filter(
                (o) => this.filteredValues.includes(o.value) && !this.capabilityIsRequired(o.value),
              )
              .map((capability) => (
                <div
                  key={capability.value}
        ...
        <CapabilitiesDisclaimer values={this.filteredValues} style={{marginTop: 5}} />
      </div>
    );
  }
}
    ...
    this.setState(
      ...
      () => {
        ...
        if (result && typeof result.then === 'function') {
          result.catch(() => {}).then(() => this.setState({disabled: false}));
        } else {
          this.setState({disabled: false});
        }
      },
  ...
  render() {
    ...
    return (
      <div className={styles.runCapabilitiesMetadataContainer}>
        <div className="cp-text" style={{fontWeight: 'bold', marginRight: 5}}>
          Run capabilities:
        </div>
        <RunCapabilities
          mode={RUN_CAPABILITIES_MODE.launch}
...
export function getEnabledCapabilities(parameters) {
  const result = [];
  const predefinedCapabilities = Object.entries(RUN_CAPABILITIES_PARAMETERS).map(
...
function correctParametersForRequiredCapabilities(parameters, preferences) {
  ...
  enabled.forEach((capability) => {
    const parent = required.find((r) =>
      (r.capabilities || []).map((o) => o.value).includes(capability),
    );
...
export function updateCapabilities(parameters = {}, capabilities = [], preferences, platform) {
...
export async function applyUserCapabilities(parameters, preferences, platform) {
  ...
  return applyCapabilities({...(parameters || {})}, userCapabilities, preferences, platform);
}
...
export function applyCustomCapabilitiesParameters(parameters, preferences) {
  ...
  const parseStringValues = (string) =>
    `${string || ''}`
      .split(',')
      .map((o) => o.trim())
      .filter((o) => o.length);
  const mergeValues = (newValue, previousValue) => {
    const values = [
      ...new Set([...parseStringValues(newValue), ...parseStringValues(previousValue)]),
    ];
    return values.join(',');
  };
  for (const customCapability of customCapabilities) {
    ...
    for (const [parameter, value] of customCapabilityParameters) {
      if (value !== undefined) {
        ...
        result[parameter] = {
          type,
          value: type === 'boolean' ? value : mergeValues(`${value}`, previous),
        };
      }
    }
  }
...
export function checkRunCapabilitiesModified(capabilities1, capabilities2, preferences) {
  const wellKnownCapabilities = Object.values(RUN_CAPABILITIES).concat(
    (preferences ? plainList(preferences.launchCapabilities) : []).map((o) => o.value),
  );
  const sorted = (array) => [
...
export function checkRequiredCapabilitiesErrors(values, preferences) {
  ...
  const requiredCapabilities = getAllPlatformCapabilities(preferences).filter(
    (capability) =>
      (values || []).includes(capability.value) &&
...
function getDisclaimersList(options = {}) {
  const {values, preferences, parameters} = options;
  if (!preferences) {
    return [];
  }
  ...
  const capabilityNames =
    values && values.length ? values : getEnabledCapabilities(parameters || {});
...
function CapabilitiesDisclaimerRenderer({
  ...
  return filteredValuesDisclaimers.length ? (
    <Alert
      showIcon={showIcon}
      type="warning"
      className={className}
      ...
      title={
        <div>
          {filteredValuesDisclaimers.map((disclaimer, idx) => (
            <p key={`disclaimer-${idx}`} style={{marginBottom: 2}}>
              {disclaimer}
            </p>
          ))}
        </div>
