...
import styles from './container-logs-modal.module.css';
...
export default class ContainerLogsModal extends React.Component {
  ...
  downloadCompleteLog = async () => {
    ...
    this._pending = false;
    try {
      downloadBlob(new Blob([request.value]), `${container.name}-full-logs.txt`);
    } catch (e) {
    ...
    return (
      ...
      >
        <Spin spinning={this.pending}>
          ...
          >
            <a onClick={this.refreshLogs}>Refresh logs</a>
          </div>
          <RunTaskLogs
