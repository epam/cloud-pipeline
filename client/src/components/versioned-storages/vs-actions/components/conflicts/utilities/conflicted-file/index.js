...
import Branches, {HeadBranch, RemoteBranch, Merged} from './branches';
...
class ConflictedFile {
  ...
  registerOperation(operation) {
    const {isFinite = true, type} = operation;
    ...
    if (
      ...
      if (current.type === type && !current.isFinite) {
        ...
        this.operations.splice(this.operationIndex, 1, {
          ...current,
          apply: (finalOperation = true) => {
            ...
            if (operation.apply) {
              operation.apply(finalOperation);
            }
          },
          revert: (finalRevertOperation = true) => {
            if (operation.revert) {
              operation.revert(false);
            }
            if (current.revert) {
              current.revert(finalRevertOperation);
            }
          },
        });
      } else {
  ...
  insertLineBefore(before, line, meta, ...conflictedBranchDirections) {
    if (before) {
      ...
      Branches.forEach((branch) => {
        ...
        let child = before;
        if (parent && parent.state[branch] === LineStates.conflictEnd) {
  ...
  static splitText(text, eof) {
    const parts = (text || '')
      .split('\n')
      .map((line, idx, arr) => (idx === arr.length - 1 ? line : line.concat('\n')));
  ...
  getLineAtOriginalIndex(branch, index) {
    return this.getLineAtIndex(branch, index, ConflictedFile.getLineAtOriginalIndexIgnoreStates);
  }
  ...
  getHeadText() {
    return this.getHeadLines()
      .map((o) => o.text[HeadBranch])
      .join('');
  }

  getRemoteText() {
    return this.getRemoteLines()
      .map((o) => o.text[RemoteBranch])
      .join('');
  }

  getMergedText() {
    return this.getMergedLines()
      .map((o) => o.text[Merged])
      .join('');
  }
  ...
  getFirstConflict(from = undefined) {
    ...
    if (startMarker) {
      const endMarker = this.getFirstConflictMarker(LineStates.conflictEnd, startMarker);
      if (endMarker) {
        return {
          start: startMarker,
          end: endMarker,
  ...
  correctLineBreaks(branch) {
    ...
    for (let i = 0; i < lines.length - 1; i++) {
      const line = lines[i];
      if (!line.text[branch] || !line.text[branch].endsWith('\n')) {
  ...
  buildLineNumbers(branch, from = undefined, last = 0) {
    ...
    const moveNumber =
      (!from.meta || !from.meta.start) &&
      (from.state[branch] === LineStates.inserted || from.state[branch] === LineStates.original);
  ...
  static processText(items, branch, revert = false) {
    const ignoreStates = new Set([
      ...
      LineStates.conflictStart,
      LineStates.conflictEnd,
      revert ? LineStates.inserted : LineStates.removed,
    ]);
    return (items || [])
  ...
  static findCommon = (o, nextFn, ...candidatesSet) => {
    ...
    const common = candidatesSet.map((set) => set.has(o)).reduce((r, c) => r && c, true);
    if (common) {
      return o;
    }
    const nextCheck = [...new Set(Branches.map((branch) => nextFn(o, branch)).filter(Boolean))];
