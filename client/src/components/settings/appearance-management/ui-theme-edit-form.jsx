...
import {sections, sectionsConfiguration, VariableTypes} from './utilities/variable-sections';
import ElementPreview from './element-preview';
import styles from './ui-theme-edit-form.module.css';
import ImageUploader from './image-uploader';
import ProviderIcon from './provider-icon';

const Title = ({className, title, required}) => (
  <span
    className={classNames(styles.title, className, 'cp-settings-form-item-label', {
      required,
    })}
  >
    {title}
...
const FormItem = ({
  ...
  if (divider) {
    return (
      <div className={classNames(styles.formItemDivider, {[styles.hidden]: hidden})}>
        {'\u00A0'}
      </div>
    );
  }
  return (
    <div
      className={classNames(styles.formItemContainer, {
    ...
    >
      ...
      <div className={classNames(styles.errorContainer, styles.formItem)}>
        <Title className={titleClassName} title={title} required={required} />
        <span className={classNames(styles.errorDescription, 'cp-error')}>
...
const Section = ({
  ...
  <section className={classNames(styles.section, 'cp-divider', 'bottom')}>
    {title && (
      <div className={styles.sectionTitle}>
        <h2>{title}</h2>
        {expandable && identifier && (
          <a
            className={classNames(styles.sectionExpandButton, 'underline')}
...
class UIThemeEditForm extends React.PureComponent {
  ...
  componentDidUpdate(prevProps, prevState, snapshot) {
    if (prevProps.theme !== this.props.theme || prevProps.themes !== this.props.themes) {
      this.updateFromProps();
    }
  }
  ...
  updateFromProps = () => {
    ...
    const {name, extends: baseThemeIdentifier} = theme;
    this.setState(
      {
        name,
  ...
  onChange = (valid) => {
    const {name, extends: baseTheme, themeProperties = {}, parsedValues} = this.state;
    ...
    return new Promise((resolve) => {
      ...
      const state = {
        validation: {
          name: validateName(
            name,
            themes.filter((o) => o.identifier !== identifier),
          ),
        },
      };
      if (!skipProperties) {
      ...
      this.setState(state, () => {
        const {validation = {}, propertiesValidation = {}} = this.state;
        resolve(
  ...
  onChangeConfigurationVariable = (variable, value) => {
    const {themeProperties = {}} = this.state;
    const newProperties = {
      ...themeProperties,
      [variable]: value,
    };
  ...
  render() {
    ...
    const {theme, themes = [], readOnly, previewClassName, removable} = this.props;
    if (!theme) {
      return null;
    }
    return (
      <div>
        <Section title="General">
          ...
          >
            <Select
              ...
              disabled={readOnly}
            >
              {getBaseThemes(themes, theme).map((baseTheme) => (
                <Select.Option key={baseTheme.identifier} value={baseTheme.identifier}>
                  {baseTheme.name}
                </Select.Option>
              ))}
            </Select>
        ...
        {sections.map((section) => (
          <Section
            ...
            title={section}
            expandable={
              sectionsConfiguration[section].some((o) => o.advanced) ? expandedState : undefined
            }
            toggleExpanded={this.expandCollapseSection}
          >
            <div className={styles.sectionContent}>
              <div className={styles.properties}>
                {sectionsConfiguration[section].map(({variable, advanced = false}, index) => (
                  <FormItem
                  ...
                  >
                    {(!variable.type || variable.type === VariableTypes.color) && (
                      <ColorVariable
                        className={classNames(styles.control, {
                        ...
                        disabled={readOnly}
                        value={themeProperties[variable.key] || mergedProperties[variable.key]}
              ...
              <ElementPreview
                className={classNames(styles.previews, previewClassName, 'themes-management')}
                section={section}
              />
            </div>
          </Section>
