...
const NGB_FILE_INDEX_PATH = {
  bam: [(path) => appendExtension(path, 'bai'), (path) => replaceExtension(path, 'bai')],
  vcf: [(path) => appendExtension(path, 'tbi'), (path) => appendExtension(path, 'idx')],
};
  ...
  if (request.loaded) {
    ...
    if (content) {
      try {
        ...
        throw new Error(
          `Error reading content for path ${path} (storage #${storageId}): ${e.message}`,
        );
      }
    } else {
...
async function readNgbSettings(
  storageId,
  ngbFilePath,
  ngbSettingsFileName = NGB_SETTINGS_FILE_DEFAULT_NAME,
) {
  try {
    const content = await readFileContents(
      storageId,
      getNgbSettingsFilePath(ngbFilePath, ngbSettingsFileName),
    );
    if (content) {
...
export async function openNgbFile(options) {
  ...
  const {pathMask} = storageRequest.value || {};
  let storagePathMask = pathMask || '';
  if (!storagePathMask.endsWith('/')) {
    storagePathMask = storagePathMask.concat('/');
  }
  const {chromosome, chr = chromosome, start, from = start, end, to = end} = fileNameSettings || {};
  const tracks = [
    {
      b: reference,
      n: reference,
  ...
  const getFilePath = (storagePath) =>
    storagePath.startsWith('/')
      ? `${storagePathMask}${storagePath.slice(1)}`
      : `${storagePathMask}${storagePath}`;
  const extension = getNgbFileExtension(path);
