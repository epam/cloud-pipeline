...
import styles from './color-variable.module.css';
...
class ColorVariable extends React.PureComponent {
  ...
  onChangeType = (newType) => {
    const {variable, onChange, parsedValue, value} = this.props;
    if (!onChange) {
      return;
    }
    if (!newType) {
      ...
      // variable
      const {type, function: f, amount} = parse(value);
  ...
  renderAsColor = () => {
    const {parsedValue, disabled, variable, onChange} = this.props;
    const onChangeColor = (color) => {
      if (onChange) {
        onChange(variable, color);
      }
    };
    return <ColorPicker color={parsedValue} onChange={onChangeColor} disabled={disabled} />;
  };

  renderFunction = () => {
    const {value, variable, onChange, error, disabled} = this.props;
    const {function: f, amount, value: arg} = parse(value);
  ...
  onRevert = () => {
    const {onChange, variable, initialValue} = this.props;
    if (onChange) {
  ...
  render() {
    ...
    const {key, type} = parse(value);
    return (
      <div className={classNames(styles.colorVariable, className)}>
        {this.renderAsColor()}
        <Select
          allowClear
          showSearch
          ...
          {groupedColorVariables.map((group) => (
            <Select.OptGroup key={group.name} label={group.name}>
              {group.variables.map((variable) => (
                <Select.Option key={variable.key} value={variable.key} title={variable.name}>
                  <div className={styles.colorSelectOption}>
                    <ColorPresenter
                      ...
                      style={{marginRight: 5}}
                    />
                    <span className={classNames(styles.colorSelectOptionName, 'cp-ellipsis-text')}>
                      {variable.name}
                    </span>
                  </div>
                </Select.Option>
