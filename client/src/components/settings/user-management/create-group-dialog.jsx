...
import {Row, Checkbox, Input, Button, Modal, message, Select} from 'antd';
...
export default class CreateGroupDialog extends React.Component {
  ...
  createGroup = () => {
    this.setState(
      ...
      () => {
        ...
        const {name, isDefault, dataStorageId} = this.state;
        const hide = message.loading('Creating group...', 0);
        const request = new RoleCreate(name, isDefault, dataStorageId);
        request
          .send({})
          .then(() => {
            if (request.error) {
  ...
  render() {
    const {visible, onClose, dataStorages} = this.props;
    const {name, isDefault, dataStorageId, pending} = this.state;
    return (
      <Modal
        title="Create group"
        closable={!pending}
        ...
        footer={
          <Row type="flex" justify="space-between">
            ...
            <Button disabled={!name || pending} type="primary" onClick={this.createGroup}>
              Create
            </Button>
          </Row>
        }
      ...
      >
        ...
        <Row style={{marginTop: 15, paddingLeft: 2, marginBottom: 2}}>Default data storage:</Row>
