...
import styles from './configure-run-as-permissions.module.css';
...
class DockerImageSelector extends React.Component {
  ...
  get tools() {
    ...
    const isDisabled = (tool) => {
      return (
        imagesToExclude && imagesToExclude.length && imagesToExclude.includes(tool.dockerImage)
      );
    };
    const result = [];
  ...
  renderSelectGroup = (group) => {
    return group.tools.map((tool) => {
      ...
      return (
        <Select.Option key={tool.id} value={`${tool.id}`} title={title}>
  ...
  render() {
    ...
    return (
      <div className={className} style={style}>
        <div className={classNames(styles.container)} style={containerStyle}>
          <Select
