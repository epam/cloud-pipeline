...
class TasksGroupList extends React.Component {
  ...
  render() {
    ...
    const filtered = tasksGroups.filter(
      (t) =>
        !filter ||
    ...
    return (
      <div className={classNames(className, styles.tasksGroups)} style={style}>
        <div className={classNames(styles.tasksGroupListHeader)}>
          <span style={{fontWeight: 'bold', flexShrink: 0}}>Processes</span>
          {pending && tasksGroups.length === 0 && <LoadingOutlined style={{marginLeft: 5}} />}
