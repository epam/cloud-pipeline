...
function removePermission(storageId, permission) {
  ...
  return new Promise((resolve) => {
    ...
    request
      ...
      .then(() => resolve())
      .catch((e) => {
        const {name, principal} = permission;
        console.warn(
...
export function getSharedStoragePermissions(storageId) {
  ...
  return new Promise((resolve) => {
    ...
    request
      ...
      .then((permissions = []) => {
        const mask = permissions.reduce(
          (reducedMask, current) => reducedMask & (current.mask || 0),
          0b1111,
        );
        return Promise.resolve({
          mask,
          permissions: permissions
            .map(({sid}) =>
              sid && sid.name ? {name: sid.name, principal: !!sid.principal} : undefined,
            )
            .filter(Boolean),
        });
      })
 ...
 */
export function grantSharedStoragePermissions(storageId, mask, permissions = [], replace = true) {
  if (!storageId) {
    return Promise.reject(new Error('Storage not specified'));
  }
