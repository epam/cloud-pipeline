...
const extractDataSets = (rawData, colors, currentPoolId, hiddenDatasets = []) => {
  const dataEntries = rawData.map((entry, index) => ({
    ...entry,
    color: getColor(index, colors),
  }));
  const labels = ((dataEntries[0] || {}).records || []).map((o) => ({
    label: o.measureTime,
  ...
  const datasets = dataEntries.map(({poolId, poolName, records, color}) => ({
    ...
    backgroundColor: isCurrentPool(poolId) ? getFadedColor(color) : undefined,
    poolId,
    label: poolName,
    hidden: isHidden(poolId),
    borderColor: color,
    borderWidth: isCurrentPool(poolId) ? 4 : 1.5,
...
function OverallPoolChart({
  ...
  const dataConfiguration = extractDataSets(rawData, colors, currentPoolId, hiddenDatasets);
  const options = {
    ...
    plugins: {
      ...
      [ChartHoverCursorPlugin.id]: {
        handler: (event, isLineHovered) => {
          event.native.target.style.cursor = isLineHovered && onClick ? 'pointer' : 'default';
        },
      },
    },
  };
