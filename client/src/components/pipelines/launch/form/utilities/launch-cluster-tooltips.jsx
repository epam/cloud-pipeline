...
const CLUSTER_MODES_TOOLTIP = (
  <div>
    <Row>
      Cluster configuration allows you specify number of compute nodes that will process a job. This
      is useful if a job is too heavy for a single node or it uses scheduling approach to process a
      series of tasks across nodes.
    </Row>
    <Row>
      <ul type="circle">
        ...
        <li style={{marginLeft: 5}}>
          - <b>Cluster</b>: a specified number of compute nodes will be created and interconnected
          via a shared filesystem (/common/) and SSH. Optionally, GridEngine can be configured for
          the cluster. See "Enable GridEngine" checkbox below
        </li>
        <li style={{marginLeft: 5}}>
          - <b>Autoscaling</b>: allows to startup a small cluster (even a single/master node) and
          then scale it up/down according to the workload. This mode will always setup GridEngine,
          as it is used to control the current jobs queue.
        </li>
      </ul>
    </Row>
  </div>
...
const ENABLE_GRID_ENGINE_TOOLTIP = (
  <div>
    <Row>
      Setting this checkbox will enable the <b>GridEngine</b> for the cluster, providing all the
      compatible command-line utilities, e.g.: <b>qsub, qstat, qhost, qconf,</b> etc.
    </Row>
    <Row>
...
const ENABLE_SPARK_TOOLTIP = (
  <div>
    <Row>
      Setting this checkbox will enable the <b>Apache Spark</b> for the cluster, with the access to{' '}
      <b>File/Object Storages</b> from the Spark Applications.
    </Row>
    <Row>
...
const ENABLE_SLURM_TOOLTIP = (
  <div>
    <Row>
      Setting this checkbox will enable the <b>SLURM</b> scheduler for the cluster, providing all
      the compatible command-line utilities, e.g.: <b>sbatch, scancel, squeue, sinfo,</b> etc.
    </Row>
    <Row>
...
const ENABLE_KUBE_TOOLTIP = (
  <div>
    ...
    <Row>
      This also enables <b>"CP_CAP_DIND_CONTAINER"</b> and <b>"CP_CAP_SYSTEMD_CONTAINER"</b>{' '}
      parameters.
    </Row>
  </div>
);
const AUTOSCALED_CLUSTER_UP_TO_TOOLTIP = (
  <div>
    <Row>
      Defines <b>maximum number of compute nodes</b>, that can be created in the cluster (besides
      the master node and the "fixed" nodes, see "Default child nodes").
    </Row>
    <Row>
      GridEngine queue will be checked for the entries in "wait" state and new compute nodes will be
      spawned to schedule the jobs. Once the autoscaled nodes are not needed - they will terminated.
    </Row>
  </div>
);
const AUTOSCALED_CLUSTER_DEFAULT_NODES_COUNT_TOOLTIP = (
  <div>
    ...
    <Row>
      These nodes will not be scaled down, even if no workload is currently running. It can be
      considered a "fixed" part of the cluster.
    </Row>
    <Row>If not set - only a master node will be available at a startup time.</Row>
  </div>
);
const HYBRID_AUTOSCALED_CLUSTER_TOOLTIP = (
  <div>
    Compute nodes of the cluster will be created according to the jobs' requirements. By default,
    the same instance family (as the master) is used. But the size of the node will vary.
  </div>
);
...
const CHILD_NODE_INSTANCE = (
  <div>
    <Row>You can specify instance type to be used for autoscaled workers.</Row>
  </div>
);
const CHILD_NODE_INSTANCE_FAMILY = (
  <div>
    <Row>
      You can specify instance family for autoscaled workers and the exact size will be selected
      automatically.
    </Row>
  </div>
);
const GPU_SCALING_TOOLTIP = (
  <div>
    <Row>This option creates a separate SGE queue for the GPU-bound tasks:</Row>
    <Row>
...
const tooltips = {
  [LaunchClusterTooltip.clusterMode]: CLUSTER_MODES_TOOLTIP,
  [LaunchClusterTooltip.cluster.enableGridEngine]: ENABLE_GRID_ENGINE_TOOLTIP,
  [LaunchClusterTooltip.cluster.enableSpark]: ENABLE_SPARK_TOOLTIP,
  [LaunchClusterTooltip.cluster.enableSlurm]: ENABLE_SLURM_TOOLTIP,
  [LaunchClusterTooltip.cluster.enableKube]: ENABLE_KUBE_TOOLTIP,
  [LaunchClusterTooltip.autoScaledCluster.autoScaledUpTo]: AUTOSCALED_CLUSTER_UP_TO_TOOLTIP,
