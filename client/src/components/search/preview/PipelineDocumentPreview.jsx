...
@inject((stores, params) => {
  ...
  return {
    ...
    pipeline:
      params.item && params.item.parentId ? pipelines.getPipeline(params.item.parentId) : null,
  };
})
@observer
export default class PipelineDocumentPreview extends React.Component {
  ...
  get structuredTableData() {
    if (
      ...
      this.props.item &&
      this.props.item.path.split('.').pop().toLowerCase() === 'csv'
    ) {
      const result = {};
  ...
  renderDescription = () => {
    ...
    return paths.reduce((result, current, index, arr) => {
      result.push(
        <span key={index * 2} style={{marginRight: 0}}>
          {current}
        </span>,
      );
