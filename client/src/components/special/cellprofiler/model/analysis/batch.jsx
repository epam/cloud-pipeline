...
export function getInputFilesPresentation(inputs = {}) {
  const {files = [], zPlanes = []} = inputs;
  const wellPresentation = (input) => {
    const {x: row, y: column} = input;
    return `${getWellRowName(row - 1)}${column}`;
  };
  const wells = [...new Set(files.map(wellPresentation))].sort(alphabeticalSorter);
  ...
  const getDescription = (items = [], title) =>
    title ? `${title}:${items.join(itemsSeparator)}` : items.join(itemsSeparator);
  return [
...
export async function submitBatchAnalysis(specification) {
  const {dockerImage, settings, specs, results, mounts, rawImagesPath, tempFilesPath} =
    await getBatchAnalysisSettings(true);
  const uuid = generateUUID();
  const specFullPath = specs.storage.joinPaths(specs.folder, uuid, 'spec.json');
  const {measurementUUID, inputs, modules, pipeline} = specification;
  const spec = {
  ...
  const setParameterValue = (parameter, value, skipIfExists = false) => {
    ...
    if (!skipIfExists || !parameters[parameter] || parameters[parameter].value === undefined) {
      parameters[parameter] = {value, type: 'string'};
    }
  };
  const limitMountsParameter = [
    ...new Set((getParameterValue(CP_CAP_LIMIT_MOUNTS) || '').split(',').concat(storagesToMount)),
  ].join(',');
  ...
  setParameterValue(CELLPROFILER_API_BATCH_PIPELINE_NAME, pipeline ? pipeline.name : undefined);
  setParameterValue(CELLPROFILER_API_BATCH_SPEC_INPUTS, getInputFilesPresentation(inputs));
  setParameterValue(CELLPROFILER_API_BATCH_SPEC_MODULES, getModulesPresentation(modules));
...
const getJobParameter = (job, parameterName) => {
  ...
  const {pipelineRunParameters = []} = job;
...
export function filtersAreEqual(a, b, ignorePagination = false) {
  ...
  return (
    aImage === bImage &&
    aPipeline === bPipeline &&
    (ignorePagination || (aPage === bPage && aPageSize === bPageSize)) &&
    aSource === bSource
  );
}
...
export async function getBatchJobs(filters = {}) {
  const {page = 0, pageSize = 30, userNames = [], source, pipeline, statuses} = filters;
  const payload = {
    page: page + 1,
    pageSize,
    userModified: false,
...
export async function getSpecification(job) {
  ...
  const {url: specsUrl, callback = () => {}} = await generateResourceUrlWithAccessCallback(options);
  if (!specsUrl) {
    return undefined;
  }
  callback();
