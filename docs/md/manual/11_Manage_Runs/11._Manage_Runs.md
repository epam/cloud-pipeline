...
| Control | Description |
|---|---|
| **BROWSE** | Allows to open the filesystem of the running instance in a Storage browser Web GUI - so the user can view, download, upload, delete files and folders for the current active run. See [15.3. Expose node filesystem](../15_Interactive_services/15.3._Expose_node_filesystem.md) for more details. |
| **COMMIT** | Allows modifying an existing tool that has been changed via ssh. See [10.4. Edit a Tool](../10_Manage_Tools/10.4._Edit_a_Tool.md#commit-a-tool) for more details. |
...
| **SSH** | Allows to ssh to the running instance filesystem. See [15.2. Using Terminal access](../15_Interactive_services/15.2._Using_Terminal_access.md) for more details. |
