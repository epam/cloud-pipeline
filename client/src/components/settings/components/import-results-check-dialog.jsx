...
import styles from './import-results-check-dialog.module.css';
...
class ImportResultsCheckDialog extends React.Component {
  ...
  render() {
    const {logs, visible, mode, onClose, users, errors} = this.props;
    if (mode.length === 0) {
      return null;
    }
    if (mode.length === 1 && mode.includes('logs')) {
      return <ImportResult logs={logs} mode="modal" visible={visible} onClose={onClose} />;
    }
    ...
    const {actionInProgress, tab} = this.state;
    return (
      <Modal
        title={false}
        open={visible}
        ...
        <Tabs
          ...
          items={[
            ...
            },
            {
              key: 'logs',
              label: 'Import logs',
              children: <ImportResult logs={logs} mode="inline" visible={visible} />,
            },
          ]}
        />
      </Modal>
