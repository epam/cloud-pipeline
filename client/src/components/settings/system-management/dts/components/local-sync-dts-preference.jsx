...
import styles from './local-sync-dts-preference.module.css';
  ...
  onChangeCronField = (field, index, value) => {
    ...
    const correctCron = (schedule) => {
      ...
      if (schedule.every === EVERY_UNITS.days) {
        const value = schedule.everyValue > 31 ? 31 : schedule.everyValue;
        cronString = `0 0 0 1/${value || 1} * ?`;
      }
      if (schedule.every === EVERY_UNITS.hours) {
        const value = schedule.everyValue > 23 ? 23 : schedule.everyValue;
        cronString = `0 0 0/${value || 1} ? * *`;
      }
      if (schedule.every === EVERY_UNITS.minutes) {
        const value = schedule.everyValue > 59 ? 59 : schedule.everyValue;
  ...
  renderScheduleInput = (schedule, dataIndex, scheduleIdx) => {
    if (dataIndex === 'cron') {
      return (
        ...
        >
          <Select
            onChange={(value) => this.onChangeCronField('scheduleType', scheduleIdx, value)}
          ...
          {schedule.scheduleType === 'every' ? (
            <div style={{display: 'flex', flexWrap: 'nowrap', gap: 4, width: '100%'}}>
              <InputNumber
                ...
                onChange={(value) => this.onChangeCronField('everyValue', scheduleIdx, value)}
                size="small"
              />
              <Select
                onChange={(value) => this.onChangeCronField('every', scheduleIdx, value)}
            ...
            <Input
              ...
              style={{flex: 1}}
              onChange={(event) =>
                this.onChangeSchedule(dataIndex, scheduleIdx, event.target.value)
              }
    ...
    return (
      <Input
        size="small"
        onChange={(event) => this.onChangeSchedule(dataIndex, scheduleIdx, event.target.value)}
        value={schedule[dataIndex] || ''}
      />
    );
  };
    ...
    return (
      <tr key={scheduleIdx}>
        {columns.map(({dataIndex, key}, index) => (
          <td key={`${key}_${index}`}>
            {key === 'controls-column' ? (
              <Button onClick={() => this.deleteScheduleRow(scheduleIdx)} size="small" danger>
                <DeleteOutlined />
              </Button>
            ) : (
      ...
      <div className={className} style={{margin: '10px 0px'}}>
        ...
        <table className={styles.table}>
          ...
          <tbody>
            ...
            <tr>
              <td>
                <Button onClick={this.addScheduleRow} style={{width: '150px'}} size="small">
