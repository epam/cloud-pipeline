...
function reduceQuotasByPeriod(quotas = []) {
  return Object.values(QuotaPeriods)
    .map((period) => {
...
function groupQuotas(quotas = []) {
  const keyFn = (quota) =>
    [quota.quotaGroup || '', quota.type || '', quota.subject || ''].join('|');
...
class Quotas {
  ...
  getUsersQuotas(group) {
    const groups =
      group && group !== QuotaGroups.global
        ? [group]
  ...
  getUserQuotas(user, group = QuotaGroups.global) {
    return this.getUsersQuotas(group).filter((quota) => quota.subject === user);
  }
  ...
  getFilteredQuotas(options = {}) {
    ...
    const test = (allowed, property) => (quota) =>
      !allowed || allowed.length === 0 || allowed.includes(quota[property]);
    return this.list
...
class QuotaProvider extends React.Component {
  ...
  render() {
    ...
    return <Provider quotas={this.quotas}>{children}</Provider>;
  }
}
