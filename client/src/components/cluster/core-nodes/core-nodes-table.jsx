...
class CoreNodesTable extends localization.LocalizedReactComponent {
  ...
  get nodes() {
    ...
    if (clusterNodes.loaded) {
      return (clusterNodes.value || []).filter((node) => {
        const masterNode = Object.entries(node.labels).some(([label, value]) => {
          const {role = 0} = parseLabel(label, value);
          return testRole(role, nodeRoles.master) || testRole(role, nodeRoles.cloudPipelineRole);
        });
  ...
  get filteredNodes() {
    ...
    const matchesAddress = (node) =>
      node.addresses && node.addresses.map((a) => a.address).find((a) => a.includes(address));
