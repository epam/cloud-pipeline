...
import styles from './PipelineConfiguration.module.css';
...
class PipelineConfiguration extends React.Component {
  ...
  get selectedConfiguration() {
    if (this.props.configurations.loaded) {
      const [configuration] = this.props.configurations.value.filter(
    ...
    if (
      ...
      if (configuration) {
        const hide = message.loading(
          `Setting '${this.selectedConfigurationName}' configuration as default...`,
          0,
        );
        this.setState(
          {
  ...
  createConfigurationForm = async ({name, description, template}) => {
    ...
    const [configuration] = this.props.configurations.value
      .filter((c) => c.name.toLowerCase() === template.toLowerCase())
  ...
  onSaveConfiguration = (opts) => {
    ...
    if (
      ...
      if (
        this.props.configurations.value.filter(
          (c) =>
        ...
        this.setState(
          ...
          async () => {
            if (this.selectedConfigurationName !== configurationName) {
              const renameRequest = new PipelineConfigurationRename(
    ...
    return (
      <Row>
        <Tabs
          ...
          type="editable-card"
          items={(this.props.configurations.value || [])
            .slice()
            .sort((cA, cB) => {
              if (cA.name > cB.name) {
                return 1;
  ...
  render() {
    ...
    return (
      <div style={{display: 'flex', flex: 1, flexDirection: 'column', height: '100%'}}>
        {this.renderTabs()}
        <Row className="cp-tabs-content" style={{flex: 1, overflowY: 'auto', height: '100%'}}>
          <LaunchPipelineForm
            ...
            canRemove={
              ...
              this.props.configurations.value.length > 1
            }
