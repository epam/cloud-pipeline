...
export default class NotificationsPanel extends React.Component {
  ...
  get notifications() {
    ...
    const list =
      notifications && notifications.loaded ? (notifications.value || []).map((o) => o) : [];
  ...
  renderSeverityIcon = (notification) => {
    switch (notification.severity) {
      case 'INFO':
        return <InfoCircleOutlined className="cp-notification-status-info" />;
      case 'WARNING':
        return <ExclamationCircleOutlined className="cp-notification-status-warning" />;
      case 'CRITICAL':
        return <CloseCircleOutlined className="cp-notification-status-critical" />;
      default:
        return undefined;
    }
  };
  ...
  renderNotification = (notification) => {
    return (
      <Row type="flex" style={{padding: '5px 0 0 5px'}}>
        <Col className={styles.iconColumn}>{this.renderSeverityIcon(notification)}</Col>
        <Col style={{paddingLeft: 10, flex: 1, wordBreak: 'break-word'}}>
          <Row type="flex" style={{fontWeight: 'bold', fontSize: 'larger'}}>
            {notification.title}
          </Row>
          <Row type="flex">
            <Markdown md={notification.body} style={{margin: '10px 0', fontSize: 'smaller'}} />
          </Row>
          <Row type="flex" style={{fontSize: 'x-small'}} className="cp-text-not-important">
  ...
  render() {
    ...
    if (this.notifications.length === 0) {
      return <div className={styles.container}>There are no system notifications.</div>;
    }
    return (
      <div className={styles.container}>
        {this.notifications.map((notification, index) => {
          return (
            <Card key={index} styles={{body: {padding: 2}}} className="cp-panel-card">
