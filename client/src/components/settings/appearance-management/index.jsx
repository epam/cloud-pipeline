...
import {Alert, Button, Checkbox, Input, message, Modal, Radio} from 'antd';
...
import styles from './appearance-management.module.css';
...
class AppearanceManagement extends React.Component {
  ...
  renderThemesSelector = () => {
    ...
    return (
      <div className={classNames(styles.themes, styles.section)}>
        {themes.map((theme) => (
          <ThemeCard
  ...
  injectEditableThemeStyles = (delayed = true) => {
    const {themes: themesStore} = this.props;
    const {editableTheme, themePayload, liveUpdate} = this.state;
  ...
  onChangeTheme = (payload, valid) => {
    const {editableTheme} = this.state;
    if (editableTheme) {
      this.setState(
        {
  ...
  onRemove = () => {
    ...
    if (editableTheme && !editableTheme.isNew) {
      const {identifier, name} = editableTheme;
      const {themes: themesStore} = this.props;
      const themesPayload = this.themes.filter((o) => !o.predefined && o.identifier !== identifier);
  ...
  onSaveTheme = async () => {
    const {editableTheme, themePayload} = this.state;
    if (editableTheme && themePayload) {
      const {isNew, identifier, predefined} = editableTheme;
      ...
      this.save(themesPayload).then(hide);
    }
  };
            ...
            .then(() => {
              if (themesStore.mode === ThemesPreferenceModes.url) {
                Modal.info({
                  ...
                  content: (
                    <div>
                      You should copy configuration file to the <code>{themesStore.themesURL}</code>
                      .
                    </div>
                  ),
                });
  ...
  renderActions = () => {
    const {editableTheme, modified, valid, liveUpdate, pending} = this.state;
    if (editableTheme) {
      return (
        <div className={styles.actions}>
          <Checkbox checked={liveUpdate} onChange={this.toggleLiveUpdate} disabled={pending}>
            Live preview
          </Checkbox>
          <Button
            key="cancel"
  ...
  renderPreferenceDisclaimer = () => {
    const {themes: themesStore, preferences} = this.props;
    const {mode, url, pending} = this.state;
    ...
    return (
      <div className={classNames(styles.section, 'cp-divider', 'bottom')}>
        <div className={styles.section}>
          <b>{cloudPipelineAppName}</b> User Interface themes are configured using{' '}
          <code>{ThemesPreferenceName}</code> preference. This preference may contain an <b>URL</b>{' '}
          to the configuration file (recommended) or <b>serialized</b> themes (JSON object).
        </div>
        <div className={styles.section}>
          <div className={styles.section}>
            <Radio
            ...
            {currentMode === ThemesPreferenceModes.url && (
              <div style={{paddingLeft: 22}}>
                <div>
                  You should <a onClick={downloadFile}>download</a> configuration file and save it{' '}
                  <b>manually</b> by the given URL.
                </div>
                <div>
                  <Input
                    disabled={pending}
          ...
          <div className={styles.section}>
            ...
            <div style={{paddingLeft: 22}}>
              This will increase overall preferences size due to all images are stored as
              base64-encoded data.
            </div>
          </div>
        </div>
        {(currentMode !== themesStore.mode || url !== undefined) && (
          <div
            className={classNames(styles.section, styles.actions)}
            style={{justifyContent: 'flex-end'}}
          >
            <Button disabled={pending} className={styles.action} onClick={onCancel}>
              CANCEL
            </Button>
            <Button
  ...
  render() {
    const {router, preferences, authenticatedUserInfo} = this.props;
    const cloudPipelineAppName = preferences.deploymentName || 'Cloud Pipeline';
    const administrator =
    ...
    const {editableTheme, pending, editableThemePreviewClassName} = this.state;
    const goBack = () => {
      if (editableTheme) {
        this.onCancelEdit();
      } else {
        if (router) {
          router.push('/settings/profile/appearance');
        }
      }
    };
    if (authenticatedUserInfo && authenticatedUserInfo.pending && !authenticatedUserInfo.loaded) {
      return (
    ...
    if (authenticatedUserInfo && authenticatedUserInfo.loaded && !administrator) {
      return (
        <div className={styles.appearanceManagement}>
      ...
      <div className={styles.appearanceManagement}>
        <div className={classNames(styles.header, 'cp-divider', 'bottom')}>
          <div className={styles.title}>
            <Button className={styles.back} size="small" onClick={goBack}>
              <LeftOutlined />
            </Button>
