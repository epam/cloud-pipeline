...
import {getExternalJobInfo, isExternalJobIdentifier} from '../model/analysis/external-evaluations';
...
import styles from './cell-profiler.module.css';
...
function getResultsPath(job) {
  const {status, outputFolder: output, result} = job || {};
...
class CellProfilerJobResults extends React.PureComponent {
    ...
    if (jobId) {
      ...
      this.setState(pendingState, async () => {
        ...
        try {
          ...
          if (!this.unmounted) {
            this.setState(state, () => this.fetchJobSpecification());
          }
        }
      });
    } else {
  ...
  fetchJobSpecification = (force = false) => {
    const {specification, job} = this.state;
  ...
  renderJobTitle = () => {
    ...
    let details;
    const input = job.input && job.input.path ? job.input.path.split('/').pop() : undefined;
    if (job.alias && input) {
      name = job.alias;
      details = input;
    } else if (job.alias) {
  ...
  renderJobInfo = () => {
    const {job, specification} = this.state;
    if (!job) {
      return null;
    }
    const {startDate, endDate, owner, input, outputFolder} = job;
    const renderInfo = (key, value) => {
      if (value) {
        return (
          <div className={styles.cellProfilerJobInfoRow}>
            <b className={styles.key}>{key}:</b>
            <span className={classNames(styles.value, 'cp-ellipsis-text')}>{value}</span>
          </div>
        );
      }
      return null;
    ...
    const renderLocation = (key, value, folder = false) => {
      ...
      const {storageId, path} = value;
      const parentFolder = folder ? path : (path || '').split('/').slice(0, -1).join('/');
    ...
    const renderSpecificationInfo = () => {
      ...
      const {inputs} = specification;
      const {files = [], zPlanes = []} = parseInputs(inputs);
      if (files.length === 0) {
        return null;
      }
    ...
    return (
      <Collapse header="Details">
        ...
        {!job.other &&
          renderInfo(
            'Job ID',
            <Link to={`/run/${job.id}`} target="_blank">
              #{job.id}
            </Link>,
          )}
  ...
  renderZScore = () => {
    ...
    const {status, outputFolder: output, result, input} = job;
    if (!/^(success)$/i.test(status) || (!output && !result)) {
      return null;
    }
    const {path: inputPath, storageId: inputStorageId} = input || {};
    const {path, storageId} = getResultsPath(job);
    if (!path || !storageId) {
      return null;
    }
    return (
      <Collapse header="Z-score" unmountOnClose={false}>
  ...
  renderJobOutput = () => {
    ...
    const {status, outputFolder: output, result, input, analysisFile} = job;
    if (!/^(success)$/i.test(status) || (!output && !result)) {
      return null;
    }
    const fileName = input && input.path ? input.path.split('/').pop() : undefined;
    const {path, downloadPath, storageId} = getResultsPath(job);
    return (
  ...
  render() {
    const {className, style, jobId} = this.props;
    if (!jobId) {
      return null;
    }
    const {error, pending, job} = this.state;
    return (
      <div
