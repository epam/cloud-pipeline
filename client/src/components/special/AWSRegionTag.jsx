...
import styles from './AWSRegionTag.module.css';
    ...
    if (this.props.awsRegions.loaded) {
      return (
        (this.props.awsRegions.value || [])
          .filter((r) => r.provider)
          .filter((p, i, a) => a.indexOf(p) === i).length > 1
      );
    }
    return false;
  }
  ...
  render() {
    ...
    if (this.provider && this.shouldDisplayProvider) {
      if (this.props.plainMode) {
        ...
        parts.push(
          <span
            className={classNames(styles.provider, 'provider', this.provider.toLowerCase())}
            key="provider"
    ...
    if (this.props.displayFlag && info) {
      if (this.props.plainMode) {
        ...
        parts.push(
          <span
            className={classNames(styles.flag, 'flag', info.result, info.zone.toLowerCase())}
            key="flag"
    ...
    if (this.props.displayName && this.region) {
      if (this.props.plainMode) {
        ...
        parts.push(
          <span className={styles.title} key="name">
            {this.region.name}
          </span>,
        );
      }
