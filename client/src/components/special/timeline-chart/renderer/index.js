...
import {drawVisibleLabels, buildLineHelpers} from './draw-utilities';
...
function hoveredItemsEqual(a, b) {
  const {dataset: aDataset, item: aItem} = a;
  const {dataset: bDataset, item: bItem} = b;
  return aDataset === bDataset && aItem === bItem;
}

function hoveredItemsSetsAreEqual(aSet, bSet) {
  return (
    ((aSet || []).length === 0 && (bSet || []).length === 0) ||
...
class TimelineChartRenderer {
  ...
  constructor(options = {}) {
    ...
    this.xAxis = new RendererAxis((item) => item.x, {
      extendArea: 0.1,
      timeline: true,
      minimumRatio: 50.0, // 50 pixels for 1 second,
    });
    this.yAxis = new RendererAxis((item) => item.y, {
      reverted: true,
      extendArea: true,
      stickToZero: true,
      vertical: true,
    });
    this.xAxis.addScaleChangedListener((fromZoom, fromDrag) =>
      this.onScaleChange(true, fromZoom, fromDrag),
    );
  ...
  attachToContainer(container) {
    ...
    if (container) {
      ...
      const {verticalLine, horizontalLine} = buildLineHelpers(this.pathProgram, this.lineBuffer);
  ...
  removeEventListener(event, listener) {
    this._listeners = this._listeners.filter(
      (config) => config.event !== event || config.listener !== listener,
    );
  }
  ...
  adaptYAxisForVisibleElements() {
    if (this._adaptValueAxis) {
      ...
      const checkInterpolatedValue = (pointA, pointB, x) => {
        ...
        let {x: aX, y: aY} = pointA;
        let {x: bX, y: bY} = pointB;
        if (aX > bX) {
          const tmpX = aX;
          const tmpY = aY;
          aX = bX;
    ...
    if (this.hoverEvents) {
      if (!hoveredItemsSetsAreEqual(items, this._hoveredItems)) {
        ...
        if (items && items.length > 0) {
          const xValues = [...new Set(items.map((item) => item.item.x))].map((x) =>
            this.xAxis.getPixelForValue(x),
          );
          const yValues = [...new Set(items.map((item) => item.item.y))].map((y) =>
            this.yAxis.getPixelForValue(y),
          );
  ...
  getMousePosition(event) {
    ...
    const {x: canvasX, y: canvasY, width: canvasWidth, height: canvasHeight} = this.canvasBox;
    x = clientX - canvasX;
    y = clientY - canvasY;
    width = canvasWidth;
    height = canvasHeight;
  ...
  onMouseUp(event) {
    if (this.dragEvent && this.dragEvent.shiftKey && this.dragEvent.shiftEnd) {
      let {valueX: start, shiftEnd = {}} = this.dragEvent;
      let {valueX: end} = shiftEnd;
      if (start > end) {
        const tmp = end;
        end = start;
        start = tmp;
      ...
      this.unHoverItems();
      const {x, y} = this.dragEvent;
      const item = this.getItemsByMouseEvent(event)[0];
      if (item) {
        this.reportEvent(TimelineChartEvents.onItemClick, {
          item,
          x,
          y,
        });
    ...
    (this.datasets || []).forEach((dataset) => {
      ...
      for (let i = 0; i < data.length; i += 1) {
        ...
        if (item) {
          ...
          if (xDistance < SEARCH_AREA_PX && this.xAxis.valueFitsRange(item.x)) {
            candidates.push({
              item,
              dataset,
  ...
  getItemsByMouseEvent = (event) => {
    const {x, valueX, y, hit} = this.getMousePosition(event);
    const result = [];
    if (hit) {
  ...
  registerDatasets(datasets = []) {
    const processed = datasets.slice().map((dataset, index) => {
      const {data = [], key = index, ...rest} = dataset;
      return {
        ...rest,
        index,
        key,
        data: data
          .map((item) => {
            ...
            const {value, date, ...itemRest} = item;
            const {unix: x, date: dateValue} = parseDate(date) || {};
            return {
              ...itemRest,
              value,
              y: Number(value),
    ...
    for (const processedDataset of processed) {
      for (const item of processedDataset.data) {
    ...
    this.xAxis.update(this.datasets, {
  ...
  registerDatasetsOptions(datasetsOptions = []) {
    this.datasetsOptions = datasetsOptions.slice().map((datasetOptions, index) => {
      const {key = index, ...rest} = datasetOptions || {};
      return {
        ...rest,
        key,
        index,
  ...
  draw() {
    ...
    const {r, g, b} = parseColor(this.backgroundColor);
    gl.enable(gl.BLEND);
  ...
  drawLines(gl) {
    ...
    const {projection, pixelResolution} = drawingContext;

    usePathProgram(this.pathProgram, projection, IDENTITY_MATRIX, pixelResolution);
    (this.datasetBuffers || []).forEach((datasetBuffer) => {
      const {dataset, buffers} = datasetBuffer;
      const {width = DEFAULT_LINE_WIDTH, color = DEFAULT_CHART_LINE_COLOR} =
        this.getDatasetOptions(dataset);
      buffers.forEach((buffer) => {
        drawPath(this.pathProgram, buffer, {
          width,
          color,
        });
      });
  ...
  drawPoints(gl) {
    ...
    for (let d = 0; d < (this.datasetBuffers || []).length; d += 1) {
      const {dataset, buffers = []} = this.datasetBuffers[d];
      const datasetItemsToDraw = [];
      for (let i = 0; i < buffers.length; i += 1) {
        const {items = []} = buffers[i];
    ...
    if (total > 0 && total < maxItemsToDrawPoints) {
      pointsToDraw.forEach((datasetPoints) => {
        const {dataset, items} = datasetPoints;
        const {
  ...
  drawCoordinates(gl) {
    ...
    const pixelResolution = new Float32Array([2.0 / this.canvas.width, 2.0 / this.canvas.height]);
  ...
  drawSelection(gl) {
    if (!gl || !this.dragEvent || !this.dragEvent.shiftKey || !this.dragEvent.shiftEnd) {
      return;
    }
    const {valueX: start, shiftEnd = {}} = this.dragEvent;
    const {valueX: end} = shiftEnd;
    if (start === undefined || end === undefined) {
      return;
    }
    const {projection} = this.prepareChartContext(gl);
  ...
  drawText() {
    if (!this.textCanvas || (!this.xAxis.ticksChanged && !this.yAxis.ticksChanged)) {
      return;
    }
    ...
    context.save();
    drawVisibleLabels(this.xAxis.ticks || [], {
      context,
      axis: this.xAxis,
    ...
    drawVisibleLabels(this.yAxis.ticks || [], {
      context,
      axis: this.yAxis,
      getElementPosition: (tick) => ({
        y: this.yAxis.getPixelForValue(tick.value),
