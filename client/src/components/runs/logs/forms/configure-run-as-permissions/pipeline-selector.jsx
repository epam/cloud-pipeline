...
class PipelineSelector extends React.Component {
  ...
  get pipelines() {
    ...
    const isDisabled = (pipeline) => {
      return (
        pipelinesToExclude && pipelinesToExclude.length && pipelinesToExclude.includes(pipeline.id)
      );
    };
    if (pipelines.loaded) {
  ...
  render() {
    const {disabled, duplicate, className, pipelines, style, containerStyle, onRemove, pipelineId} =
      this.props;
    const {search} = this.state;
    if (pipelines.error) {
      return null;
