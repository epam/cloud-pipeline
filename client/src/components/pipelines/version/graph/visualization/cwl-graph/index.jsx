...
export default class CwlGraph extends Graph {
  ...
  initializeGraphEventListeners = (graph) => {
    ...
    graph.getPlugin(SelectionPlugin).registerOnSelectionChange((selectedNode) => {
      if (this.model && selectedNode && selectedNode.dataset && selectedNode.dataset.id) {
  ...
  onDrop = (event) => {
    ...
    if (data && typeof data === 'string' && /^docker:/i.test(data)) {
  ...
  addTool = async (docker, position) => {
    ...
    try {
      ...
      const registry = ((dockerRegistries.value || {}).registries || []).find(
        (aRegistry) => (aRegistry.path || '').toLowerCase() === registryName.toLowerCase(),
      );
      if (!registry) {
        throw new Error(`Registry ${registryName} not found`);
      }
      const group = (registry.groups || []).find(
        (aGroup) => aGroup.name.toLowerCase() === groupName.toLowerCase(),
      );
      if (!group) {
        throw new Error(`Group ${groupName} not found`);
      }
      const tool = (group.tools || []).find((aTool) => aTool.image.toLowerCase() === image);
      if (!tool) {
      ...
      const getToolVersion = (versionName = '') =>
        versions.find(
          (aVersion) => (aVersion.version || '').toLowerCase() === versionName.toLowerCase(),
        );
      const toolVersion = getToolVersion(version) || getToolVersion('latest') || versions[0];
      if (!toolVersion) {
        throw new Error('Tool version not found');
      }
      const {defaultCommand, instanceType: defaultInstanceType, disk: defaultDisk} = toolInfo.value;
      const {settings: toolVersionSettings = []} = toolVersion;
      const {configuration = {}} = toolVersionSettings[0] || {};
      const {
      ...
      const coords = position ? {'sbg:x': position.x, 'sbg:y': position.y} : {};
      const toolModel = {
        class: 'CommandLineTool',
        cwlVersion: 'v1.0',
        id: `id_${dayjs.utc().unix()}`,
        ...coords,
  ...
  renderGraph() {
    ...
    if (
      (parameters.pending && !parameters.loaded) ||
    ...
    return (
      <div className={styles.cwlGraph}>
        <div className={classNames(styles.cwlControls, 'cp-panel')}>
          <Button
            shape="circle"
            style={{zIndex: 1}}
            type="primary"
        ...
        />
        <div className={styles.cwlGraphContainer} onDragOver={this.onDragOver} onDrop={this.onDrop}>
          <svg ref={this.initializeGraph} id="cwl-workflow" className="cwl-workflow" />
        </div>
      </div>
    );
  }
