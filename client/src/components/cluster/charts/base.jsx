...
class Chart extends React.Component {
  ...
  UNSAFE_componentWillReceiveProps(nextProps, nextContext) {
    const {start, end} = this.state;
    if (nextProps.followCommonScale && (nextProps.start !== start || nextProps.end !== end)) {
  ...
  renderTitle = (height) => {
    ...
    return (
      <div className={styles.title} style={{height}}>
  ...
  renderZoomControls() {
    ...
    const zoomIn = () => {
      ...
      this.onChangeRange(center - newRange / 2.0, center + newRange / 2.0, true);
    };
    const zoomOut = () => {
      if (!this.canZoomOut) {
        return;
    ...
    return (
      <div className={styles.zoomControls}>
        <PlusCircleOutlined className={zoomInClassNames.join(' ')} onClick={zoomIn} />
        <MinusCircleOutlined className={zoomOutClassNames.join(' ')} onClick={zoomOut} />
      </div>
    );
  }
  ...
  render() {
    const {className, data, disableTooltips, width, height, rangeChangeEnabled} = this.props;
    if (!data) {
      return null;
    }
    return (
