...
import styles from './run-tabs.module.css';

function RunTabs(props) {
  const {className, style, tab: currentTab, onTabChange, tabs = [], run} = props;
  if (tabs.length < 2) {
    return null;
  }
  const onChange = ({key}) => (onTabChange ? onTabChange(key) : undefined);
  return (
    <div className={classNames(className, styles.runTabs, 'cp-run-details-tabs')} style={style}>
      <Menu
