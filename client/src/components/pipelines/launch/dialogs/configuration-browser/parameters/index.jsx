...
class Parameters extends React.Component {
  ...
  get parameters() {
    const {parametersStore} = this.props;
    if (!parametersStore) {
      return [];
    }
  ...
  get sections() {
    ...
    return [...new Set(filtered.filter((o) => !o.system).map((o) => o.section))].sort((a, b) => {
      if (!a) {
        return 1;
      }
      if (!b) {
  ...
  get displaySystemParameters() {
    const {mode = Parameters.Modes.nonSystem} = this.props;
    return mode === Parameters.Modes.system;
  }

  renderAddParameterButton = () => {
    const {disabled, editable, preferences, parametersStore} = this.props;
    if (!parametersStore) {
      return null;
    }
    const {systemParameterBrowserVisible} = this.state;
    ...
    if (this.displaySystemParameters) {
      ...
      button = (
        <Space.Compact>
          ...
          >
            <Button id="add-typed-parameter-button" disabled={disabled || !editable}>
              <DownOutlined />
            </Button>
          </Dropdown>
        </Space.Compact>
    ...
    return (
      <div className={styles.addParameterRow}>
        {button}
        {systemParametersBrowser}
      </div>
    );
  ...
  renderSectionHeader = (section, index, sections = []) => {
    const line = (
      <div className={classNames('cp-divider', 'horizontal', styles.line)}>{'\u00A0'}</div>
    );
    ...
    if (sections.filter((o) => !(o && o.system)).length <= 1) {
      return null;
    }
    return (
      <div className={styles.sectionHeader}>
  ...
  renderSection = (section, index, sections = []) => {
    const {larger, disabled, editable, mode, parametersStore} = this.props;
    if (!parametersStore) {
      return null;
    }
...
export default inject('preferences')(injectParametersStore(observer(Parameters)));
