...
import {BarchartDataLabelPlugin, ChartClickPlugin} from './extensions';
...
import {getAffectiveQuotaPeriods, getQuotaGroup} from './quotas';
...
function filterTopData(data, top, dataSample = 'value') {
  const sortedData = Object.keys(data || {}).map((key) => ({name: key, item: data[key]}));
  sortedData.sort((a, b) => +b.item[dataSample] - +a.item[dataSample]);
  if (top) {
...
function getQuotaDatasets(options = {}) {
  const {quotas, quotaType, dataSample, groups, requestsWithDiscounts = []} = options;
  if (!quotas || !quotaType) {
    return [];
  }
  return requestsWithDiscounts
    ...
    .map((quotaDatasetCollection) => {
      const datasets = [];
      const {quotas: quotasData, quotaGroup, value, quotaPeriods} = quotaDatasetCollection;
      ...
      if (!isEmptyData(mainDataset.data)) {
        ...
        const filteredQuotaDatasets = quotaDatasets.filter(
          (qDataset) => !isEmptyData(qDataset.data),
        );
        if (filteredQuotaDatasets.length > 0 && !isEmptyData(mainDataset.data)) {
          datasets.push(mainDataset, ...filteredQuotaDatasets);
        }
      }
      return datasets;
    })
...
function BarChartWithQuota({
  ...
  const data = discounts.applyGroupedDataDiscounts(value, discountsFn);
  ...
  const maximum = getMaximum(...previousData, ...currentData);
  ...
  const options = {
    ...
    hover: {
      onHover: function (e) {
        ...
        e.target.style.cursor = point.length && onSelect ? 'pointer' : 'default';
      },
    },
    plugins: {
  ...
  const Container = ({style: cssStyle, children}) => {
    if (useImageConsumer) {
      return (
        <Export.ImageConsumer style={cssStyle} order={2}>
          {children}
  ...
  return (
    <Container
      style={Object.assign({height: '100%', position: 'relative', display: 'block'}, style)}
    >
      <Chart
        data={chartData}
        error={error}
