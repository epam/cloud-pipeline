...
export function getIconForStatus(status, filled = false) {
  switch ((status || '').toUpperCase()) {
    case nextflowTaskStatusCompleted:
      return filled ? CheckCircleFilled : CheckCircleOutlined;
    case nextflowTaskStatusAborted:
      return filled ? CloseCircleFilled : CloseCircleOutlined;
    case nextflowTaskStatusFailed:
      return filled ? ExclamationCircleFilled : ExclamationCircleOutlined;
    case nextflowTaskStatusRunning:
      return filled ? PlayCircleFilled : PlayCircleOutlined;
    case nextflowTaskStatusSubmitted:
      return filled ? PlusCircleFilled : PlusCircleOutlined;
    case nextflowTaskStatusCached:
      return filled ? MinusCircleFilled : MinusCircleOutlined;
    case nextflowTaskStatusCreated:
    default:
...
export function getSortedTaskStatuses(statuses, includeAllStatuses = true) {
  return nextflowTaskStatusGroups
    .map((st) => ({
      status: st.statuses[0],
      count: statuses
        .filter((s) => st.statuses.includes(s.status))
...
function NextflowTaskStatus(props) {
  const {className, style, status, filled = true, showLabel = true} = props;
  ...
  return (
    <span
      className={classNames(className, styles.nfTaskStatusIcon, {
        'cp-tag': asTag,
        filled: tagFilled,
    ...
    >
      ...
      />
      {showLabel && <span style={{marginLeft: 3}}>{status.toUpperCase()}</span>}
    </span>
  );
}
