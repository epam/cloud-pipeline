...
import styles from './hcs-z-position-selector.module.css';
...
function buildZPositionsArray(max, zSize, zUnit) {
  ...
  const format = (o) => {
    ...
    return [`${rounded}${postfix}`, zUnit].filter(Boolean).join('');
  };
  return new Array(max).fill('').map((o, z) => ({
    z,
...
class HcsZPositionSlider extends React.Component {
  ...
  arrangeMarks = () => {
    ...
    const height =
      positions.length <= 20 ? MIN_SLIDER_HEIGHT : Math.max(spaceBelow - 40, MIN_SLIDER_HEIGHT);
  ...
  render() {
    const {positions, onChange} = this.props;
    const getZPositionObject = (z) => positions.find((item) => item.z === z);
    return (
      <div
        className={styles.sliderContainer}
        ref={(el) => {
          this.sliderContainerRef = el;
        }}
      >
        <div
          className={styles.sliderContainer}
        ...
        >
          <Slider
            ...
            style={{flex: 1}}
            tooltip={{formatter: (value) => getZPositionObject(value)?.title || ''}}
          />
        </div>
      </div>
    );
...
function HcsZPositionSelector(props) {
  ...
  const {depth = 1, physicalDepthSize = 1, depthUnit = ''} = image;
  ...
  const selection = rawSelection.length > 0 ? rawSelection : [0];
  ...
  const onChangeMode = () => {
    ...
    if (onChangeModeProp) {
      onChangeModeProp(toggler[mode]);
    }
  };
  const sorted = positions.slice().sort(zPositionSorter);
  const renderBadges = () => {
    return (
      <div className={styles.badgesContainer}>
        {sorted.map((position) => (
          <div
            key={`z-${position.z}`}
            className={classNames(styles.zItem, {
        ...
        {positions.length > selection.length && multiple && (
          <a className={styles.zItem} onClick={onSelectAll}>
            Select all
          </a>
        )}
      </div>
  ...
  const renderSlider = () => {
    return (
      <HcsZPositionSlider onChange={onChangeWrapper} selection={selection} positions={sorted} />
    );
  };
  ...
  return (
    <div className={classNames(className, styles.container)} style={style}>
      <div className={styles.titleRow}>
        <b className={styles.title}>Z-planes</b>
        {sliderModeAvailable && (
          <Checkbox onChange={onChangeMode} checked={mode === Z_SELECTOR_MODES.slider}>
            Slider view
          </Checkbox>
        )}
      </div>
      {selection.length > 1 && (
        <div>
          <Checkbox onChange={onChangeMerge} checked={mergeZPlanes}>
            Use planes projection
          </Checkbox>
        </div>
      )}
      {mode === Z_SELECTOR_MODES.slider && sliderModeAvailable ? renderSlider() : renderBadges()}
    </div>
  );
}
...
HcsZPositionSelector.propTypes = {
  ...
  selection: PropTypes.oneOfType([PropTypes.object, PropTypes.arrayOf(PropTypes.number)]),
  onChange: PropTypes.func,
