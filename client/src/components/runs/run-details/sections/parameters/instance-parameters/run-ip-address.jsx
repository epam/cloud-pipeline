...
const RunIpAddress = generateRunInstanceParameterValueComponent('nodeIP', {
  render: (nodeIp, run, routing) => {
    ...
    const isCapacityBlocksRun = (pipelineRunParameters || []).some((pr) =>
      [CP_CAP_REQUESTS_CPU, CP_CAP_REQUESTS_GPU, CP_CAP_REQUESTS_RAM].includes(pr.name),
    );
    const parts = [
    ...
    const urls = [];
    if (baseUrl) {
      const buildUrl = (queryStringParts = []) =>
        queryStringParts.length > 0 ? `${baseUrl}?${queryStringParts.join('&')}` : baseUrl;
    ...
    if (urls.length > 0) {
      return <Link to={urls[0].url}>{content}</Link>;
    }
    return content;
  },
});
