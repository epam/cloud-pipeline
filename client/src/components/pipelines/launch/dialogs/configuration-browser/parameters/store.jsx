...
import {
  ...
  parseParameters,
  isSystemParameterRestrictedByRole,
  validateParameters,
} from './utilities';
...
class ParametersStore {
  ...
  get systemParameters() {
    return this.visibleParameters.filter((parameter) => parameter.system);
  }
  ...
  async initialize(parameters = {}) {
    ...
    try {
      ...
      const parameterIsSkipped = (parameter) =>
        !parameter || skipped.has(parameter.name) || isCapability(parameter.name, preferences);
      this.capabilities = getEnabledCapabilities(parameters);
      this.parameters = parseParameters(parameters).map((o) => {
        ...
        return {
          ...
          restricted:
            !!system &&
            isSystemParameterRestrictedByRole(o.name, runDefaultParameters, userInfo.value),
        };
      });
      const limitMountsParameter = this.parameters.find((o) => o.limitMounts);
      this.limitMounts = limitMountsParameter ? limitMountsParameter.value : undefined;
    } catch (e) {
  ...
  getParameterEnumeration = (parameter) => {
    ...
    const enumeration = parameterUtilities.parseEnumeration(parameter);
    if (enumeration && typeof enumeration.filter === 'function') {
      return enumeration.filter((o) => o.isVisible(builtParameters)).map((o) => o.name);
    }
    return undefined;
  };
