...
public class RunStatusConverter extends AbstractFilterValueConverter {
    @Override
    public Object convert(String field,
        ...
        String statusValue = this.clearQuotes(value);
        if (!StringUtils.isNullOrEmpty(statusValue)) {
            TaskStatus status = TaskStatus.getByName(statusValue.toUpperCase());
            if (status != null) {
                return status.getId();
            }
        }
        throw new WrongFilterException(String.format("Wrong status name: %s", value));
    }
}
