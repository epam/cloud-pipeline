...
import styles from './hcs-cell-selector.module.css';
...
export function colorToVec4(aColor) {
  const {r = 255, g = 255, b = 255, a = 1} = parseColor(aColor || 'white') || {};
...
function ElementHint({element}) {
  ...
  const {tags = {}, info = {}, x, y} = element;
  const infos = [
    `${getWellRowName(y)}${x + 1}`,
    ...Object.entries(tags).map(([key, values]) => `${key}: ${values.join(', ')}`),
...
class HcsCellSelector extends React.Component {
  ...
  updateColors = () => {
    ...
    if (themes && themes.currentThemeConfiguration) {
      backgroundColor =
        themes.currentThemeConfiguration['--cp-color-bg-elevated'] || backgroundColor;
      ...
      primaryHoverColor =
        themes.currentThemeConfiguration['--cp-color-primary-hover'] || primaryHoverColor;
      selectedColor = themes.currentThemeConfiguration['--cp-color-warning'] || selectedColor;
      selectedHoverColor =
        themes.currentThemeConfiguration['--cp-color-sensitive'] || selectedHoverColor;
    }
  ...
  get centerRestrictions() {
    ...
    const getRestrictions = (c, delta) => {
      ...
      return [c - Math.max(0, delta / 2.0), c + Math.max(0, delta / 2.0)];
    };
  ...
  get offsets() {
    const {showRulers} = this.props;
    const common = Math.max(20, FONT_SIZE_PX * 2, this.maxRowLabelSize + 4);
    return {
  ...
  set scrollBarHovered(value) {
    const {vertical = false, horizontal = false} = value || {};
    if (
  ...
  get elementsMode() {
    const {gridMode: meshMode = MESH_MODES.CIRCLES} = this.props;
  ...
  initializeViewport = () => {
    const {width = 1, height = 1, cells = []} = this.props;
    this.width = width;
    this.height = height;
    this.elements = cells.map((aCell) => {
      const {x, y, fieldWidth: aCellWidth = 1, fieldHeight: aCellHeight = 1, ...rest} = aCell;
      return {
        ...rest,
        x,
        y,
  ...
  buildDefaultScale = () => {
    if (this.textCtx) {
      ...
      this.maxRowLabelSize =
        this.textCtx.measureText(getWellRowName(this.height).replace(/./g, 'W')).width + 4;
      this.maxColumnLabelSize =
        this.textCtx.measureText(`${this.width}`.replace(/./g, '0')).width + 4;
    }
    if (this.ctx) {
  ...
  pxPointToUnitPoint = (point, round = false) => {
    ...
    const {x: xx, y: yy} = this.pxSizeToUnitSize({x: x - pxCenter.x, y: y - pxCenter.y});
  ...
  unitPointToPxPoint = (point) => {
    ...
    const {x: xx, y: yy} = this.unitSizeToPxSize({x: x - this.center.x, y: y - this.center.y});
    return {
      x: pxCenter.x + xx,
      y: pxCenter.y + yy,
    };
  ...
  getHoveredElementPointStyle = () => {
    if (this.hoveredElement) {
      const {x: left, y: top} = this.unitPointToPxPoint({
  ...
  getElementsByPosition = (unitPosition, options = {}) => {
    ...
    const {unitError = 2.0 / this.unitScale} = options;
    const filterElement = (element) => {
      const {x, y, width = 1, height = 1} = element;
      const x1 = x - unitError;
      const y1 = y - unitError;
      const x2 = x + width + unitError;
      const y2 = y + height + unitError;
      return (
        x1 <= unitPosition.x && x2 >= unitPosition.x && y1 <= unitPosition.y && y2 >= unitPosition.y
      );
    };
  ...
  getElementsWithinArea = (unitsStart, unitsEnd) => {
    ...
    const elementWithinArea = (element) => {
      ...
      return maxX - minX <= width + (x2 - x1) && maxY - minY <= height + (y2 - y1);
    };
  ...
  getScrollBars = () => {
    const getScrollBar = (options = {}) => {
      const {unitSize, totalPxSize, position} = options;
  ...
  getScrollBarUnderPosition = (point) => {
    const {x, y} = point;
    const {vertical, horizontal} = this.getScrollBars();
    if (vertical) {
      const {size, center} = vertical;
  ...
  onWheel = (event) => {
    ...
    if (pxHeight > this.drawingArea.height || pxWidth > this.drawingArea.width) {
  ...
  onMouseDown = (event) => {
    ...
    const {vertical, horizontal} = this.getScrollBarUnderPosition({x: offsetX, y: offsetY});
  ...
  onMouseMove = (event) => {
    if (
      this.mouseEvent &&
      (this.mouseEvent.verticalScrollBar || this.mouseEvent.horizontalScrollBar)
    ) {
      const {ratio: yRatio = 0} = this.mouseEvent.verticalScrollBar || {};
      const {ratio: xRatio = 0} = this.mouseEvent.horizontalScrollBar || {};
    ...
    if (event.target === this.canvas) {
      // hover
      const {offsetX: x, offsetY: y} = event;
      const {vertical, horizontal} = this.getScrollBarUnderPosition({x, y});
      this.scrollBarHovered = {
        vertical,
        horizontal,
      };
  ...
  onMouseUp = () => {
    ...
    if (
      ...
      const {offsetX: x, offsetY: y} = this.mouseEvent;
      ...
      if (!this.mouseEvent.shift && click) {
        ...
        if (onChange) {
          onChange(selected.filter((o) => o.id !== click.id));
        }
      } else if (this.mouseEvent.shift && click) {
        if (onChange) {
          onChange([...selected, click.cell]);
        }
      } else {
        if (onChange) {
          onChange([]);
      ...
      if (onChange) {
        if (this.mouseEvent.shift) {
          ...
          onChange(this.mouseEvent.elements.map((o) => o.cell));
        }
      }
    }
    this.hoveredElement = undefined;
  ...
  setUnitsDrawingUniforms = () => {
    ...
    gl.uniformMatrix4fv(
      this.defaultGlProgram.projection,
      false,
      mat4scale([2.0 / this.totalWidth, 2.0 / this.totalHeight, 1]),
    );
    gl.uniformMatrix4fv(
      this.defaultGlProgram.viewScale,
      false,
      mat4scale([this.unitScale, -this.unitScale, 1]),
    );
    gl.uniformMatrix4fv(
      this.defaultGlProgram.viewTranslate,
      false,
      mat4translate(diffPx.x, diffPx.y, 0),
    );
  };
  ...
  setViewUniforms = (x, y, width, height) => {
    ...
    gl.uniformMatrix4fv(this.defaultGlProgram.viewScale, false, mat4scale([width, height, 1]));
    gl.uniformMatrix4fv(
      this.defaultGlProgram.viewTranslate,
      false,
      mat4translate(-this.totalWidth / 2.0 + x, this.totalHeight / 2.0 - y, 0),
    );
  };
  ...
  drawMesh = () => {
    ...
    if (gl && this.buffers && this.defaultGlProgram && mode !== MESH_MODES.NONE) {
      ...
      const horizontalLines = getLinesToDraw(this.height, {
        ...options,
      ...
      const verticalLines = getLinesToDraw(this.width, {
        ...options,
      ...
      if (mode === MESH_MODES.LINES) {
        gl.uniformMatrix4fv(this.defaultGlProgram.modelScale, false, mat4scale([this.width, 0, 1]));
  ...
  drawElements = () => {
    ...
    const {getColorConfigurationForCell} = this.props;
    const gl = this.ctx;
    const mode = this.elementsMode;
    let currentColor;
    let currentBuffer;
    ...
    const getScaleMatrixForElement = (element) => {
      const {width = 1.0, height = 1.0} = element;
      ...
      if (!scaleMatrices.has(key)) {
        if (mode === MESH_MODES.CIRCLES) {
          ...
          scaleMatrices.set(key, mat4scale([element.width || 1, element.height || 1, 1]));
        }
      }
      return scaleMatrices.get(key);
    };
    const renderElement = (element, options = {}) => {
      const {fill, stroke, thickStroke} = options;
      const {x = 0, y = 0, width = 1.0, height = 1.0} = element;
      ...
      if (mode === MESH_MODES.CIRCLES) {
        ...
        gl.uniformMatrix4fv(
          this.defaultGlProgram.modelTranslate,
          false,
          mat4translate(-this.center.x + x + width / 2.0, -this.center.y + y + height / 2.0, 0),
        );
      } else {
        gl.uniformMatrix4fv(this.defaultGlProgram.modelScale, false, scaleMatrix);
        gl.uniformMatrix4fv(
          this.defaultGlProgram.modelTranslate,
          false,
          mat4translate(-this.center.x + x, -this.center.y + y, 0),
        );
      }
      if (fill) {
        prepareBuffers({
    ...
    if (getColorConfigurationForCell) {
      this.elements.forEach((element) => {
        ...
        renderElement(
          element,
          getColorConfigurationForCell(element, {
            hovered,
            selected,
      ...
      if (this.hoveredElement) {
        renderElement(this.hoveredElement, {
  ...
  renderRect = (x, y, width, height, options = {}) => {
    if (!this.ctx || !this.defaultGlProgram || !this.buffers) {
      return;
    }
    if (width < 0 || height < 0) {
      return;
  ...
  drawSelection = () => {
    if (this.mouseEvent && this.mouseEvent.startUnits && this.mouseEvent.endUnits) {
      const {x: x1, y: y1} = this.unitPointToPxPoint(this.mouseEvent.startUnits);
      const {x: x2, y: y2} = this.unitPointToPxPoint(this.mouseEvent.endUnits);
      this.renderRect(Math.min(x1, x2), Math.min(y1, y2), Math.abs(x2 - x1), Math.abs(y2 - y1), {
  ...
  drawWell = () => {
    ...
    this.renderCircle(center.x, center.y, 2.0, {
      fill: this.defaultColor,
    });
    this.renderCircle(center.x, center.y, radius * this.unitScale, {
      stroke: this.defaultColor,
    });
  };

  drawRulers = () => {
    this.renderRect(0, 0, this.totalWidth, this.offsets.top - 1, {fill: this.backgroundColor});
    this.renderRect(
      0,
    ...
    this.renderRect(0, 0, this.offsets.left - 1, this.totalHeight, {fill: this.backgroundColor});
    this.renderRect(
    ...
    for (let i = 0; i < this.width; i++) {
      ...
      if (x > previousVisiblePosition) {
        ...
        this.textCtx.fillText(`${i + 1}`, p.x * dpr, (this.offsets.top * dpr) / 2.0);
      }
    }
    ...
    for (let i = 0; i < this.height; i++) {
      ...
      if (y > previousVisiblePosition) {
        ...
        this.textCtx.fillText(getWellRowName(i), (this.offsets.left * dpr) / 2.0, p.y * dpr);
      }
    }
  };
  ...
  draw = () => {
    this.renderRAF = requestAnimationFrame(this.draw);
    if (!this.ctx || !this.needRender || !this.defaultGlProgram || !this.width || !this.height) {
      return;
    }
      ...
      <div className={styles.tags}>
        <Select
          ...
          filterOption={(input, option) => {
            return (
              (option.props.tagValue || '').toLowerCase().indexOf((input || '').toLowerCase()) >= 0
            );
          }}
        >
          {tags.map((tag) => (
            <Select.OptGroup key={tag.name} label={tag.name}>
              {tag.values.map((value) => (
                <Select.Option key={value} value={buildTagValue(tag.name, value)} tagValue={value}>
                  {value}
                </Select.Option>
              ))}
            </Select.OptGroup>
  ...
  render() {
    const {className, style, title, showElementHint} = this.props;
    return (
      <div className={classNames(className, styles.container)} style={style}>
        <div className={styles.header}>
          <span className={styles.title}>{title}</span>
          <div className={styles.zoomControls}>
            {this.fitScale && this.fitCenter && (
              <ShrinkOutlined
                className={classNames('cp-hcs-zoom-button', styles.zoomControlBtn)}
                onClick={this.fit}
              />
            )}
            <MinusCircleOutlined
        ...
        <div className={styles.canvasContainer} ref={this.initializeContainer}>
          <canvas ref={this.canvasInitializer} style={{position: 'absolute'}} />
          {this.hoveredElement && showElementHint && (
            ...
            >
              <div className={styles.tooltipPoint} style={this.getHoveredElementPointStyle()} />
            </Tooltip>
          )}
          <canvas
...
function sizeCorrection(size, cells = [], selector = () => 0) {
  return Math.max(size || 0, ...(cells || []).map(selector).map((o) => Math.ceil(o)));
}
HcsCellSelector.widthCorrection = (width, cells) => sizeCorrection(width, cells, (cell) => cell.x);
