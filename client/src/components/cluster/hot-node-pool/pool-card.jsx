...
import styles from './pool-card.module.css';
...
function displayTime(time) {
  ...
  const localTime = dayjs.utc(time, 'HH:mm:ss').toDate();
  return dayjs(localTime).format('HH:mm');
}
...
function scheduleEntryString(scheduleEntry) {
  ...
  const {from, fromTime, to, toTime} = scheduleEntry;
    ...
    <div>
      {entries.map((entry, index) => (
        <div key={index} className={styles.scheduleRow}>
          <span className={classNames(styles.schedule, 'cp-text-not-important')}>{entry}</span>
        </div>
      ))}
    </div>
  );
...
function PoolCard({
  ...
  const {[CP_USE_NODES_COUNT_INFORMATION]: cpUseNodesCountInformationLabelValue} = kubeLabels;
  const {value: cpUseNodesCountInformationLabel = true} =
    cpUseNodesCountInformationLabelValue || {};
  const cpUseNodesCountInformation =
    String(cpUseNodesCountInformationLabel).toLowerCase() === 'true';
  const poolNodes = (nodes || []).filter(
    (node) =>
      node.labels &&
      Object.hasOwn(node.labels, 'pool_id') &&
  ...
  return (
    <div
      className={classNames(styles.container, 'cp-panel-card', 'cp-hot-node-pool', {
    ...
    >
      <div className={styles.headerContainer}>
        <div
          className={classNames(styles.infoBlock, 'cp-text-not-important', {
            'cp-success': runs > 0,
          })}
        ...
        >
          ...
          {cpUseNodesCountInformation && <span>{runsCountLabel}</span>}
          {cpUseNodesCountInformation && (
            <span style={{fontWeight: 'normal', margin: '0 2px'}}>/</span>
          )}
        ...
        <div className={styles.nameBlock}>
          <div className={styles.header}>
            <div className={styles.title}>
              ...
              {nodeCount === 0 && <span className={styles.disabledLabel}>(disabled)</span>}
            </div>
            <div className={styles.actions}>
              {!readOnly && (
                <Button disabled={disabled} size="small" onClick={onEdit}>
                  <EditOutlined />
                </Button>
              )}
              <Button
              ...
              {!readOnly && (
                <Button disabled={disabled} size="small" danger onClick={onRemove}>
                  <DeleteOutlined />
                </Button>
              )}
            </div>
      ...
      <div className={styles.images}>
        {dockerImages.map((d, index) => (
          <DockerImageDetails key={index} className={styles.image} docker={d} onlyImage />
        ))}
      </div>
