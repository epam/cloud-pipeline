...
public abstract class AbstractDataStorageMapper {
    ...
    public void fillS3Fields(AbstractDataStorage dataStorage, @MappingTarget DataStorageVO dataStorageVO) {
        if (dataStorage.getType() == DataStorageType.S3 || dataStorage.getType() == DataStorageType.AZ) {
            dataStorageVO.setServiceType(StorageServiceType.OBJECT_STORAGE);
        } else if (dataStorage.getType() == DataStorageType.NFS) {
            dataStorageVO.setServiceType(StorageServiceType.FILE_SHARE);
        }
