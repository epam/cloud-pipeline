...
export default function generateProvider(options) {
  ...
  const RunOperationCheckProvider = class extends React.PureComponent {
    ...
    render() {
      ...
      return <Provider {...{[storeName]: this.storeObj}}>{children}</Provider>;
    }
  };
  ...
  function getCheckInfo(props) {
    const {[storeName]: storeObj} = props || {};
    return storeObj || {};
  }

  function getCheckResult(props) {
    const {pending, checkResult = {}} = getCheckInfo(props);
    const {result} = checkResult;
    return pending ? false : result;
  }

  function Warning(props) {
    const {className, style, type = warningDefaultType, showIcon} = props;
    const {pending, checkResult = {}} = getCheckInfo(props);
    const {result, message: checkResultMessage = []} = checkResult;
