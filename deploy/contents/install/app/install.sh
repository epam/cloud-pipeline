...
if is_service_requested cp-docker-registry; then
    ...
    if is_install_requested; then
        ...
        if [ -z "${CP_DOCKER_MANIFEST_PATH}" ]; then
          docker_manifest_import_users_from_point_in_time_configuration=$(is_module_available_in_point_in_time_configuration tools)
          if [ "${docker_manifest_import_users_from_point_in_time_configuration}" ]; then
             CP_DOCKER_MANIFEST_PATH=$(dirname "$docker_manifest_import_users_from_point_in_time_configuration")
          else
