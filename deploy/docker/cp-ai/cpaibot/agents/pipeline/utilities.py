    ...
    def pick_elements_from_batch(items: list[T]) -> list[T]:
        ...
        try:
            ...
            prompt = (f'Based on the user query and the available {element_name}s list, '
                      ...
                      f'User query:\n'
                      f'{query}\n\n'
                      f'Return ONLY a JSON array of matching {element_name} identifiers (id values). '
                      f'Do not include any explanations, markdown formatting, or additional text. '
                      f'Output format: [1, 2, 3] or [] if no matches found.')
