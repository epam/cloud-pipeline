...
export default class History extends React.Component {
  ...
  renderGenerateDockerfileModal = () => {
    const {generateDockerfileModalVisible, dockerFileFrom} = this.state;
    return (
      <Modal
        ...
        footer={
          <div className={styles.generateDockerfileFooter}>
            <Button onClick={this.closeGenerateDockerfileModal}>CANCEL</Button>
            <Button
              type="primary"
          ...
          <Alert
            ...
            title={
              <div style={{textAlign: 'justify'}}>
                Please specify a docker image name, which is going to be used as a base layer to
                generate a Dockerfile. This name will be used in the "FROM" instruction.
              </div>
            }
          />
        </div>
        ...
        <div style={{display: 'flex', flexDirection: 'row', flex: 1}}>
          <div style={{flex: 1, display: 'flex', flexDirection: 'column', overflow: 'auto'}}>
            {(this.props.history.value || []).map((layer, index) => (
              <div
                key={`layer-${index}`}
                className={classNames(styles.layer, 'cp-settings-sidebar-element', {
          ...
          <div style={{flex: 1, paddingLeft: 5}}>
            {selectedLayer && (
              <BashCode className={styles.code} code={(selectedLayer.command || '').trim()} />
            )}
          </div>
        </div>
      </div>
