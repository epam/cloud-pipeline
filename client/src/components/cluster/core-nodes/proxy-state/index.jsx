...
class ProxyState extends React.Component {
  ...
  fetchData = (immediately = false) => {
    ...
    const fetchDataCallback = () => {
      this.setState({pending: true}, async () => {
        const _mapHistogramType = async (histogramType) => {
          const request = new ClusterNetworkUsageFilter(
            histogramType,
            ensureDayjs(from).format('YYYY-MM-DD HH:mm:ss'),
            ensureDayjs(to).format('YYYY-MM-DD HH:mm:ss'),
          );
          await request.send({
        ...
        const requests = [HISTOGRAM_TYPES.time, HISTOGRAM_TYPES.run, HISTOGRAM_TYPES.resource].map(
          _mapHistogramType,
        );
  ...
  onEntryClick = (chartEntry, dataEntry, scaleHovered) => {
    ...
    if (dataEntry.type === HISTOGRAM_TYPES.resource) {
      if (scaleHovered) {
        return copyTextToClipboard(chartEntry)
          .then(() => {
            message.info(
              <span>
  ...
  render() {
    ...
    const extractData = (dataEntry) => {
      ...
      const sorted =
        dataEntry.type === HISTOGRAM_TYPES.time
          ? value
          : [...value].sort((a, b) => b.count - a.count).slice(0, TOP_ITEMS_COUNT);
    ...
    return (
      ...
      >
        ...
        />
        {(data ?? []).map((dataEntry) => {
          const dynamicHeight = Math.max((dataEntry.value ?? []).length * 30 + 50, 200);
          return (
            <ProxyStateChart
              type={dataEntry.type}
              key={dataEntry.type}
              ...
              style={{
                ...
                maxHeight: '50%',
                height:
                  dataEntry.type === HISTOGRAM_TYPES.resource ? `${dynamicHeight}px` : '300px',
                marginBottom: 10,
              }}
