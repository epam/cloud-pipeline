...
import styles from './upload-files-list.module.css';
...
class UploadFileStatus extends React.PureComponent {
  ...
  render() {
    ...
    const {done = false, indeterminate = true, progress = 0.0, error = undefined} = status || {};
    const statusIcon = (() => {
      if (error) {
    ...
    return (
      <div className={classNames(className, styles.uploadFile)} style={style}>
        ...
        {error && <div className={classNames(styles.uploadFileError, 'cp-error')}>{error}</div>}
      </div>
    );
  }
}
