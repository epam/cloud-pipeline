...
const Wrapper = ({className, style, children, showAsAlert}) => {
  if (showAsAlert) {
    return <Alert title={children} type="info" showIcon className={className} style={style} />;
  }
  return (
...
class SelectionInfo extends React.Component {
  ...
  renderInfo = () => {
    ...
    if (info) {
      ...
      if (typeof onOpenEvaluations === 'function') {
        evaluations = <a onClick={() => onOpenEvaluations()}>Evaluations</a>;
      }
      return (
        <div style={{marginTop: 5}}>
          Analysis will be submitted as a <b>batch job</b> that can be found at {evaluations} tab.
          <br />
  ...
  render() {
    const {className, style, analysis, showOnlyIfMultipleSelection, showAsAlert} = this.props;
    ...
    return (
      <Wrapper
        className={classNames(className, styles.block, styles.analysisSelectionInfo)}
        style={style}
...
export {hcsAnalysisInjected as HcsAnalysisSelectionInfo};
