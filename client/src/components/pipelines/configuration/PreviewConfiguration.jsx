...
import styles from './PreviewConfiguration.module.css';
...
export default class PreviewConfiguration extends Component {
  ...
  get currentCloudProvider() {
    if (
      ...
      const [provider] = (this.props.cloudProviders.value || []).filter(
        (p) => p.id === this.selectedEntry.configuration.cloudProviderId,
      );
      return provider;
    }
    return null;
  ...
  renderExecEnvironmentSection = () => {
    ...
    if (
      !this.selectedEntry ||
      this.props.onDemandInstanceTypes.pending ||
      this.props.spotInstanceTypes.pending ||
    ...
    res.push(
      <tr key={'pipeline_key'} className={styles.keyRow}>
        <td id={'key-column-pipeline'} colSpan={6} className={styles.keyCell}>
          Pipeline
        </td>
      </tr>,
    ...
    res.push(
      <tr key={'docker_image_key'} className={styles.keyRow}>
        <td id={'key-column-docker_image'} colSpan={6} className={styles.keyCell}>
          Docker image
        </td>
      </tr>,
    ...
    const instance = this.selectedEntry.configuration.instance_size;
    const [instanceType] = (
      this.selectedEntry.configuration.is_spot
        ? this.props.spotInstanceTypes.value
        : this.props.onDemandInstanceTypes.value
    ).filter((i) => i.name === instance);
    res.push(
      <tr key={'instance_size_key'} className={styles.keyRow}>
        <td id={'key-column-instance_size'} colSpan={6} className={styles.keyCell}>
          Instance
        </td>
      </tr>,
    ...
    res.push(
      <tr key={'instance_disk_key'} className={styles.keyRow}>
        <td id={'key-column-instance_disk'} colSpan={6} className={styles.keyCell}>
          Disk
        </td>
      </tr>,
    ...
    if (this.selectedEntry.configuration.node_count) {
      res.push(
        <tr key={'node_count_key'} className={styles.keyRow}>
          <td id={'key-column-node_count'} colSpan={6} className={styles.keyCell}>
            Run type
          </td>
        </tr>,
  ...
  renderAdvancedSection = () => {
    ...
    res.push(
      <tr key={'is_spot_key'} className={styles.keyRow}>
        <td id={'key-column-is_spot'} colSpan={6} className={styles.keyCell}>
          Price type
        </td>
      </tr>,
    ...
    res.push(
      <tr key={'timeout_key'} className={styles.keyRow}>
        <td id={'key-column-timeout'} colSpan={6} className={styles.keyCell}>
          Timeout
        </td>
      </tr>,
    ...
    res.push(
      <tr key={'cmd_template_key'} className={styles.keyRow}>
        <td id={'key-column-cmd_template'} colSpan={6} className={styles.keyCell}>
          Cmd template
        </td>
      </tr>,
  ...
  isSystemParameter = (parameter) => {
    if (this.props.runDefaultParameters.loaded) {
      return (
        (this.props.runDefaultParameters.value || []).filter((p) => p.name === parameter.name)
          .length > 0
      );
    }
    return false;
  ...
  buildDefaultParameters = (system = false) => {
    ...
    if (this.selectedEntry.configuration.parameters) {
      for (const key in this.selectedEntry.configuration.parameters) {
        if (Object.hasOwn(this.selectedEntry.configuration.parameters, key)) {
          ...
          if (
            ...
            if (this.prevParameters && this.prevParameters.params) {
              for (const paramKey in this.prevParameters.params) {
    ...
    (parameters.keys || []).forEach((key) => {
      ...
      res.push(
        <tr key={`${parameter.key}_key`} className={styles.keyRow}>
          <td id={`key-column-${parameter.key}`} colSpan={6} className={styles.keyCell}>
            {`${parameter.name}`}
          </td>
        </tr>,
  ...
  renderRootEntity = () => {
    ...
    return [
      <tr key={'root_entity_key'} className={styles.keyRow}>
        <td id={'key-column-root_entity'} colSpan={6} className={styles.keyCell}>
          Root entity
        </td>
      </tr>,
  ...
  renderConfigurationPreview = () => {
    ...
    return [
      ...
      <div key="body" style={{width: '100%', overflowY: 'auto'}}>
        {this.selectedEntry ? this.renderConfigurationEntryItem() : this.renderEmptyPlaceholder()}
      </div>,
    ];
  };
  ...
  initialize = () => {
    if (!this.props.configuration.pending && this.props.configuration.loaded) {
      ...
      if (
        ...
        this.selectedEntry &&
        this.selectedEntry.rootEntityId
      ) {
        [this.selectedRootEntity] = this.props.entitiesTypes.value.filter(
          (e) => e.id === this.selectedEntry.rootEntityId,
        );
      }
  ...
  render() {
    if (
      this.props.configuration.pending ||
      this.props.onDemandInstanceTypes.pending ||
      this.props.spotInstanceTypes.pending ||
      this.props.entitiesTypes.pending ||
