...
import {parseDate, drawVisibleLabels, percentToHexAlpha, correctPixels} from './utils';
...
class Renderer {
  ...
  constructor(options = {}) {
    ...
    this.xAxis.addScaleChangedListener((fromZoom, fromDrag) =>
      this.onScaleChange(fromZoom, fromDrag),
    );
  ...
  registerDatasets(datasets) {
    ...
    const processed = flatData.slice().map((dataset, index) => {
      const {key = index, records, ...rest} = dataset;
      return {
        ...rest,
        index,
        key,
        data: (dataset.records || [])
          .map((item) => {
            ...
            const {value, start, end, ...itemRest} = item;
            const {unix: xStart, date: dateValueStart} = parseDate(start) || {};
            const {unix: xEnd, date: dateValueEnd} = parseDate(end) || {};
            return {
              ...itemRest,
              xStart,
              xEnd,
          ...
          .filter(
            (item) => item === undefined || (item.xStart !== undefined && item.xEnd !== undefined),
          ),
      };
    });
    let minimum = Infinity;
    for (const processedDataset of processed) {
      for (const data of processedDataset.data) {
  ...
  hoverItems(hoverInfo) {
    ...
    const {gpuId, key, recordIdx} = info || {};
    const {x, y} = position;
    if (this.hoverEvents) {
      if (!hoverInfo) {
  ...
  getHoverInfo = (event) => {
    this.canvasBox = this.canvas.getBoundingClientRect();
    const {valueX, x, y, hit} = this.getMousePosition(event);
  ...
  onMouseUp(event) {
    if (this.dragEvent && this.dragEvent.shiftKey && this.dragEvent.shiftEnd) {
      ...
      this.xAxis.setRange({from: start, to: end}, {animate: true, fromZoom: true});
  ...
  drawGridLines(ctx) {
    ...
    ctx.beginPath();
    ctx.moveTo(correctPixels(this.yLabelsPadding + 8), correctPixels(this.dataHeight));
  ...
  drawSelection() {
    if (this.dragEvent && this.dragEvent.shiftKey && this.dragEvent.shiftEnd) {
      ...
      const x = delta > 0 ? this.dragEvent.x : this.dragEvent.shiftEnd.x;
      const start = Math.max(x, this.yLabelsPadding + 8);
      const offset = Math.min(0, this.dragEvent.shiftEnd.x - this.yLabelsPadding - 8);
      ctx.beginPath();
      ctx.fillRect(
        correctPixels(start),
        0,
