...
@inject((stores, params) => {
  return {
    issues:
      params.entityId && params.entityClass
...
export default class Issues extends localization.LocalizedReactComponent {
  ...
  issuesColumns = [
    {
      ...
      render: (name, issue) => {
        ...
        if (issue.author && issue.createdDate) {
          descriptions.push(
            <Row key="opened by" className={styles.openedByRow}>
              Opened {fromNowUtc(issue.createdDate)} by <b>{issue.author}</b>
            </Row>,
          );
          ...
          descriptions.push(
            <Row key="opened by" className={styles.openedByRow}>
              Opened {fromNowUtc(issue.createdDate)}
            </Row>,
          );
        }
        return (
  ...
  createIssue = async (values) => {
    ...
    if (request.error) {
      ...
      if (this.props.issues) {
        await this.props.issues.fetch();
      }
      if (this.props.onReloadIssues) {
        await this.props.onReloadIssues();
      }
    }
  };
  ...
  render() {
    if (!roleModel.readAllowed(this.props.entity)) {
      return (
        <Alert
          type="error"
          title={`You have no permissions to view ${this.localizedString('issue')}s`}
        />
      );
    }
          ...
          <Row className={styles.issuesNav} align="middle" type="flex" justify="space-between">
            ...
            <Row className={styles.actions}>
              {roleModel.readAllowed(this.props.entity) &&
                ...
                !this.props.readOnly && (
                  <Button type="primary" size="small" onClick={this.openCreateIssueDialog}>
  ...
  UNSAFE_componentWillReceiveProps(nextProps) {
    if (
      ...
      nextProps.entityClass !== this.props.entityClass
    ) {
      this.setState({
        selectedIssue: null,
        createNewIssueDialogVisible: false,
