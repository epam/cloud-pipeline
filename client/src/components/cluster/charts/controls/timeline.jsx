...
class Timeline extends React.Component {
  ...
  constructor(props) {
    super(props);
    // Cache injected store references so getters don't read this.props in reactive contexts
    this._plot = props.plot;
    this._themes = props.themes;
  }

  get backgroundColor() {
    if (this._themes && this._themes.currentThemeConfiguration) {
      return this._themes.currentThemeConfiguration['--cp-color-bg-elevated-header'] || '#ccc';
    }
    return '#ccc';
  }
  ...
  get lineColor() {
    if (this._themes && this._themes.currentThemeConfiguration) {
      return this._themes.currentThemeConfiguration['--cp-color-text'] || 'rgba(0, 0, 0, 0.65)';
    }
  ...
  get tickColor() {
    if (this._themes && this._themes.currentThemeConfiguration) {
      return this._themes.currentThemeConfiguration['--cp-color-text-secondary'] || '#777';
    }
    return '#777';
  }
  ...
  get size() {
    const {chartArea, width} = this._plot;
  ...
  get ticks() {
    ...
    return this.baseTickRule.fillRange(dayjs.unix(from), dayjs.unix(to), true);
  }
  ...
  get baseTickRule() {
    ...
    const duration = dayjs.duration(this.dataRange, 's');
    const durations = DateTimeTicksRules.map((rule) => ({
      ...rule,
      duration: rule.fn(duration),
    }));
    return durations.filter((d) => d.duration <= this.baseTicksCount).pop() || durations[0];
  }
  ...
  UNSAFE_componentWillReceiveProps(nextProps, nextContext) {
    if (nextProps.plot !== this.props.plot) {
      this._plot = nextProps.plot;
    }
    if (nextProps.themes !== this.props.themes) {
      this._themes = nextProps.themes;
    }
    const newState = {};
  ...
  renderZoomArea = () => {
    ...
    const {plot, fontSize} = this.props;
    const {chartArea, height, width} = plot;
      ...
      return (
        <g>
          ...
          <text
            ...
            style={{fontSize}}
          >
            {this.baseTickRule.getFullDescription(dayjs.unix(from))}
          </text>
          <text
            x={x2 + fontSize}
            y={height / 2.0}
            ...
            style={{fontSize}}
          >
            {this.baseTickRule.getFullDescription(dayjs.unix(to))}
          </text>
        </g>
      );
    }
  ...
  render() {
    ...
    const {chartArea, width, height} = plot;
    return (
      <Provider timeline={this}>
