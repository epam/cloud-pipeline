...
def create_data_storage(bucket_name, versioning=False, token=None, expected_status=0, folder=None, sts="", lts="",
                        backup_duration=""):
    command = ['pipe', 'storage', 'create', '--name', bucket_name, '-c', '--path', bucket_name,
               "-d", "The test bucket for integration testing", "-sts", sts, "-lts", lts, "-t", "S3",
               "-b", backup_duration]
    if folder:
