...
import {BarChart, BillingTable, DetailsChart, Summary} from './charts';
...
import {numberFormatter, costTickFormatter, DisplayUser, ResizableContainer} from './utilities';
import {
...
function renderResourcesSubData({
  ...
  const {dataSample = 'value', previousDataSample = 'previous'} =
  ...
  const columns = [
    {
      ...
      render: (value, {fullName = null}) => {
        if (fullName) {
          return (
            <Tooltip title={fullName} styles={{root: {wordWrap: 'break-word'}}}>
              {value}
            </Tooltip>
          );
        }
    ...
    },
    {
      ...
      className: styles.tableCell,
      render: (item) =>
        item && item.costDetails && item.costDetails.computeCost
    ...
    },
    {
      ...
      className: styles.tableCell,
      render: (item) =>
        item && item.costDetails && item.costDetails.diskCost
          ? costTickFormatter(item.costDetails.diskCost)
          : null,
    },
  ].filter(Boolean);
  const tableData =
    tableDataRequest && tableDataRequest.loaded
  ...
  const paginationEnabled =
    tableDataRequest && tableDataRequest.loaded ? tableDataRequest.totalPages > 1 : false;
  return (
...
class InstanceReport extends React.Component {
  get costDetailsData() {
    ...
    if (costDetails.loaded) {
      const {computeCost, diskCost} = costDetails.value;
      return {
  ...
  handleMetricsChange = (newMetrics) => {
    ...
    const {metricsNavigation} = filters;
                    ...
                    {({height}) => (
                      ...
                      >
                        <DetailsChart
                          ...
                          highlightTickFn={(_, tick) =>
                            tick._index === selectedCostDetailsMetricsIndex
                          }
                    ...
                    {({width, height}) => (
                      <ResourcesSubData
                        extra={
                          ...
                          >
                            <InstanceFilter onChange={this.handleMetricsChange} value={metrics} />
                          </div>
                        }
                        extraHeight={30}
                        request={instances}
...
export default withRouter(
  inject(
    'awsRegions',
    'reportThemes',
  )(inject(injection)(BillingNavigation.attach(observer(InstanceReport)))),
);
