...
import {renderHtml, injectCloudPipelineLinksHelpers} from '../markdown';
...
export default class Issue extends localization.LocalizedReactComponent {
  ...
  get isMineIssue() {
    if (
      this.props.issueInfo &&
      this.props.issueInfo.loaded &&
  ...
  canRemoveComment = (comment) => {
    return (
      !this.props.readOnly && !comment.rootComment && (this.isMineComment(comment) || this.isAdmin)
    );
  };

  canEditComment = (comment) => {
    return !this.props.readOnly && (this.isMineComment(comment) || this.isAdmin);
  };
  ...
  onRenameIssue = async (name) => {
    ...
    const payload = {
      labels: this.props.issueInfo.loaded
        ? (this.props.issueInfo.value.labels || []).map((l) => l)
        : undefined,
      text,
      htmlText,
      name,
  ...
  onApplyEditCommentClicked = async () => {
    if (this.state.editableCommentId === 'root') {
      await this.onUpdateIssue(
        this.state.editableCommentText,
        this.state.editableCommentAttachments,
      );
      this.setState({
        editableCommentId: null,
        editableCommentText: null,
      ...
      const attachments = await processUnusedAttachments(
        this.state.editableCommentText,
        this.state.editableCommentAttachments,
      );
  ...
  renderComment = (comment, index) => {
    ...
    const date = (
      <Tooltip overlay={displayDate(comment.createdDate)}>
        <b style={{cursor: 'pointer'}}>{fromNowUtc(comment.createdDate)}</b>
      </Tooltip>
    );
    const renderCommentActions = () => {
      ...
      if (this.state.editableCommentId === comment.id) {
        ...
        actions.push(
          <Button key="cancel" onClick={this.onCancelEditCommentClicked} size="small">
            <CloseOutlined />
          </Button>,
        );
      } else {
    ...
    return (
      <Row
        key={`comment_${index}`}
        type="flex"
        className={classNames(styles.commentContainer, {[styles.mine]: isMine})}
      >
        <Row
          type="flex"
          align="middle"
        ...
        >
          <span>
            {isMine ? 'You' : comment.author} commented {date}
          </span>
          <Row className={styles.actions}>{renderCommentActions()}</Row>
        </Row>
        <Row
          type="flex"
          className={classNames(
  ...
  sendComment = async () => {
    ...
    const attachments = await processUnusedAttachments(
      ...
      this.state.newCommentAttachments,
    );
  ...
  deleteIssue = async () => {
    ...
    if (request.error) {
      ...
      if (this.props.onNavigateBack) {
        this.props.onNavigateBack(true);
      }
    }
  };
  ...
  render() {
    ...
    return (
      <div className={styles.container}>
        <Row type="flex" className={classNames(styles.issueNav, styles.itemHeader)} align="middle">
          <Button
          ...
          <EditableField
            ...
            readOnly={
              !this.canEditIssue || this.state.operationInProgress || this.editCommentInAction
            }
            allowEpmty={false}
            onSave={this.operationWrapper(this.onRenameIssue)}
            editStyle={{flex: 1}}
            text={
              this.props.issueInfo && this.props.issueInfo.loaded
                ? this.props.issueInfo.value.name
                : ''
            }
          />
          {this.canRemoveIssue && (
