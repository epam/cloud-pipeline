...
function generateEmptySet(filters) {
  ...
  const {start: initial, end, tick} = filters;
  const emptySet = [];
  let start = dayjs(initial);
  let unit = 'day';
  if (tick === '1M') {
    unit = 'month';
  ...
  while (start.valueOf() <= end.valueOf()) {
    emptySet.push({
      dateValue: dayjs(start),
      key: dayjs(start).format('YYYY-MM-DD'),
    });
...
function generateLabels(data, filters = {}) {
  ...
  const UNIT_GETTERS = {Y: 'year', M: 'month', D: 'date'};
  const checkUnit = (test, unit) =>
    test[UNIT_GETTERS[unit]]() === currentDate[UNIT_GETTERS[unit]]();
  ...
  let previousDateFn = (date) => dayjs(date).add(-1, 'month');
  if (tick === '1M') {
    format = 'MMM';
    fullFormat = 'MMM YYYY';
    tooltipFormat = 'MMMM YYYY';
    isCurrentDateFn = (test) => checkUnits(test, 'Y', 'M');
    previousDateFn = (date) => dayjs(date).add(-1, 'year');
  }
  const labels = [];
  let year;
  for (let i = 0; i < data.length; i++) {
...
function getProcessedDatasetIsPrevious(dataset = {}) {
  const {isPrevious = false} = dataset.options || {};
  return isPrevious;
}
...
function getProcessedDatasetColor(dataset = {}, reportThemes = {}) {
  const {isPrevious = false, color = isPrevious ? reportThemes.previous : reportThemes.current} =
    dataset.options || {};
  return color;
}
...
function Summary({
  ...
  const maximum = Math.max(
    ...processedDatasets.map((aDataset) =>
      Math.max(...(aDataset.data || []).filter((anItem) => !Number.isNaN(Number(anItem))), 0),
    ),
    0,
  );
  ...
  const chartDatasets = [
    ...processedDatasets.map((processed, index) =>
      extractDataSet(
        processed.data,
    ...
    ...quotaDatasets.map((quotaDataset) =>
      extractDataSet(
        quotaDataset.data,
        quotaDataset.title,
        SummaryChart.quota,
  ...
  return (
    <Export.ImageConsumer style={style} order={1}>
      <Chart
        error={error}
        ...
        type="summary"
        options={options}
        plugins={[PointDataLabelPlugin.plugin, VerticalLinePlugin.plugin]}
      />
