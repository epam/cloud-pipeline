...
class InfiniteScroll extends React.Component {
  ...
  scrollToCurrent = (top = false) => {
    ...
    if (this.scroller) {
      const {hasElementsBefore, headerHeight} = this.props;
      if (top) {
        this.scroller.scrollTop = hasElementsBefore ? MORE_PLACEHOLDER_HEIGHT : 0;
      } else {
        const scrollTo = [...this.scroller.getElementsByClassName('infinite-scroll-element')].find(
          (e) =>
            e.dataset &&
            Object.hasOwn(e.dataset, 'documentId') &&
      ...
      if (elements && elements.length > 0) {
        ...
        const getClosestDocumentId = () => {
          const all = [...obj.getElementsByClassName('infinite-scroll-element')]
            .filter((e) => e.dataset && Object.hasOwn(e.dataset, 'documentId'))
            .map((e) => ({
              id: e.dataset.documentId,
              delta: e.offsetTop - headerHeight + e.offsetHeight - obj.scrollTop,
            }));
          const info = all.filter((o) => o.delta >= 0).sort((a, b) => a.delta - b.delta);
        ...
        if (hasElementsBefore && obj.scrollTop <= MORE_TRIGGER_HEIGHT) {
          ...
          hasElementsAfter &&
          obj.scrollTop >= obj.scrollHeight - obj.clientHeight - MORE_TRIGGER_HEIGHT
        ) {
  ...
  changePageSize = () => {
    if (this.scroller) {
      const {rowHeight, rowMargin} = this.props;
  ...
  reportPageChange = () => {
    const {onPageSizeChanged, pageSize} = this.props;
