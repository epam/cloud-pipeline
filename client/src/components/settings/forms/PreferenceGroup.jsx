      ...
      <div style={{width: '100%', flex: 1, display: 'flex', flexDirection: 'column'}}>
        ...
        <Row className={styles.actions} type="flex" justify="end">
          <Button
            ...
            size="small"
            onClick={() => this.formRef.current && this.formRef.current.resetFields()}
          >
            Revert
          </Button>
          <Button
...
class PreferenceInput extends React.Component {
  ...
  renderInput = () => {
    const {router, themes, value} = this.props;
    const themesManagementPreference =
      router &&
    ...
    if (showThemesManagementRedirection) {
      return (
        <div>
    ...
    if (value.name === 'ui.ssh.theme') {
      return (
        <Select
          ...
          size="small"
        >
          {Object.entries(sshThemesList).map(([value, text]) => (
  ...
  render() {
    ...
    return (
      <div
        className={classNames(styles.preferenceRow, {
        ...
        <Row type="flex" align="middle">
          {this.state.visible ? (
            <EyeFilled
              ...
              onClick={() =>
                !this.props.disabled && this.onVisibilityValueChanged(!this.state.visible)
              }
              style={{
                cursor: 'pointer',
                fontSize: 'large',
          ...
          <span>{highlightText(this.props.value.name, this.props.search)}</span>
        </Row>
        <Row type="flex">{this.renderInput()}</Row>
      </div>
    );
  }
