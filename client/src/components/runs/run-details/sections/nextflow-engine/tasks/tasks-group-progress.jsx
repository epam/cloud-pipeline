...
import {getBarClassNameForNextflowTaskStatus, getSortedTaskStatuses} from './utilities';
...
function TasksGroupProgress(props) {
  const {className, style, tasksGroup} = props;
  const {stats = []} = tasksGroup || {};
  const sorted = getSortedTaskStatuses(stats, true);
  const total = sorted.reduce((res, cur) => res + cur.count, 0);
  return (
    <div
      className={classNames(className, styles.tasksGroupProgress)}
      style={style}
    ...
    >
      {sorted.map((st) => (
        <div
          ...
          className={classNames(styles.tasksGroupStatusProgress)}
        >
          <div
