...
export default class ClusterNodePods extends Component {
    ...
    for (let i = 0; i < array.length; i++) {
      if (array[i] && array[i][key]) {
        ...
        if (!initialized) {
          ...
          (metrics && !metricsResult) ||
          (metrics && metricsResult && metrics !== metricsResult[0])
        ) {
          return undefined;
        }
        if (!valueResult) {
  ...
  generateNonTerminatedPodsTable(node, isLoading) {
    ...
    for (let i = 0; i < node.pods.length; i++) {
      for (let j = 0; j < node.pods[i].containers.length; j++) {
        ...
        if (container.requests) {
          for (const key in container.requests) {
            if (Object.hasOwn(container.requests, key)) {
              keys.add(mapKey(key));
            }
          }
        }
        if (container.limits) {
          for (const key in container.limits) {
            if (Object.hasOwn(container.limits, key)) {
              keys.add(mapKey(key));
            }
          }
        }
    ...
    for (let i = 0; i < node.pods.length; i++) {
      ...
      for (let k = 0; k < keys.length; k++) {
        const req = this.summarize(
          keys[k],
          containers.map((c) => c.requests),
        );
        if (req) {
          podRequests[keys[k]] = req;
        }
        const lim = this.summarize(
          keys[k],
          containers.map((c) => c.limits),
        );
        if (lim) {
  ...
  render() {
    ...
    if (!this.state.dataLoaded && this.props.node.pending) {
      ...
      return (
        <Row>
          <Alert title="The node doesn't contain non-terminated pods" type="info" />
        </Row>
      );
    } else {
