...
import styles from './storage-shared-link-button.module.css';
...
class StorageSharedLinkButton extends React.Component {
      ...
      async () => {
        ...
        try {
          ...
          if (request.loaded) {
            ...
            const {shared, type} = storageInfo;
            const sharedLinkAvailable =
              roleModel.writeAllowed(storageInfo) && !/^nfs$/i.test(type) && shared;
            if (token === this.token) {
              this.setState({
  ...
  renderModalContent = () => {
    const {pending, error, link} = this.state;
    if (link) {
      return <Input.TextArea autoSize value={link} disabled={pending} />;
    }
    if (pending) {
      return <LoadingView />;
    }
  ...
  render() {
    const {shared, visible} = this.state;
    if (!shared) {
      return null;
    }
