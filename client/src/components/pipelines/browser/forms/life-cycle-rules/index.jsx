...
class LifeCycleRules extends React.Component {
  ...
  get rules() {
    ...
    if (lifeCycleRules && lifeCycleRules.loaded && !lifeCycleRules.error) {
      return [...(lifeCycleRules.value || [])].sort((ruleA, ruleB) =>
        ruleA.pathGlob.localeCompare(ruleB.pathGlob),
      );
    }
    return [];
  }
  ...
  render() {
    const {editRule, showHistory, pending} = this.state;
        ...
        <div className={styles.container}>
          ...
          {this.rules.length > 0 ? (
            <table className={styles.table}>
              ...
              <tbody>{this.rules.map((rule) => this.renderRule(rule))}</tbody>
            </table>
          ) : null}
          {editRule ? (
            <LifeCycleEditModal
