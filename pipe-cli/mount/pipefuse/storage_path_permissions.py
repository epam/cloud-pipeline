    ...
    def _wrap(self, attr, name=None):
        ...
        def _wrapped_attr(*args, **kwargs):
            ...
            if method_name == 'access':
                ...
                path = args[0] if args[0] is not None else kwargs.get('path')
                self._permissions_manager.can_write_to_path(path)
                return attr(*args, **kwargs)
            elif method_name == 'open':
                path = args[0] if args[0] is not None else kwargs.get('path')
                flags = args[1] if args[1] is not None else kwargs.get('flags')
                if flags & os.O_CREAT:
                    self._permissions_manager.can_write_to_path(path)
    ...
    def run(self):
        ...
        while True:
            ...
            try:
                ...
                logging.warning('Storage path permissions refresher daemon iteration has failed.', exc_info=True)
