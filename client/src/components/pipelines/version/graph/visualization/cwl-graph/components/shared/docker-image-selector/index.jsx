...
class DockerImageSelector extends React.Component {
  ...
  get selectedDockerImage() {
    ...
    return this.tools.find(
      (aTool) => (aTool.dockerImage || '').toLowerCase() === dockerImage.toLowerCase(),
    );
  }
    ...
    return this.tools
      .map((group) => ({
        ...
        tools: group.tools.filter(
          (tool) =>
            (selected && (tool.dockerImage || '').toLowerCase() === selected) ||
            (search &&
              search.length >= 3 &&
  ...
  renderSelectGroup = (group) =>
    group.tools.map((tool) => {
      ...
      return (
        <Select.Option key={tool.id} value={(tool.dockerImage || '').toLowerCase()} title={title}>
  ...
  render() {
    const {className, style, dockerRegistries, disabled, dockerImage} = this.props;
    const {search} = this.state;
    const pending = dockerRegistries.pending;
    if (dockerRegistries.error) {
      return <Alert type="error" title={dockerRegistries.error} />;
    }
