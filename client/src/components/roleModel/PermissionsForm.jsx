...
import {Alert, AutoComplete, Button, Checkbox, Modal, Popover, Table, Select} from 'antd';
import {
  DeleteOutlined,
  TeamOutlined,
  UserAddOutlined,
...
export default class PermissionsForm extends React.Component {
  ...
  static propTypes = {
    ...
    objectIdentifier: PropTypes.oneOfType([PropTypes.string, PropTypes.number]),
    ...
    subObjectsToCheck: PropTypes.arrayOf(
      PropTypes.shape({
  ...
  get permissionsChanged() {
    const {originalOwner, owner, permissions, originalPermissions} = this.state;
    return (
  ...
  findGroupDataSource = () => {
    ...
    const roles =
      rolesRequest.loaded && groupSearchString
        ? (rolesRequest.value || [])
  ...
  onSelectGroup = async () => {
    const [role] = (this.props.roles.loaded ? this.props.roles.value || [] : []).filter(
      (r) => !r.predefined && this.splitRoleName(r.name) === this.selectedGroup,
    );
    const roleName = role ? role.name : this.selectedGroup;
    this.grantPermission(roleName, false, this.getDefaultMaskForSubject(roleName, false));
  ...
  grantPermission = (name, principal, mask) => {
    ...
    const selectedPermission = newPermissions.find(findPermissionBySidFn(sid));
    this.setState({
  ...
  removeUserOrGroupClicked = (item) => (event) => {
    ...
    const {permissions = [], selectedPermission} = this.state;
    const {sid} = item;
    const newPermissions = permissions.filter(filterRemovePermissionBySid(sid));
    const selectFirstPermission =
      !selectedPermission || permissionSidsEqual(selectedPermission.sid, sid);
    this.setState(
      {
        permissions: newPermissions,
  ...
  onAllowDenyValueChanged =
    (permissionMask, allowDenyMask, allowRead = false) =>
    async (event) => {
    ...
    for (let d = 0; d < granted.length; d++) {
      ...
      if (principal) {
        ...
        if (userInfo && userInfo.roles) {
          rolesToCheck.push(...(userInfo.roles || []).map(({name}) => ({name, principal: false})));
        }
      } else {
  ...
  renderUserPermission = () => {
    const {selectedPermission, pending} = this.state;
    const {permissionsColumns, availablePermissions} = this.props;
    if (selectedPermission) {
      const {sid = {}} = selectedPermission;
      const {name, principal} = sid;
      ...
      const columns = [
        {
          ...
          render: (name, item) => {
            if (!item.allowed && !item.denied) {
              return (
                <span>
                  {name} <i style={{fontSize: 'smaller'}}>(inherit)</i>
                </span>
              );
            }
            return name;
        ...
        permissionsColumns.includes(PERMISSION_COLUMNS.allow)
          ? {
              ...
              render: (item) => (
                <Checkbox
                  disabled={pending || this.props.readonly || (item.allowMask & enabledMask) === 0}
                  checked={item.allowed}
                  onChange={this.onAllowDenyValueChanged(
        ...
        permissionsColumns.includes(PERMISSION_COLUMNS.deny)
          ? {
              ...
              render: (item) => (
                <Checkbox
                  disabled={pending || this.props.readonly || (item.denyMask & enabledMask) === 0}
                  checked={item.denied}
                  onChange={this.onAllowDenyValueChanged(
                    item.allowMask | item.denyMask,
                    item.denyMask,
  ...
  renderUsers = () => {
    const {pending, error, permissions: data = [], selectedPermission} = this.state;
    if (error) {
    ...
    const title = (
      <div className={styles.tableTitle}>
        ...
        <span className={styles.tableTitleActions}>{this.renderGroupAndUsersActions()}</span>
      </div>
    );
    return (
  ...
  renderOwner = () => {
    const {pending, error, owner, ownerInput, originalOwner, fetchedUsers = []} = this.state;
    if (!pending && !error && originalOwner && this.props.showOwner) {
      const isAdminOrOwner =
        this.props.editOwnerAvailable ||
        this.isAdmin() ||
        ...
        return (
          <div className={styles.ownerContainer}>
            ...
            <AutoComplete
              ...
              value={ownerInput === undefined ? owner : ownerInput}
              onBlur={onBlur}
  ...
  revertChanges = () => {
    const {originalOwner, originalPermissions, selectedPermission} = this.state;
    this.setState({
  ...
  applyChanges = () => {
    const {owner, originalOwner, permissions, originalPermissions} = this.state;
    const {objectType, objectIdentifier, refreshPermissionsAfterUpdate = false} = this.props;
      ...
      <div className={styles.permissionsForm}>
        ...
        <Modal
          ...
          footer={
            <div className="cp-modal-footer-actions">
              <Button onClick={this.closeFindUserDialog}>Cancel</Button>
              <Button type="primary" disabled={pending} onClick={this.onSelectUser}>
                OK
              </Button>
            </div>
          }
        ...
        >
          <Select
            ...
            notFoundContent={
              this.state.searchUserTouched ? 'Not found' : 'Start typing to filter users...'
            }
            options={
              this.state.searchUserTouched
                ? this.allUsers.map((user) => ({
                    ...
                    attributes: [user.name, ...Object.values(user.attributes || {})],
                  }))
                : []
            }
          />
        ...
        <Modal
          ...
          footer={
            <div className="cp-modal-footer-actions">
              <Button onClick={this.closeFindGroupDialog}>Cancel</Button>
              <Button type="primary" disabled={pending} onClick={this.onSelectGroup}>
                OK
              </Button>
            </div>
          }
  ...
  fetchSubObjectsPermissions = () => {
    const wrapPermissionsFetch = (subObject) =>
      new Promise((resolve) => {
    ...
    this.setState(
      ...
      () => {
        Promise.all((this.props.subObjectsToCheck || []).map(wrapPermissionsFetch)).then(
          (payloads) => {
            this.setState({
  ...
  objectChanged = () => {
    const {objectIdentifier, objectType} = this.props;
    this._token = {};
      ...
      try {
        ...
        commit(() => {
          if (!request.loaded || request.error) {
            ...
            const {permissions, entity} = request.value || {};
            this.setState(
              {
                permissions: (permissions || []).map((o) => ({...o})),
                originalPermissions: (permissions || []).map((o) => ({...o})),
