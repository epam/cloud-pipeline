...
import {Modal, Input, Checkbox, Select, Button} from 'antd';
...
class LifeCycleRestoreModal extends React.Component {
  ...
  get showForceRestore() {
    const {items = [], restoreInfo, mode} = this.props;
    ...
    const checkExplicitRestores = (items) => {
      return (items || []).some((item) =>
        currentRestores.find(
          (restore) =>
            mapPathToRestorePath(item) === restore.path && restore.status === STATUS.SUCCEEDED,
        ),
      );
    };
    if ((files.length > 0 && parentRestoreApplied) || checkExplicitRestores(items)) {
      return true;
    }
    return false;
  }
  ...
  onOk = () => {
    const {onOk, mode, folderPath, items} = this.props;
    const {days, recipients, restoreVersions, restoreMode, force, notifyUsers} = this.state;
    const payload = {
      days,
      restoreVersions,
      restoreMode,
      force: this.showForceRestore ? force : true,
      notification: {
  ...
  renderHeader = () => {
    const {mode, items, folderPath} = this.props;
    ...
    return (
      <div className={classNames(styles.forceRestoreContainer, 'cp-divider', 'top')}>
        <p style={{marginBottom: '10px', textAlign: 'center'}}>
          Some items have already been restored. Check <b>Force restore</b> to apply new restore to
          this items.
        </p>
        <Checkbox
  ...
  render() {
    const {visible, onCancel, pending, mode, versioningEnabled} = this.props;
    const {days, recipients, restoreVersions, restoreMode, notifyUsers} = this.state;
    return (
      <Modal
        width="400px"
        open={visible}
        ...
        footer={
          <div className={styles.modalFooter}>
            ...
            <Button disabled={pending} type="primary" onClick={this.onOk}>
              RESTORE
            </Button>
          </div>
        }
      >
        <div className={styles.container}>
          ...
          <div className={styles.inputContainer}>
            ...
            <Input onChange={this.onChangeValue('days', 'input')} value={days} disabled={pending} />
          </div>
          <div className={styles.inputContainer}>
          ...
          {this.showForceRestore ? this.renderForceRestoreControl() : null}
          {mode === 'folder' ? (
            <p>
