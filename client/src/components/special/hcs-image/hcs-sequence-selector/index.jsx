...
import styles from './hcs-sequence-selector.module.css';
...
class HcsSequenceSelector extends React.Component {
  ...
  getSelectedTimePoints = (sequence) => {
    const {selection = []} = this.props;
    return selection.filter((o) => o.sequence === sequence).map((o) => o.timePoint);
  };
  ...
  onChangeTimePoint = (sequenceId, timePoint, event) => {
    const {onChange, selection = [], multiple} = this.props;
  ...
  renderCollapseHeader = (sequence = {}) => {
    ...
    return (
      <div>
        <span className="cp-title">{`Sequence ${sequence.sequence}`}</span>
        <span style={{marginLeft: '10px'}}>
          {`${timePoints} time point${timePoints > 1 ? 's' : ''}`}
        </span>
        {selected && (
          <span className={classNames(styles.activeBadge, 'cp-timepoint-button-active')}>
            Active
          </span>
        )}
      </div>
  ...
  renderSequence = (sequence) => (
    <div className={styles.timepointsContainer}>
      {(sequence.timeSeries || []).map((timePoint) => {
        ...
        return (
          <div
            className={classNames(styles.timepoint, {
              [styles.active]: selected,
              'cp-timepoint-button-active': selected,
              'cp-timepoint-button': !selected,
            })}
            key={timePoint.id}
            onClick={(event) => this.onChangeTimePoint(sequence.sequence, timePoint, event)}
          >
            {timePoint.name}
          </div>
        );
  ...
  renderContent = () => {
    const {showCollapseForSingleSequence, sequences = []} = this.props;
  ...
  render() {
    const {className, style, title, showTitle, sequences} = this.props;
    if (
      !sequences ||
      !sequences.length ||
    ...
    return (
      <div className={classNames(styles.container, className)} style={style}>
        {showTitle && (
          <span className={classNames(styles.header, 'cp-title')}>{title || 'Time series'}</span>
        )}
        {this.renderContent()}
      </div>
    );
