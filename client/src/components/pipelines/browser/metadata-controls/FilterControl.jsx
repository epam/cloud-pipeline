...
class FilterControl extends React.Component {
  ...
  render() {
    ...
    const {selectedTags = [], popoverVisible, emptyValue} = this.state;
    const content = (
      <div style={{width: 280, padding: '8px 0px'}}>
        {supportEmptyValue && (
          ...
          >
            <Checkbox checked={emptyValue} onChange={this.onChangeEmptyValue}>
              Empty
            </Checkbox>
          </div>
        )}
        ...
        >
          <Button danger onClick={this.resetFilter} disabled={!value}>
            Reset
          </Button>
          <Button type="primary" onClick={this.handleApplyFilter} disabled={!this.modified}>
            Apply
          </Button>
        </div>
      </div>
    ...
    return (
      <Popover
        ...
        title={
          ...
          >
            <h4>
              Specify filter for <span style={{fontWeight: 600}}>{this.props.columnName}</span>
            </h4>
