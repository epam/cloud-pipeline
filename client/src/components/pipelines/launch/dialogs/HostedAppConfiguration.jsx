...
import styles from './HostedAppConfiguration.module.css';
...
class HostedAppConfigurationDialog extends React.Component {
  ...
  validate = (updateState = true) => {
    return new Promise((resolve) => {
      ...
      let serviceError;
      const portsGlobalError =
        ports.length === 0 ? 'You must provide at least one ports configuration' : undefined;
      if (!service) {
        serviceError = 'Service name must be specified';
      } else if (!/^[a-z]([-a-z0-9]*[a-z0-9])?$/.test(service)) {
        serviceError =
          'Service name must start with letter and contain letters, digits or "-" symbols';
      }
      const checkPort = (port) =>
        port !== undefined && port !== null && port !== '' && !Number.isNaN(Number(port));
      const validatePorts = (portsConfiguration, index, array) => {
        const {port, targetPort} = portsConfiguration || {};
        let portError = checkPort(port) ? undefined : 'Port is invalid';
        const targetPortError = checkPort(targetPort) ? undefined : 'Target port is invalid';
      ...
      const validationError =
        serviceError || portsErrors.filter(Boolean).length > 0 || portsGlobalError;
      const state = {
        validationError,
      };
      if (updateState) {
  ...
  render() {
    const {configuration, visible, onClose, onRemove} = this.props;
    const {service, ports, validationError, serviceError, portsErrors, portsGlobalError} =
      this.state;
    return (
      <Modal
      ...
      >
        ...
        {serviceError && (
          <div className={classNames(styles.row, styles.error, 'cp-error')}>{serviceError}</div>
        )}
        {ports
          .map((port, index) => [
            <div className={styles.row} key={`ports-${index}`}>
              <span className={classNames(styles.label, styles.port)}>Port:</span>
              <InputNumber
                className={classNames(styles.input, styles.port, {
                ...
                max={65535}
              />
              <span className={classNames(styles.label, styles.port)}>Target Port:</span>
              <InputNumber
                className={classNames(styles.input, styles.port, {
                  [styles.error]:
                    portsErrors && portsErrors[index] && portsErrors[index].targetPort,
                  'cp-error': portsErrors && portsErrors[index] && portsErrors[index].targetPort,
                })}
                ...
                max={65535}
              />
              <Button size="small" danger onClick={this.onRemovePorts(index)}>
                <DeleteOutlined />
              </Button>
            </div>,
          ...
          .reduce((r, c) => [...r, ...c], [])}
        <div className={classNames(styles.row, styles.addPort)}>
          <Button onClick={this.onAddPorts}>
            <PlusOutlined /> Add ports configuration
          </Button>
        </div>
        {portsGlobalError && (
          <div className={classNames(styles.row, styles.error, 'cp-error')}>{portsGlobalError}</div>
        )}
      </Modal>
    );
  }
...
class HostedAppConfiguration extends React.Component {
  ...
  renderTitle = () => {
    ...
    return (
      ...
      >
        <span style={{marginRight: 5}}>{service}</span>
        <span>(</span>
        {ports.map((port, index, array) => (
          <span
            key={`${port.port}-${port.targetPort}-${index}`}
            style={index === array.length - 1 ? {} : {marginRight: 5}}
          >
  ...
  render() {
    ...
    return (
      <div>
        <a className={classNames(styles.link, 'cp-text')} onClick={this.openConfigurationDialog}>
          {this.renderTitle()}
        </a>
