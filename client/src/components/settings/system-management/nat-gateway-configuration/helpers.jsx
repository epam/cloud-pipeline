...
export function validatePort(value, otherPorts) {
  ...
  try {
    const ports = portUtilities.parsePorts(value, {
      throwError: true,
    ...
    for (const otherPortConfiguration of otherPorts) {
      const parsed = portUtilities.parsePorts(otherPortConfiguration, {
        maxPorts: portConfig.total,
        throwError: {maxPorts: true},
      });
...
function renderStatusIcon(status, record) {
  switch (status) {
    ...
    default:
      return (
        <QuestionCircleOutlined
          className={classNames(styles.routeStatus, 'cp-nat-route-status', 'cp-text-not-important')}
          style={{display: 'none'}}
        />
      );
  }
          ...
          <div>
            ...
            <Popover
              content={
                <div>
                  {value.map((item) => (
                    <div key={item}>{highlightText(item, search)}</div>
                  ))}
                </div>
              }
            >
              <a>and {all - totalSliced} more...</a>
            </Popover>
          </div>
        );
      }
    ...
    return (
      <Wrapper>
        <div className={styles.ports}>
          {slicedArray.map((row) => (
            <span className={styles.port} key={row}>
