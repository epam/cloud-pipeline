...
import {fromNowUtc, toUtcDayjs} from '../../../utils/dayjs';
...
function getTimingInfoString(from, to) {
  if (from && to) {
    const diff = toUtcDayjs(to).diff(toUtcDayjs(from), 'minute', true);
...
@inject(({multiZoneManager}, params) => {
  return {
    runInfo:
      params.item && (params.item.id || params.item.elasticId)
...
export default class PipelineRunPreview extends React.Component {
  ...
  get runName() {
    if (this.props.item) {
      ...
      if (this.props.runInfo && this.props.runInfo.loaded) {
        const {dockerImage, pipelineName, version} = this.props.runInfo.value;
  ...
  get timeFromStart() {
    ...
    return fromNowUtc(startDate, {withoutSuffix: true});
  }
  ...
  get runningTime() {
    ...
    return fromNowUtc(this.props.runTasks.value[0].started, {withoutSuffix: true});
  }

  get timings() {
    if (this.props.runInfo && this.props.runInfo.loaded) {
      const result = [];
      const {startDate, endDate, status} = this.props.runInfo.value;
      result.push({
        title: 'Scheduled:',
  ...
  get isDtsEnvironment() {
    return (
      this.props.runInfo.loaded &&
      this.props.runInfo.value.executionPreferences &&
    ...
    if (this.props.runInfo && this.props.runInfo.loaded) {
      ...
      if (instance) {
        if (RunTags.shouldDisplayTags(run, this.props.preferences, true)) {
          details.push({
            key: 'Tags',
            value: <RunTags run={run} onlyKnown theme="black" />,
        ...
        if (!this.isDtsEnvironment) {
          if (instance.cloudRegionId && instance.nodeType) {
            ...
            details.push({
              key: 'Cloud Region',
              value: (
                <AWSRegionTag style={{verticalAlign: 'top'}} regionId={instance.cloudRegionId} />
              ),
            });
          }
      ...
      if (details.length > 0) {
        return (
          <Row
            className={classNames(styles.description, 'cp-search-header-description', styles.tags)}
          >
            {details.map((d) => {
              return (
                <span
  ...
  renderInfo = () => {
    if (this.props.runInfo) {
      ...
      if (this.props.runInfo.error) {
        return (
          <div className={styles.contentPreview}>
            <span className={'cp-search-preview-error'}>{this.props.runInfo.error}</span>
          </div>
        );
      }
      ...
      const {owner} = this.props.runInfo.value;
      const headerStyle = {
        verticalAlign: 'top',
        paddingRight: 5,
        whiteSpace: 'nowrap',
          ...
          <table>
            <tbody>
              {endpointsAvailable && regionedUrls.length > 0 && (
                <tr>
                  <td style={headerStyle}>Endpoints:</td>
                  <td style={valueStyle}>
              ...
              <tr>
                <td style={headerStyle}>Estimated price:</td>
                <td style={valueStyle}>
                  <JobEstimatedPriceInfo>
                    {adjustPrice(evaluateRunPrice(this.props.runInfo.value).total)}$
                  </JobEstimatedPriceInfo>
                </td>
              </tr>
            </tbody>
  ...
  renderTasks = () => {
    if (this.props.runTasks) {
      ...
      if (this.props.runTasks.error) {
        return (
          <div className={styles.contentPreview}>
            <span className={'cp-search-preview-error'}>{this.props.runTasks.error}</span>
          </div>
        );
      }
          ...
          <div className={classNames(styles.contentPreview, 'cp-search-content-preview')}>
            {(this.props.runTasks.value || []).map((task, index) => {
              return (
                <Row
                  key={index}
                  className={classNames(styles.task, 'cp-search-content-preview-run-task')}
                >
                  <StatusIcon status={task.status} small iconSet={icons} displayTooltip={false} />
                  <code>{task.name}</code>
                </Row>
              );
            })}
        ...
        <div className={styles.header}>
          ...
          >
            {this.props.runInfo && this.props.runInfo.loaded ? (
              <StatusIcon run={this.props.runInfo.value} iconSet={icons} />
            ) : ItemIcon ? (
