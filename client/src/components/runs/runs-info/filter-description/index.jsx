...
import styles from './filter-description.module.css';

function ActiveRunsFilterDescription(props) {
  const {className, style, filters, postfix} = props;
  const {owners = [], instanceTypes = [], dockerImages = [], tags = {}, statuses = []} = filters;
  const parts = [];
  const buildArrayInfo = (array = [], mapper = (o) => o, options = {}) => {
    const {prefix: prefixText, postfix: postfixText} = options;
    ...
    if (array.length > 1) {
      parts.push(
        <div className={styles.filterDescriptionEntry}>
          {entryPrefix}
          {array.map((o, idx) => (
            <div className={styles.filterDescriptionEntry} key={`entry-${idx}`}>
              {mapper(o)}
              {idx < array.length - 1 && (
  ...
  buildArrayInfo(instanceTypes, (instanceType) => <b>{instanceType}</b>, {
    prefix: instanceTypes.length > 1 ? ', instance types ' : ', instance type ',
  });
  const tagsArray = Object.keys(tags);
  if (tagsArray.length > 0) {
    parts.push(tagsArray.length > 1 ? ', tags ' : ', tag ');
    parts.push(<RunTags onlyKnown={false} run={{status: 'RUNNING', tags}} overflow={false} />);
  }
  if (parts.length > 0) {
    return (
      <div className={classNames(className, styles.filterDescription)} style={style}>
        ...
        <span className={styles.filterDescriptionDivider}> </span>
        {parts.map((part, idx) => (
          <div key={`part-${idx}`} className={styles.filterDescriptionEntry}>
            {part}
          </div>
        ))}
