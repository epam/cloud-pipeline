...
import RequiredLaunchTags from './special/required-launch-tags';
  ...
  return new Promise((resolve) => {
    queueRequests(
      requests.map(
        ({request, apply}) =>
          () =>
            wrapRequest(request, apply),
      ),
    ).then((payloads) => {
      const error = payloads.map((payload) => payload.error).find(Boolean);
      resolve(error);
    });
  });
};
...
@inject(
  ({metadataCache, dataStorageCache, pipelines, dockerRegistries, systemDictionaries}, params) => ({
    pipelines,
    dockerRegistries,
    metadataCache,
    dataStorageCache,
...
export default class Metadata extends localization.LocalizedReactComponent {
  static propTypes = {
    ...
    entityParentId: PropTypes.oneOfType([PropTypes.string, PropTypes.number]),
    entityVersion: PropTypes.string,
    canNavigateBack: PropTypes.bool,
    onNavigateBack: PropTypes.func,
    fileIsEmpty: PropTypes.bool,
    applyChanges: PropTypes.oneOf([ApplyChanges.callback, ApplyChanges.inline]),
  ...
  applyChanges = async (modified = [], removed = []) => {
    ...
    if (applyChanges === ApplyChanges.callback && onChange) {
      ...
      if (dataStorageTags) {
        if (removed.length > 0) {
          const removeRequest = new DataStorageTagsDelete(entityParentId, entityId, entityVersion);
          const removeRequestFn = () => removeRequest.send(removed.map((item) => item.key));
          requests.push({
        ...
        if (modified.length > 0) {
          ...
          const updateRequestFn = () =>
            updateRequest.send(
              modified.map(({key, value}) => ({[key]: value})).reduce((r, c) => ({...r, ...c}), {}),
            );
          requests.push({
  ...
  saveMetadata = (opts) => async () => {
    ...
    if (this.state.addKey) {
      ...
      const {result, warnings} = this.getCascadeValues(
        this.state.addKey.key.trim(),
        this.state.addKey.value,
        this.state.addKey.secret,
        this.props.systemDictionaries.loaded ? this.props.systemDictionaries.value || [] : [],
      );
      if (warnings.size > 0) {
        message.warning(
      ...
      if (
        (field === 'key' && currentMetadataItem.key === value) ||
      ...
      if (dictValue) {
        ...
        for (let l = 0; l < links.length; l++) {
          ...
          if (existedLink) {
            ...
            const {result: childResult, warnings: childWarnings} = this.getCascadeValues(
              linkKey,
              linkValue,
              false,
              dictionaries,
  ...
  applyCascadeValues = async (key, value, secret = false) => {
    ...
    if (systemDictionaries.loaded) {
      const {result: values, warnings} = this.getCascadeValues(
        key,
        value,
        secret,
  ...
  renderAutogeneratedTag = (key, title, IconComponent, link, value) => {
    return [
      <tr key={`${key}_key`} className={classNames('cp-metadata-item-row', 'key', 'read-only')}>
        <td
          id={`key-column-${key}`}
          colSpan={6}
  ...
  renderSpecialTag = (metadataItem) => {
    ...
    if (key && Object.hasOwn(SpecialTags, key)) {
  ...
  renderJobConfigurationTag = (metadata) => {
    const pipelineIdTag = metadata.find((item) => item.key === AutoGeneratedTags.job.id);
    const pipelineVersionTag = metadata.find((item) => item.key === AutoGeneratedTags.job.version);
    const pipelineConfigurationTag = metadata.find(
  ...
  renderDockerImageTag = (metadata) => {
    ...
    if (dockerImageTag && this.props.dockerRegistries.loaded) {
      ...
      const registry = (this.props.dockerRegistries.value.registries || []).find(
        (r) => r.path.toLowerCase() === (registryPath || '').toLowerCase(),
      );
      if (registry) {
        ...
        if (group) {
          const tool = (group.tools || []).find(
            (t) => t.image.toLowerCase() === `${groupName || ''}/${toolName}`.toLowerCase(),
          );
          if (tool) {
  ...
  renderCalculationConfigTag = (metadata) => {
    const calcConfigTag = metadata.find((item) => item.key === AutoGeneratedTags.calculationConfig);
    if (calcConfigTag) {
      ...
      const value = (
        <ul style={{listStyle: 'disc inside'}}>
          {instanceType && <li>Instance type: {instanceType}</li>}
          {workersCount && <li>Workers count: {workersCount}</li>}
          {totalCPU && <li>Total CPU: {totalCPU}</li>}
        </ul>
      );
      return this.renderAutogeneratedTag(
        calcConfigTag.key,
  ...
  renderMetadataItem = (metadataItem) => {
    ...
    if (this.state.editableKeyIndex === metadataItem.index) {
      keyElement = (
        <tr
          ...
          className={classNames('cp-metadata-item-row', 'key', 'editable')}
        >
          <td colSpan={6}>
            <Input
              {...inputOptions('key')}
      ...
      keyElement = (
        ...
        >
          ...
          >
            <span style={{display: 'inline-block'}}>{metadataItem.key}</span>
          </td>
          {readOnly ? undefined : (
    ...
    const {systemDictionaries} = this.props;
    const key =
      this.state.editableKeyIndex === metadataItem.index
    ...
    if (dictionary) {
      ...
      valueElement = (
        <tr
          key={`${metadataItem.key}_value`}
          className={classNames('cp-metadata-item-row', 'value')}
        >
          <td
  ...
  getDivider = (key, span) => {
    return (
      <tr key={key}>
        <td colSpan={span || 3}>
          <div className={classNames(styles.divider, 'cp-divider', 'horizontal')} />
        </td>
      </tr>
    );
  };
  ...
  renderFilePreview = () => {
    ...
    const previewRes = [];
    const {entityParentId, metadataRenderFn, entityId, entityClass} = this.props;

    if (entityClass === 'DATA_STORAGE_ITEM' && entityParentId && entityId) {
      previewRes.push(
        <OpenStaticPreview
          key="open-static"
    ...
    if (!mayBeBinary) {
      previewRes.push(
        ...
        >
          <Col colSpan={2}>File preview</Col>
          <Col style={{textAlign: 'right'}}>
            <Button onClick={this.props.openEditFileForm} size="small" style={{border: 'none'}}>
              <ArrowsAltOutlined />
            </Button>
          </Col>
        </Row>,
      ...
      previewRes.push(
        <div id="file-preview-container" key="preview body" className="cp-text-not-important">
          <Input.TextArea
            ...
            autoCapitalize="off"
            autoSize={false}
            className={classNames(styles.disabledTextarea, 'cp-metadata-item-content-preview')}
            styles={{
  ...
  renderTableHeader = (editable) => {
    ...
    const renderTitle = () => {
      if (this.props.entityName && this.props.onNavigateBack && this.props.canNavigateBack) {
        ...
        return <span style={this.props.titleStyle}>{this.props.title || '\u00A0'}</span>;
      }
      return undefined;
    };
    const renderActions = () => {
      ...
      if (
        editable &&
        this.metadata.filter((o) => !this.isReadOnlyTag(o.key) && !this.isSpecialItem(o.key))
          .length > 0 &&
  ...
  renderMetadataTable = () => {
          ...
          <table key="body" style={{width: '100%', tableLayout: 'fixed'}}>
            <tbody>
              ...
              {this.renderAutogeneratedMetadata(
                this.metadata.filter(({key}) => !this.isReadOnlyTag(key)),
              )}
              {this.metadata
    ...
    const result = [header];
    if ((this.props.dataStorageTags && this.props.showContent) || this.props.jobList) {
      const panels = [];
      ...
      if (this.props.showMetadata) {
        if (!this.props.hideMetadataTags) {
          panels.push(metadata);
        }
      }
      if (this.props.jobList) {
        panels.push(this.props.jobList);
      }
      if (panels.length <= 1) {
  ...
  render() {
    if (this.props.pending || (this.props.metadata.pending && !this.props.metadata.loaded)) {
      return <LoadingView />;
    }
    return (
      <div
  ...
  UNSAFE_componentWillReceiveProps(nextProps) {
    if (
      this.props.entityClass !== nextProps.entityClass ||
