...
import {ResizablePanel, ResizeAnchors} from '../../../../special/resizablePanel';
import {Alert, AutoComplete, Row, Button, message, Modal, Popover, Tooltip} from 'antd';
import {
  AppstoreOutlined,
  ArrowsAltOutlined,
  CloseOutlined,
...
import {generatePipelineCommand, Primitives, testPrimitiveTypeFn} from './forms/utilities';
...
class WdlGraph extends Graph {
  ...
  initializeContainer = async (container) => {
    if (container && !this.wdlVisualizer) {
      ...
      this.wdlVisualizer = new Visualizer({
        ...
        drop: {
          allowEvent: (event) =>
            event.dataTransfer.types.map((t) => t.toLowerCase()).includes('dropdatakey'),
          onDrop: this.onDropModel.bind(this),
        },
      });
      this.wdlVisualizer.addEventListener(VisualizerEvent.selectionChanged, (e, s, item) =>
  ...
  onDropModel = async (event, parents = []) => {
    const receiver =
      parents.find((o) => isScatter(o) || isWorkflow(o) || isConditional(o)) || this.workflow;
  ...
  dropPipeline = async (target, pipelineInfo, version, configurations) => {
    // todo:
    const [defaultConfiguration] = (configurations || []).filter(
      (c) => c.default && c.configuration,
    );
    ...
    const addInputParameter = (name, p) => {
      ...
      if (p.value) {
        const expr =
          testPrimitiveTypeFn(Primitives.string, type) || testPrimitiveTypeFn(Primitives.file, type)
            ? `"${clearQuotes(p.value)}"`
            : p.value;
        inputs.push({
          name: nameCorrected,
  ...
  generateWdl() {
    if (this.wdlDocument) {
      const {success, error, content} = (this.wdlProject || Project.default).generateWdl(
        this.wdlDocument,
      );
      if (error) {
  ...
  updateData() {
    ...
    if (this.wdlVisualizer) {
      ...
      this.wdlVisualizer.getElements().forEach((e) => {
        const view = this.wdlVisualizer.paper.findViewByModel(e);
        if (
          selectedTaskName &&
          e.action &&
          e.action.reference === selectedTaskName &&
      ...
      this.setState(
        {
          modified,
          modifiedParameters:
            modified && !workflowParametersEquals(this.workflowParameters, params)
              ? params
              : undefined,
          wdlError,
        },
        () => {
          if (this.props.onBlockingChange) {
            this.props.onBlockingChange(this.state.modified);
          }
        },
      );
    } else {
  ...
  renderSidePanelsControlButtons = () => {
    ...
    return (
      <div className={styles.panelButtonsRow}>
        <Button className={styles.propertiesButton} onClick={this.togglePropertiesSidePanelState}>
          PROPERTIES
        </Button>
      </div>
    );
  ...
  renderSidePanelHeader = (title, onPanelClose) => {
    return (
      <Row
        ...
        align="middle"
        className={classNames('cp-divider', 'top', 'bottom', 'cp-content-panel-header')}
        style={{
          padding: '0px 5px',
        }}
      >
        <span>{title}</span>
        {onPanelClose && <CloseOutlined onClick={onPanelClose} style={{cursor: 'pointer'}} />}
      </Row>
    );
  };
  ...
  renderPropertiesPanel = () => {
    ...
    const panelHeader = this.renderSidePanelHeader('Properties', this.closePropertiesSidePanel);

    return (
      <ResizablePanel
        className={classNames(styles.wdlGraphSidePanel, styles.right, 'cp-content-panel')}
        resizeAnchors={[ResizeAnchors.left]}
      >
        {panelHeader}
        <div className={styles.wdlGraphSidePanelContentContainer}>
          {!selectedElement && <WdlDocumentProperties document={this.wdlDocument} />}
          {!selectedElement && (
            <WdlExecutables
  ...
  renderGraphSearch = () => {
    ...
    const renderOption = (element) => {
      const {type, name} = getEntityNameOptions(element, this.wdlDocument);
      let description;
      ...
      return (
        <AutoComplete.Option key={element.uuid} value={element.uuid} text={element.toString()}>
          <div className={styles.searchItem}>
            ...
            {description && <div className={styles.searchItemDescriptionRow}>{description}</div>}
          </div>
        </AutoComplete.Option>
      );
    };
  ...
  renderAppearancePanel = () => (
    <div
      className={classNames(styles.wdlGraphSidePanel, styles.left, 'cp-pipeline-graph-side-panel')}
    >
      ...
      {this.canModifySources && (
        <div className={classNames(styles.separator, 'cp-divider', 'horizontal')}>{'\u00A0'}</div>
      )}
      ...
      <Tooltip title={this.state.showAllLinks ? 'Hide links' : 'Show links'} placement="right">
        <Button
          className={styles.wdlAppearanceButton}
  ...
  renderGraph() {
    const {loaded, error, loadError} = this.state;
    if (!loaded) {
      return <LoadingView />;
    }
    if (loadError) {
    ...
    if (error) {
      ...
      return <Alert type="warning" title={errorContent} />;
    }
    return (
      <div
      ...
      >
        ...
        <div className={classNames(styles.wdlGraphContainer, 'cp-wdl-visualizer')}>
  ...
  loadMainFile = () => {
    const {parameters, pipeline, pipelineId, pipelineVersion} = this.props;
    this._loadMainFileToken = {};
    ...
    this.setState(
      ...
      async () => {
        ...
        try {
          await Promise.all([parameters.fetchIfNeededOrWait(), pipeline.fetchIfNeededOrWait()]);
          if (pipeline.error) {
          ...
          const {codePath: pipelineCodePath = ''} = pipeline.value || {};
          const {main_file: mainFile = ''} = parameters.value || {};
          const mainFilePath = (() => {
          ...
          const request = new VersionFile(pipelineId, mainFilePath, pipelineVersion);
          await request.fetch();
          if (request.error) {
