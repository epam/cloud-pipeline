...
If there are not enough instances of specified type to launch a run in one region - Cloud Pipeline can automatically try to launch identical instance in another region(s) of the same Cloud Provider.

This behavior is defined by the special Cloud Region setting - "**Run shift policy**":  
