    ...
    if (tool && tool.iconId) {
      return (
        <Row type="flex" align="middle" justify="start" style={{height: '100%'}}>
          ...
          <div style={{flex: 1, display: 'flex', flexDirection: 'column'}}>{renderMainInfo()}</div>
        </Row>
      );
    } else {
      return (
    ...
    if (!this.props.services.loaded && this.props.services.pending) {
      ...
      const services = (this.props.services.value || [])
        ...
        .map((r) => {
          ...
          return {
            ...
            urls: r.serviceUrl ? parseRunServiceUrlConfiguration(r.serviceUrl) || [] : [],
          };
        })
        .reduce((array, obj) => {
      ...
      const navigate = ({url, sameTab}) => {
        ...
        if (typeof url === 'object') {
          ...
          if (defaultRegion && Object.hasOwn(url, defaultRegion)) {
