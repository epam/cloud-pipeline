...
function predefinedFiltersAreTheSame(filtersA, filtersB) {
  ...
  const {filters: a} = filtersA;
  const {filters: b} = filtersB;
  if (!a && !b) {
    return true;
  }
  if (!a || !b) {
  ...
  const {startDateFrom: aStart, endDateTo: aEnd, ...restA} = a;
  const {startDateFrom: bStart, endDateTo: bEnd, ...restB} = b;
...
function generateFilterPayload(filter) {
  const {startDateFrom, endDateTo, ...filters} = filter ? filter.filters || {} : {};
  ...
  return {
    ...
    startDateFrom: start && start.isValid() ? start.format('YYYY-MM-DD HH:mm:ss.SSS') : undefined,
    endDateTo: end && end.isValid() ? end.format('YYYY-MM-DD HH:mm:ss.SSS') : undefined,
  };
}
...
class PredefinedFilterButton extends React.PureComponent {
  ...
  updateCount = () => {
    const {filter, folderId, metadataClass} = this.props;
    if (!filter) {
      this.setState({
        count: 0,
        countPending: false,
  ...
  get isApplied() {
    ...
    const {endDateTo, startDateFrom, ...predefinedFilterFields} = (filter || {}).filters || {};
    const predefinedFilterIsEmpty =
      !endDateTo && !startDateFrom && Object.keys(predefinedFilterFields).length === 0;
    ...
    const startDatesAreTheSame = datesAreTheSame(
      ...
      currentFilter.startDateFrom ? toUtcDayjs(currentFilter.startDateFrom) : undefined,
    );
    const endDatesAreTheSame = datesAreTheSame(
      endDateTo ? localDateToUTC(endDateTo) : undefined,
      currentFilter.endDateTo ? toUtcDayjs(currentFilter.endDateTo) : undefined,
    );
    ...
    return (
      ...
      (currentFilter.filters || []).every((currentFilterField) => {
  ...
  getButtonProps = () => {
    ...
    const {scheme = {}} = filter;
    const {icon, style: schemeStyle} = scheme;
    const applied = this.isApplied;
    return {
      className: classNames(className, getPredefinedFilterClassName(scheme, applied)),
  ...
  render() {
    ...
    const {count, countPending} = this.state;
    const {name, title} = filter;
    return (
      <Button {...this.getButtonProps()} onClick={this.onClick} size="small">
        {name || title}
        {countPending && <LoadingOutlined style={{marginLeft: 5}} />}
        {!countPending && <span style={{whiteSpace: 'pre'}}>{`: ${count}`}</span>}
      </Button>
    );
  }
}
