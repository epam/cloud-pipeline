...
async function parsePathPart(part, replacer) {
  ...
  const {getHcsInfo, fileName, fileNameWithExtension, parentFolder} = replacer || {};
  let result = part;
  if (/\{HCS_FILE_INFO\.[^}]+\}/i.test(part) && typeof getHcsInfo === 'function') {
    const hcsInfo = await getHcsInfo();
    const hcsInfoPlaceholders = [...new Set(part.match(/\{HCS_FILE_INFO\.[^}]+\}/gi))];
      ...
      if (property) {
        ...
        if (
          ...
          result = result.replace(
            new RegExp(`\\{HCS_FILE_INFO\\.${property}\\}`, 'ig'),
            hcsInfo[key],
          );
        }
      }
  ...
  if (parentFolder) {
    result = result.replace(/\{(FILE_PARENT_FOLDER|HCS_FILE_PARENT_FOLDER)\}/gi, parentFolder);
  }
  if (fileName) {
    result = result.replace(/\{(FILE_NAME|HCS_FILE_NAME|FILE)\}/gi, fileName);
  }
  if (fileNameWithExtension) {
    result = result.replace(
      /\{(FILE_NAME_WITH_EXT|HCS_FILE_NAME_WITH_EXT|FILE_WITH_EXT)\}/gi,
  ...
  if (placeholdersRegExp.test(result)) {
    let escaped = escapeRegExp(
      result,
      ESCAPE_CHARACTERS.filter((o) => !['{', '}'].includes(o)),
    );
...
function parseFullPath(path) {
  const [, , storageName, relativePath] =
    path.match(/^(s3:|nfs:|http:|https:)\/\/([^/]+)\/(.+)$/i) || [];
  return {
    storageName,
    path: relativePath,
  };
  ...
  const getHcsInfo = () => {
    if (!hcsInfoPromise) {
      hcsInfoPromise = new Promise((resolve) => {
        const {storageName, path} = parseFullPath(parentFolder);
...
function extractInfo(info, path, pathComponent) {
  ...
  const {path: rawMask, containsPlaceholders, mask} = pathComponent;
  if (!containsPlaceholders || !mask) {
    return {};
  }
  if (mask.test(path)) {
    const [, ...substitutions] = mask.exec(path);
    const infoKeys = rawMask.match(/\{[^}]+\}/gi).map((o) => o.slice(1, -1));
...
async function getObjectStorageFromSession(path, session) {
  ...
  if (typeof path === 'string') {
    const {storageName} = parseFullPath(path);
    storage = storageName;
    isURL = true;
  }
...
async function getEvaluationsByComponents(components, session = {}) {
  ...
  const raw = evaluations.map(({path, info}) => {
    const {[EVALUATION_PLACEHOLDER]: evaluation, [OWNER_PLACEHOLDER]: owner} = info || {};
    return {
      evaluation,
      owner,
      path,
 ...
 * @returns {string}
 */
function buildExternalJobIdentifier(input, spec, result, analysisFile) {
...
function parseExternalJobIdentifier(identifier) {
  ...
  try {
    ...
    return {
      ...
      analysisFile:
        analysisStorageId && analysisPath
...
function evaluationSorter(a, b) {
  ...
  const {alias: aAlias, startDate: aStartDate} = a;
  const {alias: bAlias, startDate: bStartDate} = b;
  ...
  const aDate = toUtcDayjs(aStartDate);
  const bDate = toUtcDayjs(bStartDate);
...
export async function getExternalEvaluations(hcsFileName) {
  ...
  const {fileName, fullPath} = replacer;
  const {specPath, resultsPath, analysisPath} = await getExternalEvaluationsSettings();
  ...
  const evaluations = specs
    .map((aSpec) => {
      const {evaluation, owner, path} = aSpec;
  ...
  return evaluationsInfos.filter(Boolean).sort(evaluationSorter);
}
...
export async function getExternalJobInfo(jobId, session) {
  try {
    const {input, spec, result, analysisFile} = parseExternalJobIdentifier(jobId);
    ...
    const {date, owner, name, timepoints = [], fields = [], wells = []} = specData || {};
    const inputs = [];
