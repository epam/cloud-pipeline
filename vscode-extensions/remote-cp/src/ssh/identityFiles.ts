...
const PATH_SSH_CLIENT_ID_ED25519_SK = path.join(
  homeDir,
  ".ssh",
  "/id_ed25519_sk",
);
...
export async function gatherIdentityFiles(
  ...
  const identityFileContentsResult = await Promise.allSettled(
    identityFiles.map(async (keyPath) => {
      keyPath = (await fileExists(keyPath + ".pub"))
        ? keyPath + ".pub"
        : keyPath;
  ...
  const fileKeys: SSHKey[] = identityFileContentsResult
    .map((result, i) => {
      ...
      if (parsedResult instanceof Error || !parsedResult) {
        logger.error(
          `Error while parsing SSH public key ${identityFiles[i]}:`,
          parsedResult,
        );
        return undefined;
      }

      const parsedKey = Array.isArray(parsedResult)
        ? parsedResult[0]
        : parsedResult;
  ...
  for (const agentKey of sshAgentKeys) {
    const foundIdx = fileKeys.findIndex(
      (k) =>
        agentKey.parsedKey.type === k.parsedKey.type &&
        agentKey.fingerprint === k.fingerprint,
    );
    if (foundIdx >= 0) {
  ...
  logger.trace(
    ...
    preferredIdentityKeys.length
      ? preferredIdentityKeys
          .map(
            (k) => `${k.filename} ${k.parsedKey.type} SHA256:${k.fingerprint}`,
          )
          .join("\n")
      : "None",
  );
