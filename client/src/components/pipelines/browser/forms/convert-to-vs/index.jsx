...
import styles from './convert-to-vs.module.css';
...
class ConvertToVersionedStorage extends React.Component {
  ...
  onConvertClicked = () => {
    ...
    this.setState(
      ...
      () => {
        const promise = onConvert.then
          ? onConvert
          : (...opts) => Promise.resolve(onConvert(...opts));
        promise()
          .catch(() => {})
  ...
  render() {
    const {visible, storageName, onCancel} = this.props;
    ...
    return (
      <Modal
        ...
        title={<span>Convert {storage} to Versioned Storage?</span>}
        footer={
      ...
      >
        <div className={styles.accent}>All the data will be moved under the version control.</div>
        <div className={styles.info}>This operation cannot be undone.</div>
      </Modal>
    );
  }
}
