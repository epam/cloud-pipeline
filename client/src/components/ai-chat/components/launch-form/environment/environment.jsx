  ...
  render() {
    ...
    if (environment.cluster) {
      ...
      clusterDescription = parts.concat(
        environment.autoScaled ? ' auto-scaled cluster' : ' cluster',
      );
      if (environment.autoScaled && environment.defaultNodesCount > 1) {
        clusterDescription = clusterDescription.concat(
          ` (${environment.defaultNodesCount} - ${environment.nodeCount} nodes)`,
        );
      } else {
        clusterDescription = clusterDescription.concat(` (${environment.nodeCount} nodes)`);
      }
    }
    return (
