...
import styles from './cloud-credentials-profile-form.module.css';
...
class CloudCredentialsProfileForm extends React.Component {
  ...
  constructor(props) {
    super(props);
    makeObservable(this, {profileRequest: observable});
  }
  ...
  get modified() {
    ...
    return (
      assumedRole !== initialAssumedRole ||
  ...
  get otherProfiles() {
    const {isNew, cloudCredentialProfiles, credentialsIdentifier, provider} = this.props;
  ...
  get valid() {
    return (
      !this.profileNameError && !this.assumedRoleError && !this.providerError && !this.policyError
    );
  }
  ...
  renderContent = () => {
    const {isNew, provider} = this.props;
    if (!isNew && this.profileRequest && this.profileRequest.error) {
      return <Alert type="error" title={this.profileRequest.error} />;
    }
    if (
      !isNew &&
      (!this.profileRequest || (this.profileRequest.pending && !this.profileRequest.loaded))
    ) {
      return <LoadingView />;
    }
    const {assumedRole, cloudProvider, profileName, initialPolicy, pending} = this.state;
    ...
    return (
      <div>
        ...
        {this.profileNameError && (
          <div className={classNames(styles.errorDescription, 'cp-error')}>
            {this.profileNameError}
          </div>
        )}
        <div className={styles.row}>
        ...
        {this.props.visible && (
          <div className={classNames(styles.row, styles.policy)}>
            <span
              className={classNames(styles.label, {
        ...
        {this.policyError && (
          <div className={classNames(styles.errorDescription, 'cp-error')}>{this.policyError}</div>
        )}
      </div>
    );
  };
  ...
  onSaveClicked = () => {
    this.setState(
      ...
      () => {
        const {isNew, credentialsIdentifier, cloudCredentialProfiles, onClose} = this.props;
        ...
        const {assumedRole, profileName, cloudProvider, policy} = this.state;
        request
          .send({
            assumedRole,
            profileName,
  ...
  render() {
    const {isNew, visible, onClose} = this.props;
    let title;
    if (isNew) {
      ...
      <Modal
        ...
        footer={
          <div className={styles.footer}>
            ...
            <div>
              <Button className={styles.button} disabled={loading} onClick={onClose}>
                Cancel
              </Button>
              <Button
                className={styles.button}
