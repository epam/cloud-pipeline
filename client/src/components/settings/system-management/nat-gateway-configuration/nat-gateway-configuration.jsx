...
import {Button, Spin, Table, message, Tooltip, Input} from 'antd';
...
import styles from './nat-gateway-configuration.module.css';
...
function getRouteIdentifier(route) {
  ...
  const {externalIp, internalIp, grouped = false, externalPort, isNew, protocol} = route;
  return [
    externalIp,
    protocol,
...
export default class NATGateway extends React.Component {
  ...
  get sortedContent() {
    const {routes = [], addedRoutes = []} = this.state;
    ...
    const sorted = [...portUtilities.groupRoutes(routes), ...portUtilities.groupRoutes(addedRoutes)]
      .map((route) => ({
        ...route,
  ...
  get filteredAndSortedContent() {
    ...
    const searchFilter = (route = {}, search = '') => {
      const {divider, routes, isTouched} = route;
      ...
      if (routes && routes.length > 0) {
        return routes.some((route) => {
          ...
          return (
            checkStringifiedProperty(externalIp, search) ||
  ...
  removeRoute = (route) => {
    const {addedRoutes = [], removedRoutes = []} = this.state;
    const {externalIp, externalPort, isNew, grouped, routes = []} = route;
    if (isNew) {
      const ports = grouped ? routes.map((o) => o.externalPort) : [externalPort];
      const state = {
        addedRoutes: addedRoutes.filter(
          (r) => !(r.externalIp === externalIp && ports.includes(r.externalPort)),
        ),
      };
      if (grouped) {
      ...
      this.setState({
        removedRoutes: [...removedRoutes, ...routesToRemove],
      });
    }
  };

  revertRoute = (route) => {
    const {externalIp, externalPort, grouped, routes = []} = route;
    const {removedRoutes = []} = this.state;
    if (grouped) {
      const ports = routes.map((subRoute) => subRoute.externalPort);
      this.setState({
        removedRoutes: removedRoutes.filter(
          (r) => !(r.externalIp === externalIp && ports.includes(r.externalPort)),
        ),
      });
    } else {
      this.setState({
        removedRoutes: removedRoutes.filter(
          (r) => !(r.externalIp === externalIp && r.externalPort === externalPort),
        ),
      });
    }
  };
  ...
  routeIsRemoved = (route) => {
    const {externalIp, externalPort, grouped, routes = []} = route;
    if (grouped) {
      const removed = routes.filter((subRoute) => this.routeIsRemoved(subRoute)).length;
      return removed === routes.length;
    }
    const {removedRoutes = []} = this.state;
    return !!removedRoutes.find(
      (r) => r.externalIp === externalIp && r.externalPort === externalPort,
    );
  };
  ...
  addNewDataToTable = async (formData) => {
    ...
    this.setState({
      addedRoutes: [...addedRoutes, ...formattedData],
    });
  };
  ...
  render() {
    const {pending, expandedRowKeys = [], search} = this.state;
    return (
      <div className={styles.container}>
        ...
        >
          <Input.Search
            ...
            value={search}
          />
          <div className={classNames(styles.refreshButtonContainer)}>
            <Button onClick={this.onRefreshTable} disabled={pending} size="small">
              REFRESH
            </Button>
          </div>
          <Spin spinning={pending}>
            <Table
              ...
              pagination={false}
              rowKey={(record) =>
                record.divider ? record.key || record.description : getRouteIdentifier(record)
              }
              rowClassName={(record) =>
                classNames({
              ...
              <ColumnGroup>
                <Column
                  ...
                  render={(description, record, index) => {
                    const content = (
                      <Tooltip title={description}>{highlightText(description, search)}</Tooltip>
                    );
                    if (record.divider) {
                      return {
                        children: content,
        ...
        <div className={styles.tableContentActions}>
          <div className={styles.tableActions}>
            <Button onClick={() => this.openAddRouteModal()} size="small">
              ADD ROUTE
            </Button>
          </div>
          <div className={styles.tableActions}>
