...
class DownloadFileButton extends React.Component {
  ...
  handleClick = (event) => {
    ...
    const {downloadOtherFileFetched} = this.state;
    const {storageId, path, version, preferences} = this.props;
  ...
  render() {
    const {className, style, storageId, path, version} = this.props;
    const {pending, downloadOtherFileFetched, downloadOtherFilePath, visible} = this.state;
    if (!downloadOtherFileFetched || !downloadOtherFilePath) {
      return (
        <a
          id={`download-${path}`}
          className={classNames('cp-button', className)}
          href={GenerateDownloadUrlRequest.getRedirectUrl(storageId, path, version)}
          target="_blank"
