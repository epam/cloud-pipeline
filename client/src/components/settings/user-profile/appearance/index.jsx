...
function AppearanceSettings({authenticatedUserInfo, themes, preferences, router, management}) {
  ...
  if (management) {
    return <AppearanceManagement router={router} />;
  }
  const {synchronizeWithSystem, singleTheme, systemDarkTheme, systemLightTheme, isSystemDarkMode} =
    themes;
  ...
  const renderActiveLabel = (mode) => {
    return (
      <span
        className={classNames(styles.status, 'cp-themes-group-active-tag', {
  ...
  const renderThemesCards = (systemMode) => {
    ...
    const isSelected = (themeId) =>
      themeIsLight ? themeId === systemLightTheme : themeId === systemDarkTheme;
    return (
      <div className={classNames(styles.modesPanel, styles.centered)}>
        {themes.themes.map((theme) => (
          <ThemeCard
            key={theme.name}
            name={theme.name}
  ...
  const renderSyncModesPanels = () => (
    <div className={styles.systemModesPanels}>
      {Object.entries(systemModes).map(([name, value]) => (
        <div
          key={name}
          className={classNames(styles.systemModes, 'cp-themes-group', {
            selected: isCurrentSystemModeSelected(value),
          })}
        >
          <div className={classNames(styles.systemModesHeader, 'cp-themes-group-header')}>
  ...
  return (
    <div className={styles.appearanceSettings}>
      ...
      >
        <h2 className="cp-title">{cloudPipelineAppName} UI Theme Preferences</h2>
        {administrator && (
      ...
      <div className={classNames(styles.block, 'cp-divider', 'bottom')}>
        ...
        synchronize with your system appearance preferences and automatically switch between day and
        night themes.
      </div>
...
export default roleModel.authenticationInfo(
  inject('themes', 'preferences')(observer(AppearanceSettings)),
);
