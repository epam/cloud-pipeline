...
import {ApiServiceLogLevel} from '../types';
import {
  buildFullUrl,
  checkResponseStatus,
  extractApiError,
...
import {ApiError, ApiInitializationError, AuthorizationError, NetworkError} from './errors';
  ...
  implements ApiService<RequestOptions>
{
  ...
  private _authenticationLogic: ApiServiceAuthentication<this, RequestOptions> | undefined;
  ...
  protected constructor(identifier: string, logLevel = ApiServiceLogLevel.info) {
    super();
  ...
  protected set authorizationBearerToken(authorizationBearerToken: string | undefined) {
  ...
  removeApiServiceStateChangedListener(listener: ApiServiceStateChangedCallback): void {
    this._stateChangedListeners = this._stateChangedListeners.filter((l) => l !== listener);
  }
  ...
  initialize(
    options: Options,
    authenticationLogic?: ApiServiceAuthentication<this, RequestOptions>,
  ): void {
    this.log('initializing with options', {...options});
    this.base = normalizeBaseUrl(options.base);
  ...
  setAuthenticationLogic(
    ...
    this._authenticationLogic = authenticationLogic
      ? (authenticationLogic.bind(this) as ApiServiceAuthentication<this, RequestOptions>)
      : undefined;
  }

  createWaitUntilAuthenticatedLogic(): ApiServiceAuthentication<this, RequestOptions> {
  ...
  async authenticate<AuthenticateOptions>(options?: AuthenticateOptions): Promise<boolean> {
  ...
  async getFetchOptions(options: RequestOptions): Promise<{
    ...
    const fetchBody = serializeRequestBody(body, headers, contentType);
  ...
  async apiRequest(options: RequestOptions): Promise<Response> {
    if (!this.initialized) {
      ...
      throw new ApiInitializationError(`${this.identifier} service not initialized`);
    }
  ...
  async apiStreamRequest(options: RequestOptions): Promise<ReadableStream<Uint8Array>> {
  ...
  async textRequest(options: ApiTextRequestOptions<RequestOptions>): Promise<string> {
    const cacheOption = options.cached ?? false;
    const useCache = cacheOption === true || typeof cacheOption === 'number';
    const ttlMs = typeof cacheOption === 'number' ? cacheOption : undefined;
    const {url, method = 'GET'} = await this.getFetchOptions(options as RequestOptions);
    const doRequest = async (): Promise<string> => {
      if (method === 'GET' && useCache && this._requestsCache.hasCachedRequest(url)) {
      ...
      return this.queue(async () => {
        ...
        const response = await this.apiRequest({...options} as RequestOptions);
        const text = await response.text();
        if (method === 'GET' && useCache) {
          this._requestsCache.addCachedRequest(url, text, ttlMs);
        }
        return text;
      }, options);
    };
  ...
  async textGet(options: Omit<ApiTextRequestOptions<RequestOptions>, 'method'>): Promise<string> {
    return this.textRequest({...options, method: 'GET'} as ApiTextRequestOptions<RequestOptions>);
  }

  async textPost(options: Omit<ApiTextRequestOptions<RequestOptions>, 'method'>): Promise<string> {
    return this.textRequest({...options, method: 'POST'} as ApiTextRequestOptions<RequestOptions>);
  }

  async textPut(options: Omit<ApiTextRequestOptions<RequestOptions>, 'method'>): Promise<string> {
    return this.textRequest({...options, method: 'PUT'} as ApiTextRequestOptions<RequestOptions>);
  }
  ...
  async voidRequest(options: Omit<ApiVoidRequestOptions<RequestOptions>, 'void'>): Promise<void> {
    await this.queue(
  ...
  async voidGet(
    ...
    await this.voidRequest({...options, method: 'GET'} as ApiVoidRequestOptions<RequestOptions>);
  }

  async voidPost(
    options: Omit<ApiVoidRequestOptions<RequestOptions>, 'method' | 'void'>,
  ): Promise<void> {
    await this.voidRequest({...options, method: 'POST'} as ApiVoidRequestOptions<RequestOptions>);
  }

  async voidPut(
    options: Omit<ApiVoidRequestOptions<RequestOptions>, 'method' | 'void'>,
  ): Promise<void> {
    await this.voidRequest({...options, method: 'PUT'} as ApiVoidRequestOptions<RequestOptions>);
  }

  async voidDelete(
    options: Omit<ApiVoidRequestOptions<RequestOptions>, 'method' | 'void'>,
  ): Promise<void> {
    await this.voidRequest({...options, method: 'DELETE'} as ApiVoidRequestOptions<RequestOptions>);
  }
  ...
  protected getBasicFetchOptions(options: RequestOptions): {
    ...
    const {
      ...
      uri,
      baseUrl = this.base,
      url = uri ? buildFullUrl(baseUrl, uri, query) : buildFullUrl(baseUrl, undefined, query),
      body,
  ...
  private async performApiRequest(options: RequestOptions): Promise<Response> {
    try {
      const {url, options: fetchOptions} = await this.getFetchOptions(options);
