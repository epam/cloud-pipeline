...
import styles from './RestrictDockerDialog.module.css';
...
function compareToolsToMount(a, b) {
  ...
  for (let i = 0; i < toolsA.length; i++) {
    ...
    const vA = [...new Set(tA.versions || [])].map((v) => v.version).sort(defaultSorter);
    const vB = [...new Set(tB.versions || [])].map((v) => v.version).sort(defaultSorter);
  ...
  renderRadioBlock = () => {
    ...
    return (
      <div className={classNames(styles.modalRadioContainer, 'cp-divider', 'bottom')}>
        ...
        >
          <Radio className={styles.radioButton} value={KEYS.mountAll}>
            Mount to all available images
          </Radio>
          <Radio className={styles.radioButton} value={KEYS.limitMounts}>
            Limit mounts
          </Radio>
        </Radio.Group>
      </div>
    ...
    return (
      <Modal
        ...
        footer={
          ...
          >
            <Button onClick={this.onCancel} style={{marginRight: 5}}>
              CANCEL
            </Button>
            <Button
              type="primary"
              ...
              disabled={
                limitMounts &&
                (toolsToMount || []).filter((tool) => tool.id && tool.image && tool.registry)
                  .length === 0
              }
            >
              OK
...
class RestrictDockerImages extends React.Component {
  ...
  getStringPresentation = () => {
    ...
    return value
      .map((tool) => [tool.image, getVersions(tool)].filter(Boolean).join(' '))
      .join(', ');
  };
  ...
  render() {
    const {className, style, value = [], disabled} = this.props;
