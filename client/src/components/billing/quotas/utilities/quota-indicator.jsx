...
import styles from './quota-indicator.module.css';
...
function QuotaLimitIndicator(props) {
  ...
  const renderIndicator = () => (
    <div className={styles.circleContainer}>
      <svg height="10" width="10">
        <circle
          className={classNames(statusClassNames[status], 'hide')}
          cx="5"
          cy="5"
          r="4"
          strokeWidth={1}
  ...
  if (quotaHasTriggeredActions(quota)) {
    return (
      <Popover content={getQuotaExpense(quota)} title={false} trigger="hover">
        {renderIndicator()}
      </Popover>
    );
  } else {
