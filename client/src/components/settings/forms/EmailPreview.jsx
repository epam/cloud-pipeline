...
export default class EmailPreview extends React.Component {
  ...
  codeTemplateStyle =
    'padding: 2px 4px;font-size: 90%;color: #c7254e;background-color: #f9f2f4;border-radius: 4px;box-shadow: inset 0 -1px 0 rgba(0, 0, 0, 0.25);';
  ...
  processHtml = (regexp, fieldRegex, html) => {
    ...
    while (result && result.length === 2) {
      ...
      if (isHREForSRC) {
        html =
          html.substring(0, result.index) + paths + html.substring(result.index + result[0].length);
