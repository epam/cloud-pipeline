...
export async function loadAvailablePipelineGroups() {
  ...
  const pipelines = await Promise.all(folders.map((o) => loadPipelineGroup(objectStorage, o.path)));
  return pipelines
    .filter(Boolean)
...
async function loadPipelineGroup(objectStorage, pipelineFolderPath) {
  ...
  try {
    ...
    const {name, latest} = JSON.parse(metaContents);
    if (!latest) {
      throw new Error('No latest version is specified');
    }
    const loadVersion = async (versionFile) => {
      const {key, version, name: versionName} = versionFile || {};
      const versionContents = await objectStorage.getFileContent(
        `${pipelineFolderPath}/${version}.version`,
      );
      ...
      async loadVersions() {
        ...
        const versions = directoryContents
          ...
          .map((o) => ({
            ...
            changeDate: toUtcDayjs(o.changed),
          }))
    ...
    console.log(
      `Error reading pipeline info. Folder "${pipelineFolderPath}", error: ${error.message}`,
    );
    return undefined;
  }
}
...
export async function savePipeline(pipeline) {
  ...
  pipeline.modifiedDate = dayjs.utc();
