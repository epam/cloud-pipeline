...
import {Dropdown, message, DatePicker, Button, Spin, Alert} from 'antd';
...
import styles from './gpu-info.module.css';
...
function Divider() {
  return <div className={classNames('cp-divider', 'vertical')}>{'\u00A0'}</div>;
}
...
class GPUInfoTab extends React.Component {
  ...
  constructor(props) {
    super(props);
    makeObservable(this, {
      _chartsData: observable.ref,
    });
    this._chartsData = props.chartsData;
  }

  componentDidMount() {
    this.disposeReaction = reaction(
      () => this._chartsData?.refreshToken,
  ...
  componentDidUpdate(prevProps, prevState) {
    const {nodeName, chartsData} = this.props;
    if (chartsData !== this._chartsData) {
      this._chartsData = chartsData;
    }
    if (prevProps.nodeName !== nodeName || prevProps.chartsData !== chartsData) {
      this.initRanges(false);
    }
  }
  ...
  get chartsBounds() {
    return {
      ...
      max: this.chartsData?.instanceTo || parseDate(dayjs()).unix,
    };
  }
  ...
  initRanges = (resetToDefaultRange = true) => {
    ...
    let from = fromUnix(to).local().subtract(1, 'day').unix();
  ...
  fetchMetrics = (updateChartsRange = false) => {
    ...
    this.setState({pending: true}, async () => {
      ...
      const fromString = fromUnix(newFrom).utc().format('YYYY-MM-DD HH:mm:ss');
      const toString = fromUnix(newTo).utc().format('YYYY-MM-DD HH:mm:ss');
      const request = new ClusterNodeGPUUsage(nodeName, fromString, toString, {
        runId,
      });
      await request.fetch();
      if (request.error) {
  ...
  onRangeChanged = (range = {}) => {
    ...
    const {fromZoom = false, fromDrag = false, from, to} = range;
        ...
        {[
          {
            ...
            value: (
              <span style={{color: '#09ab5a'}} className={classNames(styles.value, 'cp-primary')}>
          ...
          },
          {
            ...
            value: (
              <span style={{color: '#f04134'}} className={classNames(styles.value, 'cp-warning')}>
  ...
  renderChartControls = () => {
    ...
    const toggleDataset = (key) => {
      ...
      this.setState({
        hideDatasets: hideDatasets.includes(key)
    ...
    const renderLegendMarker = (stroke) => (
      <svg height="10" width="20">
        <line x1={0} y1={5} x2={20} y2={5} stroke={stroke} fill={'none'} strokeWidth={2} />
        <circle
          cx="10"
          cy="5"
          r="3"
    ...
    const onStartChange = (date) => {
      let {to} = this.state;
      const {chartsData} = this.props;
      const start = ensureDayjs(date);
      if (!chartsData.initialized || !start) {
        return;
      }
      to = to ? fromUnix(to) : null;
      ...
      if (to && (newStart.isAfter(to) || newStart.isSame(to))) {
        to = dayjs(newStart).add(1, 'day').subtract(1, 'second');
      }
      this.setState(
        {
    ...
    const onEndChange = (date) => {
      let {from} = this.state;
      const {chartsData} = this.props;
      const end = ensureDayjs(date);
      if (!chartsData.initialized || !end) {
        return;
      }
      from = from ? fromUnix(from) : null;
      const newEnd = calendarDate([end.year(), end.month(), end.date(), end.hour(), end.minute()]);
      if (from && (from.isAfter(newEnd) || from.isSame(newEnd))) {
        from = dayjs(newEnd).subtract(1, 'day').add(1, 'second');
      }
      this.setState(
        {
    ...
    const setRange = ({key}) => {
      ...
      let from;
      if (key === RANGES.hour) {
        from = Math.max(fromUnix(to).local().subtract(1, 'hour').unix(), this.chartsBounds.min);
      }
      if (key === RANGES.day) {
        from = Math.max(fromUnix(to).local().subtract(1, 'day').unix(), this.chartsBounds.min);
      }
      if (key === RANGES.week) {
        from = Math.max(fromUnix(to).local().subtract(7, 'day').unix(), this.chartsBounds.min);
      }
      this.setState(
        {
          from,
    ...
    const disabledDate = (date) => {
      return (
        ...
        (date.unix() < this.chartsBounds.min || date.valueOf() > Date.now())
      );
    };
    const renderRangeControls = () => (
      <div className={styles.rangeControls}>
        <Dropdown
          menu={{
            items: Object.keys(RANGES).map((rangeKey) => ({
              key: rangeKey,
              label: `Last ${rangeKey}`,
            })),
            onClick: setRange,
        ...
        <DatePicker
          ...
          value={this.state.from ? fromUnix(this.state.from) : null}
        ...
        <DatePicker
          ...
          value={this.state.to ? fromUnix(this.state.to) : null}
          disabledDate={disabledDate}
        />
      </div>
    );
    return (
      <div style={{display: 'flex', flexDirection: 'column'}}>
        {chartsData?.runId ? (
          <a style={{marginRight: 'auto', marginLeft: 50}} onClick={this.navigateToRun}>
            Showing statistics for run #{chartsData.runId}
          </a>
        ) : null}
        <div
          className={classNames(styles.chartControls, {
  ...
  renderTelemetry = () => {
    const {measure, metrics, chartsFrom, chartsTo, hideDatasets} = this.state;
    const themeConfiguration = this.themeConfiguration;
    ...
    return (
      <div className={styles.telemetry}>
        ...
        <TimelineChart
          ...
          to={chartsTo}
          hover={{
            getHoveredElementsInfo: (hoveredItems, styles) =>
        ...
        {heatmapDataset.length ? (
          <div style={{display: 'flex', flexDirection: 'column', gap: '5px'}}>
            <HeatMapChart
              ...
              to={chartsTo}
              hover={{
                getHoveredElementsInfo: (hoveredItem, styles) =>
  ...
  render() {
    ...
    if (!gpuStatisticsAvailable) {
      return <Alert type="warning" title="GPU statistics is not available for this node" />;
    }
    return (
