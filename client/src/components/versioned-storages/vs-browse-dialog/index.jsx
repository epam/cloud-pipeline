...
import styles from './vs-browse-dialog.module.css';
...
class VSBrowseDialog extends React.Component {
  ...
  onVersionsCellClicked = (storage) => {
    const {disabled, pipelineId} = this.state;
  ...
  onSelectStorage = (storage) => {
    ...
    const {disabled, pipelineId} = this.state;
    if (
      (storage && repositories.find((r) => +r.id === +storage.id)) ||
      disabled ||
  ...
  onSelect = () => {
    ...
    const {pipelineId, pipelineInfo = {}, version, versions} = this.state;
    if (onSelect) {
      return onSelect({
        ...pipelineInfo,
        id: pipelineId,
  ...
  renderVSVersions = (storage) => {
    const {pipelineId, version} = this.state;
    const {repositories} = this.props;
  ...
  renderVSTable = () => {
    ...
    const {pipelineId, filter} = this.state;
  ...
  render() {
    const {visible, onClose, pipelines} = this.props;
    const {disabled, pipelineId, version, pending} = this.state;
    return (
      <Modal
        width="60%"
        ...
        footer={
          <div className={styles.footer}>
            <Button id="cancel-vs-storage-button" disabled={disabled} onClick={onClose}>
              CANCEL
            </Button>
            <Button
      ...
      >
        {pipelines && pipelines.error && <Alert type="error" title={pipelines.error} />}
