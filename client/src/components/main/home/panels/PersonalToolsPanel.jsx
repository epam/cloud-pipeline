...
class PersonalToolsPanel extends React.Component {
  ...
  get tools() {
    if (this.props.dockerRegistries.loaded) {
      ...
      if (this.props.completedRuns.loaded) {
        const allRuns = (this.props.completedRuns.value || [])
          .map((a) => a)
          .map((r) => r.dockerImage)
          .filter((r) => !!r)
          .map((r) => {
  ...
  onRunToolClicked = async (tool) => {
    ...
    const registry = this.registries.find((r) => r.id === tool.registryId);
    if (toolRequest.error) {
      ...
      const [versionSettings] = (toolSettings.value || []).filter(
        (v) => (v.version || '').toLowerCase() === defaultTag,
      );
      ...
      if (!defaultTag || !defaultCommandIsNotEmpty || !instanceTypeIsNotEmpty || !diskIsNotEmpty) {
        const version = defaultTag || anyTag;
        const {allowedToExecute, tooltip, launchTooltip} = getVersionRunningInfo(
          version,
          versions,
          scanPolicy.value,
          this.isAdmin(),
        ...
        if (
          ...
          if (this.props.dataStorageAvailable.loaded) {
            ...
            const filtered = selection.filter(
              filterNFSStorages(this.props.preferences.nfsSensitivePolicy, hasSensitive),
            );
            if (filtered.length) {
        ...
        if (allowedToExecute) {
          ...
          const validation = await getUserTagsValidationResult(defaultPayload.tags || {}, {
    ...
    const renderMainInfo = () => {
      ...
      return [
        <Row key="name" type="flex" align="middle">
          ...
          <PlatformIcon platform={tool.platform} style={{marginLeft: 5}} />
        </Row>,
        <Row key="description">
          <span style={{fontSize: 'smaller'}}>{tool.shortDescription}</span>
        </Row>,
        <Row key="group">
  ...
  updateUserTagsValidationInfo = async () => {
    ...
    if (runToolInfo) {
      ...
      const {validation, visibility, payload} = info || {};
      this.setState({
        runToolInfo: {
          ...runToolInfo,
          userTagsValidation: validation,
  ...
  render() {
    ...
    let toolVersion;
    if (this.state.runToolInfo) {
      const fullImageInfo = (this.state.runToolInfo.payload.dockerImage || '').split('/').pop();
        ...
        <Modal
          ...
          footer={
            <Row type="flex" align="middle" justify="space-between">
              <Button
                onClick={() => {
                  this.runToolWithCustomSettings(
                    this.state.runToolInfo.tool.id,
                    this.state.runToolInfo.tag,
                  );
                  this.cancelRunTool();
                }}
              >
              ...
              <Col>
                <Button onClick={this.cancelRunTool}>Cancel</Button>
                <Button
                  disabled={
                    !this.state.runToolInfo ||
                    !this.state.runToolInfo.payload ||
