  ...
  get storages() {
    if (
      ...
      const result = (this.props.dataStorages.value || [])
        ...
        .filter(
          (s) => roleModel.writeAllowed(s) || roleModel.readAllowed(s) || roleModel.isOwner(s),
        );
      result.sort((sA, sB) => {
        const sAisOwner =
          sA.owner &&
          sA.owner.toLowerCase() === this.props.authenticatedUserInfo.value.userName.toLowerCase();
        const sBisOwner =
          sB.owner &&
          sB.owner.toLowerCase() === this.props.authenticatedUserInfo.value.userName.toLowerCase();
    ...
    return [
      ...
      >
        {storage.type.toUpperCase() === 'NFS' && (
          <span className={classNames(styles.nfsStorageType, 'cp-nfs-storage-type')}>NFS</span>
        )}
        <span type="main">
      ...
      <Row key="path">
        ...
        {storage.sensitive ? (
          <span className="cp-sensitive" style={{marginLeft: 5}}>
            sensitive
          </span>
        ) : null}
      </Row>,
  ...
  renderContent = () => {
    const navigate = ({id}) => {
      if (this.props.router) {
        this.props.router.push(`/storage/${id}`);
      }
    };
    return (
