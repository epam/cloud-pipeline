...
function splitLine(line) {
  ...
  for (let i = 0; i < parts.length; i++) {
    ...
    processed = processed.concat(processed.length ? ',' : '').concat(part.replace(/""/g, "'"));
...
export function parseSampleSheet(content) {
  ...
  return {
    ...
    titles: dataSection && dataSection.data.length > 0 ? dataSection.data[0].data : [],
  };
}
