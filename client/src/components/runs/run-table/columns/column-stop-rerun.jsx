...
import {canStopRun, openReRunForm, runPipelineActions, stopRun, terminateRun} from '../../actions';
...
class StopReRunButtonComponent extends localization.LocalizedReactComponent {
  ...
  render() {
      ...
      switch ((run.status || '').toLowerCase()) {
        ...
        case 'resuming':
          ...
          if (run.commitStatus && run.commitStatus.toLowerCase() === 'committing') {
            return (
              <span key="committing" style={{fontStyle: 'italic'}}>
                COMMITTING
              </span>
            );
          }
        ...
        case 'success':
          if (
            ...
            return (
              <a key="rerun" id={`run-${run.id}-rerun-button`} onClick={this.reRunPipeline}>
                RERUN
              </a>
            );
          }
    ...
    if (runSupportsContinue(run)) {
      actions.push(
        <a key="continue" id={`run-${run.id}-continue-button`} onClick={this.onContinueRun}>
          CONTINUE
        </a>,
      );
    }
    if (actions.length > 0) {
      return <div style={{display: 'flex', flexDirection: 'column', flexGap: 2}}>{actions}</div>;
    }
    return <div />;
  }
}
...
const getColumn = (localization, reload) => ({
  ...
  render: (run) => (
    <RunLoadingPlaceholder run={run} empty>
      <StopReRunButton run={run} reload={reload} />
