...
import {Button, Select, Dropdown, Pagination, Input, Spin, Alert, message, Modal} from 'antd';
...
import mainStyles from '../tickets.module.css';
...
@inject((stores, props) => {
  ...
  const {page, search, statuses, default: defaultFilters} = parseTicketsFilters(location.search);
  return {
    page,
    search,
    statuses,
...
class TicketsList extends React.Component {
  ...
  get tableGridTemplate() {
    ...
    const [longestWidth = 0] = measureTextWidth(labelsOnPage, '13px').sort((a, b) => b - a);
    const minWidth = 50;
  ...
  fetchCurrentPage = () => {
    const {search, statuses, default: defaultFilters, page = 0, preferences} = this.props;
    ...
    this.setState(
      ...
      async () => {
        ...
        try {
          ...
          let statusesPayload = [];
          if (defaultFilters) {
            const {not = false, labels: initialLabels = []} =
              preferences.gitlabIssueDefaultFilters || {};
            payload.labelsFilter = preferences.gitlabIssueDefaultFilters;
            statusesPayload = this.predefinedLabels.filter(
              (aLabel) =>
                (not && !initialLabels.includes(aLabel)) ||
                (!not && initialLabels.includes(aLabel)),
            );
          } else if (statuses && statuses.length > 0) {
            const correctedStatuses = this.predefinedLabels.filter(
              (aLabel) => !(statuses || []).includes(aLabel),
            );
  ...
  changeFilters = (filters, reFetchPage = true) => {
    ...
    if (
      ...
      const url = query && query.length ? `/tickets?${query}` : '/tickets';
      router.push(url);
    } else if (reFetchPage) {
      this.fetchCurrentPage();
    }
  ...
  onSelectTicket = (ticket) => {
    ...
    const url =
      query && query.length ? `/tickets/${ticket.iid}?${query}` : `/tickets/${ticket.iid}`;
    router.push(url);
  };
    ...
    return (
      ...
      >
        ...
        <div className={styles.controls}>
          {this.filtersTouched && (
            <a
              className={classNames(styles.tableControl)}
  ...
  renderTicket = (ticket) => {
    const {filters, pending} = this.state;
    const getLabel = (labels) =>
      (labels || []).find((label) => this.predefinedLabels.includes(label));
    ...
    return (
      <div
        key={ticket.iid}
        className={classNames(styles.ticketContainer, 'cp-divider', 'bottom', 'cp-table-element', {
          'cp-table-element-disabled': pending,
        })}
        style={{grid: this.tableGridTemplate}}
        onClick={() => this.onSelectTicket(ticket)}
      >
        <Label className={styles.status} label={currentLabel} />
        <div className={styles.title}>
          <b>{highlightText(ticket.title, filters.search)}</b>
          <span className="cp-text-not-important" style={{fontSize: 'smaller'}}>
            <span style={{marginRight: '5px'}}>{`#${ticket.iid}`}</span>
            {'Opened '}
            {displayDate(ticket.created_at, 'D MMM YYYY, HH:mm')}
            {' by '}
            <UserName showIcon userName={getAuthor(ticket)} />
          </span>
        </div>
        <div
          className={styles.controls}
          onClick={this.enableControls ? (event) => event.stopPropagation() : undefined}
        >
          {this.enableControls && (
            <Dropdown
              menu={{
  ...
  onSelectNewStatus = (status, ticket) => {
    if (this.predefinedLabels.find((label) => label === status)) {
      const ticketLabels = (ticket.labels || []).filter(
        (label) => !this.predefinedLabels.includes(label),
      );
    ...
    return (
      ...
      >
        <div className={mainStyles.headerContainer}>
          <b className={mainStyles.heading}>Tickets</b>
          <Button onClick={this.reload} style={{marginLeft: 5}} disabled={pending}>
            Refresh
          </Button>
          <Button type="primary" onClick={this.openNewTicketModal} style={{marginLeft: 5}}>
            Create new ticket
          </Button>
        </div>
        {error && <Alert title={`Error retrieving tickets: ${error}`} type="error" />}
        {!error && (
          <div className={styles.ticketsTable}>
            {this.renderTableHeader()}
            <Spin spinning={pending} classNames={{root: styles.tableSpin}}>
