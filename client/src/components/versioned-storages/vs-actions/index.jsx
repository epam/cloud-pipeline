...
import {CheckoutDialog, GitCommitDialog, ConflictsDialog} from './components';
...
class VSActions extends React.Component {
  ...
  get isDtsEnvironment() {
    ...
    return run && run.executionPreferences && run.executionPreferences.environment === 'DTS';
  }

  get fsBrowserAvailable() {
    const {run} = this.props;
    return (
      run && run.initialized && run.podIP && /^running$/i.test(run.status) && !this.isDtsEnvironment
    );
  }
  ...
  fetchRepositoryStatus = (repository) =>
    new Promise((resolve) => {
      ...
      this.setState(
        {
          ...
          storagesStatuses: this.repositories.reduce(
            (acc, cur) => ({
              ...acc,
              [cur.id]: {
                pending: true,
        ...
        () => {
          Promise.all(this.repositories.map(this.fetchRepositoryStatus)).then((payloads) => {
            const storagesStatuses = payloads.reduce(
              (acc, cur) => ({
                ...acc,
                ...cur,
              }),
  ...
  pushOperation = (operation, promise) => {
    ...
    (promise || operation.fetchUntilDone()).catch(console.error).then(() => {
  ...
  performRequestWithStatus = (request, resolve, onConflicts = undefined, options = {}) => {
    request
      .send()
      .then(() => {
        if (request.error) {
    ...
    if (versionedStorage && diff) {
      ...
      this.setState({
        gitCommit: {
          ...
          files: diff,
          mergeInProgress:
            storagesStatuses && storagesStatuses[versionedStorage.id]
  ...
  doCommit = (versionedStorage, commitMessage, filesToCommit = []) => {
    return new Promise((resolve) => {
      ...
      this.performRequestWithStatus(
        new VSCommit(this.props.run?.id, id, commitMessage, filesToCommit),
        resolve,
        this.onConflictsDetected,
        {
          hide,
  ...
  onConflictsDetected = (conflicts, storage, mergeInProgress) => {
    this.fetchRepositoryStatus(storage)
      ...
      .then((status) => {
        ...
        if (status && Object.hasOwn(status, storage.id)) {
          storagesStatuses = {...(storagesStatuses || {}), ...status};
        }
        let filesInfo = [];
        if (storagesStatuses && Object.hasOwn(storagesStatuses, storage.id)) {
  ...
  onAbortChanges = () => {
    ...
    const {run} = this.props;
    const {storage, mergeInProgress} = conflicts;
    if (!mergeInProgress) {
      ...
      () => {
        ...
        Promise.all(
          Object.entries(files || {}).map(([file, content]) =>
            resolveFileConflict(run?.id, storage?.id, file, content),
          ),
        )
          .then(() => {
            hide();
  ...
  onResolveConflictsVS = (storage) => {
    const {storagesStatuses} = this.state;
    if (storagesStatuses && Object.hasOwn(storagesStatuses, storage?.id)) {
      const {files = [], merge_in_progress: mergeInProgress} = storagesStatuses[storage?.id];
      this.onConflictsDetected(
    ...
    this.setState(
      ...
      () => {
        if (onDropDownVisibleChange) {
          onDropDownVisibleChange(this.state.dropDownVisible);
        }
      },
    );
  };
  ...
  getOverlayMenuItems = () => {
    ...
    storages.forEach((storage, index, array) => {
      const status = Object.hasOwn(storagesStatuses, storage.id)
      ...
      const saveEnabled =
        ...
        !pending &&
        !mergeInProgress &&
        ((hasModifications && !hasConflicts) || unsaved);
        ...
        },
        {
          ...
          label: (
            <span>
              Save
              {storage.detached && (
                <span style={{marginLeft: 5}}>(current revision is not the latest)</span>
              )}
            </span>
          ),
        },
  ...
  render() {
    ...
    const {dropDownVisible, gitDiff, gitCheckout, gitCommit, conflicts} = this.state;
