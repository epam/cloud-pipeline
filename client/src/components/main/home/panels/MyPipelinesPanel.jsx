...
class MyPipelinesPanel extends localization.LocalizedReactComponent {
  ...
  get pipelines() {
    if (
      ...
      result.sort((pA, pB) => {
        const pAisOwner =
          pA.owner &&
          pA.owner.toLowerCase() === this.props.authenticatedUserInfo.value.userName.toLowerCase();
        const pBisOwner =
          pB.owner &&
          pB.owner.toLowerCase() === this.props.authenticatedUserInfo.value.userName.toLowerCase();
  ...
  renderPipeline = (pipeline, search) => {
    return [
      ...
      >
        <span type="main">{highlightText(pipeline.name, search)}</span>
      </Row>,
      <Row
