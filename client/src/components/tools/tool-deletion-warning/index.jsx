...
class ToolDeletionWarning extends React.Component {
  ...
  renderGroupAlert = () => {
    const {name, groupAlertConfirmed} = this.state;
    ...
    return (
      <Row>
        <Row>
          Group <b>{name || ''}</b> has child tools, do you want to delete them?
        </Row>
        <Row
          style={{
            fontSize: 12,
        ...
        >
          <Checkbox checked={groupAlertConfirmed} onChange={onConfirmChanged}>
            Delete child tools
          </Checkbox>
        </Row>
      </Row>
        ...
        <Row style={{marginTop: 5, marginBottom: 5}}>
          <Row>
            ...
            <div style={{display: 'inline-block', marginLeft: 5}}>
              (
              {runs.map(({id}, index) => (
                <span key={id} style={{marginLeft: index > 0 ? 5 : 0}}>
                  #{id}
                </span>
              ))}
              {total > runs.length ? (
          ...
          <Row>
            While the operation will succeed - this may cause unexpected behavior for the users
            running the jobs.
          </Row>
        </Row>
      );
  ...
  render() {
    ...
    return (
      <Modal
        ...
        footer={
          <Row type="flex" align="center" justify="end">
            <Button id="remove-button-cancel" onClick={resolve ? () => resolve(false) : undefined}>
              Cancel
            </Button>
            <Button
      ...
      >
        ...
        <Row>This operation cannot be undone.</Row>
      </Modal>
    );
  }
}
...
async function deleteToolConfirm({registry, group, tool: toolId, version, link}, router) {
  const dockerImages = [];
  ...
  return await toolDeletionWarningDialog.display(title, runs, {
    error,
    showGroupAlert,
    name,
    total,
    onNavigateToRuns: navigate,
  });
}
