...
import styles from './cwl-tools-repository.module.css';
...
class CWLToolsRepository extends React.Component {
    ...
    if (dockerRegistries.loaded) {
      ...
      for (let r = 0; r < registries.length; r++) {
        ...
        groups
          .filter(
            (aGroup) =>
              toolGroupIds.includes(aGroup.id) ||
  ...
  renderTool = (tool) => {
    const {id, image: toolImage = '', iconId, shortDescription} = tool;
    const image = toolImage.split('/').pop();
    return (
      <div
        key={id}
        className={classNames(styles.toolRow, 'cp-panel')}
        draggable
        onClick={this.onToolClick(tool)}
        onDragStart={this.onDragStart(tool)}
      >
        {iconId && <img className={styles.toolIcon} src={ToolImage.url(id, iconId)} />}
  ...
  renderContent = () => {
    const {dockerRegistries} = this.props;
    const {loaded, pending, error} = dockerRegistries;
    if (!loaded && pending) {
      return <LoadingView />;
    }
    if (error) {
    ...
    const {tools = []} = this;
    if (tools.length === 0) {
      return <Alert title="Tools not found" type="info" />;
    }
    const {search} = this.state;
    const filtered = tools.filter(
      (aTool) =>
        !search || !search.length || aTool.image.toLowerCase().includes(search.toLowerCase()),
    );
    ...
    return <div className={styles.tools}>{filtered.map(this.renderTool)}</div>;
  };
  ...
  render() {
    ...
    const {search} = this.state;
    return (
      <div className={classNames(className, styles.cwlToolsRepositoryContainer)} style={style}>
        <div className={styles.searchRow}>
          <Input style={{width: '100%'}} value={search} onChange={this.onChangeSearch} />
        </div>
        {this.renderContent()}
      </div>
    );
