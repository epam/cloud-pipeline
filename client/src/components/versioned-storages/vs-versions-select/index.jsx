...
import styles from './vs-versions-select.module.css';
    ...
    request
      ...
      .then(() => {
        if (request.error) {
          ...
          const {listing = [], has_next: hasMore} = request.value;
          resolve({
            hasMore,
...
class VSVersions extends React.Component {
      ...
      this.setState(
        ...
        () => {
          loadVersions(repository, page)
            .then((payload) => {
              const {versions: currentVersions = []} = this.state;
              const {hasMore, versions} = payload || {};
              this.setState({
                pending: false,
                error: undefined,
  ...
  fetchRepositoryVersions = () => {
    this.setState(
      ...
      () => {
        const {repository} = this.props;
        if (repository) {
          this.appendPage(0).then(() => {
            const {versions = []} = this.state;
            const {value: version, setFirstVersionByDefault} = this.props;
            if (setFirstVersionByDefault && !version && versions.length > 0) {
              const {onChange} = this.props;
              if (onChange) {
                onChange(versions[0].commit);
              }
            }
          });
        }
  ...
  render() {
    const {className, value, onChange, style, rowClassName, popupMatchSelectWidth} = this.props;
    const {versions, hasMore, pending, error} = this.state;
    if (error) {
        ...
        {versions.map((version) => (
          ...
          >
            <div className={styles.commit}>
              <UserName userName={version.author} />
              <span style={{marginRight: 5}}>,</span>
              <span>{displayDate(version.committer_date, 'MMM D, YYYY, HH:mm')}</span>
              <span style={{marginRight: 5}}>:</span>
              <span className={styles.accent}>{version.commit_message}</span>
            </div>
          </Select.Option>
        ))}
        {hasMore && (
