...
class LocalizedPath:
    ...
    def __init__(self, path, s3_path, local_path, type, prefix=None):
        self.path = path
...
class InputDataTask:
                ...
                if param_type in parameter_types:
                    ...
                    for path in original_paths:
                        ...
                        if self.match_dts_path(resolved_path, dts_registry):
                            ...
                            paths.append(self.build_s3_path(resolved_path, param_type))
        ...
        for prefix in dts_registry:
            if path.startswith(prefix):
                ...
                Logger.info('Found remote {} path {} matching DTS prefix {}. '
                            ...
                            .format(input_type, path, prefix, s3_path,
                                    'from' if input_type == ParameterType.OUTPUT_PARAMETER else 'to',
                                    local_path), task_name=self.task_name)
                return LocalizedPath(path, s3_path, local_path, PathType.DTS, prefix=prefix)
    ...
    def create_dts_path(self, path, upload, rules):
        return LocalToS3(path.path, path.s3_path, rules) if upload else S3ToLocal(path.s3_path, path.path, rules)
    ...
    def is_file(self, source):
        ...
        if self.match_s3_path(source):
            ...
            s3_paths = S3Bucket().pipe_ls(get_path_without_trailing_delimiter(source),
    ...
    def fetch_source_files(self, source):
        ...
        """
        if self.match_s3_path(source):
            s3_paths = S3Bucket().pipe_ls(get_path_with_trailing_delimiter(source),
                                          TRANSFER_ATTEMPTS, recursive=True, all=True, show_info=True)
            s3_paths = filter(lambda x: x[0] == 'File', s3_paths)
            files = [File(self.get_path_without_folder(source, path[1]), int(path[2])) for path in s3_paths]
        else:
            files = []
