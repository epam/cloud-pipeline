...
export default class FireCloudBrowser extends React.Component {
  ...
  get currentMethodIsSelected() {
    return (
      this.props.namespace === this.state.selectedNameSpace &&
  ...
  get currentMethod() {
    if (this.state.selectedMethod) {
      return this.methods
        .filter(
          (m) =>
            m.name === this.state.selectedMethod && m.namespace === this.state.selectedNameSpace,
        )
        .shift();
    }
    return null;
  ...
  get snapshots() {
    if (this.currentMethod) {
      return (this.currentMethod.snapshotIds || []).map((id) => id);
    }
    return [];
  }
  ...
  render() {
    ...
    if (this.state.selectedMethod) {
      ...
      content = (
        <FireCloudMethodSnapshotConfigurations
    ...
    const googleInfoContent = this.props.googleApi.userAuthEnabled ? (
      <Row type="flex" justify="end" align="middle">
        ...
        <Button onClick={this.props.googleApi.signOut} size="small">
          Sign Out
        </Button>
      </Row>
    ) : undefined;
    ...
    if (this.state.selectedMethod) {
      contentHeader = (
        <Row type="flex" justify="space-between" align="middle">
          ...
          {googleInfoContent && <Col>{googleInfoContent}</Col>}
        </Row>
      );
    } else {
      contentHeader = (
