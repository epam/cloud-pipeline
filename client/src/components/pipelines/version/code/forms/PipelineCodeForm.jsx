...
import styles from './PipelineCodeForm.module.css';
...
class PipelineCodeForm extends React.Component {
    ...
    if (pipelineId && version && path) {
      this.setState(
        ...
        async () => {
          ...
          try {
            const {content, binary, error} = await parsePipelineFile(
              pipelineId,
              version,
              path,
              byteLimit,
  ...
  render() {
    const {showRevision, download, path, name, visible} = this.props;
    const {pending} = this.state;
    const title = path ? (
      <Row type="flex" justify="space-between">
        {name ? <Col>{name}</Col> : null}
        <Col>{showRevision ? `At revision ${this.props.version}:` : ''}</Col>
        <Col>
      ...
      >
        <div className={styles.spinContainer}>
          <Spin spinning={pending}>
            <div
              className={classNames(styles.editorContainer, styles.tableEditor, {
