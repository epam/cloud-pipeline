...
import AddDockerRegistryControl from './add-docker-registry-control/index.ts';
...
import styles from './edit-hot-node-pool.module.css';
...
class EditHotNodePool extends React.Component {
  ...
  updateFromProps = () => {
    ...
    if (pool) {
      ...
      const {scheduleEntries: schedule = []} = scheduleObj;
      this.setState(
        {
          disk,
          initialDisk: disk,
            ...
            .then(() => {
              if (this.state.region === region && this.state.spot === spot) {
                if (request.loaded) {
                  ...
                  const priceTypes = (payload[priceTypesKey] || ['SPOT', 'ON_DEMAND']).map((o) =>
                    o.toUpperCase(),
                  );
  ...
  getValidationErrors = () => {
    ...
    const filtersError = getFiltersError(filters);
    if (autoscaled) {
      if (!minSize || Number.isNaN(minSize) || Number(minSize) < 1) {
        minSizeError = 'Min Size should be a positive integer larger than or equal 1';
      }
      if (!maxSize || Number.isNaN(maxSize) || Number(maxSize) < 1) {
        maxSizeError = 'Max Size should be a positive integer larger than or equal 1';
      }
      if (!Number.isNaN(minSize) && !Number.isNaN(maxSize) && Number(minSize) > Number(maxSize)) {
        if (!minSizeError) {
      ...
      if (Number.isNaN(scaleStep) || Number(scaleStep) < 1) {
        scaleStepError = 'Scale Step should be a positive integer larger than or equal 1';
      }
    } else {
      if (Number.isNaN(Number(count)) || Number(count) < COUNT_MIN_SIZE) {
  ...
  isModified = () => {
    ...
    return (
      Number(disk) !== Number(initialDisk) ||
  ...
  onSave = () => {
    ...
    if (!valid) {
      ...
      const payload = {
        ...
        instanceDisk,
        dockerImages: dockerImages.filter((image) => !image.removed).map((image) => image.image),
        instanceType,
        name,
        priceType,
        regionId,
  ...
  onChangeAutoScaled = (e) => {
    const {count, touched, scaleDownThreshold, scaleUpThreshold, scaleStep} = this.state;
  ...
  renderHintControl = (hint) => {
    if (hint) {
      return (
        <Popover content={hint} placement="right">
  ...
  renderScheduleControls = () => {
    const {validation, schedule, touched} = this.state;
    return (
      <div>
        {schedule.map((s) => (
          <div key={s.id}>
        ...
        <div style={{marginTop: 5}}>
          <Button onClick={this.onAddSchedule} type="dashed">
            <PlusOutlined />
            Add schedule
          </Button>
        </div>
  ...
  renderNameControl = () => {
    ...
    const {validation, touched, name} = this.state;
    return (
      <div>
        <div className={styles.formRow}>
          <span className={styles.label}>Pool name:</span>
          <div className={classNames(styles.formItem, styles.small)}>
            <Input disabled={disabled} value={name} onChange={this.onChangeName} />
          </div>
        </div>
  ...
  renderAutoScaledControl = () => {
    ...
    return (
      <div>
        <div className={styles.formRow}>
          <span className={styles.label}>Autoscaled:</span>
          <Checkbox
  ...
  renderNodeCountControl = () => {
    const {disabled} = this.props;
    const {autoscaled, validation, touched, count} = this.state;
    if (autoscaled) {
      return null;
    }
    return (
      <div>
        <div className={styles.formRow}>
          <span className={styles.label}>Nodes count:</span>
          <InputNumber
            className={classNames(styles.formItem, styles.small)}
            disabled={disabled}
            value={count}
  ...
  renderMinSizeControl = () => {
    const {disabled} = this.props;
    const {autoscaled, validation, touched, minSize} = this.state;
    if (!autoscaled) {
      return null;
    }
    return (
      <div style={{paddingLeft: 10}}>
        <div className={styles.formRow}>
          <span className={styles.label}>Min Size:</span>
          <InputNumber
            className={classNames(styles.formItem, styles.small)}
            disabled={disabled}
            value={minSize}
  ...
  renderMaxSizeControl = () => {
    const {disabled} = this.props;
    const {autoscaled, validation, touched, maxSize} = this.state;
    if (!autoscaled) {
      return null;
    }
    return (
      <div style={{paddingLeft: 10}}>
        <div className={styles.formRow}>
          <span className={styles.label}>Max Size:</span>
          <InputNumber
            className={classNames(styles.formItem, styles.small)}
            disabled={disabled}
            value={maxSize}
  ...
  renderScaleDownThresholdControl = () => {
    const {disabled} = this.props;
    const {autoscaled, validation, touched, scaleDownThreshold} = this.state;
    if (!autoscaled) {
      return null;
    }
    return (
      <div style={{paddingLeft: 10}}>
        <div className={styles.formRow}>
          <span className={styles.label}>Scale Down Threshold:</span>
          <InputNumber
            className={classNames(styles.formItem, styles.small)}
            disabled={disabled}
  ...
  renderScaleUpThresholdControl = () => {
    const {disabled} = this.props;
    const {autoscaled, validation, touched, scaleUpThreshold} = this.state;
    if (!autoscaled) {
      return null;
    }
    return (
      <div style={{paddingLeft: 10}}>
        <div className={styles.formRow}>
          <span className={styles.label}>Scale Up Threshold:</span>
          <InputNumber
            className={classNames(styles.formItem, styles.small)}
            disabled={disabled}
  ...
  renderScaleStepControl = () => {
    const {disabled} = this.props;
    const {autoscaled, validation, touched, scaleStep} = this.state;
    if (!autoscaled) {
      return null;
    }
    return (
      <div style={{paddingLeft: 10}}>
        <div className={styles.formRow}>
          <span className={styles.label}>Scale Step:</span>
          <InputNumber
            className={classNames(styles.formItem, styles.small)}
            disabled={disabled}
            value={scaleStep}
  ...
  renderInstanceImageControl = (readOnly) => {
    const {disabled} = this.props;
    const {validation, touched, instanceImage} = this.state;
    return (
      <div>
        <div className={styles.formRow}>
          <span className={styles.label}>AMI:</span>
          <div className={classNames(styles.formItem, styles.small)}>
            <Input
  ...
  renderInstanceTypeControl = (readOnly) => {
    const {disabled} = this.props;
    const {validation, instanceType, touched, allowedInstanceTypes, allowedInstanceTypesPending} =
      this.state;
    return (
      <div>
        <div className={styles.formRow}>
          <span className={styles.label}>Instance Type:</span>
          <Select
            ...
            {this.instanceTypes.map((group) => (
              <Select.OptGroup key={group.family} label={group.family}>
                {group.instances.map((instance) => (
                  <Select.Option key={instance.sku} value={instance.name} title={instance.name}>
                    <InstanceDetails instance={instance.name} instances={allowedInstanceTypes} />
                  </Select.Option>
                ))}
              </Select.OptGroup>
            ))}
  ...
  renderSpotControl = (readOnly) => {
    const {disabled} = this.props;
    const {spot, allowedInstanceTypesPending} = this.state;
    return (
      <div>
        <div className={styles.formRow}>
  ...
  renderRegionControl = (readOnly) => {
    const {disabled} = this.props;
    const {validation, region, touched} = this.state;
          ...
          >
            {data.map(({provider, regions}) => (
              <Select.OptGroup key={provider} label={provider}>
                {regions.map((region) => (
                  <Select.Option key={region.id} value={`${region.id}`} title={region.name}>
  ...
  renderDiskControl = (readOnly) => {
    const {disabled} = this.props;
    const {validation, disk, touched} = this.state;
    return (
      <div>
        <div className={styles.formRow}>
  ...
  renderDockerImagesControl = (readOnly) => {
    const {disabled} = this.props;
    const {validation, dockerImages, touched} = this.state;
    const images = (dockerImages || [])
      .filter((o) => !o.removed)
      .map((o) => (o.image || '').trim())
      .filter(Boolean);
    ...
    return (
      <div>
        <div className={classNames(styles.formRow, styles.multiRow)}>
          <span className={styles.label}>Docker images:</span>
          <div className={styles.column}>
            ...
            {!readOnly && (
              <div>
                <Button disabled={disabled} onClick={this.onAddDockerImage} type="dashed">
                  <PlusOutlined />
                  Add docker image
                </Button>
              </div>
            )}
            {readOnly && filteredDockerImages.length === 0 ? <span>Not specified</span> : null}
          </div>
        </div>
  ...
  renderLabelsControl = (readOnly) => {
    const {disabled} = this.props;
    const {validation, labels = []} = this.state;
    ...
    const renderLabelControl = (label) => {
      const duplicate =
        labels.filter(
          (lb) => (lb.name || '').trim().toLowerCase() === (label.name || '').trim().toLowerCase(),
        ).length > 1;
      ...
      if (readOnly) {
        return (
          <div key={label.key} style={st}>
            <span style={{flexShrink: 0}}>{label.name}:</span>
            <span
              style={{
                flexShrink: 1,
                overflow: 'auto',
            ...
            {label.monitored && <span style={{flexShrink: 0}}>(monitored)</span>}
          </div>
        );
      }
      return (
      ...
      <div>
        <div className={classNames(styles.formRow, styles.multiRow)}>
          ...
          <div className={styles.column}>
            ...
            {!readOnly && (
              <div>
                <Button disabled={disabled} onClick={addNodeLabel} type="dashed">
                  <PlusOutlined />
                  Add node label
                </Button>
              </div>
  ...
  renderFiltersControl = () => {
    const {disabled} = this.props;
    const {validation, filters, touched} = this.state;
    return (
      <Provider roles={this.roles}>
        <div style={{marginTop: 5}}>
          <div>
            <FiltersControl disabled={disabled} filters={filters} onChange={this.onChangeFilters} />
          </div>
  ...
  render() {
    const {onCancel, pool, visible, disabled} = this.props;
    const isNew = !pool;
    return (
      <Modal
        ...
        footer={
          <div className={styles.modalFooter}>
            ...
            <Button disabled={disabled} onClick={this.onSave} type="primary">
              {isNew ? 'CREATE' : 'SAVE'}
            </Button>
          </div>
        }
