...
import styles from './hcs-image-analysis.module.css';
...
const HcsAnalysisTabName = {
  [HcsAnalysisTabs.analysis]: (
    <span>
      <PlayCircleFilled /> Analysis
    </span>
  ),
  [HcsAnalysisTabs.batch]: (
    <span>
      <SwitcherOutlined /> Evaluations
    </span>
  ),
};
...
class HcsImageAnalysis extends React.Component {
  ...
  get activeTab() {
    const {batchMode, toggleBatchMode} = this.props;
  ...
  get currentJobId() {
    const {batchJobId, toggleBatchMode} = this.props;
  ...
  renderAnalysis() {
    const {hcsAnalysis, expandSingle, onToggleResults, resultsVisible} = this.props;
    return (
      <CellProfiler
  ...
  render() {
    ...
    return (
      <div
        className={classNames(styles.container, className, 'cp-panel', 'cp-panel-borderless')}
        style={style}
      >
        <Tabs
...
export default inject('hcsAnalysis')(roleModel.authenticationInfo(observer(HcsImageAnalysis)));
