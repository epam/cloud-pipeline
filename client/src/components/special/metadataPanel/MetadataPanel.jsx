...
import MetadataStyles from '../metadata/Metadata.module.css';
...
@inject((args, params) => ({
  ...
  entityId:
    params.currentItem && params.currentItem.rowKey ? params.currentItem.rowKey.value : null,
}))
@observer
export default class MetadataPanel extends React.Component {
  ...
  saveMetadata =
    ...
    async () => {
      ...
      if (field === 'value') {
        ...
        for (const k in this.props.currentItem) {
  ...
  confirmDeleteMetadata = () => {
    Modal.confirm({
      ...
      cancelText: 'Cancel',
      onOk: async () => {
        const request = new MetadataEntityDelete(this.props.entityId);
        await request.fetch();
        if (request.error) {
  ...
  confirmDeleteKey = (item) => {
    Modal.confirm({
      ...
      cancelText: 'Cancel',
      onOk: async () => {
        const request = new MetadataEntityDeleteKey(this.props.entityId, item);
        await request.fetch();
        if (request.error) {
          message.error(request.error, 5);
        } else {
  ...
  renderAddKeyRow = () => {
    if (this.state.addKey) {
      ...
      const refKeyInput = (input) => {
        if (this.state.addKey && !this.state.addKey.autofocused && input && input.focus) {
          input.focus();
      ...
      return [
        ...
        <tr className={MetadataStyles.newKeyRow} key="new key title row">
          <td colSpan={3} style={{textAlign: 'right'}}>
            ...
            <Button id="cancel-add-metadata-item-button" size="small" onClick={addKeyCancelClicked}>
  ...
  renderTableHeader = () => {
    ...
    const renderActions = () => {
      ...
      if (this.props.currentItem && !this.state.addKey) {
        actions.push(
          <Button id="add-key-button" key="add button" size="small" onClick={addKeyClicked}>
            <PlusOutlined /> Add
          </Button>,
        );
      }
  ...
  renderDivider = (key, span) => {
    return (
      <tr key={key}>
        <td colSpan={span || 3}>
          <div className={classNames(MetadataStyles.divider, 'cp-divider', 'horizontal')} />
        </td>
      </tr>
    );
  };
  ...
  renderMetadataItem = (metadataItem) => {
    ...
    for (const key in metadataItem) {
      let isReadOnlyItem = this.props.readOnly || (this.props.readOnlyKeys || []).indexOf(key) >= 0;
      if (key !== 'rowKey' && metadataItem[key]) {
        ...
        if (this.state.editableKey === key && (this.props.readOnlyKeys || []).indexOf(key) === -1) {
          valueElement.push(
            <tr key={`${key}_key`} className={classNames('cp-metadata-item-row', 'key')}>
              <td colSpan={6}>
          ...
          valueElement.push(
            <tr
              key={`${key}_key`}
              className={classNames('cp-metadata-item-row', 'key', {
            ...
            >
              <td
                id={`key-column-${key}`}
                colSpan={
                  this.props.readOnly || (this.props.readOnlyKeys || []).indexOf(key) >= 0 ? 6 : 5
                }
              ...
              {this.props.readOnly ||
              (this.props.readOnlyKeys || []).indexOf(key) >= 0 ? undefined : (
        ...
        if (runsValue) {
          valueElement.push(
            <tr key={`${key}_value`} className={classNames('cp-metadata-item-row', 'value')}>
              <td colSpan={6}>
                <RunsAttribute value={value} />
              </td>
            </tr>,
          );
        } else if (
          ...
          valueElement.push(
            <tr key={`${key}_value`} className={classNames('cp-metadata-item-row', 'value')}>
              <td colSpan={6}>
                <Input.TextArea {...inputOptions('value', key, metadataItem[key].value)} autoSize />
              </td>
            </tr>,
          );
        } else {
  ...
  renderEmptyPlaceholder = () => {
    if (!this.props.currentItem) {
      return (
        <tr style={{height: 40}} className="cp-text-not-important">
    ...
    return (
      <div style={{flex: 1, display: 'flex', flexDirection: 'column'}}>
        <table key="header" style={{width: '100%'}}>
          ...
          <tbody>{this.renderAddKeyRow()}</tbody>
        </table>
