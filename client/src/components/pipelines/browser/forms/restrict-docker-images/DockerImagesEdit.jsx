...
import AddDockerRegistryControl from '../../../../../components/cluster/hot-node-pool/add-docker-registry-control/index.ts';
import styles from './RestrictDockerDialog.module.css';

const getImageName = (tool) =>
  tool.registry && tool.image ? `${tool.registry}/${tool.image}` : '';

class DockerImagesEdit extends React.Component {
  get toolsToMountImages() {
    const {toolsToMount} = this.props;
    return (toolsToMount || []).map((tool) => getImageName(tool));
  }
  ...
  renderDockerImage = (tool) => {
    ...
    return (
      <div className={styles.imageRow} key={image || `tool-${index}`}>
  ...
  render() {
    const {disabled, toolsToMount = []} = this.props;
    return (
