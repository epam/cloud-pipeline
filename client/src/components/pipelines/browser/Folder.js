...
import DataStorageEditDialog from './forms/DataStorageEditDialog';
...
export default class Folder extends localization.LocalizedReactComponent {
  ...
  createStorage = async (storage) => {
    ...
    await request.send({
      ...
      path: path,
      shared: storage.type === 'S3' && storage.shared,
      storagePolicy: {
      ...
      regionId: storage.type === 'S3' && storage.regionId ? storage.regionId : undefined
    });
    hide();
    if (request.error) {
      message.error(request.error, 5);
  ...
  editStorage = async (storage) => {
    ...
    if (request.error) {
      ...
      if (storage.type !== 'NFS' && (storage.longTermStorageDuration !== undefined ||
