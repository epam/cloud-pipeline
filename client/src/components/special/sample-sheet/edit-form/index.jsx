...
class SampleSheetEditForm extends React.Component {
  ...
  componentDidUpdate(prevProps, prevState, snapshot) {
    if (prevProps.content !== this.props.content || prevProps.editable !== this.props.editable) {
      this.updateFromProps();
    }
  }
  ...
  onChange = () => {
    const {header, sections = [], data} = this.state;
  ...
  render() {
    const {className, style, editable} = this.props;
    const {header, sections = [], data, error} = this.state;
    const {name: dataSectionName = 'Data', data: samplesData = []} = data || {};
    if (error) {
      return (
      ...
      <div className={className} style={style}>
        ...
        {editable && (
          <div className={styles.actionsRow}>
            <Button className={styles.addSection} onClick={this.onAddSection}>
              <PlusOutlined /> Add section
            </Button>
          </div>
        )}
        <SamplesEditor data={samplesData} onChange={this.onChangeData} editable={editable} />
      </div>
    );
  }
}
