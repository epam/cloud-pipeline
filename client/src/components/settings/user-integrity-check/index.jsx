...
import {build, buildPatterns, generatePatternSorter, getDefaultColumns, update} from './links';
import styles from './user-integrity-check.module.css';
...
class UserIntegrityCheck extends React.Component {
  ...
  get filteredUsers() {
    ...
    return (users || []).filter((user) => {
      ...
      const neverLoggedInFilter = !filters.includes(FILTERS.neverLoggedIn) || user.firstLoginDate;
  ...
  get columns() {
    const {columns = [], defaultColumns = [], showFullMetadata} = this.state;
    if (showFullMetadata) {
      return columns;
    }
  ...
  isModifiedValue = (userId, key) => {
    ...
    const isEmpty = (o) => !o;
    return (
      isEmpty(userData[key]) !== isEmpty(initialUserData[key]) ||
  ...
  fetchUsersAttributes = () => {
    ...
    this.setState(
      ...
      () => {
        systemDictionariesRequest.fetchIfNeededOrWait().then(() => {
          loadUsersMetadata(users.map((user) => user.id))
            .then((metadata) => {
              ...
              const usersMetadata = users
                .map((user) => {
                  const userMetadata = (metadata || []).find((m) => m.entity?.entityId === user.id);
                  const data = userMetadata ? userMetadata.data : {};
                  attributes.push(...Object.keys(data || {}));
                  return {
                ...
                .reduce((r, c) => ({...r, ...c}), {});
              const attributesToDisplay = [...new Set(attributes)].sort((a, b) => {
              ...
              let defaultColumns = [
                ...new Set(
                  (defaultAttributesToDisplay || []).concat(
                    getDefaultColumns(patterns, usersMetadata),
                  ),
                ),
              ];
  ...
  onFieldChange = (opts) => {
    ...
    const {data: newData, paths: newPaths} = update(opts, metadata, this.dictionaries, paths);
    this.setState({
      data: newData,
      paths: newPaths,
    });
  ...
  saveNewValuesToDictionaries = () => {
    ...
    return new Promise((resolve, reject) => {
      const promises = Object.entries(newDictionaryValues).map(([dictionary, newValues]) =>
        addValueToSystemDictionary(this.getSystemDictionary(dictionary), newValues),
      );
      Promise.all(promises)
        ...
        .then(() => {
          ...
          this.dictionaries
            .filter((dictionary) =>
              dictionary.linksTo.find((link) => modifiedDictionaries.includes(link)),
            )
            .forEach((dictionary) => {
              ...
              const {values = []} = modifiedDictionary;
              const {key, linksTo = []} = dictionary;
              for (let v = 0; v < values.length; v++) {
                const {value, links = []} = values[v];
                const usersData = Object.values(data).filter((ud) => ud[key] === value);
                ...
                for (let l = 0; l < missingLinks.length; l++) {
                  ...
                  const linkDictionaryValue = (linkDictionary.values || []).find(
                    (v) => userData && v.value === userData[linkKey],
                  );
                  if (linkDictionaryValue) {
                    ...
                    console.info(
                      `Creating new link ${key}:${value} -> ${linkKey}:${linkDictionaryValue.value}`,
                    );
      ...
      this.setState({actionInProgress: true}, () => {
        ...
        this.saveNewValuesToDictionaries()
          .then(() => {
            ...
            return Promise.all(
              usersWithChanges.map((userId) => {
                const user = data[userId] || {};
                const userPayload = Object.entries(user).reduce((acc, [key, value]) => {
                  if (value) {
                    acc[key] = {value};
                  }
                  return acc;
  ...
  renderTableContent = () => {
    ...
    const {currentPage, data: metadata = {}, actionInProgress} = this.state;
      ...
      return data.map((user) => {
        ...
        return (
          <tr key={user.userName}>
            <th key="userName">
              <div
                className={classNames(styles.userNameCellContainer, {
                    ...
                    overlay={
                      <ul style={{listStyle: 'disc', paddingLeft: 20}}>
                        {userErrors.map((error, index) => (
                          <li key={`${error.user.userName}-error-${index}`}>{error.error}</li>
                        ))}
                      </ul>
                    }
                  >
                ...
                return (
                  <td key={column}>
                    <div className={styles.cell}>
                      <AutoComplete
                        className={classNames('cp-settings-users-integrity-check', {
                          modified: this.isModifiedValue(user.id, column),
                          new: this.isNewValue(dictionary, userMetadata[column]),
                        })}
                        size="large"
                        style={{flex: 1}}
                        allowClear
                        ...
                        value={
                          Object.hasOwn(userMetadata, column) ? userMetadata[column] : undefined
                        }
                      ...
                      >
                        ...
                        {(dictionary.values || []).map((value) => (
                          <AutoComplete.Option key={value.id} value={value.value || ''}>
                          ...
                          overlay={
                            <div>
                              ...
                              <div className={styles.linkDescription}>
                                <span>
                                  {fieldParentLink.from}:{fieldParentLink.fromValue}
                                </span>
                                <CaretRightOutlined />
                                <span>
                                  {fieldParentLink.to}:{fieldParentLink.toValue}
                                </span>
                              </div>
                            </div>
                          }
              ...
              return (
                <td key={column}>
                  <Input
                    ...
                    size="large"
                    style={{width: '100%'}}
                    value={Object.hasOwn(userMetadata, column) ? userMetadata[column] : undefined}
                    onChange={(e) =>
                      this.onFieldChange({
                        id: user.id,
  ...
  renderContent = () => {
    ...
    return [
      <div key="filter" className={styles.filters}>
      ...
      <div key="table" tabIndex={0} className={styles.tableContainer} onScroll={blur}>
  ...
  render() {
    const {visible, onClose, users, mode} = this.props;
    const {defaultColumns, columns, showFullMetadata, currentPage} = this.state;
    const pagesCount = Math.ceil(this.filteredUsers.length / PAGE_SIZE);
    const {actionInProgress} = this.state;
    if (/^inline$/i.test(mode)) {
      if (users.length === 0) {
        return <Alert type="success" title="No issues found" showIcon />;
      }
      return (
    ...
    if (users.length === 0) {
      return (
        <Modal title="Integrity check results" footer={false} onCancel={onClose} open={visible}>
          <Alert type="success" title="No issues found" showIcon />
        </Modal>
      );
    }
    return (
      <Modal
        ...
        footer={
          <div className={styles.footer}>
            ...
            <Button type="primary" disabled={actionInProgress} onClick={this.onSave}>
              SAVE
            </Button>
          </div>
        }
