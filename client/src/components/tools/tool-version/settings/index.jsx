...
export default class ToolSetttings extends React.Component {
  ...
  get toolVersionOS() {
    ...
    if (versions.loaded) {
      ...
      const versionInfo = versionsInfo.find((o) => o.version === version);
      if (
        versionInfo &&
  ...
  get settings() {
    if (this.props.settings.loaded) {
      if (
        (this.props.settings.value || []).length > 0 &&
  ...
  updateTool = async (tool, configuration, allowCommit) => {
    ...
    if (updateRequest.error) {
      ...
      if (this.versionSettingsForm) {
        this.versionSettingsForm.reset();
      }
      hide();
    }
  };
  ...
  render() {
    if (
      (!this.props.settings.loaded && this.props.settings.pending) ||
