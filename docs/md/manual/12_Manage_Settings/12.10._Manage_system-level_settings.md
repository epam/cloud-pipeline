...
| **`cluster.node.unavailable.grace.period.minutes`** | How long a node that stopped reporting its state is waited for before it is scaled down and its run fails (in minutes, `30` by default, `0` or more). See [11.6. Node availability monitoring](../11_Manage_Runs/11.6._Node_availability_monitoring.md) |
