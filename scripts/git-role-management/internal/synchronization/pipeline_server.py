...
class PipelineServer(object):
    ...
    def synchronize(self, pipeline_ids=[], force_sync_user_ssh_keys=True):
        try:
            ...
            for pipeline in self.list_pipelines():
                if pipeline_ids is None or len(pipeline_ids) == 0 or pipeline.identifier in pipeline_ids:
                    self.synchronize_pipeline(pipeline, force_sync_user_ssh_keys)
                    print ''
    ...
    def synchronize_users(self, git_servers=[], force_sync_user_ssh_keys=True):
        try:
            self.__users__ = self.__users_api__.list()
            __users_with_email__ = filter(lambda x: x.email is not None, self.__users__)
            emails = map(lambda x: x.email.lower(), __users_with_email__)
                ...
                if git_server.initialized:
                    for user in self.__users__:
                        if user.username is not None:
                            git_server.synchronize_user(user.username, force_sync_user_ssh_keys)
        except RuntimeError as error:
            print error.message
        except KeyboardInterrupt:
            raise
    ...
    def synchronize_pipeline(self, pipeline, force_sync_user_ssh_keys):
        try:
            ...
            git_server.synchronize(pipeline, force_sync_user_ssh_keys)
        except RuntimeError as error:
            print error.message
        except KeyboardInterrupt:
            raise
