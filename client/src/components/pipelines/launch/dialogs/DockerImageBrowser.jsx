...
export default class DockerImageBrowser extends React.Component {
  ...
  get currentRegistry() {
    const [currentRegistry] =
      this.state.registry && this.props.registries
  ...
  renderRegistrySelector = () => {
    ...
    if (
      this.props.registries.filter((r) => !this.currentRegistry || r.id !== this.currentRegistry.id)
        .length === 0
    ) {
      return (
    ...
    const registries = this.props.registries
      .filter((r) => !this.currentRegistry || r.id !== this.currentRegistry.id)
  ...
  renderGroupSelector = () => {
    ...
    let groups = this.groups.filter(
      (r) => !r.privateGroup && (!this.currentGroup || r.id !== this.currentGroup.id),
    );
              ...
              {groups
                .filter(
                  (g) =>
                    ...
                    !this.state.groupSearch.length ||
                    g.name.toLowerCase().indexOf(this.state.groupSearch.toLowerCase()) >= 0,
                )
                .map((group) => {
                  return (
          ...
          .then(() => {
            if (this.state.tagsRequest === tool.id) {
              if (request.loaded) {
                ...
                let selectedTool =
                  this.state.registry && this.state.shorthandNotation !== this.state.registry
          ...
          if (toolIsSelected(tool)) {
            ...
            return (
              ...
              >
                {tags.map((tag) => {
                  return (
                    <Select.Option key={tag} data={tag}>
                      {tag}
                    </Select.Option>
                  );
                })}
  ...
  UNSAFE_componentWillReceiveProps(nextProps) {
    if (
      nextProps.dockerImage !== this.props.dockerImage ||
  ...
  loadToolTags = async () => {
    ...
    const [tool] = this.tools.filter(
      (t) => t.image === `${this.currentGroup.name}/${this.state.tool}`,
    );
