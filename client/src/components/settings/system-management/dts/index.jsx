...
import {Alert, Modal, message, Button, Select} from 'antd';
...
import styles from './dts.module.css';
...
class DtsManagement extends React.Component {
  ...
  get filteredDtsList() {
    ...
    return this.dtsList.filter(
      (dts) => (dts.status || '').toLowerCase() === statusFilter.toLowerCase(),
    );
  }
    ...
    const statusIcon = (
      <svg height="10" width="10">
        <circle
          ...
          strokeWidth={1}
          className={
            (dts.status || '').toLowerCase() === 'online' ? 'cp-status-online' : 'cp-status-offline'
          }
        />
      </svg>
    );
    return (
      <div className={styles.dtsCardContainer}>
        <div style={{display: 'flex', flexWrap: 'nowrap'}}>
          <span className={classNames(styles.sectionText, styles.ellipsis)} style={{marginLeft: 5}}>
            {statusIcon} {dts.name}
          </span>
        </div>
        <span
          className={classNames(styles.sectionText, styles.notImportant, 'cp-text-not-important')}
        >
  ...
  render() {
    const {refreshToken, showCreateDtsModal, statusFilter} = this.state;
    const pending = this.pending;
    const filteredDtsList = this.filteredDtsList;
    const sections = filteredDtsList.map((dts) => ({
      key: dts.id,
    ...
    const emptySectionPlaceholder = [
      {
        ...
        name: 'empty',
        render: () => <Alert title="DTS not found." type="info" />,
      },
    ];
    ...
    const beforeListRenderer = () => (
      <div className={styles.dtsListControlsRow}>
        ...
        <Button size="small" disabled={pending} onClick={this.dtsListFetch}>
          <ReloadOutlined />
        </Button>
      </div>
    );
    return (
      <div style={{height: '100%'}}>
        ...
        />
        <CreateDtsModal visible={showCreateDtsModal} onClose={this.closeCreateDtsModal} />
      </div>
    );
  }
}
