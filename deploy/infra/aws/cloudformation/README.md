...
#### Create VPC
To start, create a Virtual Private Cloud (VPC) with public and private subnets. The deployment will create all necessary resources for Cloud Pipeline in this VPC both during and after the stack creation.
#### Create AWS Elastic IP
Generate an AWS Elastic IP, which will be used later to deploy an AWS Elastic Load Balancer (ELB) routing users' traffic to Cloud-Pipeline services. This EIP should also be used when requesting the creation of DNS records from your DNS provider.
#### Grant Permission to Create Infrastructure Resources
You must give permission to initiate the creation of infrastructure resources.  
1. If you have Administrator role in AWS account or role that Allowed to created resources: EC2 instance, IAM Roles/Policies, EKS cluster, security groups, cloudwatch logs, s3 buckets, efs/fsx luste file systems, kms keys, rds etc. you can use its credentials' security, access keys and token to create AWS secret with credential settings as plain text.
...
```
Then set this secret name as stack parameter `AWSCredentialSecretId`. For more information you can look [deploy parameters](#stack-parameters-description)

2. If `AWSCredentialSecretId` not set, then the additional IAM Role will be created with full administrator access and assumed to Jump-Server role during deploy process.
#### Create DNS records
...
#### (Optional) Add authority signed certificates for services
By default, if no authority-signed certificates are set, Cloud Pipeline will create self-signed certificates during the deployment. To avoid browser security certificate errors, request a single SSL/TLS certificate from the Certificate Authority (CA).:
...
> Alternative names (optional) - DNS: auth.\<cloud-pipeline-name>.\<your-domain\> - This must be set if there is no integration with organization Identity Provider.  

#### Optional: Integrate with an Identity Provider (IdP)
Although Cloud-pipeline creates a dummy IdP service suitable for testing, it is not recommended for production due to security concerns. For secure access, you should integrate your Cloud Pipeline with your organization's IdP Service. Integration is implemented with SAML2 protocol. The following IdP connections shall be requested from the IdP team:
...
#### (Optional) Create a Zip File with Additional Assets
Once all additional certificates (and optional metadata) are ready, you should create a zip archive with a specific structure. <br>
...
> Optional: <br>
> If you don't have s3 bucket you can create it in AWS Console or using AWS CLI command:<br> aws s3api create-bucket --bucket <*s3-bucket-name*>
...
Save this link for future set as parameter in AWS Console stack parameters(for more information you can look at [parameters description](#stack-parameters-description)) or as additional parameter in .json file(if create stack using awc cli) :

Link must be like: s3://*s3-bucket-name*/cp-assets.zip
...
1. Use the AWS CLI to deploy the Cloud Pipeline. Create Cloudformation stack using AWS Console and file jump-server.yaml and set parameters (see [deployment parameters](#stack-parameters-description)) or create file jump-server.json like:
```
 [
    {
...
Wait approximately 40-50 minutes until all resources and services are deployed. Then, verify the deployment by visiting https://<service\>.<user-domain-name\>/
