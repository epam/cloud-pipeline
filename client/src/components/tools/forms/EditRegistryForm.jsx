    ...
    this.formRef.current
      ...
      .then((values) => {
        if (
          ...
          this.fileInputControl.files &&
          this.fileInputControl.files[0]
        ) {
  ...
  renderCredentialsSection = () => {
    if (!this.props.registry || roleModel.isOwner(this.props.registry)) {
      ...
      if (this.state.displayCredentials) {
        ...
        return [
          <Form.Item key="user name" {...this.formItemLayout} label="User name" name="userName">
            <Input disabled={this.props.pending} />
          </Form.Item>,
          <Form.Item key="password" {...this.formItemLayout} label="Password" name="password">
            <Input type="password" autoComplete="off" disabled={this.props.pending} />
          </Form.Item>,
          <Row key="upload control" type="flex">
            <Col xs={24} sm={6} style={{textAlign: 'right', paddingRight: 10}}>
              Certificate:{' '}
            </Col>
            <Col xs={24} sm={18}>
              <input
          ...
          <Row style={{marginTop: 5, marginBottom: 5}} key="certificate link" type="flex">
            <Col xs={24} sm={6} />
            <Col xs={24} sm={18}>
              <Link id="certificate-link" onClick={this.onClickCertificateLink}>
          ...
          <Row key="pipeline auth">
            <Col xs={24} sm={6} />
            <Col xs={24} sm={18}>
              <Form.Item name="pipelineAuth" valuePropName="checked">
                <Checkbox disabled={this.props.pending}>
                  {this.localizedString('Pipeline')} authentication
                </Checkbox>
              </Form.Item>
            </Col>
          </Row>,
    ...
    const modalFooter = this.props.pending ? (
      ...
      <div className="cp-modal-footer-actions cp-modal-footer-actions--split">
        <div className="cp-modal-footer-actions-group">
          <Button danger onClick={this.props.onDelete}>
            DELETE
          </Button>
        </div>
                ...
                children: (
                  <Form
                    ...
                    initialValues={{
                      ...
                      description: this.props.registry
                        ? this.props.registry.description
                        : undefined,
                      securityScanEnabled: this.props.registry
                        ? this.props.registry.securityScanEnabled
                        : true,
                      ...
                      externalUrl: this.props.registry
                        ? this.props.registry.externalUrl
                        : undefined,
                      pipelineAuth: this.props.registry
                        ? this.props.registry.pipelineAuth
                        : undefined,
                    }}
                  >
                    <Form.Item
