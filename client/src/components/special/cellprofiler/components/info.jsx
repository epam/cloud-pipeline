...
function AnalysisPipelineInfo(props) {
  ...
  const renderEditableProperty = (options = {}) => {
    ...
    return (
      <div className={classNames(styles.cellProfilerParameter)}>
        <div className={styles.cellProfilerParameterTitle}>{title}</div>
        <Input
  ...
  return (
    <div>
      ...
      <div className={classNames(styles.cellProfilerParameter, 'cp-text-not-important')}>
        <span>Created {displayDate(pipeline.createdDate)}</span>
        {pipeline.author && (
          <span style={{whiteSpace: 'pre'}}>
            {' by '}
            <UserName userName={pipeline.author} showIcon />
          </span>
        )}
      </div>
    </div>
