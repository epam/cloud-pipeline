...
class CommitDiffButton extends React.Component {
  ...
  fetchDiff = () => {
    const {commit, versionedStorageId, path} = this.props;
            ...
            request
              ...
              .then(() => {
                if (request.error || !request.loaded) {
                  ...
                  const {diff = undefined} = request.value;
                  if (!diff) {
                    this.setState({
                      diff: undefined,
                      diffError: undefined,
  ...
  renderPresentation = () => {
    ...
    return (
      <div className={classNames(styles.emptyContent, 'cp-text-not-important')}>No content</div>
    );
  };

  render() {
    const {className, commit, disabled, style, versionedStorageId} = this.props;
    const {diffPending, visible} = this.state;
    return (
      <div className={className} style={style}>
        <Button
          size="small"
          className={styles.button}
          disabled={disabled || !versionedStorageId || !commit || diffPending}
