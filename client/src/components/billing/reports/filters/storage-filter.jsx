...
function StorageFilter({filters}) {
  const {metrics, metricsNavigation} = filters;
  return (
    <Radio.Group
      ...
      size="small"
    >
      <Radio.Button key={StorageMetrics.costs} value={StorageMetrics.costs}>
        Costs
      </Radio.Button>
      <Radio.Button key={StorageMetrics.volume} value={StorageMetrics.volume}>
        Volume
      </Radio.Button>
    </Radio.Group>
  );
...
export default BillingNavigation.attach(observer(StorageFilter));
