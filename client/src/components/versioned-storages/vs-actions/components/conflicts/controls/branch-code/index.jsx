...
function findBranchCodeLine(element) {
  ...
  if (element.dataset && Object.hasOwn(element.dataset, 'branchCodeLine')) {
    return element;
  }
...
class BranchCode extends React.PureComponent {
  static defaultContentRenderer = (item, props) => (
    <div data-branch-code-line={item.key}>{getLineTextWithoutBreak(item, props?.branch) || ''}</div>
  );
  ...
  componentDidUpdate(prevProps, prevState, snapshot) {
    ...
    if (prevProps.file !== this.props.file || prevProps.branch !== this.props.branch) {
  ...
  paste = (e) => {
    if (e.clipboardData && e.clipboardData.getData) {
      ...
      if (pastedText && focused && editable && caret) {
        const {line: currentLine, offset: prevOffset = 0} = caret;
  ...
  selectCode = (from, to) => {
    const {file, branch, editable, lineIdentifier} = this.props;
    if (editable && file && branch && from && to && from.line && to.line && document.createRange) {
  ...
  moveCursor = (options, callback) => {
    ...
    const {line: prevLine, offset: prevOffset} = caret;
    let {line, offset} = options;
    const maxLineIndex = Math.max(...lines.map((line) => line.lineNumber[branch]));
  ...
  extendSelection = (previousLine, previousOffset) => {
    const {caret} = this.state;
    const {file, branch} = this.props;
    const {line, offset} = caret || {};
    if (
      line &&
      offset !== undefined &&
      file &&
      ...
      if (selection) {
        ...
        if (start && end) {
          const {lineKey: startLineKey, offset: startOffset} = start;
          const {lineKey: endLineKey, offset: endOffset} = end;
      ...
      if (from.line && to.line) {
        if (
          ...
          (from.line.lineNumber[branch] === to.line.lineNumber[branch] && from.offset > to.offset)
        ) {
          const temp = to;
          to = from;
          from = temp;
  ...
  keyDown = (e) => {
    const {branch, editable, file, lineHeight, verticalScroll} = this.props;
    const {focused, caret} = this.state;
    if (file && focused && editable && caret) {
      ...
      if (handled) {
        ...
        this.moveCursor(
          ...
          () => {
            const {caret: newCaret} = this.state;
  ...
  correctCharacterSizeInfo = () => {
    ...
    if (span && span.textContent && span.textContent.length) {
      ...
      if (this.state.characterSize !== characterSize || this.state.marginLeft !== marginLeft) {
        this.setState({
          characterSize,
          marginLeft,
        });
  ...
  renderCodeLine = (line) => {
    ...
    const firstMarker = modification && isFirstLineOfModification && (
      <div
        ...
        className={getClassNameForChange(modification, {
          isFirst: true,
          hidden,
        })}
      >
    ...
    let content;
    if (state === LineStates.original || state === LineStates.inserted) {
      content = (
        <div
          id={`${branch}-${lineIdentifier(line)}`}
          data-vs-line={line.key}
          className={getClassNameForChange(modification, {hidden})}
          key={line.key}
          style={Object.assign(
            {},
            {
          ...
          onMouseDown={editable ? (e) => this.onCaretUpdate(e, line) : undefined}
          onMouseUp={editable ? (e) => this.onCaretUpdate(e, line) : undefined}
        >
          {renderContent(line, {
            ...this.props,
            modification,
    ...
    return [firstMarker, content, lastMarker].filter(Boolean);
  };
  ...
  render() {
    ...
    const {caret, focused, marginLeft, characterSize} = this.state;
    if (!file) {
      return null;
    }
    const lines = file.getLines(branch, new Set([]));
    ...
    return (
      ...
      >
        ...
        >
          <span ref={this.initializeCharacter}>{caret?.text || '_'}</span>
        </div>
        {caret && cursorLine && (
          <Caret
        ...
        {lines.map(this.renderCodeLine).reduce((result, array) => [...result, ...array])}
      </div>
    );
  }
}
...
function renderLineNumberWithActions(line, props) {
  ...
  if (index >= 0) {
    ...
    const actions =
      modification && !hideModificationActions && modification.status === ChangeStatuses.prepared
        ? [
            {rtlIcon},
            <CloseOutlined
              className={classNames(styles.action, 'cp-conflict-action')}
    ...
    const controls = [
      <div key="line number" className={classNames(styles.number, 'cp-text-not-important')}>
        {index}
      </div>,
