...
class TestCopyWithFolders(object):
    ...
    def test_copy_file_to_bucket_with_folder_with_same_name(self):
        ...
        try:
            pipe_storage_cp(os.path.abspath(self.test_file_1), "s3://%s/%s/%s/" % (self.bucket_name, case,
                                                                                   self.test_file_1))
            assert s3_object_exists(self.bucket_name, "%s/%s/%s" % (case, self.test_file_1, self.test_file_1))
            pipe_storage_cp(os.path.abspath(self.test_file_1), "s3://%s/%s/" % (self.bucket_name, case))
            assert s3_object_exists(self.bucket_name, "%s/%s" % (case, self.test_file_1))
    ...
    def test_copy_folder_structure(self):
        ...
        destination = "s3://%s/%s/" % (self.bucket_name, case)
        logging.info("Test case: %s. Ready to perform operation from %s to %s" % (case, source, destination))
        try:
            pipe_storage_cp(source, destination, recursive=True)
            assert s3_object_exists(self.bucket_name, os.path.join(case, self.test_folder, self.test_file_1))
            assert s3_object_exists(self.bucket_name, os.path.join(case, self.test_folder_2, self.test_file_1))
    ...
    def test_upload_file_to_not_empty_folder(self):
        ...
        try:
            ...
            assert s3_object_exists(self.bucket_name, "%s/%s" % (test_case, self.test_file_1))

            source2 = "cp://{}/{}/".format(self.bucket_name, test_case)
            pipe_storage_cp(os.path.abspath(self.test_file_2), source2, expected_status=0)
            assert s3_object_exists(self.bucket_name, "%s/%s" % (test_case, self.test_file_2))
        except BaseException as e:
    ...
    def test_upload_folders_with_similar_keys(self):
        ...
        try:
            ...
            pipe_storage_cp(os.path.join(source_folder, test_folder1), "s3://%s/%s/" % (self.bucket_name, case),
                            recursive=True)
            assert s3_object_exists(self.bucket_name, os.path.join(case, self.test_file_1))
            assert not s3_object_exists(self.bucket_name, os.path.join(case, self.test_file_2))
        except BaseException as e:
            pytest.fail("Test case {} failed. {}".format(case, e.message))
    ...
    def test_download_folders_with_similar_keys(self):
        ...
        try:
            pipe_storage_cp(os.path.abspath(self.test_file_1), "s3://%s/%s/folder/" % (self.bucket_name, case))
            assert s3_object_exists(self.bucket_name, os.path.join(case, "folder", self.test_file_1))
            pipe_storage_cp(os.path.abspath(self.test_file_2), "s3://%s/%s/folder2/" % (self.bucket_name, case))
            assert s3_object_exists(self.bucket_name, os.path.join(case, "folder2", self.test_file_2))

            pipe_storage_cp("s3://%s/%s/folder" % (self.bucket_name, case),
    ...
    def test_folder_with_slash_should_upload_content_only(self, case, has_destination_slash, has_source_slash):
        ...
        try:
            pipe_storage_cp(source, destination, recursive=True)
            assert s3_object_exists(self.bucket_name, os.path.join(case, self.test_file_1))
            assert s3_object_exists(self.bucket_name, os.path.join(case, self.test_file_with_other_extension))
            assert s3_object_exists(self.bucket_name, os.path.join(case, self.test_folder, self.test_file_1))
        except BaseException as e:
            pytest.fail("Test case {} failed. {}".format(case, e.message))
    ...
    def test_folder_with_slash_should_download_content_only(self, case, has_destination_slash, has_source_slash):
        source = "s3://%s/%s" % (self.bucket_name, case)
        destination = os.path.abspath(os.path.join(self.output_folder, case))
        source, destination = prepare_paths_with_slash(source, destination, has_source_slash, has_destination_slash)
        try:
    ...
    def test_folder_with_slash_should_copy_between_buckets_content_only(self, case, has_destination_slash,
                                                                        has_source_slash):
        source = "s3://%s/%s" % (self.bucket_name, case)
        destination = "s3://%s/%s" % (self.other_bucket_name, case)
        source, destination = prepare_paths_with_slash(source, destination, has_source_slash, has_destination_slash)
        try:
            self._create_folder_on_bucket(case)

            pipe_storage_cp(source, destination, recursive=True)
            assert s3_object_exists(self.other_bucket_name, os.path.join(case, self.test_file_1))
            assert s3_object_exists(self.other_bucket_name, os.path.join(case, self.test_file_with_other_extension))
            assert s3_object_exists(self.other_bucket_name, os.path.join(case, self.test_folder, self.test_file_1))
        except BaseException as e:
            pytest.fail("Test case {} failed. {}".format(case, e.message))
    ...
    def test_upload_folder_with_skip_existing_option_should_skip(self):
        ...
        try:
            ...
            assert s3_object_exists(self.bucket_name, key)
            assert s3_object_exists(self.bucket_name, os.path.join(case, self.test_file_with_other_extension))
            assert s3_object_exists(self.bucket_name, os.path.join(case, self.test_folder, self.test_file_1))
    ...
    def test_upload_folder_with_skip_existing_option_should_not_skip(self):
        ...
        try:
            ...
            pipe_storage_cp(source, destination, force=True, recursive=True, skip_existing=True)
            assert s3_object_exists(self.bucket_name, key1)
            assert s3_object_exists(self.bucket_name, os.path.join(case, self.test_file_with_other_extension))
            assert s3_object_exists(self.bucket_name, os.path.join(case, self.test_folder, self.test_file_1))
    ...
    def test_download_folder_with_skip_existing_option_should_skip(self):
        ...
        source_folder = "s3://%s/%s/" % (self.bucket_name, case)
        try:
            ...
            assert s3_object_exists(self.bucket_name, os.path.join(case, self.test_file_1))
            assert s3_object_exists(self.bucket_name, os.path.join(case, self.test_file_2))
    ...
    def test_copy_folder_between_buckets_with_skip_existing_option_should_skip(self):
        ...
        source_folder = "s3://%s/%s/" % (self.bucket_name, case)
        destination_folder = "s3://%s/%s/" % (self.other_bucket_name, case)
        key1 = os.path.join(case, self.test_file_1)
        ...
        try:
            ...
            pipe_storage_cp(os.path.abspath(self.test_file_1), "s3://%s/%s" % (self.bucket_name, key1))
            pipe_storage_cp(os.path.abspath(self.test_file_2), "s3://%s/%s" % (self.bucket_name, key2))
            assert s3_object_exists(self.bucket_name, key1)
            assert s3_object_exists(self.bucket_name, key2)

            pipe_storage_cp(source_folder, destination_folder, force=True, recursive=True, skip_existing=True)
            assert s3_object_exists(self.other_bucket_name, key1)
            assert s3_object_exists(self.other_bucket_name, key2)
    ...
    def _create_folder_on_bucket(self, case):
        ...
        pipe_storage_cp(source1, "s3://%s/%s/%s" % (self.bucket_name, case, self.test_file_1))
        pipe_storage_cp(source2, "s3://%s/%s/%s" % (self.bucket_name, case, self.test_file_with_other_extension))
        pipe_storage_cp(source3, "s3://%s/%s/%s/%s" % (self.bucket_name, case, self.test_folder, self.test_file_1))
