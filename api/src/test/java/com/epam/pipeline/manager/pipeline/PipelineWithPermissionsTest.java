...
public class PipelineWithPermissionsTest extends AbstractAclTest {
    ...
    @Test
    public void shouldReturnPermissionsWhenGrantedToPipeline() {
        ...
        final List<Pair<AbstractSecuredEntity, List<AbstractGrantPermission>>> pipelines = Arrays.asList(
        ...
        initAclEntity(pipelines);

        doReturn(Arrays.asList(pipeline1, pipeline2))
                .when(mockEntityManager)
                .loadAllWithParents(eq(AclClass.PIPELINE), any(), any());
        ...
        permissionsForPipeline1.put(TEST_USER_1.toUpperCase(), 4);
...
//        assertPipelineWithPermissions(expectedMap);
    ...
    private void assertPipelineWithPermissions(final Map<Long, Map<String, Integer>> expectedMap) {
        final Set<PipelineWithPermissions> loaded = permissionManager.loadAllPipelinesWithPermissions(null, null)
                .getPipelines();
        loaded.forEach(pipelineWithPermissions -> {
            final Map<String, Integer> expectedPermissions = expectedMap.get(pipelineWithPermissions.getId());
            ...
            actualPermissions.forEach(permission -> {
                final String sid = permission.getSid().getName().toUpperCase();
    ...
    private Folder getFolder(final String name, final String owner) {
