...
import styles from './job-notifications.module.css';
...
class JobNotifications extends React.Component {
  ...
  updateState = () =>
    new Promise((resolve) => {
      const {value: notifications = []} = this.props;
      this.setState(
        {
  ...
  renderTitle = () => {
    const {value: notifications = [], multiple, linkClassName, linkStyle} = this.props;
    if (notifications.length === 0) {
      return null;
    }
    let title = <span>Configure</span>;
    if (notifications.length > 0) {
      const [first] = notifications;
      const {triggerStatuses = [], recipients = []} = first;
      if (multiple && notifications.length > 1) {
        title = (
          <span>
        ...
        title = [
          <span key="title">Notification will be sent on</span>,
          triggerStatuses.map((status) => (
            <span key={status} style={{marginLeft: 5}}>
              {status}
            </span>
          )),
        ...
        title = [
          <span key="title">Notification will be sent to</span>,
          recipients.map((recipient) => (
            <UserName key={recipient} userName={recipient} style={{marginLeft: 5}} />
          )),
        ];
      }
    }
    return (
      <div
        className={classNames('cp-text', 'underline', styles.link, linkClassName)}
        style={linkStyle}
  ...
  cancelClicked = () => {
    this.closeConfigurationDialog().then(this.updateState);
  };
  ...
  addNotification = () =>
    new Promise((resolve) => {
      const {defaultNotificationType} = this.props;
      const {notifications = []} = this.state;
      this.setState(
        {
          notifications: [
  ...
  removeNotification = (index) => () => {
    const {multiple} = this.props;
    const {notifications = []} = this.state;
    if ((!multiple && notifications.length < 2) || index < 0 || notifications.length <= index) {
      return;
    }
  ...
  enableNotifications = (e) => {
    ...
    if (enable && notifications.length === 0) {
      this.addNotification().then(this.handleChange);
    } else if (!enable && notifications.length > 0) {
      this.clearNotifications().then(this.handleChange);
    }
  };

  render() {
    const {availableNotificationTypes, defaultNotificationType, multiple} = this.props;
    const {visible, notifications} = this.state;
    return (
      <div className={styles.container}>
        <Checkbox checked={notifications.length > 0} onChange={this.enableNotifications}>
          Enabled
        </Checkbox>
        {this.renderTitle()}
        <Modal
        ...
        >
          ...
          {notifications.length === 0 && (
            <div className={styles.noNotificationsWarning}>
              ...
              <a onClick={this.addNotification} style={{marginLeft: 5}}>
                Create notification
              </a>
            </div>
          )}
          {multiple && notifications.length > 0 && (
            <div>
              <Button size="small" onClick={this.addNotification}>
