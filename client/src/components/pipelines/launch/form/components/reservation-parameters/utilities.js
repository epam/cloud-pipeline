...
export function reservationParametersDiffer(a, b) {
  const {cpu: aCpu = 0, gpu: aGpu = 0, ram: aRam = 0} = a || {};
  const {cpu: bCpu = 0, gpu: bGpu = 0, ram: bRam = 0} = b || {};
...
export async function getInstanceResources(config) {
  const {kube_assign_policy: _kubeAssignPolicy = {}, kubeAssignPolicy = _kubeAssignPolicy} =
    config || {};
  const {selector} = kubeAssignPolicy || {};
  const {label, value} = selector || {};
  if (!label || !value) {
    return [];
  }
...
export function getInstanceResourcesRestrictions(options) {
  const {config, instanceType: instanceTypeObj} = options || {};
  if (!config) {
    return {
      cpu: [0, 0],
      ram: [0, 0],
  ...
  let maxRam = Number.isFinite(maxRamRaw) ? parseRAMRequest(maxRamRaw, ramRequestsUnit) : maxRamRaw;
  const subReserved = (value, reserved) =>
    Number.isFinite(value) && Number.isFinite(reserved) ? Math.max(1, value - reserved) : value;
...
export function correctReservationParameters(parameters, options) {
  ...
  let {cpu = 1, gpu = 1, ram = 1} = parameters || {};
  const {
    cpu: cpuRange = [],
    gpu: gpuRange = [],
    ram: ramRange = [],
...
export async function buildLaunchParametersFromReservationParameters(
  ...
  const {applyAdditionalParameters = true} = options || {};
  const [instanceTypeObj = {}, cfg] = await Promise.all([
  ...
  const {
    ...
    gpu = 1,
    ram = 1,
  } = correctReservationParameters(reservationParameters, {
    config: cfg,
...
export async function ensureValidReservationParametersForLaunchPayloads(payloads) {
  ...
  for (let i = 0; i < reservationRequests.length; i++) {
    const {parameters: appliedReservationParameters, podAssignPolicy} = reservationRequests[i];
...
const binaryUnitsSorted = Object.entries(binaryUnits).sort((a, b) => b[1] - a[1]);
...
const decimalUnitsSorted = Object.entries(decimalUnits).sort((a, b) => b[1] - a[1]);
...
function transformK8sRAMRequestStringToBytes(request, defaultUnit = DEFAULT_RAM_REQUESTS_UNIT) {
  ...
  console.assert(
    match,
    `Wrong RAM request input "${request}", check https://kubernetes.io/docs/concepts/configuration/manage-resources-containers/#meaning-of-memory`,
  );
...
export function transformBytesToK8sRAMRequest(bytes, options) {
  const {decimal = false, appendBytesLetter = false, appendSuffix = true, unit} = options || {};
  ...
  if (decimalUnit || binaryUnit) {
    return transformBytesToK8sRAMUnitRequest(bytes, {
...
export function getNodeAvailability(nodeResources) {
  const {total = {}, used = {}} = nodeResources;
  const {cpu: totalCPU = 0, gpu: totalGPU = 0, memory: totalRAM = 0} = total || {};
  const {cpu: usedCPU = 0, gpu: usedGPU = 0, memory: usedRAM = 0} = used || {};
...
export function getInstanceResourcesAvailability(resources, requests, config) {
  ...
  const {cpu: cpuRequest = 0, gpu: gpuRequest = 0, ram: ramRequest = 0} = requests || {};
  const getNodeScore = (node) => {
    const {available = {}, total = {}} = node || {};
    const {
      cpu: cpuAvailable = 0,
      gpu: gpuAvailable = 0,
      memory: ramAvailable = 0,
    } = available || {};
    const {cpu: cpuTotal = 0, gpu: gpuTotal = 0, memory: ramTotal = 0} = total || {};
    ...
    const buildScore = (opts) => {
      const {request, available, total, fits} = opts;
