...
import { useProjectsStore, useReloadProjects } from '../../state/projects/hooks.ts';
...
export const ProjectsList = memo(({ mode = 'standard', withFilters, showDescription = false }: Props) => {
  useReloadProjects();

  const { data: projects, error: projectsLoadError, pending: isProjectsLoading } = useProjectsStore();

  const pipelines = usePipelines();

  const getRandomPipeline = useCallback(
    () => (pipelines.length > 0 ? pipelines[Math.floor(Math.random() * pipelines.length)] : undefined),
    [pipelines],
  );
    ...
    <ItemsPanel
      ...
      title={
        <div className="min-h-6 fill-current flex items-center flex-nowrap gap-1">
          ...
          <span data-cy="projects-list-title">Projects</span>
        </div>
      }
      ...
      afterSearch={withFilters && <NgsFilters filters={filters} onFiltersChange={onFiltersChanged} config={config} />}
      itemKey="id"
      searchClassName={classNames({
        'py-1': mode === 'extended',
      })}
      searchInputClassName="projects-list-search"
      viewAll={mode === 'standard' ? { title: 'View all projects', link: '/projects' } : undefined}
