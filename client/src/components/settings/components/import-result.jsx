...
function ImportResult({logs, onClose, visible, mode = 'modal'}) {
  let content;
  if (!logs || !logs.length) {
    content = <Alert type="info" title="No logs are available" />;
  } else {
    content = (
      <BashCode
  ...
  if (/^inline$/i.test(mode)) {
    return <div className={styles.logs}>{content}</div>;
  }
  return (
    <Modal onCancel={onClose} footer={null} title="Import logs" open={visible} width="80%">
      <div className={styles.logs}>{content}</div>
    </Modal>
  );
}
