...
class EditQuotaDialog extends React.Component {
  ...
  get groups() {
    ...
    if (roles && roles.loaded) {
      ...
      return (roles.value || []).slice().concat(adGroups.filter((ad) => !uniqueNames.has(ad.name)));
    }
    return adGroups;
  }
  ...
  get isNotifyActionAdded() {
    ...
    return actions
      .filter(Boolean)
      .some((action) => action.actions && action.actions.includes(quotaActions.notify));
  }
  ...
  validate = () => {
    ...
    const {type} = quota;
    const {actions, subject, value, filter, recipients} = this.state;
    const errors = {};
    if (isNaN(value)) {
  ...
  onSaveClicked = () => {
    const {onSave, quota} = this.props;
    const {actions, recipients, subject, value, period} = this.state;
  ...
  renderUsersSelection = () => {
    const {errors = {}, subject, filter} = this.state;
    ...
    const filtered = this.users.filter(
      (user) =>
        ...
        (user.userName.toLowerCase().includes(filter.toLowerCase()) ||
          (user.attributes &&
            Object.entries(user.attributes).some(([_attribute, value]) =>
    ...
    return (
      <Select
        ...
        className={classNames(styles.targetSelect, {'cp-error': error})}
        value={subject}
      ...
      >
        {filtered.map((user) => (
          <Select.Option key={user.userName} value={user.userName}>
            <UserName userName={user.userName} showIcon={false} />
          </Select.Option>
        ))}
      </Select>
    );
  ...
  renderGroupsSelection = () => {
    ...
    const onChangeSearchString = (e) => {
      this.setState({filter: e}, () => {
        GroupFind.findGroups(e).then((groups = []) => {
    ...
    return (
      ...
      >
        {this.billingCenters.map((billingCenter) => (
          <Select.Option key={billingCenter} value={billingCenter}>
            {billingCenter}
          </Select.Option>
        ))}
      </Select>
  ...
  renderSubject = () => {
    ...
    switch (type) {
      case quotaTypes.user:
        selector = this.renderUsersSelection();
        break;
      case quotaTypes.group:
        selector = this.renderGroupsSelection();
        break;
      case quotaTypes.billingCenter:
        selector = this.renderBillingCentersSelection();
        break;
      default:
        return null;
    }
    return (
      <div className={styles.targetSelectContainer}>
        <span className={styles.label}>{quotaSubjectName[type]}:</span>
        {selector}
      </div>
    );
  };
  ...
  renderQuotaInput = () => {
    const {disabled} = this.props;
    const {errors, value, period} = this.state;
    ...
    return (
      <div className={styles.quotaInputContainer}>
        ...
        <InputNumber
          disabled={disabled || !this.isNewQuota}
          className={classNames(styles.quotaInput, {
            'cp-error': error,
          })}
          value={value}
        ...
        >
          <Select.Option key={quotaPeriods.month} value={quotaPeriods.month}>
            per month
          </Select.Option>
          <Select.Option key={quotaPeriods.quarter} value={quotaPeriods.quarter}>
            per quarter
          </Select.Option>
          <Select.Option key={quotaPeriods.year} value={quotaPeriods.year}>
            per year
          </Select.Option>
        </Select>
      </div>
  ...
  renderActions = () => {
    const {quota, disabled} = this.props;
    if (!quota) {
      return null;
    }
    const {actions, errors} = this.state;
    const {type, quotaGroup} = quota;
    ...
    const showErrorStyles = (index) =>
      error && Array.isArray(error) && !!error[index] && !disabled && this.isNewQuota;
    return (
        ...
        <div className={styles.actionsColumn}>
          ...
          {this.isNewQuota && (
            <div className={styles.add}>
              <Button disabled={disabled || !this.isNewQuota} onClick={onAddAction}>
                <PlusOutlined /> Add action
              </Button>
            </div>
          )}
          {error && !Array.isArray(error) && (
            <span className={classNames(styles.actionsError, 'cp-error')}>{error}</span>
          )}
        </div>
      </div>
    );
      ...
      <div className={styles.quotaInputContainer}>
        ...
        <UsersRolesSelect
          ...
          onChange={(newRecipients) =>
            this.setState({recipients: newRecipients, modified: true}, this.validate)
          }
          disabled={disabled || !this.isNewQuota}
  ...
  render() {
    const {visible, onCancel, quota, disabled} = this.props;
    ...
    return (
      <Modal
        ...
        onCancel={onCancel}
        footer={
          <Row type="flex" justify={this.isNewQuota ? 'end' : 'space-between'}>
            {!this.isNewQuota && (
              <Button danger disabled={disabled} onClick={this.onRemove}>
                REMOVE
              </Button>
            )}
            <div>
