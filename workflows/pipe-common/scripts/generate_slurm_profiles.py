...
PROFILE_QUEUE_FORMAT = 'slurm_profile_{}_queue.sh'
PROFILE_QUEUE_PATTERN = '^slurm_profile_(.+)_queue\\.sh$'
PROFILE_AUTOSCALING_FORMAT = 'slurm_profile_{}_autoscaling.sh'
PROFILE_AUTOSCALING_PATTERN = '^slurm_profile_(.+)_autoscaling\\.sh$'


def generate_slurm_profiles():
    logging_dir = os.getenv('CP_CAP_SLURM_PROFILE_GENERATION_LOG_DIR', default=os.getenv('LOG_DIR', '/var/log'))
    logging_level_run = os.getenv('CP_CAP_SLURM_PROFILE_GENERATION_LOGGING_LEVEL_RUN', default='INFO')
    logging_level_file = os.getenv('CP_CAP_SLURM_PROFILE_GENERATION_LOGGING_LEVEL_FILE', default='DEBUG')
    logging_level_console = os.getenv('CP_CAP_SLURM_PROFILE_GENERATION_LOGGING_LEVEL_CONSOLE', default='INFO')
    logging_format = os.getenv('CP_CAP_SLURM_PROFILE_GENERATION_LOGGING_FORMAT', default='%(asctime)s:%(levelname)s: %(message)s')
    logging_task = os.getenv('CP_CAP_SLURM_PROFILE_GENERATION_LOGGING_TASK', default='GenerateSLURMProfiles')
    logging_file = os.getenv('CP_CAP_SLURM_PROFILE_GENERATION_LOGGING_FILE', default='generate_slurm_profiles.log')

    api_url = os.environ['API']
    run_id = os.environ['RUN_ID']

    cap_scripts_dir = os.getenv('CP_CAP_SCRIPTS_DIR', '/common/cap_scripts')
    default_queue_disabled = os.getenv('CP_CAP_SLURM_DISABLE_DEFAULT_QUEUE', 'false').lower() == 'true'
...
def _generate_profiles(default_queue_disabled, logger):
    common_profile, secondary_profiles = build_environment_profiles(prefixes=['CP_CAP_SLURM', 'CP_CAP_AUTOSCALE'],
                                                                    logger=logger)
...
def _enhance_common_profile(common_profile):
    common_profile['CP_CAP_SLURM_QUEUE_NAME'] = os.getenv('CP_CAP_SLURM_QUEUE_NAME', 'main.q')
    common_profile['CP_CAP_SLURM_HOSTLIST_NAME'] = os.getenv('CP_CAP_SLURM_HOSTLIST_NAME', '@allhosts')
    common_profile['CP_CAP_SLURM_QUEUE_STATIC'] = 'true'
    common_profile['CP_CAP_SLURM_QUEUE_DEFAULT'] = 'true'
    common_profile['CP_CAP_AUTOSCALE_TASK'] = 'SLURMAutoscaling'
    common_profile['CP_CAP_SGE'] = 'false'
    return common_profile


def _enhance_secondary_profiles(common_profile, profiles):
    for profile_index, profile in profiles.items():
        if 'CP_CAP_SLURM_QUEUE_NAME' not in profile:
            if 'CP_CAP_AUTOSCALE_INSTANCE_TYPE' in profile:
                profile['CP_CAP_SLURM_QUEUE_NAME'] = '{}.q'.format(profile['CP_CAP_AUTOSCALE_INSTANCE_TYPE'])
            elif 'CP_CAP_AUTOSCALE_HYBRID' in profile:
                profile['CP_CAP_SLURM_QUEUE_NAME'] = '{}.family.q'.format(
                    profile.get('CP_CAP_AUTOSCALE_HYBRID_FAMILY', os.environ['instance_size']))
            else:
                profile['CP_CAP_SLURM_QUEUE_NAME'] = '{}.q'.format(os.environ['instance_size'])
        if 'CP_CAP_AUTOSCALE' not in profile:
            profile['CP_CAP_AUTOSCALE'] = 'false'
    profile_indexes = profiles.keys()
    profile_queues = [profiles[index]['CP_CAP_SLURM_QUEUE_NAME'] for index in profile_indexes]
    unique_profile_queues = suffix_non_unique(profile_queues, suffix_func=_append_suffix)
    for profile_index, profile in profiles.items():
        profile['CP_CAP_SLURM_QUEUE_NAME'] = unique_profile_queues[profile_indexes.index(profile_index)]
        profile['CP_CAP_SLURM_HOSTLIST_NAME'] = '@{}'.format(profile['CP_CAP_SLURM_QUEUE_NAME'])
        profile['CP_CAP_SGE'] = 'false'
...
def _write_profiles(params, profiles, cap_scripts_dir, logger):
    ...
    for profile_index, profile in profiles.items():
        ...
        profile_queue_name = profile.get('CP_CAP_SLURM_QUEUE_NAME')
