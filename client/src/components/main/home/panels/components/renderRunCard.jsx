...
function renderTitle(run) {
  ...
  if (run.childRunsCount > 0) {
    const cluster = [(run.activeChildRunsCount || 0) + 1, run.childRunsCount + 1]
      .join(' / ')
      .concat(' nodes');
    nodeCount = `cluster (${cluster})`;
  }
  return [podId, nodeCount, nodeType, priceType, nodeDisk].filter((i) => !!i).join(', ');
}
...
function renderPipeline(run) {
  ...
  let clusterIcon;
  if (run.nodeCount > 0 || run.clusterRun || run.childRunsCount > 0) {
    clusterIcon = <DatabaseOutlined />;
  }
  const runName = (
    <span type="main">
      <RunName run={run}>{displayName}</RunName>
    </span>
  );
  if (run.serviceUrl && run.initialized) {
    ...
    return (
      <span>
        ...
        <Popover
          ...
          trigger="hover"
        >
          <ExportOutlined /> {clusterIcon} {runName}
        </Popover>
      </span>
    );
  } else {
    return (
      <span>
        <StatusIcon run={run} small /> {clusterIcon} {runName}
      </span>
    );
  }
}
  ...
  if (run.commitStatus && run.commitStatus.toLowerCase() === 'committing') {
    return (
      <Row style={{fontStyle: 'italic'}}>
        <CodeFilled style={{fontWeight: 'large'}} className={styles.blink} />{' '}
        <span style={{fontSize: 'smaller'}}>Committing...</span>
      </Row>
    );
  }
  return undefined;
...
export default function renderRunCard(run, showEstimatedPrice = true) {
  return [
    ...
    <Row key="title" style={{fontSize: 'smaller'}}>
      ...
      {run.sensitive ? ',' : null}
      {run.sensitive ? (
        <span className="cp-sensitive" style={{whiteSpace: 'pre'}}>
          {' '}
          sensitive
        </span>
      ) : null}
    ...
    <Row key="time" style={{fontSize: 'smaller'}}>
      {renderTime(run)}
      {showEstimatedPrice && renderEstimatedPrice(run)}
    </Row>,
    <Row key="commit status">{renderCommitStatus(run)}</Row>,
    <Row key="region and platform" type="flex" align="middle">
      {renderRegion(run)}
      <PlatformIcon platform={run.platform} />
    </Row>,
    <RunTags key="run-tags" run={run} onlyKnown />,
  ];
}
