...
import {Select, Input, Button, Table, Badge, message, Space} from 'antd';
...
export default class CoreServicesTable extends React.Component {
  ...
  onClearFilters = () => {
    ...
    this.setState({filters: {...this._initialFilters}}, this.onCollapseExpandedRows);
  };
  ...
  renderTable = () => {
    ...
    const nodeNames = [...new Set((this.pods || []).map((pod) => pod.nodeName))];
    const namespaces = [...new Set((this.pods || []).map((pod) => pod.namespace))];
    const getFiltersState = (key) => ({
      filterDropdownProps: {
        open: this.state.filterDropdownVisible === key,
        onOpenChange: (visible) => {
          this.setState({
            filterDropdownVisible: visible ? key : undefined,
          });
        },
      },
      filtered: filters[key] !== this._initialFilters[key],
      filteredValue: filters[key] !== this._initialFilters[key] ? ['filtered'] : null,
    });
    const columns = [
      {
        dataIndex: 'name',
        ...
        render: (text, record) => (
          <span>
            {record.isContainer ? (
              <span className={classNames(styles.podContainerIndicator, 'cp-tag')}>CONTAINER</span>
            ) : null}
        ...
        filterDropdown: (
          ...
          >
            ...
            >
              <a onClick={this.onClearFilterField('name')}>Clear</a>
            </div>
          </div>
        ),
      ...
      },
      {
        ...
        filterDropdown: (
          ...
          >
            ...
            >
              <a onClick={this.onClearFilterField('nodeName')}>Clear</a>
            </div>
          </div>
        ),
      ...
      },
      {
        ...
        filterDropdown: (
          ...
          >
            ...
            >
              <a onClick={this.onClearFilterField('namespace')}>Clear</a>
            </div>
          </div>
        ),
      ...
      },
      {
        ...
        filterDropdown: (
          ...
          >
            ...
            >
              <a onClick={this.onClearFilterField('status')}>Clear</a>
            </div>
          </div>
        ),
      ...
      },
      {
        ...
        render: (text, record) => (
          <span>
            {typeof record.status === 'string' && (record.status || '').toUpperCase() === 'RUNNING'
  ...
  render() {
    ...
    return (
      <div>
        ...
        {this.renderTable()}
        <PodInfoModal pod={podInfoModal} onClose={this.closeInfoModal} />
        <ContainerLogsModal container={containerLogsModal} onClose={this.closeInfoModal} />
      </div>
    );
  }
}
