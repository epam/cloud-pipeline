...
class Plot extends React.Component {
  ...
  constructor(props) {
    super(props);
    this.chartArea = props.chartArea;
    this.width = props.width;
    this.height = props.height;
    this.plots = props.plots || [];
    this.dataGroup = props.dataGroup || 'default';
    makeObservable(this, {
      chartArea: observable.ref,
      width: observable,
      height: observable,
      plots: observable.ref,
      dataGroup: observable,
      syncFromProps: action,
    });
  }
  ...
  componentDidUpdate(prevProps) {
    const {chartArea, width, height, plots, dataGroup} = this.props;
    if (
      prevProps.chartArea !== chartArea ||
      prevProps.width !== width ||
      prevProps.height !== height ||
      prevProps.plots !== plots ||
      prevProps.dataGroup !== dataGroup
    ) {
  ...
  render() {
    ...
    const percentRenderers = (plots || [])
      .filter((p) => p.isPercent)
      .map((p) => ({...p, renderer: p.renderer || 'percent-renderer'}))
      .filter(
        (plot, index, array) => array.map((p) => p.renderer).indexOf(plot.renderer) === index,
      );
    return (
      <Provider plot={this} data={data}>
        <svg ref={this.canvasRef} width={width} height={height}>
          <Timeline
          ...
          >
            ...
            {percentRenderers.map((plot) => (
              <ValueAxis
                ...
                maximum={100}
              >
                <ChartRenderer identifier={plot.renderer} isPercent />
              </ValueAxis>
            ))}
