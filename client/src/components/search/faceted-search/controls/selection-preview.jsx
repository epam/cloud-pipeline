...
class SelectionPreview extends React.Component {
  ...
  get actualSelection() {
    const {items = [], preferences, notDownloadableStorages = []} = this.props;
    const {removedItems = []} = this.state;
    const notRemoved = items.filter(
      (o) => !removedItems.find(elasticItemUtilities.filterMatchingItemsFn(o)),
    );
  ...
  renderSelectionDocument = (document) => {
    ...
    const isDownloadable = !this.notAllowedToDownload.find((item) => item.id === document.id);
    return (
      <div
        key={`${document.elasticId}`}
        id={`search-result-item-${document.elasticId}`}
        className={classNames(styles.resultItem, 'cp-even-odd-element', 'cp-search-result-item', {
  ...
  render() {
    const {onClose, onClear, title = 'Selected documents', visible} = this.props;
    const {selection = [], downloadCommandVisible} = this.state;
        ...
        <div style={{maxHeight: '50vh', overflow: 'auto'}}>
          {skipped > 0 && (
            <Alert
              ...
              title={
                <div>
                  {skipped} file{skipped === 1 ? ' is' : 's are'} not allowed to be downloaded and
