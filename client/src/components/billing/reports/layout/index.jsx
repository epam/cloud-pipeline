...
function LayoutPanel({children, containerClassName, containerStyle, className, style}) {
  return (
    <div className={classNames(styles.panelContainer, containerClassName)} style={containerStyle}>
      <div className={classNames(styles.panel, 'cp-panel', className)} style={style}>
        {children}
...
const Layout = inject('layoutDimensions')(restoreLayoutConsumer(LayoutComponent));
      ...
      if (containerWidth !== this.current.width || containerHeight !== this.current.height) {
        ...
        if (!immediate) {
          this.sizeUpdateTimer = setTimeout(() => {
            this.store.setSize(this.current.width, this.current.height);
