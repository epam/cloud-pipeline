...
const getCalcPresentation = (widthCss) => {
  const {em = 0, px = 0} = widthCss || {};
  const parts = [em && {value: em, unit: 'em'}, px && {value: px, unit: 'px'}].filter(Boolean);
  if (parts.length === 0) {
    return '0px';
  }
...
class TextFileConflict extends React.PureComponent {
  ...
  onResizeMove = (e) => {
    if (this.resizeInfo) {
      e.stopPropagation();
      e.preventDefault();
      const {start, branch, panels} = this.resizeInfo;
      const delta = (e.clientX - start) * (branch === HeadBranch ? 1 : -1);
      const total = [branch, Merged].reduce(
        (sum, affectedBranch) => sum + panels[affectedBranch],
        0,
      );
      const correctSize = (size) =>
        Math.min(total - MINIMUM_PANEL_SIZE_PIXELS, Math.max(MINIMUM_PANEL_SIZE_PIXELS, size));
  ...
  onFinishResizing = (e) => {
    if (this.resizeInfo) {
      ...
      const total = Object.values(gridColumns).reduce((sum, current) => sum + current, 0);
    ...
    if (conflictedFile) {
      ...
      const correctBranchItemIndex = (index, branch) => {
        if (branch && Array.isArray(branches[branch])) {
          return Math.max(0, Math.min(branches[branch].length - 1, index));
        }
        return index;
      };
      ...
      const onCursorPositionChanged = (branch, options = {}) => {
        const {pxFrom = {}, pxTo = {}} = options;
        const {top: topFrom, left: leftFrom} = pxFrom;
        const {top: topTo, left: leftTo} = pxTo;
          ...
          case HeadBranch:
            right = (
              <div
                ...
                className={classNames(styles.numbersPanel, styles.right)}
              >
          ...
          case RemoteBranch:
            left = (
              <div
                ...
                className={classNames(styles.numbersPanel, styles.left)}
              >
      ...
      const renderCanvas = (branch) => (
        <canvas
          className={classNames(styles.resize, 'cp-branch-code-resize')}
      ...
      const renderVerticalScroll = (branch, offsets) => {
        const {left = 0, right = 0, top = 0, bottom = Scrollbar.size} = offsets || {};
        return (
          <div
            className={classNames(styles.scrollContainer, styles.vertical)}
      ...
      const renderHorizontalScroll = (branch, offsets) => {
        const {left = 0, right = 0, top = -Scrollbar.size, bottom = 0} = offsets || {};
        return (
          <div
            className={classNames(styles.scrollContainer, styles.horizontal)}
      ...
      return [
        ...
        renderHorizontalScroll(HeadBranch, {
          left: Scrollbar.size,
          right: getCalcPresentation(numbersWidths[HeadBranch]),
        }),
        renderCanvas(HeadBranch),
        renderBranch(Merged),
        renderHorizontalScroll(Merged, {
          left: getCalcPresentation(numbersWidths[Merged]),
          right: getCalcPresentation(addWidthsCss(numbersWidths[Merged], {px: Scrollbar.size})),
        }),
        renderVerticalScroll(Merged, {
          left: getCalcPresentation(
            addWidthsCss(multiplyWidthCssByTimes(numbersWidths[Merged], -1), {px: -Scrollbar.size}),
          ),
        }),
        renderCanvas(RemoteBranch),
        renderBranch(RemoteBranch),
        renderHorizontalScroll(RemoteBranch, {
  ...
  draw = () => {
    ...
    renderChanges(this.canvases[HeadBranch], conflictedFile, HeadBranch, {
      width: CANVAS_WIDTH,
      top: this.scrolls[HeadBranch] ? this.scrolls[HeadBranch].scrollTop : 0,
      mergedTop: this.scrolls[Merged] ? this.scrolls[Merged].scrollTop : 0,
      lineHeight: LINE_HEIGHT,
      colorsConfig,
    });
    renderChanges(this.canvases[RemoteBranch], conflictedFile, RemoteBranch, {
      width: CANVAS_WIDTH,
      top: this.scrolls[RemoteBranch] ? this.scrolls[RemoteBranch].scrollTop : 0,
      mergedTop: this.scrolls[Merged] ? this.scrolls[Merged].scrollTop : 0,
      lineHeight: LINE_HEIGHT,
      rtl: true,
      colorsConfig,
  ...
  render() {
    ...
    return (
      <div className={styles.conflictContainer}>
        <ResolveChanges className={styles.resolveChanges} conflictedFile={conflictedFile} />
        <div
        ...
        >
          <div
            className={classNames(styles.header, 'cp-divider', 'bottom')}
            style={{
              gridColumn: 'SCROLL-HEAD / MERGED',
              paddingRight: getCalcPresentation(
                addWidthsCss(numbersWidths[HeadBranch], {px: CANVAS_WIDTH}),
              ),
            }}
          >
            <div className={styles.title}>Your changes</div>
          </div>
          <div
            className={classNames(styles.header, 'cp-divider', 'bottom')}
            style={{
              gridColumn: 'MERGED / CANVAS-REMOTE',
              paddingLeft: getCalcPresentation(numbersWidths[Merged]),
              paddingRight: getCalcPresentation(numbersWidths[Merged]),
            }}
          >
          ...
          <div
            ...
            style={{
              ...
              paddingLeft: getCalcPresentation(
                addWidthsCss(numbersWidths[RemoteBranch], {px: CANVAS_WIDTH}),
              ),
            }}
          >
            <div className={styles.title}>Changes from remote</div>
          </div>
          {this.renderIDE()}
        </div>
      </div>
