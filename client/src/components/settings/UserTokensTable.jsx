...
export default class UserTokensTable extends React.Component {
  ...
  componentDidUpdate(prevProps) {
    ...
    if (prevId !== currentId || prevProps.refreshToken !== this.props.refreshToken) {
      this.fetchData();
    }
  }
  ...
  fetchData = async () => {
    ...
    this.setState({
      pending: false,
      userTokens: userNamedTokens.loaded ? (userNamedTokens.value || []).map((v) => v) : [],
    });
  };
  ...
  revokeToken = async (record) => {
    ...
    try {
      ...
      if (revoke.loaded && revoke.value) {
        const msg = record.tokenName ? `Token ${record.tokenName} revoked.` : 'Token revoked.';
        message.success(msg);
