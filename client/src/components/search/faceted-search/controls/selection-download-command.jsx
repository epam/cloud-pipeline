...
import {Button, Select, Modal, message, Alert} from 'antd';
...
class SelectionDownloadCommand extends React.Component {
  ...
  componentDidMount() {
    ...
    preferences.fetchIfNeededOrWait().then(() => this.setDefaultOperationSystem());
  ...
  get code() {
    ...
    const {after, before, template} = currentConfiguration || {};
    const templateParts = (template || '').split(/({.*?})/gi).filter(Boolean);
      ...
      return templateParts
        .map((string) => {
          ...
          if (placeholder.toLowerCase().startsWith('storage.')) {
            const storage = this.dataStorages.find((d) => Number(d.id) === Number(item.storageId));
            const field = placeholder.split('.').pop();
            return storage && storage[field] ? storage[field] : '';
          }
          const placeholderInfo = SELECTION_PLACEHOLDERS.find(({keys}) =>
            keys.includes(placeholder.toUpperCase()),
          );
    ...
    return [before, ...items.map(mapItemTemplate), after].filter(Boolean).join('\n');
  }
  ...
  renderOperationSystemSelector = () => {
    ...
    return (
      ...
      >
        ...
        <Select style={{width: 200}} value={this.state.os} onChange={onChange}>
          {available.map((os) => (
  ...
  render() {
    const currentConfiguration = this.getCurrentConfiguration();
    const {style, items, visible, skipped = 0, filtered = 0} = this.props;
    ...
    return (
      <Modal
        ...
        footer={
          ...
          >
            <Button onClick={this.onCopy}>COPY TO CLIPBOARD</Button>
            ...
            <Alert
              ...
              title={
                <div>
                  {skipped > 0 && (
                    <div>
                      {skipped} file{skipped === 1 ? ' is' : 's are'} not allowed to be downloaded
                      and therefore will be skipped
                    </div>
                  )}
                  {filtered > 0 && (
                    <div>
                      {filtered} file{filtered === 1 ? ' is' : 's are'} not allowed to be downloaded
                      using <b>pipe cli</b> due to the data storage type
                    </div>
                  )}
                </div>
              }
