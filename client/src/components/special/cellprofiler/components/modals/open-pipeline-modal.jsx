...
class OpenPipelineModal extends React.Component {
  ...
  renderPipelinesTable = () => {
    const {pipelineFiles = [], error, page, pending, selectedPipeline, expandedKeys} = this.state;
    if (error) {
    ...
    if (pipelineFiles.length === 0) {
      return <Alert title="Analysis pipelines not found" type="info" />;
    }
    const pipelineIsSelected = (pipeline) =>
      pipeline &&
      selectedPipeline &&
    ...
    const columns = [
      ...
      },
      {
        ...
        render: (obj) => {
          ...
          return <UserName userName={obj.pipeline ? obj.pipeline.author : undefined} showIcon />;
        },
      },
      {
        key: 'description',
  ...
  loadSelectedPipeline = async () => {
    const {selectedPipeline, pipelineFiles = []} = this.state;
    if (!selectedPipeline) {
      return undefined;
    }
    const {path, version} = selectedPipeline;
  ...
  render() {
    ...
    const {selectedPipeline, opening} = this.state;
    return (
      <Modal
        open={visible}
        width="50%"
