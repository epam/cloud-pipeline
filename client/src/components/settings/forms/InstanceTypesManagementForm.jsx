...
import {Button, Input, InputNumber, message, Row, Select} from 'antd';
...
export default class InstanceTypesManagementForm extends React.Component {
  ...
  get pending() {
    return (
      this.valuePending(valueNames.allowedInstanceMaxCount) ||
  ...
  reportModified = () => {
    ...
    if (onModified) {
      onModified(this.getModified());
    }
  };

  getModified() {
    return (
      this.valueModified(valueNames.allowedInstanceMaxCount) ||
      ...
      >
        <div style={{padding: 2, width: '100%'}}>
          ...
          {this.props.level === 'USER' && (
            <Row type="flex">
              {this.valueInputDecorator(valueNames.allowedInstanceMaxCount, disabled, 'number')}
            </Row>
          )}
          ...
          <Row type="flex">
            <Select
              ...
              disabled={disabled}
            >
              <Select.Option key="on_demand" value="on_demand">
                On demand
              </Select.Option>
          ...
          {(this.props.level === 'ROLE' || this.props.level === 'USER') && (
            <Row type="flex">
              <Select
                ...
                disabled={disabled}
              >
                <Select.Option key={GcpSpotInstanceType.SPOT} value={GcpSpotInstanceType.SPOT}>
