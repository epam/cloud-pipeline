...
public class ListComposer extends AbstractFilterComposer {
    @Override
    public String generateExpression(LogicalExpression expression)
            throws WrongFilterException {
        String sqlExpression;
        switch (expression.getOperandType()) {
            case EQUALS:
                sqlExpression = String.format("(%s in (:%s))",
                        expression.getDatabaseFieldName(),
                        expression.getDatabaseValuePlaceholder());
                break;
            case NOT_EQUALS:
                sqlExpression = String.format("(%s not in (:%s))",
                        expression.getDatabaseFieldName(),
                        expression.getDatabaseValuePlaceholder());
                break;
            default:
