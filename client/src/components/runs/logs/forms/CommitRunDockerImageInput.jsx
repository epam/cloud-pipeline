...
import styles from './CommitRunDockerImageInput.module.css';

const isPersonalGroup = (group) => {
  const {name = '', owner: ownerName = ''} = group || {};
  const owner = (ownerName || '').replace(/[^a-zA-Z0-9-]/g, '-').toLowerCase();
...
export default class CommitRunDockerImageInput extends React.Component {
  ...
  updateTags = async () => {
    ...
    if (group) {
      const [tool] = (group.tools || []).filter(
        (t) => t.image === `${this.state.group}/${this.state.tool}`,
      );
      if (tool) {
  ...
  constructor(props) {
    super(props);
    makeObservable(this, {_tags: observable});
  }
  ...
  get groups() {
    ...
    return (this.currentRegistry.groups || [])
      ...
      .filter((g) => {
        ...
        return (
          (roleModel.writeAllowed(g) || hasToolsWithRights) &&
          (this.isAdmin || g.privateGroup || !isPersonalGroup(g))
        );
      });
  }
  ...
  get tools() {
    ...
    const links = originalToolsAndLinks
      .filter((t) => !!t.link)
      .map((link) => {
    ...
    const tools = originalTools
      ...
      .filter(
        (t) =>
          !this.state.tool || t.name.toLowerCase().indexOf(this.state.tool.toLowerCase()) === 0,
      );
    if (
      this.state.tool &&
      this.state.tool.length &&
      roleModel.writeAllowed(this.currentGroup) &&
  ...
  render() {
    ...
    return (
      <Row type="flex">
        <div className={classNames(styles.addon, 'cp-input-group-addon')}>
          <RegistrySelector
          ...
          {this.tools.map((tool) => {
            return (
              <AutoComplete.Option key={tool.name} text={tool.name}>
                {tool.isNew ? `Add new tool '${tool.name}'` : tool.name}
              </AutoComplete.Option>
            );
          })}
        </AutoComplete>
          ...
          {this.tags.map((tag) => {
            return (
              <AutoComplete.Option key={tag.name} text={tag.name}>
                {tag.isNew ? `Add new version '${tag.name}'` : tag.name}
              </AutoComplete.Option>
            );
          })}
        </AutoComplete>
