    ...
    >
      ...
      <div className={styles.rolesFiltersContainer}>
        {rolesSetEntries.map((entry) => (
          <Row style={{margin: 5}} key={entry.key}>
            <Checkbox onChange={onChangeRolesFilter(entry)} checked={entry.checked}>
              {entry.key}
            </Checkbox>
          </Row>
        ))}
...
function getColumnFilter(state, setState) {
  ...
  const {[ownersParameter]: ownersValue = [], [rolesParameter]: rolesValue = []} = values;
  ...
  const onChange = (newOwners, newRoles) => {
    if (newOwners && newOwners.length > 0) {
      onChangeOwnersAndRoles(newOwners.slice(), undefined);
    } else {
