...
class HcsImageAnalysisJobs extends React.Component {
  ...
  render() {
    ...
    const {filters, filtersInitialized, jobId} = this.state;
...
function HcsImageAnalysisJobsModal(props) {
  const {visible, className, style, onClose} = props;
  return (
    <Modal
      ...
      footer={false}
    >
      {visible && <HcsImageAnalysisJobs className={className} style={style} />}
    </Modal>
  );
}
