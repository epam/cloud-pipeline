...
class SharingControl extends React.Component {
  ...
  get groupedItems() {
    const storages = [...new Set(this.shareableItems.map((item) => Number(item.storageId)))].filter(
      (id) => !Number.isNaN(id),
    );
  ...
  get downloadableItems() {
    const {preferences, notDownloadableStorages = []} = this.props;
    if (!preferences.loaded) {
      return [];
    }
    return filterDownloadableItems(this.items, preferences, notDownloadableStorages);
  }
  ...
  getMenuItems = () => {
    ...
    const getStorageName = (id) => {
      ...
      return storage && storage.name ? storage.name : id;
    };
    const items = [];
        ...
        label: (
          <span>
            ...
            {skipDownloadCount > 0 && (
              ...
              >
                {skipDownloadCount} file{skipDownloadCount === 1 ? ' is' : 's are'} not allowed to
                be downloaded
                <br />
  ...
  render() {
    const {items, visible, extraColumns, size, notDownloadableStorages} = this.props;
    ...
    return (
      <div style={{margin: '0 5px'}}>
        <Badge count={(items || []).length} style={{zIndex: 999}}>
          ...
          >
            <Button size={size} style={{width: 35, padding: 0}}>
              <ExportOutlined />
            </Button>
          </Dropdown>
        </Badge>
