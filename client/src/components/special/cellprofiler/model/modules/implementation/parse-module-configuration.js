...
function processParameter (input) {
  ...
  const aliasPart = joinParts(extractByCriteria(startsWithCriteria(/^ALIAS\s*(\s|=)\s*/)));
  const aliasExec = /^ALIAS\s*(\s|=)\s*(.+)$/.exec(aliasPart);
  let alias;
  if (aliasExec && aliasExec[2]) {
    alias = aliasExec[2];
  }
  const [namePart, typeParts, defaultValueParts] = splitParts;
  const name = joinParts(namePart);
  alias = alias || joinParts(name);
  const type = getParameterType(typeParts);
  const defaultValue = parseDefaultValue(defaultValueParts, type);
  return {
    advanced,
    local,
    required,
