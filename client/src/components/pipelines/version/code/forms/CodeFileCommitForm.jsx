...
export default class CodeFileCommitForm extends localization.LocalizedReactComponent {
  ...
  renderChangeConfigFormItems = () => {
    ...
    if (this.props.configChanged) {
      return [
        <Row type="flex" justify="center" key="config warning">
          {this.props.configChangedWarning ||
            `Configuration changed for ${this.localizedString('pipeline')}`}
        </Row>,
        <Row key="update config">
          <Col xs={24} sm={6} />
          <Col xs={24} sm={18}>
            <Form.Item className="code-file-commit-form-update-config-container">
              <Checkbox checked={this.state.updateConfig} onChange={onUpdateConfigChanged}>
    ...
    return (
      ...
      >
        <Spin spinning={this.props.pending}>
          <Form ref={this.formRef} initialValues={{configName: this.getDefaultConfigurationName()}}>
            <Form.Item
              {...this.formItemLayout}
