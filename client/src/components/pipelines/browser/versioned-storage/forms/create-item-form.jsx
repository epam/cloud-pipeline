...
class CreateItemForm extends React.Component {
  ...
  checkPathExistence = async () => {
    ...
    const pathExist = await checkFileExistence(pipelineId, `${path || ''}${values.name}`);
    ...
    return (
      ...
      >
        <Spin spinning={this.props.pending || checkInProgress}>
          <Form ref={this.formRef} initialValues={{name: this.props.name, comment: undefined}}>
            <Form.Item
              {...this.formItemLayout}
              label="Name"
              name="name"
              validateStatus={pathOccupied ? 'error' : undefined}
              help={pathOccupied ? `${documentType} with that name already exists` : undefined}
              rules={[
