...
import {Button, Checkbox, Col, Collapse, Form, Input, Modal, Row, Select, Tag} from 'antd';
import {SettingOutlined} from '@ant-design/icons';
import classNames from 'classnames';

import ToolEndpointsFormItem from '../elements/ToolEndpointsFormItem';
import KubeLabels, {kubeLabelsHasChanges, prepareKubeLabelsPayload} from '../elements/KubeLabels';
...
import {notificationArraysAreEqual} from '../../pipelines/launch/dialogs/job-notifications/notifications-equal';
import {mapObservableNotification} from '../../pipelines/launch/dialogs/job-notifications/job-notification';
...
export default class EditToolForm extends React.Component {
      ...
      .then(async (values) => {
        if (
          ...
          if (this.toolFormParameters && this.toolFormSystemParameters) {
            ...
            for (const key in this.props.configuration.parameters) {
              if (Object.hasOwn(this.props.configuration.parameters, key)) {
                parameters[key] = {
          ...
          const configuration = {
            ...
            cloudRegionId:
              values.cloudRegionId === regionNotConfiguredValue ? null : +values.cloudRegionId,
          ...
          const {users: shareWithUsers, roles: shareWithRoles} = sidsToConfigurationShare(
            this.state.shareSids,
          );
  ...
  getInitialValue = (field) => {
    switch (field) {
      case 'is_spot':
        return this.getPriceTypeInitialValue();
      case 'instance_size':
        return this.getInstanceTypeInitialValue();
      case 'instance_image':
        return this.getInstanceImageInitialValue();
      case 'instance_disk':
        return this.getDiskInitialValue();
      case 'allowSensitive':
        return this.getAllowSensitiveInitialValue();
      case 'allowCommit':
        return this.getAllowCommitInitialValue();
      case 'friendly_url':
        return this.getPrettyUrlInitialValue();
      default:
        return this.props.configuration ? this.props.configuration[field] : undefined;
    }
  };
  ...
  getPrettyUrlInitialValue = () => {
    return (
      (this.props.configuration && this.props.configuration.friendly_url) ||
  ...
  getInstanceTypeValue = () => {
    ...
    const name =
      (form ? form.getFieldValue('instanceType') : undefined) || this.getInstanceTypeInitialValue();
  ...
  getCloudRegion = () => {
    const instanceType = this.getInstanceTypeValue();
    if (this.props.awsRegions.loaded) {
      return this.awsRegions.find(
        (a) => (instanceType && a.id === instanceType.regionId) || !instanceType,
      );
    }
    return undefined;
  };
  ...
  getDiskInitialValue = () => {
    return (
      (this.props.configuration && this.props.configuration.instance_disk) ||
  ...
  getInstanceImageInitialValue = () => {
    return (
      (this.props.configuration && this.props.configuration.instance_image) ||
  ...
  getAllowSensitiveInitialValue = () => {
    ...
    return this.props.tool ? `${this.props.tool.allowSensitive}`.toLowerCase() === 'true' : true;
  };

  getInitialCloudRegionNotAvailable = () => {
    const form = this.formRef.current;
    const getFieldValue = form ? (name) => form.getFieldValue(name) : () => undefined;
    const initialValue = this.getCloudRegionInitialValue();
    const currentValue = getFieldValue('cloudRegionId');
    return (
      initialValue !== regionNotConfiguredValue &&
  ...
  rebuildComponent(props) {
    ...
    this.defaultCommand =
      props.tool && props.tool.defaultCommand ? props.tool.defaultCommand : null;
    if (props.configuration) {
      (async () => {
        ...
        state.autoScaledCluster =
          props.configuration && autoScaledClusterEnabled(props.configuration.parameters);
        state.fsConfig = props.configuration
          ? getFsConfigFromParameters(props.configuration.parameters)
          : undefined;
        state.hybridAutoScaledClusterEnabled =
          props.configuration && hybridAutoScaledClusterEnabled(props.configuration.parameters);
        const regions = this.props.awsRegions.loaded ? this.props.awsRegions.value : [];
        const instanceTypeName =
          (props.configuration ? props.configuration.instance_size : undefined) ||
        ...
        state.gridEngineEnabled =
          props.configuration && gridEngineEnabled(props.configuration.parameters);
        ...
        state.rescheduleRun =
          props.configuration && rescheduleRunParameterValue(props.configuration.parameters);
        state.autoScaledPriceType =
          props.configuration && getAutoScaledPriceTypeValue(props.configuration.parameters);
        ...
        this.defaultCommand =
          props.configuration && props.configuration.cmd_template
        ...
        const gpuScalingParameters = state.gpuScalingConfiguration
          ...
          : [];
        const kubeLabels = Object.entries((props.configuration || {}).kubeLabels || {}).map(
          ([key, value]) => ({key, value}),
        );
        ...
        if (props.configuration && props.configuration.parameters) {
          for (const key in props.configuration.parameters) {
            if (
              !Object.hasOwn(props.configuration.parameters, key) ||
            ...
            if (
              ...
              (this.props.runDefaultParameters.value || []).filter((p) => p.name === key).length ===
                1
            ) {
        ...
        if (this.toolFormParameters?.reset) {
  ...
  componentDidMount() {
    ...
    if (this.props.allowedInstanceTypes) {
      ...
      const cloudRegionId =
        this.props.configuration && this.props.configuration.cloudRegionId
          ? this.props.configuration.cloudRegionId
          : undefined;
  ...
  componentDidUpdate(prevProps, prevState, snapshot) {
    const form = this.formRef.current;
    const getFieldsValue = form ? () => form.getFieldsValue() : () => ({});
  ...
  get instanceTypes() {
    const isSpot =
      this.formRef.current.getFieldValue('is_spot') !== undefined
  ...
  modified = () => {
    ...
    const configurationFormFieldChanged = (field, formFieldName, formValueAccessor = (v) => v) => {
      formFieldName = formFieldName || field;
      const formField = this.formRef.current.getFieldValue(formFieldName);
    ...
    const limitMountsFieldChanged = () => {
      const fieldValue = this.formRef.current.getFieldValue('limitMounts');
    ...
    const cloudRegionFieldChanged = () => {
      return (
        this.getCloudRegionInitialValue() !== this.formRef.current.getFieldValue('cloudRegionId')
      );
    };
    const toolEndpointArray = this.props.tool
      ? (this.props.tool.endpoints || []).map((e) => e)
      : null;
    const endpointArrayFormValue = (this.formRef.current.getFieldValue('endpoints') || []).map(
      (e) => e,
    );
    ...
    const maxNodesCount =
      ...
      this.props.configuration.parameters[CP_CAP_AUTOSCALE_WORKERS]
        ...
        : 0;
    const autoScaledCluster =
      this.props.configuration && autoScaledClusterEnabled(this.props.configuration.parameters);
    const fsConfig = this.props.configuration
      ? getFsConfigFromParameters(this.props.configuration.parameters)
      : null;
    const hybridAutoScaledCluster =
      this.props.configuration &&
    ...
    const childNodeInstanceConfiguration = this.props.configuration
      ...
      : undefined;
    const gridEngineEnabledValue =
      this.props.configuration && gridEngineEnabled(this.props.configuration.parameters);
    const sparkEnabledValue =
      this.props.configuration && sparkEnabled(this.props.configuration.parameters);
    const slurmEnabledValue =
      this.props.configuration && slurmEnabled(this.props.configuration.parameters);
    const kubeEnabledValue =
      this.props.configuration && kubeEnabled(this.props.configuration.parameters);
    const rescheduleRunValue =
      this.props.configuration && rescheduleRunParameterValue(this.props.configuration.parameters);
    const autoScaledPriceTypeValue =
      this.props.configuration && getAutoScaledPriceTypeValue(this.props.configuration.parameters);
    ...
    return (
      configurationFormFieldChanged('is_spot') ||
      ...
      configurationFormFieldChanged('friendly_url', 'friendly_url', (v) =>
        v ? prettyUrlGenerator.build(v) : v,
      ) ||
      ...
      cloudRegionFieldChanged() ||
      additionalCapabilitiesChanged() ||
      kubeLabelsHasChanges(this.state.initialKubeLabels, this.state.kubeLabels) ||
      reservationParametersDiffer(
  ...
  onChangeClusterConfiguration = (configuration) => {
    ...
    this.setState(
      ...
      () => {
        ...
        const priceType =
          this.formRef.current.getFieldValue('is_spot') || this.getPriceTypeInitialValue();
        this.formRef.current.setFieldsValue({
          is_spot: this.correctPriceTypeValue(priceType),
        });
      },
  ...
  renderExecutionEnvironmentSummary = () => {
    ...
    let disk = +(this.formRef.current.getFieldValue('disk') || 0);
    let maxCPU = cpu;
    let maxRAM = ram;
    let maxGPU = gpu;
    let maxDISK = disk;
  ...
  correctSensitiveMounts = async (e) => {
    ...
    const storages = initialLimitMounts.split(',').map((id) => +id);
    await dataStorageAvailable.fetchIfNeededOrWait();
    const hasSensitive =
      (dataStorageAvailable.value || []).filter((s) => s.sensitive && storages.indexOf(+s.id) >= 0)
        .length > 0;
  ...
  onChangeRescheduleRun = (value) =>
    this.setState({
      rescheduleRun: value,
    });

  onChangeReservationParameters = (value) =>
    this.setState({
  ...
  renderShareSidName = (sid) => {
    ...
    const roles = this.props.roles?.loaded ? this.props.roles.value || [] : [];
    ...
    return (
      <Row style={{marginBottom: 10, marginTop: 10}}>
        <Col
          ...
          className={classNames('cp-accent', styles.toolSettingsTitle)}
        >
          Share with:
        </Col>
        <Col xs={24} sm={12}>
  ...
  renderExecutionEnvironment = () => {
    ...
    const renderExecutionEnvironmentSection = () => {
      const {preferences} = this.props;
      const skippedSystemParams = getSkippedSystemParametersList(this);
      const form = this.formRef.current;
      const getFieldValue = form ? (name) => form.getFieldValue(name) : () => undefined;
      ...
      return (
        <div>
          ...
          <Row>
            <Col sm={24} lg={12}>
              ...
              <Row>
                ...
                <Col xs={24} sm={12}>
                  <ReservationParameters
                    instanceType={this.getInstanceTypeValue()}
                    parameters={this.state.reservationParameters}
                    onChange={this.onChangeReservationParameters}
              ...
              >
                <Select
                  disabled={
                    this.state.pending || this.props.readOnly || isInstanceTypeWithReservation
                  }
                  onChange={this.handleIsSpotChange}
                >
                  {this.allowedPriceTypes.map((t) => (
                    <Select.Option key={`${t.isSpot}`}>{t.name}</Select.Option>
                  ))}
                </Select>
              </Form.Item>
                ...
                rules={[
                  ...
                  },
                  {
                    validator: (rule, value) => {
                      if (!isNaN(value)) {
                        if (+value > 15360) {
                          return Promise.reject(new Error('Maximum value is 15360'));
                        }
                        if (+value < 15) {
                          return Promise.reject(new Error('Minimum value is 15'));
                        }
                      }
              ...
              {!this.isWindowsPlatform && (
                ...
                >
                  <div>
                    ...
                    >
                      <Form.Item style={{marginBottom: 0, flex: 1}} name="limitMounts">
                        <LimitMountsInput
              ...
              >
                <Checkbox disabled={this.state.pending || this.props.readOnly} />
              </Form.Item>
              {!this.isWindowsPlatform && (
                <Form.Item
                  {...this.formItemLayout}
                ...
                >
                  <Checkbox
                    disabled={
                      this.state.pending || this.props.readOnly || this.props.mode === 'version'
                    }
              ...
              )}
              {
                <Row style={{marginBottom: 10, marginTop: 10}}>
                  ...
                  <Col xs={24} sm={12}>
                    <KubeLabels labels={this.state.kubeLabels} onChange={this.onKubeLabelsChange} />
                  </Col>
                </Row>
              }
              {!this.isWindowsPlatform && (
              ...
              <ConfigureClusterDialog
                instanceName={
                  this.formRef.current && this.formRef.current.getFieldValue('instanceType')
                }
              ...
              <Row style={{marginBottom: 10}}>
                <Col xs={24} sm={6} className={styles.toolSettingsTitle}>
                  Cmd template:
                </Col>
                <Col xs={24} sm={12}>
                  <CodeEditor
          ...
          <EditToolFormParameters
            ...
            isSystemParameters
            getSystemParameterDisabledState={(parameterName) =>
              skippedSystemParams.indexOf(parameterName) >= 0
            }
            skippedSystemParameters={skippedSystemParams}
            value={this.defaultSystemProperties}
            onInitialized={this.onEditToolFormSystemParametersInitialized}
            testSkipParameter={(name) => isCustomCapability(name, preferences)}
          />
  ...
  getFormInitialValues = () => {
    return {
      friendly_url: prettyUrlGenerator.parse(this.getPrettyUrlInitialValue()),
      instanceType: this.getInstanceTypeInitialValue(),
      instanceImage: this.getInstanceImageInitialValue(),
      is_spot: this.getPriceTypeInitialValue(),
      disk: this.getDiskInitialValue(),
      limitMounts: this.defaultLimitMounts || '',
      allowCommit: this.getAllowCommitInitialValue(),
      allowSensitive: this.getAllowSensitiveInitialValue(),
      cloudRegionId: this.getCloudRegionInitialValue(),
      endpoints: this.props.tool ? (this.props.tool.endpoints || []).map((e) => e) : [],
    };
  };
  ...
  render() {
    ...
    return (
      <Form ref={this.formRef} initialValues={this.getFormInitialValues()}>
