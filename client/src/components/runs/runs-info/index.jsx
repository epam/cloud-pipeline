...
class RunsInfo extends React.Component {
  ...
  get filtersApplied() {
    ...
    const {owners = [], instanceTypes = [], tags = [], dockerImages = [], statuses = []} = filters;
    return [owners, instanceTypes, tags, dockerImages, statuses].some(
  ...
  fetchFiltersConfiguration = async () => {
    ...
    this.setState({
      filtersConfigurationPending: false,
      filtersConfiguration: Object.entries(request.value || {}).reduce((acc, [key, value = {}]) => {
        acc[key] = [
          ...new Set([...Object.keys(value.PAUSED || {}), ...Object.keys(value.RUNNING || {})]),
        ];
        return acc;
      }, {}),
    });
  ...
  renderFilters = () => {
    ...
    const {owners = [], dockerImages = [], instanceTypes = [], tags = []} = filtersConfiguration;
    ...
    return (
      <div style={{display: 'flex', alignItems: 'flex-start', gap: 5}}>
          ...
          {dockerImages.length ? (
            ...
            >
              {dockerImages.map((docker) => (
                <Select.Option key={docker} value={docker} title={formatDockerImage(docker)}>
        ...
        <div style={{marginLeft: 'auto'}}>
          ...
          <Button onClick={this.fetchStatistics} style={{marginLeft: 5}}>
            Refresh
          </Button>
        </div>
      </div>
  ...
  onEntryClick(entry, field) {
    ...
    if (!detailsFilters.statuses || detailsFilters.statuses.length === 0) {
      detailsFilters.statuses = ['RUNNING', 'PAUSING', 'PAUSED', 'RESUMING'];
    }
  ...
  render() {
    ...
    const {statistics, topEntries} = this.state;
    const {owners, dockerImages, instanceTypes, tags} = extractDatasets(statistics, topEntries);
    const determineWidth = (labels = []) => (labels.length > LABELS_THRESHOLD ? '100%' : '50%');
    const charts = [
      {
        key: 'owners',
        title: 'Owners',
      ...
      >
        ...
        <div style={{display: 'flex', flexWrap: 'wrap'}}>
          {charts.map(({title, data, key}) => {
            ...
            const dataSets = Object.entries(rest || {}).map(([key, dataset]) => ({
              label: key,
              data: dataset,
