...
function HcsImageChannelsControl({allowLockChannels = true, hcsViewerState}) {
  const {channels = [], pending, allChannelsLocked, lockedChannels} = hcsViewerState || {};
    ...
    <div className={styles.channels}>
      <div className={styles.header}>
        ...
        {allowLockChannels && (
          <Checkbox
            ...
            indeterminate={lockedChannels.length > 0 && lockedChannels.length < channels.length}
          >
      ...
      {channels.map((channel) => (
        <Channel
          ...
          onLockedChanged={(locked) => hcsViewerState.setChannelLocked(channel, locked)}
          onContrastLimitsChanged={(limits) =>
            hcsViewerState.changeChannelContrastLimits(channel, limits)
          }
          onColorChanged={(color) => hcsViewerState.changeChannelColor(channel, color)}
        />
      ))}
    </div>
  );
