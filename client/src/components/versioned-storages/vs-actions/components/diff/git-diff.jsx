...
class GitDiff extends React.Component {
  ...
  fetchDiffs = () => {
    const {run, storage, fileDiffs, mergeInProgress} = this.props;
    if (run && storage && fileDiffs) {
      this.setState(
        {
          files: (fileDiffs || []).map((f) => f.path),
          diffs: (fileDiffs || []).reduce(
            (res, cur) => ({
              ...res,
              [cur.path]: {
                pending: true,
        ...
        () => {
          const fetchSingleFileDiff = (fileDiff) => {
            loadDiff(run, storage, fileDiff, mergeInProgress).then((result) => {
  ...
  onSelectAll = () => {
    ...
    if (onSelectionChanged) {
      onSelectionChanged(this.groupedFiles.tracked);
    }
  };
      ...
      <div className={styles.container}>
        {selectable && (
          <div className={styles.selectionActions}>
            ...
            <div>
              <Button onClick={this.onSelectAll} disabled={allFilesSelected}>
                Select All
              </Button>
              <Button onClick={this.onClearAll} disabled={!selectedFiles.length}>
                Clear selection
              </Button>
            </div>
          </div>
        )}
        {this.groupedFiles.tracked.map((file) => (
          <FileDiffPresenter
            key={file}
            file={file}
