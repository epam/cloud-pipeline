...
export default class LaunchForm extends React.Component {
  ...
  get dockerRegistry() {
    if (this.registries.length > 0 && this.formStore.toolInfo) {
      return this.registries.find((r) => r.id === this.formStore.toolInfo.registryId);
    }
    return null;
  }
  ...
  initializeToolData = async () => {
    ...
    const {tool} = getDockerImage(dockerImage, this.props.dockerRegistries) || {};
    if (!tool) {
    ...
    await Promise.all([toolVersions, toolInfo, versions].map((request) => request.fetch()));
    if (
      !toolInfo.loaded ||
      toolInfo.error ||
      !toolVersions.loaded ||
    ...
    this.formStore.initializeData({
      ...
      toolVersion: toolVersions.value.find(
        ({version}) => version === (toolInfo.value.version || 'latest'),
      ),
  ...
  initializePipelineData = async () => {
    ...
    const rawPipeline = pipelines.value.find((pipeline) => {
      return pipelineId ? pipeline.id === pipelineId : pipeline.name === pipelineName;
    });
    if (!rawPipeline) {
    ...
    await Promise.all(
      [pipelineRequest, versionsRequest].map((request) =>
        request.fetchIfNeededOrWait ? request.fetchIfNeededOrWait() : request.fetch(),
      ),
    );
  ...
  runTool = async () => {
    this.setState({launchPending: true}, async () => {
      ...
      const versionSettingValue = (settingName) => {
        if (
          this.formStore.toolVersion?.settings &&
      ...
      <div className={classNames(styles.launchForm, 'cp-panel', this.props.className)}>
        ...
        <div className={styles.controlBtn}>
          ...
          >
            ...
            {this.formStore.runLaunched ? 'LAUNCHED' : 'LAUNCH'}
          </Button>
        </div>
      </div>
    );
