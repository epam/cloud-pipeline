...
@Component
@Slf4j
public class DisableNewJobsQuotaHandler implements QuotaHandler {

    @Override
    public QuotaActionType type() {
        return QuotaActionType.DISABLE_NEW_JOBS;
    }

    @Override
    public void applyActionType(final AppliedQuota appliedQuota, final QuotaActionType type) {
        log.debug("Running new jobs will be disabled for {} {}", appliedQuota.getQuota().getType(),
                appliedQuota.getQuota().getSubject());
    }
}
