...
import {Checkbox, Dropdown, Form, Modal, Button, Row, Input, Select, Space} from 'antd';
import {DeleteOutlined, DownOutlined} from '@ant-design/icons';
import SelectMetadataItems from './SelectMetadataItems';
import compareArrays from '../../../../utils/compareArrays';
import styles from './AddInstanceForm.module.css';
...
export default class AddInstanceForm extends React.Component {
  ...
  onSelectEntityType = (id) => {
    if (this.formRef.current) {
      this.formRef.current.setFieldsValue({entityClass: id});
    }
    this.rebuildEntityTypeParameters(id);
  };

  rebuildEntityTypeParameters = (entityTypeId) => {
    const [entityTypeInfo] = (this.props.entityTypes || []).filter(
      (e) => `${e.metadataClass.id}` === `${entityTypeId}`,
    );
    let fields = [];
    if (entityTypeInfo) {
  ...
  validateCustomParameterName = (name, identifier) => {
    ...
    const [predefinedField] = this.state.fields.filter(
      (f) => f.name.toLowerCase() === name.toLowerCase(),
    );
    const [customField] = this.state.customFields.filter(
      (f) => f.identifier !== identifier && f.name.toLowerCase() === name.toLowerCase(),
    );
  ...
  renderReferenceField = (field, index, custom = false) => {
    ...
    trs.push(
      <div key={`parameter_${index}`} className={styles.parameterRow}>
  ...
  render() {
    ...
    const footer = this.props.pending ? (
      ...
      <Row type="flex" justify="end" className={styles.actions}>
        <Button id="add-instance-form-cancel-button" onClick={this.props.onCancel}>
          Cancel
        </Button>
        <Button
    ...
    return (
      ...
      >
        <Form
          ...
          initialValues={{
            entityClass:
              this.props.entityType !== undefined && this.props.entityType !== null
        ...
        >
          ...
          {this.state.fields &&
            ...
            this.state.fields.map(this.renderField(false)).reduce((array, trs) => {
              array.push(...trs);
              return array;
            }, [])}
          {this.state.customFields.map(this.renderField(true)).reduce((array, trs) => {
            array.push(...trs);
            return array;
          }, [])}
          ...
          <SelectMetadataItems
            ...
            type={this.state.selectMetadataItems ? this.state.selectMetadataItems.type : undefined}
            selection={this.state.selectMetadataItems ? this.state.selectMetadataItems.value : []}
            multiple={
              this.state.selectMetadataItems ? this.state.selectMetadataItems.multiple : false
            }
  ...
  UNSAFE_componentWillReceiveProps(nextProps) {
    ...
    if (
      this.props.entityTypes !== nextProps.entityTypes ||
