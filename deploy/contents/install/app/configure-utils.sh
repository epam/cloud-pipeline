...
function api_get_region_id {
    local region_name="$1"
    local regions_json=$(call_api "/entities?identifier=$region_name&aclClass=AWS_REGION" "$CP_API_JWT_ADMIN")
...
function api_register_region {
    ...
    if [ "$CP_CLOUD_PLATFORM" == "$CP_AWS" ]; then
        ...
        local cors_rules="$(get_file_based_preference storage.cors.policy $CP_AWS)"
        ...
        if [ "$CP_CLOUD_REGION_FILE_STORAGE_HOSTS" ]; then
            local region_fs_targets=", \"efsHosts\":[${CP_CLOUD_REGION_FILE_STORAGE_HOSTS}]"
        fi
...
read -r -d '' payload <<-EOF
{
    ...
    "corsRules":"$cors_rules" ${region_fs_targets}
}
EOF
        call_api_register_region_response=$(call_api "/aws/region" "$CP_API_JWT_ADMIN" "$payload")
    ...
    if [ $call_api_grant_region_result -ne 0 ]; then
        print_err "Error occured while granting permissions for $region_role_grant to $region_name region (id: $CP_CLOUD_REGION_INTERNAL_ID):"
        return 1
    else
...
function get_file_based_preference {
    ...
    local pref_dir=${CP_PREFERENCES_CONFIG_PATH:-"$INSTALL_SCRIPT_PATH/../preferences"}
    if [ "$cloud_provider" ]; then
        pref_dir=${CP_CLOUD_PREFERENCES_CONFIG_PATH:-"$CP_CLOUD_CONFIG_PATH/preferences"}
    fi
...
function api_setup_base_preferences {
    ...
    api_set_preference "cluster.instance.image" "$CP_PREF_CLUSTER_INSTANCE_IMAGE" "true"
    ...
    ## Storage
    # FIXME: storage.temp.credentials.role is required only for AWS
    if [ "$CP_PREF_STORAGE_TEMP_CREDENTIALS_ROLE" ]; then
        api_set_preference "storage.temp.credentials.role" "${CP_PREF_STORAGE_TEMP_CREDENTIALS_ROLE}" "false"
    else
        print_warn "\"storage.temp.credentials.role\" preference is NOT set. \"pipe storage ...\" commands will NOT work correctly. Specify it using \"-env CP_PREF_STORAGE_TEMP_CREDENTIALS_ROLE=\" option"
    fi
    if [ "$CP_AWS_KMS_ARN" ]; then
        api_set_preference "storage.security.key.arn" $CP_AWS_KMS_ARN "false"
        api_set_preference "storage.security.key.id" "$(id_from_arn $CP_AWS_KMS_ARN)" "false"
    else
        print_warn "\"storage.security.key.arn\" and \"storage.security.key.id\" preferences are NOT set. Nodes creation and data management will NOT work correctly. Specify it using \"-env CP_AWS_KMS_ARN=\" option"
    fi
    ...
    ## Cloud specific hacks
    if [ "$CP_CLOUD_PLATFORM" == "$CP_AWS" ]; then
        api_set_preference "cluster.ssh.key.name" "$CP_PREF_CLUSTER_SSH_KEY_NAME" "false"
    fi

    if [ "$CP_CLOUD_PLATFORM" == "$CP_AZURE" ]; then
        api_set_preference "cluster.ssh.key.path" "$CP_PREF_CLUSTER_SSH_KEY_PATH" "false"
    fi
