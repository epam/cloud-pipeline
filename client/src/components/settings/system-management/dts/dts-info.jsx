...
import styles from './dts-info.module.css';
...
class DtsInfo extends React.Component {
  ...
  validate = () => {
    ...
    if (onChange) {
      onChange(modifiedPreferences.length > 0);
    }
  };
  ...
  onChangePreference = (preference, update) => {
    ...
    if (preference.draft) {
      const draftIndex = drafts.findIndex(({draftIndex}) => draftIndex === preference.draftIndex);
      if (draftIndex >= 0) {
        drafts.splice(draftIndex, 1, update);
        this.setState({drafts: [...drafts]});
      }
    } else {
      const preferenceIndex = preferences.findIndex(({key}) => key === preference.key);
  ...
  renderCheckboxPreference = (preference, index) => {
    ...
    return (
      <div key={preference.key} style={{margin: '10px 0'}}>
        <Checkbox onChange={onChange} checked={checked} className={styles.preference}>
          <b>{preference.key}</b>
        </Checkbox>
      </div>
    );
  ...
  getPreferenceError = (preference) => {
    ...
    return errors.find(({text, preference: errorPreference}) => {
      ...
      return errorPreference.draftIndex === undefined && preference.key === errorPreference.key;
    });
  };
  ...
  renderStringPreference = (preference, index) => {
    ...
    return (
      <div
        key={index}
        className={classNames(styles.preferenceContainer, {
          [styles.error]: !!error,
        })}
      >
        <div
          className={classNames(styles.preference, styles.stringPreference, {
        ...
        >
          ...
          <div className={styles.inputContainer} style={{marginLeft: 3}}>
            <span>Value:</span>
            <Input size="small" value={preference.value} onChange={onChange('value')} />
          </div>
          <Button onClick={() => this.removePreference(preference)} size="small" danger>
            <DeleteOutlined />
          </Button>
        </div>
        {error ? (
          <div className={classNames('cp-error', {[styles.errorText]: !!error})}>{error.text}</div>
        ) : null}
      </div>
    );
  };
  ...
  renderDtsTab = () => {
    ...
    return (
      <div style={{display: 'flex', flexDirection: 'column'}}>
        ...
        <Button onClick={this.addPreference} style={{width: 150}} size="small">
          <PlusOutlined /> Add preference
        </Button>
        <div style={{display: 'flex', justifyContent: 'flex-end', gap: 5}}>
          <Button onClick={this.onDeleteDts} size="small" danger style={{marginRight: 15}}>
            Delete
          </Button>
          <Button disabled={modifiedPreferences.length === 0} onClick={this.onRevert} size="small">
            Revert
          </Button>
          <Button
        ...
        items={[
          {
            key: 'dts',
            label: 'DTS',
            children: <Spin spinning={pending}>{this.renderDtsTab()}</Spin>,
          },
          ...(logsFolder
            ? [
                {
