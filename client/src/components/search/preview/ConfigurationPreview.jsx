...
@inject((stores, params) => {
  const {cloudProviders, configurations, dtsList, pipelines, preferences, runDefaultParameters} =
    stores;
...
export default class ConfigurationPreview extends React.Component {
  ...
  componentDidUpdate() {
    ...
    if (
      ...
      this.props.runDefaultParameters.value.length &&
      !this._systemParams
    ) {
  ...
  get description() {
    ...
    if (
      ...
      this.props.configuration.loaded &&
  ...
  get configurationEntry() {
    ...
    return this.props.configuration.value.entries.filter((e) => e.name === this.props.entryName)[0];
  }
  ...
  get currentCloudProvider() {
    if (
      ...
      const [provider] = (this.props.cloudProviders.value || []).filter(
        (p) => p.id === this.configurationEntry.configuration.cloudProviderId,
      );
      return provider;
    }
    return null;
  ...
  get isDtsEnvironment() {
    return (
      this.configurationEntry && this.configurationEntry.executionEnvironment === DTS_ENVIRONMENT
    );
  }
      ...
      <div className={styles.contentPreview}>
        <table>
          <tbody>
            {this.pipelineRow}
            <tr>
              <td className={firstCellClass}>Execution environment</td>
              <td>{this.ExecEnvString}</td>
            </tr>
            {this.isDtsEnvironment && configuration.coresNumber && (
              <tr>
                <td className={firstCellClass}>Cores</td>
                <td>{configuration.coresNumber}</td>
              </tr>
            )}
            {dockerImage && (
              <tr>
                <td className={firstCellClass}>Docker image</td>
                <td>{dockerImage}</td>
              </tr>
            )}
            {configuration.configuration && configuration.configuration.instance_size && (
              <tr>
                <td className={firstCellClass}>Instance type</td>
                <td>{configuration.configuration.instance_size}</td>
              </tr>
            )}
            {cloudRegion && (
              <tr>
                <td className={firstCellClass}>Cloud region</td>
                <td>{cloudRegion}</td>
              </tr>
            )}
            {configuration.configuration && configuration.configuration.instance_disk && (
              <tr>
                <td className={firstCellClass}>Disk size (Gb)</td>
                <td>{configuration.configuration.instance_disk}</td>
              </tr>
            )}
          </tbody>
        </table>
  ...
  renderAdvancedSection = () => {
    ...
    const cmdTemplate = this.isDtsEnvironment
      ...
      : configuration.configuration.cmd_template;
    const timeout =
      !this.isDtsEnvironment && !this.isFireCloudEnvironment
      ...
      <div className={styles.contentPreview}>
        <table>
          <tbody>
            {!this.isDtsEnvironment && !this.isFireCloudEnvironment && (
              <tr>
                <td className={firstCellClass}>Price type</td>
                <td>{getSpotTypeName(this.isSpot, this.currentCloudProvider)}</td>
              </tr>
            )}
            {timeout !== null && (
              <tr>
                <td className={firstCellClass}>Timeout (minutes)</td>
                <td>{timeout}</td>
              </tr>
            )}
            {cmdTemplate && (
              <tr>
                <td className={firstCellClass}>Cmd template</td>
                <td>{cmdTemplate}</td>
              </tr>
            )}
          </tbody>
  ...
  renderParameters = (isSystemSection = false) => {
    ...
    for (const key in parameters) {
      if (Object.hasOwn(parameters, key)) {
        ...
        items.push(
          <tr key={key}>
            <td className={classNames(styles.firstCell, styles.bold)}>{key}</td>
            <td>{parameters[key].value}</td>
          </tr>,
        );
      }
    }
        ...
        <table>
          <tbody>
            {parameters.map((param) => (
              <tr key={param.name}>
                <td className={classNames(styles.firstCell, styles.bold)}>{param.name}</td>
                <td>{param.value}</td>
              </tr>
            ))}
          </tbody>
