...
import {NotificationPreferences, PreferencesSectionTitle} from './configuration';
...
class NotificationPreferencesControl extends React.Component {
  ...
  componentDidUpdate(prevProps, prevState, snapshot) {
    if (this.props.session !== prevProps.session || this.props.type !== prevProps.type) {
      this.update();
    }
  }
  ...
  onChange = () => {
    ...
    if (onChange) {
      onChange(
        Object.values(this.state.values).map((v) => v.value),
        modified,
      );
    }
  };
