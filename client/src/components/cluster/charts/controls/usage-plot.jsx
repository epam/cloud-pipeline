...
class UsagePlot extends React.Component {
  ...
  renderSingleUsageBar = (config, index, barHeight, totalCount) => {
    const {chartArea, data, fontSize, height, margin, width} = this.props;
    if (!chartArea || !config || !data || !data.data || !data.data[config.group]) {
      return null;
    }
    ...
    const y =
      chartArea.top +
      (height - chartArea.top - chartArea.bottom) / 2.0 -
      (totalCount / 2.0 - index) * barHeight;
  ...
  render() {
    ...
    return (
      <svg width={width} height={height} shapeRendering={'crispEdges'}>
        {config.map((c, index) => this.renderSingleUsageBar(c, index, barHeight, config.length))}
      </svg>
    );
  }
}
