...
import styles from './EmbeddedMiew.module.css';
...
export default class MiewPage extends React.Component {
          ...
          <Col span={20} className={styles.toolbarActions}>
            ...
            >
              {this.displayModes.map((displayMode) => {
                return (
                  <Select.Option key={displayMode.mode} value={displayMode.mode}>
                    {displayMode.title}
                  </Select.Option>
                );
              })}
            ...
            >
              {this.colorModes.map((colorMode) => {
                return (
                  <Select.Option key={colorMode.mode} value={colorMode.mode}>
                    {colorMode.title}
                  </Select.Option>
                );
              })}
  ...
  componentDidMount() {
    if (this.embeddedMiew && this.miewTerminal) {
      $(this.miewTerminal).terminal(this.onCommand, {
