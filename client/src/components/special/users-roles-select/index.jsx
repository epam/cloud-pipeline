...
class UsersRolesSelect extends React.Component {
  ...
  get items() {
    const {searchString = '', adGroups = []} = this.state;
    const {value = [], showRoles = true, adGroups: showADGroups = showRoles} = this.props;
    ...
    const itemIsSelected = (item) =>
      !!value.find((v) => v.name === item.name && v.principal === item.principal);
    ...
    if (searchString.length >= MINIMUM_SEARCH_LENGTH) {
      ...
      return items
        .filter(
          (item) =>
            itemIsSelected(item) || !!item.search.find((o) => o.includes(lowerCasedSearchString)),
        )
        .sort(nameSorter);
    }
    return items.filter(itemIsSelected).sort(nameSorter);
  }

  onChangeSearchString = (e) => {
    const {showRoles = true, adGroups: showADGroups = showRoles} = this.props;
    this.setState(
      {
        searchString: e || '',
      },
  ...
  render() {
    ...
    const placeholder = showRoles ? 'Specify user or group name' : 'Specify user name';
    return (
      <Select
        disabled={disabled}
        className={className}
        ...
        getPopupContainer={(o) => (popupContainerFn ? popupContainerFn(o) : o.parentNode)}
          ...
          {this.items
            ...
            .map((user) => (
              <Select.Option
                ...
                title={user.name}
              >
                <UserName userName={user.name} showIcon />
              </Select.Option>
            ))}
        </Select.OptGroup>
