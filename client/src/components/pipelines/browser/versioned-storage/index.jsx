...
import {Alert, message, Pagination, Row, Splitter} from 'antd';
...
import VersionedStorageListWithInfo from '../../../../models/versioned-storage/vs-contents-with-info';
...
import styles from './versioned-storage.module.css';
...
function generateItemsFilter(preferences) {
  const ignorePatterns = (preferences.versionStorageIgnoredFiles || []).map((pattern) =>
    pattern.startsWith('/') ? new RegExp(`^${pattern}`, 'i') : new RegExp(`${pattern}$`, 'i'),
  );
...
class VersionedStorage extends localization.LocalizedReactComponent {
  ...
  componentDidUpdate(prevProps) {
    if (prevProps.path !== this.props.path || prevProps.pipelineId !== this.props.pipelineId) {
  ...
  get lastCommitId() {
    ...
    if (pipeline && pipeline.value && pipeline.loaded && pipeline.value.currentVersion) {
  ...
  get writeAllowed() {
    const {pipeline} = this.props;
    if (pipeline && pipeline.value && pipeline.loaded) {
  ...
  fetchPage = (page) => {
    const {path, pipelineId} = this.props;
    this.setState(
      {
        pending: true,
      },
      () => {
        ...
        const request = new VersionedStorageListWithInfo(pipelineId, {
          page,
          pageSize: PAGE_SIZE,
          path,
        });
        request
          ...
          .then(() => {
            if (request.error || !request.loaded) {
              ...
              const {listing = [], max_page: lastPage} = request.value;
  ...
  refreshSelectedFile = (path) => {
    ...
    if (path) {
      const request = new PipelineFileInfo(this.props.pipelineId, undefined, path);
      request
        .fetch()
        .then(() => {
          if (request.loaded) {
  ...
  openLaunchVSAdvancedForm = (tool, tag) => {
    const {router, pipelineId, pipeline} = this.props;
  ...
  launchVS = (tool, tag) => {
    const {awsRegions, preferences, pipeline, pipelineId, router} = this.props;
    this.closeLaunchVSForm();
    const hide = message.loading('Launching Versioned Storage...', 0);
    const {currentVersion = {}} = pipeline.value || {};
  ...
  editVersionedStorage = async ({name, description, token}) => {
    const {folders, onReloadTree, pipeline, pipelinesLibrary} = this.props;
    if (pipeline && pipeline.loaded) {
      const {id, parentFolderId} = pipeline.value;
  ...
  deleteVersionedStorage = async (keepRepository) => {
    const {folders, onReloadTree, pipeline, pipelinesLibrary} = this.props;
    if (pipeline && pipeline.loaded) {
      const {id, name, parentFolderId} = pipeline.value;
  ...
  onDeleteDocument = async (document, comment) => {
    if (this.lastCommitId) {
      const {selectedFile} = this.state;
      const {pipeline, pipelineId, folders, pipelinesLibrary} = this.props;
      let request;
  ...
  onRenameDocument = async ({name, content}) => {
    const {renameDocument, selectedFile} = this.state;
    const {path} = this.props;
    if (this.lastCommitId) {
      const {pipeline, pipelineId, folders, pipelinesLibrary} = this.props;
      let request;
  ...
  afterUpload = async (files = []) => {
    const {pipeline, folders, pipelinesLibrary, path} = this.props;
    const {selectedFile} = this.state;
    const pathCorrected = path && path.endsWith('/') ? path.slice(0, -1) : path;
    const uploadedFiles = files.map((file) =>
      pathCorrected && pathCorrected.length ? `${pathCorrected}/${file}` : file,
    );
    const parentFolderId = pipeline.value.parentFolderId;
  ...
  generateReport = async (settings = {}) => {
    ...
    const {authors, extensions, dateFrom, dateTo, includeDiff, splitDiffsBy, downloadAsArchive} =
      settings;
    ...
    if (request.error) {
      ...
      if (request.value.type?.includes('application/json')) {
        checkForBlobErrors(request.value).then((error) =>
          error ? message.error(error, 5) : downloadBlob(request.value, `${fileName}.${extension}`),
        );
      } else {
  ...
  render() {
    const {pipeline, pipelineId, readOnly, path, preferences} = this.props;
    const {error, lastPage, page, selectedFile, generateReportDialog, launchVSFormVisible} =
      this.state;
    const {editStorageDialog, showHistoryPanel, pending} = this.state;
    if ((!pipeline.loaded && pipeline.pending) || (!preferences.loaded && preferences.pending)) {
      return <LoadingView />;
    }
    if (pipeline.error) {
      return <Alert type="error" title={pipeline.error} />;
    }
    const tableContent = (
        ...
        <div className={styles.paginationRow}>
          {lastPage >= 0 && (
            <Pagination
              ...
              size="small"
              onChange={(newPage) =>
                newPage === page + 1 ? undefined : this.fetchPage(newPage - 1)
              }
            />
          )}
        </div>
      ...
      <div className={styles.vsContainer} style={{height: 'calc(100vh - 30px)'}}>
        ...
        {showHistoryPanel ? (
          <Splitter style={{flex: 1, overflow: 'auto', width: 'inherited', height: 'auto'}}>
            <Splitter.Panel defaultSize="75%" min="33%" style={{overflow: 'auto'}}>
              {tableContent}
            </Splitter.Panel>
            <Splitter.Panel
              defaultSize="25%"
            ...
            >
              ...
              >
                ...
                <CloseOutlined onClick={this.closeHistoryPanel} style={{cursor: 'pointer'}} />
              </Row>
              <div
                style={{
                  display: 'flex',
