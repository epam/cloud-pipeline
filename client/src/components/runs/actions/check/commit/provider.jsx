...
const {CheckProvider, Warning, store, inject, getCheckInfo, getCheckResult} = generateProvider({
  check: commitCheck,
...
function CommitCheckProvider({active = true, children, runId, skipContainerCheck}) {
  return (
    <CheckProvider active={active} objectId={runId} skipContainerCheck={skipContainerCheck}>
      {children}
    </CheckProvider>
  );
}
