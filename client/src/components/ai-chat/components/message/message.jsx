...
export default class Message extends React.Component {
  ...
  renderMessagePart = (part, message) => {
    const {role = 'user', pending = false} = message;
    const assistantMessage = role.toLowerCase() === 'assistant';
    const {identifier, text, type = 'text', data = {}, errors = [], warnings = [], status} = part;
    const contentParts = [];
    if (type.toLowerCase() === 'launch') {
      const {launch_payload: launchPayload} = data;
      if (launchPayload) {
        contentParts.push(
          <LaunchForm
    ...
    if (type.toLowerCase() === 'run') {
      const {run_payload: runPayload} = data;
      if (runPayload) {
        contentParts.push(<RunStatusMessage key={`${identifier}-run`} run={runPayload} />);
      }
    }
          ...
          title={
            <div>
              {warnings.map((warn, idx) => (
                <Markdown key={`error-${idx}`} md={warn} />
              ))}
            </div>
          }
          showIcon
  ...
  renderContent = () => {
    const {message} = this;
    const {identifier, parts = [], pending = false, status} = message;
    return (
      <div key={identifier} style={{width: '100%'}} className={styles.messageParts}>
        {parts.map((part) => this.renderMessagePart(part, message))}
        {pending && (
          <div className="cp-text-not-important" style={{fontSize: 'smaller', minHeight: 24}}>
            {status}
          </div>
        )}
        {pending && (
  ...
  render() {
    const {className, style, message, first, last} = this.props;
    const {role = 'user'} = message;
    const userMessage = role.toLowerCase() === 'user';
    return (
      <div
        className={classNames(className, 'cp-panel', 'cp-panel-no-hover', styles.message, {
          [styles.first]: first,
          [styles.last]: last,
