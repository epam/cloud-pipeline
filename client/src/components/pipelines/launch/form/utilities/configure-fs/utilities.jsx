...
export const CP_CAP_FS_PARAMETERS_HINTS = {
  [CP_CAP_SHARE_FS_TYPE]: (
    <div>Defines which file system shall be used as shared for cluster runs</div>
  ),
  [CP_CAP_SHARE_FS_DEPLOYMENT_TYPE]: (
    <div>Allows to specify shared file deployment type (supported for Lustre FS only)</div>
  ),
  [CP_CAP_SHARE_FS_SIZE]: (
    <div>Allows to specify shared file system size (supported for Lustre FS only)</div>
  ),
  [CP_CAP_SHARE_FS_THROUGHPUT]: (
    <div>
  ...
  [CP_CAP_SHARE_FS_IOPS]: (
    <div>Allows to specify shared file system IOPS (supported for persistent Lustre FS only).</div>
  ),
};
...
export function getDeploymentType(parameters) {
  ...
  if (fsType === ShareFsType.lustre) {
    ...
    const deploymentType =
      getParameterValue(parameters, CP_CAP_SHARE_FS_DEPLOYMENT_TYPE, defaultType) || defaultType;
...
export function validateFsDeploymentType(config) {
  const {fsType = ShareFsType.lfs, deploymentType: original} = config || {};
...
export function validateFsVolume(config) {
  const {fsType = ShareFsType.lfs, volume: original} = config || {};
...
export function validateFsThroughput(config) {
  const {fsType = ShareFsType.lfs, deploymentType, throughput: original} = config || {};
...
export function validateFsIops(config) {
  const {fsType = ShareFsType.lfs, deploymentType, iops: original} = config || {};
...
export function normalizeFsConfig(config) {
  const {fsType = ShareFsType.lfs} = config || {};
  if (fsType === ShareFsType.lfs) {
    return {
      fsType,
      deploymentType: undefined,
...
export function getParametersFromFsConfig(fsConfig, parameters, provider = undefined) {
  ...
  if (fsConfig) {
    const {fsType, deploymentType, volume, throughput, iops} = normalizeFsConfig(fsConfig);
