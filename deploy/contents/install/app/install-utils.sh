...
function parse_options {
    ...
    if is_deployment_type_requested aws-native ; then
        if [ -z "$CP_CSI_DRIVER_TYPE" ] || [ -z "$CP_CSI_EXECUTION_ROLE" ] || [ -z "$CP_SYSTEM_FILESYSTEM_ID" ] || [ -z "$CP_MAIN_SERVICE_ROLE" ] ; then
            print_err "For AWS native deployment you must set variable CP_MAIN_SERVICE_ROLE, CP_CSI_DRIVER_TYPE, CP_SYSTEM_FILESYSTEM_ID and CP_CSI_EXECUTION_ROLE to mount EFS or FSx filesystem"
            return 1
        fi
