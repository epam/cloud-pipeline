...
class RunsInfoChart extends Component {
  ...
  componentDidUpdate(prevProps) {
    if (this.ctx) {
      ...
      const {data, options, title, displayEmptyTitleRow, onEntryClick} = this.props;
      if (
        data !== prevData ||
        options !== prevOptions ||
        title !== prevTitle ||
  ...
  chartRef = (ctx, props) => {
    if (ctx) {
      this.ctx = ctx;
      const {data, options = {}, title, displayEmptyTitleRow, onEntryClick} = props || this.props;
      const {labels = []} = data;
      ...
      const opts = {
        ...
        hover: {
          onHover: function (e) {
            ...
            e.target.style.cursor = point.length > 0 ? 'pointer' : 'default';
          },
        },
        tooltips: {
          intersect: false,
