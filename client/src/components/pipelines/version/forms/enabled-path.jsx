...
class EnabledPath extends React.Component {
  ...
  toggleEnabled = (event) => {
    this.setState(
      ...
      () => {
        if (this.state.enabled && this.ref && typeof this.ref.focus === 'function') {
          this.ref.focus();
          this.setState(
            {
  ...
  render() {
    const {className, disabled, style, placeholder} = this.props;
    const {enabled, value} = this.state;
    return (
      <div
        className={className}
      ...
      >
        <Checkbox disabled={disabled} checked={enabled} onChange={this.toggleEnabled}>
          Enabled
        </Checkbox>
        <Input
