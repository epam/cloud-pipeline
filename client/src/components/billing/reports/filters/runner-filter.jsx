...
function runnersEqual(runnersA, runnersB) {
  ...
  const idsA = (a && (Array.isArray(a) || isObservableArray(a)) ? a : []).sort(defaultSorter);
  const idsB = (b && (Array.isArray(b) || isObservableArray(b)) ? b : []).sort(defaultSorter);
...
function getUserSearchOptions(user) {
  if (user) {
    const searchParts = [(user.name || '').toLowerCase()];
    if (user.attributes) {
...
function RenderUserName({user, myUserName}) {
  ...
  const you = user.name === myUserName ? <b>{' (you)'}</b> : undefined;
...
class RunnerFilter extends React.Component {
  ...
  get currentRunnerValue() {
    ...
    const items = this.users.map((user) => ({
      ...
      label: user.name === this.myUserName ? `${user.name} (you)` : user.name,
    }));
    items.push(
  ...
  get users() {
    const {usersInfo: usersRequest} = this.props;
    let users = [];
  ...
  get centers() {
    const {billingCenters: billingCentersRequest} = this.props;
    let centers = [];
    if (billingCentersRequest && billingCentersRequest.loaded) {
      centers = (billingCentersRequest.value || [])
        .map((c) => c)
        .sort((a, b) => {
          if (a > b) {
            return 1;
  ...
  findRunner = (search) => {
    ...
    this.setState(
      ...
      async () => {
        const {billingCenters: billingCentersRequest, usersInfo: usersRequest} = this.props;
        ...
        if (initialSearch === searchCriteria) {
          if (!searchCriteria || !searchCriteria.length) {
            ...
            const filteredCenters = this.centers.filter((center) =>
              filterRunner([(center || '').toLowerCase()], searchCriteria),
            );
            const filteredUsers = this.users.filter((user) =>
              filterRunner(user.searchOptions, searchCriteria),
            );
            const filteredAdGroups = this.adGroups.filter((group) =>
              filterRunner([(group || '').toLowerCase()], searchCriteria),
            );
            this.setState({
              filteredCenters,
              filteredUsers,
  ...
  render() {
    const {focused, filteredCenters, filteredUsers, filteredAdGroups, searchCriteria} = this.state;
    const showBillingCenters = (filteredCenters || this.centers).length > 0;
    const showUsers = filteredUsers.length > 0;
    const open = focused && showBillingCenters && showUsers;
    return (
      <Select
        mode="multiple"
        labelInValue
        ...
        notFoundContent={searchCriteria ? 'Not found' : 'Specify user or billing center name'}
...
export default inject(
  'billingCenters',
  'usersInfo',
)(roleModel.authenticationInfo(BillingNavigation.attach(observer(RunnerFilter))));
