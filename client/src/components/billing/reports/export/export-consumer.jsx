...
class ExportConsumer extends React.Component {
  ...
  getExportData = (...options) => {
    const {discounts: discountsConfiguration, exportConfiguration} = this.props;
    const [defaultOptions = {}] = options;
    const {format} = defaultOptions;
    const {computeValue: computes = 0, storageValue: storages = 0} = discountsConfiguration || {};
    const discount = {
      computes: -computes,
      storages: -storages,
    };
    ...
    const {start, endStrict: end} = getPeriod(period, range);
    const filters = {
      owner: user,
...
const ExportConsumerInjected = inject('export')(observer(ExportConsumer));
