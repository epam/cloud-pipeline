...
import dayjs, {ensureDayjs, toUtcDayjs} from '../../../../../utils/dayjs';
...
import styles from './users-activity-chart.module.css';
...
function fetchData(options) {
  return new Promise((resolve, reject) => {
    const {period, range, runner = []} = options || {};
    ...
    request
      ...
      .then(() => {
        if (request.loaded) {
          const data = (request.value || []).map((item) => ({
...
class Chart extends React.PureComponent {
  ...
  render() {
    ...
    return <canvas ref={this.initializeCanvas} className={className} />;
  }
}
...
class UsersActivityChart extends React.Component {
  ...
  get backgroundColor() {
    const {themes} = this.props;
    if (themes && themes.currentThemeConfiguration) {
      return themes.currentThemeConfiguration['--cp-color-bg-elevated'] || DEFAULT_BACKGROUND_COLOR;
    }
  ...
  get data() {
    const {data = [], mode = Modes.average} = this.state;
  ...
  componentDidUpdate(prevProps, prevState, snapshot) {
    ...
    const {period = Period.day, range, runner} = filters;
    if (
        ...
        const setError = (error) =>
          new Promise((resolve) => {
            ...
            if (requestedPeriod === period && requestedRange === range) {
              this.setState({error, pending: false}, () => resolve());
            }
          });
        const setData = (data = []) =>
          new Promise((resolve) => {
            ...
            if (requestedPeriod === period && requestedRange === range) {
              ...
              const periodTitle =
                period === Period.day ? start.format('D MMMM, YYYY') : start.format('MMMM YYYY');
              const result = ticks.map((tick) => {
                const value = data.find(
                  (o) => ensureDayjs(o.date).format(format) === tick.tick.format(format),
                );
                if (value !== undefined) {
                  return {
                    total: value.total,
                    ...
                    date: toUtcDayjs(value.date),
                  };
                }
                return {
                  active: undefined,
                  total: undefined,
                  date: dayjs(tick.tick),
                  activeUsers: [],
                  totalUsers: [],
                };
              });
  ...
  renderTooltipInfo = () => {
    const {data = [], tooltip, period, mode} = this.state;
    if (tooltip) {
      const {index} = tooltip;
      const info = data[index];
      if (info) {
        const format = period === Period.day ? 'D MMMM YYYY, HH:mm' : 'D MMMM YYYY';
        const {date, activeUsers = [], totalUsers = []} = info;
        const users = mode === Modes.average ? activeUsers : totalUsers;
        return (
          <div className={styles.tooltip}>
            <div className={styles.header}>Online users at {date.format(format)}</div>
            <div className={styles.usersList}>
              {users.map((user) => (
                <UserName key={`${user}`} className={styles.user} userName={`${user}`} />
              ))}
            </div>
          </div>
        );
  ...
  onItemClick = (index, {x, y}) => {
    ...
    const clickIsAvalable =
      (period === Period.month && mode !== Modes.average) || period === Period.day;
  ...
  render() {
    ...
    return (
      <div className={classNames(className, styles.container, 'cp-panel')} style={style}>
...
export default inject('themes', 'users')(UsageNavigation.attach(observer(UsersActivityChart)));
