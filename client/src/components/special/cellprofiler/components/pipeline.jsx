...
class CellProfilerPipeline extends React.Component {
  ...
  render() {
    const {pipeline, viewer, className, style, hidden} = this.props;
    if (!pipeline || hidden) {
      return null;
    }
    let infoHeader = (
    ...
    return (
      <div className={className} style={style}>
        <Collapse header={infoHeader}>
          <AnalysisPipelineInfo pipeline={pipeline} />
        </Collapse>
        {(pipeline.modules || [])
        ...
        {pipeline && pipeline.objects && pipeline.objects.length > 0 && (
          <Collapse
            header={
              <CellProfilerModuleHeader
          ...
          >
            <CellProfilerModule cpModule={pipeline.defineResults} />
          </Collapse>
        )}
        {pipeline &&
          ...
          pipeline.graphicsOutput.configurations.length > 0 && (
            <Collapse header="Display objects">
              <ObjectsOutlineRenderer pipeline={pipeline} viewer={viewer} />
            </Collapse>
          )}
      </div>
    );
