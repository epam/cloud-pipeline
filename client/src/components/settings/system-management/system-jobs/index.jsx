...
class SystemJobs extends React.Component {
  ...
  get currentJobParameters() {
    ...
    return Object.entries(this.jobScriptParameters[selectedJobId] || {}).map(
      ([parameter, info]) => ({
        ...info,
        parameter,
  ...
  get currentJobHasRequiredParameters() {
    return this.currentJobParameters.some((parameter) => parameter.required);
  }
  ...
  launchJob = (job, parameters) => {
    const {systemJobs} = this.props;
    const {launchingJobs = [], refreshToken = 0} = this.state;
    ...
    this.setState(
      ...
      async () => {
        try {
          ...
          hide();
          const {launchingJobs: currentLaunchingJobs = []} = this.state;
          this.setState({
  ...
  renderSystemJob = (job, routing) => {
    ...
    const {launchingJobs = [], refreshToken} = this.state;
    ...
    const launchMenuItems = [{key: 'custom', label: 'Launch with parameters'}];
    return (
      <div className={styles.systemJobContainer}>
        <div className={styles.systemJobHeader}>
          <span className={styles.systemJobTitle}>{job.identifier}</span>
          <div className={styles.systemJobActions}>
            <Button size="small" className={styles.systemJobActionButton} onClick={onRefresh}>
              REFRESH
            </Button>
            {isLaunching || this.currentJobHasRequiredParameters ? (
              <Button
                ...
                onClick={!isLaunching ? () => this.openLaunchParametersModal(job) : undefined}
              >
                LAUNCH
              </Button>
            ) : (
              <Space.Compact style={{marginLeft: 5}}>
                <Button type="primary" size="small" onClick={() => this.launchJob(job)}>
                  LAUNCH
                </Button>
                <Dropdown
                  menu={{
        ...
        <RunTable
          ...
          customRunRenderer={(run) => (
            <div className={styles.systemJobRun}>
              <StatusIcon className={styles.systemJobStatusIcon} run={run} small />
              <b style={{marginLeft: 5}}>#{run.id}</b>
              <UserName userName={run.owner} showIcon style={{marginLeft: 5}} />
              <span>
  ...
  render() {
    const {systemJobs = {}} = this.props;
    const {loaded, pending, error} = systemJobs;
    const {jobs} = this;
    if (pending && !loaded) {
      return <LoadingView />;
    }
    ...
    if (!jobs || !jobs.length) {
      return <Alert title="System jobs not configured" type="info" />;
    }
    ...
    const sections = jobs.map((aJob) => ({
      ...
      render: () => this.renderSystemJob(aJob, routing),
    }));
    const {job, launchParameters} = this.state;
    return (
      <SubSettings
