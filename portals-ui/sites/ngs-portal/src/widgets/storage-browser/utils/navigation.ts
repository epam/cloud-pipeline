...
export function ensureMarkersForPath(
  path = '',
  markers: PageMarkers = {},
): PageMarkers {
...
export function insertNextPageMarker(
  ...
  const { currentPage, markers: pathMarkers = [undefined] } =
    newMarkers[correctedPath];
  if (currentPage + 1 <= pathMarkers.length && marker) {
    const newPathMarkers = pathMarkers
...
export function setCurrentPage(
  ...
  const correctedPath = correctPath(path) || ROOT_PLACEHOLDER;
  const newMarkers = ensureMarkersForPath(correctedPath, markers);
  const { currentPage = 0, markers: pathMarkers = [undefined] } =
    newMarkers[correctedPath];
