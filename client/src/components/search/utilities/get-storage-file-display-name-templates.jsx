...
function parseTemplate(storageId, template) {
  ...
  const displayName = (item) => {
    ...
    for (const tag of tags) {
      const {placeholder, tagName, defaultValue, flags = []} = tag;
 ...
 */
export async function getStorageFileDisplayNameTemplates(preferences) {
  if (!preferences) {
    return [];
  }
    ...
    if (attribute) {
      ...
      if (request.loaded) {
        return (request.value || [])
          ...
          .map((metadata) =>
            parseTemplate(Number(metadata.entity.entityId), metadata.data[attribute].value),
          );
      } else {
        return [];
      }
