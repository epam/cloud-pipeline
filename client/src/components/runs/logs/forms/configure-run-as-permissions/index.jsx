...
class ConfigureRunAsPermissions extends React.Component {
  ...
  get availablePipelines() {
    ...
    if (pipelines.loaded && pipelines.loaded) {
  ...
  renderToolsSection = () => {
    ...
    return (
      <div style={sectionStyles}>
        <div>
          <Checkbox checked={toolsAllowed} disabled={disabled} onChange={this.onChangeToolsAllowed}>
            Allow docker images
          </Checkbox>
        </div>
        {toolsAllowed && (
  ...
  render() {
    const {className, style, sid} = this.props;
    if (!sid) {
      return null;
    }
    return (
