...
class AddModulesButton extends React.Component {
  ...
  render() {
    ...
    const filtered = allModules
      ...
      .filter(
        (module) => !filter || (module.name || '').toLowerCase().includes(filter.toLowerCase()),
      );
    const onSelect = ({key}) => {
    ...
    const rootLevelModules = filtered.filter(
      (module) => !module.group || !groups.includes(module.group),
    );
    const menuItems = [
    ...
    const menu = (
      <div className={classNames(styles.modulesDropdownContainer, 'cp-panel')}>
        <div className={styles.modulesFilter}>
          <Input
            value={filter}
        ...
        {filtered.length === 0 && (
          <div
            className={classNames(styles.modulesDropdown, 'cp-text-not-important')}
            style={{
              padding: 5,
              lineHeight: '24px',
            }}
