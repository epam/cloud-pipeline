...
> The version is checked before the launch, see [Run a tool](#run-a-tool).
...
Before the launch, `pipe` checks that the specified version (tag) of the Docker image exists. If the version is not specified, `latest` is checked.  
If the tool has no such version, the run is not scheduled - `pipe` prints an error with the list of the available versions and exits with the code `1`, e.g.:

``` bash
pipe run -di library/ubuntu:latet -it n1-highcpu-2 -id 27 -cmd "sleep infinity" -y
```

``` text
Docker image library/ubuntu:latet has no tag "latet". Available tags: 18.04, latest
```

The same check is performed when the `-di` (`--docker-image`) option is used to change the Docker image of a pipeline run.

> **_Note_**: if `pipe` can't load the tool versions (e.g. the user has no `READ` permission on the tool, or the Docker registry is unavailable), it prints a warning and continues the launch. Whether the run is allowed is still decided by the Cloud Pipeline API.
