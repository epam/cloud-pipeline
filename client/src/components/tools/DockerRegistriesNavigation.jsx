...
export default class DockerRegistriesNavigation extends React.Component {
  ...
  renderRegistrySelector = () => {
    if (this.props.currentRegistry) {
      if (
        this.props.registries.filter((r) => r.id !== this.props.currentRegistry.id).length === 0
      ) {
        return (
          <Button
      ...
      const registries = this.props.registries
        .filter((r) => r.id !== this.props.currentRegistry.id)
  ...
  renderGroupSelector = () => {
    ...
    if (group) {
      const getGroupName = (group, filter = '') => {
        if (filter) {
          const currentFilter = (this.props.filters.groups || []).find(
            (group) => group.id === filter,
          );
          return currentFilter ? currentFilter.title : filter;
        }
      ...
      const groups = this.props.groups
        .filter((g) => (group.privateGroup ? !g.privateGroup : g.id !== group.id))
        .sort((groupA, groupB) => {
