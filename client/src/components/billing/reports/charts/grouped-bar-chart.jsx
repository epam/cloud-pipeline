...
class GroupedBarChart extends React.Component {
  ...
  generateImage = () => {
    ...
    const totalWidth = (groups || [])
      ...
      .map(({width}) => width || 0)
      .reduce((r, c) => r + c, 0);
    const reports = (groups || []).map((group) => this.charts[group] || null).filter(Boolean);
    const titleHeight = 30;
    const totalHeight =
      titleHeight +
      Math.max(
        0,
        ...(groups || []).map((group) => this.charts[group] || {}).map(({height}) => height || 0),
      );
    return new Promise((resolve) => {
      ...
      ctx.fillText(title, totalWidth / 2, titleHeight);
      let x = 0;
  ...
  render() {
    const {title, height, request, discountsMapper, onSelect, displayQuotasSummary} = this.props;
    const {data, itemsCount, groups, total, quotaGroups} = this.billingData;
          ...
          {groups.length > 0
            ? groups.map((group, index) => (
                <BarChart
                  ...
                  request={request}
                  discounts={
                    discountsMapper && Object.hasOwn(discountsMapper, group)
...
const GroupedBarChartWithImageGenerator = Export.ImageConsumer.Generator(observer(GroupedBarChart));
