...
import {Alert, Button, Input, Dropdown, Tabs, Splitter} from 'antd';
import {
  CaretDownOutlined,
  CaretUpOutlined,
  CloseCircleFilled,
...
import styles from './FacetedSearch.module.css';
...
class FacetedSearch extends React.Component {
  ...
  componentDidMount() {
    ...
    if (facetedFilters) {
      const {query, filters, advanced = false} = facetedFilters;
      this.setState(
        {
          query,
  ...
  get documentTypeFilter() {
    const {facetsLoaded, facetsCount, initialFacetsCount = {}, disableCounts} = this.state;
    ...
    return {
      ...
      values: Object.keys(filter)
        ...
        .filter(
          (v) =>
            disableCounts ||
            (FacetedSearch.HIDE_VALUE_IF_EMPTY
              ? v.count > 0
              : Object.hasOwn(initialFacetsCount, DocumentTypeFilterName) &&
                Object.hasOwn(initialFacetsCount[DocumentTypeFilterName], v.name) &&
  ...
  get filters() {
    const {facetsLoaded, facets, facetsCount, initialFacetsCount = {}, disableCounts} = this.state;
    if (!facetsLoaded || !facetsCount) {
      return [];
    }
    return facets
      .filter(
        (d) =>
          disableCounts ||
          (FacetedSearch.HIDE_VALUE_IF_EMPTY
            ? Object.hasOwn(initialFacetsCount, d.name)
            : Object.hasOwn(facetsCount, d.name)),
      )
      .map((d) => ({
        name: d.name,
        domain: d.domain,
            ...
            (v) =>
              ...
              (FacetedSearch.HIDE_VALUE_IF_EMPTY
                ...
                : initialFacetsCount[d.name] &&
                  Object.hasOwn(initialFacetsCount[d.name], v.name) &&
  ...
  get filterDomains() {
    const {createOtherDomain} = this.state;
    const filters = this.filters.filter((f) => f.name !== DocumentTypeFilterName);
    const domains = [...new Set(filters.map((filter) => filter.domain).concat(undefined))].sort(
      (a, b) => {
        if (!a) {
          return 1;
        }
    ...
    return domains
      .map((domain) => ({
        domain,
        enabled: filters.filter((f) => f.domain === domain).some((f) => f.values.length > 0),
      }))
  ...
  get filteredSortingFields() {
    ...
    return getAvailableSortingFields(this.columns).filter((key) => !excludedKeys.includes(key));
  }
  ...
  get metadataFields() {
    const {storageFileDisplayNameTemplates = [], facets = []} = this.state;
    const additionalTags = [
      ...new Set(
        storageFileDisplayNameTemplates.reduce(
          (result, current) => [...result, ...current.tags],
          [],
        ),
      ),
    ];
    return [
      ...new Set(
        this.allColumns
          ...
          .filter(
            (column) => !DocumentColumns.some((documentColumn) => documentColumn.key === column),
          ),
      ),
    ];
  }
  ...
  get documentTypes() {
    const {activeFilters, userDocumentTypes = []} = this.state;
    return Array.from(
      new Set([
  ...
  get allColumns() {
    ...
    if (!documentTypes || !documentTypes.length) {
      ...
      return all.filter(
        (column) => !column.types || documentTypes.find((type) => column.types.has(type)),
      );
    }
  }
  ...
  getFilterPreferences = (filterName) => {
    ...
    if (systemDictionaries.loaded && facetedFiltersDictionaries) {
      const [filter] = (facetedFiltersDictionaries.dictionaries || []).filter(
        (dict) => dict.dictionary === filterName,
      );
      if (!filter) {
        return null;
      }
      ...
      let preferenceAmount =
        filter.defaultDictEntriesToDisplay !== undefined
      ...
      if (preferenceAmount >= 0 || minByActiveFilters) {
        entriesToDisplay = Math.max(...[minByActiveFilters, preferenceAmount].filter(Number));
      }
      return {
        entriesToDisplay,
      };
  ...
  fetchNotDownloadableStorages = () => {
    ...
    getNotDownloadableStorages(authenticatedUserInfo, preferences).then((storages = []) =>
      this.setState({
  ...
  onChangeFilter = (group) => (selection) => {
    ...
    this.setState({activeFilters: newFilters}, () => {
      this.correctSorting().then(() => this.doSearch(undefined, true));
    });
  };
  ...
  updateCurrentRouting = () => {
    const {query, activeFilters, advancedSearchMode} = this.state;
    let queryString = facetedQueryString.build(query, activeFilters, advancedSearchMode);
    if (queryString) {
    ...
    this.setState(
      ...
      () => {
        this.loadFacets().then(() => {
          ...
          if (facets.length === 0) {
            console.warn(
              'No facets configured. Please, check "faceted.filter.dictionaries" preference and system dictionaries',
            );
            this.setState({
              pending: false,
            });
          ...
          this.setState({searchToken}, () => {
            ...
            facetsSearch({
              ...
              const {error, facetsCount, facetsToken, documents: receivedDocuments = []} = result;
              const {searchToken: actualSearchToken, facetsToken: actualFacetsToken} = this.state;
              const state = {};
              let documents;
              ...
              documents = documents.map((document) => ({
                ...
                downloadOverride: this.downloadFileTag ? document[this.downloadFileTag] : undefined,
              }));
    ...
    return new Promise((resolve) => {
      const onDone = (storageFileDisplayNameTemplates = []) => {
        ...
        const documentTypes = searchDocumentTypes
          .map((type) =>
            Object.hasOwn(SearchGroupTypes, type) ? SearchGroupTypes[type].types : [],
          )
          .reduce((r, c) => [...r, ...c], []);
        if (systemDictionaries.loaded && configuration) {
          ...
          const orders = dictionaries
            ...
            .reduce((r, c) => ({...r, ...c}), {});
          const filtered = (systemDictionaries.value || []).filter((d) =>
            Object.hasOwn(orders, d.key),
          );
          filtered.sort((a, b) => orders[a.key] - orders[b.key]);
          ...
          fetchFacets(
            ...
            const {facetsCount, facetsToken} = result || {};
            if (result && result.aborted) {
              return;
            }
            this.setState(
              ...
              () => {
                ...
                this.correctSorting().then(resolve);
              },
            );
          });
        } else {
  ...
  onLoadNextPage = (document, forward = true) => {
    const {sortingOrder} = this.state;
    if (document) {
      const docSortFields = sortingOrder.reduce(
        (acc, {field}) => ({
          ...acc,
  ...
  onDeselectItem = (item) => {
    ...
    const selectedItems = this.state.selectedItems.filter(filterFn);
  ...
  renderSortingControls = () => {
    const {sortingOrder, pending} = this.state;
      ...
      <div className={styles.sortingControlsContainer}>
        ...
        {sortingOrder.map((sort) => (
          ...
          >
            ...
            <CloseOutlined
              className={classNames(styles.removeSortingBtn, 'cp-icon-button', {
                'cp-disabled': pending,
              })}
  ...
  renderControlsRow = () => {
    const {presentationMode} = this.state;
    return (
      <div className={classNames(styles.actions, 'cp-search-actions')}>
        <TogglePresentationMode
  ...
  render() {
    ...
    if (systemDictionaries.error) {
      return <Alert title={systemDictionaries.error} type="error" />;
    }
    ...
    const inputSuffix = (
      <div className={styles.suffixContainer}>
        ...
        <span
          className={classNames(styles.searchModeButton, 'cp-button', {
        ...
        <div className={styles.content}>
          {!noFilters ? (
            <Splitter
              ...
              style={{height: '100%'}}
            >
              <Splitter.Panel defaultSize="25%" min={300} max="50%">
                <div className={classNames(styles.panel, styles.facetedFiltersContainer)}>
                  {this.filterDomains.length > 1 && (
                    <Tabs
                      className={classNames('cp-tabs-no-content', 'cp-faceted-filters')}
                      hideAdd
                      type="card"
                  ...
                  <div
                    className={classNames(styles.facetedFilters, {
                  ...
                  >
                    ...
                    {this.filters.filter(filterFacetByDomain).map((filter) => (
                      <FacetedFilter
                        key={filter.name}
                        name={filter.name}
