...
function parseFilters(filters = {}) {
  const {userNames = [], source} = filters;
  return {
    hcsFile: source,
    owners: userNames,
  };
...
class CellProfilerExternalJobs extends React.Component {
  ...
  get currentUserSelected() {
    const {owners = []} = parseFilters(this.props.filters);
  ...
  handleChangeOwnersFilters = (newOwners = []) => {
    ...
    if (typeof onChangeFilters === 'function') {
      const {userNames, ...restFilters} = filters;
  ...
  render() {
    const {pending, expanded} = this.state;
    const {className, style, selected, onSelect} = this.props;
    const {hcsFile, owners = []} = parseFilters(this.props.filters);
    if (!hcsFile) {
      return null;
    }
          ...
          <div>
            ...
            {!pending && filteredJobs.length === 0 && (
              <span>
                ...
                {owners.length > 0 && <span> and owner{owners.length > 1 ? 's' : ''} </span>}
                {owners.map((owner) => (
                  <UserName key={owner} userName={owner} style={{marginRight: 5}} showIcon />
                ))}
              </span>
            )}
