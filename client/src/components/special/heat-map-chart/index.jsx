...
import styles from './heat-map-chart.module.css';
...
class HeatMapChart extends React.Component {
  ...
  componentDidMount() {
    ...
    this.updateRange();
    if (this.renderer) {
      this.renderer.addEventListener(HeatmapTimelineChartEvents.scaleChanged, this.onRangeChanged);
      this.renderer.addEventListener(HeatmapTimelineChartEvents.onItemsHover, this.onHover);
    }
    if (themes) {
  ...
  onRangeChanged = (range) => {
    ...
    if (typeof onRangeChanged === 'function') {
      const {from, to, fromZoom = false, fromDrag = false} = range;
  ...
  startHoverPositioning = () => {
    ...
    const callback = () => {
      if (this.renderer && this.hoverContainer && this.hoverPosition) {
        ...
        const newY = Math.max(0, positionY - hoverHeight / 2.0);
  ...
  renderHoveredInfo = () => {
    ...
    return (
      <div
        key="hover-container"
        className={classNames(styles.hoverContainer, 'cp-panel')}
        style={{
