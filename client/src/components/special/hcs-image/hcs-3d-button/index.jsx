...
import {Slider, Select, Checkbox, Popover, Button} from 'antd';
import {DownOutlined} from '@ant-design/icons';
import displaySize from '../../../../utils/displaySize';
import styles from './hcs-3d-button.module.css';
...
function getSliceEnabled(range) {
  if (
    !range ||
    typeof range !== 'object' ||
    !(Array.isArray(range) || isObservableArray(range)) ||
    range.length !== 2
  ) {
    return false;
  }
  ...
  renderDropdownContent = () => {
    ...
    const TitleWrapper = ({title, children}) => (
      <div className={styles.selectorWrapper}>
        <span className={styles.title} style={{minWidth: 130}}>
          {title}
        </span>
    ...
    return (
      <div className={styles.overlayContainer}>
        ...
        <Checkbox checked={this.use3dMode} onChange={this.toggle3DMode} className={styles.title}>
          Use volume renderer
        </Checkbox>
