...
import styles from './PipelinesLibrary.module.css';
...
class PipelinesLibrary extends localization.LocalizedReactComponent {
  ...
  loadData = async (node) => {
    ...
    if (item.type === ItemTypes.pipeline && item.children.length === 0) {
      ...
      item.children = generateTreeData(childrenRequest.value, {
        parent: item,
        expandedKeys: getExpandedKeys(rootItems),
        filter: this.props.hiddenObjectsTreeFilter(),
      });
  ...
  renderItemTitle(item) {
    ...
    switch (item.type) {
      ...
      case ItemTypes.storage:
        ...
        subTitle = <AWSRegionTag className={styles.regionFlags} regionId={item.regionId} />;
        break;
      case ItemTypes.configuration:
    ...
    return (
      <span id={`pipelines-library-tree-node-${item.key}-name`} className={treeItemTitleClassName}>
        {IconComponent && (
          <IconComponent
  ...
  renderLibrary() {
    return (
      ...
      >
        <Row type="flex" justify="space-between" align="middle" className={styles.headerRow}>
  ...
  render() {
    if (!this.props.uiNavigation.libraryExpanded) {
      return (
        <PipelinesLibraryContent
          ...
          onReloadTree={(reloadRoot, folder) =>
            this.reloadTree(reloadRoot === undefined ? true : reloadRoot, folder)
          }
          style={{overflow: 'auto'}}
        >
          <Outlet
    ...
    return (
      <Row id="pipelines-library-container" className={styles.container}>
        <Splitter className="pipelines-library-split-pane" style={{height: '100%', width: '100%'}}>
          <Splitter.Panel
    ...
    switch (item.type) {
      ...
      case ItemTypes.projectHistory:
        if (item.id === 'root') {
          ...
          item.children = generateTreeData(this.props.pipelinesLibrary.value, {
            parent: item,
          ...
          const childExpandedKeys = getExpandedKeys(item.children);
          item.children = generateTreeData(reloadFolderRequest.value, {
            parent: item,
            expandedKeys: childExpandedKeys,
            filter: this.props.hiddenObjectsTreeFilter(),
          });
      ...
      case ItemTypes.metadata: {
        ...
        if (parentFolder) {
          if (parentFolder.id === 'root') {
            ...
            parentFolder.children = generateTreeData(this.props.pipelinesLibrary.value, {
              parent: parentFolder,
              expandedKeys: childExpandedKeys,
              filter: this.props.hiddenObjectsTreeFilter(),
            });
            ...
            const childExpandedKeys = getExpandedKeys(parentFolder.children);
            parentFolder.children = generateTreeData(reloadFolderRequest.value, {
              parent: parentFolder,
              expandedKeys: childExpandedKeys,
              filter: this.props.hiddenObjectsTreeFilter(),
            });
  ...
  async reloadTree(reload, folderToReload) {
    ...
    let metadataClass, selectedKey, history, metadata;
    const [placeholderOrPipelineId, idOrVersionName, section, subSection] = parts;
    ...
    const selectedItem = getTreeItemByKey(selectedKey, rootItems);
    if (selectedItem) {
