...
export default class EditPipelineForm extends localization.LocalizedReactComponent {
  ...
  handleSubmit = (e) => {
    ...
    this.formRef.current
      ...
      .then((values) => {
        ...
        if (Object.hasOwn(payload, 'githubRepository')) {
          payload.repository = payload.githubRepository;
          delete payload.githubRepository;
        }
        if (Object.hasOwn(payload, 'githubBranch')) {
  ...
  onRepositoryTypeChanged = (repositoryType) => {
    if (!this.props.pipeline && this.formRef.current) {
      ...
      this.formRef.current
        .validateFields(['repository', 'githubRepository', 'branch', 'githubBranch', 'token'])
        .catch(() => {});
    }
  };
  ...
  renderForm = () => {
    ...
    const nameLabel = isVersionedStorage ? 'Name:' : `${this.localizedString(objectName)} name`;
    const formItems = [];
    formItems.push(
      <Form.Item
        {...this.formItemLayout}
    ...
    return (
      <div className="cp-modal-footer-actions cp-modal-footer-actions--split">
        <div className="cp-modal-footer-actions-group">
          <Button id="edit-pipeline-delete-dialog-cancel-button" onClick={this.closeDeleteDialog}>
            Cancel
          </Button>
        </div>
  ...
  getModalFooter = (isNewPipeline, isVersionedStorage) => {
    ...
    const deleteAllowed =
      ...
      isManager;
    const saveAllowed = isNewPipeline ? isManager : roleModel.writeAllowed(this.props.pipeline);
                          ...
                          <PermissionsForm
                            ...
                            subObjectsPermissionsErrorTitle={
                              <span>
                                ...
                                used in a current {this.localizedString(objectName)}. Please review
                                and fix permissions issues below:
                              </span>
                            }
                            editOwnerAvailable={
  ...
  componentDidUpdate(prevProps) {
    if (prevProps.visible !== this.props.visible) {
      ...
      if (this.props.visible) {
        ...
        const githubType =
          pipeline && pipeline.repositoryType === RepositoryTypes.GitHub
