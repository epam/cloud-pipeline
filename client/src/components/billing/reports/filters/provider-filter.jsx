...
import styles from './provider-filter.module.css';
...
class ProviderFilter extends React.Component {
  ...
  updateValue = () => {
    ...
    this.correctValue(newValue).then((corrected) => {
      this.setState({
        filter: value,
  ...
  correct = () => {
    this.correctValue(this.state.value).then((corrected) => {
      if (corrected) {
  ...
  correctValue = async (value) => {
    ...
    if (cloudRegionsInfo.loaded) {
      ...
      const newValue = [];
      let modified = false;
      [...new Set((cloudRegionsInfo.value || []).map((o) => o.provider))].forEach((provider) => {
  ...
  onBlur = () => {
    this.setState({focused: false}, async () => {
      ...
      if (cloudRegionsInfo.loaded) {
        ...
        if (onChange && filterToString(correctedPayload) !== filterToString(region)) {
    ...
    return (
      <Select
        ...
        filterOption={(input, option) =>
          option.props.filter.some((f) => f.indexOf(input.toUpperCase()) >= 0)
        }
      ...
      >
        ...
        <Select.OptGroup label="Regions">
          {regions.map((region) => (
            ...
            >
              <AWSRegionTag showProvider regionUID={region.regionId} style={{fontSize: 'larger'}} />
...
export default inject('cloudRegionsInfo')(BillingNavigation.attach(observer(ProviderFilter)));
