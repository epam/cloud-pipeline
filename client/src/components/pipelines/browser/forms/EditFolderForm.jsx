...
export default class EditFolderForm extends React.Component {
  ...
  render() {
    const modalFooter =
      this.props.pending || this.state.activeTab !== 'info' ? (
        false
      ) : (
        <Row>
          <Button id="folder-edit-form-cancel-button" onClick={this.props.onCancel}>
            Cancel
          </Button>
          <Button
    ...
    const isEditInfo =
      !!this.props.folderId &&
      (roleModel.isOwner({mask: this.props.mask}) ||
        roleModel.writeAllowed({mask: this.props.mask}));
    const isEditPermissions = !!this.props.folderId && roleModel.isOwner({mask: this.props.mask});

    const InfoContent = (
      <Form ref={this.formRef} className="folder-edit-form" initialValues={{name: this.props.name}}>
        <Form.Item
