...
import styles from './quota-description.module.css';
...
function QuotaDescription({className, onClick, onRemove, quota, roles, style}) {
  if (!quota) {
    return null;
  }
  const {actions = [], value, period = 'MONTH', id} = quota;
    ...
    <div className={classNames(styles.container, className)} style={style} onClick={onClick}>
      <div className={styles.description}>
        <div style={{display: 'flex', alignItems: 'center'}}>
          ...
          <QuotaTarget addonAfter=":" quota={quota} roles={roles} />
          {quotaValue}
        </div>
      </div>
      <div className={styles.actionsContainer}>
        <div className={styles.actions}>
          {actions.sort(sortActionsByThreshold).map((action, index) => (
            <QuotaAction
        ...
        <Button icon={<CloseOutlined />} size="small" danger onClick={onRemoveQuota} />
      </div>
    </div>
  );
}
