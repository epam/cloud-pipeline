...
if [ -z "$CP_IMAGE_PREFIX" ]; then
    export CP_IMAGE_PREFIX="CloudPipeline_Image"
...
for region_item in "${regions_arr[@]}"; do
    ...
    build_ami   "${CP_CLOUD_PROVIDER}" \
                ...
                "${CP_SUBNET:-NA}" \
                "${CP_IMAGE_PREFIX}_Common" \
                "$INSTALL_SCRIPT_PATH/aws/install-common-node.sh" \
                "$CP_OUTPUT"
    ...
    build_ami   "${CP_CLOUD_PROVIDER}" \
                ...
                "${CP_SECURITY_GROUPS:-NA}" \
                "${CP_SUBNET:-NA}" \
                "${CP_IMAGE_PREFIX}_GPU" \
                "$INSTALL_SCRIPT_PATH/aws/install-gpu-node.sh" \
                "$CP_OUTPUT"
