...
class Folder extends localization.LocalizedReactComponent {
  ...
  get items() {
    ...
    if (folder.loaded) {
      ...
      return generateTreeData(payload, {
        ignoreChildren: true,
    ...
    return (
      ...
      >
        <div key={key} className={styles.metadataItemContainer}>
          ...
          <Row className={classNames(styles.metadataItemValue, 'cp-library-metadata-item-value')}>
            {value}
          </Row>
        </div>
      </Tooltip>
  ...
  renderContent = () => {
    ...
    const items = this.items.filter(filterItem);
    const columns = [
      {
        key: 'type',
        ...
        render: (name, item) => {
          ...
          if (item.locked) {
            nameComponent = (
              <span>
                <LockOutlined />
                {item.type === ItemTypes.storage && <AWSRegionTag regionId={item.regionId} />}
                <span className={`object-name ${styles.objectNameBold}`}>{nameSearch}</span>
              </span>
            );
          } else {
            nameComponent = (
      ...
      <div style={{display: 'flex', flexDirection: 'column', height: '100%'}}>
        ...
        <Row type="flex" align="middle" style={{margin: '5px 0px'}}>
          <Input
            ...
            placeholder={
              isStorages ? 'Search storages' : `Search ${this.localizedString('pipeline')}s`
            }
          />
        </Row>
        {this.renderContent()}
