...
function RunOperationWarningAlert({className, style, type, showIcon, message, checkResult}) {
...
class RunOperationWarning extends React.PureComponent {
  ...
  componentDidUpdate(prevProps, prevState, snapshot) {
    const {optionsComparator = (a, b) => a === b} = this.props;
  ...
  checkRun = () => {
    const {objectId, check, options} = this.props;
    if (objectId && typeof check === 'function') {
      this.setState(
        ...
        () => {
          check(objectId, options).then((result) =>
            this.setState({
              pending: false,
              checkResult: result,
            }),
  ...
  render() {
    const {className, style, objectId, type = 'error', showIcon, warning} = this.props;
    const {pending, checkResult} = this.state;
    if (!objectId || pending) {
      return null;
    }
