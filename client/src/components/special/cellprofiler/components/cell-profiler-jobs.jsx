...
class CellProfilerJobs extends React.Component {
  ...
  get selectedJobId() {
    const {jobId, onJobSelected} = this.props;
  ...
  get filters() {
    const {filters: propsFilters, onFiltersChange} = this.props;
  ...
  onSelectJob = (job) => {
    const {onJobSelected} = this.props;
    if (typeof onJobSelected === 'function') {
      // controlled
  ...
  render() {
    ...
    const {error, pending, blocking, jobs = [], total = 0, page = 0, pageSize} = this.state;
    return (
      <div
        ...
        style={style}
      >
        <CellProfilerJobsFilters filters={this.filters} onChange={this.onChangeFilters} />
        ...
        />
        {error && <Alert className={styles.cellProfilerJobsRow} title={error} type="error" />}
        {!pending && !total && (
          <Alert className={styles.cellProfilerJobsRow} title="Jobs not found" type="warning" />
        )}
        <div className={styles.cellProfilerJobs} ref={this.onInitializeJobsContainer}>
          {jobs.length === 0 && pending && (
            <div className={styles.cellProfilerJobsRow}>
              <LoadingOutlined />
          ...
          {jobs.slice(0, pageSize).map((aJob) => (
            <CellProfilerJob
              key={aJob.id}
              job={aJob}
              className={classNames('cell-profiler-job', {
        ...
        {this.paginationControlAvailable && (
          <div className={styles.cellProfilerPagination}>
            <Pagination
              disabled={blocking}
              total={total}
              current={page + 1}
