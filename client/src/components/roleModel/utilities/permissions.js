...
export function getPermissionChanges(state) {
  const {owner, originalOwner, permissions, originalPermissions} = state;
  const updateOwner = originalOwner !== owner && owner !== undefined;
  const removedPermissions = originalPermissions.filter(
    (p) => !permissions.some(findPermissionByPermission(p)),
  );
  const updatedPermissions = permissions.filter((p) =>
    originalPermissions.some((op) => permissionSidsEqual(p.sid, op.sid) && p.mask !== op.mask),
  );
  const createdPermissions = permissions.filter(
    (p) => !originalPermissions.some(findPermissionByPermission(p)),
  );
  return {
...
async function removePermission(permission, objectIdentifier, objectType) {
  ...
  const {sid} = permission;
  const {principal, name} = sid;
  console.log(
    `Removing ${name} ${principal ? 'user' : 'group'} permission of ${objectType} #${objectIdentifier}`,
  );
  const request = new GrantRemove(objectIdentifier, objectType.toUpperCase(), name, principal);
  await request.fetch();
  if (request.error) {
    console.log(request.error);
    throw new Error(request.error);
...
async function changePermission(permission, objectIdentifier, objectType) {
  ...
  const {sid, mask} = permission;
  const {principal, name} = sid;
  console.log(
    `Changing ${name} ${principal ? 'user' : 'group'} permission of ${objectType} #${objectIdentifier}, new mask: ${mask} (${Number(mask).toString(2).padStart(8, '0')})`,
  );
...
export async function applyPermissionChanges(changes, objectIdentifier, objectType) {
  const {owner, removedPermissions, createdPermissions, updatedPermissions} = changes;
