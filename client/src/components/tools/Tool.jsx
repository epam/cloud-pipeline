...
import {getVersionRunningInfo, ScanStatuses} from './utils';
import {CONTENT_PANEL_KEY, METADATA_PANEL_KEY, ISSUES_PANEL_KEY} from '../special/splitPanel';
...
class Tool extends localization.LocalizedReactComponent {
  ...
  get defaultVersionSettingsConfiguration() {
    if (this.defaultVersionSettings && this.defaultVersionSettings.loaded) {
      if (
        (this.defaultVersionSettings.value || []).length > 0 &&
  ...
  get registries() {
    if (this.props.docker.loaded) {
      return this.props.hiddenToolsTreeFilter(this.props.docker.value).registries;
    }
    return [];
  }
  ...
  get dockerImageWithoutVersion() {
    ...
    return registry ? `${registry.path}/${image}` : `${image}`;
  }

  get dockerRegistry() {
    if (this.registries.length > 0 && this.props.tool.loaded) {
      return this.registries.find((r) => r.id === this.props.tool.value.registryId);
    }
    return null;
  }
  ...
  get toolGroup() {
    ...
    if (dockerRegistry && tool && tool.loaded) {
      const {toolGroupId} = tool.value;
      const {groups = []} = dockerRegistry;
  ...
  updateTool = async (values, configuration, allowCommit) => {
    ...
    if (request.error) {
      ...
      if (this.defaultVersionSettings) {
        await this.defaultVersionSettings.fetch();
      }
      if (this.props.versionSettings) {
        await this.props.versionSettings.fetch();
      }
    }
  };
  ...
  deleteToolConfirm = () => {
    ...
    deleteToolConfirmModal({tool: this.props.toolId, link: this.link}, this.props.router).then(
      (confirm) => {
        if (confirm) {
  ...
  deleteToolVersionConfirm = (version) => {
    const deleteToolVersion = this.deleteToolVersion;
    deleteToolConfirmModal({tool: this.props.toolId, version}, this.props.router).then(
      (confirm) => {
        if (confirm) {
  ...
  deleteToolVersion = async (version) => {
    ...
    const hide = message.loading(
      version ? `Removing version '${version}'...` : 'Removing tool...',
      0,
    );
  ...
  onSave = (isShortDescription) => async () => {
    ...
    const tool = {
      ...
      labels: this.props.tool.value.labels
        ? (this.props.tool.value.labels || []).map((l) => l)
        : undefined,
      endpoints: this.props.tool.value.endpoints
      ...
      shortDescription: isShortDescription
        ...
        : this.props.tool.value.shortDescription,
    };
    await request.send(tool);
    hide();
    if (request.error) {
  ...
  renderToolImageControl = () => {
    ...
    return (
      <div
        className={classNames(styles.toolImageContainer, 'cp-tool-icon-container')}
  ...
  renderDescription = () => {
    const renderShortDescription = () => {
      if (this.state.editShortDescriptionMode) {
        ...
        if (shortDescription && shortDescription.trim().length) {
          return (
            <span id="short-description-text" className={styles.description}>
              {this.props.tool.value.shortDescription}
            </span>
          );
        } else {
    ...
    const renderDescription = () => {
      if (this.state.editDescriptionMode) {
        ...
        if (description && description.trim().length) {
          return <Markdown id="description-text-container" md={description} />;
        } else {
          return (
            <span
              id="description-text"
    ...
    const renderActions = (isShortDescription) => {
      ...
      const method = isShortDescription
        ? this.toggleEditShortDescriptionMode
      ...
      return <Row className={styles.descriptionActions}>{buttons}</Row>;
    };
    ...
    return (
      <div className={styles.descriptionContainer}>
        ...
        <Row type="flex" style={{marginBottom: 10}}>
          ...
          <div style={{flex: 1}}>{shortDescriptionAndPullCommand}</div>
        </Row>
        <div className="cp-tool-panel">
          <Row
            type="flex"
  ...
  isAdvancedUser = () => {
    ...
    if (authenticatedUserInfo.loaded) {
      const {roles = []} = authenticatedUserInfo.value;
      ...
      switch (item.status.toUpperCase()) {
        ...
        case ScanStatuses.pending:
          if (item.successScanDate) {
            scanningInfo = (
              <span>
                Scanning in progress. Last successful scan: {displayDate(item.successScanDate)}
              </span>
            );
          } else {
        ...
        case ScanStatuses.failed:
          if (item.successScanDate) {
            scanningInfo = (
              <span>
                Scanning{' '}
                <span className={classNames(styles.scanningError, 'cp-error')}>failed</span>
                {item.scanDate ? ` at ${displayDate(item.scanDate)}` : ''}. Last successful scan:{' '}
                {displayDate(item.successScanDate)}
              </span>
            );
          } else {
            scanningInfo = (
              <span>
                Scanning{' '}
                <span className={classNames(styles.scanningError, 'cp-error')}>failed</span>
                {item.scanDate ? ` at ${displayDate(item.scanDate)}` : ''}
              </span>
            );
          }
          break;
  ...
  get toolVersionScanResults() {
    ...
    if (
      ...
      versions.forEach((currentVersion) => {
        ...
        const digestAliases =
          versionAttributes && versionAttributes.digest
            ? versionsByDigest[versionAttributes.digest].filter(
        ...
        data.push({
          ...
          modificationDate:
            versionAttributes && versionAttributes.modificationDate
          ...
          vulnerabilitiesStatistics:
            scanResult.status === ScanStatuses.notScanned
              ? null
              : {
                  critical: Critical,
                  high: High,
  ...
  get toolVersionOS() {
    const scanInfo = this.toolVersionScanResults.find((o) => o.name === this.defaultTag);
    if (scanInfo && scanInfo.toolOSVersion && scanInfo.toolOSVersion.distribution) {
      const {distribution, version = ''} = scanInfo.toolOSVersion;
  ...
  renderDigestAliases = (aliases) => {
    const renderAlias = (alias) => {
      return (
        <span key={alias} className={classNames(styles.versionAliasItem, 'cp-tag')}>
          {alias}
        </span>
      );
    };
      ...
      return (
        <Row>
          ...
          >
            <Popover
              content={
                <div style={{minWidth: 200, display: 'flex'}}>{aliases.map(renderAlias)}</div>
              }
            >
  ...
  setVersionListStatus = async (e, version, listType, enable) => {
    e.preventDefault();
    e.stopPropagation();
    const request =
      listType === 'white'
    ...
    return (
      ...
      >
        <Button
          ...
          style={{lineHeight: 1}}
        >
          {blackListWarning && <ExclamationCircleFilled style={{marginRight: 5}} />}
          <span>{label}</span>
  ...
  renderVersions = () => {
    ...
    const columns = [
      ...
      },
      {
        ...
        render: (modificationDate) =>
          modificationDate ? displayDate(modificationDate) : emptyField,
      },
      {
        ...
        title: 'CUDA',
        className: styles.osColumn,
        render: (cudaAvailable) => (cudaAvailable ? 'Available' : undefined),
      },
      {
        key: 'actions',
        ...
        render: (version) => {
          return (
            <Row type="flex" justify="end" className={styles.toolVersionActions}>
              ...
              {roleModel.executeAllowed(this.props.tool.value) &&
                this.renderRunButton(version.name)}
    ...
    const containsUnScannedVersion =
      this.props.preferences.toolScanningEnabledForRegistry(this.dockerRegistry) &&
      data.filter((item) => item.platform !== 'windows' && item.status === ScanStatuses.notScanned)
        .length > 0;
    if (
      this.props.preferences.toolScanningEnabledForRegistry(this.dockerRegistry) &&
    ...
    return (
      <Row style={{width: '100%'}}>
        {this.props.versions.error && (
          <Alert type="error" title={this.props.versions.error} style={{margin: '5px 0 10px 0'}} />
        )}
        <Input
        ...
        <Table
          ...
          showHeader
          rowClassName={(item) =>
            classNames(styles.versionTableRow, {
              [styles.whiteListedVersionRow]:
                item.fromWhiteList &&
                !/^windows$/i.test(item.platform) &&
                this.props.preferences.toolScanningEnabledForRegistry(this.dockerRegistry),
              'cp-tool-white-listed-version':
                item.fromWhiteList &&
                !/^windows$/i.test(item.platform) &&
                this.props.preferences.toolScanningEnabledForRegistry(this.dockerRegistry),
            })
          ...
          <Alert
            ...
            title={
              <div>
                ...
                <Row>
                  To edit instance configuration, default parameters and cluster configuration,
                  please create{' '}
                  <b>
                    <i>latest</i>
                  </b>{' '}
                  version or modify specific version settings.
                </Row>
              </div>
            }
          />
  ...
  renderToolHistory = () => {
    ...
    return <ToolHistory image={this.toolImage} />;
  };
  ...
  renderToolContent = () => {
    switch (this.props.section) {
      ...
      case 'settings':
        return this.renderToolSettings();
      case 'history':
        return this.renderToolHistory();
    }
    return undefined;
  };
      ...
      <Splitter style={{flex: 1, overflow: 'auto'}}>
        {panels.map(({key, info, content}) => (
          <Splitter.Panel
            key={key}
            defaultSize={
              info.size?.percentDefault != null
                ? `${info.size.percentDefault}%`
                : (info.size?.pxDefault ?? undefined)
            }
            min={
              info.size?.percentMinimum != null
                ? `${info.size.percentMinimum}%`
                : (info.size?.pxMinimum ?? 200)
            }
            resizable
          >
            <div
            ...
            >
              {(info.title || info.closable) && (
                ...
                >
                  ...
                  {info.closable && (
                    <CloseOutlined
                      onClick={() => onPanelClose && onPanelClose(key)}
                      style={{cursor: 'pointer'}}
                    />
                  )}
                </Row>
  ...
  runToolDefault = async (version) => {
    this.setState({launchPending: version}, async () => {
      ...
      const [versionSettings] = (this.props.versionSettings.value || []).filter(
        (v) => v.version === version,
      );
      const [defaultVersionSettings] = (this.props.versionSettings.value || []).filter(
        (v) => v.version === this.defaultTag,
      );
      ...
      const titleFn = (runName) => [
        <span key="launch">Are you sure you want to launch</span>,
        runName,
        <span key="question">with default settings?</span>,
      ];
  ...
  navigateBack = () => {
    ...
    this.props.router.push(
      `/tools/${this.props.tool.value.registryId}/${this.props.tool.value.toolGroupId}`,
    );
  };
  ...
  renderRunButton = (version) => {
    const parameterIsNotEmpty = (parameter, additionalCriteria) =>
      ...
      (!additionalCriteria || additionalCriteria(parameter));
    const [versionSettings] = (this.props.versionSettings.value || []).filter(
      (v) => v.version === (version || this.defaultTag),
    );
    const [defaultVersionSettings] = (this.props.versionSettings.value || []).filter(
      (v) => v.version === this.defaultTag,
    );
    ...
    if (!allowedToExecute) {
      ...
      const onSelect = ({key}) => {
        switch (key) {
          case runDefaultKey:
            this.runToolDefault(version);
            break;
          case runCustomKey:
            this.runTool(version);
            break;
        }
      };
      const runMenuItems = [
        ...
        <Space.Compact className={styles.runButton}>
          <Tooltip placement="left" title={tooltip} trigger="hover">
            <Button
              ...
              type="primary"
              size="small"
              disabled={!!this.state.launchPending || (!allowedToExecute && !this.isAdmin())}
  ...
  renderDisplayOptionsMenu = () => {
    const onSelectDisplayOption = ({key}) => {
      switch (key) {
        case 'metadata':
          this.toggleMetadataChange();
          break;
        case 'issues':
          this.toggleShowIssuesChange();
          break;
        case 'instanceTypeManagement':
          this.toggleInstanceTypesManagementPanelChange();
          break;
      }
    };
    ...
    return (
      ...
      >
        <Button id="display-attributes" size="small" style={{lineHeight: 1}}>
  ...
  renderActionsMenu = () => {
    ...
    const onClick = ({key}) => {
      switch (key) {
        case permissionsKey:
          this.openPermissionsForm();
          break;
        case deleteKey:
          this.deleteToolConfirm();
          break;
      }
    };
    const menuItems = [
  ...
  render() {
    ...
    const isPersonal = toolGroup && toolGroup.privateGroup && roleModel.isOwner(toolGroup);
    const isAdmin = this.isAdmin();
    const isAdvancedUser = this.isAdvancedUser();
    const restrictions =
      isPersonal && !isAdmin && !isAdvancedUser ? this.permissionsRestrictions : [];
      ...
      >
        <div className={classNames(styles.toolsHeader, 'cp-tool-header')}>
          <div className={styles.title} style={{flex: 1}}>
            <Button onClick={this.navigateBack} size="small" style={{lineHeight: 1}}>
              <ArrowLeftOutlined />
            </Button>
            <ToolLink link={this.link} style={{marginLeft: 5}} />
            <span style={{marginLeft: 5}}>{this.props.tool.value.image}</span>
            <PlatformIcon platform={this.defaultVersionPlatform} style={{marginLeft: 5}} />
            <Owner subject={this.props.tool.value} style={{marginLeft: 5}} />
          </div>
          <div style={{flex: 1}}>{this.renderMenu()}</div>
        ...
        >
          <PermissionsForm
            ...
            editOwnerAvailable={roleModel.isManager.toolAdmin(this)}
          />
        </Modal>
      </Card>
    );
  ...
  componentDidUpdate() {
    ...
    if (!this.defaultVersionSettings && this.defaultTag) {
      this.defaultVersionSettings = new LoadToolVersionSettings(
        this.props.tool.value.id,
        this.defaultTag,
      );
