...
import {Alert, Button, Checkbox, Input, Modal, Pagination, Table} from 'antd';
...
import styles from './SelectMetadataItems.module.css';
...
class SelectMetadataItems extends React.Component {
  ...
  renderItems = () => {
    ...
    const {items, pending, error, fields, fieldsPending, fieldsError, page, total} = this.state;
    ...
    const columns = [
      ...
      ...fields.map((field) => ({
        dataIndex: /^externalId$/i.test(field) ? field : `data.${field}`,
        key: field,
        fixed: /^externalId$/i.test(field) ? 'left' : undefined,
            ...
            <div>
              {multiple ? (
                <div style={{marginTop: 5}}>
                  ...
                  <Button size="small" onClick={selectPage}>
                    Select page
                  </Button>
                </div>
              ) : (
  ...
  fetchItems = (page) => {
    ...
    this.setState(
      ...
      () => {
        fetchMetadataItems(folderId, type, searchCriteria, page, PAGE_SIZE)
  ...
  render() {
    ...
    const {error, pending, type, fieldsError, fieldsPending, search, selection = []} = this.state;
    const disabled = fieldsPending || pending;
    return (
      <Modal
        open={visible}
        ...
        footer={
          <div className={styles.footer}>
            ...
            <Button type="primary" disabled={disabled} onClick={this.onOkClicked}>
              OK {selection.length > 0 ? `(${selection.length} entities)` : ''}
            </Button>
          </div>
        }
      >
        {(error || fieldsError) && <Alert type="error" title={error || fieldsError} />}
        {!error && !fieldsError && (
