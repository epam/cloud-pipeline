...
| `-t` / `--type` | Type of the Cloud for the object storage. Depends on the Cloud Provider. Possible values - `S3` (for AWS), `AZ` (for MS Azure), `GS` (for GCP) and `NFS` (for FS mounts).<br/>Additionally, for AWS HealthOmics Storages - `AWS_OMICS_REF` (for [reference store](../08_Manage_Data_Storage/8.14._Omics_storages.md#reference-store)) and `AWS_OMICS_SEQ` (for [sequence store](../08_Manage_Data_Storage/8.14._Omics_storages.md#sequence-store)). |
...
**Path** - defines a full path to the datastorage.  
**Path** value must begin from the Cloud prefix:

- `s3` (for AWS) / `az` (for MS Azure) / `gs` (for GCP) / common `cp` (instead of described ones, regardless of Provider)
- _OR_ `omics` (for [AWS HealthOmics Storages](../08_Manage_Data_Storage/8.14._Omics_storages.md))
...
| `-a` / `--additional-options` | _For `cp` command only_. Comma-separated list of additional arguments to be used during file copy |
...
| `-w` / `--timeout` | Maximum time to wait for the storage to be mounted, in ms. By default, `10000`. The command fails if the storage is not mounted within this time |
...
`pipe storage mount` finishes only when the storage is mounted. It checks the mount point every `500` ms - this delay can be changed by the **`CP_PIPE_FUSE_MOUNT_DELAY`** environment variable (in ms). If the mount point is a symlink, its target is checked. The command fails if the mount process exits, or if the storage is not mounted by `pipe` FUSE when the `-w` timeout expires (e.g. something else stays mounted at the mount point). In the last case, `pipe` stops the mount process. If **`CP_PIPE_FUSE_MOUNT_DELAY`** is not a positive integer, the default `500` ms is used.
