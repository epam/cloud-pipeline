...
import { isDataStorageItem, isNavigateBackItem } from '../../utils';

type Props = CommonProps & {
  item: UIStorageItem;
};

export const StorageItemActions = ({ className, style, item }: Props) => {
  const { onDownloadItem, onDeleteItem, onEditItem } = useStorageContext();

  const onEdit = useCallback(
    (e: ReactMouseEvent<HTMLElement, MouseEvent>) => {
      if (isDataStorageItem(item)) {
        e.stopPropagation();
        onEditItem(item);
      }
    },
