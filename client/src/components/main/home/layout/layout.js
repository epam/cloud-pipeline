...
import {DefaultPanelsState, PanelDefaultSizes, PanelNeighbors} from './model';
...
export function rebuildLayout (layout, rebuildHeights = true, composing = false, move = true) {
...
function findAvailablePlaces (oldLayout, key, box) {
  ...
  const size = PanelDefaultSizes[key] || {w: 1, h: 1};
...
function extendLayout (currentLayout, keys) {
  keys.sort((keyA, keyB) => {
    const sizeA = PanelDefaultSizes[keyA] || {w: 1, h: 1};
    const sizeB = PanelDefaultSizes[keyB] || {w: 1, h: 1};
  ...
  let maxWidth = MAX_LAYOUT_COLUMNS;
  let maxHeight = Infinity;
  for (let key in PanelDefaultSizes) {
    if (PanelDefaultSizes.hasOwnProperty(key)) {
      const size = PanelDefaultSizes[key];
      if (size.w > maxWidth) {
        maxWidth = size.w;
      }
      if (size.h > maxHeight) {
  ...
  if (!minLayout) {
    ...
    return rebuildLayout(minLayout || [], true, true);
  }
}

export function restoreDefaultLayout () {
  setPanelsLayout(DefaultPanelsState, false, false);
}

export function getPanelsLayout (rebuildIfNull = true) {
  const info = localStorage.getItem(PANELS_LAYOUT_STORAGE_KEY);
  if (rebuildIfNull && (!info || JSON.parse(info).length === 0)) {
    return setPanelsLayout(DefaultPanelsState, false, false);
  }
  if (info) {
...
export function setPanelsLayout (layout, rebuildHeights = true) {
  if (layout) {
    localStorage.setItem(
      PANELS_LAYOUT_STORAGE_KEY,
      JSON.stringify(rebuildLayout(layout, rebuildHeights))
    );
  }
  return layout;
}
...
export function removePanel (panel) {
  const panels = getPanelsLayout();
  const [item] = panels.filter(p => p.i === panel);
  if (item) {
    panels.splice(panels.indexOf(item), 1);
    setPanelsLayout(panels, false);
  }
}

export function addPanels (panels) {
  let currentLayout = getPanelsLayout(false);
  if (currentLayout.length > 1) {
    ...
    const size = PanelDefaultSizes[currentLayout[0].i] || {w: 1, h: 1};
    currentLayout[0].x = 0;
    currentLayout[0].y = 0;
    currentLayout[0].w = size.w;
    currentLayout[0].h = size.h;
  }
  setPanelsLayout(extendLayout(currentLayout, panels), false, false);
}
