...
class SearchDialog extends localization.LocalizedReactComponent {
  ...
  get searchTypesArray() {
    const result = [];
    for (const key in SearchGroupTypes) {
      if (Object.hasOwn(SearchGroupTypes, key)) {
  ...
  processAggregates = () => {
    ...
    if (this.props.searchEngine.loaded && this.props.searchEngine.value.aggregates) {
      ...
      for (const key in resultAggregates) {
        if (Object.hasOwn(resultAggregates, key)) {
          for (let i = 0; i < this.searchTypesArray.length; i++) {
            if (this.searchTypesArray[i].types.indexOf(key) >= 0) {
              aggregates[this.searchTypesArray[i].key] =
                (aggregates[this.searchTypesArray[i].key] || 0) + resultAggregates[key];
            }
          }
        }
      }
  ...
  getSearchQuery = (searchCriteria) => {
    ...
    return getSearchPrompt(searchCriteria, template, false, true);
  };
  ...
  performSearch = (force = false) => {
    ...
    this.setState(
      ...
      async () => {
        await this.props.searchEngine.send(query, undefined, PAGE_SIZE, this.generateSearchTypes());
  ...
  renderSearchResultItem = (resultItem, index) => {
    ...
    return (
      ...
      >
        <Row type="flex" align="middle" onMouseEnter={this.onHover(index)}>
          <div style={{display: 'inline-block'}}>{this.renderIcon(resultItem)}</div>
          <span className={styles.title}>{renderName()}</span>
        </Row>
      </div>
    );
  };
  ...
  renderPreview = () => {
    if (
      ...
      return <Preview item={this.state.searchResults[this.state.hoveredIndex]} />;
    }
    return null;
  };
    ...
    if (
      ...
      this.setState(
        ...
        async () => {
          ...
          const lastResult =
            searchResults.length > 0 ? searchResults[searchResults.length - 1] : undefined;
  ...
  navigateToAdvancedFilter = (e) => {
    ...
    const docTypes = (selectedGroupTypes || [])
      .map((group) => (Object.hasOwn(SearchGroupTypes, group) ? SearchGroupTypes[group].types : []))
      .reduce((r, c) => [...r, ...c], []);
    let queryString = facetedQueryString.build(searchString, {[DocumentTypeFilterName]: docTypes});
    if (queryString) {
  ...
  render() {
    ...
    return (
      ...
      >
        <div
          className={classNames(styles.searchBackground, {
            [styles.visible]: this.state.visible,
          })}
          style={this.state.visible ? getStyle({ie: {opacity: 0.75}}) : {}}
        ...
        >
          ...
          >
            <div className={classNames(styles.hintIconContainer, 'cp-search-type-button')}>
              <QuestionOutlined />
            </div>
          </Tooltip>
        </div>
        <div
          className={classNames(styles.typesForm, {
            [styles.resultsAvailable]: this.state.searchResults.length,
          })}
          onClick={this.closeDialog}
        >
          <div style={{display: 'flex', overflowX: 'auto'}}>
            {this.searchTypesArray.map((type, index) => {
              ...
              return (
                <div
                  className={classNames(styles.typeButton, 'cp-search-type-button', {
                    disabled,
                    selected,
                  })}
        ...
        >
          ...
          {!this.state.searching &&
            ...
            !this.state.searchResults.length && (
              <Row
                type="flex"
                className={classNames(styles.searchingInProgressContainer, 'cp-text-not-important')}
                align="middle"
                justify="center"
              >
  ...
  componentDidMount() {
    const {preferences} = this.props;
    preferences.fetchIfNeededOrWait().then(() => {
