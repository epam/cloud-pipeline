...
exports.findExistingTunnels = findExistingTunnels;
const ps_list_1 = __importDefault(require("ps-list"));
function parseTunnelArgs(cmdline) {
 ...
 */
async function findExistingTunnels(logger) {
            ...
            if (proc.name &&
                ...
                if (cmdlineArray.includes("tunnel") && cmdlineArray.includes("start")) {
                    ...
                    if (parsed && parsed.runId !== undefined && parsed.localPort) {
                        logger?.debug(`Found tunnel: pid=${proc.pid}, runId=${parsed.runId}, lp=${parsed.localPort}`);
                        tunnels.push({
                            pid: proc.pid,
