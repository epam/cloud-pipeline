...
export function canCommitRun(run) {
  // Checks only run state, not user permissions
  return (
    canStopRun(run) && runIsCommittable(run) && checkRunActionAvailable(run, runActions.commit)
  );
}
  ...
  return new Promise((resolve) => {
    dockerRegistries
      ...
      .then(() => {
        ...
        if (imageRegistry) {
          const [imageGroup] = (imageRegistry.groups || []).filter((g) => g.name === group);
          if (imageGroup) {
            const [image] = toolAndVersion.split(':');
            const [tool] = (imageGroup.tools || []).filter((i) => i.image === `${group}/${image}`);
...
export function canPauseRun(run, preferences) {
  // Checks only run state, not user permissions
  const {instance, pipelineRunParameters, pauseDisabled = false, podIP, initialized} = run;
  return (
    !pauseDisabled &&
    canStopRun(run) &&
    initialized &&
    ...
    diskSizeAllowsPause(preferences, instance ? instance.nodeDisk : 0) &&
...
export function stopRun(parent, callback) {
  if (!parent) {
    ...
    throw new Error(
      '"stopRun" function should be called with parent component passed to arguments:',
    );
  }
  const {localization, dockerRegistries, hiddenObjects, preferences, authenticatedUserInfo} =
    parent.props;
  return function (run) {
    return stopRunFn(run, callback, {
      preferences,
      localization,
      dockerRegistries,
      hiddenObjects,
...
export function terminateRun(parent, callback) {
  if (!parent) {
    console.warn(
      '"terminateRun" function should be called with parent component passed to arguments:',
    );
...
async function stopPipeline(run) {
  ...
  await request.send({
    endDate: dayjs.utc().format('YYYY-MM-DD HH:mm:ss.SSS'),
    status: 'STOPPED',
  });
  hide();
...
class TerminateRunDialog extends React.Component {
    ...
    this.setState(
      ...
      async () => {
        ...
        this.setState(
          ...
          () => {
            if (this.displayPromiseResolve) {
              this.displayPromiseResolve(!error);
            }
          },
        );
      },
  ...
  hide = () => {
    this.setState(
      ...
      () => {
        if (this.displayPromiseResolve) {
          this.displayPromiseResolve(true);
        }
      },
    );
  };
  ...
  render() {
    ...
    return (
      <Modal footer={null} closable={false} title={null} width="50%" open={this.state.visible}>
        <div>
          <Row style={{marginBottom: 10}} type="flex" align="middle">
            ...
            />
            <b style={{marginLeft: 10, fontSize: 14}}>Terminate {this.run.podId}?</b>
          </Row>
          ...
          <Row type="flex" justify="end" style={{marginTop: 10}}>
            <Button disabled={this.state.pending} onClick={this.hide}>
              CANCEL
            </Button>
            <Button
...
terminateRunDialogRoot.render(<TerminateRunDialog onInitialized={initializeTerminateRunDialog} />);

async function terminateRunFn(run, callback) {
...
class StopRunConfirmation extends React.Component {
  ...
  componentDidMount() {
    ...
    checkCommitAllowedForTool(dockerImage, dockerRegistries).then((allowed) =>
      this.setState({commitAllowed: allowed}),
    );
  }
  ...
  render() {
    ...
    return (
      <div>
        ...
        {this.props.canCommitRun && commitAllowed && !maintenanceMode && (
          <Row type="flex" style={{marginBottom: 5}}>
            <Checkbox checked={this.state.persistState} onChange={this.onChange}>
