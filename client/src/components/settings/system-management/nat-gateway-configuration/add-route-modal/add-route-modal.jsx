...
import {Checkbox, Modal, Input, Button, message, Popover, Spin, Select} from 'antd';
...
import {validateIP, validatePort, validateServerName, validateDescription} from '../helpers';
import styles from './add-route-modal.module.css';
...
export default class AddRouteForm extends React.Component {
  ...
  get formIsInvalid() {
    const {ports = {}, ip, serverName, errors = {}, useIP} = this.state;
    const flattenForm = {
      ip,
      serverName,
      ...ports,
    ...
    return (
      !!Object.values(errors)
        ...
        .filter((error) => error).length || Object.values(flattenForm).includes(undefined)
    );
  }
  ...
  validate = () => {
    const {ports = {}, serverName, ip, useIP, description} = this.state;
    const {routes = []} = this.props;
    const currentIpRoutes = routes.filter((route) =>
      useIP ? route.externalIp === ip : serverName === route.externalIp,
    );
    const getOtherPorts = (identifier) =>
      Object.keys(ports)
        .filter((o) => o !== identifier)
        .map((o) => (ports[o] || {}).value)
        .concat(
          currentIpRoutes.map((o) => o.externalPorts || []).reduce((r, c) => [...r, ...c], []),
        );
    const errors = {
  ...
  handleUseIP = (event) => {
    ...
    this.setState(
      ...
      () => {
        this.onAutoResolveIp().then(() => this.validate());
      },
    );
  };
  ...
  onSubmit = async () => {
    if (!this.formIsInvalid) {
      const {ports = {}, serverName, description, ip, useIP} = this.state;
      const portValues = [];
  ...
  onAutoResolveIp = () => {
    const {ipManualInput, ipResolveForServer, serverName, useIP} = this.state;
  ...
  renderFooter = () => (
    <div className={styles.footerButtonsContainer}>
      ...
      <Button type="primary" disabled={this.formIsInvalid} onClick={this.onSubmit}>
        ADD
      </Button>
    </div>
  );
  ...
  render() {
    ...
    const {serverName, ip, description, pending, useIP} = this.state;
    const FormItemError = ({identifier}) => (
      <div
        className={classNames(styles.formItemValidation, 'cp-error', {
    ...
    return (
      <Modal title="Add new route" open={visible} onCancel={onCancel} footer={this.renderFooter()}>
        <div>
          <Spin spinning={pending}>
            <div>
              <div className={styles.formItemContainer}>
                ...
                <Input
                  className={classNames(styles.formItem, this.getValidationClassName('serverName'))}
                ...
                <Hint style={{marginLeft: 5}}>
                  {}
                  Domain name is composed of a sequence of <b>lowercased</b> labels concatenated
                  with dots.
                  <br />
                  Each label must be from 1 to 63 characters long.
                  <br />
                  {}
                  The entire domain name, including the delimiting dots, has a maximum of 253
                  characters.
                  <br />
                  {}
                  Supported characters are <b>"a"</b>-<b>"z"</b> (<b>lowercase</b>), digits{' '}
                  <b>"0"</b>-<b>"9"</b>, hyphen (<b>"-"</b>) and underscore (<b>"_"</b>).
                  <br />
                  {}
                  Each label must starts with a character (<b>"a"-"z"</b>) and must not ends with a
                  hyphen (<b>"-"</b>)
                </Hint>
              </div>
              ...
              <div className={styles.formItemContainer}>
                ...
                <Checkbox className={styles.formItem} checked={useIP} onChange={this.handleUseIP}>
                  Specify IP address
                </Checkbox>
              </div>
              {useIP && (
                <div className={styles.formItemContainer}>
                  ...
                  <Input
                    ...
                    className={classNames(styles.formItem, this.getValidationClassName('ip'))}
                    value={ip}
                    onChange={this.handleChange('ip')}
                  />
                  <Button
                    disabled={!serverName || this.isError('serverName') || pending}
                    style={{marginLeft: 5}}
              ...
              {this.ports.map(([portIdentifier, port], index, ports) => (
                <div
                  ...
                  className={classNames(styles.portFormItemContainer, 'cp-nat-route-port-control')}
                >
                  <div className={styles.formItemContainer}>
              ...
              <div className={styles.addButtonContainer}>
                <Button icon={<PlusOutlined />} onClick={this.addPortInput}>
                  Add port
                </Button>
              </div>
              <div className={styles.formItemContainer}>
