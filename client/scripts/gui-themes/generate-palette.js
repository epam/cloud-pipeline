...
function parseColor (color) {
  ...
  if (color.startsWith('#')) {
    ...
    const values = color
      ...
      .split(',')
      .map(v => Number(v.trim()));
    if (values.length < 3 || values.length > 4 || values.some(v => Number.isNaN(v))) return undefined;
...
function main () {
  ...
  console.log(`Wrote ${path.relative(path.resolve(__dirname, '../..'), outputPath)} (${css.length} bytes)`);
}
