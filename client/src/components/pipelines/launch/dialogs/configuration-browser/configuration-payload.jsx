...
import {Alert, Checkbox, Collapse, InputNumber, Select} from 'antd';
...
import styles from './configuration-browser.module.css';
...
function validateTimeout(timeout) {
  ...
  if (Number.isNaN(Number(timeout)) || Number(timeout) < 0) {
...
class ConfigurationPayload extends React.Component {
  ...
  get pending() {
    const {pending, metadataPending} = this.state;
    return pending || metadataPending;
  }

  get error() {
    const {error, metadataError} = this.state;
  ...
  get currentEntityType() {
    const {classes = [], rootEntityId} = this.state;
    return classes.find((o) => o.id === Number(rootEntityId));
  }
      ...
      this.setState(
        ...
        async () => {
          ...
          try {
            ...
            const entries = ((request.value || {}).entries || []).filter(
              (entry) => !!entry.rootEntityId,
            );
            newState.name = name;
      ...
      this.setState(
        ...
        async () => {
          ...
          try {
            ...
            const folderMetadata = (metadataRequest.value || []).map((o) => o.data)[0];
  ...
  onPayloadChanged = () => {
    ...
    const {docker_image: dockerImage, is_spot: isSpot, cloudRegionId} = payload;
    this.setState(
      {
      ...
      async () => {
        ...
        try {
          ...
          if (dockerImage) {
            ...
            const [registryPath, groupName, imageWithVersion = ''] = (dockerImage || '').split('/');
            const imageNameRegExp = new RegExp(
              `^${groupName}/${imageWithVersion.split(':')[0]}$`,
              'i',
            );
            const registry = ((dockerRegistries.value || {}).registries || []).find(
              (o) => o.path === registryPath,
            );
            const group = ((registry || {}).groups || []).find((o) => o.name === groupName);
            tool = ((group || {}).tools || []).find((o) => imageNameRegExp.test(o.image));
            if (!tool) {
  ...
  reportChanged = () => {
    ...
    const entry = this.entry || {};
    const {payload, parametersValid, rootEntityId, configuration = {}} = this.state;
    const {
      parameters,
      modifiedParameters,
    ...
    try {
      valid =
        !!instanceType && parametersValid && validateDisk(instanceDisk) && validateTimeout(timeout);
    } catch (_) {
      valid = false;
    }
    const result = {
  ...
  renderPipeline = () => {
    ...
    return (
      <div className={styles.row}>
        ...
        {!pipelineId && <span className="cp-text-not-important">Pipeline is not specified</span>}
        {pipelineId && <span>{pipelineName}</span>}
      </div>
    );
  };
  ...
  renderDockerImage = () => {
    ...
    const {docker_image: dockerImage} = payload;
    return (
      <div className={styles.row}>
        <span className={styles.title}>Docker image:</span>
        {!dockerImage && (
          <span className="cp-text-not-important">Docker image is not specified</span>
        )}
        {dockerImage && <span>{dockerImage}</span>}
      </div>
    );
  };

  renderInstanceType = () => {
    const {payload, instanceTypes = [], instanceTypesPending, instanceTypesError} = this.state;
    const {disabled} = this.props;
    if (!payload) {
      return null;
    }
    if (instanceTypesError) {
      return <Alert title={instanceTypesError} type="error" />;
    }
    const {instance_size: instanceType} = payload;
    const onChange = (newInstanceType) =>
      this.setState(
        {
          payload: {
            ...payload,
    ...
    return (
      <div className={styles.row}>
        <span className={styles.title}>Instance type:</span>
        <Select
  ...
  renderCloudRegion = () => {
    ...
    const {cloudRegionId} = payload;
    const onChange = (newCloudRegionId) =>
      this.setState(
        {
          payload: {
            ...payload,
    ...
    return (
      <div className={styles.row}>
        <span className={styles.title}>Cloud region:</span>
        <Select
          className={classNames({
  ...
  renderDisk = () => {
    ...
    const {instance_disk: instanceDisk} = payload;
    const onChange = (disk) =>
      this.setState(
        {
          payload: {
    ...
    return (
      <div className={styles.row}>
        <span className={styles.title}>Disk (Gb):</span>
        <InputNumber
          disabled={disabled}
          onChange={onChange}
          style={{flex: 1, maxWidth: 300}}
  ...
  renderPriceType = () => {
    const {payload, priceTypes = [], instanceTypesPending} = this.state;
    if (!payload) {
      return null;
    }
    const {disabled} = this.props;
    const {is_spot: isSpot} = payload;
    ...
    return (
      <div className={styles.row}>
        <span className={styles.title}>Price type:</span>
        <Select
          value={`${isSpot}`}
          onChange={onChange}
          style={{flex: 1, maxWidth: 300}}
  ...
  renderCmdTemplate = () => {
    ...
    const {disabled} = this.props;
    const {cmd_template: cmdTemplate, startIdle} = payload;
    const onChange = (newCmdTemplate) =>
      this.setState(
        {
          payload: {
            ...payload,
    ...
    return (
      <div>
        <div className={styles.row}>
          <span className={styles.title}>Cmd template:</span>
          <Checkbox disabled={disabled} checked={startIdle} onChange={onChangeStartIdle}>
            Start idle
          </Checkbox>
        </div>
        {!startIdle && (
  ...
  renderRootEntity = () => {
    const {rootEntityId, classes = []} = this.state;
    const {disabled, rootEntityDisabled} = this.props;
    if (classes.length === 0) {
      return null;
    }
    const onChange = (newRootEntityId) =>
      this.setState(
        {
    ...
    return (
      <ParameterRow>
        <ParameterName>Root entity:</ParameterName>
        <ParameterValue>
          <Select
          ...
          >
            {classes.map((aType) => (
              <Select.Option key={`${aType.id}`} value={`${aType.id}`} title={aType.name}>
                {aType.name}
              </Select.Option>
            ))}
          </Select>
  ...
  renderCapabilities = () => {
    const {payload, tool} = this.state;
    const {disabled} = this.props;
    if (!payload) {
      return null;
    }
    const {docker_image: dockerImage, instance_size: instanceType} = payload;
    return (
      <div className={styles.row}>
        <span className={styles.title}>Run capabilities:</span>
  ...
  renderNotifications = () => {
    ...
    const onChange = (newNotifications) =>
      this.setState(
        {
          payload: {
            ...payload,
  ...
  render() {
    const entry = this.entry;
    if (!entry) {
      return <Alert title="Select configuration" type="info" />;
    }
    const {payload = {}, projectMetadata = {}} = this.state;
    const {className, style, disabled} = this.props;
    const {parameters = {}} = payload;
    const changeParameters = (modifiedParameters, valid) =>
      this.setState(
        {
          payload: {
            ...payload,
    ...
    return (
      <Parameters.Provider
        className={classNames(className, styles.configurationPayload)}
        style={style}
