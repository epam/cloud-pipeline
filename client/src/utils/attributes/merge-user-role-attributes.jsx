...
async function getUserRolesAttributeValues(attribute) {
  try {
    ...
    const {id, roles: userRoles = []} = whoAmI.value;
    const selfEntityOptions = [{entityId: id, entityClass: 'PIPELINE_USER'}];
    const rolesEntitiesOptions = userRoles.map((role) => ({
      entityId: role.id,
      entityClass: 'ROLE',
    }));
    const [[selfAttributes], rolesAttributes] = await Promise.all(
      [selfEntityOptions, rolesEntitiesOptions].map(safeFetchMetadata),
    );
    const extractAttribute = (attributesResponse) => {
      if (attributesResponse && attributesResponse.data && attributesResponse.data[attribute]) {
        return attributesResponse.data[attribute].value;
      }
      return undefined;
    };
    const user = extractAttribute(selfAttributes);
    const roles = rolesAttributes
      .map((roleAttributes) => {
...
export async function pickUserRoleAttributes(options) {
  ...
  const {attribute, picker = (a, b) => (isEmpty(a) ? b : a)} = options || {};
  const {user, roles = {}} = await getUserRolesAttributeValues(attribute);
  return [user, ...Object.values(roles)].reduce(picker, undefined);
}
