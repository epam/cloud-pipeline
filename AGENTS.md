...
Gradle build. The version is declared in the root `build.gradle`.
...
- **`api/src/main/resources/static/`** — a module copying an artifact here owes two more edits: the
  filename on the `exclude` list in `client/build.gradle`'s `buildUI`, which deletes what is not listed,
  and the path in `api.security.public.urls` — in every file `git grep -l api.security.public.urls`
  names, not just one.
...
`implement-task` skill** (`.agents/skills/implement-task/SKILL.md`). Open that file before the first
edit. Answering a question and reading code need none of it; reviewing someone else's diff may, to
check what the change owed.
