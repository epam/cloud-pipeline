...
import {Button, Row, Tag, Collapse} from 'antd';
import {CheckOutlined} from '@ant-design/icons';
import {observer} from 'mobx-react';
import styles from './DockerRegistryGroupsList.module.css';
...
class DockerRegistryGroupsList extends React.Component {
  ...
  groupsFilter = (group) => {
    ...
    if (mode === MODES.extended && group.privateGroup) {
      return true;
    }
  ...
  onSelectGroup = (id) => {
    if (this.props.onNavigate) {
      this.props.onNavigate(encodeURIComponent(id));
    }
  };
    ...
    return (
      ...
      >
        <div className={styles.card}>
          <span className={styles.heading}>{group.title}</span>
          {group.description ? (
            <span className={classNames(styles.comment, 'cp-text-not-important')}>
              {group.description}
            </span>
          ) : null}
        </div>
        {isActive ? <CheckOutlined className={styles.activeGroupIcon} /> : null}
      </Button>
    );
  };
  ...
  renderGroupsSelector = () => {
    ...
    const filteredGroups = groups.filter(this.groupsFilter).map((group) => {
      return (
    ...
    return <div className={styles.section}>{filteredGroups}</div>;
  };

  render() {
    const {mode, filters, groupSearch} = this.props;
    if (mode === MODES.groups) {
      return <div style={{overflowY: 'auto', flex: '1 1 auto'}}>{this.renderGroupsSelector()}</div>;
    }
    const {allGroupsExpanded} = this.state;
