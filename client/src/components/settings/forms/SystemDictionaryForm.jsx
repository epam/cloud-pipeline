...
import styles from './SystemDictionaryForm.module.css';
...
function mapValue(filter) {
  return function map(value) {
    const {autofill = true, value: linkValue, id, links = []} = value || {};
    return {
      autofill,
      value: linkValue,
      id,
...
class SystemDictionaryForm extends React.Component {
  ...
  onChangeLinks = (links) => {
    ...
    items[editableLinksIndex || 0].links = (links || []).map((link) => ({
      key: link.key,
      value: link.value,
    }));
    this.setState(
  ...
  render() {
    ...
    const {name, items, errors} = this.state;
    ...
    return (
      <div className={styles.container}>
        ...
        <div className={classNames(styles.name, styles.row)}>
          ...
          />
          <EditSystemDictionaryPermissions objectId={this.props.id} />
        </div>
        {nameError && <div className="cp-error">{nameError}</div>}
        <div className={styles.row} style={{marginTop: 10}}>
          <span className={styles.label}>Items:</span>
        </div>
        {itemsValidationError && <div className="cp-error">{itemsValidationError}</div>}
        <div ref={this.onItemsPanelInitialized} className={styles.items}>
          {items.map((item, index) => (
            <div
              key={index}
              className={classNames('cp-even-odd-element', styles.item, {
            ...
            >
              <div className={styles.row}>
                ...
                />
                <Button size="small" danger onClick={this.onItemRemove(index)}>
                  <DeleteOutlined />
                </Button>
              </div>
              {itemsError[index] && <div className="cp-error">{itemsError[index]}</div>}
        ...
        <div style={{padding: 2}}>
          <Button disabled={disabled} onClick={this.onItemAdd}>
            <PlusOutlined />
            <span>Add value</span>
          </Button>
        </div>
        ...
        <SystemDictionaryLinksForm
          ...
          value={linksFormVisible ? items[editableLinksIndex || 0].links || [] : []}
