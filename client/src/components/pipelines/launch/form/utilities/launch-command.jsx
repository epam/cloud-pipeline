...
import styles from './launch-command.module.css';
...
const DEFAULT_OS = getOS() === OperationSystems.windows ? OSFamily.windows : OSFamily.linux;

function wrapCommand(command, template) {
  if (!template) {
    return `# PIPE CLI command: \n${command || ''}`;
  }
  return template.replace(/\{LAUNCH_COMMAND\}/gi, command);
}
...
class LaunchCommand extends React.Component {
  ...
  rebuild = (payload) => {
    ...
    this.setState({pending: true}, async () => {
      ...
      const {params = {}, ...payloadRest} = payload || {};
      const pipelineStart = {
        ...payloadRest,
  ...
  renderCLICommand() {
    const {code, error, osType} = this.state;
    if (error) {
    ...
    return (
      <div>
        <div style={{display: 'flex', flexDirection: 'row', alignItems: 'center', marginBottom: 5}}>
          ...
          <Select value={osType} onChange={onChangeOS} style={{width: 150}}>
        ...
        <BashCode
          ...
          code={wrapNewLines(wrapCommand(code, this.launchCommandTemplate))}
        />
      </div>
    );
  }
  ...
  renderAPICommand() {
    ...
    if (!payload) {
      return <Alert type="warning" title="Payload is not set" />;
    }
    return (
  ...
  render() {
    ...
    return (
      <Modal onCancel={onClose} title="Launch commands" open={visible} width="50%" footer={false}>
        <Tabs
          items={[
