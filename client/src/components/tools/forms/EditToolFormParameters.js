...
function AddParameterButton ({vm, readOnly, isSystemParameters}) {
  if (isSystemParameters) {
    return (
      <Button
        onClick={vm.openSystemParameterBrowser}
        disabled={readOnly}
      >
  ...
  return (
    ...
    >
      ...
      >
        ...
        {
          nameError && (
            <span className={classNames(styles.error, 'cp-error')}>
              {nameError}
            </span>
          )
        }
...
function EditToolFormParameters (props) {
  ...
  const bucketBrowserParameter = bucketBrowserIndex !== null
    ? vm.parameters[bucketBrowserIndex]
    : null;
...
export default inject('runDefaultParameters')(
  roleModel.authenticationInfo(
    observer(EditToolFormParameters)
  )
);
