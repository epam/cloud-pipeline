...
function parseExpression(parts = []) {
  ...
  const validate = (parsedExpression) => {
    ...
    if (typeof parsedExpression[0].value !== 'string' && Array.isArray(parsedExpression[0].value)) {
  ...
  const wrap = (part) => {
    ...
    const {left, right, operator} = part;
    ...
    .map((object) => {
      ...
      return props
        .map((prop) => {
          ...
          return getObjectPropertyFunction(prop).map((stat) => ({
            object,
            property: prop,
            function: stat,
          }));
...
function OutputFormula(props) {
  const {formula, pipeline, onChange = () => {}, onRemove = () => {}} = props || {};
  if (!formula || !pipeline) {
    return null;
  }
  const {name, value, variables = {}} = formula;
  ...
  return (
    <div>
      <div className={styles.row}>
        <span className={styles.title}>Formula:</span>
        <Input
          className={classNames(styles.value, {'cp-error': !!error})}
          value={value}
          onChange={onChangeFormula}
        />
        <Button danger size="small" style={{marginLeft: 5}} onClick={onRemove}>
          <DeleteOutlined />
        </Button>
      </div>
      {sortedVariables.map((variable) => (
        <div key={variable} className={styles.row} style={{paddingRight: 33}}>
          ...
          >
            {properties.map((property) => (
              <Select.Option
                ...
                statistics={
                  property.function ? PropertyFunctionNames[property.function] : undefined
                }
                title={`${property.object}: ${ObjectPropertyName[property.property]}`.concat(
                  property.function ? ` - ${PropertyFunctionNames[property.function]}` : '',
                )}
              >
      ...
      <div className={styles.row} style={{paddingRight: 33}}>
        <span className={styles.title}>Name:</span>
        <Input className={styles.value} value={name} onChange={onChangeFormulaName} />
      </div>
    </div>
  );
}
