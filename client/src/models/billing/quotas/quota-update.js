...
class QuotaUpdate extends RemotePost {
  async send (body) {
    if (!this._postIsExecuting) {
      ...
      try {
        ...
        const [existingQuota] = quotas
          .filter(q => body.target ? `${q.target}` === `${body.target}` : !q.target);
        if (existingQuota) {
          const index = quotas.indexOf(existingQuota);
          quotas.splice(index, 1, body);
        } else {
          quotas.push(body);
        }
