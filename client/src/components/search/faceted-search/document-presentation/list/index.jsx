...
export default function DocumentListPresentation({className, document, extraColumns}) {
  ...
  switch (type) {
    case SearchItemTypes.run:
      extra.push(
        <DatePresentation key="started" document={document} label="started" field="startDate" />,
      );
      extra.push(
        <DatePresentation key="finished" document={document} label="finished" field="endDate" />,
      );
      break;
    default:
      break;
    ...
    >
      {extraColumns &&
        ...
        (extraColumns || []).map((column) => (
          <KeyValue key={column.key} field={column.key} name={column.name} document={document} />
        ))}
    </GeneralPresentation>
  );
}
