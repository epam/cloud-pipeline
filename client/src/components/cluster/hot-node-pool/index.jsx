...
import styles from './hot-node-pool.module.css';
...
class HotCluster extends React.Component {
  ...
  onSaveEditablePool = async (pool, schedule) => {
    ...
    try {
      ...
      const currentScheduleId = currentSchedule && !createNewPool ? currentSchedule.id : undefined;
      let scheduleId = currentScheduleId;
      const hasSchedule =
        schedule && schedule.scheduleEntries && schedule.scheduleEntries.length > 0;
      if (hasSchedule) {
  ...
  render() {
    const {pools: poolsRequest, clusterNodes: nodes, routing} = this.props;
    if ((poolsRequest.pending && !poolsRequest.loaded) || (nodes.pending && !nodes.loaded)) {
      return <LoadingView />;
    }
    if (poolsRequest.error || !poolsRequest.loaded) {
      return <Alert type="error" title={poolsRequest.error || 'Error fetching hot node pools'} />;
    }
    if (nodes.error || !nodes.loaded) {
      return <Alert type="error" title={nodes.error || 'Error fetching cluster nodes'} />;
    }
    const {value: pools = []} = poolsRequest;
    const {createNewPool, editablePool, pending} = this.state;
    const readOnly = this.isReadOnly;
    return (
      <div className="cp-panel cp-panel-transparent">
        <div className={styles.header}>
          ...
          <div className={styles.actions}>
            ...
            <Button disabled={pending} onClick={this.refresh}>
              Refresh
            </Button>
          </div>
        </div>
