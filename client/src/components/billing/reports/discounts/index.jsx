...
class DiscountsStore {
  ...
  setCompute(value) {
    ...
    if (this.compute !== value) {
      ...
      this.setComputeDebounce = setTimeout(() => this.apply(value, this.storage), 100);
    }
  }
  ...
  setStorage(value) {
    ...
    if (this.storage !== value) {
      ...
      this.setStorageDebounce = setTimeout(() => this.apply(this.compute, value), 100);
    }
  }
}
...
function SingleDiscountsSliderComponent({value, onChange, title}) {
  return (
    <div className={styles.subContainer}>
      ...
      <Slider min={-100} max={100} onChange={onChange} value={value} step={1} style={{flex: 1}} />
      <InputNumber
...
class DiscountsModalComponent extends React.Component {
  render() {
    ...
    return (
      <Modal onCancel={onClose} open={visible} footer={false} title="Configure discounts">
...
class Discounts extends React.Component {
  ...
  render() {
    ...
    return <MobxProvider discounts={this.store}>{children}</MobxProvider>;
  }
}
