...
const Badged = ({enabled = false, children}) => {
  if (enabled) {
    return <Badge dot>{children}</Badge>;
  }
  return children;
};
...
class VSHistory extends React.Component {
  ...
  get path() {
    const {path: rawPath, isFolder} = this.props;
    let path = rawPath;
  ...
  fetchCommits = () => {
    const {versionedStorageId} = this.props;
    ...
    this.setState(
      ...
      () => {
        ...
        const request = new LoadVSCommits(versionedStorageId, page, pageSize);
        ...
        request
          ...
          .then(() => {
            if (request.loaded) {
              const {has_next: hasMorePages = false, listing: commits = []} = request.value;
  ...
  fetchLastCommit = () => {
    ...
    this.setState(
      ...
      () => {
        const request = new LoadVSCommits(versionedStorageId, 0, 1);
        const filtersPayload = {
          path: this.path,
        };
        request
          ...
          .then(() => {
            if (request.loaded) {
              const {listing: commits = []} = request.value;
  ...
  render() {
    const {className, style, versionedStorageId, isFolder, onRefresh, readOnly} = this.props;
    if (!versionedStorageId) {
      return null;
    }
    const {commits, pending, error, filtersVisible, filters, lastCommitHash} = this.state;
    return (
      <div className={classNames(styles.container, className)} style={style}>
        ...
        >
          <div className={styles.title}>Revision history</div>
          <Badged enabled={this.filtersEnabled}>
            <Button
