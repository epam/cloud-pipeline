  ...
  onTransfer = async () => {
    if (this.props.onTransfer) {
      await this.props.onTransfer({
        ...
        threadsCount: this.state.threadsCount ? this.state.threadsCount : undefined,
      });
    }
  };
  ...
  render() {
    return (
      <Modal
        ...
        footer={
          <Row type="flex" align="middle" justify="end">
            <Button onClick={this.props.onClose} style={{marginRight: 5}}>
              Cancel
            </Button>
            <Button
      ...
      >
          ...
          <div style={{flex: 1}}>
            ...
            >
              {this.props.pathFields.map((f) => {
                return <Select.Option key={f}>{f}</Select.Option>;
              })}
            </Select>
          </div>
        </Row>
        <Row type="flex" align="middle" style={{marginTop: 5}}>
          <span style={{fontWeight: 'bold', marginRight: 10}}>Select name field:</span>
          <div style={{flex: 1}}>
            <Select
              allowClear
              style={{width: '100%'}}
        ...
        <Row type="flex" align="middle" style={{marginTop: 5}}>
          <span style={{fontWeight: 'bold', marginRight: 5}}>Max threads count:</span>
          <div style={{flex: 1}}>
            <InputNumber
        ...
        <Row type="flex" align="middle" style={{marginTop: 5}}>
          <Checkbox checked={this.state.createFolders} onChange={this.onCreateFolderChanged}>
            Create folders for each <i>path</i> field
          </Checkbox>
        </Row>
        <Row type="flex" align="middle" style={{marginTop: 5}}>
          <Checkbox checked={this.state.updatePathValues} onChange={this.onUpdatePathValuesChanged}>
            Update path values
          </Checkbox>
        </Row>
        <BucketBrowser
