...
export default {
  [Panels.runs]: (localizedStringFn) => `List of the jobs/tools that are currently in RUNNING or PAUSED state. If this list is empty - start a new run from the TOOLS or ${localizedStringFn('Pipeline')}s widgets. Hover a run item to view a list of available action. Use STOP/PAUSE/RESUME actions to change the state of the run or use OPEN to navigate to the GUI of the interactive job. Click a run item to navigate to the details and logs page. Note: only top 20 active runs will be shown, if more than 50 jobs/tools are running - use EXPLORE ALL ACTIVE RUNS link`,
