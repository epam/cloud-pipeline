...
from src.utilities.bucket_operations_manager import S3BucketOperations
...
class DataStorageOperations(object):
    ...
    def cp(cls, source, destination, recursive, force, exclude, include, quiet, tags, file_list, clean=False,
        ...
        try:
            ...
            # copying a file to s3 bucket, we need to set folder/file flag correctly
            ...
            manager = S3BucketOperations.get_operation_manager(source_wrapper, destination_wrapper, command)
            ...
            for item in items:
                ...
                if not include and not exclude:
                    if source_wrapper.is_file() and not source_wrapper.path == full_path:
                        continue
                    if not source_wrapper.is_file():
                        possible_folder_name = source_wrapper.path \
                            if source_wrapper.path.endswith(S3BucketOperations.S3_PATH_SEPARATOR) \
                            else source_wrapper.path + S3BucketOperations.S3_PATH_SEPARATOR
        ...
        except (ConfigNotFoundError, requests.exceptions.RequestException, RuntimeError, ValueError,
                S3UploadFailedError) as error:
    ...
    def storage_remove_item(cls, path, yes, version, hard_delete, recursive, exclude, include):
        ...
        try:
            ...
            manager = S3BucketOperations.get_delete_manager(source_wrapper, versioning=version or hard_delete)
    ...
    def save_data_storage(cls, name, description, sts_duration, lts_duration, versioning,
        ...
        try:
            DataStorage.save(name, path, description, sts_duration, lts_duration, versioning, backup_duration, type,
                             directory.id if directory else None, on_cloud)
        except (ConfigNotFoundError, requests.exceptions.RequestException, RuntimeError, ValueError) as error:
            click.echo('Error: %s' % str(error), err=True)
            sys.exit(1)
    ...
    def restore(cls, path, version):
        try:
            ...
            if not source_wrapper.bucket.policy.versioning_enabled:
                click.echo('Versioning is not enabled for storage "{}"'.format(source_wrapper.bucket), err=True)
                sys.exit(1)

            manager = S3BucketOperations.get_restore_manager(source_wrapper)
    ...
    def __print_data_storage_contents(cls, bucket_model, relative_path,
        ...
        if bucket_model is not None:
            try:
                wrapper = S3BucketWrapper(bucket_model, relative_path)
                manager = S3BucketOperations.get_list_manager(wrapper, show_versions=show_versions)
        ...
        if show_details:
            ...
            for item in items:
                ...
                size = ''
                labels = ''
                if item.type is not None and item.type.lower() == 's3':
                    name = item.path
