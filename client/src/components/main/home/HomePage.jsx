...
export default class HomePage extends React.Component {
  ...
  render() {
    ...
    return (
      <div ref={this.initializeContainer} className={styles.globalContainer}>
        <Row
  ...
  refreshActiveRuns = async () => {
    ...
    for (const key in this.panels) {
      if (Object.hasOwn(this.panels, key) && this.panels[key] && this.panels[key].contentPanel) {
        update = update || this.panels[key].contentPanel.usesActiveRuns;
      }
    }
    if (update) {
  ...
  refreshCompletedRuns = async () => {
    ...
    for (const key in this.panels) {
      if (Object.hasOwn(this.panels, key) && this.panels[key] && this.panels[key].contentPanel) {
        update = update || this.panels[key].contentPanel.usesCompletedRuns;
      }
    }
    if (update) {
  ...
  forceUpdatePanels = () => {
    for (const key in this.panels) {
      if (Object.hasOwn(this.panels, key) && this.panels[key] && this.panels[key].update) {
  ...
  componentDidMount() {
    const initializeContinuousFetch = (id, call) => {
      const {stop, fetch: fetchData} = continuousFetch({
        continuous: true,
    ...
    initializeContinuousFetch(ContinuousFetchIdentifiers.services, this.refreshServices.bind(this));
    initializeContinuousFetch(ContinuousFetchIdentifiers.issues, this.refreshIssues.bind(this));
