...
class WdlRuntimeNode extends React.Component {
  ...
  get instanceTypes() {
    const {allowedInstanceTypes} = this.props;
  ...
  render() {
    const {className, style, value, onChange, disabled} = this.props;
    const filtered = this.instanceTypes.filter((o) => o.name.includes(this.valueWithoutQuotes));
    ...
    const options = [].concat(defaultValueOptions).concat(
      getSelectOptions(filtered, {
