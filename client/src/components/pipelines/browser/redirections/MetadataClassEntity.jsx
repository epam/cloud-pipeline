...
class MetadataClassEntity extends React.PureComponent {
  ...
  componentDidUpdate(prevProps, prevState, snapshot) {
    ...
    const {folder: propFolder, entity: propEntity} = routeParams;
    const {folder, entity} = this.state;
  ...
  navigate = () => {
    const {routeParams = {}, router} = this.props || {};
    const {folder, entity} = routeParams;
    let error;
    if (!folder) {
          ...
          metadataRequest.fetch().then(() => {
            if (folder === this.state.folder && entity === this.state.entity) {
              if (metadataRequest.error || !metadataRequest.loaded) {
                ...
                const {classEntity} = result;
                if (!classEntity) {
                  router.push(`/folder/${folder}/metadata`);
                } else {
                  const {name} = classEntity;
                  if (!name) {
                    router.push(`/folder/${folder}/metadata`);
                  } else {
