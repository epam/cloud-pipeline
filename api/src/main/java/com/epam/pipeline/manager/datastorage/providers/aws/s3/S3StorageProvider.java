...
public class S3StorageProvider implements StorageProvider<S3bucketDataStorage> {
    ...
    public String createStorage(S3bucketDataStorage storage) {
        try {
            ...
            AwsRegionsConfiguration configuration = preferenceManager.getObjectPreferenceAs(
                    SystemPreferences.CLUSTER_NETWORKS_CONFIG, new TypeReference<AwsRegionsConfiguration>() {});
    ...
    public void applyStoragePolicy(S3bucketDataStorage dataStorage) {
        getS3Helper(dataStorage).applyStoragePolicy(dataStorage.getPath(), dataStorage.getStoragePolicy());
    }
    ...
    public S3Helper getS3Helper(S3bucketDataStorage dataStorage) {
        ...
        return new RegionAwareS3Helper(region.getAwsRegionName());
    }

    private AwsRegion getAwsRegion(S3bucketDataStorage dataStorage) {
        return awsRegionManager.getAwsRegion(dataStorage);
    }
}
