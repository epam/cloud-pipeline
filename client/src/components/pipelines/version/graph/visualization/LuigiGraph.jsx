...
export default class LuigiGraph extends Graph {
  ...
  onSelectHandler(event) {
    const data = event.type === 'select' ? event.target.data() : null;
  ...
  buildGraphData = () => {
    if (!this.props.graph.pending && !this.props.graph.error) {
      ...
      const createNode = ({taskName, toolName, id}, isMainTask, additionalInfo) => {
        ...
        return {data: {id: `${id}`, name, main: `${isMainTask}`, ...additionalInfo}};
      };
      ...
      for (const task of tasks) {
        ...
        const nodeData = {taskName: task.task.name, toolName, id: task.task.id};
