...
import {Alert, Input, Row, Select} from 'antd';
...
export default class Packages extends React.Component {
  ...
  filterDependenciesFn = (dependency) => {
    if (this.state.filterDependencies) {
      return (
        (dependency.name || '')
          .toLowerCase()
          .indexOf(this.state.filterDependencies.toLowerCase()) >= 0
      );
    }
    return true;
  };
  ...
  get dependencies() {
    if (this.props.versions.loaded && this.state.selectedEcosystem && this.ecosystems.length > 0) {
      const result = (this.props.versions.value.scanResult.dependencies || []).filter(
  ...
  get filteredDependencies() {
    if (
      ...
      const result = (this.props.versions.value.scanResult.dependencies || []).filter(
        (d) =>
          d.ecosystem !== this.state.selectedEcosystem &&
    ...
    (d, index) => {
      return (
        <li key={index} className={styles.dependency}>
          <b>
            {highlightText(d.name, this.state.filterDependencies)} v{d.version}
          </b>
          {includePackage ? <span>({d.ecosystem})</span> : undefined}
          {d.description ? (
