    ...
    if (run) {
      ...
      if (dockerImage) {
        ...
        const {
          tool,
          version: diVersion
        } = getDockerImage(dockerImage, dockerRegistriesTree) ?? {};
        if (tool) {
...
export function pluginAssignmentsAreEqual (plugin1, plugin2) {
  ...
  return pluginData1.id === pluginData2.id &&
...
export async function updatePluginsAssignments (assignments, initialAssignments = []) {
  const toRemove = initialAssignments
    .filter((i) => !assignments.some((pl) => pl.id === i.id));
