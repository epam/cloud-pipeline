...
function UserManagementForm({authenticatedUserInfo, router, location}) {
  ...
  const groupsHasSharedPermissions = (rolesList || []).some((r) => roleModel.readAllowed(r));
  if (
    !isReader &&
    !isAdmin &&
    !isUserAdmin &&
  ...
  const sections = [
    ...
    isReader || isAdmin || isUserAdmin
      ? {
          key: 'roles',
          title: 'Roles',
          render: () => <GroupsManagement predefined />,
        }
      : false,
    isReader || isAdmin || isUserAdmin
      ? {
          key: 'report',
          title: 'Usage report',
          render: () => <UsageReport router={router} location={location} />,
        }
      : false,
  ].filter(Boolean);
  return (
    <Provider roles={roles} usersWithActivity={usersWithActivity} quotas={quotas}>
      <SubSettings sections={sections} router={router} root="user" hideListForSingleSection />
    </Provider>
  );
}
