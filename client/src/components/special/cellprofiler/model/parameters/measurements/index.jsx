...
function isUnitsMeasurement(measurement) {
  const {group, measurement: measurementName} = parseMeasurement(measurement);
  switch (group) {
    case Groups.shape:
...
function isIntensityMeasurement(measurement) {
  const {group, measurement: name} = parseMeasurement(measurement);
  switch (group) {
  ...
  if (isSquareUnits && physicalSize) {
    if (DEBUG) {
      console.log(
        ...
        value,
        'px2 to',
        physicalSize.getSquarePhysicalSize(value),
        'units2',
      );
    }
...
function buildMeasurements(cpModule, measurements = []) {
  ...
  return measurements.map((obj) => {
    const wrapNumber = (o) =>
      o !== undefined && o !== '' && !Number.isNaN(Number(o)) ? Number(o) : undefined;
    const {
      measurement,
      use_min_filter: useMin,
      use_max_filter: useMax,
...
function MeasurementSelector(props) {
  ...
  return (
    ...
    >
      {groups.map((aGroup) => (
        <Select.OptGroup key={aGroup} label={GroupNames[aGroup] || aGroup}>
          {MeasurementsList.filter((measurement) => measurement.group === aGroup).map(
            (measurement) => (
              <Select.Option
