...
function Addon({children}) {
  ...
  const child = React.cloneElement(children, {
    className: classNames(props.className, styles.addon),
  });
  return (
    <span
      className={classNames(styles.parameterAddon, 'cp-input-group-addon')}
      style={{
        display: 'inline-flex',
      }}
    >
...
class AutoCompleteInput extends React.Component {
  get options() {
    ...
    const sets = Object.entries(autoCompleteOptions).map(([group, options]) => ({
      group,
      options,
    }));
  ...
  render() {
    const {className, style, value, disabled, size, error, addonBefore, addonAfter} = this.props;
    return (
      <Input.Group
        compact
        size={size}
