...
**A `.less` edit does nothing until the generated files are regenerated.**
`npm run gui-themes-prepare` writes `src/themes/utilities/theme.less.template.js` and each theme's
`index.js`, and both are committed. Neither `npm start` nor `npm run build` regenerates them — so a
themed change means editing the LESS, running that command, and committing what it changes.
...
names the rest, including the `verify-client-live` skill.
...
- **`:client:buildUI` builds with `PUBLIC_URL=/pipeline`.** Asset paths go through the bundler; never
  write one absolute. `npm start` serves at `/`, so an absolute path works in dev and 404s behind the
  API.
