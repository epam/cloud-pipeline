...
export default class CudaWarning extends React.Component {
  ...
  render() {
    const {versionInfo} = this.state;
    const {className, showIcon, style, gpuEnabledRun, instanceType} = this.props;
    ...
    const getMessage = () => {
      if (gpuEnabledRun && !this.cudaAvailable) {
        return (
          <p>
            You are going to start a tool <i>without a CUDA toolkit</i>, using a{' '}
            <i>GPU-enabled node ({instanceType})</i>. No CUDA environment will be available, but the
      ...
      if (!gpuEnabledRun && this.cudaAvailable) {
        return (
          <p>
            You are going to start a tool <i>with a CUDA toolkit installed</i>, but using a{' '}
            <i>CPU-only node ({instanceType})</i>. No CUDA/Nvidia environment will be available.
          </p>
        );
      }
      return '';
