...
function diskSizeAllowsPause(preferences, disk) {
  return getDiskSizeThresholdConfigurationRestrictions(preferences, disk).pause;
}
...
function formatMessage(configuration, disk) {
  ...
  result = result.replace(/\{(disk|size)}/gi, disk);
  result = result.replace(/\{(threshold|max)}/gi, configuration.threshold);
  return result;
}

const emptyStore = {loaded: false};

function DiskSizeWarning({className, style, disk, preferences, showIcon, type}) {
  ...
  return (
    ...
    >
      <Alert
        title={<Markdown className="no-margin-markdown" md={formatMessage(match, disk)} />}
