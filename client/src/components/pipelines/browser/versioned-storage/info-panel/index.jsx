...
import {Alert, Row, Button, Spin, Input, message, Splitter} from 'antd';
...
class InfoPanel extends localization.LocalizedReactComponent {
  ...
  getFileContent = () => {
    const {pipelineId, lastCommitId, file} = this.props;
    if (!pipelineId || !file || !file.path || file.size > MAX_SIZE_TO_PREVIEW || !lastCommitId) {
      this.setState({
        fileIsFetching: false,
        fileFetchingError: undefined,
    ...
    this.setState(
      ...
      () => {
        parsePipelineFile(pipelineId, lastCommitId, file.path, MAX_SIZE_TO_PREVIEW).then((info) => {
          const {content, error, binary, tabular} = info;
          this.setState({
            fileContent: content,
            binaryFile: binary,
            tabularFile: tabular,
  ...
  renderDownloadLink = (description) => {
    ...
    return (
      <span>
        <span className={styles.downloadBtn} onClick={handleDownload}>
          Download file
        </span>
        {description}
      </span>
  ...
  renderPreviewHeader = () => {
    ...
    const {fileFetchingError, fileIsFetching, binaryFile} = this.state;
    ...
    if (fileIsFetching) {
      ...
      content = (
        <Row type="flex" style={{color: '#777', marginTop: 5, marginBottom: 5}}>
          <Alert
            type="error"
            title={
              <Row>
                <span style={{marginRight: 5}}>{fileFetchingError}</span>
      ...
      content = (
        <Row type="flex" style={{color: '#777', marginTop: 5, marginBottom: 5}}>
          <span style={{marginRight: 5}}>File preview is not available.</span>
          {this.renderDownloadLink('to view full contents')}
        </Row>
      );
    } else {
      content = (
        <Row type="flex" justify="space-between" align="middle" className={styles.previewHeaderRow}>
          <b>File preview</b>
          <Button
            size="small"
  ...
  renderFilePreview = () => {
    const {file} = this.props;
    const {binaryFile, tabularFile, fileContent, fileFetchingError, fileIsFetching} = this.state;
    if (!file || binaryFile || fileFetchingError || file.size > MAX_SIZE_TO_PREVIEW) {
      return null;
    }
    if (tabularFile && tabularFile.length > 0) {
      const columnsLength = tabularFile.reduce((length, row) => Math.max(length, row.length), 0);
      return (
            ...
            <thead>
              <tr>
                ...
                {tabularFile[0].map((cell, index) => (
                  <th key={`column-${index}`}>{cell}</th>
                ))}
                {fillEmptyCells(columnsLength - tabularFile[0].length, (index) => (
                  <th key={`column-${index + tabularFile[0].length}`}>{'\u00A0'}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {tabularFile.slice(1).map((columns, row) => (
                <tr key={`row-${row}`}>
                  <th key={`row-${row}`}>{row + 1}</th>
                  {columns.map((cell, column) => (
                    <td key={`cell-${row}-${column}`}>{cell}</td>
                  ))}
                  {fillEmptyCells(columnsLength - columns.length, (index) => (
                    <td key={`cell-${row}-${index + columns.length}`}>{'\u00A0'}</td>
                  ))}
                </tr>
              ))}
            </tbody>
      ...
      return (
        <Input.TextArea
          ...
          style={
            fileContent ? {height: '100%'} : {color: '#aaa', height: '100%', fontStyle: 'italic'}
          }
          readOnly
        />
      );
  ...
  saveFile = async (contents, comment) => {
    const {pipelineId, onRefresh, file, lastCommitId} = this.props;
    if (!file) {
      return;
    }
  ...
  render() {
    const {lastCommitId, pipelineId, path, file, readOnly, onRefresh} = this.props;
    const {filePreviewVisible} = this.state;
    const historyPanel = (
      <VSHistory
    ...
    return (
      <>
        <Splitter style={{overflow: 'auto'}} orientation="vertical">
          <Splitter.Panel
            defaultSize="33%"
            min={100}
