...
import styles from './limit-mounts.module.css';
...
class LimitMountsUserPreference extends React.Component {
    ...
    return (
      <div className={styles.limitMountsRow}>
        {this.doNotMountStorages ? (
          <span>Do not mount storages</span>
        ) : (
          <LimitMountsInput
            showOnlySummary
            disabled
  ...
  render() {
    const {readOnly, metadata = {}, showOnlySummary} = this.props;
    const {value = undefined} = metadata;
    if (showOnlySummary) {
    ...
    return (
      <div>
        <div className={classNames(styles.limitMountsRow, styles.header, 'cp-text')}>
          Limit mounts:
        </div>
        <div className={styles.limitMountsRow}>
          <Checkbox
        ...
        {!this.doNotMountStorages && (
          <div className={classNames(styles.limitMountsRow)}>
            <LimitMountsInput
              value={value}
              allowSensitive={false}
              disabled={readOnly}
