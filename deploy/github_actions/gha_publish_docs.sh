...
if [ "$GITHUB_REPOSITORY" != "epam/cloud-pipeline" ] || \
    [ "$GITHUB_REF_NAME" != "develop" ] && \
    [[ "$GITHUB_REF_NAME" != "release/"* ]] && \
    [[ "$GITHUB_REF_NAME" != "stage/"* ]]; then
...
echo "Building docs"
./gradlew -PbuildNumber=${CLOUD_PIPELINE_BUILD_NUMBER}.${GITHUB_SHA} -Pprofile=release buildDoc --no-daemon
...
cp -Rf $HOME/cloud-pipeline-docs/* ./$GITHUB_REF_NAME
...
git add -f .
git commit -m "Docs update via Appveyor Build $CLOUD_PIPELINE_BUILD_NUMBER"
