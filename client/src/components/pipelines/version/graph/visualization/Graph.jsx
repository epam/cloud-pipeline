...
export default class Graph extends React.Component {
  ...
  toggleFullScreen = () => {
    this.setState({fullScreen: !this.state.fullScreen}, () => this.onFullScreenChanged());
  };
  ...
  saveChanges = async (opts) => {
    ...
    if (
      ...
      const [configuration] = this.props.configurations.value.filter(
        (c) => c.name.toLowerCase() === opts.configName.toLowerCase(),
      );
      if (configuration) {
  ...
  renderBottomGraphControls = () => {
    ...
    return (
      <Row type="flex" justify="end" className={classNames(styles.graphInterface, 'cp-primary')}>
        ...
        <Col className={styles.graphInterfaceButton}>
          {this.state.fullScreen ? (
            <ShrinkOutlined onClick={this.toggleFullScreen} className={getIconClassName(true)} />
          ) : (
            <ArrowsAltOutlined onClick={this.toggleFullScreen} className={getIconClassName(true)} />
          )}
        </Col>
      </Row>
    );
  ...
  render() {
    let containerStyle = classNames('cp-content-panel', styles.graphContainer, {
