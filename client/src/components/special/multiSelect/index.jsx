...
class MultiSelect extends React.Component {
  ...
  onSelectItem = (e, key) => {
    ...
    const [lastInactiveKey] =
      options.length - values.length === 1 ? options.filter((o) => !values.includes(o)) : [];
  ...
  render() {
    ...
    return (
      ...
      >
        <div className={classNames(styles.input, {[styles.disabled]: disabled || pending})}>
          {pending ? (
            <Spin
              style={{
                width: '100%',
          ...
          <DownOutlined
            className={classNames(styles.inputArrow, {[styles.expanded]: dropdownVisible})}
            style={{fontSize: '10px'}}
          />
        </div>
      </Dropdown>
