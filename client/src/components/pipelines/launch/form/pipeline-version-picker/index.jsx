...
import styles from './pipeline-version-picker.module.css';
...
function filterPipelineVersion(version, filter, exact = false) {
  ...
  if (typeof version === 'object') {
    const {name, commitId} = version;
    if (exact) {
      return name.toLowerCase() === filter.trim().toLowerCase();
    }
    return (
      name.toLowerCase().includes(filter.trim().toLowerCase()) ||
      commitId.toLowerCase().includes(filter.trim().toLowerCase())
    );
  }
  return false;
}
...
class PipelineVersionPicker extends React.PureComponent {
  ...
  onSelectVersion = (version) => {
    const {pipelineVersion, onPipelineVersionChange} = this.props;
    if (
      version &&
      ...
      const existing = (versions || []).find(
        (v) => (v.name || '').toLowerCase() === version.trim().toLowerCase(),
      );
      const realVersion =
        existing || version.trim().toLowerCase().startsWith('draft-')
          ? version
          : `draft-${version}`;
      ...
      Modal.confirm({
        title: (
          <span>
            Are you sure you want to change version to <b>{version}</b>?
          </span>
        ),
        style: {
          wordWrap: 'break-word',
  ...
  render() {
    const {className, style, disabled, pipelineVersion} = this.props;
    const {pending, versions, filter} = this.state;
    const value = filter === undefined ? pipelineVersion : filter;
    let filteredVersions = versions;
    if (filter && filter.length > 0) {
      filteredVersions = versions.filter((version) =>
        filterPipelineVersion(version, filter, false),
      );
    }
