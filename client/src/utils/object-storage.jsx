...
class ObjectStorage {
  constructor(options = {}, permissions = {}) {
    const {id, type, path, pathMask, region, delimiter = '/', mountPoint} = options;
    this.id = id;
    this.type = type;
    this.region = region;
    this.path = path;
  ...
  async generateFileUrl(file) {
    if (this.id && this.type && this.path && /^s3$/i.test(this.type)) {
      ...
      return s3Storage.getSignedUrlAsync(file);
    }
  ...
  async getFileContent(file, options = {}) {
    ...
    if (this.id && this.type && this.path && /^s3$/i.test(this.type)) {
      ...
      await s3Storage.updateCredentials();
      const url = await s3Storage.getSignedUrlAsync(file);
  ...
  async getFolderContents(folder) {
    ...
    function fetchPage(marker) {
      return new Promise((resolve) => {
        const request = new DataStorageRequest(id, decodeURIComponent(folder), false, false, 50);
        request
          .fetchPage(marker)
          .then(() => {
            if (request.loaded) {
          ...
          .then(({items, nextPageMarker}) => {
            if (!nextPageMarker) {
              ...
              return Promise.all([Promise.resolve(items), fetchPage(nextPageMarker)]);
            }
          })
...
function findStorageByPathFn(storagePath) {
  return function predicate(storage) {
    ...
    const {name, mountPoint = `/cloud-data/${name}`} = storage;
    const cloudDataMask = new RegExp(`^${escapeRegExp(mountPoint)}(/|$)`, 'i');
    return (
      storageMask.test(storagePath) ||
 ...
 */
export async function createObjectStorageWrapper(storages, storage, permissions, options) {
  const {isURL = false, generateCredentials = true} = options || {};
  let obj;
  let storagesArray = [];
  try {
  ...
  const filteredStoragesArray =
    storagesArray && typeof storagesArray.filter === 'function'
  ...
  } else if (typeof storage === 'string' && filteredStoragesArray) {
