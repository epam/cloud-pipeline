...
class CustomTagsEditor extends React.Component {
  ...
  componentDidUpdate(prevProps, prevState, snapshot) {
    if (this.props.tags !== prevProps.tags || this.props.visible !== prevProps.visible) {
      this.updateFromProps();
    }
  ...
  updateFromProps = () => {
    const {tags: propsTags = {}} = this.props;
    this.setState(
      {
        tags: {...propsTags},
      },
  ...
  updateRequiredAndVisibleTags = () => {
    ...
    (async () => {
      const [required, visible] = await Promise.all([
  ...
  updateValidation = () => {
    const {tags, required: requiredTags = [], visible: visibleTags = []} = this.state;
    ...
    (async () => {
      const validation = await getUserTagsValidationResult(tags, {requiredTags, visibleTags});
  ...
  handleSave = () => {
    ...
    this.props.onSave(filtered, tagsTouched);
  };
