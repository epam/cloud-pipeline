...
- [Exclude storages from file indexing](#exclude-storages-from-file-indexing)
...
## Exclude storages from file indexing

By default, the files of AWS S3, Google Cloud Storage, Azure Blob storage and NFS storages are indexed, so they can be found via the search.  
Separate storages can be left out of the file indexing - e.g. a large NFS storage with many changing files that are not needed in the search.

To exclude a storage, add the following attribute to it (how to add attributes see [here](../17_Tagging_by_attributes/17._CP_objects_tagging_by_additional_attributes.md#add-attributes)):

- key - `Billing status`
- value - `Exclude`

This attribute can be added by any user who can manage the storage attributes - an admin or the storage owner.

The search service checks this attribute at the start of each indexing cycle and skips the files of such storages.  
It works for all four storage types above. For NFS storages, the file change events sent by the NFS observer (if it is configured) are skipped as well.

**_Notes_**:

- the storage itself stays in the search - it can still be found by its name. Only its files are not indexed
- the files that were indexed before the attribute was added stay in the search results, but they are not updated anymore
- the same attribute is used by default to exclude a storage from the billing reports. If a storage shall be excluded from only one of them - change the attribute via the environment variables below

The attribute key and value can be changed separately for each storage type - via the environment variables of the search service (the `cp-config-global` config map):

| Storage type | Attribute key variable | Attribute value variable |
| --- | --- | --- |
| AWS S3 | `CP_SEARCH_S3_FILE_STORAGE_EXCLUDE_METADATA_KEY` | `CP_SEARCH_S3_FILE_STORAGE_EXCLUDE_METADATA_VALUE` |
| Google Cloud Storage | `CP_SEARCH_GS_FILE_STORAGE_EXCLUDE_METADATA_KEY` | `CP_SEARCH_GS_FILE_STORAGE_EXCLUDE_METADATA_VALUE` |
| Azure Blob storage | `CP_SEARCH_AZ_FILE_STORAGE_EXCLUDE_METADATA_KEY` | `CP_SEARCH_AZ_FILE_STORAGE_EXCLUDE_METADATA_VALUE` |
| NFS | `CP_SEARCH_NFS_FILE_STORAGE_EXCLUDE_METADATA_KEY` | `CP_SEARCH_NFS_FILE_STORAGE_EXCLUDE_METADATA_VALUE` |

If these variables are not set, `Billing status` and `Exclude` are used.  
The change of these variables is applied after the search service restart.
