...
function getCSVColumnValues(csvString, columnName, filterEmptyValues = false) {
  ...
  return filterEmptyValues ? values.filter((v) => v !== '') : values;
}
 ...
 */
async function validateItemSize({parameter, options, maxSizeMb, type = 'warning', message = ''}) {
  try {
    if (!parameter.value) {
      return {};
    }
    ...
    if (fileSizeMb > maxSizeMb) {
      const errorMessage =
        message || `File size is too large (maximum file size is ${maxSizeMb}MB).`;
...
async function validateCSVHeader({
  ...
  try {
    ...
    if (missingColumns.length > 0) {
      const errorMessage = message || `File has missing columns: ${missingColumns.join(', ')}`;
      return {[type]: errorMessage};
    }
    return {};
  } catch (error) {
