...
from cpaibot.pipeline.tools import (get_docker_images,
...
from cpaibot.llm.base import llm_simple_query
...
from cpaibot.agents.pipeline.utilities import (batched_execution,
                                               pick_best_elements,
                                               wrap_launch_payload_response)
from cpaibot.common.logger import CpLogger as Logger


default_logger = agents_logger
...
_docker_image_score_prompt_old = (f'Calculate score based on the following rules:\n'
...
_docker_image_score_prompt = (
    'Score each Docker image from 0.0 to 2.0 based on how well it matches the user query.\n\n'
    'Scoring rules (apply the FIRST rule that matches, OR calculate from rule 4):\n\n'
    '1. EXACT ID MATCH (score = 2.0):\n'
    '   - User mentions the exact Docker image ID number\n\n'
    '2. FULL PATH MATCH (score = 1.9):\n'
    '   - User query exactly matches full "registry/group/image" path\n\n'
    '3. GROUP + IMAGE MATCH (score = 1.8):\n'
    '   - User query exactly matches "group/image" format\n\n'
    '4. COMPONENT-BASED SCORING (sum applicable parts):\n'
    '   Start at 0.0, then add bonuses:\n\n'
    '   Image name matching:\n'
    '   - Exact match (user query == image name): +1.0\n'
    '   - Partial match (user query is substring of image name OR vice versa): +0.5\n'
    '   - IMPORTANT: Use ONLY ONE of the above (exact OR partial, never both)\n\n'
    '   Additional attributes:\n'
    '   - is_common == true: +0.3\n'
    '   - is_personal == true: +0.5\n\n'
    'Examples:\n'
    '- Query: "centos", Image: "centos", is_common=true\n'
    '  → 1.0 (exact) + 0.3 (common) = 1.3\n\n'
    '- Query: "centos", Image: "centos-commit", is_personal=true\n'
    '  → 0.5 (partial) + 0.5 (personal) = 1.0\n\n'
    '- Query: "nginx", Image: "my-nginx-server", is_personal=true\n'
    '  → 0.5 (partial) + 0.5 (personal) = 1.0\n\n'
    '- Query: "#42", Image id: 42\n'
    '  → 2.0 (exact ID)\n\n'
    'CRITICAL: Never add both exact (+1.0) AND partial (+0.5) bonuses together.'
)
...
def pick_docker_image_by_query(
    ...
    logger.info(f"loading docker images...")
    all_images = get_docker_images(bearer=bearer)
    logger.info(f"{len(all_images)} docker images loaded")
    if len(all_images) == 0:
        return []

    exact = find_docker_image(query, docker_images=all_images)
    if exact is not None:
        logger.info(f'docker image "{query}" found (exact match)')
        return [exact]

    if docker_image is not None:
        exact = find_docker_image(docker_image, docker_images=all_images)
        if exact is not None:
            logger.info(f'docker image "{docker_image}" found (exact match)')
            return [exact]

    logger.debug(f'picking docker images by query -> docker images count: {len(all_images)}')

    if not is_image_name:
        ...
        logger.debug(f'pick_docker_image_by_query -> specific tool llm response: {specific_tool_resp}')
...
def get_docker_image_version_from_query(
        query: str,
        docker_image: DockerImage,
        /,
...
def launch_tool_by_user_query(
    ...
    try:
        ...
        version = get_docker_image_version_from_query(
            query,
            docker_image,
