...
User groups, in their turn, have a higher priority than system roles, e.g. if a user's group is allowed to read an object and one of the user's system roles is denied to read it, the user will have an access to that object. Within the same level - several user groups, or several system roles - a **deny** rule has a higher priority than an **allow** one.  
