...
export default class AdaptedLink extends Component {
  ...
  get currentPath() {
    ...
    return location.pathinfo ? location.pathinfo : location.pathname;
  }
  ...
  render() {
    ...
    if (!ignoreCurrentPath && (!to || (this.currentPath && to === this.currentPath))) {
      return (
        <span {...additionalProps} style={linkStyle}>
          {children}
        </span>
      );
    }
    return (
      <Link {...additionalProps} style={linkStyle} to={to} onClick={(e) => e.stopPropagation()}>
        {children}
      </Link>
    );
  }
