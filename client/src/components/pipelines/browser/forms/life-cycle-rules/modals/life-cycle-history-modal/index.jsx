...
import {Button, Modal, Row, Select, Table, message, Spin} from 'antd';
...
class LifeCycleHistoryModal extends React.Component {
  ...
  get history() {
    ...
    const executionsData = executions.map((execution) => ({
      ...
      action:
        DESTINATIONS[execution.storageClass] === DESTINATIONS.DELETION
    ...
    const prolongationsData = this.prolongations.map((prolongation) => ({
      date: dayjs.utc(prolongation.prolongedDate),
      action: ACTION_TYPES.prolongation,
      user: prolongation.userId ? this.getUserById(prolongation.userId) : '',
  ...
  get filteredHistory() {
    ...
    if (actionFilter && actionFilter.length > 0) {
      return this.history.filter((entry) => actionFilter.includes(entry.action));
    }
    return this.history;
  }
