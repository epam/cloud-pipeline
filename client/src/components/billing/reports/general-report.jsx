...
import {BarChartWithQuota, GroupedBarChart, BillingTable, PieChart, Summary} from './charts';
...
import {costTickFormatter, getUserDisplayInfo, DisplayUser, ResizableContainer} from './utilities';
...
class BillingCenters extends React.Component {
  ...
  render() {
    const {requests, discounts: discountsFn, onSelect, height, title} = this.props;
    ...
    return (
      <div>
        <Radio.Group
          ...
          size="small"
        >
          <Radio.Button key={BillingCentersDisplayModes.bar} value={BillingCentersDisplayModes.bar}>
            Bar chart
          </Radio.Button>
          <Radio.Button key={BillingCentersDisplayModes.pie} value={BillingCentersDisplayModes.pie}>
            Pie chart
          </Radio.Button>
        </Radio.Group>
        {mode === BillingCentersDisplayModes.bar && (
...
function UserReport({resources, summaryCompute, summaryStorages, filters, user}) {
      ...
      {(computeDiscounts, storageDiscounts) => (
        <Export.Consumer
          exportConfiguration={{
            ...
            range,
            filters: {
              cloudRegionId: cloudRegionId && cloudRegionId.length > 0 ? cloudRegionId : undefined,
            },
          }}
        >
          <Layout
          ...
          >
            ...
            <div key={GeneralReportLayout.Panels.runners}>{'\u00A0'}</div>
          </Layout>
        </Export.Consumer>
      )}
    </Discounts.Consumer>
...
class UsersChartComponent extends React.Component {
  ...
  render() {
    const {requests, discounts, title, onSelect, style, users, preferences} = this.props;
    return (
      <BarChartWithQuota
  ...
  return (
    <Discounts.Consumer>
      {(computeDiscounts, storageDiscounts) => (
        ...
        >
          <Layout layout={GeneralReportLayout.Layout} gridStyles={GeneralReportLayout.GridStyles}>
            <div key={GeneralReportLayout.Panels.summary}>
              <Layout.Panel
                style={{
                  display: 'flex',
...
function DefaultReport(props) {
  ...
  if ((billingGroup && billingGroup.length === 1) || (adGroup && adGroup.length === 1)) {
...
export default withRouter(
  inject(
    ...
    inject(injection)(
      BillingNavigation.attach(roleModel.authenticationInfo(observer(DefaultReport))),
    ),
  ),
);
