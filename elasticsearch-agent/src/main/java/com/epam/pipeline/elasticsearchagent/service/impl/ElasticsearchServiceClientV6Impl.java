...
@Slf4j
@Service
@ConditionalOnProperty(name = "elasticsearch.client.version", value = "6")
public class ElasticsearchServiceClientV6Impl implements ElasticsearchServiceClient {

    private RestHighLevelClient client;

    @Autowired
    public ElasticsearchServiceClientV6Impl(
    ...
    public ElasticBulkResponse sendRequests(String indexName, List<? extends ElasticActionRequest> docWriteRequests) {
        ...
        docWriteRequests.stream().map(ElasticActionRequest::toElasticV6Request).forEach(bulkRequest::add);
        ...
        try {
            ...
            return ElasticBulkResponse.builder().version(ElasticStackVersion.V6)
                    .v6Response(bulkResponse).build();
        } catch(IOException e) {
