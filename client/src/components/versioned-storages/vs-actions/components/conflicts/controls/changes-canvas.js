...
function drawCurve(context, start, end, options = {}) {
  const {reverse = false, lineToFirstPoint = false} = options;
...
export default function renderChanges(canvas, conflictedFile, branch, options = {}) {
  if (canvas && canvas.getContext) {
    ...
    if (context) {
      ...
      const currentModifications = (conflictedFile?.changes || []).filter(
        (m) => m.branch === branch,
      );
      for (let i = 0; i < currentModifications.length; i++) {
        ...
        const currentY1 = current.start * lineHeight + currentModificationsBefore * 2.0 + 0.5 - top;
        const currentY2 =
          current.end * lineHeight + (currentModificationsBefore + 1) * 2.0 - 0.5 - top;
        const mergedY1 =
          merged.start * lineHeight + mergedModificationsBefore * 2.0 + 0.5 - mergedTop;
        const mergedY2 =
          merged.end * lineHeight + (mergedModificationsBefore + 1) * 2.0 - 0.5 - mergedTop;
