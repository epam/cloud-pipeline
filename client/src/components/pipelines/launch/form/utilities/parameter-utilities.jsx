...
import {systemCapabilitiesParameters, reservedParameters} from './parameters';
...
function normalizeParameters(parameters) {
  ...
  if (keys.length > 0) {
    return keys
      .map((key) =>
        Object.hasOwn(params, key) && params[key].name
...
function isVisibleFieldFn(parameter) {
  return (
    parameter &&
    Object.hasOwn(parameter, 'visible') &&
...
function mapKey(normalizedParameters) {
  return (key) => {
    if (Object.hasOwn(normalizedParameters, key) && normalizedParameters[key].value !== undefined) {
...
function buildValidationFn(element) {
  if (
    !element ||
    !Object.hasOwn(element, 'validation') ||
...
function buildValidationRuleFn(validationRule) {
  ...
  const {message = 'Validation error', throw: rule = 'false'} = validationRule;
  return (normalizedParameters) => {
...
function parseEnumeration(parameter) {
  if (
    !parameter ||
    !Object.hasOwn(parameter, 'enumeration') ||
    !(Array.isArray(parameter.enumeration) || isObservableArray(parameter.enumeration)) ||
    parameter.enumeration.length === 0
  ) {
    return undefined;
  }
  return parameter.enumeration.map(parseSingleEnumValue).filter(Boolean);
}

function getDependencyLevel(tree, node) {
  if (!tree || !Object.hasOwn(tree, node)) {
    return 0;
  }
  if (tree[node].marked) {
      ...
      if (param) {
        ...
        if (param.enumeration && param.enumeration.length && param.value) {
          const enums = param.enumeration
            ...
            .map((e) => e.name);
          const clearValue = enums.filter((v) => param.value === v).length === 0;
          if (clearValue) {
            modified = true;
            param.value = undefined;
          }
...
function getParameterValue(parameters, parameter, defaultValue = undefined) {
  const value =
    parameters && Object.hasOwn(parameters, parameter) ? parameters[parameter].value : undefined;
  if (value === undefined) {
    return defaultValue;
  }
  return value;
  ...
  try {
    ...
    if (typeof result === 'string') {
      try {
        ...
        // not a JSON string
        console.warn(
          `${errorTitle}: it is expected to be a string (base64) representing a JSON array`,
        );
        console.warn(error);
      }
    }
  ...
  for (const param of metadataEntityParams) {
    ...
    const externalIds = config.multiple
      ? typeof value === 'string'
        ...
        : [value]
      : [value];
    const matchingElements = metadataEntries.elements.filter((element) =>
      externalIds.includes(element.externalId),
    );
    ...
    for (const [paramName, metadataField] of Object.entries(paramsMapping)) {
      ...
      const resolvedValue = config.multiple ? values.join(',') : values[0];
      if (result[paramName]) {
        result[paramName] = {
          ...result[paramName],
          value: resolvedValue,
...
async function getMetadataForParameters(parameters = [], pipeline, pipelinesLibrary) {
  const metadataEntityParams = parameters.filter(
    (p) => p.type === 'metadata_entity' && p.config?.metadata_config?.metadataClass,
  );
  if (metadataEntityParams.length === 0) {
    return {};
  }
  ...
  const requests = Object.entries(requestsTemp).map(([key, {name, folderId, metadataClass}]) => {
      ...
      filter
        ...
        .then(() => {
          if (filter.error || !filter.loaded) {
            console.error(`Error fetching metadata for ${name} parameter:`, filter.error);
...
async function readParametersFromConfiguration(configuration, options = undefined) {
  try {
    const {detached = false, pipeline = false} = options || {};
    await Promise.all([
    ...
    for (const [condition, paramsMap] of Object.entries(conditionalParameters || {})) {
      for (const [name, parameter] of Object.entries(paramsMap || {})) {
        const normalized = getParameterConfig(name, parameter, {condition, detached, pipeline});
...
async function mergeParametersWithConfiguration(parameters, options) {
  ...
  const cfg =
    configurationParameters || (await readParametersFromConfiguration(configuration, options));
...
function getParameterSchemeConfig(parameterScheme) {
  ...
  try {
    const {properties = {}} = parameterScheme;
    const props = [];
...
function mapObjectParameterValue(value, options) {
  const {parameterName, scheme} = options || {};
  if (VERBOSE) {
    console.groupCollapsed(
      parameterName ? `Parsing "${parameterName}" value` : 'Parsing object parameter value',
    );
  }
  try {
    if (VERBOSE) {
 ...
 * @returns {ParameterConfig}
 */
export function getParameterConfig(name, parameter, options = undefined) {
  if (VERBOSE) {
    console.groupCollapsed(name);
    console.log(parameter);
  }
  try {
    const {condition = undefined, detached = false, pipeline = false} = options || {};
    const configId = condition ? `condition: ${condition}` : 'default';
    const system =
      isSystemParameter(name) ||
    ...
    if (typeof parameter === 'object') {
      ...
      let type = pType;
      if (type === 'object') {
        const {value: mappedValue, scheme: mappedScheme} = mapObjectParameterValue(parameterValue, {
          scheme,
          parameterName: name,
        });
        scheme = mappedScheme;
      ...
      if (type === 'object' && scheme === undefined) {
        if (VERBOSE) {
          console.log(
            'correcting parameter type from "scheme" to "string" because scheme was not provided (or scheme.properties is empty)',
          );
        }
        type = 'string';
      }
      ...
      return {
        ...
        icon,
        enumeration:
          enumeration && (Array.isArray(enumeration) || isObservableArray(enumeration))
            ? [...enumeration]
            : undefined,
        multiple,
...
function getParameterValidationError(parameters, config) {
  ...
  if (validation) {
    const rules =
      Array.isArray(validation) || isObservableArray(validation) ? validation : [validation];
...
function isRestrictedParameter(parameterName) {
  ...
  if (sp) {
    const {admin, roles: userRoles = []} = whoAmI.value || {};
    if (admin) {
      return false;
    }
...
function validateParameter(parameter, parameters, rawEdit = false) {
  ...
  try {
    ...
    const duplicates = parameters.filter(
      (p) => p.name.toLowerCase() === parameter.name.toLowerCase(),
    );
    if (duplicates.length > 1) {
      throw new Error('No duplicates are allowed');
    }
    if (!actualConfig.system && isSystemParameter(parameter.name)) {
...
function validateParametersIteration(parameters, rawEdit = false) {
  ...
  for (let i = 0; i < parameters.length; i += 1) {
    const {modified} = validateParameter(result[i], parameters, rawEdit);
    if (modified) {
      changed = true;
...
function parseCustomValidatorsModule(moduleCode) {
  ...
  try {
    ...
    const validators = moduleFunction({exports: moduleExports}, moduleExports, (moduleName) => {
...
async function customValidate(customValidators, parameters, opts) {
  ...
  const result = parameters.slice();
  let changed = false;
  const _validateFn = async (parameter) => {
    try {
      ...
      console.error(`Custom validator for "${parameter.name}" failed:`, error.message);
  ...
  validationResults.forEach((parameter) => {
    ...
    if (idx >= 0) {
      const shouldUpdate =
        parameter.error !== result[idx].error ||
...
function validateParameters(parameters, rawEdit = false) {
  ...
  while (idx < maxIterations && (lastIterationChanged || idx === 0)) {
    idx += 1;
    const {modified, parameters: result} = validateParametersIteration(modifiedParameters, rawEdit);
...
function addSystemParameter(parameters, systemParameter) {
  const cfg = getParameterConfig(systemParameter.name, {
...
function hasResolvedValues(parameters) {
  return parameters.some(
    (p) => p.config.resolvedValue !== undefined && String(p.config.resolvedValue).trim().length > 0,
  );
}
...
export function getSystemParameter(parameterName, options = {}) {
  const {runDefaultParameters: rdf = runDefaultParameters} = options || {};
...
function isSystemParameterRestrictedByRole(parameter, options = {}) {
  if (parameter && isSystemParameter(parameter.name, options)) {
    const {runDefaultParameters, userInfo} = options;
    const {admin} = userInfo || {};
    const [systemParam] = (runDefaultParameters?.value || []).filter(
    ...
    if (!admin && userInfo && systemParam?.roles?.length > 0) {
      return !systemParam.roles.some((roleName) => userRoleNames.includes(roleName));
    }
    return false;
  }
  return false;
...
export function mapSystemParameter(parameter, options = {}) {
  const {runDefaultParameters: rdf = runDefaultParameters, userInfo} = options || {};
  if (rdf.loaded && userInfo && parameter) {
    try {
      const value = rdf.value || [];
      const systemParameter = value.find(
        (p) => p.name.toLowerCase() === parameter.name.toLowerCase(),
      );
      if (systemParameter) {
...
export function getCapabilitiesParameters(options = {}) {
  const {preferences: prefs = preferences, onlyCapabilityFlags = true} = options || {};
  if (prefs.loaded) {
    ...
    return systemCapabilitiesParameters
      .slice()
      .concat(capabilities.reduce((acc, cur) => acc.concat(getParameters(cur)), []));
  }
  return [];
}
...
export function isReservationRequestParameter(parameterName, options = {}) {
  const {preferences: prefs = preferences} = options || {};
...
function getTypedValue(value, config) {
  const {type = 'string', scheme} = config || {};
...
export function parametersModified(parameters, initialParameters) {
  const parameterExists = (parameter, array) => {
    ...
    if (existing) {
      return (
        getParameterTypedValue(existing) === getParameterTypedValue(parameter) &&
...
function getParametersWarning(parameters = []) {
  const withWarnings = parameters.filter(
    (parameter) => parameter.warning && parameter.warning.trim().length > 0,
  );
