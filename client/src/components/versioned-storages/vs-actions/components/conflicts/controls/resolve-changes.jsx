...
import styles from './resolve-changes.module.css';
...
function ResolveChanges({className, conflictedFile}) {
  if (!conflictedFile) {
    return null;
  }
  const {changes = []} = conflictedFile;
  const notProcessedChanges = changes.filter((change) => change.status === ChangeStatuses.prepared);
  const changesCount = new Set(
    notProcessedChanges.filter((change) => !change.conflict).map((change) => change.key()),
  ).size;
  const conflictsCount = new Set(
    notProcessedChanges.filter((change) => change.conflict).map((change) => change.key()),
  ).size;
  ...
  return (
    ...
    >
      ...
      <span id="conflict-changes-summary" className={styles.total}>
        {[
          ...
          .filter(Boolean)
          .join('. ')}
        {changesCount === 0 && conflictsCount === 0 && 'All conflicts have been resolved'}
      </span>
    </div>
  );
}
