...
function isAutoFillEntitiesMarker(element) {
  return element && element.dataset && Object.hasOwn(element.dataset, 'autoFillEntities');
}
function isAutoFillInteractionTarget(element) {
  if (!element || !(element instanceof Node)) {
    return false;
  }
  return !!element.closest?.(
    '[data-auto-fill-actions], .ant-dropdown, .ant-dropdown-menu, .ant-dropdown-menu-item',
  );
}
...
function mapItemSaveOperation(item, options) {
  const {classId, className, parentId} = options || {};
...
function buildClearPropertiesAction(options) {
  const {targetItems, removedItems, targetColumns, removedColumns} = options;
  if (removedItems.length > 0) {
    ...
    return {
      ...
      action: () =>
        new Promise((resolve, reject) => {
          Promise.all(removedItems.map(processItem)).then(resolve).catch(reject);
        }),
    };
  }
  if (removedColumns.length > 0) {
    ...
    return {
      ...
      action: () =>
        new Promise((resolve, reject) => {
          Promise.all(targetItems.map(processItem)).then(resolve).catch(reject);
        }),
    };
  }
  return undefined;
...
function buildCopyAction(options) {
  ...
  if (insertedItems.length > 0) {
    const processItem = (item, index) => {
      ...
      for (let c = 0; c < targetColumns.length; c++) {
        const column = targetColumns[c].key;
        const value = Object.hasOwn(source, column) ? source[column].value : undefined;
        const type = Object.hasOwn(source, column) ? source[column].type : undefined;
        if (value === undefined) {
          delete payload[column];
        } else {
          payload[column] = {
    ...
    return {
      ...
      action: () =>
        new Promise((resolve, reject) => {
          Promise.all(insertedItems.map(processItem)).then(resolve).catch(reject);
        }),
    };
  }
  if (insertedColumns.length > 0) {
    const processItem = (item) => {
      ...
      for (let c = 0; c < insertedColumns.length; c++) {
        ...
        const value = Object.hasOwn(payload, sourceColumn)
          ? payload[sourceColumn].value
          : undefined;
        const type = Object.hasOwn(payload, sourceColumn) ? payload[sourceColumn].type : undefined;
        if (value === undefined) {
...
function buildAutoFillAction(options) {
  ...
  const getIncrementedValue = (table) => {
    ...
    return (source, incrementRatio) => {
      ...
      const shiftedNumber = getShiftedNumber(!last.string ? number : Math.abs(number), shift);
...
function buildAutoFillActions(elements, columns, source, target, backup, options) {
  ...
  const sourceItems = elements.filter(
    (item) => item.row >= source.start.row && item.row <= source.end.row,
  );
  const targetItems = elements.filter(
    (item) => item.row >= target.start.row && item.row <= target.end.row,
  );
  const insertedItems = targetItems.filter((item) => sourceItems.indexOf(item) === -1);
  if (target.start.row < source.start.row) {
    insertedItems.reverse();
  }
  const removedItems = sourceItems.filter((item) => targetItems.indexOf(item) === -1);
  ...
  const targetColumns = columns
    ...
    .filter((item) => !/^(ID|createdDate|SampleSheet)$/i.test(item.key));
  const insertedColumns = targetColumns.filter((item) => sourceColumns.indexOf(item) === -1);
  if (target.start.column < source.start.column) {
    insertedColumns.reverse();
  }
  const removedColumns = sourceColumns.filter((item) => targetColumns.indexOf(item) === -1);
...
class AutoFillEntitiesActions extends React.Component {
  ...
  render() {
    const {actions, callback, className} = this.props;
    if (actions && actions.length > 0) {
      const handleMenuItem = (info) => {
        info.domEvent?.stopPropagation?.();
        info.domEvent?.preventDefault?.();
        this.props.onMenuInteract?.();
        const index = +info.key;
        ...
        >
          <Dropdown
            ...
            onOpenChange={(visible) => {
              ...
              this.handleDropdownVisibility(visible);
            }}
          >
            <Button
              type="primary"
