...
function RootEntityTypeParameter(props) {
  const {className, style, disabled, onChange, currentMetadataEntity = [], rootEntityId} = props;
  if (currentMetadataEntity.length === 0) {
    return null;
  }
  return (
    <div className={classNames(className, styles.launchFormParameter)} style={style}>
      <StaticParameterName style={{paddingRight: 30}}>Root entity type</StaticParameterName>
      <div style={{display: 'flex', flexWrap: 'nowrap', fontSize: 'larger', paddingRight: 30}}>
        <Form.Item hasFeedback style={{flex: 1, marginBottom: 0}}>
          <Select
            disabled={disabled}
            allowClear
            value={rootEntityId}
