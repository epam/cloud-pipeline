...
import styles from './configure-file-system.module.css';
...
class ConfigureFileSystem extends React.Component {
  ...
  reportChange = () => {
    const {fsType, deploymentType, volume, throughput, iops} = normalizeFsConfig(this.state);
  ...
  render() {
    const {notSupported, className, style, cloudRegionProvider, disabled = false} = this.props;
    ...
    return (
      <div
        className={classNames(className, styles.configureFileSystemContainer, {
          [styles.large]: fsType === ShareFsType.lustre && throughputOptions.length > 0,
        })}
        style={style}
      >
        <div style={{fontWeight: 'bold'}}>Configure File System</div>
        <div className={styles.configureFsRowWithErrorContainer}>
          <div className={styles.configureFsRow}>
            <span className={styles.configureFsTitle}>Type:</span>
            <Select
        ...
        {fsType === ShareFsType.lustre && (
          <div className={styles.configureFsRowWithErrorContainer}>
            <div className={styles.configureFsRow}>
              <span className={styles.configureFsTitle}>Deployment type:</span>
              <Select
        ...
        {fsType === ShareFsType.lustre && (
          <div className={styles.configureFsRowWithErrorContainer}>
            ...
            {volumeError && (
              <div className={classNames(styles.configureFsError, 'cp-error')}>{volumeError}</div>
            )}
          </div>
        )}
        ...
        {fsType === ShareFsType.lustre && iopsOptions.length > 0 && (
          <div className={styles.configureFsRowWithErrorContainer}>
            ...
            {iopsError && (
              <div className={classNames(styles.configureFsError, 'cp-error')}>{iopsError}</div>
            )}
          </div>
        )}
      </div>
