...
export function mapObservableNotification(notification) {
  ...
  const {type, recipients = [], triggerStatuses = [], subject, body} = notification;
  return {
    type,
...
class JobNotification extends React.Component {
  ...
  get statuses() {
    return ['FAILURE', 'PAUSED', 'PAUSING', 'RESUMING', 'RUNNING', 'STOPPED', 'SUCCESS'];
  }
  ...
  validate = () =>
    new Promise((resolve) => {
      const {type, recipients, triggerStatuses, body, subject} = this.state;
  ...
  handleChange = () => {
    ...
    if (onChange) {
      const {type, recipients, triggerStatuses, subject, body} = this.state;
      onChange({
        type,
        recipients,
        triggerStatuses,
  ...
  render() {
    ...
    const {type, triggerStatuses, recipients, body, subject, validationErrors} = this.state;
    ...
    if (availableNotificationTypesSet.size > 0) {
      filteredTemplates = [...templates].filter((template) =>
        availableNotificationTypesSet.has(template),
      );
    }
    return (
      <div className={classNames(styles.notification, 'cp-even-odd-element')}>
        <div className={styles.row}>
          {(filteredTemplates.length > 1 || !!validationErrors.type) && (
          ...
          <div className={styles.labeledGroup} style={{flex: 1}}>
            <span
              className={classNames(styles.label, {
            ...
            >
              {this.statuses.map((status) => (
                <Select.Option key={status} value={status}>
                  <StatusIcon status={status} className={styles.statusIcon} /> {status}
                </Select.Option>
              ))}
            </Select>
          </div>
          <div className={styles.labeledGroup} style={{flex: 1}}>
            ...
            >
              {(usersInfo.loaded ? usersInfo.value || [] : []).map((user) => (
                <Select.Option
                  key={user.name}
                  value={user.name}
                  attributes={[user.name, ...Object.values(user.attributes || {})].join('|')}
                >
        ...
        <div className={styles.row}>
          <div className={styles.labeledGroup} style={{alignItems: 'flex-start', flex: 1}}>
            <span
              className={classNames(styles.label, {
        ...
        {removable && (
          <div className={styles.row} style={{justifyContent: 'flex-end'}}>
            <Button size="small" danger onClick={this.handleRemove}>
