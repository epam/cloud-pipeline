...
import styles from './storage-report.module.css';

function StorageTable({storages, discounts: discountsFn, height, aggregate, showDetails}) {
  if (!storages || !storages.loaded) {
    return null;
  }
  const storageClass =
    aggregate && aggregate !== StorageAggregate.default
  ...
  const getLayerValue = (item, key) => {
    const {costDetails = {}, value, usage, usageLast} = item || {};
    if (!showDetails) {
      switch (key) {
        case 'size':
          return usageLast || 0;
  ...
  const getDetailedCellsTitle = (title, measure) => {
    const details = [measure, storageClassName].filter(Boolean);
    if (details.length > 0) {
    ...
    [
      {
        ...
        className: showDetails
          ...
          : styles.cell,
        headerClassName: showDetails ? classNames(styles.cell, styles.centeredCell) : styles.cell,
      },
      showDetails
        ? {
  ...
  const columns = [
    ...
    },
    {
      ...
      title: 'Created date',
      dataIndex: 'created',
      render: (value) => (value ? displayDate(value, 'DD MMM YYYY') : value),
      className: styles.cell,
    },
  ];
  ...
  const paginationEnabled = storages && storages.loaded ? storages.totalPages > 1 : false;
  ...
  return (
    <div className={styles.storageReport}>
      <div
        className={classNames(styles.storageReportTableContainer, 'cp-bordered')}
        style={{
          maxHeight: height - (paginationEnabled ? 40 : 10),
        }}
      >
        <table className={classNames('cp-report-table', styles.storageReportTable)}>
          <thead>
            <tr>
              {columns
          ...
          <tbody>
            {dataSource.map((item, index) => (
              <tr key={`item-${index}`} className={getRowClassName(item)}>
                {columns.map((column) => (
                  <td
                    key={column.key}
                    className={classNames(column.className, styles.cell, {
