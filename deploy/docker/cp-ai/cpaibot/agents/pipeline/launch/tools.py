...
from cpaibot.pipeline.types import (Configuration,
                                  Pipeline,
                                  InstanceType)
from cpaibot.pipeline.preferences import (get_run_capabilities_parameters,
                                        get_autoscaling_configuration_parameters)
from cpaibot.pipeline.parameters import get_system_parameters, KNOWN_SYSTEM_PARAMETERS
from cpaibot.pipeline.instances import get_allowed_instance_types
from cpaibot.pipeline.tools import find_docker_image
from cpaibot.common.utils import extract_json_response
from cpaibot.llm.base import llm_simple_query
from typing import Any, Callable
from cpaibot.agents.pipeline.types import LaunchPayload, ClusterInfo
...
def generate_launch_payload(
        ...
        instance_disk: float | None = None,
        parameters: dict | None = None,
        spot: bool | None = None,
        bearer: str | None = None,
        generation_info_callback: Callable[[str], Any] | None = None,
        launch_payload: LaunchPayload | None = None,
        cluster_config: ClusterInfo | None = None,
        **kwargs
) -> LaunchPayload:
        ...
        for parameter, parameter_value in parameters.items():
            if parameter_value is not None:
                if isinstance(parameter_value, dict):
                    parameter_value = parameter_value.get('value', None)
                existing = params.get(parameter, {})
                existing.update({'value': parameter_value})
                params.update({parameter: existing})
    # ------------
    def remove_parameter(p: str):
        if p in params:
            del params[p]

    def set_parameter_value(p: str, p_value: str | bool | int | None):
        remove_parameter(p)
        if p_value is not None:
            if isinstance(p_value, bool):
                params.update({p: {"value": "true" if p_value else "false", "type": "boolean"}})
            elif isinstance(p_value, int):
                params.update({p: {"value": p_value, "type": "int"}})
            else:
                params.update({p: {"value": str(value), "type": "string"}})

    # ------------
    node_count = None
    if cluster_config is not None:
        if cluster_config.is_cluster:
            node_count = cluster_config.node_count or 1
            if cluster_config.is_auto_scaled:
                set_parameter_value('CP_CAP_AUTOSCALE', True)
                set_parameter_value('CP_CAP_AUTOSCALE_HYBRID', cluster_config.hybrid)
                set_parameter_value('CP_CAP_AUTOSCALE_WORKERS', node_count)
                node_count = cluster_config.default_nodes_count
            else:
                remove_parameter('CP_CAP_AUTOSCALE')
                remove_parameter('CP_CAP_AUTOSCALE_HYBRID')
                remove_parameter('CP_CAP_AUTOSCALE_WORKERS')
            set_parameter_value('CP_CAP_SGE', cluster_config.sge)
            set_parameter_value('CP_CAP_SLURM', cluster_config.slurm)
            set_parameter_value('CP_CAP_SPARK', cluster_config.spark)
            set_parameter_value('CP_CAP_KUBE', cluster_config.kubernetes)
            set_parameter_value('CP_CAP_AUTOSCALE_HYBRID', cluster_config.hybrid)
        else:
            node_count = None
            remove_parameter('CP_CAP_AUTOSCALE')
            remove_parameter('CP_CAP_AUTOSCALE_HYBRID')
            remove_parameter('CP_CAP_AUTOSCALE_WORKERS')
            remove_parameter('CP_CAP_SGE')
            remove_parameter('CP_CAP_SLURM')
            remove_parameter('CP_CAP_SPARK')
            remove_parameter('CP_CAP_KUBE')
            remove_parameter('CP_CAP_AUTOSCALE_HYBRID')
    # ------------
