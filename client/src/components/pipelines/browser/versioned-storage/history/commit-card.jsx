...
class CommitCard extends React.PureComponent {
  ...
  onRevert = (payload) => {
    ...
    const {versionedStorageId, path, commit, onRefresh} = this.props;
    this.setState(
      {
        pending: true,
      },
  ...
  render() {
    ...
    const {filePreviewVisible, pending, revertDialogVisible} = this.state;
    if (!commit) {
      return null;
    }
    return (
      ...
      >
        <div className={styles.commitControls}>
          {isFile && !isLatestFileCommit && !readOnly && (
            <Button size="small" style={{marginRight: '2px'}} onClick={this.openRevertDialog}>
              <RetweetOutlined />
            </Button>
          )}
          {isFile && (
            <Button onClick={this.showFilePreview} size="small" style={{marginRight: '2px'}}>
              <DownloadOutlined />
            </Button>
          )}
          <CommitDiffButton
        ...
        <div>
          {commit.commit_message && (
            <div className={classNames(styles.line, styles.message, 'cp-accent')}>
          ...
          <div className={styles.line}>
            ...
            <UserName className={classNames(styles.author, 'cp-accent')} userName={commit.author} />
