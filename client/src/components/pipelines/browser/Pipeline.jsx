...
import GitRepositoryControl, {normalizeRepositoryType} from '../../special/git-repository-control';
import {Alert, Button, Col, Dropdown, message, Popover, Row, Select, Table} from 'antd';
import {
  AppstoreFilled,
  CheckCircleFilled,
  CopyOutlined,
...
class Pipeline extends localization.LocalizedReactComponent {
  ...
  get showMetadata() {
    ...
    if (this.state.metadata === undefined && this.props.pipeline.loaded) {
      return (
        this.props.pipeline.value.hasMetadata && roleModel.readAllowed(this.props.pipeline.value)
      );
    }
  ...
  renderTreeItemType = (item) => {
    ...
    switch (item.type) {
      case ItemTypes.pipeline:
        return <ForkOutlined className={className} />;
      case ItemTypes.folder:
        return <FolderOutlined className={className} />;
      case ItemTypes.version:
        return <TagFilled className={className} />;
      default:
        return <div />;
    }
  };
  ...
  renderTreeItemActions = (item) => {
    if (this.props.listingMode || this.props.readOnly) {
      if (this.props.configurationSelectionMode) {
        if (
          this.props.lazyLoadingConfigurations &&
          this.state.configurationsPending.includes(item.id)
        ) {
          ...
          return (
            ...
            >
              {configurations.map((c) => {
                return <Select.Option key={c.name}>{c.name}</Select.Option>;
              })}
            </Select>
          );
        }
            ...
            <Row type="flex" justify="end">
              ...
              {roleModel.executeAllowed(this.props.pipeline.value) && (
                <Button
                  ...
                  size="small"
                  type="primary"
                  onClick={(event) => this.runPipeline(event, item.name)}
                >
                  RUN
                </Button>
              )}
  ...
  reload = async () => {
    ...
    await Promise.all([this.props.pipeline.fetch(), this.props.versions.fetch()]);
    if (this.props.onReloadTree) {
      ...
      this.props.onReloadTree(!parentFolderId, parentFolderId);
    }
  };
  ...
  openRegisterVersionDialog = (releaseCandidate, event) => {
    event.stopPropagation();
    this.setState({releaseCandidate});
  };
  ...
  renderConfigAction = () => {
    ...
    const onClick = ({key}) => {
      ...
      switch (key) {
        case 'edit':
          this.openEditPipelineDialog();
          break;
        case 'clone':
          this.openClonePipelineDialog();
          break;
      }
    };
    ...
    if (actions.length > 0) {
      return (
        <Dropdown
          ...
          key="edit"
        >
          <Button key="edit" id="edit-pipeline-menu-button" style={{lineHeight: 1}} size="small">
            <SettingOutlined />
          </Button>
        </Dropdown>
      );
  ...
  render() {
    ...
    if (this.props.versions.loaded) {
      ...
      if (this.props.allowSelectLatestVersion) {
        this._versions = [LATEST_VERSION_PLACEHOLDER, ...this._versions];
      }
      versionsContent = (
        <Table
  ...
  loadConfigurations = async () => {
    ...
    if (this.props.allowSelectLatestVersion) {
      versions = [LATEST_VERSION_PLACEHOLDER, ...versions];
    }
  ...
  componentDidUpdate(prevProps) {
    ...
    if (
      ...
      const versionsReady =
        this._lastVersionsPending === true && this.props.versions.pending === false;
