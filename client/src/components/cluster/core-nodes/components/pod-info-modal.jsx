...
import {Button, Spin, Modal, Alert, Tabs, message} from 'antd';
...
export default class PodInfoModal extends React.Component {
  ...
  renderEventsTab = () => {
    ...
    if (!this.events.length) {
      return <div>No events information</div>;
    }
  ...
  renderInformationTab = () => {
    ...
    return (
      ...
      >
        <CodeEditor readOnly delayedUpdate language="javascript" lineWrapping code={description} />
      </div>
    );
  };
        ...
        <div style={{display: 'flex', flex: 1}}>
          <Tabs
            ...
            items={tabs.map((tab) => ({
              key: tab.key,
              label: tab.title,
              children: <Spin spinning={this.pending}>{tab.render()}</Spin>,
            }))}
          />
        </div>
      </Modal>
