...
export default class UsersManagement extends React.Component {
  ...
  get users() {
    ...
    if (users && users.loaded) {
      return (users.value || []).map(this.mapUserRolesAndGroups).sort(this.alphabeticNameSorter);
    }
    return [];
  }
  ...
  get filteredUsers() {
    const {userSearchText, filterUsers} = this.state;
    return this.users.filter(usersFilter(userSearchText)).filter((user) => {
      switch (filterUsers) {
  ...
  onUserSearchChanged = (e) => {
    const userSearchText = e.target.value;
    this.setState({
      userSearchText,
      usersTableCurrentPage: 1,
    });
  ...
  renderUsersTableControls = () => {
    ...
    return (
      <Row type="flex" style={{marginBottom: 10}}>
        ...
        >
          <Select.Option key={USERS_FILTERS.all} value={USERS_FILTERS.all}>
            All
          </Select.Option>
          <Select.Option key={USERS_FILTERS.blocked} value={USERS_FILTERS.blocked}>
            Blocked
          </Select.Option>
          <Select.Option
          ...
          {(this.isAdmin || this.isUsersAdmin) && (
            <Select.Option key={USERS_FILTERS.online} value={USERS_FILTERS.online}>
              Online
            </Select.Option>
          )}
          {(this.isAdmin || this.isUsersAdmin) && (
            <Select.Option key={USERS_FILTERS.quota} value={USERS_FILTERS.quota}>
              Exceeded quotas
            </Select.Option>
          )}
        </Select>
        {(this.isAdmin || this.isUsersAdmin) && (
          <Button style={{marginLeft: 5}} onClick={this.openCreateUserDialog}>
            <PlusOutlined />
            Create user
          </Button>
        )}
        {(this.isAdmin || this.isUsersAdmin) && (
          <ImportUsersButton style={{marginLeft: 5}} onImportDone={this.onImportDone} />
        )}
        {(this.isReader || this.isAdmin || this.isUsersAdmin) && (
          <Space.Compact style={{marginLeft: 5}}>
            <Button onClick={() => doExport()} icon={<DownloadOutlined />}>
              Export users
            </Button>
            <Dropdown
              menu={{
      ...
      const tagRenderer = (tag, index) => (
        <span
          ...
          className={classNames(
            tagClassName,
            `tag-${tag.displayName.replace(/\s/g, '-')}`,
            'cp-tag',
            {
        ...
        return (
          <Row type="flex">
            {[
              ...
              <Dropdown
                ...
                popupRender={() => (
                  ...
                  >
                    {tags.map((tag, index) => (
                      <Row
                        className={classNames(`tag-${index}`, {
              ...
              >
                <a id="more-info-link" className={styles.moreInfoLabel}>
        ...
        render: (name, user) => {
          ...
          const offlineInfo = (
            ...
            >
              <p>Offline</p>
              {user.lastLoginDate && (
                <p>Last visited: {displayDate(user.lastLoginDate, 'D MMMM YYYY, HH:mm')}</p>
              )}
            </div>
          );
          const onlineInfo = (
          ...
          if (user.blocked) {
            blockedSpan = <span style={{fontStyle: 'italic', marginLeft: 5}}>- blocked</span>;
          }
          ...
          if (user.attributes) {
            ...
            return (
              <Row type="flex" style={{flexDirection: 'column'}}>
                ...
                <Row>
                  <span style={{fontSize: 'smaller'}} className={styles.lineBreak}>
                    {attributesString}
                  </span>
  ...
  openExportUserDialog = () => {
    ...
    this.props.userMetadataKeys.fetch().then(() => {
      hide();
      this.setState({
  ...
  render() {
    ...
    if (!this.props.users.loaded && this.props.users.pending) {
      return null;
    }
    if (!this.isReader && !this.isAdmin && !this.isUsersAdmin && !this.userHasReadPermissions) {
      return <Alert type="error" title="Access is denied" />;
    }
    const {exportUserDialogVisible, userDataToExport, metadataKeys} = this.state;
    return (
