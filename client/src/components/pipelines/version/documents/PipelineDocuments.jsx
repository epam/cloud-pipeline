...
import {Alert, Input, Row, Button, Table, message, Modal} from 'antd';
...
import styles from './PipelineDocuments.module.css';
...
export default class PipelineDocuments extends Component {
  ...
  updateDocuments = (force = false) => {
    const {pipelineId, version, pipelines} = this.props;
      ...
      this.setState(
        ...
        async () => {
          ...
          try {
            ...
            commitState(state, () => this.setDefaultFile(getDefaultDocument(this.state.documents)));
          }
        },
      );
    } else {
  ...
  updateAndNavigateToNewVersion = async () => {
    const {pipeline, pipelineVersions, onReloadTree} = this.props;
    await Promise.all([pipeline.fetch(), pipelineVersions.fetch()]);
  ...
  navigateToNewVersion = () => {
    ...
    routing.push(`/${value?.id}/${value?.currentVersion?.name}/documents`);
  };
  ...
  setDefaultFile = (defaultFile) => {
    ...
    const {pipelineId, version} = this.props;
    this.setState(
      {
        defaultFile,
        editMode: false,
      ...
      async () => {
        ...
        try {
          if (defaultFile) {
            const request = new VersionFile(pipelineId, defaultFile.path, version);
            await request.fetch();
            if (request.error) {
              throw new Error(request.error);
            }
  ...
  toggleEditMode = (mode) => {
    const editMode = mode !== undefined ? mode : !this.state.editMode;
    this.setState({editMode});
  };

  onSaveDefaultFile = () => {
    if (this.state.defaultFileModifiedContent !== this.state.defaultFileContent) {
  ...
  saveEditableFile = async (comment) => {
    const {pipelineId, pipeline, pipelineVersions, onReloadTree} = this.props;
    if (!this.state.defaultFile || !pipeline || !pipeline.value || !pipeline.value.currentVersion) {
      return;
    }
    ...
    return (
      <span className={styles.rowActionButtonsContainer}>
        {this.canModifySources && (
          <span>
            <Button size="small" danger onClick={(e) => this.deleteFileConfirm(file, e)}>
              <DeleteOutlined />
              Delete
            </Button>
            <Button size="small" onClick={(e) => this.openRenameFileDialog(file, e)}>
              <EditOutlined />
              Rename
            </Button>
        ...
        <Button onClick={(e) => this.downloadPipelineFile(file, e)} size="small">
          <DownloadOutlined />
          Download
        </Button>
        {docx && (
          <Button disabled={!graphReady} onClick={() => this.generateDocument(file)} size="small">
            <FileTextOutlined />
            Generate
          </Button>
        )}
    ...
    const columns = [
      {
        ...
        title: 'Name',
        render: (name) => (
          <span className={classNames(styles.documentName, 'cp-primary')}>{name}</span>
        ),
      },
      {
        key: 'actions',
  ...
  generateDocument = async (file) => {
    ...
    try {
      ...
      const res = pipelineGenerateFile.response;
      if (res.type?.includes('application/json') && res instanceof Blob) {
        this.checkForBlobErrors(res).then((error) =>
          error ? message.error('Error generating document', 5) : downloadBlob(res, file.name),
        );
      } else if (res) {
  ...
  renderMarkdownControls = () => {
    ...
    if (this.state.editMode) {
      buttons.push(
        <Button key="cancel" size="small" onClick={() => this.toggleEditMode(false)}>
          CANCEL
        </Button>,
      );
      buttons.push(
        <Button key="save" size="small" type="primary" onClick={this.onSaveDefaultFile}>
          SAVE
        </Button>,
      );
    } else {
      buttons.push(
        <Button key="edit" size="small" onClick={() => this.toggleEditMode(true)}>
          EDIT
        </Button>,
      );
    }
    return <Row className={styles.mdActions}>{buttons}</Row>;
  };
  ...
  renderMarkdown = () => {
    ...
    if (defaultFileError) {
      return <Alert title={defaultFileError} type="error" />;
    }
    if (editMode) {
      return (
        <Input.TextArea
    ...
    if (defaultFileContent && defaultFileContent.trim().length) {
      return <Markdown className={styles.markdown} md={defaultFileContent} />;
    }
    return (
      <span className={classNames(styles.noMdContent, 'cp-text-not-important')}>No content</span>
    );
  };

  render() {
    const {pending, error, defaultFile} = this.state;
    const {pipelineId, version} = this.props;
    if (pending) {
      return <LoadingView />;
    }
    ...
    if (defaultFile) {
      return (
        <div style={{overflowY: 'auto'}}>
          ...
          >
            <span className={styles.mdTitle}>{defaultFile.name}</span>
  ...
  redirectBitBucketPipelineToCode() {
    ...
    if (pipeline && pipeline.loaded) {
      ...
      if (/^bitbucket$/i.test(repositoryType)) {
        const {router, pipelineId, version} = this.props;
