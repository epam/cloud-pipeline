...
import styles from './imported-jobs.module.css';
...
export default class JobList extends React.Component {
  ...
  setDefaultFilter = () => {
    this.onFiltersChange({
      timestampFrom: dayjs.utc().subtract(1, 'day').format(DATE_FORMAT),
      status: 'IN_PROGRESS',
    });
  };
  ...
  renderHeader = () => {
    ...
    const {timestampFrom, timestampTo, status} = filters;

    const onFieldChanged =
      (field, converter = (o) => o, eventField = 'value') =>
      (event) => {
        let value = event;
      ...
      <div className={styles.jobFilters}>
        <Filter label="From">
          <DatePicker
            ...
            placeholder="From"
            style={commonStyle}
            value={parseStoredDate(timestampFrom, DATE_FORMAT)}
            onChange={(date) =>
              onFieldChanged('timestampFrom', (d) => formatStoredDate(d, DATE_FORMAT))(date)
            }
            disabledDate={getDisabledDate({max: parseStoredDate(timestampTo, DATE_FORMAT)})}
          />
        </Filter>
        <Filter label="To">
          <DatePicker
            ...
            placeholder="To"
            style={commonStyle}
            value={parseStoredDate(timestampTo, DATE_FORMAT)}
            onChange={(date) =>
              onFieldChanged('timestampTo', (d) => formatStoredDate(d, DATE_FORMAT))(date)
            }
            disabledDate={getDisabledDate({min: parseStoredDate(timestampFrom, DATE_FORMAT)})}
          />
        </Filter>
        <Filter label="Status">
          <Select
