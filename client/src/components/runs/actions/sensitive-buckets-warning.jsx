...
function getSensitiveBuckets(parameters, dataStoragesStore) {
  if (!parameters || !Object.hasOwn(parameters, CP_CAP_LIMIT_MOUNTS) || !dataStoragesStore) {
    return [];
  }
  ...
  return selection.filter((storage) => storage.sensitive);
}

function SensitiveBucketsWarning({dataStorages, parameters, message, showIcon = true, style = {}}) {
    ...
    <Alert
      ...
      title={
        message || (
          <div>
            <div>
              You are going to launch a job with <b>sensitive storages</b>.
            </div>
            <div>
              This will apply a number of restrictions for the job: no Internet access, all the
              storages will be available in a read-only mode, you won't be able to extract the data
              from the running job and other.
            </div>
          </div>
        )
      }
