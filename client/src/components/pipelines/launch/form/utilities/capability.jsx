    ...
    return (
      <Tooltip
        title={
          <div>
            ...
            {os.length > 0 && <div>Supported OS versions:</div>}
            {os.length > 0 && (
            ...
            {cloud.length > 0 && <div>Supported Cloud Providers:</div>}
            {cloud.length > 0 && (
              <ul style={{listStyle: 'disc inside'}}>
                {cloud.map((o, i) => (
                  <li key={`${o}-${i}`}>{renderCloudRestriction(o, awsRegions)}</li>
                ))}
              </ul>
            )}
          </div>
      ...
      >
        ...
        >
          ...
          <div>
            {nested.length > 0 && <EllipsisOutlined />}
