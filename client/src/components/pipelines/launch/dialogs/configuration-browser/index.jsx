...
import {generateTreeData, ItemTypes, getTreeItemByKey} from '../../../model/treeStructureFunctions';
...
function findFirstConfiguration(items = []) {
  ...
  const folders = items.filter(
    (o) => o.type === ItemTypes.folder && o.children !== undefined && o.children.length,
  );
...
function getSelectionFromConfiguration(config) {
  ...
  const defaultEntry = filteredEntries.find((o) => o.default) || filteredEntries[0];
  return {
...
class ConfigurationBrowser extends React.Component {
  ...
  get selectedConfigurationID() {
    ...
    if (selectedConfiguration) {
      const {id} = selectedConfiguration;
  ...
  fetchFolderStructure = () => {
    const {folderId, folders, hiddenObjectsTreeFilter, metadataClassName} = this.props;
    if (!folderId || !folders) {
      this.setState({
    ...
    this.setState(
      ...
      async () => {
        ...
        try {
          ...
          const folderStructure = generateTreeData(request.value || {}, {
          ...
          if (!config) {
            if (metadataClassName) {
              throw new Error(`Configurations with "${metadataClassName}" root entity not found`);
            }
            throw new Error('Configurations not found');
          }
          const {selection = [], entryName} = getSelectionFromConfiguration(config);
          newState.selection = selection;
          newState.entryName = entryName;
          newState.folderStructure = folderStructure;
          newState.metadataClassFields = metadataClass
            ? metadataClass.fields.map((o) => o.name)
            : [];
        } catch (e) {
  ...
  renderModalContent = () => {
    ...
    return (
      <Splitter>
        <Splitter.Panel defaultSize="200px" min={100} className={styles.treePanel}>
          {this.renderConfigurationsTree()}
        </Splitter.Panel>
        <Splitter.Panel min={200} className={styles.panel} style={{overflow: 'auto'}}>
  ...
  renderConfigurationsTree = () => {
    ...
    const renderTreeItem = (item) => {
      ...
      return (
        <span id={`configurations-library-tree-node-${item.key}-name`}>
    ...
    const getTreeData = (items) => {
      return (items || []).map((item) => ({
        ...
        isLeaf: item.isLeaf,
        className: classNames(`configurations-library-tree-node-${item.key}`, styles.treeItem),
    ...
    const onSelect = (keys = []) => {
      ...
      const {selection: newSelection = [], entryName} =
  ...
  renderConfigurationHeader() {
    ...
    const {entryName} = this.state;
    const {name: configName} = configuration;
    const onChangeConfigName = ({key}) =>
      this.setState({
        entryName: key,
      });
    ...
    if (entries.length > 1) {
      configNameComponent = (
        <Dropdown menu={{items: menuItems, onClick: onChangeConfigName}} trigger={['click']}>
          <a className="cp-text">
            {entryName}
            <DownOutlined />
          </a>
  ...
  renderExpression = () => {
    const {expression, metadataClassFields} = this.state;
    const disabled = !this.selectedConfiguration;
    const onChange = (newExpression) =>
      this.setState(
        {
    ...
    return (
      <ParameterRow
        className={classNames(styles.row, 'cp-divider', 'top')}
        noPadding
        style={{
          marginTop: 5,
          paddingTop: 10,
  ...
  onSelect = () => {
    const {configurationPayload, expression} = this.state;
  ...
  render() {
    const {className, visible, onCancel} = this.props;
    const {configurationPayload, valid} = this.state;
    return (
      <Modal
        className={className}
        open={visible}
...
export default inject('folders')(HiddenObjects.injectTreeFilter(ConfigurationBrowser));
