...
import {Button, Collapse, Checkbox, Modal} from 'antd';
import {DocumentColumns} from '../../utilities';
import styles from './export.module.css';
import {defaultSorter} from '../../../../../utils/sorting';

function columnsAreEqual(a, b) {
  const aa = [...new Set((a || []).map((column) => column.key))].sort(defaultSorter);
  const bb = [...new Set((b || []).map((column) => column.key))].sort(defaultSorter);
...
class ExportConfigurationModal extends React.Component {
  ...
  changeColumnSelection = (columnKey, selected) => {
    ...
    if (columnIsSelected) {
      this.setState({
        configuration: [...configuration.filter((key) => key !== columnKey), columnKey],
      });
    } else {
      this.setState({
        configuration: [...configuration.filter((key) => key !== columnKey)],
      });
    }
  };
  ...
  renderCollapseHeader = (title, group) => {
    const allGroupSelected = !this.groupedColumns[group].some(
      (column) => !this.columnSelected(column.key),
    );
    const hasSelection = this.groupedColumns[group].some((column) =>
      this.columnSelected(column.key),
    );
    return (
      <div>
        <Checkbox
  ...
  render() {
    ...
    const {configuration = [], expanded = ['main']} = this.state;
    const allSelected =
      columns.length > 0 && !columns.some((column) => !this.columnSelected(column.key));
