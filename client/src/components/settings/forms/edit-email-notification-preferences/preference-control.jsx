...
function processExcludedParameters(excludedParametersObj) {
  ...
  return Object.entries(obj || {}).map(([parameterName, parameterObj]) => ({
    name: parameterName,
...
class PreferenceControl extends React.Component {
        ...
        request
          ...
          .then(() => {
            if (request.loaded) {
              ...
              const isExcludedParameters =
                preferenceName && preferenceName === 'system.notifications.exclude.params';
              if (preference) {
                this.setState(
                  {
                    error: undefined,
  ...
  renderHint = (preference) => {
    ...
    return (
      <Popover content={preference.hint} placement="left">
        <QuestionCircleFilled
          style={{marginLeft: 5, marginRight: 10, fontSize: 'larger', cursor: 'pointer'}}
        />
      </Popover>
    );
  };
  ...
  renderStringControl = (preference) => {
    ...
    return (
      <div className={styles.controlRow}>
        <span className={styles.label}>{preference.name}</span>
        <Input
          disabled={pending}
  ...
  renderExcludeParamsControl = (preference) => {
    ...
    const {value, pending} = this.state;
    return (
      <div className={styles.controlRow} style={{alignItems: 'baseline'}}>
        <span className={styles.label}>{preference.name}</span>
  ...
  render() {
    ...
    if (preference) {
      switch (preference.type) {
        case 'number':
          control = this.renderNumberControl(preference);
          break;
        case 'enum':
          control = this.renderEnumControl(preference);
          break;
        case 'string':
          control = this.renderStringControl(preference);
          break;
    ...
    return (
      <div>
        <div>{control}</div>
        {error && <div className={classNames(styles.error, 'cp-error')}>{error}</div>}
      </div>
    );
  }
}
