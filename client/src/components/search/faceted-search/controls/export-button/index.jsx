...
import {Dropdown, message, Button, Modal, Space} from 'antd';
...
function checkTemplatePermissions(userInfo, template) {
  ...
  return template.permissions.some((permission) => {
    ...
    return (
      roleModel.userHasRole(userInfo, (permission.name || '').toUpperCase()) ||
      groups.includes(permission.nzme)
    );
  });
}

function getExportMenuItems({templates, selectedItems = []}) {
...
class ExportButton extends React.Component {
  ...
  getFacetedSearchExportPayload = (configuration) => {
    ...
    return {
      query: !advanced && currentQuery ? `*${currentQuery}*` : currentQuery || '*',
      filters,
  ...
  onExport = async (configuration = []) => {
    ...
    try {
      const keys = columnsToExport.map((column) => column.key);
      const facetedSearchExportVO = Object.entries(exportVOColumns).reduce(
        (acc, [key, exportVOKey]) => ({
          ...acc,
      ...
      if (request.value instanceof Blob) {
        ...
        downloadBlob(request.value, csvFileName);
      } else {
  ...
  onExportTemplate = async (template, payload) => {
    const downloadExport = async (template) => {
      ...
      if (request.value instanceof Blob) {
        ...
        downloadBlob(request.value, fileName);
      }
    };
  ...
  renderSavedExportContent = () => {
    ...
    return (
      <div style={{paddingRight: 25}}>
        Data exported to
        <Link style={{marginLeft: 5}} to={`/storage/${savedExport.storageId}?path=${folder}`}>
  ...
  render() {
    const {columns, className, style, size, type} = this.props;
    const {modalVisible, dropdownVisible} = this.state;
    const templates = parseExportTemplates(this.exportTemplates).filter((template) => {
    ...
    return (
      <>
        <Space.Compact className={className} style={style}>
          ...
          >
            <Button size={size} type={type} icon={<DownOutlined />} />
          </Dropdown>
        </Space.Compact>
        ...
        <Modal
          ...
          footer={
            <Button type="primary" onClick={this.closeExportResultModal}>
              OK
            </Button>
          }
        >
