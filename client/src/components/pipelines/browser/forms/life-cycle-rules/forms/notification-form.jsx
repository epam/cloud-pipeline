...
import {Button, Input, Form, Row, Col, Checkbox, Space} from 'antd';
...
class NotificationForm extends React.Component {
  ...
  fetchEmailSettings = () => {
    this.setState({pending: true}, async () => {
      ...
      this.setState({pending: false}, () => {
        if (!request.error) {
          this.systemTemplate = (request.value || []).find(
  ...
  onChangeUseDefaultNotify = (event) => {
    ...
    if (onChangeUseDefaultNotify) {
      onChangeUseDefaultNotify(event.target.checked);
    }
  };

  renderNotificationTemplate = () => {
    const {form, notificationsDisabled, useDefaultNotify} = this.props;
  ...
  render() {
    const {notificationsDisabled, useDefaultNotify} = this.props;
    const {pending} = this.state;
    return (
      <div
        style={{width: '100%'}}
        ref={(el) => {
          this.notifyFormContainer = el;
        }}
