...
import styles from './PipelineCode.module.css';
...
export default class PipelineCode extends Component {
  ...
  renameFolder = async ({name, comment}) => {
    const {renameFolder} = this.state;
    if (!renameFolder) {
      this.closeRenameFolderDialog();
      return;
    }
    const {path: folderPreviousFullName} = renameFolder;
    const parentPath = (folderPreviousFullName || '').split('/').slice(0, -1).join('/');
  ...
  renameFile = async ({name, comment}) => {
    ...
    const {path: filePreviousFullName} = renameFile;
    const parentPath = (filePreviousFullName || '').split('/').slice(0, -1).join('/');
  ...
  renderNavigationControl = () => {
    ...
    return <Breadcrumb className={styles.navigationItem} items={items} />;
  };
  ...
  navigateToNewVersion = () => {
    const {currentVersion = {}} = this.props.pipeline.value || {};
          ...
          {this.canModifySources && (
            <Row type="flex" justify="end" className={styles.actionButtonsContainer}>
              ...
              <UploadButton
                ...
                title={'Upload'}
                action={PipelineFileUpdate.uploadUrl(
