...
import {Checkbox, Row, Table, Button} from 'antd';

import roleModel from '../../../../../utils/roleModel';
import UsersRolesSelect from '../../../../special/users-roles-select';
import styles from './DataStorageItemPermissionsForm.module.css';
...
class DataStorageItemPermissionsForm extends React.Component {
  ...
  onAllowDenyValueChanged =
    ...
    async (event) => {
      ...
      const {mask: currentMask} = this.state;
  ...
  renderUserPermission = () => {
    const {mask = 0, sids = []} = this.state;
    const {readonly, writeAvailable} = this.props;
    if (sids.length > 0) {
      const columns = [
        {
          title: 'Permissions',
  ...
  render() {
    return (
      <div>
        ...
        <Row type="flex" justify="space-between" style={{marginTop: '10px', paddingRight: 6}}>
