...
import {Button, Pagination, Modal, message, Select, Spin} from 'antd';
...
import styles from './NotificationBrowser.module.css';
...
function dateSorter(a, b) {
  const dateA = toUtcDayjs(a.createdDate);
  const dateB = toUtcDayjs(b.createdDate);
  if (dateA === dateB) {
    return 0;
  } else if (dateA > dateB) {
    return -1;
...
export default class NotificationBrowser extends React.Component {
  ...
  fetchPage = (page, isRead) => {
    this.setState({pending: true}, async () => {
      const request = new NotificationsRequest(page, PAGE_SIZE, isRead);
      await request.fetch();
      if (request.error) {
        message.error(request.error, 5);
      }
  ...
  renderHeader = () => {
    return (
      ...
      >
        <div
          className={classNames(styles.notificationCell, styles.notificationStatus, styles.header)}
        >
          Status
        </div>
        <div
          className={classNames(styles.notificationCell, styles.notificationTitle, styles.header)}
        >
          Title
        </div>
        <div
          className={classNames(styles.notificationCell, styles.notificationBody, styles.header)}
        >
          Text
        </div>
        <div
          className={classNames(styles.notificationCell, styles.notificationDate, styles.header)}
        >
          Created date
        </div>
        <div
        ...
        <div
          className={classNames(styles.notificationCell, styles.notificationActions, styles.header)}
        />
      </div>
    );
  };
  ...
  renderNotifications = () => {
    ...
    const emptyPlaceholder = (
      <div className={styles.emptyPlaceholder}>
        {MODES[mode] === MODES.new ? 'Notifications not found' : 'No new notifications'}
      </div>
    );
    return (
          ...
          <Spin spinning={pending}>
            {this.notifications.length > 0
              ? this.notifications.map((notification) => (
                  <div
                    className={classNames(styles.notificationGridRow, 'cp-divider', 'bottom', {
                  ...
                  >
                    <div className={classNames(styles.notificationCell, styles.notificationStatus)}>
                      <MailOutlined
                        className={notification.isRead ? 'cp-disabled' : 'cp-setting-message'}
                      />
                    </div>
                    <b className={classNames(styles.notificationCell, styles.notificationTitle)}>
                      {notification.subject}
                    </b>
                    <div className={classNames(styles.notificationCell, styles.notificationBody)}>
                      <PreviewNotification
                    ...
                    <div className={classNames(styles.notificationCell, styles.notificationDate)}>
                      {displayDate(notification.createdDate, 'YYYY-MM-DD HH:mm:ss')}
                    </div>
                    <div
                      className={classNames(styles.notificationCell, styles.notificationReadDate)}
                    >
                      {displayDate(notification.readDate, 'YYYY-MM-DD HH:mm:ss')}
                    </div>
                    <div
                      className={classNames(styles.notificationCell, styles.notificationActions)}
  ...
  render() {
    const {currentPage, mode, previewNotification, pending} = this.state;
    return (
      <div
        className={classNames(
          styles.container,
      ...
      >
        <div className={styles.header}>
          <div>
            <b className={styles.title}>Notifications list</b>
            <Select
              value={mode}
              style={{width: 130}}
        ...
        {previewNotification ? (
          <Modal
            ...
            open
          >
            <PreviewNotification text={previewNotification.text} sanitize />
          </Modal>
        ) : null}
      </div>
    );
