...
- [Runs on unavailable nodes](#runs-on-unavailable-nodes)
...
| **`limit.network.bandwidth.command.timeout`** | Specifies the duration in seconds.<br>Till this duration platform will try to perform the limit network bandwidth command in case when admin sets the network limit boundary manually via API - see details [below](#limit-pods-network-bandwidth). |
...
## Runs on unavailable nodes

When the node of a run stops reporting its state, the run is marked by the **`NODE_UNAVAILABLE`** label, at
the **Runs** page and at the **Run logs** page, and the `NodeAvailabilityMonitor` task appears in its logs.
If the node does not come back within the grace period, set by the
**`cluster.node.unavailable.grace.period.minutes`** preference, the node is scaled down and the run fails.
See [11.6. Node availability monitoring](11.6._Node_availability_monitoring.md) for details.
