...
import {Alert, Row, Button, Card, Col, message} from 'antd';
...
class Tools extends React.Component {
  ...
  get isPrivate() {
    return (
      this.props.groupId === PERSONAL_GROUP_ID ||
  ...
  get registries() {
    if (this.props.dockerRegistries.loaded) {
      return this.props.hiddenToolsTreeFilter(this.props.dockerRegistries.value).registries;
    }
    return [];
  }
  ...
  get allTools() {
    return (this.groups || []).reduce((acc, current) => {
  ...
  get currentGroup() {
    if (this.filter) {
      ...
      if (this.filter === TOP_USED_FILTER.id) {
        ...
        const currentFilter = (this.filters.groups || []).find((group) => group.id === this.filter);
    ...
    if (this.currentGroup) {
      const checkIssues = (tool = {}) => {
        if (this._toolsWithIssues && this._toolsWithIssues.loaded) {
          const toolWithIssues = (this._toolsWithIssues.value.toolsWithIssues || []).filter(
            (t) => t.id === tool.id,
          )[0];
          if (toolWithIssues && Object.hasOwn(toolWithIssues, 'issuesCount')) {
      ...
      return (this.currentGroup.tools || [])
        ...
        .filter(
          (t) =>
            ...
            (t.labels &&
              t.labels.filter((l) => l.toLowerCase().indexOf(this.state.search.toLowerCase()) >= 0)
                .length > 0),
        )
        .map(checkIssues);
    }
  ...
  fetchCompletedRuns = async () => {
    ...
    const request = new PipelineRunFilter({
      ...
      pageSize: 50,
      userModified: false,
      statuses: ['SUCCESS', 'FAILURE', 'RUNNING', 'STOPPED', 'PAUSING', 'PAUSED', 'RESUMING'],
      ...myRunsSubFilter,
    });
        ...
        <ContentIssuesMetadataPanel style={{flex: 1, overflow: 'auto'}} onPanelClose={onPanelClose}>
          ...
          {this.state.metadata && this.currentGroup && !this.currentGroup.missing && (
            <Metadata
              ...
              entityId={this.currentGroup ? this.currentGroup.id : undefined}
  ...
  defaultGroupLinkMessage = () => {
    if (this.defaultGroup) {
      ...
      return <span>You can explore {link} repository</span>;
    }
    return null;
  };
  ...
  renderCreatePrivateToolBody = () => {
    const navigateToDefaultGroupMessage = () => {
      ...
      if (message) {
        return <Row style={{fontSize: 'larger', marginBottom: 10}}>{message}</Row>;
      }
      return null;
    };
    return (
  ...
  render() {
    ...
    if (this.props.dockerRegistries.error) {
      content = (
        <Alert style={{width: '100%'}} type="error" title={this.props.dockerRegistries.error} />
      );
      isError = true;
    } else if (this.registries.length === 0) {
      content = <Alert style={{width: '100%'}} type="warning" title="No registries configured" />;
      isError = true;
    } else if (this.currentRegistry && this.groups.length === 0) {
      content = <Alert style={{width: '100%'}} type="warning" title="No groups configured" />;
      isError = true;
      ...
      (this.filter && this._completedRunsRequest && this.filter === TOP_USED_FILTER.id
      ...
      const noToolsMessage = () => {
        ...
        if (this.isPrivate && defaultGroupLink) {
          return <span>No tools found. {defaultGroupLink}</span>;
        }
        return 'No tools found';
      };
      content = <Alert style={{width: '100%'}} type="info" title={noToolsMessage()} />;
      isError = true;
    } else if (
      this.filter &&
  ...
  getDefaultGroup = (groups, usePersonal = false) => {
    const {authenticatedUserInfo} = this.props;
    if (this.filters && this.filters.groups && this.filters.groups.length > 0) {
    ...
    if (authenticatedUserInfo && authenticatedUserInfo.loaded) {
      ...
      const rolesGroups = (authenticatedUserInfo.value.roles || []).map((r) =>
        performGroupName(r.name),
      );
    ...
    const personalGroup = usePersonal
      ...
      : null;
    return (
      findGroupByName(groups, 'library') ||
  ...
  redirectIfNeeded = () => {
    ...
    if (
      ...
      if (!this.currentRegistry) {
        const registryToRedirect =
          this.props.registryId || (this.registries[0] ? this.registries[0].id : undefined);
  ...
  componentDidMount() {
    ...
    const {dockerRegistries, preferences} = this.props;
    Promise.all([dockerRegistries.fetch(), preferences.fetchIfNeededOrWait()])
      .then(() => this.redirectIfNeeded())
      .catch(() => {});
    if (this.filter && this.filter === 'top-used' && !this._completedRunsRequest) {
  ...
  componentDidUpdate(prevProps) {
    ...
    if (
      prevProps.registryId !== this.props.registryId ||
  ...
  UNSAFE_componentWillReceiveProps(nextProps) {
    if (
      nextProps.registryId !== this.props.registryId ||
