...
from ..utils.aws.utilities import *
...
class TestCopyWithFiles(object):
    ...
    def test_file_should_be_uploaded(self, case, source, relative_path, add_file, switch_dir, force):
        ...
        if not force and s3_object_exists(self.bucket_name, path_to_check):
    ...
    def test_file_should_be_downloaded(self, case, relative_path, destination, add_file, force):
        ...
        if not s3_object_exists(self.bucket_name, "{}/{}".format(case, relative_path)):
    ...
    def test_file_should_be_copied(self, case, relative_path, add_file, force):
        ...
        if force:
            create_test_files_on_bucket(os.path.abspath(force), self.other_bucket_name,
                                        os.path.join(case, relative_path))
        if not s3_object_exists(self.bucket_name, path_to_check):
            pytest.fail("Object {} to copy does not exist!".format(source))
        if not force and s3_object_exists(self.other_bucket_name, path_to_check):
            pytest.fail("Object {} already exists!".format(destination))
        logging.info("Ready to perform operation from {} to {}".format(source, destination))
        pipe_storage_cp(source, destination, force=bool(force))
    ...
    def test_copy_with_wrong_scheme(self, source, destination):
        ...
        try:
            ...
            assert_error_message_is_present(error, 'Error: Supported schemes for datastorage are: "cp", "s3".')
        except AssertionError as e:
    ...
    def test_copy_with_local_paths(self, source, destination):
        try:
            ...
            assert_error_message_is_present(error, 'Error: Transferring files between local paths is not supported.')
        except AssertionError as e:
    ...
    def test_copy_with_similar_keys(self):
        try:
            ...
            assert s3_object_exists(self.bucket_name, file_name)
            pipe_storage_cp(source, destination_without_extension)
            assert s3_object_exists(self.bucket_name, file_name_without_extension)
            pipe_storage_rm(destination_without_extension)
            assert not s3_object_exists(self.bucket_name, file_name_without_extension)
            assert s3_object_exists(self.bucket_name, file_name)
            pipe_storage_rm(destination)
            assert not s3_object_exists(self.bucket_name, file_name)
        except AssertionError as e:
    ...
    def test_copy_with_tags(self):
        ...
        try:
            ...
            assert s3_object_exists(self.bucket_name, file_name)
            actual_tags = s3_list_object_tags(self.bucket_name, file_name)
    ...
    def test_download_data_with_similar_keys(self):
        ...
        try:
            ...
            assert s3_object_exists(self.bucket_name, "%s/%s" % (test_case, file_name1))
            ...
            assert s3_object_exists(self.bucket_name, "%s/%s" % (test_case, file_name2))
    ...
    def test_upload_files_with_similar_keys(self):
        ...
        try:
            ...
            pipe_storage_cp(source, "s3://%s/%s/" % (self.bucket_name, case))
            assert s3_object_exists(self.bucket_name, os.path.join(case, "cp-files-test_file"))
    ...
    def test_upload_file_with_skip_existing_option_should_skip(self):
        ...
        try:
            ...
            pipe_storage_cp(source, "s3://%s/%s" % (self.bucket_name, key), skip_existing=True, force=True)
    ...
    def test_download_file_with_skip_existing_option_should_skip(self):
        ...
        source = "s3://%s/%s" % (self.bucket_name, key)
        try:
    ...
    def test_copy_between_buckets_file_with_skip_existing_option_should_skip(self):
        ...
        key = os.path.join(case, self.test_file_1)
        destination = "s3://%s/%s" % (self.other_bucket_name, key)
        source = "s3://%s/%s" % (self.bucket_name, key)
        try:
            pipe_storage_cp(os.path.abspath(self.test_file_1), source)
            assert s3_object_exists(self.bucket_name, key)
            ...
            pipe_storage_cp(source, destination, skip_existing=True, force=True)
            assert s3_object_exists(self.other_bucket_name, key)
    ...
    def upload_file_to_bucket(self, source):
        if source.startswith("cp://") and not s3_object_exists(self.bucket_name, self.test_file_1):
