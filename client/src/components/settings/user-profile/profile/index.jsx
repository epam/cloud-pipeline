...
import {METADATA_KEY as LIMIT_MOUNTS_USER_PREFERENCE} from '../../../special/metadata/special/limit-mounts';
import {METADATA_KEY as RUN_CAPABILITIES} from '../../../pipelines/launch/form/utilities/run-capabilities';
...
class ProfileSettings extends React.Component {
  ...
  render() {
    ...
    if (authenticatedUserInfo.error) {
      return <Alert type="error" title={authenticatedUserInfo.error} />;
    }
      ...
      <div className={styles.container}>
        ...
        <table className={styles.attributes}>
          <tbody>
            ...
            {userInfo.groups && userInfo.groups.length > 0 && (
              <tr>
                <td className={styles.info}>Groups:</td>
                <td className={styles.infoValue}>
                  <div className={styles.roles}>
                    {userInfo.groups.map((group) => (
                      <span key={group} className={styles.role}>
                        {group}
                      </span>
                    ))}
                  </div>
        ...
        <Metadata
          ...
          extraKeys={[
            ...
            this.notificationsEnabled ? MuteEmailNotifications.metadataKey : undefined,
