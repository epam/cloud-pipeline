...
class QuotasSection extends React.Component {
  ...
  addNewQuota = () => {
    const {quotaType, quotaGroup} = this.props;
    this.setState({
      editableQuota: {
        quotaGroup,
        type: quotaType,
  ...
  onEditQuota = async (quota) => {
    ...
    const {quotas = []} = this.props;
    const hasSameQuota = !!quotas.find(
      (q) =>
        q.id !== quota.id &&
        q.type === quota.type &&
  ...
  render() {
    const {disabled, title, quotas = [], roles} = this.props;
    const {editableQuota, inProgress} = this.state;
    return (
      <div className={classNames(styles.sectionContainer, 'cp-divider', 'bottom')}>
        <div className={styles.section}>
          ...
          {quotas.length === 0 && (
            <div className="cp-text-not-important">No quotas configured.</div>
          )}
          <div>
            {quotas.sort(subjectSorter).map((quota) => (
              <QuotaDescription
                key={`quota-${quota.id}`}
                className={classNames(styles.quota, 'cp-table-element', 'cp-even-odd-element')}
