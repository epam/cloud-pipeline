                ...
                {this.toolVersionScanResults.map((version, index) => {
                  return (
                    <tr key={index}>
                      ...
                      <td style={cellStyle}>
                        {this.props.preferences.toolScanningEnabledForRegistry(
                          this.dockerRegistry,
                        ) && (
                          <VersionScanResult
  ...
  renderHeader = () => {
    const renderMainInfo = () => {
      ...
      const path = this.path && (
        <Row
          key="path"
          className={classNames(styles.subTitle, 'cp-search-header-sub-title')}
          type="flex"
          align="middle"
        >
      ...
      const description = this.description && (
        <Row
          key="description"
          className={classNames(styles.toolDescription, 'cp-search-header-description')}
        >
          {this.description}
        </Row>
      );
    ...
    if (this.props.tool && this.props.tool.loaded && this.props.tool.value.iconId) {
      return (
        <Row type="flex" className={styles.header} align="middle">
          ...
          />
          <div style={{paddingLeft: 10, flex: 1}}>{renderMainInfo()}</div>
        </Row>
      );
    }
    return <div className={styles.header}>{renderMainInfo()}</div>;
  };
