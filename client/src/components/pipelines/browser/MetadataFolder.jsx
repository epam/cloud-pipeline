...
import {Checkbox, Modal, Table, Row, Col, Button, message, Alert} from 'antd';
...
class MetadataFolder extends React.Component {
  ...
  get metadataFolderClassEntities() {
    ...
    if (folder && folder.loaded) {
      const dataFolder = generateTreeData(this.props.folder.value, {
  ...
  get listingItems() {
    return [this.parentFolderLinkItem, ...this.metadataFolderClassEntities];
  }

  get entityTypes() {
    const {entityFields: entityFieldsRequest, metadataClasses} = this.props;
    if (
      entityFieldsRequest &&
      ...
      return [...entityFields, ...otherClasses];
    }
    return [];
  }
  ...
  addInstance = async (values) => {
    const {entityFields, folder, folderId, onReloadTree} = this.props;
    ...
    if (metadataClass) {
      ...
      if (request.error) {
        ...
        if (typeof onReloadTree === 'function') {
          const {parentId} = folder.loaded ? folder.value : {};
  ...
  selectMetadataClass = (metadataClass) => () => {
    ...
    const selectedItemIndex = newSelection.findIndex(
      (s) => s.id === metadataClass.id && s.type === metadataClass.type,
    );
  ...
  renderContent = () => {
    ...
    const metadataClassIsSelected = (metadataClass) => {
      return !!selectedItems.find(
        (selected) => selected.id === metadataClass.id && selected.type === metadataClass.type,
      );
    };
  ...
  renderActions = () => {
    ...
    if (
      ...
      !this.props.hideUploadMetadataBtn
    ) {
      actions.push(
