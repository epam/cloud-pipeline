...
class TestLsFolder(object):
    ...
    @pytest.mark.run()
    @pytest.mark.parametrize("path,show_details,recursive,length,case", test_paths_without_delimiter)
    def test_list_folder_without_trailing_delimiter(self, path, show_details, recursive, length, case):
        try:
            ...
            compare_listing(pipe_files, aws_files, length, show_details=show_details)
    ...
    @pytest.mark.run()
    @pytest.mark.parametrize("show_details,recursive,length", bucket_parameters_without_delimiter)
    def test_list_bucket_without_trailing_delimiter(self, show_details, recursive, length):
        try:
            pipe_files = get_pipe_listing("cp://{}".format(self.bucket_name),
                                          show_details=show_details, recursive=recursive)
            aws_files = get_aws_listing("s3://{}".format(self.bucket_name), recursive=recursive)
            compare_listing(pipe_files, aws_files, length, show_details=show_details)
        except AssertionError as e:
    ...
    @pytest.mark.run()
    @pytest.mark.parametrize("path,show_details,recursive,length", test_paths_with_delimiter)
    def test_list_folder_with_trailing_delimiter(self, path, show_details, recursive, length):
        try:
            pipe_files = get_pipe_listing("cp://{}/{}".format(self.bucket_name, path),
                                          show_details=show_details, recursive=recursive)
            aws_files = get_aws_listing("s3://{}/{}".format(self.bucket_name, path), recursive=recursive)
    ...
    @pytest.mark.run()
    @pytest.mark.parametrize("show_details,recursive,length", bucket_parameters_with_delimiter)
    def test_list_bucket_with_trailing_delimiter(self, show_details, recursive, length):
        try:
            pipe_files = get_pipe_listing("cp://{}/".format(self.bucket_name),
                                          show_details=show_details, recursive=recursive)
            aws_files = get_aws_listing("s3://{}/".format(self.bucket_name), recursive=recursive)
    ...
    def test_list_with_wrong_scheme(self):
        try:
            ...
            assert_error_message_is_present(error, 'Supported schemes for datastorage are: "cp", "s3".')
        except AssertionError as e:
    ...
    def test_list_not_existing_path(self, path, recursive):
        try:
            pipe_files = get_pipe_listing("cp://{}/{}".format(self.bucket_name, path), recursive=recursive)
            assert len(pipe_files) == 0, \
                "Returned non empty result: '{}' for non existing path '{}'".format(str(pipe_files), path)
        except AssertionError as e:
