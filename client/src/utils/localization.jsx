...
class Localization {
  ...
  getDictionaryElement = (key, extraDictionary = []) => {
    return [...this.dictionary, ...extraDictionary].filter(
  ...
  localizedString = (key, extraDictionary = []) => {
    ...
    if (key && dictionaryElement) {
      ...
      const capitalized =
        key[0].toUpperCase() === key[0] && key.substring(1).toLowerCase() === key.substring(1);
      if (uppercased) {
...
class LocalizedReactComponent extends React.Component {
  localizedString = (key, extra) =>
    this.props.localization ? this.props.localization.localizedString(key, extra) : key;
}
