...
import styles from './SystemDictionaries.module.css';
...
class SystemDictionaries extends React.Component {
  ...
  componentWillUnmount() {
    if (this.resetChangesStateTimeout) {
      clearTimeout(this.resetChangesStateTimeout);
    }
  }
      ...
      ...this.dictionaries
        .filter(
          (dict) =>
            !filter ||
            (dict.values || []).find(
              (v) => (v.value || '').toLowerCase().indexOf(lowerCasedFilter) >= 0,
            ),
        )
        .sort(nameSorter),
    );
    return dataSource.map((dictionary) => ({
  ...
  checkModifiedBeforeLeave = (nextLocation) => {
    ...
    if (modified && !changesCanBeSkipped) {
      this.confirmChangeDictionary().then((confirmed) =>
        confirmed ? makeTransition() : undefined,
      );
      return false;
    }
  };
  ...
  render() {
    ...
    const dictionaries = this.dictionaries.slice();
    return (
      <div className={styles.container}>
        ...
        <SubSettings
          ...
          emptyDataPlaceholder={
            <span className="cp-text-not-important" style={{flex: 1, textAlign: 'center'}}>
              Nothing found
            </span>
          }
        >
