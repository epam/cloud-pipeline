...
class TimelineChart extends React.Component {
  ...
  componentDidUpdate(prevProps) {
    const {datasets, datasetOptions, from, to} = this.props;
  ...
  startHoverPositioning = () => {
    ...
    const callback = () => {
      if (this.renderer && this.hoverContainer && this.hoverPosition) {
        ...
        const {x: positionX, y: positionY} = this.hoverPosition;
        let newX;
  ...
  updateDatasetsOptions = () => {
    const {datasetOptions = []} = this.props;
  ...
  onRangeChanged = (range) => {
    ...
    if (typeof onRangeChanged === 'function') {
      const {from, to, fromZoom, fromDrag} = range;
      const {unix: fromUnix, date: fromDate} = parseDate(from) || {};

      const {unix: toUnix, date: toDate} = parseDate(to) || {};
  ...
  onHover = (hoverInfo) => {
    ...
    const {position = {}, items = []} = hoverInfo;
  ...
  renderHoveredItemsContent = () => {
    ...
    const {hover: hoverConfig} = this.props;
    const getColorForDataset = (dataset) => {
      if (this.renderer) {
        const {color} = this.renderer.getDatasetOptions(dataset) || {};
        return color;
      }
      return undefined;
    };
    ...
    return getHoveredElementsInfo(hoveredItems, cssStyles);
  };

  renderHoveredInfo = () => {
    const {hover, hoverContainerClassName} = this.props;
    if (!hover) {
      return null;
    }
    ...
    return (
      <div
        key="hover-container"
        className={classNames(styles.hoverContainer, hoverContainerClassName, {
  ...
  render() {
    ...
    return (
      <div className={classNames(className, styles.timelineChartContainer)} style={style}>
        <div key="renderer" className={styles.renderer} ref={this.initializeContainer} />
