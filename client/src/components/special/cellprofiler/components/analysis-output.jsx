...
class AnalysisOutputWithDownload extends React.Component {
  ...
  updateUrl = () => {
    const {storageId, path, downloadPath} = this.props;
    this.setState(
      {
        pending: true,
        error: undefined,
  ...
  updateAnalysisUrl = () => {
    const {analysisPath, analysisStorageId} = this.props;
    this.setState(
      {
  ...
  handleDownload = async (e) => {
    e.preventDefault();
    const {downloadUrl, downloadPath} = this.state;
    if (!downloadUrl) {
      return null;
    }
    const {input = 'analysis', analysisDate, analysisName, storageId} = this.props;
  ...
  handleDownloadAnalysisFile = async (e) => {
    e.preventDefault();
    const {analysisUrl} = this.state;
    if (!analysisUrl) {
      return null;
    }
    const {input = 'analysis', analysisName, analysisStorageId, analysisPath} = this.props;
    const hcsFileName = input
      .split(/[\\/]/)
      .pop()
      .split('.')
      ...
      .join('.')
      .replace(/\s/g, '_');
    let fileName = [hcsFileName, analysisName ? analysisName.replace(/\s/g, '_') : undefined]
      .filter(Boolean)
      .join('_');
  ...
  renderHeader() {
    const {url, pending, analysisUrl, analysisPending} = this.state;
    ...
    return (
      <div className={styles.analysisOutputHeader}>
        ...
        {url && (
          <a style={{marginLeft: 5}} onClick={this.handleDownload}>
            Download results
          </a>
        )}
        {analysisUrl && (
          <a style={{marginLeft: 5}} onClick={this.handleDownloadAnalysisFile}>
  ...
  render() {
    ...
    const {url, pending, data} = this.state;
    return (
      <div className={classNames(styles.analysisOutputContainer, className)} style={style}>
        {this.renderHeader()}
        {pending && !url && <LoadingOutlined />}
        {!pending && (!url || !data) ? (
          <Alert title="Analysis results not found." type="info" />
        ) : null}
        <div className={styles.analysisOutputTableContainer}>
          <AnalysisOutputTable data={data} />
        </div>
      </div>
    );
  }
...
function AnalysisOutput({analysis, className, style, onClose}) {
  if (!analysis || !analysis.defineResultsOutputs || analysis.defineResultsOutputs.length === 0) {
    return null;
  }
