...
function call_api() {
    ...
    if [ $_EXIT_CODE -ne 0 ]; then
        pipe_log_fail "Couldn't get response from API on: ${_api_method} after $_MAX_RETRIES retries" "${_TASK_NAME}"
        exit 1
    fi
    echo "$_result"
}
...
function prepare_workflow_parameters() {
    _PARAMS_TO_EXCLUDE="ENGINE,OUTPUT_DIR,RESYNC_IMAGES"
