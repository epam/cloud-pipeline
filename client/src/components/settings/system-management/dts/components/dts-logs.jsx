...
import styles from './dts-logs.module.css';
...
class DtsLogs extends React.Component {
  ...
  fetchLogs = () => {
    ...
    this.setState({pending: true}, async () => {
      ...
      this.storage = await createObjectStorageWrapper(dataStorageAvailable.value, folder);
      if (!this.storage) {
  ...
  render() {
          ...
          title={
            <p>
              ...
              {this.downloadAvailable ? (
                <a style={{marginLeft: 5}} onClick={this.downloadLogsFile}>
                  Download logs file.
                </a>
              ) : null}
            </p>
    ...
    if (!pending && !logsString) {
      return <Alert title={<p>Logs are not available or empty.</p>} type="error" />;
    }
    return (
      <Spin classNames={{root: styles.spin}} spinning={pending}>
        <div className={styles.logsContainer}>
          <div className={styles.controls}>
            {this.downloadAvailable ? <a onClick={this.downloadLogsFile}>Download logs</a> : null}
          </div>
          <RunTaskLogs
