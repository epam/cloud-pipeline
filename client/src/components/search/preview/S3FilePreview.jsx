...
const previewLoad = (params, dataStorageCache) => {
  if (params.item && params.item.parentId && params.item.id) {
    return dataStorageCache.getContent(params.item.parentId, params.item.id);
  } else {
    return null;
  }
};
...
const downloadUrlLoad = (params, dataStorageCache) => {
  if (params.item && params.item.parentId && params.item.id) {
    return dataStorageCache.getDownloadUrl(params.item.parentId, params.item.id, undefined, true);
  } else {
    return null;
  }
};
...
@inject((stores, params) => {
  ...
  return {
    ...
    dataStorageInfo:
      params.item && params.item.parentId ? dataStorages.load(params.item.parentId) : null,
...
export default class S3FilePreview extends React.Component {
  ...
  renderInfo = () => {
    ...
    const path =
      this.props.item.type !== SearchItemTypes.NFSFile
          ...
          <tbody>
            <tr>
              ...
              <td style={{paddingLeft: 5}}>
                ...
                <AWSRegionTag regionId={this.props.dataStorageInfo.value.regionId} />
              </td>
            </tr>
            <tr>
    ...
    return (
      <div className={styles.contentPreview}>
        ...
        {this.filePreview.extension === 'html' ? (
          <HTMLRenderer htmlString={this.filePreview.preview} />
        ) : (
              ...
              return (
                <tr key={`row-${rowIndex}`}>
                  {row.map((cell, columnIndex) => {
                    return (
                      <td
                        className={classNames(styles.csvCell, 'cp-search-csv-table-cell')}
                        key={`col-${columnIndex}`}
                      >
                        {cell}
                      </td>
  ...
  renderVSIPreview = () => {
    return (
      ...
      >
        {renderAttributes(this.props.metadata, {
          tags: true,
          column: true,
          showError: false,
  ...
  renderPDBPreview = () => {
    ...
    return (
      <div className={styles.contentPreview} style={{height: '50vh'}}>
        <EmbeddedMiew
