...
While ($True) {
    try {
        ...
        Log "Removing existing aws cli distribution..."
        RemoveFileIfExists "$env:AWSCLI_DISTRIBUTION_PATH"

        Log "Downloading aws cli distribution..."
        # https://awscli.amazonaws.com/AWSCLIV2.msi
        Invoke-WebRequest "$env:AWSCLI_DISTRIBUTION_URL" -OutFile "$env:AWSCLI_DISTRIBUTION_PATH" -ErrorAction Stop
        
        Log "Installing aws cli distribution..."
        Start-Process msiexec -ArgumentList "/a ""$env:AWSCLI_DISTRIBUTION_PATH"" /qn TARGETDIR=""$env:AWSCLI_DIR""" -Wait -NoNewWindow -RedirectStandardOutput "$env:DTS_LOGS_DIR\awscli.install.out.log" -RedirectStandardError "$env:DTS_LOGS_DIR\awscli.install.err.log"

        Log "Setting up AWS CLI credentials provider..."
        Start-Process -FilePath "$env:DTS_DIR\pipe\pipe\pipe.bat" -ArgumentList "cloud configure --provider aws --default-profile $env:DTS_AWSCLI_DEFAULT_PROFILE --force --config ""$env:AWS_CONFIG_FILE""" -Wait -NoNewWindow -RedirectStandardOutput "$env:DTS_LOGS_DIR\awscli.config.out.log" -RedirectStandardError "$env:DTS_LOGS_DIR\awscli.config.err.log"
