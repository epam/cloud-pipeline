...
class SystemJobParameters extends React.Component {
  ...
  componentDidUpdate(prevProps, prevState, snapshot) {
    if (prevProps.job !== this.props.job || prevProps.visible !== this.props.visible) {
      this.updateFromProps();
    }
  }
  ...
  renderAlerts = () => {
    ...
    const alerts = (parametersFromScript || []).filter((parameter) => parameter.description);
  ...
  render() {
    const {className, style, visible, onCancel} = this.props;
    const {parameters} = this.state;
    return (
      <Modal
        className={className}
