...
import {Alert, Table, Row, Input, Button, Modal, message} from 'antd';
...
import {alphabeticSorter, capitalized, getRoleType, splitRoleName} from './utilities';
  ...
  get filteredRoles() {
    ...
    return this.roles.filter((r) => {
      const hasPermissions =
        this.isAdmin || this.isUsersAdmin || this.isReader || roleModel.readAllowed(r);
      return (
        hasPermissions &&
        r.displayName.toLowerCase().includes((groupsSearchText || '').toLowerCase())
      );
    });
  }
    ...
    return (
      <div className={styles.container}>
        <Row type="flex" style={{marginBottom: 10}}>
          ...
          {(this.isAdmin || this.isUsersAdmin) && !predefined && (
            <div style={{paddingLeft: 10}}>
              <Button type="primary" onClick={this.openCreateGroupDialog}>
        ...
        <EditRoleDialog
          ...
          readOnly={
            !this.isAdmin && !this.isUsersAdmin && !roleModel.writeAllowed(this.state.editableGroup)
          }
