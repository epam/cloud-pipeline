...
class TooltipRenderer extends React.Component {
      ...
      return [
        {
          title: 'Date',
          color: this.color,
          value: dayjs.unix(xPoint).format('D MMM, YYYY HH:mm'),
        },
        ...tooltips,
      ];
    }
  ...
  renderTooltip = (x) => {
    ...
    const renderSingleTooltip = (tooltip, index) => {
      ...
      const textX = x + xShift + MARGIN;
      let textY = canvasHeight / 2.0 - totalHeight / 2.0 + getHeightBefore(index);
      if (size) {
        textY += size.height / 2.0;
      }
      return (
        <text key={key} ref={ref} x={textX} y={textY} style={style} alignmentBaseline={'middle'}>
    ...
    return (
      <g shapeRendering={'crispEdges'}>
        <path d={tooltipBorderPoints} stroke={'none'} fill={this.backgroundColor} />
        <path d={tooltipBorderPoints} stroke={this.borderColor} fill={'none'} strokeWidth={1} />
  ...
  render() {
    const {visible, xPoint, left, top, bottom, ratio, timelineStart, tooltips} = this.props;
    ...
    return (
      <g shapeRendering={'cripsEdges'}>
        <line x1={x} x2={x} y1={top} y2={bottom} stroke={this.borderColor} strokeWidth={1} />
...
class Tooltip extends React.Component {
  ...
  render() {
    ...
    const {from} = timeline.state;
    const {chartArea, dataGroup, height, plots, width} = plot;
