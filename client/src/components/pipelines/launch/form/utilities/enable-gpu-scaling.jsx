...
function parseCPUConfiguration(configuration) {
  const {Param, param = Param, DefaultValue, defaultValue = DefaultValue} = configuration || {};
  return {
    parameter: param,
    value: defaultValue,
  };
...
function parseRestConfiguration(configuration) {
  return Object.entries(configuration || {}).map(([parameter, value]) => ({parameter, value}));
}
...
function parseGPUScalingPreference(preferences) {
  ...
  return providers
    .map((provider) => {
      ...
      const {Hybrid = {}, hybrid = Hybrid, General = {}, general = General} = providerValue || {};
...
function getScalingConfigurationForProvider(provider, preferences) {
  return parseGPUScalingPreference(preferences).find(
...
function getGPUScalingDefaultConfiguration(options, preferences) {
  const {provider, hybrid = false} = options;
...
function readGPUScalingPreference(options, preferences) {
  const {autoScaled = false, parameters = {}} = options || {};
  ...
  if (configuration && autoScaled) {
    ...
    const {cpu = {}, gpu = {}, ...rest} = configuration;
...
const InstanceTypeSelector = ({
  ...
  const filterSelect = (input, option) => {
    const value = option.props.value === emptyValueKey ? option.props.title : option.props.value;
    ...
    >
      {allowEmpty && (
        <Select.Option key={emptyValueKey} value={emptyValueKey} title={emptyTooltip}>
          {emptyName || <span className="cp-text-not-important">Not set</span>}
        </Select.Option>
      )}
...
function applyParameters(configuration, parameters) {
  if (configuration) {
    const {gpu, cpu, other = []} = configuration;
...
function applyParametersArray(configuration, parametersArray) {
  if (configuration) {
    ...
    const applyParameter = (parameter) => {
      if (parameter && parameter.parameter) {
        const param = parametersArray.find((o) => o.name === parameter.parameter);
        if (!param) {
...
function configurationChanged(previous, current) {
  ...
  return (
    previous.gpu?.parameter !== current.gpu?.parameter ||
