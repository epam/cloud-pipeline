...
async function prepareAuditItems (items) {
  ...
  const storages = storagesIds.map((id) => dataStorages.load(id));
  await Promise.all(storages.map((request) => request.fetchIfNeededOrWait()));
  const loadedStorages = storages
    .map((request) => request.loaded ? request.value : undefined)
    .filter(Boolean);
  return items
    .filter((anItem) => !!anItem.object)
    .map((anItem) => {
