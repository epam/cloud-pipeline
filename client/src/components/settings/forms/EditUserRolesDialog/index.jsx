...
import {CONTENT_PANEL_KEY, METADATA_PANEL_KEY} from '../../../special/splitPanel';
...
import {runAsPermissionsEqual} from '../../../runs/logs/forms/configure-run-as-permissions/utilities';
import styles from './EditUserRolesDialog.module.css';
import RequiredLaunchTags from '../../../special/metadata/special/required-launch-tags';
import UserTokensTable from '../../UserTokensTable';

const RESTRICTED_METADATA_KEYS = [MuteEmailNotifications.metadataKey].filter(Boolean);
...
@inject((common, params) => ({
  ...
  roles: new Roles(),
  credentialProfiles: params.user ? new LoadEntityCredentialProfiles(params.user.id, true) : null,
...
export default class EditUserRolesDialog extends React.Component {
  ...
  updateValues = () => {
    ...
    if (!runnersInitialized && this.props.runners && this.props.runners.loaded) {
  ...
  assignRole = () => {
    ...
    const [role] = (this.props.roles.value || []).filter(
      (r) => `${r.id}` === `${this.state.selectedRole}`,
    );
    if (role) {
      roles.push(role);
  ...
  renderUserRolesList = () => {
    ...
    return this.renderRolesList(roles, (role) => {
      return (
  ...
  get addedRoles() {
    ...
    return roles.filter((r) => rolesInitial.filter((rI) => rI.id === r.id).length === 0);
  }

  get removedRoles() {
    const {roles, rolesInitial} = this.state;
    return rolesInitial.filter((rI) => roles.filter((r) => r.id === rI.id).length === 0);
  }
  ...
  get runnersModified() {
    const {runners = [], runnersInitial = []} = this.state;
  ...
  get dataStorages() {
    if (this.props.dataStorages.loaded) {
      return (this.props.dataStorages.value || [])
        .filter((d) => roleModel.writeAllowed(d))
        .map((d) => d);
    }
    return [];
  }
  ...
  get cloudCredentialProfiles() {
    if (this.props.cloudCredentialProfiles.loaded) {
      return (this.props.cloudCredentialProfiles.value || []).map((o) => o);
    }
    return [];
  }
  ...
  get defaultProfileId() {
    ...
    if (cloudCredentialProfiles.loaded) {
      const profile = (cloudCredentialProfiles.value || []).find((d) => d.id === +defaultProfileId);
      if (!profile) {
        return this.isAdmin || this.isUsersAdmin ? undefined : 'Access is denied';
      }
  ...
  get restrictedMetadataKeys() {
    ...
    return [...RESTRICTED_METADATA_KEYS, ...(preferences.metadataSystemKeys || [])];
  }
  ...
  saveChanges = async () => {
    if (this.modified) {
      ...
      if (this.addedRoles.length > 0) {
        ...
        const requests = this.addedRoles.map(
          (role) => new RoleAssign(role.id, this.props.userInfo.value.id),
        );
        await Promise.all(requests.map((request) => request.send({})));
        const [error] = requests.map((request) => request.error).filter(Boolean);
        hide();
        if (error) {
      ...
      if (this.removedRoles.length > 0) {
        ...
        const requests = this.removedRoles.map(
          (role) => new RoleRemove(role.id, this.props.userInfo.value.id),
        );
        await Promise.all(requests.map((request) => request.send({})));
        const [error] = requests.map((request) => request.error).filter(Boolean);
        hide();
      ...
      if (this.runnersModified) {
        ...
        const request = new RunnersUpdate(this.props.userInfo.value.id);
      ...
      if (metadata) {
        ...
        const fetchMetadata = this.props.metadataCache.getMetadata(
          this.props.userId,
          'PIPELINE_USER',
        );
        ...
        Object.keys(data || {}).forEach((key) => {
          if (!this.restrictedMetadataKeys.includes(key) && !Object.hasOwn(metadata, key)) {
            removed[key] = {
  ...
  renderRunners = () => {
    ...
    const {runners: runnersRequest, roles: rolesRequest} = this.props;
    ...
    if (runners && runners.length > 0) {
      return runners.map((s, index, array) => {
        return (
          <span key={s.name} style={{marginRight: 5, whiteSpace: 'nowrap'}}>
            {s.principal ? <UserName userName={s.name} /> : renderRole(s.name)}
  ...
  onImpersonate = () => {
    ...
    if (user && user.userName && impersonation) {
      impersonation.impersonate(user.userName).then((error) => {
        if (error) {
          message.error(error, 5);
        }
      });
  ...
  renderUserRolesTab = () => {
    ...
    const credentialProfilesPending = this.props.credentialProfiles
      ...
      : false;
    const runnersPending = this.props.runners ? this.props.runners.pending : false;
    const rightPanel = (
          ...
          >
            ...
            <div key="INSTANCE_MANAGEMENT">
              <div style={{marginTop: 5, padding: 2}}>
                <span style={{fontWeight: 'bold', float: 'left'}}>Can run as this user:</span>
                <a
                ...
                <ShareWithForm
                  ...
                  roles={this.props.roles.loaded ? (this.props.roles.value || []).slice() : []}
                  sids={this.state.runners.map((runner) => ({
                    ...runner,
              ...
              <div style={{padding: '0 2px'}}>
                <Select
                  ...
                  showSearch
                  mode="multiple"
                  disabled={this.state.operationInProgress || readOnly || credentialProfilesPending}
  ...
  renderFooter = () => {
      ...
      return (
        <div className="cp-modal-footer-actions cp-modal-footer-actions--split">
          <div className="cp-modal-footer-actions-group">
            <Button disabled={readOnly} id="delete-user-button" danger onClick={this.onDelete}>
              DELETE
            </Button>
            <Button disabled={readOnly} danger onClick={() => this.onBlockUnBlock(!blocked)}>
    ...
    return (
      <div className="cp-modal-footer-actions">
        <Button id="close-edit-user-form" type="primary" onClick={() => this.onClose()}>
          OK
        </Button>
      </div>
    );
        ...
        items={[
          {
            ...
            label: (
              <div>
                <span>PROFILE</span>
                {blocked && <span style={{fontStyle: 'italic', marginLeft: 5}}>- blocked</span>}
              </div>
            ),
            children: null,
          },
          ...
          ...(this.isAdmin || this.isUsersAdmin
            ...
            : []),
          ...(this.isAdmin ||
          (roleModel.readAllowed(this.props.user) && roleModel.writeAllowed(this.props.user))
            ? [
                {
                  key: 'api-tokens',
                  label: 'API TOKENS',
  ...
  renderContent = () => {
    ...
    if (activeTab === 'user-statistics') {
      return <UserInfoSummary user={user} style={{flex: 1, overflow: 'auto'}} />;
    }
    if (activeTab === 'permissions') {
      return (
        <PermissionsForm objectType={'PIPELINE_USER'} objectIdentifier={userId} showOwner={false} />
      );
    }
    if (activeTab === 'api-tokens') {
      return <UserTokensTable userId={userId} />;
    }
  ...
  renderImpersonateButton = () => {
    const {activeTab} = this.state;
    const {user} = this.props;
    if (activeTab === 'user' && user && roleModel.executeAllowed(user)) {
      return (
        <Button type="primary" onClick={this.onImpersonate} className={styles.impersonateBtn}>
          IMPERSONATE
        </Button>
      );
    }
