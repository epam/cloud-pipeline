...
const renderEstimatedPrice = (item) => {
  ...
  return <JobEstimatedPriceInfo>Cost: {info.total.toFixed(2)}$</JobEstimatedPriceInfo>;
};

const renderRunningTime = (item, estimatedPriceVisible = true) => {
  const renderTime = () => {
    if (item.endDate && item.startDate && item.status !== 'RUNNING') {
      const start = toUtcDayjs(item.startDate);
      const end = toUtcDayjs(item.endDate);
      const diff = end.diff(start, 'minute', true);
      return <span>{diff.toFixed(2)} min</span>;
    }
    const start = toUtcDayjs(item.startDate);
    const diff = dayjs.utc().diff(start, 'minute', true);
    return <span>Running for {diff.toFixed(2)} min</span>;
  };
  ...
  if (estimatedPrice) {
    return (
      <div>
        <Row>{time}</Row>
        {estimatedPriceVisible ? <Row>{estimatedPrice}</Row> : null}
      </div>
    );
  }
