...
import styles from './define-results.module.css';
...
function Statistics(props) {
  const {property, stats = [], onChange = () => {}} = props;
  const functions = getObjectPropertyFunction(property);
  if (functions.length === 0) {
    return;
  }
  return (
    <Select mode="multiple" style={{flex: 1}} value={(stats || []).slice()} onChange={onChange}>
      {functions.map((aFunction) => (
        <Select.Option key={aFunction} value={aFunction} title={PropertyFunctionHints[aFunction]}>
...
function OutputProperty(props) {
  ...
  const {object, properties = []} = outputProperty;
  if (!pipeline) {
    return null;
  }
  ...
  const changeObject = (newObject) => {
    if (newObject !== object) {
      ...
      const newProperties =
        newObjectIsSpot === isSpot && newObjectHasSpots === hasSpots
          ? properties
  ...
  const canConfigureStatistics = (property) => {
    return (
      property.name !== ObjectProperty.all && getObjectPropertyFunction(property.name).length > 0
    );
  };
  return (
    <div>
      <div className={styles.row}>
        ...
        <Select className={styles.value} value={object} onChange={changeObject}>
          {objects.map((anObject) => (
            <Select.Option key={anObject} value={anObject}>
              {anObject}
            </Select.Option>
          ))}
        </Select>
        <Button danger size="small" style={{marginLeft: 5}} onClick={removeObject}>
          <DeleteOutlined />
        </Button>
      </div>
      {properties
        .map((property, index) => [
          <div key={`${outputProperty.id}_property_${index}`} className={styles.row}>
            ...
            >
              <Select.Option key={ObjectProperty.all} value={ObjectProperty.all}>
                {ObjectPropertyName[ObjectProperty.all]}
              </Select.Option>
              <Select.Option disabled key="divider" value="divider">
                <div className="cp-divider horizontal">{'\u00A0'}</div>
              </Select.Option>
              {objectProperties.map((aPropertyName) => (
                <Select.Option key={aPropertyName} value={aPropertyName}>
                  {ObjectPropertyName[aPropertyName]}
                </Select.Option>
              ))}
            </Select>
            <Button danger size="small" style={{marginLeft: 5}} onClick={removeProperty(index)}>
              <CloseOutlined />
            </Button>
          </div>,
