...
class SettingsButton extends React.Component {
  ...
  getConfigureDiscountsButtonTitle = () => {
    ...
    return (
      <div>
        <div>Configure discounts</div>
        {parts.length > 0 && (
          <div style={{lineHeight: '12px', fontSize: 'smaller'}} className={styles.discounts}>
            {parts}
          </div>
        )}
      </div>
  ...
  render() {
    ...
    const menuItems = [
      ...
      {type: 'divider', key: 'd2'},
      ...(exportItems.length > 0 ? [{key: 'export', label: 'Export', children: exportItems}] : []),
    ];
...
export default inject(
  ...
  'quotas',
  ...exportStores,
)(BillingNavigation.attach(inject('layoutContext')(observer(SettingsButton))));
