...
class Conflicts extends React.Component {
  ...
  updateFromProps = () => {
    const {conflicts, conflictsInfo, run, storage, mergeInProgress} = this.props;
    this.session.setFiles(run, storage, mergeInProgress, conflicts, conflictsInfo);
    this.setState({
      current: (conflicts || [])[0],
    });
  };
  ...
  resize = (e) => {
    if (this.resizeInfo && this.ideContainer) {
      e.stopPropagation();
      e.preventDefault();
      const {x, width} = this.resizeInfo;
      const delta = e.clientX - x;
      const min = 200;
      const max = this.ideContainer.clientWidth / 2.0;
      const correctSize = (size) => Math.min(max, Math.max(min, size));
  ...
  reportSessionState = () => {
    ...
    if (onSessionStateChanged) {
      onSessionStateChanged(this.session);
    }
  };
  ...
  renderFiles = () => {
    ...
    return (
      <Table
        ...
        pagination={false}
        rowClassName={(file) =>
          classNames(styles.file, {[styles.selectedFile]: file.name === current})
        }
  ...
  render() {
    const {current, filesWidth} = this.state;
    const {disabled, run, storage} = this.props;
    return (
      <div
      ...
      >
        ...
        <Provider conflictsSession={this.session} colorsConfig={this.changesDisplayConfig}>
          <div
