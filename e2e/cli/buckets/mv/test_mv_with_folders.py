...
class TestMoveWithFolders(object):
    ...
    #         pipe_storage_cp(source, "s3://%s/%s/%s/" % (self.bucket_name, case, self.test_file_1))
    ...
    #         pipe_storage_mv(source, "s3://%s/%s/" % (self.bucket_name, case))
    ...
    #
    #         pipe_storage_mv(source, "s3://%s/%s/" % (self.bucket_name, case), recursive=True)
    ...
    #         create_test_file(source1, TestFiles.DEFAULT_CONTENT)
    #         pipe_storage_cp(source1, "s3://%s/%s/%s/" % (self.bucket_name, case, "folder1"))
    #         assert s3_object_exists(self.bucket_name, "%s/%s/%s" % (case, "folder1", self.test_file_1))
    #         source_for_check1 = ObjectInfo(True).build(source1)
    #
    #         source2 = os.path.join(source, self.test_file_2)
    #         create_test_file(source2, TestFiles.COPY_CONTENT)
    #         pipe_storage_cp(source2, "s3://%s/%s/%s/" % (self.bucket_name, case, "folder2"))
    #         assert s3_object_exists(self.bucket_name, "%s/%s/%s" % (case, "folder2", self.test_file_2))
    #         source_for_check2 = ObjectInfo(True).build(source2)
    #
    #         destination = os.path.join(source, "output") + "/"
    #         pipe_storage_mv("s3://%s/%s/" % (self.bucket_name, case), destination, recursive=True)
    #         assert_copied_object_info(source_for_check1, ObjectInfo(True)
    #                                   .build(os.path.join(destination, "folder1", self.test_file_1)), case)
    #         assert not s3_object_exists(self.bucket_name, "%s/%s/%s" % (case, "folder1", self.test_file_1))
    #         assert_copied_object_info(source_for_check2, ObjectInfo(True)
    #                                   .build(os.path.join(destination, "folder2", self.test_file_2)), case)
    #         assert not s3_object_exists(self.bucket_name, "%s/%s/%s" % (case, "folder2", self.test_file_2))
    #     except BaseException as e:
    #         pytest.fail("Test case {} failed. {}".format(case, e.message))
    #
    # @pytest.mark.run(order=6)
    ...
    #
    #         pipe_storage_mv(os.path.join(source_folder, test_folder1), "s3://%s/%s/" % (self.bucket_name, case),
    #                         recursive=True)
    ...
    #         create_test_file(source2, TestFiles.COPY_CONTENT)
    #
    #         pipe_storage_cp(source1, "s3://%s/%s/folder/" % (self.bucket_name, case))
    #         assert s3_object_exists(self.bucket_name, os.path.join(case, "folder", self.test_file_1))
    #         pipe_storage_cp(source2, "s3://%s/%s/folder2/" % (self.bucket_name, case))
    #         assert s3_object_exists(self.bucket_name, os.path.join(case, "folder2", self.test_file_2))
    #
    #         pipe_storage_mv("s3://%s/%s/folder" % (self.bucket_name, case),
    ...
    #         assert not s3_object_exists(self.bucket_name, os.path.join(case, "folder", self.test_file_1))
    #         assert s3_object_exists(self.bucket_name, os.path.join(case, "folder2", self.test_file_2))
    #     except BaseException as e:
    #         pytest.fail("Test case {} failed. {}".format(case, e.message))
    ...
    def test_folder_with_slash_should_download_content_only(self, case, has_destination_slash, has_source_slash):
        ...
        try:
            ...
            assert not s3_object_exists(self.bucket_name, os.path.join(case, self.test_file_1))
            assert not s3_object_exists(self.bucket_name, os.path.join(case, self.test_file_2))
        except BaseException as e:
            pytest.fail("Test case {} failed. {}".format(case, e.message))
    ...
    def test_folder_with_slash_should_copy_between_buckets_content_only(self, case, has_destination_slash,
        ...
        try:
            ...
            assert s3_object_exists(self.other_bucket_name, os.path.join(case, self.test_file_2))
    ...
    def test_copy_folder_between_buckets_with_skip_existing_option_should_skip(self):
        ...
        try:
            ...
            pipe_storage_cp(source_file1, "s3://%s/%s" % (self.bucket_name, key1))
            pipe_storage_cp(source_file2, "s3://%s/%s" % (self.bucket_name, key2))
            assert s3_object_exists(self.bucket_name, key1)
            assert s3_object_exists(self.bucket_name, key2)
            ...
            assert s3_object_exists(self.bucket_name, key1)
            assert not s3_object_exists(self.bucket_name, key2)
    ...
    def test_copy_folder_between_buckets_with_skip_existing_option_should_not_skip(self):
        ...
        try:
            ...
            assert s3_object_exists(self.other_bucket_name, key1)
            assert s3_object_exists(self.other_bucket_name, key2)
            assert not s3_object_exists(self.bucket_name, key1)
            assert not s3_object_exists(self.bucket_name, key2)
            actual = ObjectInfo(False).build(self.other_bucket_name, key1)
            assert not expected.size == actual.size, "Sizes must be the different."
            assert not expected.last_modified == actual.last_modified, \
    ...
    def _create_folder_on_bucket(self, case):
        ...
        pipe_storage_cp(source2, "s3://%s/%s/%s" % (self.bucket_name, case, self.test_file_2))
