...
function HcsLensControl(hcsViewerState) {
  const {channels = [], useLens, lensEnabled, lensChannel, pending} = hcsViewerState || {};
      ...
      <div className={styles.lensContainer}>
        ...
        >
          {channels.map((channel) => (
            <Select.Option key={channel.identifier} value={`${channel.index}`}>
              {channel.name}
            </Select.Option>
          ))}
        </Select>
