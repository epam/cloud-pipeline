...
import {Button, Checkbox, Collapse, Dropdown, Input, message, Modal} from 'antd';
...
import styles from './wdl-properties-form.module.css';
...
class WdlPropertiesForm extends React.Component {
  ...
  onEntityPropertiesChanged = (entityChanged = false) =>
    this.setState({
  ...
  onRemove = () => {
    const {canRemoveEntity} = this.state;
    const {entity} = this.props;
    if (!entity || isWorkflow(entity) || !canRemoveEntity) {
      return false;
    }
    this.setState({
  ...
  onConfirmRemove = () => {
    const {removeExecutable, canRemoveEntity} = this.state;
    const {entity, disabled, onRemoved, wdlDocument} = this.props;
    if (!disabled && entity && canRemoveEntity && !isWorkflow(entity)) {
      const parts = [<span key="main">{getEntityName(entity, wdlDocument)}</span>];
      if (removeExecutable && entity.executable) {
        parts.push(<span key="and">{' and '}</span>);
        parts.push(<span key="executable">{getEntityName(entity.executable, wdlDocument)}</span>);
      }
  ...
  createAction = (actionFn) => {
    const {onActionAdded} = this.props;
    try {
  ...
  renderHeader = () => {
    const {disabled, wdlDocument} = this.props;
    const {
      type,
    ...
    if (type) {
      ...
      if (canAddSubAction && !disabled) {
        ...
        actionButton = (
          <DropDownWrapper key="create actions" visible={this.state.actionsDropdownVisible}>
            <Dropdown
        ...
        <div className={styles.propertiesRow}>
          ...
          <div className={styles.actions}>
            ...
            {!disabled && canRemoveEntity && (
              <Button danger size="small" onClick={() => this.onRemove()}>
    ...
    return (
      <div>
        <div className={styles.propertiesRow}>
          {title && (
            <div
              className={classNames(styles.propertyTitle, {
          ...
          <Input
            ...
            className={classNames(styles.propertyValue, {
              'cp-error': issues.length > 0,
            })}
  ...
  renderAlias = () => {
    const {executableName, nameIssues = []} = this.state;
    return this.renderInputProperty({
      title: 'Alias',
  ...
  renderExecutable = () => {
    const {executableType = 'Executable'} = this.state;
    return this.renderInputProperty({
  ...
  renderParametersBlock = (options = {}) => {
    const {entity, disabled, wdlDocument} = this.props;
    const {
      available = true,
      title,
      parameters = [],
    ...
    const addParameter = (parameterOwner) => {
      if (parameterOwner && typeof parameterOwner.addParameters === 'function') {
        const aName =
          typeof parameterOwner.generateParameterName === 'function'
  ...
  renderRuntimeAttributes = () => {
    const {task, runtime = [], runtimeAttributesAvailable, runtimeAttributesEditable} = this.state;
    const {entity, allowedInstanceTypes, disabled} = this.props;
    if (
      entity &&
      task &&
      ...
      const onChangeRuntimeGenerator =
        (property, isEvent = true) =>
        (event) =>
          onChangeRuntimeProperty(property, isEvent ? event.target.value : event);
      const onRenameRuntimeGenerator =
        (property, isEvent = true) =>
        (event) =>
          onRenameRuntimeProperty(property, isEvent ? event.target.value : event);
      ...
      const renderNameInput = (r) => {
        ...
        return (
          <Input
            ...
            value={r.property}
            className={classNames(styles.propertyTitle, styles.propertyValue, {
              'cp-error': !r.valid,
            })}
  ...
  renderCommand = () => {
    ...
    if (task && isTask(task) && commandAvailable) {
      const onChange = (code) => {
        task.command = code;
      };
      return (
        <div>
          <div
            className={classNames(styles.propertiesRow, {
  ...
  renderRemoveConfirmationModal = () => {
    ...
    const renderFooter = () => (
      ...
      >
        ...
        <Button danger style={{marginLeft: 5}} onClick={() => this.onConfirmRemove()}>
          Remove
        </Button>
      </div>
    );
    const {removeExecutable, onRemoveRequest} = this.state;
    const canRemoveExecutable =
      entity &&
      isCall(entity) &&
      entity.executable &&
    ...
    return (
      <Modal
        open={!disabled && !!entity && !isWorkflow(entity) && onRemoveRequest}
        closable
      ...
      >
        ...
        >
          ...
          <b>Are you sure you want to remove {getEntityName(entity, wdlDocument)}?</b>
        </div>
        {canRemoveExecutable && (
  ...
  render() {
    ...
    return (
      <div className={classNames(className, styles.wdlPropertiesForm)} style={style}>
        {this.renderHeader()}
