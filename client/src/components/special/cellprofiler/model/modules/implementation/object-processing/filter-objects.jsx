...
class Measurements extends React.Component {
  ...
  get physicalSize() {
    if (this.cpModule && this.cpModule.pipeline) {
  ...
  get multiple() {
    ...
    return this.cpModule.getParameterValue('mode') === 'Measurements' && this.method === 'Limits';
  }
  ...
  renderMeasurementConfiguration = (measurement, index) => {
    ...
    const renderMinMaxFilter = (useProperty, valueProperty, title) => {
      const {[useProperty]: use, [valueProperty]: filter} = measurement;
      const onChangeUse = (e) => {
        const measurements = this.measurements;
      ...
      return (
        ...
        >
          <div style={{width: 100}}>
            <Checkbox checked={use} onChange={onChangeUse} style={{marginRight: 5}}>
              {title}
            </Checkbox>
          </div>
          {use && <Input style={{flex: 1}} value={filter} onChange={onChangeMinMax} />}
          {use && isUnits && <span style={{marginLeft: 5}}>{this.units}</span>}
          {use && isSquareUnits && (
    ...
    return (
      <div key={`measurement-${index}`} className="cp-even-odd-element" style={{padding: 5}}>
        <div
          style={{
            display: 'flex',
            alignItems: 'center',
        ...
        >
          <MeasurementSelector value={value} style={{flex: 1}} onChange={onChangeValue} />
          {this.multiple && this.measurements.length > 1 && (
            <Button style={{marginLeft: 5}} onClick={onRemove} size="small" danger>
              <DeleteOutlined />
            </Button>
          )}
        </div>
        {this.method === 'Limits' && (
          <div>
            {renderMinMaxFilter('use_min_filter', 'min_value', 'Minimum')}
            {renderMinMaxFilter('use_max_filter', 'max_value', 'Maximum')}
          </div>
        )}
      </div>
    );
  ...
  render() {
    ...
    return (
      <div className={className} style={style}>
        ...
        <div>{measurements.map(this.renderMeasurementConfiguration)}</div>
        {this.multiple && (
          <div style={{marginTop: 5}}>
            <Button onClick={this.onAddMeasurement} size="small">
              <PlusOutlined />
              Add measurement
            </Button>
          </div>
 ...
 */
function wrapLastModuleWithFilterObjectsModule(
  baseModule,
  moduleConfiguration,
  ...
  subModules: [
    {
      ...
      values: {
        ...
        measurements: (cpModule, modules) => {
          ...
          const isValid = (o) =>
            o !== undefined && o !== '' && !Number.isNaN(Number(o)) && `${o}`.trim().length > 0;
...
export {FilterObjectsBySize, filterObjectsBySizeParameters, wrapLastModuleWithFilterObjectsModule};
export default {
  name: 'FilterObjects',
  ...
  parameters: [
    ...
    'Assign overlapping child to|[Both parents, Parent with most overlap]|Both parents|IF (mode=="Measurements") AND (method=="Minimal per object" OR method=="Maximal per object")|ALIAS assignChildTo',
    {
      ...
      renderer: (moduleParameterValue, className, style) => (
        <Measurements parameterValue={moduleParameterValue} className={className} style={style} />
      ),
    },
