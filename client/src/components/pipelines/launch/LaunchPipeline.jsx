...
import styles from './LaunchPipeline.module.css';
...
@inject(({allowedInstanceTypes, routing, pipelines, preferences}) => {
  ...
  if (isVersionedStorage) {
    versionedStorageLaunchInfo = {
      ...
      version: components.tool && components.version ? components.version : undefined,
    };
  }
...
class LaunchPipeline extends localization.LocalizedReactComponent {
  ...
  get configurations() {
    const {value: configurations} = this.loadingUtilities.getLoadingState(
      CONFIGURATIONS_STATE_KEY,
      this.state,
    );
    if (configurations && (Array.isArray(configurations) || isObservableArray(configurations))) {
  ...
  resolveCurrentConfiguration = (configurationsList) => {
    ...
    const defaultConfiguration =
      configurationsList.find((cfg) => cfg.default) || configurationsList[0];
    if (configurationName) {
      const cfgName = configurationName.toLowerCase();
      return (
        configurationsList.find((cfg) => (cfg.name || '').toLowerCase() === cfgName) ||
  ...
  selectMetadataConfirm = async (entitiesIds) => {
    ...
    if (configurationRequest.error) {
      ...
      const title = `Launch ${configuration.name} configuration ?`;
      Modal.confirm({
        title,
        style: {
          wordWrap: 'break-word',
    ...
    const launchFn = async () => {
      ...
      const applyMetadataParamsOverRunParams = (runParams, configParams) => {
        ...
        for (const key in mappedParams) {
          if (Object.hasOwn(configParams, key) && metadataControlParameter(configParams[key])) {
      ...
      if (configuration) {
        ...
        if (configuration.executionEnvironment === DTS_ENVIRONMENT) {
          for (const key in runPayload) {
            if (Object.hasOwn(runPayload, key) && runPayload[key] !== undefined) {
  ...
  onPipelineChanged = (pipelineId, pipelineVersion) => {
    ...
    if (continueRun) {
      router.push(
        `/launch/${pipelineId}/${pipelineVersion}/${configurationName}/${continueRun}?continue=true`,
      );
    } else {
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    const versionedStorageChanged =
      propChanged('isVersionedStorage') ||
  ...
  updateFromProps = () => {
    ...
    const {toolId: vsToolId, version: vsVersion} = versionedStorageLaunchInfo;
    (async () => {
      try {
        ...
        const toolInfoRequest = this.loadingUtilities.loadWithSetStateCallbacks(
          ...
          async () => {
            ...
            await Promise.all([settingsRequest.fetch(), toolRequest.fetch()]);
        ...
        const configurationsRequest = this.loadingUtilities.loadWithSetStateCallbacks(
          ...
          async () => {
            ...
            const request = new PipelineConfigurations(pipelineId, pipelineVersion);
            await request.fetch();
            if (request.error) {
              throw new Error(request.error);
            }
        ...
        const versionedStoragePayloadRequest = this.loadingUtilities.loadWithSetStateCallbacks(
          ...
          async () => {
            ...
            return getToolLaunchingOptions(this.props, tool, vsVersion);
          },
          setState,
        );
        const payloadRequest = await this.loadingUtilities.loadWithSetStateCallbacks(
          ...
          async () => {
            ...
            const [_, toolInfo, runInfo, pipelineInfo, configurations = [], vsPayload] =
              await Promise.all([
                preferencesRequest,
                toolInfoRequest,
                runIdRequest,
            ...
            if (vsPayload) {
              return {
                ...
                pipelineId:
                  pipelineId !== undefined && pipelineId !== null && !Number.isNaN(pipelineId)
                    ? Number(pipelineId)
                    : undefined,
                pipelineVersion,
              };
  ...
  renderDefault() {
    ...
    const {value: pipeline} = this.loadingUtilities.getLoadingState(PIPELINE_STATE_KEY, this.state);
    ...
    const {configurations, currentConfiguration} = this;
      ...
      >
        <LaunchPipelineForm
          ...
          currentConfigurationName={
            currentConfiguration ? currentConfiguration.name : this.configurationName
          }
          pipeline={pipeline}
        ...
        <MetadataBrowser
          ...
          visible={this.state.showMetadataBrowser && !!this.state.currentProjectId}
