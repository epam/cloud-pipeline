...
import {Button, Input, message, Upload, Tooltip, Checkbox} from 'antd';
import {
  CameraOutlined,
  DeleteOutlined,
...
class ImageUploader extends React.PureComponent {
  ...
  updateFromProps = () => {
    const {value} = this.props;
    this.setState({
      imageURL: !this.isUnset && !this.isBase64encodedImage ? extractUrlValue(value) : undefined,
      check: undefined,
      error: undefined,
    });
  };
  ...
  handleFileChange = (file) => {
    ...
    if (this.props.maxSize && size > this.props.maxSize) {
      message.error(`File size exceeds ${displaySize(this.props.maxSize, false)}`, 5);
    } else {
  ...
  renderImageUploader = () => {
    ...
    return (
      ...
      >
        <div
          className={classNames(styles.imageContainer, 'image-uploader', {
            [styles.disabled]: disabled,
          })}
        >
          <div
            className={classNames(styles.thumbImagePreview)}
            style={this.isBase64encodedImage && !imageURL ? {backgroundImage: value} : {}}
          >
            {'\u00A0'}
          </div>
          <div className={classNames(styles.thumbImage, 'uploader-thumb-image')}>
  ...
  checkImageURLDelayed = () => {
    ...
    if (!imageURL) {
      ...
      this.setState(
        ...
        () => {
          this.checkTimer = setTimeout(() => this.checkImageURL(), CHECK_IMAGE_URL_DELAY_MS);
        },
      );
    }
  };
  ...
  checkImageURL = () => {
    const {imageURL, check} = this.state;
    const {variable, onChange} = this.props;
    fetch(imageURL)
      .then((response) => {
  ...
  renderImageUrlInput = () => {
    ...
    const {imageURL, check, error} = this.state;
  ...
  renderRevertButton = () => {
    const {disabled, initialValue, modifiedValue, onChange, variable} = this.props;
  ...
  renderResetButton = () => {
    const {disabled, extended} = this.props;
    return (
      <Checkbox
  ...
  render() {
    ...
    return (
      <div className={classNames(className, styles.uploaderContainer)}>
