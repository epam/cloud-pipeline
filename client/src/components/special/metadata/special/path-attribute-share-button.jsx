...
class PathAttributeShareButton extends React.PureComponent {
  ...
  static shareButtonAvailable = (options = {}) => {
    const {key, value, pathKeys = [], testValue = false} = options;
  ...
  updatePathInfo = () => {
    ...
    if (pathArray && pathArray.length) {
      Promise.all(pathArray.map(getStorageFileAccessInfo)).then((infos) => {
  ...
  render() {
    const {shareDialogVisible, itemsToShare = []} = this.state;
    if (itemsToShare.length === 0) {
      return null;
    }
    const {className, style, id} = this.props;
    return (
      <Button id={id} size="small" onClick={this.share} style={style} className={className}>
        <ShareAltOutlined />
        <SharedItemInfo
