...
export default class FireCloudMethodSnapshotConfigurations extends React.Component {
  ...
  get currentMethod() {
    if (this.props.fireCloudMethods.loaded) {
      return (this.props.fireCloudMethods.value || [])
        ...
        .map((m) => ({...m, key: `${m.namespace}_${m.name}`}))
        .shift();
    }
    return null;
  }
  ...
  get configurations() {
    if (this._configurations.loaded) {
      ...
      (this._configurations.value || []).forEach((c, i, arr) => {
  ...
  renderTreeItemType = (item) => {
    switch (item.type) {
      case ItemTypes.fireCloudMethodConfiguration:
        return (
          <SettingOutlined style={{color: '#2796dd', fontSize: 'larger', lineHeight: 'inherit'}} />
        );
      default:
        return <div />;
    }
      ...
      return (
        <div>
          {this.currentMethod && this.currentMethod.synopsis && (
            <Row type="flex">{this.currentMethod.synopsis}</Row>
          )}
          <Row type="flex" justify="center" style={{margin: 10}}>
            <Button onClick={this.props.onCreateNew} type="primary">
              Create new
            </Button>
          </Row>
        </div>
