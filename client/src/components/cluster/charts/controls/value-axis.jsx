...
class ValueAxis extends React.Component {
  ...
  get size() {
    const {chartArea, height} = this._plot;
  ...
  processDataGroup = (props) => {
    const {from: propsFrom, to: propsTo, data, dataGroup, minimum, maximum} = props;
    ...
    let dataTo = maximum;
    if (data && dataGroup && Object.hasOwn(data, dataGroup)) {
  ...
  render() {
    ...
    const isLeft = position === 'left';
    return (
      <Provider valueAxis={this} dataGroup={dataGroup}>
