...
class CreateLinkForm extends React.Component {
  ...
  get registries() {
    if (this.props.dockerRegistries.loaded) {
      return (this.props.dockerRegistries.value.registries || []).filter(
        (r) => this.getRegistryWritableGroups(r.id).length > 0,
      );
    }
    return [];
  }
  ...
  getRegistryWritableGroups = (registryId) => {
    if (registryId && this.props.dockerRegistries.loaded && this.props.source) {
      ...
      if (registry) {
        return (registry.groups || []).filter(
          (g) => +g.id !== sourceToolGroupId && roleModel.writeAllowed(g),
        );
      }
    }
    return [];
    ...
    return (
      ...
      >
        ...
        <Row type="flex" align="middle" style={{margin: '5px 0'}}>
          ...
          >
            {this.getRegistryWritableGroups(
              selectedRegistryId ? +selectedRegistryId : undefined,
            ).map((g) => (
