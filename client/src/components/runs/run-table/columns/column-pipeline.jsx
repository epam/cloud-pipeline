...
function PipelinesFilterComponent({
  ...
  const {pending, value: pipelinesValue = [], loaded} = pipelinesRequest;
  if (pending && !loaded) {
    return (
  ...
  const pipelines = pipelinesValue
    .filter(
      (pipeline) =>
        enabled.has(Number(pipeline.id)) ||
        (search &&
  ...
  return (
    ...
    >
      <Row>
        <Input.Search placeholder="Filter pipelines" value={search} onChange={onSearchChanged} />
      </Row>
      <Row>
        <div style={{maxHeight: 400, overflowY: 'auto'}}>
          {pipelines.map((pipeline) => (
            <Row style={{margin: 5}} key={pipeline.id}>
              <Checkbox
