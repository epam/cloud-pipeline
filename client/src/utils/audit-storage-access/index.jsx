...
function getEventIdFromTimestamp(eventIndex = 0) {
  return `${dayjs.utc().valueOf()}${formatEventIndex(eventIndex)}`;
}
...
async function prepareAuditItems(items) {
  ...
  return items
    ...
    .map((anItem) => {
      const {object, timestamp, accessType} = anItem;
      const {fullPath, storageId, path, reportStorageType} = object || {};
      if (!storageId) {
        return undefined;
      }
      const storageIdValue =
        storageId && !Number.isNaN(Number(storageId)) ? Number(storageId) : storageId;
      if (fullPath) {
        return {
          timestamp,
      ...
      const storage = loadedStorages.find(
        (aStorage) =>
          aStorage.id === Number(storageId) &&
