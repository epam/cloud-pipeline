...
import {Alert, Menu, message, Row, Button, Col, Dropdown, Space} from 'antd';
...
class PipelineDetails extends localization.LocalizedReactComponent {
  ...
  get tabs() {
    const {pipelineId: id, version} = this.props;
    const workflow =
      this.displayGraph && this.isCWLPipeline
        ? {
            key: 'workflow',
            title: 'Workflow',
    ...
    switch (this.repositoryType) {
      case 'BITBUCKET':
        return [this.codePath ? code : false, configuration, history, storage].filter(Boolean);
      default:
        return [
          workflow,
  ...
  redirectIfRequired = () => {
    const {pipeline, language} = this.props;
    if (pipeline && pipeline.loaded) {
      ...
      if (language && (language.loaded || (language.error && !language.pending))) {
        const currentTab = this.tabs.find((o) => o.key === this.activeTabPath);
        const [first] = this.tabs;
        if (!currentTab && first) {
          if (this.props.router) {
            this.props.router.push(first.link);
          }
        }
      }
    }
  ...
  updatePipeline = (values) => {
    this.setState({updating: true}, async () => {
      ...
      if (updatePipeline.error) {
        message.error(
          `Error updating ${this.localizedString('pipeline')}: ${updatePipeline.error}`,
        );
  ...
  deletePipeline = (keepRepository) => {
    this.setState({deleting: true}, async () => {
      ...
      if (deletePipeline.error) {
        message.error(
          `Error deleting ${this.localizedString('pipeline')}: ${deletePipeline.error}`,
        );
      ...
      if (!cfg) {
        ...
        try {
          ...
          const defaultConfiguration =
            values.find((v) => v.default) || values.find((v) => /^default$/i.test(v));
  ...
  render() {
    const {pipeline, language} = this.props;
    if ((!pipeline.loaded && pipeline.pending) || (!language.loaded && language.pending)) {
      return <LoadingView />;
    }
    if (pipeline.error) {
      ...
      <div className={styles.fullHeightContainer}>
        <Row type="flex" justify="space-between" align="middle" style={{minHeight: 41}}>
          ...
          <Col className={styles.actionButtons}>
            {roleModel.executeAllowed(this.props.pipeline.value) && this.renderRunButton()}
            <Button
        ...
        <Row
          ...
          type="flex"
          justify="center"
          className={classNames(styles.rowMenu, styles[activeTab])}
        >
          <Menu
            mode="horizontal"
            ...
            items={this.tabs.map((tab) => ({
              key: tab.key,
              label: (
                <AdaptedLink to={tab.link} location={location}>
                  {tab.title}
                </AdaptedLink>
              ),
            }))}
