...
import styles from './hcs-image-controls.module.css';

function LockCheckbox({disabled, style, locked, onLockedChanged}) {
  const onClick = (event) => {
    if (event) {
...
class Channel extends React.PureComponent {
  ...
  renderColorConfiguration = () => {
    const {color = [], onColorChanged} = this.props;
    if (color.length === 3) {
      ...
      return (
        <ColorPicker color={`rgb(${color.join(',')})`} onChange={onChange} ignoreAlpha channels />
      );
    }
    return null;
  };
  ...
  renderContrastConfiguration = () => {
    const {domain = [], contrastLimits = [], loading, onContrastLimitsChanged} = this.props;
    if (domain.length === contrastLimits.length && contrastLimits.length === 2) {
      return (
        <Slider
  ...
  render() {
    ...
    return (
      <div className={classNames(className, styles.channel)} style={style}>
        <div className={styles.header}>
          ...
          >
            <b>{name}</b>
            {locked && (
              <i className="cp-text-not-important" style={{fontWeight: 'normal'}}>
                {' - locked'}
              </i>
            )}
          </Checkbox>
          <div style={{marginLeft: 'auto'}}>{this.renderColorConfiguration()}</div>
        </div>
