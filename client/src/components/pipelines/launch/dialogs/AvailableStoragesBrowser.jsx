...
export function filterNFSStorages(nfsSensitivePolicy, sensitiveStoragesAreSelected) {
  return (a) =>
    !sensitiveStoragesAreSelected || !/^skip$/i.test(nfsSensitivePolicy) || a.type !== 'NFS';
}
...
export default class AvailableStoragesBrowser extends Component {
  ...
  get onlyNonSensitiveStoragesAreSelected() {
    ...
    return (
      ids.size === this.availableNonSensitiveStorages.length &&
  ...
  get allAvailableNonSensitiveStoragesAreSelected() {
    ...
    const ids = new Set(selectedStorages.map((id) => +id));
    return (
      this.availableNonSensitiveStorages.filter((s) => ids.has(+s.id)).length ===
      this.availableNonSensitiveStorages.length
    );
  }
  ...
  get allStoragesAreSelected() {
    const {selectedStorages} = this.state;
    const ids = new Set(selectedStorages.map((id) => +id));
    const allAllowedStorages = (this.props.availableStorages || []).filter(
      filterNFSStorages(this.nfsSensitivePolicy, this.hasSelectedSensitiveStorages),
    );
    return allAllowedStorages.filter((s) => ids.has(+s.id)).length === allAllowedStorages.length;
  }
  ...
  get hasSelectedSensitiveStorages() {
    ...
    const ids = new Set(selectedStorages.map((id) => +id));
    return (
      (this.props.availableStorages || []).filter((s) => s.sensitive && ids.has(+s.id)).length > 0
    );
  }
  ...
  get selectedStorages() {
    ...
    const hasSensitive = (this.props.availableStorages || []).find(
      (s) => s.sensitive && ids.has(+s.id),
    );
    const allAllowedStorages = (this.props.availableStorages || []).filter(
      filterNFSStorages(this.nfsSensitivePolicy, !!hasSensitive),
    );
          ...
          return (
            <Row>
              ...
              >
                ...
                {storage.sensitive && (
                  <span
                    className={classNames('cp-sensitive-tag', 'bordered')}
                    style={{
                      fontSize: 'smaller',
                      marginLeft: 5,
                      padding: '0 2px',
              ...
              <Row style={{fontSize: 'smaller'}}>
                {storage.pathMask}
                <span style={{marginLeft: 5}}>{storage.description}</span>
              </Row>
            </Row>
          );
        },
        ...
        footer={
          <Row type="flex" justify="end">
            ...
            <Button type="primary" onClick={this.onSave}>
              OK{!!this.selectedStorages.length > 0 && ` (${this.selectedStorages.length})`}
            </Button>
          </Row>
        }
      >
        <div style={{maxHeight: '60vh', display: 'flex', flexDirection: 'column'}}>
          <Row type="flex" align="middle" style={{marginBottom: 10}}>
            ...
            {this.state.selectedStorages.length && (
              <Button danger style={{marginLeft: 5}} onClick={this.clearSelection}>
                Clear selection
              </Button>
            )}
          </Row>
          <SensitiveBucketsWarning
            ...
            message={
              <div>
                Selection contains <b>sensitive storages</b>. This will apply a number of
                restrictions for the job: no Internet access, all the storages will be available in
                a read-only mode, you won't be able to extract the data from the running job and
                other.
              </div>
            }
          />
