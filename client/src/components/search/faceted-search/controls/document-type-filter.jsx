  ...
  render() {
    ...
    const filterMenuItems = this.filters.map((filter) => {
      ...
      return {
        key: filter.key,
        style: {
          cursor: !filter.enabled && filter.count === 0 ? 'default' : 'pointer',
        },
        label: (
          <div
            className={classNames(styles.documentFilter, 'cp-search-faceted-button', {
          ...
          >
            ...
            {filter.count > 0 && showCounts ? (
              <span className={styles.count}>({displayCount(filter.count, true)})</span>
            ) : undefined}
            {filter.enabled ? (
              <CheckCircleFilled
    ...
    const filterMenu = (
      <Menu onClick={this.handleFilterClick} className={styles.azaza} items={filterMenuItems} />
    );
    return (
      <Dropdown
      ...
      >
        <Button
          size={size}
          className={classNames(styles.documentFilterBtn, 'cp-search-faceted-button')}
        >
          <FilterOutlined
            className={classNames('cp-icon-larger', {selected: this.activeFilters.length > 0})}
          />
        </Button>
      </Dropdown>
    );
