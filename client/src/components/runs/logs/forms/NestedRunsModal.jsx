...
import styles from './NestedRunsModal.module.css';
...
function getHoveredElementsInfo(hoveredItems = [], styles = {}) {
  ...
  const renderInfoForDate = (date) => {
    ...
    return (
      <div className={styles.row} key={`element-${date}`}>
        {color && <div className={styles.color} style={{background: color}} />}
        <span className={styles.dataset}>Nodes:</span>
        <span className={styles.value}>{min === max ? min : `${min} - ${max}`}</span>
      </div>
    );
  };
  return (
    <div>
      {dates.map((date) => (
        <div key={`${date}`}>
          <div>
            <b>{dayjs.unix(date).format('D MMMM YYYY, HH:mm:ss')}</b>
          </div>
          <div>{renderInfoForDate(date)}</div>
        </div>
      ))}
    </div>
  );
...
class NestedRunsChart extends React.Component {
      ...
      this.setState(
        ...
        async () => {
          ...
          try {
            ...
            await Promise.all([runRequest.fetch(), request.fetch()]);
            if (runRequest.error) {
            ...
            state.periodTitle = [
              ...
              run.endDate ? displayDate(run.endDate, 'D MMMM, YYYY HH:mm:ss') : 'till now',
            ].join(' - ');
            ...
            const runsData = [
              ...
              const {runId, status, startDate, endDate, master = false} = item;
              const start = toUtcDayjs(startDate).startOf('second').unix();
              const end = endDate ? toUtcDayjs(endDate).startOf('second').unix() : undefined;
              return {
                runId,
                status,
                start,
  ...
  renderTooltipInfo = () => {
    ...
    if (tooltip) {
      const {item} = tooltip;
      if (item) {
        const {items: nestedRuns = [], date} = item;
        return (
          <div className={styles.tooltip}>
            <div className={styles.header}>
              Running jobs at {dayjs.unix(date).format('D MMMM YYYY, HH:mm:ss')}
            </div>
            <div className={styles.runsList}>
              {nestedRuns.map((run) => (
                <a
                  key={`${run.runId}`}
                  className={classNames(styles.run, 'cp-text')}
                  onClick={(event) => handleClick(event, run)}
                >
                  <StatusIcon status={run.status} small />
                  <span style={{marginLeft: 2}}>{run.runId}</span>
                </a>
              ))}
            </div>
          </div>
  ...
  render() {
    const {error, pending, tooltip, datasets = [], periodTitle} = this.state;
    if (error) {
      return (
        <div>
    ...
    return (
      <div className={styles.chartContainer}>
        ...
        />
        {periodTitle && <div className={styles.chartTimelineTitle}>{periodTitle}</div>}
        <div className={classNames(styles.popoverContainer, {[styles.visible]: !!tooltip})}>
          {tooltip && (
            <Popover
              open={!!tooltip}
...
function NestedRunsModal(props) {
  const {visible, runId, onCancel, onRunClick} = props;
  return (
    <Modal width="70%" title="Cluster usage" open={visible} onCancel={onCancel} footer={null}>
      <NestedRunsChart onRunClick={onRunClick} runId={visible ? runId : undefined} />
    </Modal>
  );
}
