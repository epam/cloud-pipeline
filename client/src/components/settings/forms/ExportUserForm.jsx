...
import {Collapse, Checkbox, Modal, Button, message} from 'antd';
import {downloadBlob} from '../../../utils/download-blob';
import ExportUsers from '../../../models/user/Export';
import styles from './ExportUserForm.module.css';
...
async function doExport(fields = DefaultValues, metadataKeys = []) {
  try {
    ...
    const payload = Keys.map(({value}) => ({[value]: fields.indexOf(value) >= 0})).reduce(
      (r, c) => ({...r, ...c}),
      {},
    );
    ...
    if (request.value instanceof Blob) {
      ...
      if (error) {
        ...
        downloadBlob(request.value, 'export.csv');
      }
...
export default function ExportUserForm({
  ...
  const modalControls = [
    ...
    <Button
      ...
      onClick={() => {
        if (onSubmit) {
          onSubmit(values, selectedMetadataKeys);
        }
        if (onCancel) {
          onCancel();
        }
  ...
  return (
    <Modal title="CSV fields" open={visible} footer={modalControls} onCancel={onCancel}>
      {modalContent}
    </Modal>
  );
}
