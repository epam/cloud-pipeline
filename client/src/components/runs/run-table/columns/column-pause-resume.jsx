...
class PauseResumeButtonComponent extends localization.LocalizedReactComponent {
    ...
    Modal.confirm({
      title: (
        <span>
          ...
          {renderPipelineName(run, true, true) || this.localizedString('pipeline')}?
        </span>
      ),
      style: {
        wordWrap: 'break-word',
  ...
  render() {
    const {preferences, run} = this.props;
    if (!run) {
      return null;
    }
    if (
      ...
      switch (run.status.toLowerCase()) {
        case 'pausing':
          return <span id={`run-${run.id}-pausing`}>PAUSING</span>;
        case 'resuming':
          return <span id={`run-${run.id}-resuming`}>RESUMING</span>;
        case 'running':
          if (canPauseRun(run, preferences)) {
            ...
            return (
              <a id={buttonId} onClick={this.showPauseConfirmDialog}>
                PAUSE
              </a>
            );
          }
        ...
        case 'paused': {
          ...
          return (
            <a id={buttonId} onClick={this.showResumeConfirmDialog}>
              {resumeFailureReason ? (
                <Popover
                  title={null}
                  placement="left"
                  content={<div style={{maxWidth: '40vw'}}>{resumeFailureReason}</div>}
                >
...
const getColumn = (localized, reload) => ({
  ...
  render: (text, record) => (
    <RunLoadingPlaceholder run={record} empty>
      <PauseResumeButton run={record} reload={reload} />
