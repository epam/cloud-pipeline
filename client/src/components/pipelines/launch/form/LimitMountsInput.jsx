...
import AvailableStoragesBrowser, {filterNFSStorages} from '../dialogs/AvailableStoragesBrowser';
import AWSRegionTag from '../../../special/AWSRegionTag';
import styles from './LimitMountsInput.module.css';
import {
...
export class LimitMountsInput extends React.Component {
  ...
  updateFromProps = () => {
    const {allowSensitive, dataStorageAvailable, value, cloudRegion} = this.props;
    if (value && !/^none$/i.test(value)) {
      dataStorageAvailable
        ...
        .then(() => {
          this.setState(
            {
              value: correctLimitMountsParameterValue(value, dataStorageAvailable.value || [], {
                allowSensitive,
                cloudRegion,
  ...
  get availableStorages() {
    const {cloudRegion} = this.props;
    if (this.props.dataStorageAvailable.loaded) {
      const storages = (this.props.dataStorageAvailable.value || [])
        .filter(
          (s) =>
            !s.mountDisabled && (this.props.allowSensitive || !s.sensitive) && !s.sourceStorageId,
        )
        .map((s) => s);
  ...
  allNonSensitiveStorages(selection) {
    return (
      (selection || []).length === this.availableNonSensitiveStorages.length &&
      this.availableNonSensitiveStorages.filter((s) =>
        storageMatchesIdentifiers(s, selection || []),
  ...
  allStorages(selection) {
    ...
    const all = this.availableStorages.filter(
      filterNFSStorages(this.nfsSensitivePolicy, hasSensitive),
    );
    return (
      (selection || []).length === all.length &&
  ...
  renderContent = () => {
    ...
    if (this.selectedStorages.length === 0) {
      return <span>No storages will be mounted</span>;
    }
    if (this.allNonSensitiveStorages(this.selectedStorages.map((s) => s.id))) {
      return <span>All available non-sensitive storages</span>;
    }
    if (this.allStorages(this.selectedStorages.map((s) => s.id))) {
      return <span>All available storages</span>;
    }
    const filteredSelectedStorages = this.selectedStorages.filter(
      filterNFSStorages(this.nfsSensitivePolicy, this.hasSelectedSensitiveStorages),
    );
    if (showOnlySummary) {
      return <p>{filteredSelectedStorages.map((s) => s.id).join(',')}</p>;
    }
    return filteredSelectedStorages.map((s) => (
      <span key={s.id} className={classNames(styles.storage, 'cp-limit-mounts-input-tag')}>
