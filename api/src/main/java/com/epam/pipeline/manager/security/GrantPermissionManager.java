...
@Service public class GrantPermissionManager {
    ...
    public boolean listedStoragePermissions(List<DataStorageAction> actions) {
        for (DataStorageAction action : actions) {
            ...
            if ((action.getReadVersion() || action.getWriteVersion()) && !isOwnerOrAdmin(storage.getOwner())) {
                return false;
            }
            if (action.getRead() && !permissionsHelper.isAllowed("READ", storage)) {
                return false;
            }
            if (action.getWrite() && !permissionsHelper.isAllowed("WRITE", storage)) {
                return false;
            }
        }
        return true;
