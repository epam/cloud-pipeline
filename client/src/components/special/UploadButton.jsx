...
class UploadButton extends React.Component {
  ...
  triggerClick = () => {
    if (this.uploadButtonRef.current) {
      const fileInput = this.uploadButtonRef.current.querySelector('input[type="file"]');
  ...
  onUploadStatusChangedSynchronous = async () => {
    ...
    const uploadFile = async (file) => {
      ...
      if (this.props.uploadToAzure) {
        ...
        const urlRequest = new DataStorageGenerateUploadUrl(this.props.storageId, targetPath);
    ...
    this.setState({uploadInfoVisible: true, uploadingFiles}, async () => {
  ...
  uploadItemToStorageSDK = (file) => {
    ...
    const onProgress = (percent) => {
      ...
      if (uFile && !uFile.aborted) {
        uFile.percent = uFile.done ? 100 : Math.min(100, Math.round(percent * 100));
        if (!uFile.done) {
    ...
    const setAbort = (abortFn) => {
      ...
      if (uFile) {
        uFile.cancelCb = () => {
          startAborting();
          if (abortFn) {
            abortFn().then(abort).catch(abort);
          }
        };
        this.setState({uploadingFiles});
      }
    ...
    uploadingFile.retryCb = () => {
      ...
      if (uFile) {
        ...
        uFile.status = 'uploading...';
        uFile.percent = 0;
        this.setState({uploadingFiles}, () => doUpload(uploadID, partNumber, multipartParts));
      }
    };
    return doUpload();
  };
  ...
  render() {
    ...
    } else if (this.props.synchronous || this.props.uploadToAzure || this.props.uploadToNFS) {
      uploadProps.beforeUpload = (file) => {
        const files = this.state.synchronousUploadingFiles;
        files.push(file);
        this.setState(
    ...
    return (
      <div ref={this.uploadButtonRef} style={{display: 'inline'}}>
        <Upload
          {...uploadProps}
          disabled={this.props.disabled || (this.props.uploadToS3 && !!this.s3StorageError)}
        ...
        >
          {this.props.uploadToS3 && this.s3StorageError && !this.props.uploadToNFS && (
            <Tooltip title={this.s3StorageError} trigger="hover">
              {button}
            </Tooltip>
          )}
          {this.props.uploadToS3 && !this.s3StorageError && !this.props.uploadToNFS && (
        ...
        >
          {this.state.uploadingFiles.map((f) => {
            ...
            return (
              <Row style={{marginBottom: 20, marginTop: 0}} type="flex" key={f.uid}>
                <Row type="flex" style={{width: '100%'}}>
                  <span>
                    {title}
                    {f.error && (
                      <span className="cp-error" style={{fontSize: 'small', marginLeft: 5}}>
                        - {f.error}
                      </span>
                    )}
                  </span>
