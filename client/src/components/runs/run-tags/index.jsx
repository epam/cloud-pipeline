...
import styles from './run-tags.module.css';
...
const isKnownTag = (tagName, preferences = {}) => {
  ...
  return mergePredefinedAndUserTags(PREDEFINED_TAGS, userTags).some(
...
const isKnownTagWithDateSuffix = (tagName, preferences = {}) => {
  ...
  const userTags = preferences.uiRunsTags || [];
  const knownTagsWithDateSuffix = mergePredefinedAndUserTags(PREDEFINED_TAGS, userTags).map(
    ({tag}) => `${tag}${suffix}`,
  );
...
const getDateInfo = (tags, tag, preferences) => {
  ...
  if (Object.hasOwn(tags, tagName)) {
    const since = toUtcDayjs(tags[tagName]);
    if (since.isValid()) {
      return since;
    }
  }
...
const skipTag = (tag, tags, preferences) => {
  return (
    `${tags[tag]}` === 'false' || /^alias$/i.test(tag) || isKnownTagWithDateSuffix(tag, preferences)
  );
};
...
function Tag({
  ...
  const tagOptions =
    predefinedTags.find(({tag}) => tag.toLowerCase() === tagName.toLowerCase()) || {};
  const isInstanceLink =
    instance &&
    instance.nodeName &&
...
function RunTagsComponent({
  ...
  const result = [];
  const predefinedTags = mergePredefinedAndUserTags(PREDEFINED_TAGS, preferences.uiRunsTags || []);
  const timestampTagHasCounterpart = (tagName) => {
    ...
    return (
      ...
      Object.hasOwn(tags, tagName.slice(0, tagName.length - suffix.length))
    );
  };
  ...
  for (const tagName in tags) {
    if (
      ...
      result.push({
        ...
        element: (
          <RunTagPopover date={info} value={value} key={tagName} tag={tagName}>
            <Tag
  ...
  const popover = (
    ...
    >
      <a className={styles.moreLabel}>+{result.length - tagsToDisplayCount} more</a>
    </Popover>
  );
  return (
    <div className={className} style={style}>
