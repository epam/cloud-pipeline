...
function LegendItem({color}) {
  return (
    <div className={styles.legend} style={{backgroundColor: color}}>
      {'\u00A0'}
    </div>
  );
}
...
function BillingTable({
  ...
  const {start, endStrict: end, previousStart, previousEndStrict: previousEnd} = filters;
  const {quota, previousQuota, values} = data || {};
  const renderQuotaColumn = showQuota && (quota || previousQuota);
  const lastValue = (values || []).filter((v) => v.value && v.initialDate <= end).pop();
  ...
  const renderWarning = (currentInfo = {}, previousInfo = {}) => {
    ...
    const containerClassNames = classNames(styles.warningContainer, {
  ...
  const renderInfo = (title, color, info, isCurrent) => {
    const dateClassNames = classNames(styles.date, {
      [styles.pending]: !info,
    ...
    const quotaClassNames = classNames(styles.value, {
      [styles.pending]: !info,
      'cp-billing-table-pending': !info,
    });
    return (
  ...
  return (
    <Export.ImageConsumer className={styles.container} order={0}>
      <table className={classNames(styles.table, 'cp-billing-table')}>
        <tbody>
          {renderQuotaColumn && (
            <tr>
              <td className={styles.borderless} colSpan={2}>
                {'\u00A0'}
              </td>
              <td>Quota</td>
