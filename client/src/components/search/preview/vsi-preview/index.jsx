...
import {Button, Tabs, Input, Popover, message} from 'antd';
import {
  ArrowsAltOutlined,
  CameraFilled,
  CopyOutlined,
...
function getFolderContents(storageId, folder) {
  return new Promise((resolve) => {
    const request = new DataStorageRequest(storageId, decodeURIComponent(folder), false, false, 50);
    request
      .fetchPage()
      .then(() => {
        if (request.loaded) {
          const pathRegExp = new RegExp(`^${escapeRegExp(folder)}\\/`, 'i');
          const items = ((request.value || {}).results || []).filter((item) =>
...
function getTilesFromFolder(storageId, folder, name) {
  return new Promise((resolve) => {
    getFolderContents(storageId, folder).then((tiles) => {
      const nameRegExp = new RegExp(`^${escapeRegExp(name)}\\.ome\\.tiff$`, 'i');
      const offsetsRegExp = new RegExp(`^${escapeRegExp(name)}\\.offsets\\.json$`, 'i');
      const omeTiff = (tiles || []).find(
        (aFile) => /^file$/i.test(aFile.type) && nameRegExp.test(aFile.name),
      );
      const omeTiffOffsets = (tiles || []).find(
        (aFile) => /^file$/i.test(aFile.type) && offsetsRegExp.test(aFile.name),
      );
      if (omeTiff) {
        resolve({
          folder,
      ...
      if (tiles && tiles.length > 0) {
        readStorageFileJson(storageId, `${folder}/info.json`).then((info) => {
          resolve({
            folder,
            info,
...
async function getPreviewConfiguration(storageId, file) {
  try {
    ...
    const {tilesFolders, name, parserFile} = info;
...
class VSIPreview extends React.Component {
  ...
  createS3Storage = () => {
    const wrapCreateStorageCredentials = (storage) =>
  ...
  reportPreviewLoaded = () => {
    const {onPreviewLoaded, onHideInfo} = this.props;
    const {tiles, omeTiff} = this.state;
    if (onPreviewLoaded) {
      onPreviewLoaded({
  ...
  fetchDeepZoomData = async (configuration, parserContent) => {
    const {file, storageId, preferences, dataStorageCache, dataStorages} = this.props;
    try {
      const {label} = parserContent || {};
      ...
      if (label) {
        labelRequest = (async () => {
          try {
            ...
            let os = await createObjectStorageWrapper(dataStorages.value || [], label);
            if (!os) {
              console.log(`Data storage not found for wsi label "${label}". Using current storage`);
              os = await createObjectStorageWrapper(dataStorages.value || [], storageId);
            }
            if (os) {
              return {
                storage: os.id,
      ...
      if (
        tagsRequest.loaded &&
        tagsRequest.value &&
        Object.hasOwn(tagsRequest.value, magnificationTagName)
      ) {
        const [, value = ''] = /([\d\\.\\,]+)/.exec(tagsRequest.value[magnificationTagName]) || [];
        if (value) {
  ...
  fetchSlideShowData = (folder) => {
    const {storageId, dataStorageCache} = this.props;
    getFolderContents(storageId, folder).then((items) => {
      const files = items
        ...
        .map((item) => ({
          ...item,
          preview: dataStorageCache.getDownloadUrl(storageId, item.path, undefined, true),
        }));
      this.setState(
        {
          items: files,
  ...
  fetchPreviewItems = () => {
    ...
    if (!file || !storageId || !dataStorageCache) {
      ...
      this.setState(
        ...
        () => {
          getPreviewConfiguration(storageId, file).then((data) => {
  ...
  renderSlides = () => {
    const {active, items = [], tiles, omeTiff} = this.state;
    if (tiles || omeTiff) {
      return null;
    }
    if (items.length === 0) {
  ...
  renderPreview = () => {
    const {active, preview, pending, items, tiles, omeTiff} = this.state;
    if (!preview || !items || !items.length || tiles || omeTiff) {
      return null;
    }
    const {pending: previewPending, error, url} = preview;
    let content;
    if (pending || previewPending) {
      ...
      content = <img src={url} alt={active} />;
    }
    return <div className={styles.vsiContentPreview}>{content}</div>;
  };
  ...
  saveImage = (data, opts) => {
    ...
    canvasElement.toBlob((blob) => {
      downloadBlob(blob, `${fileName}-${z}-${x}-${y}.png`);
  ...
  startSAViewerCameraUpdate = () => {
    ...
    const callback = () => {
      ...
      const {RollTarget: roll, ZoomTarget: zoom} = this.saViewer;
    ...
    this.saViewerCameraCallbackTimer = setInterval(callback, SA_CAMERA_CALLBACK_TIMEOUT);
  };
  ...
  generateShareUrl = () => {
    ...
    const {RollTarget: roll, ZoomTarget: zoom} = this.saViewer;
    const [x, y] = this.saViewer.MainView.Camera.GetWorldFocalPoint();
    const {storageId, file} = this.props;
    const query = [
      `storage=${storageId}`,
      `file=${file}`,
      `zoom=${zoom}`,
  ...
  renderShareUrl = () => {
    ...
    const copy = (e) => {
      e.stopPropagation();
      e.preventDefault();
      if (this.pathElement && this.pathElement.select) {
  ...
  renderLabelPreview = () => {
    const {showLabel, labelUrl} = this.state;
    if (showLabel && labelUrl) {
      return (
        <div className={styles.vsiLabelPreviewContainer}>
          <LabelPreview
  ...
  renderTiles = () => {
    ...
    const {tiles, shareUrl, labelUrl, showLabel} = this.state;
    if (!tiles || !storageId) {
      return null;
    }
    const initializeTiles = (element) => {
      if (element && !this.saViewer) {
        ...
        if (zoom || roll || x || y) {
          ...
          const center = [x || current[0] || 0, y || current[1] || 0];
          this.saViewer.AnimateCamera(center, rollTarget, zoomTarget);
        }
        setTimeout(() => this.startSAViewerCameraUpdate(), SA_CAMERA_CALLBACK_START_TIMEOUT);
    ...
    const capture = () => {
      ...
      const hideElements = this.saViewer
        .GetDiv()
    ...
    const icon = fullscreen ? (
      <ShrinkOutlined onClick={goFullScreen} className={styles.vsiPreviewFullscreenButton} />
    ) : (
      <ArrowsAltOutlined onClick={goFullScreen} className={styles.vsiPreviewFullscreenButton} />
    );
    return (
      <div
        onClick={() => {
        ...
        className={classNames(styles.vsiContentPreview, styles.tiles, {
          [styles.fullscreen]: fullscreen,
        })}
      >
        {}
        <div
          className={classNames(styles.vsiSaView, {
        ...
        <div className={styles.vsiPreviewButtonContainer}>
          <Button
  ...
  renderOmeTiff() {
    const {storageId, fullscreen, fullScreenAvailable, onFullScreenChange, children} = this.props;
    const {omeTiff} = this.state;
    if (!omeTiff || !storageId) {
      return null;
    }
    const {path, offsets} = omeTiff;
    const goFullScreen = () => {
      if (fullScreenAvailable && onFullScreenChange) {
        onFullScreenChange(!fullscreen);
      }
    ...
    return (
      <div className={classNames(styles.omeTiffContentPreview, {[styles.fullscreen]: fullscreen})}>
  ...
  render() {
    ...
    const {pending: loading, s3storageWrapperPending} = this.state;
