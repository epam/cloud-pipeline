...
import formStyles from './edit-hot-node-pool.module.css';
...
function compareSchedules(scheduleA, scheduleB) {
  ...
  const {from: startA, fromTime: startTimeA, to: endA, toTime: endTimeA} = scheduleA;
  const {from: startB, fromTime: startTimeB, to: endB, toTime: endTimeB} = scheduleB;
  return startA === startB && startTimeA === startTimeB && endA === endB && endTimeA === endTimeB;
}
...
function scheduleIsValid(schedule) {
  ...
  const {from, fromTime, to, toTime} = schedule;
...
function parse(date) {
  ...
  const localTime = dayjs.utc(date, 'HH:mm:ss').toDate();
...
function parseDay(day, date) {
  if (date && day) {
    const curr = dayjs.utc(date, 'HH:mm:ss').day();
    const act = dayjs.utc(date, 'HH:mm:ss').add(-new Date().getTimezoneOffset(), 'm').day();
...
function formatDay(day, date) {
  if (date && day) {
    const curr = dayjs.utc(date).day();
    const act = dayjs.utc(date).add(-new Date().getTimezoneOffset(), 'm').day();
...
class ScheduleControl extends React.Component {
  ...
  onChangeFromTime = (time) => {
    this.setState(
      {
        fromTime: formatTime(ensureDayjs(time)),
      },
      this.handleChange,
    );
  };
  ...
  onChangeToTime = (time) => {
    this.setState(
      {
        toTime: formatTime(ensureDayjs(time)),
      },
      this.handleChange,
    );
  };
  ...
  handleChange = () => {
    ...
    if (onChange) {
      onChange({
        ...
        fromTime: fromTime ? dayjs.utc(fromTime.valueOf()).format('HH:mm:ss') : undefined,
        to: formatDay(to, toTime),
        toTime: toTime ? dayjs.utc(toTime.valueOf()).format('HH:mm:ss') : undefined,
      });
    }
  };
    ...
    return (
      <div className={className} style={{justifyContent: 'space-between'}}>
        <div>
          ...
          >
            {Dates.map((date) => (
              <Select.Option key={date} value={date.toUpperCase()}>
                {date}
              </Select.Option>
            ))}
          </Select>
          <TimePicker
            className={classNames({'cp-error': invalid})}
            style={{marginLeft: 5}}
            value={ensureDayjs(fromTime)}
            format="HH:mm"
            onChange={this.onChangeFromTime}
          />
          <span className={formStyles.label} style={{textAlign: 'right'}}>
            Ends on:
          </span>
          <Select
            className={classNames({'cp-error': invalid})}
