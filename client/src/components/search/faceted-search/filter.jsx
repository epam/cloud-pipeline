...
class FacetedFilter extends React.Component {
  ...
  get filteredValues() {
    ...
    return this.values.filter((v) => v.name.toLowerCase().includes(searchString.toLowerCase()));
  }
  ...
  renderSearchInput = () => {
    ...
    return (
      <Input
        ...
        className={classNames(styles.searchInput, {[styles.optionHidden]: !filterGroupExpanded})}
        suffix={
          searchString ? (
            <CloseCircleOutlined onClick={this.clearSearch} className={styles.clearBtn} />
          ) : null
        }
      />
    );
  ...
  render() {
    const {className, disabled, name, showCounts = true} = this.props;
    const {searchString} = this.state;
    const {filtersExpanded, filterGroupExpanded} = this.state;
    ...
    return (
      <div className={classNames(styles.filter, 'cp-panel', 'cp-search-filter', className)}>
        <div
          className={classNames(styles.header, 'cp-search-filter-header', {
            'cp-search-filter-header-expanded': filterGroupExpanded,
          })}
          onClick={this.toggleFilterGroup}
        >
          <div className={classNames(styles.headerCaret, 'cp-search-filter-header-caret')}>
            <CaretRightOutlined />
          </div>
