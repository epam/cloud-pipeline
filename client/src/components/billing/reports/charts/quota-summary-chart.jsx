...
function QuotaSummaryChart({request, discountFn, data, quotaGroup, quotas}) {
  const {usage, quota, exceeds, quotaSummary} = getQuotaSummary({
    request,
    discount: discountFn,
    data,
    quotas,
  ...
  return (
    <Tooltip
      overlay={
        <div>
          <div>
            <b>Usage: {Math.round(usage * 100) / 100}$</b>
          </div>
    ...
    >
      <span
        className={classNames(styles.quotaSummaryInfo, {'cp-danger': exceeds})}
...
function QuotaSummaryChartsTitle({
  ...
  return (
    <div
      className={classNames(styles.quotaSummary, {
        'cp-danger': exceeded,
        'cp-text-not-important': !exceeded,
      })}
      style={style}
    >
      <span style={{cursor: 'pointer'}} onClick={onClick} className={styles.quotaSummaryTitle}>
        {children}
      </span>
      {displayQuotasSummary &&
        requestsWithDiscounts.map(({request, discount}, index) => (
          <QuotaSummaryChart
            key={`summary-${index}`}
            request={request}
            discountFn={discount}
