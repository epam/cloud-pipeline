  ...
  const stats = data.reduce(
    (acc, record) => {
      const CPUUtilization =
        record.totalCPUCount > 0 ? (record.activeCPUCount || 0) / record.totalCPUCount : 0;
      const GPUUtilization =
        record.totalGPUCount > 0 ? (record.activeGPUCount || 0) / record.totalGPUCount : 0;
      const RAMUtilization =
        record.totalMemoryCount > 0 ? (record.activeMemoryCount || 0) / record.totalMemoryCount : 0;
  ...
  onExport = () => {
    ...
    const rows = originalRecords
      ...
      .map((record) => {
        ...
        return [date, gpuUsage, gpuCapacity, cpuUsage, cpuCapacity, ramUsage, ramCapacity].join(
          ',',
        );
      });
    const csvContent = [header, ...rows].join('\n');
    const blob = new Blob([csvContent], {type: 'text/csv;charset=utf-8'});
    const fileName = `${data.poolName}-utilization-${dayjs().format('YYYY-MM-DD-HHmmss')}.csv`;
