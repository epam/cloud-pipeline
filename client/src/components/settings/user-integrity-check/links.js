...
function pathsMatch(path1, path2, end) {
  ...
  let matches = 0;
  const length = end !== undefined ? path1.pattern.indexOf(end) + 1 : path1.length;
...
function joinPaths(path1, path2) {
  ...
  return new Path(result, path1.pattern, path1.rows.concat(path2.rows));
}
...
function detachUserPath(path, user) {
  return [
    ...
    new Path(path, path.pattern, [user]),
  ];
}
...
function updatePath(path, updateObject) {
  for (let p = 0; p < path.pattern.length; p++) {
    if (Object.hasOwn(updateObject, path.pattern[p])) {
...
function update(trigger, data, dictionaries, paths) {
  const {id: identifier, key, value = ''} = trigger;
  const updateObject = updateLinkedValues(key, value, dictionaries);
  data[identifier] = Object.assign(data[identifier] || {}, updateObject);
  ...
  const affectedPaths = paths.filter((path) => path.testUser(identifier));
  const affectedUsers = [
    ...new Set(
      affectedPaths
        .map((path) => path.rows)
  ...
  for (let a = 0; a < affectedUsers.length; a++) {
    ...
    const userPaths = affectedPaths.filter((path) => path.testUser(userId));
...
function buildPatterns(dictionaries = []) {
  const rootDictionaries = dictionaries.filter(
    (dictionary) => dictionary.linksFrom.length === 0 && dictionary.linksTo.length > 0,
  );
...
function build(patterns, data = {}) {
  ...
  Object.entries(data).forEach(([identifier, dataItem]) => processDataItem(identifier, dataItem));
...
function getDefaultColumns(patterns, data = {}) {
  ...
  Object.values(data).forEach(processDataItem);
  return [...columns];
}
...
export {buildPatterns, build, generatePatternSorter, getDefaultColumns, update};
