...
`description`. `AGENTS.md` loads on every session instead; put facts and conventions there, and
multi-step procedures here.

`.agents/skills/` is the source of truth. Claude Code reads only `.claude/skills/`, which is a symlink
to this directory — so a new skill needs no linking step. The link is committed; if a checkout loses
it:

```bash
ln -s ../.agents/skills .claude/skills
.agents/skills/modifying-instructions/scripts/verify-instructions.sh   # checks it
```

**On Windows the link is what breaks.** Git checks a symlink out as a plain text file holding its
target unless it is allowed to create real ones, and nothing errors — Claude simply does not see
the skills, and the check above reports the path as "a regular file, not a symlink". Enable
symlink support once, then replace the file:
...
layout, the `SKILL.md` format, the `CLAUDE.md` twin, and which vendor enforces what.
