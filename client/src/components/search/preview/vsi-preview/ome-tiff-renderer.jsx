...
import styles from './ome-tiff-renderer.module.css';
...
class OmeTiffRenderer extends React.Component {
  ...
  fetchURLs = () => {
    const {storageId, omeTiffPath, offsetsJsonPath} = this.props;
  ...
  loadImage = () => {
    const {url, offsets} = this.state;
    if (this.hcsImageViewer && url && offsets) {
      this.hcsImageViewer.setData(url, offsets).then(() => {
        if (this.imagesAnnotations) {
  ...
  renderAttributes = () => {
    ...
    const {attributesVisible} = this.state;
    return (
      <div className={styles.attributes}>
        {!attributesVisible && (
          <Button size="small" onClick={showAttributes}>
            Show attributes
          </Button>
        )}
        {attributesVisible && (
          <div className={classNames('cp-panel', styles.panel)}>
  ...
  renderControls = () => {
    const {fullscreen, onChangeFullScreen, fullScreenAvailable} = this.props;
        ...
        {!controlsVisible && (
          <div>
            {fullScreenAvailable && typeof onChangeFullScreen === 'function' && (
              <Button size="small" onClick={onChangeFullScreen} style={{marginRight: 5}}>
                {fullscreen ? (
            ...
            <Button size="small" onClick={showControls}>
        ...
        {controlsVisible && (
          <div className={classNames('cp-panel', styles.panel)}>
            ...
            <div className={styles.content}>
              <ColorMap />
              <Channels allowLockChannels={false} />
              <OMETiffAnnotations />
            </div>
          </div>
        )}
  ...
  render() {
    ...
    return (
      ...
      >
        <div className={classNames(className, styles.omeTiffRendererContainer)} style={style}>
          <div
            className={classNames(styles.omeTiffRenderer, 'cp-dark-background')}
            ref={this.initialize}
          >
            {'\u00A0'}
          </div>
