...
import styles from './restarted-runs-info.module.css';

function RunLink({runId, regionId}) {
  return (
    <span className={styles.restartedRun}>
      <Link to={`/run/${runId}`}>
        <span className={classNames(styles.restartedRunLink, 'cp-run-nested-run-link')}>
          <b>#{runId}</b>
          {regionId && (
            <AWSRegionTag
              regionId={regionId}
...
class RestartedRunsInfo extends React.Component {
  ...
  componentDidUpdate(prevProps, prevState, snapshot) {
    if (prevProps.run !== this.props.run || prevProps.runId !== this.props.runId) {
      this.updateFromProps();
    }
  }

  updateFromProps = () => {
    const {run: runProps, runId} = this.props;
    this.fetchToken += 1;
    const token = this.fetchToken;
    const parseRunInfo = async (run) => {
      const {id: currentRunId, restartedRuns = []} = run;
      ...
      const parentRun = restartedRuns.find((aRun) => aRun.restartedRunId === currentRunId);
  ...
  render() {
    ...
    const {currentRun, restartedRuns = [], initialRun} = this.state;
    if (!currentRun) {
      return null;
    }
    const alerts = [];
        ...
        <Alert
          ...
          title={
            <div className={styles.restartedRunInfoAlertContent}>
              ...
              <RunLink runId={initialRun.id} regionId={initialRun.regionId} />
            </div>
          }
        />,
      );
          ...
          title={
            <div className={styles.restartedRunInfoAlertContent}>
              ...
              {restartedRuns.map((run) => (
                <RunLink key={run.id} runId={run.id} regionId={run.regionId} />
              ))}
            </div>
          }
        />,
