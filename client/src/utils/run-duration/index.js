 ...
 * @property {import('dayjs').Dayjs} start - interval start date
 * @property {import('dayjs').Dayjs} end - interval end date
 */
 ...
 * @property {import('dayjs').Dayjs} scheduledDate
 * @property {import('dayjs').Dayjs} runningDate
 */

/**
 * Creates interval
 * @param {number} phase - 0 - scheduled, 1 - running, 2 - paused, 3 - stopped
 * @param {import('dayjs').Dayjs | string} [start]
 * @param {import('dayjs').Dayjs | string} [end]
 * @returns {RunInterval}
 */
function getInterval(phase, start, end) {
  const startDate = start ? toUtcDayjs(start) : dayjs.utc();
  const endDate = end ? toUtcDayjs(end) : undefined;
  return {
    phase,
    start: startDate,
    end: endDate,
 ...
 * @param {import('dayjs').Dayjs | string | undefined} [end]
 * @returns {RunInterval}
 */
function updateIntervalEndDate(interval, end) {
  interval.end = end ? toUtcDayjs(end) : undefined;
  return interval;
}
...
export function getIntervalDuration(interval) {
  return (interval.end || dayjs.utc()).diff(interval.start, 'second', true);
}
...
function getRunningDuration(fromDate, intervals, ...phase) {
  ...
  const date = toUtcDayjs(fromDate);
 ...
 */
export default function getRunDurationInfo(run, analyseSchedulingPhase = false, tasks = []) {
  if (!run) {
    return undefined;
  }
