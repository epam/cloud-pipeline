...
class MyProjectsPanel extends localization.LocalizedReactComponent {
  ...
  searchProjectFn = (project, search) => {
    return (
      ...
      project.tags.filter((t) => (t.value || '').toLowerCase().indexOf(search.toLowerCase()) >= 0)
        .length > 0
    );
  };
  ...
  get projects() {
    if (this.props.projects.loaded && this.props.hiddenObjects.loaded) {
      return (this.props.projects.value.childFolders || [])
        .map((project) => {
          return {
            ...project,
            tags: (() => {
              const result = [];
              for (const key in project.data) {
                if (Object.hasOwn(project.data, key)) {
  ...
  renderProject = (project, search) => {
    const renderTag = (tag, index) => {
      return (
        <div className={styles.projectTag} key={index}>
          <Row className={classNames(styles.projectTagKey, 'cp-project-tag-key')}>{tag.key}</Row>
          <Row className={classNames(styles.projectTagValue, 'cp-project-tag-value')}>
    ...
    return [
      ...
      <Row key="tags" type="flex" justify="start">
  ...
  getProjectActions = (project) => {
    const history = ({id}) => {
      if (this.props.router) {
        this.props.router.push(`/folder/${id}/history`);
      }
    };
    const actions = [
      {
                ...
                return (
                  <Row key={index}>
                    <a
                      onClick={() => {
                        if (this.props.router) {
                          this.props.router.push(`/storage/${storage.id}`);
                        }
                      }}
                    >
                      {storage.name}
  ...
  renderContent = () => {
    const navigate = ({id}) => {
      if (this.props.router) {
        this.props.router.push(`/folder/${id}`);
      }
    };
    return (
