...
import {createAnsiUp} from '../../../utils/ansi-up';
...
import styles from './run-task-logs.module.css';
...
function markSearch(source, search, options = {}) {
  ...
  const positions = [];
  while (e) {
    const classes = classNames('cp-search-highlight-text', {
...
function findText(html, text, options = {}) {
  ...
  const {result: other, positions: otherPositions = []} = markSearch(html.slice(position), text, {
    lastIndex: index,
    currentIndex,
  });
...
function downloadCurrentLog(logs = '', fileName = 'logs') {
  downloadBlob(new Blob([logs]), `${fileName}.txt`);
}
...
async function downloadTaskLogs(
  ...
  const logs = (request.value || [])
    ...
    .map((log) => log.logText)
    .join('\n');
  const fileName = fileNameProp || [runId, task, 'logs.txt'].filter(Boolean).join('-');
  downloadBlob(new Blob([logs]), fileName);
}
...
class RunTaskLogs extends React.Component {
  ...
  get maxLinesToDisplay() {
    const {maxLinesToDisplay: maxLinesToDisplayProps = MAX_LINES_TO_DISPLAY} = this.props;
    const {maxLinesToDisplay = maxLinesToDisplayProps} = this.state;
  ...
  get currentLogs() {
    const {logs = [], searchResults = [], searchResultIndex} = this.state;
    const sliceFrom = Math.max(0, logs.length - this.maxLinesToDisplay);
    return logs.slice(sliceFrom).map((log, index) => {
  ...
  get scrolledDown() {
    if (this.consoleElement) {
      const {scrollTop, scrollHeight, clientHeight} = this.consoleElement;
  ...
  loadData = () => {
    ...
    if (logsProps && typeof logsProps === 'string' && !runId) {
      ...
      this.setState(
        ...
        () => {
          const request =
            taskName || fetchAllLogs
              ? new PipelineRunLog(runId, taskName, {
          ...
          const after = () => {
            ...
            this.setState(
              ...
              () => {
                ...
                } else if (!scrolledDown && currentLine && currentLine.index < firstLineIndex) {
                  this.scrollUp();
                }
              },
            );
  ...
  getCurrentLine = () => {
    if (this.consoleElement) {
      ...
      const {scrollTop, childNodes} = this.consoleElement;
      let lastVisible;
      let first;
  ...
  scrollToLine = (line, offset = 0) => {
    if (this.consoleElement) {
      const {searchControlVisible} = this.state;
      const {childNodes} = this.consoleElement;
      let lineElement;
      for (const child of childNodes) {
  ...
  onDownloadClicked = () => {
    ...
    downloadTaskLogs(runId, taskName, taskParameters, taskInstance, fileName);
  };
  ...
  registerSearchHotKeys = () => {
    this.unregisterSearchHotKeys();
    const {searchAvailable} = this.props;
    if (searchAvailable) {
  ...
  nextSearchResult = () => {
    ...
    this.setState(
      {
        searchResultIndex:
          searchResultIndex >= searchResults.length - 1 ? 0 : searchResultIndex + 1,
      },
  ...
  previousSearchResult = () => {
    ...
    this.setState(
      {
        searchResultIndex:
          searchResultIndex === 0 ? Math.max(0, searchResults.length - 1) : searchResultIndex - 1,
      },
      this.scrollToSearchResult,
    );
  };
  ...
  doSearch = () => {
    ...
    const searchResults = currentLogs
      .map((log) => {
        ...
        const {logHTML} = log;
        const {result, positions} = findText(logHTML, search, {currentIndex: -1});
  ...
  render() {
    ...
    const showExpandMore = !pending && this.logsTruncated;
    ...
    return (
      <div className={classNames(styles.container, className)} style={style}>
        <div
          ...
          className={classNames('cp-console-output', styles.console)}
        ...
        >
            ...
            <div className={styles.expandMore}>
              {showExpandMore && (
                <span>
                  ...
                  <a style={{marginLeft: 5}} onClick={this.onExpandClicked}>
                    Expand more
                  </a>
                </span>
              )}
              {showDownloadComplete && (
                <p>
                  ...
                  >
                    {showExpandMore ? 'download complete log' : 'Download complete log'}
                  </a>
                </p>
              )}
              {showDownloadCurrentLog && (
                <p>
                  {(showExpandMore || showDownloadComplete) && (
                    <span
                      style={{
                        marginLeft: 5,
                        marginRight: 5,
                  ...
                  <a onClick={this.onDownloadClicked}>
                    {showExpandMore || showDownloadComplete
          ...
          {logs.length === 0 && (
            <div
              className={classNames(styles.emptyLogs, {
                [styles.emptyLogs]: !pending,
                [styles.pendingContainer]: pending,
              })}
            >
              {pending ? <LoadingOutlined /> : <span>No data</span>}
            </div>
          )}
          {searchControlVisible && <div className={styles.searchPlaceholder} />}
          {logs.map((log) => (
            <div
            ...
            >
              {showLineNumber && (
                <span className={classNames(styles.consoleLineInfoData, styles.noAccent)}>
                  {formatLineNumber(log.index + 1, lineNumberLength)}:
                </span>
              )}
              {showDate && <span className={styles.consoleLineInfoData}>[{log.date}]</span>}
              <span
        ...
        {searchControlVisible && (
          <div className={classNames(styles.searchContainer, 'cp-panel cp-panel-borderless')}>
            <Input
            ...
            />
            <Button size="small" style={{marginLeft: 5}} onClick={this.previousSearchResult}>
              Prev
            </Button>
            <Button size="small" style={{marginLeft: 5}} onClick={this.nextSearchResult}>
              Next
            </Button>
            ...
            <Button size="small" onClick={this.closeSearch} style={{marginLeft: 'auto'}}>
              Close
            </Button>
            {logs.length > 0 && (
              <Checkbox
        ...
        {autoUpdate && logs.length > 0 && !searchControlVisible && (
          <div className={classNames('cp-console-follow-log', styles.followLogContainer)}>
            <Checkbox checked={followLog} onChange={this.onChangeFollowLog}>
              Follow log
            </Checkbox>
          </div>
        )}
        {!scrolledDown && (
          <div
            className={classNames('cp-console-scroll-down-indicator', styles.scrollDownContainer)}
