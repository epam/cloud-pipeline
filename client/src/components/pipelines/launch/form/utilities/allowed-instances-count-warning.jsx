...
export default class AllowedInstancesCountWarning extends React.Component {
  ...
  get instancesToLaunch() {
    ...
    const min = correctValue(nodeCount) + masterNode;
  ...
  fetchUserRuns = async () => {
    ...
    if (!this.userRunsCount) {
      try {
        ...
        this.userRunsCount = new UserRunCount(userName, ['RUNNING', 'PAUSING', 'RESUMING'], true);
      } catch (e) {
  ...
  fetchData = async () => {
    ...
    if (limitsRequest.error) {
      ...
      if (limitsRequest.loaded) {
        const anyValue = Object.values(limitsRequest.value || {}).pop();
  ...
  render() {
    ...
    return (
      <div>
        <Alert title={this.warningMessage} type="warning" showIcon style={style} />
      </div>
    );
  }
}
