...
@inject(({dataStorages}, params) => {
  return {
    dataStorageInfo: params.item && params.item.id ? dataStorages.load(params.item.id) : null,
    items:
      params.item && params.item.id
        ? new DataStorageRequest(params.item.id, null, false, false, PAGE_SIZE)
        : null,
  };
})
@observer
              ...
              return (
                <tr key={index}>
                  <td className={styles.firstCell}>
                    {item.type.toLowerCase() === 'folder' ? <FolderOutlined /> : <FileOutlined />}
      ...
      <div className={classNames(styles.container, 'cp-search-container')}>
        <div className={styles.header}>
          ...
          >
            ...
            {this.props.dataStorageInfo && this.props.dataStorageInfo.loaded && (
              <AWSRegionTag regionId={this.props.dataStorageInfo.value.regionId} displayName />
            )}
