...
import {Button, Input, Modal, Row, Spin, Alert, message, Popover} from 'antd';
...
import {shareStorageItem, getSharedStorageItemInfo} from '../../../../../utils/share-storage-item';
...
import styles from './SharedItemInfo.module.css';
...
const SidInfo = ({sid, style}) => {
  if (sid.principal) {
    return <UserName showIcon userName={sid.name} style={style} />;
  }
  return (
    <span style={style}>
      <TeamOutlined />
...
class SharedItemInfo extends React.Component {
  ...
  updateFromProps() {
    const {dataStorages, shareItems = []} = this.props;
    const storageIds = [
        ...
        async () => {
          ...
          try {
            ...
            const info = CHECK_PREVIOUS_PERMISSIONS
              ? await getSharedStorageItemInfo(this.props.preferences, storage, rootPath)
              : undefined;
            if (info) {
              const {url, permissions = {}, permissionsModificationAllowed} = info;
              const {mask = 0, permissions: usersToShare = []} = permissions;
              ...
              newState.usersToShare = parseGroups(groups).map((g) => ({name: g, principal: false}));
            }
  ...
  get itemName() {
    ...
    const {storage, rootPath: path} = this.state;
  ...
  onShare = async (options = {}) => {
    const {mask = 0, sids = []} = options;
    const {preferences} = this.props;
    const {rootPath: path, storage, itemLinks = []} = this.state;
    this.setState({
      pending: true,
    });
    const newState = {
    ...
    try {
      newState.sharedLink = await shareStorageItem(preferences, storage, path, itemLinks, {
        mask,
  ...
  renderShareInfo = () => {
    ...
    const {editPermissionsMode, usersToShare: sids = [], sharedLink} = this.state;
    ...
    if (sharedLink && sids.length > 0) {
      ...
      if (sids.length > MAX_SIDS_TO_PREVIEW) {
        info = (
            ...
            content={
              <div className={styles.sharedWithPopover}>
                {sids
                  ...
                  .map((sid) => (
                    <div key={`${sid.principal ? 'user' : 'group'}-${sid.name}`}>
                      <SidInfo sid={sid} />
                    </div>
                  ))}
              </div>
          ...
          >
            <a className={styles.noLink}>Folder shared with {sharedWith.join(' and ')}</a>
          </Popover>
        );
    ...
    return (
      <div className={styles.container}>
        ...
        {sharedLink && <span className={styles.hint}>Make sure you copy the link below.</span>}
        {sharedLink && <div style={{width: '100%'}}>{this.renderSharedLink()}</div>}
        <div className={styles.shareInfo}>{info}</div>
      </div>
    );
  };
  ...
  renderSharedLink = () => {
    ...
    return (
      <div className={styles.urlSection}>
        <Input value={sharedLink} readOnly style={{flex: 1}} size="large" />
        <Button
          type="primary"
  ...
  renderUserSelectForm = () => {
    const {editPermissionsMode, sharedLink, initialized, storage} = this.state;
  ...
  renderModalFooter = () => {
    const {editPermissionsMode, initialized, permissionsModificationAllowed, globalError} =
      this.state;
    ...
    if (!permissionsModificationAllowed) {
      return (
        <Row type="flex" justify="end">
          <Button onClick={this.closeShareDialog}>CLOSE</Button>
        </Row>
      );
    }
    return (
      <Row type="flex" justify="space-between">
        <Button onClick={this.closeShareDialog}>CLOSE</Button>
        <Button type="primary" onClick={this.onEditPermissions}>
          EDIT PERMISSIONS
        </Button>
      </Row>
    );
  ...
  render() {
    const {error, editPermissionsMode} = this.state;
    ...
    return (
      <Modal
        ...
        title={editPermissionsMode ? selectUsersTitle : defaultTitle}
      ...
      >
        <div>
          {error && <Alert type="error" title={error} style={{marginBottom: 5}} />}
