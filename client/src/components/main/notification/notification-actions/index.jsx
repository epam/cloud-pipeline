...
import styles from './notification-actions.module.css';
...
class NotificationActions extends React.Component {
  ...
  groupedActions = {
    [NOTIFICATION_TYPES.BILLING_QUOTA_EXCEEDING]: [ACTIONS.viewBilling],
    [NOTIFICATION_TYPES.IDLE_RUN]: [ACTIONS.viewRun, ACTIONS.pauseRun, ACTIONS.stopRun],
    [NOTIFICATION_TYPES.IDLE_RUN_PAUSED]: [
      ACTIONS.viewRun,
      ACTIONS.resumeRun,
      ACTIONS.terminateRun,
    ],
    [NOTIFICATION_TYPES.IDLE_RUN_STOPPED]: [ACTIONS.viewRun],
    [NOTIFICATION_TYPES.HIGH_CONSUMED_RESOURCES]: [ACTIONS.viewRun, ACTIONS.viewNodeMonitor],
    [NOTIFICATION_TYPES.LONG_INIT]: [ACTIONS.viewRun, ACTIONS.stopRun],
    [NOTIFICATION_TYPES.LONG_PAUSED]: [ACTIONS.viewRun, ACTIONS.resumeRun, ACTIONS.terminateRun],
    [NOTIFICATION_TYPES.LONG_PAUSED_STOPPED]: [ACTIONS.viewRun],
    [NOTIFICATION_TYPES.LONG_RUNNING]: [ACTIONS.viewRun, ACTIONS.pauseRun, ACTIONS.stopRun],
    [NOTIFICATION_TYPES.LONG_STATUS]: [ACTIONS.viewRun],
    [NOTIFICATION_TYPES.PIPELINE_RUN_STATUS]: [ACTIONS.viewRun],
    [NOTIFICATION_TYPES.DATASTORAGE_LIFECYCLE_RESTORE_ACTION]: [ACTIONS.openDatastorage],
    [NOTIFICATION_TYPES.STORAGE_QUOTA_EXCEEDING]: [ACTIONS.openDatastorage],
    [NOTIFICATION_TYPES.DATASTORAGE_LIFECYCLE_ACTION]: [ACTIONS.openDatastorage],
    [NOTIFICATION_TYPES.FULL_NODE_POOL]: [ACTIONS.openPoolsUsage],
  };
  ...
  get actions() {
    ...
    const entityValue =
      this.entityInfoRequest && this.entityInfoRequest.loaded
        ? this.entityInfoRequest.value
        : undefined;
    return actions.filter(Boolean).filter(({available}) => available(entityValue, preferences));
  }
  ...
  render() {
    const {pending, notification, style} = this.props;
