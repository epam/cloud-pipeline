...
export default class GenerateUserTokenModal extends React.Component {
  ...
  get jwtTokenDateTo() {
    ...
    if (seconds > 0) {
      return dayjs().add(seconds, 'second').endOf('day');
    }
    return undefined;
  }
  ...
  disabledDate = (date) => {
    ...
    if (jwtTokenDateTo && d.isAfter(jwtTokenDateTo)) {
      return true;
    }
    return false;
  };
  ...
  resetForm = () => {
    ...
    (async () => {
      ...
      this.setState({
        validTill: this.jwtTokenDateTo || dayjs().add(1, 'month'),
      });
    })();
  };

  onValidTillChanged = (date) => {
    const validTill = ensureDayjs(date);
    if (validTill && validTill.isBefore(dayjs().startOf('day'))) {
  ...
  handleOk = async () => {
    ...
    try {
      ...
      let seconds = expiration.diff(dayjs(), 'second');
