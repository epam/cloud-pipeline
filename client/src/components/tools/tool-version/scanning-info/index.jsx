...
export default class ToolScanningInfo extends React.Component {
  ...
  checkToolPlatform() {
    if (/^windows$/i.test(this.toolPlatform)) {
      const {router, toolId, version} = this.props;
      if (router) {
  ...
  get toolPlatform() {
    ...
    if (versions.loaded && versions.value && versions.value.attributes) {
          ...
          if (item.isFeature) {
            ...
            if (item.description) {
              nameComponentWithDesciption = (
                <Popover
                  placement="right"
                  content={<div style={{width: 300}}>{item.description}</div>}
                >
      ...
      },
      {
        ...
        render: (severity, item) => {
          if (item.isFeature && item.info) {
            ...
            return (
              <span className={classNames('cp-scan-result', severity).toLowerCase()}>
                {severity}
              </span>
            );
          } else {
  ...
  render() {
    ...
    if (
      (!this.props.versions.loaded && this.props.versions.pending) ||
    ...
    if (!roleModel.readAllowed(this.props.tool.value)) {
      return <Alert type="error" title="You have no permissions to view tool details" />;
    }
    if (/^windows$/i.test(this.toolPlatform)) {
      return null;
    }
