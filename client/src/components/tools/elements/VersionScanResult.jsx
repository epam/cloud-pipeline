...
import styles from './VersionScanResult.module.css';
...
export default class VersionScanResult extends React.Component {
  ...
  render() {
    ...
    const getDataItem = (name) => {
      if (Object.hasOwn(this.props.result, name)) {
        return {
          name,
    ...
    const renderInfo = (item) => {
      ...
      return (
        <div
          key={name}
          className={classNames(styles.containerItem, 'cp-scan-result', 'bar', name)}
          style={{
            flex: value / total,
          }}
        >
