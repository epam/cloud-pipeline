...
import {Alert, Button, Input, Modal, Row, Table} from 'antd';
...
export default class SystemParametersBrowser extends Component {
  ...
  get currentParameters() {
    const checkUserRoles = (parameter) => {
      if (!parameter.roles || !parameter.roles.length || this.isAdmin) {
        return true;
      }
      return parameter.roles.some((roleName) =>
        this.authenticatedUserRolesNames.includes(roleName),
      );
    };
    const needToShow = (parameter) => {
      return !(
        this.props.notToShow &&
    ...
    const parameterMatches = (parameter) => {
      ...
      return (
        parameter.name.toLowerCase().includes(this.state.searchString.toLowerCase()) ||
  ...
  renderParametersTable = () => {
    const {runDefaultParameters, authenticatedUserInfo} = this.props;
          ...
          if (parameter.description) {
            return (
              <Row>
                ...
                >
                  {this.isParameterSelected(parameter) && <CheckCircleFilled style={{width: 20}} />}
                  {name}
                </Row>
                <Row
                  style={{
