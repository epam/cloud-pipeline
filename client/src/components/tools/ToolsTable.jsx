...
export default class ToolsTable extends React.Component {
  ...
  renderTool = (tool, index, arr, isGlobalSearch = false) => {
    const renderLabel = (label, index) => {
      return (
        <span key={index} className={classNames(styles.toolLabel, 'cp-tag')}>
    ...
    return (
      <Row
        className={classNames(styles.toolRow, 'cp-panel-card', {
      ...
      >
        ...
        {!isGlobalSearch && (
          <Row type="flex" justify="end" style={{padding: '0 5px'}}>
            ...
            >
              <MessageOutlined />
              {tool.issuesCount > 0 ? ` ${tool.issuesCount}` : undefined}
            </Button>
          </Row>
        )}
      </Row>
