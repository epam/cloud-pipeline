...
class PickUserModal extends React.Component {
  ...
  get users() {
    const {usersInfo, exclude = []} = this.props;
  ...
  render() {
    const {className, style, visible, onClose, usersInfo} = this.props;
    ...
    return (
      <Modal
        ...
        footer={
          ...
          >
            ...
            <Button type="primary" disabled={!selection} onClick={this.onSelectClicked}>
              Select
            </Button>
          </div>
        }
      >
        <Select
          ...
          notFoundContent={
            filter && filter.length > 2 ? 'Not found' : 'Start typing to filter users...'
          }
        >
