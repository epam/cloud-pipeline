...
function MarkedToBeBlockedInfo({className, style, externalBlockDate, preferences}) {
  if (!externalBlockDate) {
    return null;
  }
  const markedDate = toUtcDayjs(externalBlockDate);
  ...
  if (markedDate && markedDate.isValid()) {
    ...
    const blockedDay = markedDate.add(gracePeriodDays, 'day');
    infos.push(
      <div key="block">
