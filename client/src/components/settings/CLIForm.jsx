...
export default class CLIForm extends React.Component {
  ...
  get hasWritableNFSStorages() {
    if (this.props.dataStorages.loaded) {
      return (
        (this.props.dataStorages.value || []).filter(
          (s) => s.type.toLowerCase() === 'nfs' && roleModel.writeAllowed(s),
        ).length > 0
      );
    }
    return false;
  ...
  get jwtTokenDateTo() {
    ...
    if (seconds > 0) {
      const now = dayjs();
      return now.add(seconds, 'second').endOf('day');
    }
    return undefined;
  }
  ...
  recalculateDefaultValidTillDate = () => {
    (async () => {
      try {
        ...
        const dt = this.jwtTokenDateTo || dayjs().add(1, 'month');
        this.setState({
          cli: {
            validTill: dt,
            accessKey: null,
  ...
  renderPipeCLIContent = (preferences) => {
    const getSettingsValue = (key) => {
      if (preferences.loaded && preferences.getPreferenceValue(key)) {
    ...
    let cliConfigureCommand, pipInstallCommand, operationSystems, operationSystem;

    if (preferences.pending && !preferences.loaded) {
      ...
      let pipInstallCommandTemplate = preferences.replacePlaceholders(
        getSettingsValue('ui.pipe.cli.install.template') || generatePipInstallUrl() || '',
      );
      try {
        ...
        operationSystems = [];
        for (const os in pipInstallCommandTemplateJson) {
          if (Object.hasOwn(pipInstallCommandTemplateJson, os)) {
            operationSystems.push(os);
          }
        }
        if (
          this.state.operationSystems &&
          ...
          const index = operationSystems
            .map((o) => o.toLowerCase())
            .indexOf(detectedSystem.toLowerCase());
          if (index >= 0) {
      ...
      let cliConfigureCommandTemplate =
        getSettingsValue('ui.pipe.cli.configure.template') || generateCliConfigureCommand() || '';
      ...
      cliConfigureCommandTemplate = preferences.replacePlaceholders(
        cliConfigureCommandTemplate.replace(/\{user\.jwt\.token}/g, this.state.cli.accessKey || ''),
      );
      cliConfigureCommand = (
        <BashCode
      ...
      <div>
          ...
          <Row type="flex" align="middle">
            ...
            >
              {operationSystems.map((system) => {
                return <Select.Option key={system}>{system}</Select.Option>;
              })}
            </Select>
          </Row>
        )}
        ...
        {accessKey && (
          <Row style={{marginTop: 10}}>
            <b>Access key: </b>
            <BashCode id="access-key" className={styles.mdPreview} code={accessKey} copyable />
          </Row>
        )}
        {accessKey && (
          <Row style={{marginTop: 10}}>
        ...
        <UserTokensTable refreshToken={this.state.refreshTableToken} />
  ...
  renderVscodeContent = (preferences) => {
    const template = preferences.uiVscodeExtensionInstallTemplate || {};
    ...
    const mdWithPlaceholders = preferences.replacePlaceholders(md);
    return (
      <div>
        {keys.length > 1 && (
        ...
        <Markdown md={mdWithPlaceholders} target="_blank" />
      </div>
    );
  };

  renderGitCLIContent = (preferences, pipelineGitCredentials) => {
    if (pipelineGitCredentials.pending && pipelineGitCredentials.loaded) {
      return <LoadingView />;
    }
    if (pipelineGitCredentials.error) {
      return <Alert type="error" title={pipelineGitCredentials.error} />;
    }
    const {email, userName, url, token} = pipelineGitCredentials.value;
    const getSettingsValue = (key) => {
      if (preferences.loaded && preferences.getPreferenceValue(key)) {
        return preferences.getPreferenceValue(key);
      }
    ...
    return (
      <BashCode id="git-cli-configure-command" className={styles.mdPreview} code={code} copyable />
    );
  };

  renderDrive = (preferences, dataStorages) => {
    const hasWritableNFSStorages =
      dataStorages.loaded &&
      (dataStorages.value || []).some(
        (s) => s.type.toLowerCase() === 'nfs' && roleModel.writeAllowed(s),
      );
    if (!hasWritableNFSStorages) {
      return (
        <Row>
          <Alert
            type="info"
            title={
              <div>
                <center>
                  <b>No supported data storage available</b>
                </center>
                <center style={{marginTop: '10px'}}>
                  <b>Drive mapping</b> feature allows to mount a cloud data storage to your local
                  workstation and manage files/folders as with any general hard drive
                </center>
                <center>
                  Currently this is supported only for the <b>NFS data storages</b>, other types are
                  NOT supported
                </center>
                <center style={{marginTop: '10px'}}>
                  Please make sure that you have <b>READ/WRITE</b> access to any{' '}
                  <b>NFS data storage</b> or ask support team to assist with a creation of such
                  storage
                </center>
              </div>
            }
    ...
    let fileBrowserApp = {};
    try {
      driveMapping = JSON.parse(preferences.getPreferenceValue(DRIVE_MAPPING_KEY) || '{}');
    } catch (___) {}
    try {
      fileBrowserApp = JSON.parse(preferences.getPreferenceValue(FILE_BROWSER_KEY) || '{}');
    } catch (___) {}
    ...
    if (!this.state.driveMapping.accessKey) {
      (async () => {
        const duration =
          preferences.getPreferenceValue('launch.jwt.token.expiration') || 60 * 60 * 24; // 1 day
    ...
    const loadCode = (config, key) => {
      let code = preferences.replacePlaceholders(config);
      if (code && code.indexOf('{user.jwt.token}') >= 0) {
        code = code.replace(/\{user\.jwt\.token}/g, this.state.driveMapping.accessKey || '');
      }
      return (
        <BashCode
          ...
          loading={!code}
          className={styles.mdPreview}
          code={preferences.replacePlaceholders(code)}
          copyable
        />
      );
    };
    if (driveMappingConfig) {
      ...
      content = parseDriveMappingConfig(driveMappingConfig).map(renderSingleConfig).filter(Boolean);
      if (content.length === 0) {
        content = undefined;
      }
    }
    ...
    return [
      ...
      content && <Row key="drive-mapping-content">{content}</Row>,
      fileBrowserContent && (
        <Row key="file-browser-application-header" style={{margin: '10px 0'}}>
          <b style={{fontSize: 'larger'}}>File Browser Application</b>
        </Row>
      ),
      fileBrowserContent && <Row key="file-browser-application">{fileBrowserContent}</Row>,
    ].filter(Boolean);
  };

  getSections = (preferences, pipelineGitCredentials, dataStorages) => {
    const sections = [];
    sections.push({
      key: CLI_KEY,
      title: 'Pipe CLI',
      render: () => this.renderPipeCLIContent(preferences),
    });
    sections.push({
      key: GIT_CLI_KEY,
      title: 'Git CLI',
      render: () => this.renderGitCLIContent(preferences, pipelineGitCredentials),
    });
    if (this.driveMappintAuthUrl) {
      sections.push({
        key: DRIVE_KEY,
        title: 'File System Access',
        render: () => this.renderDrive(preferences, dataStorages),
      });
    }
    ...
    if (
      ...
      sections.push({
        ...
        render: () => this.renderVscodeContent(preferences),
      });
    }
    return sections;
  };
  ...
  render() {
    const {preferences, pipelineGitCredentials, dataStorages} = this.props;
    return (
      <SubSettings sections={this.getSections(preferences, pipelineGitCredentials, dataStorages)} />
    );
  }
}
