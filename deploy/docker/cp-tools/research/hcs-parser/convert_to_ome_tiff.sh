...
RAW_IMAGE_DIR="$IMAGE_PREVIEW_DATA_ROOT/$SEQUENCE_ID/data.raw/"
OME_TIFF_IMAGE_PATH="$IMAGE_PREVIEW_DATA_ROOT/$SEQUENCE_ID/data.ome.tiff"
...
bioformats2raw "$INDEX_FILE_PATH" "$RAW_IMAGE_DIR"
if [ $? -ne 0 ]; then
...
raw2ometiff "$RAW_IMAGE_DIR" "$OME_TIFF_IMAGE_PATH"
if [ $? -ne 0 ]; then
