...
import styles from './correct-inputs-modal.module.css';
...
class CorrectInputsModal extends React.Component {
  ...
  updateFromProps = () => {
    const {inputs = [], availableInputs = []} = this.props;
    this.setState({
      correction: inputs
        .map((input) => ({
  ...
  setCorrectionValue = (input) => (value) =>
    this.setState({
      correction: {
  ...
  render() {
    const {className, style, visible, onCancel, inputs, availableInputs} = this.props;
    const {correction} = this.state;
    ...
    return (
      <Modal
        ...
        footer={
          <div className={styles.footer}>
            ...
            <Button type="primary" onClick={this.onCorrectClicked}>
              CORRECT
            </Button>
          </div>
        }
      >
        <Alert
          ...
          title={
            <p>
              There are inputs (image channels) that are not available for the current image.
              <br />
              Pick the available channel or leave it blank (you won't be able to submit analysis
              until correct values are provided).
            </p>
          }
        />
        {inputs.map((input) => (
          <div key={input} className={styles.inputRow}>
