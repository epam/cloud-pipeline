...
import styles from './hcs-video.module.css';
...
function HcsVideoPlayer({className, style, videoSource}) {
  if (videoSource) {
    ...
    if (videoSource.videoError) {
      return (
        <div className={classNames(styles.hcsVideoContainer, className)} style={style}>
          <Alert title={videoSource.videoError} type="error" />
        </div>
      );
    }
    ...
    const onLoadStart = () =>
      typeof videoSource.videoAccessCallback === 'function'
