 ...
 * @returns {boolean}
 */
function autoUpdateConfigurationsEqual(configurationA, configurationB, compareTokens = true) {
  const {
    enabled: enabledA,
...
function prepareRun(item, childrenRunsCollection, isChildRun = false) {
  const {id, pipelineRunParameters = [], childRunsCount = 0} = item;
  let parentRunId;
  let children;
  ...
  if (!isChildRun && childRunsCount > 0) {
    const collection = childrenRunsCollection.find(
      (childRuns) => childRuns.parentId === Number(id),
    );
    if (!collection) {
      children = [
        {
      ...
      const childRuns = (collection.childRuns || []).map((child) => prepareRun(child, [], true));
...
class RunTable extends localization.LocalizedReactComponent {
  ...
  componentDidUpdate(prevProps) {
    ...
    const {filters, searchFilters, search, autoUpdate} = this.props;
    if (
      search !== prevPropsSearch ||
      searchFilters !== prevPropsSearchFilters ||
      ...
      this.updateFromProps();
    } else if (!autoUpdateConfigurationsEqual(prevPropsAutoUpdate, autoUpdate, true)) {
      this.reload();
    }
  }
  ...
  fetchCurrentPage = () => {
    this.setState(
      ...
      async () => {
        this.finishFetching();
        const {page, filters: userFilters = {}, additionalFilters} = this.state;
        this.fetchToken += 1;
        ...
        const {enabled: autoUpdate, intervalSeconds: interval = DEFAULT_INTERVAL_SECONDS} =
  ...
  filtersChanged = (newFilters, callback) => {
    ...
    this.setState(newFilters, () => {
      ...
      if (!filtersAreEqual(currentFilters, filters)) {
  ...
  manageChildrenRunsCollection = () => {
    const {expandedRows = [], runs} = this.state;
    ...
    this.childrenRunsCollection.forEach((collection) => {
    ...
    const newRequestIds = corrected.filter(
      (id) => !this.childrenRunsCollection.some((collection) => collection.parentId === Number(id)),
    );
    const newRequests = newRequestIds.map((id) => new ChildRuns(Number(id)));
    this.childrenRunsCollection.push(...newRequests);
  ...
  fetchChildrenRunCollection = async (collection) => {
    ...
    const {childRunsCount = 0} = run || {};
    await collection.fetch(childRunsCount);
  };

  fetchChildrenRunCollections = () =>
    Promise.all(
  ...
  onExpandedRowsChanged = (expandedRows) => {
    const {expandedRows: currentExpandedRows = []} = this.state;
    if (!simpleArraysAreEqual(currentExpandedRows, expandedRows)) {
      this.setState({expandedRows}, async () => {
  ...
  renderCustomTable = (runs) => {
    ...
    return (
      <div
        className={classNames('runs-table', tableClassName, styles.customTable, {
          [styles.pending]: pending,
        })}
      >
        {runs.length === 0 && !pending && <Alert title={emptyDataMessage} type="info" />}
        {runs.map((run) => (
          <div
            key={run.id}
  ...
  renderTable = (runs) => {
    ...
    const {pending, expandedRows} = this.state;
    return (
      <Table
        className={classNames(styles.table, 'runs-table', tableClassName)}
        columns={[ExpandColumn, ...columns]}
        rowKey={(record) => record.id}
        rowClassName={(record) =>
          classNames(`run-${record.id}`, runRowClassName(record), styles.run, {
  ...
  renderAdditionalFilters = () => {
    ...
    return (
      <Row style={{gap: 5, padding: 2, marginBottom: 9}} align="middle" type="flex">
  ...
  render() {
    ...
    const {runs: rawRuns = [], page, total, pending, error} = this.state;
    ...
    return (
      <div className={className}>
        ...
        {total > 0 && total > pageSize && (
          <div className={classNames(paginationClassName, styles.pagination)}>
            <Pagination
              total={total}
              pageSize={pageSize}
              current={page + 1}
...
RunTable.propTypes = {
  ...
  disableFilters: PropTypes.oneOfType([PropTypes.bool, PropTypes.arrayOf(RunTableColumnPropType)]),
