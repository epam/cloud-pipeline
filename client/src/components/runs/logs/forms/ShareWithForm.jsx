...
import styles from './ShareWithForm.module.css';
import UserName from '../../../shared/user-name';
import ConfigureRunAsPermissions from './configure-run-as-permissions';
import {correctRunAsPermissions} from './configure-run-as-permissions/utilities';
...
export function shouldCombineRoles(sids, combinableRoles, accessType) {
  ...
  return combinableRoles.every(
    (roleName) =>
      !!sids.find(
        (role) => !role.isPrincipal && role.accessType === accessType && role.name === roleName,
      ),
  );
}

function excludeSidFromList(sids, sid) {
  const filterSids = ({name, isPrincipal}) => {
    if (sid.name === ROLE_ALL.name) {
      return !ROLE_ALL.includedRoles.includes(name);
    }
    return !(name.toLowerCase() === sid.name.toLowerCase() && isPrincipal === sid.isPrincipal);
  };
  return sids.filter(filterSids);
}

function correctActiveSid(sids, activeSid) {
  let corrected = activeSid;
  if (
    corrected &&
    !sids.some(
      (s) =>
        s.name.toLowerCase() === corrected.name.toLowerCase() &&
        s.isPrincipal === corrected.isPrincipal,
    )
  ) {
    corrected = sids[0];
  }
...
export default class ShareWithForm extends React.Component {
  static propTypes = {
    ...
    runAsUserConfiguration: PropTypes.bool,
  };
  ...
  get combineRolesIntoAllRoles() {
    ...
    return {
      ...
      endpoint:
        runSharing && shouldCombineRoles(sids, ROLE_ALL.includedRoles, AccessTypes.endpoint),
    };
  }
  ...
  findGroupDataSource = () => {
    ...
    const roles = this.state.groupSearchString
      ? this.props.roles
          ...
          .filter(
            (name) => name.toLowerCase().indexOf(this.state.groupSearchString.toLowerCase()) >= 0,
          )
      : [];
    if (this.groupFind && !this.groupFind.pending && !this.groupFind.error) {
      ...
      return set.sort((u1, u2) => sortByOverlap(u1, u2, query));
    }
  ...
  onSelectGroup = async () => {
    ...
    const [role] = this.props.roles.filter(
      (r) => !r.predefined && this.splitRoleName(r.name) === selectedGroup,
    );
  ...
  grantPermission = (name, isPrincipal) => {
    if (name) {
      ...
      if (!sidItem) {
        ...
        this.setState({sids, activeSid: {name, isPrincipal}});
      }
    } else {
  ...
  updateSid = (sid) => {
    const {sids: currentSids, activeSid: currentActiveSid} = this.state;
    const sids = excludeSidFromList(currentSids, sid);
    sids.push({...sid});
    const activeSid = correctActiveSid(sids, currentActiveSid);
    this.setState({sids, activeSid});
  };
  ...
  renderUsers = () => {
    ...
    const changeAccessLevel = (item) => (e) => {
      ...
      const updateSidAccess = (sid, currentSids) =>
        currentSids.map((s) => {
          if (
    ...
    const columns = [
      ...
      this.props.endpointsAvailable
        ? {
            ...
            key: 'ssh',
            render: (level, item) => (
              <Checkbox checked={level === AccessTypes.ssh} onChange={changeAccessLevel(item)}>
    ...
    const {activeSid} = this.state;
    const getRowClassName = (item) => {
      if (
        !this.hasDetailsConfiguration ||
        !activeSid ||
        activeSid.name !== item.name ||
        activeSid.isPrincipal !== item.isPrincipal
      ) {
        return styles.row;
      } else {
  ...
  renderUserConfiguration = () => {
    const {runAsUserConfiguration} = this.props;
    const {activeSid, sids} = this.state;
    if (!runAsUserConfiguration || !activeSid) {
      return null;
    }
    const sid =
      sids.find(
        (s) =>
          s.name.toLowerCase() === activeSid.name.toLowerCase() &&
          s.isPrincipal === activeSid.isPrincipal,
      ) || activeSid;
    const onChange = (updates) => this.updateSid(updates);
    return <ConfigureRunAsPermissions style={{marginTop: 10}} sid={sid} onChange={onChange} />;
  };

  onSave = () => {
    const {sids = []} = this.state;
    let payload = sids.slice();
    if (this.hasDetailsConfiguration) {
      payload = correctRunAsPermissions(payload);
    }
