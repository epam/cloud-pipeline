...
import styles from './commit-dialog.module.css';
...
class GitCommitDialog extends React.Component {
  ...
  get messageIsCorrect() {
    ...
    return lines.filter((line) => !line.startsWith('#')).some((line) => line.trim().length > 0);
  }

  render() {
    const {visible, storage, files, run, mergeInProgress} = this.props;
    const {commitMessage, commitInProgress, selectedFiles} = this.state;
    ...
    return (
      <Modal title={title} open={visible} footer={footer} width="80%" onCancel={this.onCancel}>
        <div className={styles.modalContent}>
          <div className={styles.inputContainer}>
            <label htmlFor="commit-message" className={styles.textareaLabel}>
              Commit message:
            </label>
            <Input.TextArea
              id="commit-message"
