...
import {Button, Col, Input, Checkbox, Row} from 'antd';
import {MinusCircleOutlined} from '@ant-design/icons';
import styles from './KubeLabels.module.css';
...
function kubeLabelsHasChanges(initialData = [], actualData = []) {
  ...
  return (
    !kubeLabelArraysEqual(initialData, actualDataFiltered) ||
...
class KubeLabels extends React.Component {
  ...
  get labels() {
    ...
    const keys = [...new Set((predefinedKeys || []).map((key) => key.trim()))].filter(Boolean);
  ...
  onRemoveLabel = (labelIndex) => {
    const filteredLabels = [...this.labels].filter((label, index) => index !== labelIndex);
    this.submitChanges(filteredLabels);
  };

  onLabelChange = (label, index, field) => (event) => {
    const value = label.predefined ? event.target.checked : event.target.value;
  ...
  validateLabels = (labels = []) => {
    ...
    for (let i = 0; i < labels.length; i++) {
      ...
      const hasDuplicates = labels.filter((filtered) => filtered.key === currentKey).length > 1;
      if (!currentKey) {
  ...
  renderLabelRow = (label, index) => {
    ...
    return (
      <Row
        ...
        align="top"
      >
        <Col span={8} style={{textAlign: 'right'}} className={styles.inputContainer}>
          <Input
            className={classNames(styles.labelInput, styles.keyInput, {'cp-error': hasError})}
            value={label.key}
            onChange={this.onLabelChange(label, index, 'key')}
          />
          {hasError ? (
            <span className={classNames(styles.error, 'cp-error')}>{errors[label.key]}</span>
          ) : null}
        </Col>
        <Col span={14} className={styles.inputContainer}>
          <Input
        ...
        <Col>
          <MinusCircleOutlined
            ...
            className={classNames('dynamic-delete-button', styles.removeButton)}
  ...
  renderPredefinedLabelRow = (label, index) => {
    return (
      <Row
        ...
        align="top"
      >
        <Checkbox checked={label.value} onChange={this.onLabelChange(label, index, 'value')}>
          {label.key}
        </Checkbox>
      </Row>
    );
