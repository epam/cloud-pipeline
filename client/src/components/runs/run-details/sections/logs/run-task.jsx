...
class RunTask extends Component {
  ...
  render() {
    const {task, searchText, active, className, style, onTaskClick} = this.props;
    ...
    if (this.props.timings) {
      if (this.props.task.created) {
        infos.push(
          <div key="scheduled" className={styles.timeInfo}>
            <span>
              <b>Scheduled:</b> {displayDate(this.props.task.created)}
            </span>
          </div>,
        );
      }
    ...
    return (
      ...
      >
        <div className={styles.runTaskTitle}>
          ...
          <span className={styles.runTaskName}>{highlightText(name, searchText)}</span>
        </div>
        {infos}
      </div>
    );
