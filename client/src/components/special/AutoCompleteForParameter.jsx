...
import styles from './AutoCompleteForParameter.module.css';
  ...
  handleSearch = (value) => {
    if (
      ...
      if (value.indexOf('this.') === 0) {
        let [currentRootEntity] = this.props.currentMetadataEntity.filter(
          (matadataEntity) => `${matadataEntity.metadataClass.id}` === this.props.rootEntityId,
        );
        ...
        if (parseValue.length === 2) {
          filteredEntityFields = (
            this.props.currentProjectMetadata ? Object.keys(this.props.currentProjectMetadata) : []
          )
            .filter(
              (metadata) =>
                metadata.toLowerCase().indexOf(parseValue[parseValue.length - 1].toLowerCase()) >=
                0,
            )
            .map((metadata) => {
              return {
