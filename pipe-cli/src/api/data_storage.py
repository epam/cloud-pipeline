...
class DataStorage(API):
    ...
    def load_from_uri(cls, path):
        url = urlparse(path)
        if url.scheme != 'cp' and url.scheme != 's3':
            raise RuntimeError('Supported schemes for datastorage are: "cp", "s3". '
                               'Actual scheme is "{}".'.format(url.scheme))
        return DataStorage.get(url.netloc), url.path[1:]
