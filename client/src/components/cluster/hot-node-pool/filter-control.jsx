...
const operators = [EQUAL, NOT_EQUAL, EMPTY, NOT_EMPTY];
...
function criteriaEqual(a, b) {
  ...
  if (/^RUN_PARAMETER$/i.test(aType) && /^RUN_PARAMETER$/i.test(bType)) {
    ...
    for (let i = 0; i < aKeys.length; i++) {
      const key = aKeys[i];
      if (!Object.hasOwn(bValue, key) || `${bValue[key]}` !== `${aValue[key]}`) {
        return false;
      }
    }
    return true;
...
class FilterControl extends React.Component {
  ...
  renderUsersSelection = () => {
    ...
    if (users.error) {
      return <span className="cp-danger">Error fetching users: {users.error}</span>;
    }
    ...
    return (
      <Select
        ...
        filterOption={(input, option) =>
          option.props.description.find(
            (d) => (d || '').toLowerCase().indexOf(input.toLowerCase()) >= 0,
          )
        }
      >
        {list.map((u) => (
          <Select.Option key={u.userName} value={u.userName} description={description(u)}>
  ...
  renderGroupSelection = () => {
    ...
    if (roles.error) {
      return <span className="cp-danger">Error fetching groups: {roles.error}</span>;
    }
  ...
  renderPipelineSelection = () => {
    ...
    if (pipelines.error) {
      return <span className="cp-danger">Error fetching pipelines: {pipelines.error}</span>;
    }
    return (
      <Select
        showSearch
  ...
  renderValueControl = () => {
    ...
    return (
      <Input disabled={disabled} value={value} onChange={this.onChangeValue} style={{flex: 1}} />
    );
  };
