...
class SearchResults extends React.Component {
  ...
  renderResultsItemActions = (resultItem) => {
    ...
    return (
      <div className="cp-search-result-item-actions">
        <InfoCircleOutlined
          className={classNames('cp-search-result-item-action', 'cp-icon-larger')}
          onClick={(e) => {
            if (!disabled) {
              if (e) {
                e.stopPropagation();
  ...
  renderSearchResultItem = (resultItem) => {
    ...
    return (
      ...
      >
        ...
        <div id={`search-result-item-${resultItem.elasticId}`} className={styles.resultItem}>
  ...
  renderPreview = () => {
    ...
    return <PreviewModal preview={preview} onClose={this.closePreview} />;
  };
  ...
  renderResultsList = () => {
    ...
    if (showResults && (documents || []).length === 0) {
      return <Alert type="info" title="Nothing found" />;
    }
    return (
  ...
  getGridTemplate = (headerTemplate) => {
    ...
    const rowHeight = headerTemplate ? TABLE_HEADER_HEIGHT : TABLE_ROW_HEIGHT;
    const cellDefault = '100px';
    const divider = '4px';
    const columnString = `'${columns.map(cellStringWithDivider).join(' ')}' ${rowHeight}px /`;
  ...
  onResize = (event) => {
    ...
    if (resizingColumn) {
      ...
      const minWidth = 50;
      if (rect.width + offset > maxWidth || rect.width + offset < minWidth) {
        return null;
      }
  ...
  itemSelected = (item) => {
    ...
    return (
      this.itemSelectionAvailable(item) &&
  ...
  itemSelectionAvailable = (item) => {
    const {dataStorageSharingEnabled, notDownloadableStorages, preferences} = this.props;

    return (
      (dataStorageSharingEnabled && elasticItemUtilities.itemSharingAvailable(item)) ||
  ...
  renderTableRow = (resultItem, rowIndex) => {
    ...
    return (
      <a
        href={!disabled && resultItem.url ? `${PUBLIC_URL || ''}/#${resultItem.url}` : undefined}
        className={classNames('cp-search-result-item', 'cp-search-result-table-item', {
          disabled,
        })}
        ...
        {columns.map(({key, className, renderFn}, index) => [
          ...
          >
            {renderFn ? (
              ...
              <span className={classNames('cp-ellipsis-text', className)}>{resultItem[key]}</span>
            )}
          </div>,
          <div
            className={classNames('cp-search-results-table-divider', {
  ...
  renderTableHeader = () => {
    ...
    const renderSortingIcon = (key) => {
      const currentIndex = sortingOrder.findIndex((sorting) => sorting.field === key);
      const currentSorting = sortingOrder[currentIndex];
      if (!currentSorting) {
        return null;
      }
      return (
        <span className={styles.sortingContainer}>
          {currentSorting.asc ? (
          ...
          {sortingOrder.length > 1 ? (
            <sup className={styles.sortingNumber}>{currentIndex + 1}</sup>
          ) : null}
        </span>
      );
    };
    return (
      ...
      >
        {columns.map(({key, name}, index) => [
          <div
            key={index}
            className={classNames('cp-ellipsis-text', 'cp-search-results-table-header-cell')}
            ref={(ref) => (this.dividerRefs[key] = ref)}
            style={{
              width: columnWidths[key],
              minWidth: '0px',
              cursor: ExcludedSortingKeys.includes(key) ? 'default' : 'pointer',
            }}
  ...
  renderResultsTable = () => {
    ...
    return (
      <div className={classNames(styles.tableContainer, 'cp-panel')} onBlur={this.stopResizing}>
        <InfiniteScroll
  ...
  render() {
    const {className, style, showResults, mode, loading} = this.props;
    const {preview} = this.state;
    if (!mode) {
      return null;
    }
    return (
      <div
        className={classNames(styles.container, {[styles.loading]: loading}, className)}
        style={style}
...
SearchResults.propTypes = {
  ...
  sortingOrder: PropTypes.arrayOf(
    PropTypes.shape({
