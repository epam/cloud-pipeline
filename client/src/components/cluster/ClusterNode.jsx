...
import parentStyles from './Cluster.module.css';
...
class ClusterNode extends Component {
  ...
  renderNodeLabels = () => {
    ...
    if (this.props.node.value && this.props.node.value.pipelineRun) {
      ...
      if (pipelineName) {
        labels[PIPELINE_INFO_LABEL] = `${pipelineName} (${version})`;
      } else if (dockerImage) {
        const parts = dockerImage.split('/');
        labels[PIPELINE_INFO_LABEL] = `${parts[parts.length - 1]}`;
      }
    }
    return generateNodeLabels(labels, {
  ...
  onResize = () => {
    ...
    const {show} = (this.labelRefs || []).reduce(
      (acc, label) => {
        if (label) {
  ...
  render() {
    ...
    let allowToTerminate =
      this.props.node.loaded &&
    ...
    return (
      ...
      >
        ...
        >
          ...
          <div style={{whiteSpace: 'nowrap'}} className={parentStyles.nodeMainInfo}>
