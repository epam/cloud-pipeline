...
function RunTimeline(props) {
  const {className, style, run, runTasks = [], showIcon, loaded = true} = props;
  if (!run) {
    return null;
  }
  const {status} = run;
  const info = getRunDurationInfo(run, true, runTasks);
  const {scheduledDate, runningDate, schedulingDuration, totalDuration} = info;
  const scheduled = {
    key: 'scheduled',
    title: 'Scheduled',
  ...
  return (
    <Popover content={details}>
      <div className={classNames(className, styles.runTimeline)} style={style}>
        {showIcon && <ClockCircleOutlined style={{marginRight: 5, fontSize: '0.75rem'}} />}
