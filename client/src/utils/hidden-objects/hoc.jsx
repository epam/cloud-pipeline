...
function HOC(...objects) {
  ...
  return function (WrappedComponent) {
    class Component extends React.Component {
      render() {
        const {props} = this;
        const {hiddenObjects, routing} = props;
        // Merge routing.params so identifier functions like
        // (props => props?.params?.id) keep working without withRouter.
        const propsWithParams = routing ? {...props, params: routing.params} : props;
        const objectsToCheck = objects.map(mapObjectConfiguration(propsWithParams));
        if (hiddenObjects && !hiddenObjects.loaded) {
          return null;
        }
        const hidden = objectsToCheck.find((o) => hiddenObjects.isHidden(o.type, o.identifier));
        if (hidden) {
          return <Alert type="warning" title="Access denied" />;
        }
...
export function checkObjectsWithParentHOC(type, parentOptional = false) {
  return function checkObjects(parent, ...objects) {
    ...
    if (parent && parentOptional) {
      objectsToCheck.push(...objects.map((o) => ({type, identifier: o})));
    }
