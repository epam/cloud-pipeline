...
class EditSection extends React.Component {
  ...
  validate = () => {
    return new Promise((resolve) => {
      ...
      const {readOnly, titleReadOnly, otherSectionNames = []} = this.props;
      let titleError;
  ...
  renderSectionItem = (item, index) => {
    ...
    return (
      <div className={styles.dataItem} key={`section-item-${index}`}>
        <InputWrapper
        ...
        />
        {editable && (
          <Button disabled={readOnly} onClick={onRemove} danger>
            <DeleteOutlined />
          </Button>
        )}
      </div>
  ...
  render() {
    const {className, style, titleReadOnly, readOnly, removable, onRemove, editable} = this.props;
    const {title, data = []} = this.state;
    return (
      <div className={classNames(className, styles.section, 'cp-divider', 'bottom')} style={style}>
        ...
        {editable && (
          <div className={classNames(styles.dataItem, styles.actions)}>
            <Button
              disabled={readOnly}
...
EditSection.propTypes = {
  ...
  data: PropTypes.arrayOf(
    PropTypes.oneOfType([KeyValuePropType, PropTypes.arrayOf(PropTypes.string)]),
  ),
