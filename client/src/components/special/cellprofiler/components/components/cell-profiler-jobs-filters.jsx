...
class CellProfilerJobsFilters extends React.Component {
  ...
  render() {
    const {className, style, filters = {}, onChange} = this.props;
    ...
    const renderHeader = (expanded) => {
      ...
      if (userNames.length > 1) {
        ...
        parts.push(
          <UserName key="owner" userName={userNames[0]} showIcon style={{marginRight: 5}} />,
        );
      }
      if (source && source.length) {
        parts.push(
          <span key="source" style={{marginRight: 5}}>
            <FileOutlined />
            {source}
          </span>,
        );
      ...
      return <span className="cp-ellipsis-text">Filters: {parts}</span>;
    };
    return (
      <Collapse className={className} style={style} header={renderHeader}>
