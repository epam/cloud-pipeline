...
import { getBrandName, getVsixUpdateUrl } from './extensionEnv';
...
async function checkForUpdate(vsixUrl: string): Promise<void> {
  ...
  try {
    ...
    const answer = await vscode.window.showInformationMessage(
      `A new version of the ${brand} extension is available (installed: ${EXTENSION_VERSION}.${COMPONENT_VERSION.slice(0, 8)}).`,
      'Update',
      'Later'
    );
    if (answer !== 'Update') {
    ...
    await downloadAndInstall(vsixUrl, brand);
  } catch {
    // best-effort — silent on network errors
  }
}

async function downloadAndInstall(vsixUrl: string, brand: string): Promise<void> {
  const tmpPath = path.join(os.tmpdir(), 'cloud-pipeline-remote-update.vsix');
  try {
    await vscode.window.withProgress(
      { location: vscode.ProgressLocation.Notification, title: `${brand}: Downloading update…`, cancellable: false },
      () => downloadFile(vsixUrl, tmpPath)
    );
  } catch (e) {
    const msg = e instanceof Error ? e.message : String(e);
    vscode.window.showErrorMessage(`${brand}: Failed to download update: ${msg}`);
    return;
  }
  ...
  try {
    ...
    const msg = e instanceof Error ? e.message : String(e);
    vscode.window.showErrorMessage(`${brand}: Failed to install update: ${msg}`);
    return;
  } finally {
    fs.unlink(tmpPath, () => {});
  }
  ...
  const answer = await vscode.window.showInformationMessage(
    `${brand} extension updated. Reload window to apply.`,
    'Reload'
  );
  if (answer === 'Reload') {
