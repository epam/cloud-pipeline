...
export default class SSHConnection extends EventEmitter {
  ...
  private activeTunnels: {
    [index: string]: SSHTunnelConfig & { server: Server };
  } = {};
  ...
  constructor(options: SSHConnectConfig) {
    ...
    this.config.uniqueId =
      this.config.uniqueId || `${this.config.username}@${this.config.host}`;
  }
  ...
  ): Promise<SSHTunnelConfig & { server: Server }> {
  ...
  closeTunnel(name?: string): Promise<void> {
    if (name && this.activeTunnels[name]) {
      ...
      const tunnels = Object.keys(this.activeTunnels).map((key) =>
