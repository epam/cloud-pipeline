...
export default class FolderPreview extends React.Component {
  ...
  get folderMetadataTags() {
    const matadataValue = {};
    for (const tag in this.props.folder.value.objectMetadata || {}) {
      if (Object.hasOwn(this.props.folder.value.objectMetadata, tag)) {
  ...
  renderItems = () => {
    ...
    const renderName = (item) => {
      ...
      if (
        item.type === SearchItemTypes.s3Bucket ||
        item.type === SearchItemTypes.gsStorage ||
        item.type === SearchItemTypes.azStorage
      ) {
        nameComponent = (
          <span>
          ...
          <Row>
            ...
            >
              {item.description && (
                <Row
                  style={
                    item.hasMetadata ? {marginBottom: 5, wordWrap: 'normal'} : {wordWrap: 'normal'}
                  }
    ...
    const mapStorageType = (item) => {
      ...
      switch ((item.type || '').toLowerCase()) {
        case 'az':
          type = SearchItemTypes.azStorage;
          break;
        case 's3':
          type = SearchItemTypes.s3Bucket;
          break;
        case 'nfs':
          type = SearchItemTypes.NFSBucket;
          break;
        case 'gs':
          type = SearchItemTypes.gsStorage;
          break;
        default:
          type = item.aclClass;
          break;
    ...
    const mapChild = (item) => ({
      type: item.aclClass.toLowerCase() === 'data_storage' ? mapStorageType(item) : item.aclClass,
      name: item.name,
    ...
    return (
      <div className={styles.contentPreview}>
        <table>
          <tbody>
            {(items.length > MAX_ITEMS ? items.slice(0, MAX_ITEMS) : items || []).map(
              (item, index) => {
