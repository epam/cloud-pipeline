...
def storage(name, mount_point, storage_id=None):
    data_storage = Mock(id=storage_id or abs(hash(name)), mount_point=mount_point, path=name,
                        storage_type='S3', mask=1, tools_to_mount=None, sensitive=False)
...
def record_audit_events(task):
    # Mock does not record calls made from several threads reliably
    events = []
    events_lock = threading.Lock()

    def log_audit_event(message, user, **kwargs):
        with events_lock:
            events.append((message, user, kwargs))

    task.api.log_audit_event = log_audit_event
    return events


def audit_event(mnt):
    return ('MOUNT ' + mnt.storage.path, 'owner',
            {'severity': 'INFO', 'type': 'audit', 'extra_args': {'storageId': mnt.storage.id}})


def test_sequential_mount_audits_mounted_storages(task):
    events = record_audit_events(task)
    storage_mounters = mounters(['/cloud-data/a', '/cloud-data/b'], behaviours={'/cloud-data/b': raise_runtime_error})
    task._mount_sequentially(storage_mounters, MOUNT_ROOT)
    assert events == [audit_event(storage_mounters[0])]


def test_parallel_mount_audits_mounted_storages(task):
    events = record_audit_events(task)
    storage_mounters = mounters(['/cloud-data/a', '/cloud-data/b', '/cloud-data/c'],
                                behaviours={'/cloud-data/b': raise_value_error})
    task._mount_in_parallel(storage_mounters, MOUNT_ROOT, 4)
    assert sorted(events) == sorted([audit_event(storage_mounters[0]), audit_event(storage_mounters[2])])
...
def test_mount_command_is_executed_with_the_start_lock(monkeypatch, logger, start_lock):
    ...
    with patch.object(mount_storage.common, 'execute_cmd_command', return_value=0) as execute:
        StorageMounter.execute_mount('mount-command', {'path': 'bucket', 'mount': '/cloud-data/bucket'}, TASK_NAME)
    execute.assert_called_once_with('mount-command', executable='/bin/bash', start_lock=start_lock)
...
def run_task(task, storages, recorder, behaviours=None):
    ...
    task.api.get_preference.return_value = {'value': None}
