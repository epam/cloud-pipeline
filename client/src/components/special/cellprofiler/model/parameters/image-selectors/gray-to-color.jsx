...
import styles from './image-selectors.module.css';

const VALUE_SUPPORTED_SCHEMES = ['Composite'];
...
class GrayToColorRenderer extends React.Component {
  ...
  render() {
    const {className, style, valueLabel = 'weight'} = this.props;
    if (!this.parameter) {
      return null;
    }
    return (
      <div className={className} style={style}>
        {this.filesToProcess.map((objectToOutline, index) => (
          <div className={styles.imageRow} key={`${objectToOutline.name || ''}_${index}`}>
            <Select
            ...
            {['Composite'].includes(this.scheme) && (
              <span style={{whiteSpace: 'pre'}}>{` ${valueLabel} `}</span>
            )}
            {['Composite'].includes(this.scheme) && (
              <Input
        ...
        <div>
          <Button size="small" onClick={this.addConfiguration}>
            Add image
          </Button>
        </div>
      </div>
