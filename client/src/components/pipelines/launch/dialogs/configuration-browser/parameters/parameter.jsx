...
class Parameter extends React.Component {
  ...
  render() {
    ...
    const {bucketBrowserVisible} = this.state;
    const {name, value, type, required, description, system, restricted, nameError, valueError} =
      parameter;
    ...
    const renderParameterName = () => {
      if (system || required || readOnly || disabled || !editable) {
        return <ParameterName error={!!nameError}>{name}</ParameterName>;
      }
      return (
        <Input
          value={name}
          ...
          return (
            <div className={className}>
              <ParametersAutoCompleteInput
                ...
                addonBefore={
                  <IconComponent style={{cursor: 'pointer'}} onClick={this.openBucketBrowser} />
                }
                style={{width: '100%'}}
              />
            </div>
    ...
    const renderRemoveButton = () => {
      ...
      return (
        <div className={styles.remove}>
          {removeAllowed && <MinusCircleOutlined className={styles.button} onClick={remove} />}
          {!removeAllowed && '\u00A0'}
        </div>
      );
    };
    return (
      ...
      >
        ...
        {(nameError || valueError) && (
          <ParameterRow error>
            <ParameterName>{nameError || '\u00A0'}</ParameterName>
            <ParameterValue>{valueError || '\u00A0'}</ParameterValue>
          </ParameterRow>
        )}
      </div>
    );
...
export {ParameterRow, ParameterName, ParameterValue};
