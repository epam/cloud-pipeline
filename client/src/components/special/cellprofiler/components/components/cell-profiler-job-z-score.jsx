...
import styles from './cell-profiler-job-z-score.module.css';
...
function getWellName(well) {
  const {column, row} = well;
  const columnName =
    !Number.isNaN(Number(column)) && Number(column) < 10 ? '0'.concat(column) : column;
...
class CellProfilerJobZScore extends React.Component {
  ...
  updateFromProps = () => {
    const {dataPath, dataStorageId, hcsFilePath, hcsFileStorageId} = this.props;
      ...
      this.setState(
        ...
        async () => {
          ...
          try {
            ...
            const {columns = [], rows = []} = await fetchContents(url);
            let plateWidth = 0;
            let plateHeight = 0;
            let allWells = [];
            ...
            if (!plateWidth || !plateHeight) {
              plateWidth = Math.max(
                0,
                ...wells.map((well) => Number(well.column)).filter((n) => !Number.isNaN(n)),
              );
              plateHeight = Math.max(
                0,
                ...wells.map((well) => Number(well.row)).filter((n) => !Number.isNaN(n)),
              );
            }
            ...
            const readOutParameters = columns.filter(
              (aColumn) => !nonReadOutParametersRegExp.test(aColumn),
            );
  ...
  calculateZScores = () => {
    ...
    const {wells = [], timePoints = [], zPlanes = []} = data;
    if (
      wells.length === 0 ||
    ...
    const filteredData = readOutData.filter(
      (item) =>
        (!checkTimePoint || item.timePoint === timePoint) &&
        (!checkZPlane || item.zPlane === zPlane),
    );
    const selectedWells = wells.filter(
      (aWell) => !negative || negativeWells.includes(aWell.identifier),
    );
    const statisticsData = filteredData.filter(
      (item) =>
        !!selectedWells.find((aWell) => aWell.column === item.column && aWell.row === item.row),
    );
    const mean =
      statisticsData.reduce((total, item) => total + item.value, 0) / statisticsData.length;
  ...
  readData = () => {
    const {readOutParameter, data = {}} = this.state;
    const {
      columns = [],
      rows = [],
      wellColumnIndex,
  ...
  renderNegativeControlToggle = () => {
    const {data, negative} = this.state;
    if (!data) {
      return null;
    }
    const {wells = []} = data;
    ...
    return (
      <div className={styles.negativeToggleContainer}>
        <Checkbox checked={negative} onChange={this.onChangeUseNegativeControlWell}>
  ...
  renderNegativeControlWellsSelector = () => {
    const {data, negative, negativeWells} = this.state;
    if (!data || !negative) {
      return null;
    }
    const {wells = []} = data;
    ...
    return (
      <div className={styles.wellsSelectorContainer}>
        <span className={styles.title}>Negative wells:</span>
        <Select
          mode="multiple"
  ...
  renderReadOutSelector = () => {
    ...
    const {readOutParameters = []} = data;
    if (!readOutParameters || readOutParameters.length === 0) {
      return null;
    }
    return (
      <div className={styles.readOutSelectorContainer}>
        <span className={styles.title}>Readout value:</span>
        <Select
          allowClear
          showSearch
          className={styles.input}
  ...
  renderValueSelector = (value, values = [], onChangeValue, title) => {
    if (values.length < 2) {
      return null;
    }
    const sequence = {
    ...
    const onChange = (newSelection) => {
      const newValue =
        newSelection && newSelection.length > 0 ? newSelection[0].timePoint : undefined;
  ...
  renderTimePointsSelector = () => {
    const {data, timePoint} = this.state;
    if (!data) {
      return null;
    }
    const {timePoints = []} = data;
    const onChange = (newTimePoint) =>
      this.setState(
        {
    ...
    return this.renderValueSelector(timePoint, timePoints, onChange, 'Time point');
  };

  renderZPlaneSelector = () => {
    const {data, zPlane} = this.state;
    if (!data) {
      return null;
    }
    const {zPlanes = []} = data;
    const onChange = (newZPlane) =>
      this.setState(
        {
          zPlane: newZPlane,
        },
    ...
    return this.renderValueSelector(zPlane, zPlanes, onChange, 'Z plane');
  };

  renderExportButton = () => {
    const {data, zScores, readOutParameter} = this.state;
    const {hcsFilePath, analysisDate, analysisName} = this.props;
    ...
    const hcsFileName = input
      ...
      .join('.')
      .replace(/\s/g, '_');
    const dateTime = analysisDate ? displayDate(analysisDate, 'YYYYMMDD_HHmmss') : undefined;
    const fileName = [
      hcsFileName,
      ...
      .filter(Boolean)
      .join('-');
    const {timePointIndex, zPlaneIndex} = data;
    const header = [
      'WellRow',
      'WellColumn',
    ...
    const handleDownload = () => {
      downloadBlob(new Blob([exportData]), `${fileName}-z-scores.csv`);
    };
    return (
      <div style={{marginTop: 10}}>
        <a onClick={handleDownload}>Download z-scores table</a>
      </div>
    );
  };

  renderPlate = () => {
    const {data, negative, negativeWells, zScores} = this.state;
    if (!data) {
      return null;
    }
    const {allWells, plateWidth, plateHeight} = data;
  ...
  onChangeGradientColors = (colors) => {
    const {positive = [1, 0, 0, 1], negative = [0, 0, 1, 1], zero = [1, 1, 1, 1]} = colors || {};
  ...
  renderContent = () => {
    ...
    return [
      <div key="plate" className={styles.plateContainer}>
        <div className={styles.plate}>{this.renderPlate()}</div>
      ...
      <div key="configuration" className={styles.configuration}>
  ...
  render() {
    const {className, style, dataPath, dataStorageId} = this.props;
    if (!dataPath || !dataStorageId) {
      return null;
    }
    return (
      <div className={classNames(className, styles.zScoreContainer)} style={style}>
        {this.renderContent()}
      </div>
    );
  }
