...
class CellProfiler extends React.Component {
  ...
  get analysisDisabled() {
    ...
    return (
      ...
      analysis.analysing ||
      analysis.isEmpty ||
      (analysis.batch && analysis.pipeline && analysis.pipeline.defineResultsAreEmpty) ||
      (analysis.pipeline && (analysis.pipeline.parametersWithErrors || []).length > 0)
    );
  }
  ...
  onNameSpecified = () => {
    ...
    const {asNew = false} = savePipelineOptions || {};
  ...
  renderTitle = () => {
    ...
    const {openPipelineModalVisible, savePipelineOptions} = this.state;
    const openModal = () => {
    ...
    const onPipelineSelected = (pipelineFile) => {
      ...
      if (analysis) {
        ...
        if (missingInputs.length && analysis.pipeline.channels.length > 0) {
          const inputs = [...new Set([...missingInputs, ...analysis.pipeline.channels])].sort(
            defaultSorter,
          );
          this.setState({
  ...
  openSimilarJob = (aJob) => {
    const {onOpenEvaluations} = this.props;
    this.setState(
      {
        similarJobs: [],
      },
  ...
  runAnalysis = (options = {}) => {
    const {ignoreSimilar = false} = options;
    const {analysis, onOpenEvaluations} = this.props;
    if (!analysis) {
      return;
    }
    if (analysis.batch) {
  ...
  toggleShowResults = () => {
    const {analysis, onToggleResults} = this.props;
    if (!analysis) {
      return null;
    }
  ...
  render() {
    const {analysis, className, style, resultsVisible, onOpenEvaluations} = this.props;
    if (!analysis) {
      return null;
    }
    const {similarJobs = [], missingInputsCorrection} = this.state;
    return (
      <div className={classNames(className, styles.cellProfiler)} style={style}>
        <div className={styles.cellProfilerHeader}>
          ...
          >
            <CaretRightOutlined />
            {analysis.pending && <LoadingOutlined />}
          </Button>
        ...
        {analysis.defineResultsOutputs.length > 0 && (
          <div className={classNames(styles.cellProfilerFooter)}>
            <Checkbox checked={resultsVisible} onChange={this.toggleShowResults}>
              Show results
            </Checkbox>
          </div>
        )}
        {(analysis.status || analysis.error) && (
          <div
            className={classNames(styles.cellProfilerFooter, {
