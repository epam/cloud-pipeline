...
| **4** | **Import users** | Allows to import users from the list in `.csv` format. More details see [here](12.3._Create_a_new_user.md#import-users). |
...
| **ROLE\_STORAGE\_READER** | User gets Read permission to all data storages in the Platform. See details [here](../13_Permissions/13._Permissions.md#storage-reader-role). |
...
| **FULL\_NODE\_POOL** | tells that all nodes of some node pool(s) are already in use. See details [here](12.9._Change_email_notification.md#full-node-pool) |
| **HIGH\_CONSUMED\_NETWORK\_BANDWIDTH** | tells that network consuming is higher than a bandwidth threshold for a specific period of time. See details [here](12.9._Change_email_notification.md#high-consumed-network-bandwidth-runs) |
| **HIGH\_CONSUMED\_RESOURCES** | tells that memory or disk consuming is higher than a threshold value for a specific period of time. See details [here](12.9._Change_email_notification.md#high-consumed-runs) |
| **IDLE\_RUN** | tells that the job is idle for a long time. See details [here](12.9._Change_email_notification.md#idle-runs) |
| **IDLE\_RUN\_PAUSED** | tells that the job was paused because it was idle for a long time. See details [here](12.9._Change_email_notification.md#idle-runs) |
| **IDLE\_RUN\_STOPPED** | tells that the job was stopped because it was idle for a long time. See details [here](12.9._Change_email_notification.md#idle-runs) |
| **INACTIVE\_USERS** | tells that some users are not active for a long period of time |
| **LDAP\_BLOCKED\_POSTPONED\_USERS** | tells that specific users were blocked in LDAP therefore they will be blocked and in the Platform in several days |
...
| **LONG\_PAUSED** | tells that the job has been paused for a long time. See details [here](12.9._Change_email_notification.md#notifications-for-long-paused-runs) |
| **LONG\_PAUSED\_STOPPED** | tells that the job, that has been paused for a long time, is stopped at the moment. See details [here](12.9._Change_email_notification.md#notifications-for-long-paused-runs) |
...
The **System jobs** subtab allows to create and easily launch system scripts for different needs:
...
- list of all stored system jobs (scripts)
- history of the selected job runs
- controls to manage the selected job (launch/view details/view and download logs etc.)

![CP_ManageSettings](attachments/ManageSettings_39.png)

For more details see [here](12.15._System_jobs.md).

### DTS

The **DTS** subtab allows to create and configure data transfer services (DTS).  
Via these services automatic data transfer can be executed, for example, between some local workstation or network host and datastorage available on the Cloud Pipeline Platform. Such services can be helpful for the periodic loading of raw data to the Cloud storage.

The **DTS** subtab contains:

- list of all created DTS
- selected DTS details:
    - schedule rules
    - key-value pairs of metadata needed for the correct DTS work
    - extra preferences
    - logs of previous transfers
- controls to manage the selected DTS (create/edit/remove etc.)

![CP_ManageSettings](attachments/ManageSettings_40.png)
...
- to configure the view (UI theme) of the Platform pages for the current user
- to view statistics of the Platform usage by the current user
...
- attributes section:
    - set of different options that could be configured as default settings for the Platform usage by the current user
    - user metadata

![CP_ManageSettings](attachments/ManageSettings_27.png)

**Attributes** section allows to configure the following options:

- _Limit mounts_ - list of storages that will be mounted to the user runs by default. See example of that option configuration [here](../06_Manage_Pipeline/6.1._Create_and_configure_pipeline.md#example-limit-mounted-storages).
- _Run capabilities_ - list of capabilities that will be enabled in the user runs by default. See example of that option configuration [here](../10_Manage_Tools/10.9._Run_capabilities.md#109-run-capabilities).
- _SSH terminal theme_ - allows to set default theme of the web-terminal for the user runs. See details [here](../15_Interactive_services/15.2._Using_Terminal_access.md#terminal-view).
- _Mute email notifications_ - allows to "mute" push-notifications in the GUI, i.e. notifications will still appear in the **Notifications** section, but corresponding pushs will not be showing. See example of that option configuration [here](12.9._Change_email_notification.md#mute-notifications).
- _User metadata_ - key-value pairs of attributes that can be assigned to the user. They are fully the same that can be managed via the user form - see details [here](12.4._Edit_delete_a_user.md#attributes).
...
The Statistics subtab allows to view main summary statistics of the Platform usage by the current loginned user for a specific month:  
    ![CP_ManageSettings](attachments/ManageSettings_41.png)

Subtab contains sections:

- summary section:
    - controls to switch a month to view statistics. By default, the current calendar month is being selected
    - most used tools (top 3) in the selected month
    - most used pipelines (top 3) in the selected month
    - last login date
    - summary costs of the Platform usage (on compute instances and datastorages) by the current user:
        - in the selected month
        - in the month preciding the selected month
    - button to download a list of all user runs performed in the selected month
    ![CP_ManageSettings](attachments/ManageSettings_42.png)
- detailed usage section - section with two tabs:
    - _Compute instances_ (opened by default) - costs and usage of different instances launched by the user in the selected month. Statistics are aggregated by the tool names used for the instances runs  
    ![CP_ManageSettings](attachments/ManageSettings_43.png)
    - _Storages_ - costs and volumes of datastorages owned by the current user. Statistics are displayed for the selected month  
    ![CP_ManageSettings](attachments/ManageSettings_44.png)
