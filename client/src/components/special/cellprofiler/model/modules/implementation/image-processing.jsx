...
const correctIlluminationCalculate = {
  ...
  group: 'Image Processing',
  outputs: ['output|file', 'averaged|file|IF img==true', 'dilated|file|IF dilatedimg==true'],
  parameters: [
    'Select the input image|file',
    'Name the output image|ALIAS output',
  ...
  parameters: [
    ...
    'Relative weight for the brightness image|float|1.0|IF brightness!="" AND scheme==CMYK',
    {
      ...
      valueFormatter: (value, cpModule) => {
        ...
        return (value || []).map(({name, color, value}) =>
          colorAndWeight ? `${name}|${value}|${color}` : name,
        );
      },
      renderer: (moduleParameterValue, className, style) => (
        <GrayToColor parameterValue={moduleParameterValue} className={className} style={style} />
      ),
      visibilityHandler: (cpModule) =>
        cpModule && ['Composite', 'Stack'].includes(cpModule.getParameterValue('scheme')),
    },
  ],
...
const overlayOutlines = {
  ...
  parameters: [
    ...
    'Select method to determine brightness of outlines|[Max of image,Max possible]|Max of image|IF mode==Grayscale|ALIAS method',
    {
      ...
      valueFormatter: (value, cpModule) => {
        const grayScale = cpModule && cpModule.getParameterValue('mode') === 'Grayscale';
        return (value || []).map(({name, color}) => (grayScale ? name : `${name}|${color}`));
      },
      renderer: (moduleParameterValue, className, style) => (
        <OutlineConfig parameterValue={moduleParameterValue} className={className} style={style} />
      ),
    },
  ],
};
