...
| 14 | Click **Type** field and select `audit` value from drop-down list | Records <br>`[run_id #<run_id>] READ storage1/file1` <br> `[run_id #<run_id>] WRITE storage2/file1` <br> , where `<run_id>` is Id of run launched at step 4, are shown in the top of logs records |
| 15 | Login as the non-admin user from the prerequisites and repeat steps 5-14 for `pipe storage mv storage1/file2 storage2/` command | Records <br> `[run_id #<run_id>] READ storage1/file2` <br> `[run_id #<run_id>] WRITE storage2/file2` <br> `[run_id #<run_id>] DELETE storage1/file2` <br> are shown in the top of logs records |
| 16 | Login as the non-admin user from the prerequisites and repeat steps 5-14 for `pipe storage rm storage1/file1 -y` command | Record <br> `[run_id #<run_id>] DELETE storage1/file1` <br> is shown in the top of logs records |
