...
class CellProfilerParameter extends React.Component {
  ...
  renderSliderControl() {
    const {parameterValue} = this.props;
    if (!parameterValue || !parameterValue.parameter || !parameterValue.parameter.range) {
      return null;
    }
  ...
  renderBooleanControl() {
    ...
    return (
      <Checkbox className={styles.control} checked={!!parameterValue.value} onChange={onChange} />
    );
  }
  ...
  renderRangeControl() {
    ...
    const [from, to] =
      parameterValue.value && parameterValue.value.length ? parameterValue.value : [];
    ...
    return (
      <div className={styles.control} style={{display: 'flex'}}>
        ...
        <Input style={{flex: 1}} value={from} onChange={onChangeFrom} />
        <span style={{whiteSpace: 'pre'}}> to </span>
        <Input style={{flex: 1}} value={to} onChange={onChangeTo} />
      </div>
    );
  }
    ...
    if (
      ...
      return (
        <span className={styles.units}>
          ...
          {parameterValue.parameter.type === AnalysisTypes.units2 && <sup>2</sup>}
        </span>
      );
    }
    return null;
  ...
  render() {
    const {parameterValue, className, style} = this.props;
    if (!parameterValue || !parameterValue.parameter) {
      return null;
    }
    const {showTitle} = parameterValue.parameter;
    return (
      <div className={classNames(styles.cellProfilerParameter, className)} style={style}>
        <div
          className={classNames(styles.cellProfilerParameterTitle, {
        ...
        <div className={styles.cellProfilerParameterValue}>{this.renderValueControl()}</div>
      </div>
    );
  }
}
