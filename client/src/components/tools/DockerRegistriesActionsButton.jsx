...
export default class DockerRegistriesActionsButton extends React.Component {
  ...
  get groupHasChildTools() {
    return this.props.group ? (this.props.group.tools || []).length > 0 : false;
  }
  ...
  _createRegistry = async (registryData) => {
    ...
    if (request.error) {
      ...
      if (this.props.onNavigate) {
        this.props.onNavigate(request.value.id);
      }
    }
  };

  _registryOperationWrapper =
    (operation) =>
    (...props) => {
      this.setState(
        {
  ...
  _confirmDeleteRegistry = () => {
    ...
    deleteToolConfirmModal({registry: this.props.registry}, this.props.router).then((confirm) => {
      if (confirm) {
    ...
    this.setState(
      ...
      async () => {
        ...
        if (!request.error) {
          ...
          if (this.props.onNavigate) {
            this.props.onNavigate(this.props.registry.id, request.value.id);
          }
        }
        hide();
        this.setState({
  ...
  _deleteGroup = async () => {
    ...
    if (request.error) {
      ...
      if (this.props.onNavigate) {
        this.props.onNavigate(this.props.registry.id);
      }
    }
  };

  _confirmDeleteGroup = () => {
    deleteToolConfirmModal({group: this.props.group}, this.props.router).then((confirm) => {
      if (confirm) {
  ...
  _onMenuSelect = ({key}) => {
    this.setState(
      ...
      () => {
        switch (key) {
          case 'add-registry':
            this._openCreateRegistryForm();
            break;
          case 'edit-registry':
            this._openEditRegistryForm();
            break;
          case 'configure-registry':
            this._openDockerConfigurationForm();
            break;
          case 'add-private-group':
            this._createPrivateGroup();
            break;
          case 'add-group':
            this._openCreateToolGroupForm();
            break;
          case 'edit-group':
            this._openEditGroupForm();
            break;
          case 'delete-group':
            this._confirmDeleteGroup();
            break;
          case 'enable-tool':
            this._openEnableToolForm();
            break;
        }
      },
    );
  ...
  _getActionsMenuItems = () => {
    ...
    if (
      this.props.registry &&
      ((toolGroupAdmin && roleModel.writeAllowed(this.props.registry)) || toolAdmin)
    ) {
      groupActions.push({
        key: 'add-group',
        icon: <PlusOutlined />,
