...
if iptables --version | grep -q nf_tables &> /dev/null; then
  _img_suffix="-nft"
fi
...
wget $_WO "${_KUBE_SYSTEM_PODS_DISTR}/calico-node-v3.14.1${_img_suffix}.tar" -O $_DOCKER_SYS_IMGS/calico-node-v3.14.1.tar && \
...
wget $_WO "${_KUBE_SYSTEM_PODS_DISTR}/k8s.gcr.io-kube-proxy-v1.15.4${_img_suffix}.tar" -O $_DOCKER_SYS_IMGS/k8s.gcr.io-kube-proxy-v1.15.4.tar && \
wget $_WO "${_KUBE_SYSTEM_PODS_DISTR}/quay.io-coreos-flannel-v0.11.0${_img_suffix}.tar" -O $_DOCKER_SYS_IMGS/quay.io-coreos-flannel-v0.11.0.tar && \
