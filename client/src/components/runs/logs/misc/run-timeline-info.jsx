...
import styles from './run-timeline-info.module.css';
...
function RunTimelineInfoDetails(props) {
  ...
  const {pausedIntervals} = getRunDurationInfo(run, analyseSchedulingPhase, runTasks || []);
  if (pausedIntervals.length > 0) {
    return (
      <div className={classNames(styles.runTimelineInfoDetails, className)} style={style}>
        <table className={classNames('cp-run-timeline-table', styles.pausedDurationTable)}>
          <thead>
            <tr>
              <th>Paused</th>
...
RunTimelineInfoDetails.available = function (run, runTasks, analyseSchedulingPhase) {
  const {pausedIntervals, runningDate} = getRunDurationInfo(
    run,
    analyseSchedulingPhase,
    runTasks || [],
  );
...
function getRunTimelineInfoDescription(run, runTasks, analyseSchedulingPhase, dateFormat) {
  if (!run) {
    return null;
  }
  const {status} = run;
  const {info, last, runningDate, pausedDuration, totalRunningDuration, wasPaused} =
  ...
  if (!isStopped && wasPaused && last && last.phase === RunHistoryPhase.running) {
    durationInfos.push(`${displayDurationInSeconds(getIntervalDuration(last))} since last resume`);
  }
  const durationInfo = durationInfos.join(' / ');
  return isStopped
    ? `${displayDate(info.end || dayjs.utc(), dateFormat)} (${durationInfo})`
    : durationInfo;
}
