...
import styles from './request-dav-access.module.css';
...
function davAccessInfo(value) {
  ...
  if ((typeof value === 'string' && !Number.isNaN(Number(value))) || typeof value === 'number') {
...
function RequestDavAccess({metadata = {}, info, reload, preferences, showOnlySummary}) {
  const {start_request: startRequest, done_request: doneRequest} =
    preferences.requestFileSystemAccessTooltip || {};
  const {storageId, storageMask = 0} = info || {};
  const readOnly = !roleModel.writeAllowed({mask: storageMask}) || !storageId || showOnlySummary;
  ...
  return (
    ...
    >
      <span style={{marginRight: 5}} onClick={!readOnly && !enabled ? enableAccess : undefined}>
        {infoString}
      </span>
      <InfoTooltip tooltip={hint} />
      {enabled && !readOnly && (
        <a className={classNames(styles.disableButton, 'cp-danger')} onClick={disableAccess}>
          Disable
        </a>
      )}
    </div>
