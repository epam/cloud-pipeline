...
import {Button, Popover, Row, Dropdown, Space} from 'antd';
...
function checkPermission(parameter, storageList, permissionChecker, permissionName) {
  ...
  for (let i = 0; i < values.length; i++) {
    ...
    const [storage] = storageList.filter((s) => s.pathMask.test(value) && permissionChecker(s));
    if (!storage) {
      errors.push(
        <span>
          <b>{value}</b>
          <span>
            {' '}
            ({parameter.name} parameter): {permissionName} operation is denied
          </span>
        </span>,
      );
    }
...
async function checkDockerImage(props) {
  const {dockerImage, dockerRegistries} = props;
  if (dockerImage && dockerRegistries && dockerRegistries.loaded) {
    ...
    const [reg] = registries.filter((r) => r.path.toLowerCase() === (registry || '').toLowerCase());
    ...
    if (reg) {
      ...
      const [gr] = groups.filter((g) => g.name.toLowerCase() === (group || '').toLowerCase());
      if (gr) {
        let [imageName, version] = image.split(':');
        imageName = `${group}/${imageName}`.toLowerCase();
        const [tool] = (gr.tools || []).filter((t) => t.image.toLowerCase() === imageName);
...
function checkInputs(props) {
  const {dataStorages, inputs} = props;
  if (inputs && dataStorages && dataStorages.loaded) {
    ...
    for (let i = 0; i < (inputs || []).length; i++) {
      result.push(...checkPermission(inputs[i], storages, roleModel.readAllowed, 'read'));
    }
    return result;
  }
  return [];
...
function checkOutputs(props) {
  const {dataStorages, outputs} = props;
  if (outputs && dataStorages && dataStorages.loaded) {
    ...
    for (let i = 0; i < (outputs || []).length; i++) {
      result.push(...checkPermission(outputs[i], storages, roleModel.writeAllowed, 'write'));
    }
    return result;
  }
  return [];
...
export async function performAsyncCheck(props, state = undefined) {
  const {dataStorages, dockerRegistries, skipCheck} = props;
  await Promise.all(
    [
  ...
  if (skipDockerVerify) {
    dockerImageErrors = [];
  } else if (!skipCheck && (!dockerImageChecked || props.dockerImage !== dockerImageChecked)) {
  ...
  const errors = [...dockerImageErrors, ...inputsErrors, ...outputsErrors];
  return {
    errors,
    state: {
      dockerImageErrors,
...
class SubmitButton extends React.Component {
  ...
  render() {
    ...
    const pending =
      (dataStorages.pending && !dataStorages.loaded) ||
