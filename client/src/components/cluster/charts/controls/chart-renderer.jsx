...
function ChartRenderer({
  ...
  if (
    ...
    !data ||
    !data.data ||
    !(data.data && Object.hasOwn(data.data, dataGroup))
  ) {
    return null;
  }
  const {plots} = plot;
...
class ChartRendererWithOffset extends React.Component {
  ...
  get data() {
    ...
    return plots.map((plot) => ({
      ...
      data: splitDataParts(
        (data || []).map((i) => ({x: getXCoordinate(i.x), y: getYCoordinate(i[plot.name])})),
      ),
    }));
  }
  ...
  renderPlot = (plot, index) => {
    ...
    const areaParts = plot.data
      ...
      .join(' ');
    return (
      <g key={index} shapeRendering={'auto'}>
  ...
  render() {
    ...
    return <g mask={`url(#${maskIdentifier})`}>{this.data.map(this.renderPlot)}</g>;
  }
}
