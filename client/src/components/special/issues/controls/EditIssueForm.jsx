...
export default class EditIssueForm extends localization.LocalizedReactComponent {
  ...
  getDeleteModalFooter = () => {
    return (
      <Row type="flex" justify="space-between">
        <Button id="edit-issue-delete-dialog-cancel-button" onClick={this.closeDeleteDialog}>
          Cancel
        </Button>
        <Button
  ...
  renderActions = () => {
    if (this.props.issue) {
      return (
        <Row type="flex" justify="space-between">
          <Button id="delete-issue" danger onClick={this.openDeleteDialog} size="small">
            Remove
          </Button>
          <Row type="flex" className={styles.actions} justify="end">
            <Button id="cancel-edit-issue-button" onClick={this.onClose} size="small">
              Cancel
            </Button>
            <Button
      ...
      return (
        <Row type="flex" className={styles.actions} justify="end">
          <Button id="cancel-create-issue-button" onClick={this.onClose} size="small">
            Cancel
          </Button>
          <Button id="create-issue-button" type="primary" onClick={this.handleSubmit} size="small">
            Create
          </Button>
        </Row>
      );
        ...
        <div style={{flex: 1, display: 'flex', flexDirection: 'column'}}>
          ...
          >
            <Form.Item
              ...
              name="name"
              rules={[
                {required: true, message: `${this.localizedString('Issue')} title is required`},
              ]}
            >
              <Input
                size="default"
