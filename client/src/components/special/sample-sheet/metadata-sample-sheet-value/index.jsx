...
import styles from './metadata-sample-sheet-value.module.css';
...
async function fetchSampleSheetContents(options = {}) {
  const {dataStorageAvailable, path} = options;
  if (dataStorageAvailable) {
    await dataStorageAvailable.fetchIfNeededOrWait();
    const match = (storage) =>
      storage.pathMask && new RegExp(`^${storage.pathMask}/`, 'i').test(path);
...
class MetadataSampleSheetValue extends React.Component {
  ...
  onEditClick = async (e) => {
    ...
    const {dataStorageAvailable, value} = this.props;
    let hide;
    if (value) {
  ...
  renderEditActions() {
    if (this.sampleSheetAvailable) {
      return <a onClick={this.onEditClick}>{this.sampleSheetFileName}</a>;
    }
    return (
  ...
  render() {
    ...
    const {editDialogVisible, sampleSheetContent} = this.state;
    return (
      <div
        className={classNames(className, styles.buttons)}
        style={style}
