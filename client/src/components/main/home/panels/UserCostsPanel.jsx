...
function getTotalCosts(billing) {
  const {current, previous, filters} = billing;
  const {endStrict, previousEndStrict} = filters || {};
...
function fetchUserName(userName, authenticatedUserInfo) {
  ...
  return new Promise((resolve) => {
    authenticatedUserInfo.fetchIfNeededOrWait().then(() => {
...
export default class UserCostsPanel extends React.Component {
  ...
  componentDidUpdate(prevProps) {
    ...
    if (prevProps.userName !== userName || (period && period !== prevProps.period)) {
      this.updateFromProps();
    }
  }
  ...
  updateFromProps = () => {
    ...
    fetchUserName(userName, authenticatedUserInfo).then(this.fetchBillingInfo.bind(this));
  };
  ...
  fetchBillingInfo = (userName) => {
    const {billingPeriods = BILLING_PERIODS, period: periodDate} = this.props;
    this.setState({pending: true}, async () => {
      const promises = billingPeriods.map((period) => {
        return new Promise((resolve) => {
          const rangeFormat = period === Period.month ? 'YYYY-MM' : 'YYYY';
          const range = periodDate ? ensureDayjs(periodDate).format(rangeFormat) : undefined;
          const periodInfo = getPeriod(period, range);
          const request = new GetBillingData({
            filters: {
              user: userName,
              ...periodInfo,
              filterBy: [GetBillingData.FILTER_BY.compute, GetBillingData.FILTER_BY.storages],
            },
          });
          request
            .fetch()
  ...
  renderDisclaimer = () => {
    ...
    const message = <Markdown md={this.disclaimer} className={styles.markdown} />;
    return (
      <Alert title={message} type="warning" style={{padding: '8px 15px', marginBottom: '10px'}} />
    );
  };

  renderBillingInfo = (billing) => {
    const {showPeriodHeaders = true} = this.props;
    const {filters, period, percent, current, previous} = billing;
    const {start, previousStart, endStrict, previousEndStrict} = filters || {};
    let format;
    ...
    const longDash = <span style={{fontWeight: 'normal'}}>&#8211;</span>;
    const popoverContent = (dateFrom, dateTo, period) => {
      const from = ensureDayjs(dateFrom);
      const to = ensureDayjs(dateTo);
      if (period === Period.month) {
    ...
    return [
      showPeriodHeaders && (
        <span key="period-header" className={classNames(styles.subHeader, 'cp-divider', 'bottom')}>
          {period}
        </span>
      ),
      <div key="period-name" className={classNames(styles.cell, styles.leftColumn)}>
        <Tooltip placement="topRight" title={popoverContent(start, endStrict, period)}>
          <span>{ensureDayjs(endStrict).format(format)}:</span>
        </Tooltip>
      </div>,
      <div
        key="period-current-billing"
        className={classNames(styles.cell, styles.spendingsContainer)}
      >
        <span className={styles.bold}>{current ? `$${current}` : longDash}</span>
        {renderStatistics(percent)}
      </div>,
      <div key="period-previous" className={classNames(styles.cell, styles.leftColumn)}>
        <Tooltip
          placement="topRight"
          title={popoverContent(previousStart, previousEndStrict, period)}
        >
          <span>{ensureDayjs(previousEndStrict).format(format)}:</span>
        </Tooltip>
      </div>,
      <span key="period-previous-billing" className={styles.cell}>
  ...
  render() {
    ...
    return (
      <Spin spinning={pending} classNames={{root: styles.spinner}}>
        <div className={styles.container}>
          {showDisclaimer ? this.renderDisclaimer() : null}
          <div className={styles.gridContainer}>
            {this.billingInfo.map((billing) => this.renderBillingInfo(billing))}
          </div>
        </div>
      </Spin>
    );
