...
- [Example: Limit mounted storages](#example-limit-mounted-storages)
    - [Mount storages in parallel](#mount-storages-in-parallel)
...
### Mount storages in parallel

By default, the storages are mounted one by one during the `MountDataStorages` task. If many storages are mounted to a run, this task can take a long time.

To mount the storages in parallel, add the **`CP_CAP_MOUNT_THREADS`** system parameter to the run and set it to the number of threads, e.g. `4`, `8` or `16`.

- The default value is `1`: the storages are mounted one by one. To go back to this behavior, set the parameter to `1` or remove it.
- If the value is not a positive integer, it is ignored with a warning in the `MountDataStorages` log, and the storages are mounted one by one.
- If a storage is mounted inside the mount point of another storage, it is mounted only after the mount command of that storage has finished. `pipe` FUSE finishes its mount command only once the storage is mounted. Some other mount tools (e.g. `goofys`, `s3fs`, `gcsfuse`) finish their command before the storage is fully mounted, as without this parameter.
- If some storages fail to mount, the other storages are still mounted. The `MountDataStorages` task fails at the end and lists the storages that were not mounted.

> **_Note_**: storages mounted with `pipe` FUSE request the storage details and credentials from the API, one request per storage. A large number of threads makes these requests at the same time and increases the load on the API and on the cloud provider when a run starts. This is especially true for a cluster, as each node of the cluster mounts the storages. It also uses more CPU on small nodes. Start with a small value, e.g. `4`. If `pipe` FUSE mounts fail by a timeout under a high load, increase the **`CP_PIPE_FUSE_MOUNT_TIMEOUT`** parameter as well (by default, `10000` ms). By default, this parameter is not passed to the worker nodes of a cluster. To pass it, add its name to the **`CP_CAP_AUTOSCALE_INHERITABLE_PARAMETER_NAMES`** parameter.
...
6. In the configuration, specify custom node image in the format as described above, e.g.:  
    ![CP_CreateAndConfigurePipeline](attachments/CreateAndConfigurePipeline_59.png)  
...
9. Check that for the run the custom node image specified at step 6 is used:  
    ![CP_CreateAndConfigurePipeline](attachments/CreateAndConfigurePipeline_62.png)
