...
import TabularDataGrid from '../../../special/tabular-data-grid/TabularDataGrid';
import Papa from 'papaparse';
import styles from './DataStorageCodeForm.module.css';
...
function validateTXT(content = '') {
  ...
  if ((lines.length === 1 && !lastLine.trim()) || (typeof lastLine === 'string' && !lastLine)) {
    return undefined;
  }
  return true;
}
...
export default class DataStorageCodeForm extends React.Component {
  ...
  onClose = () => {
    ...
    if (this.tableEditor && this.state.editMode) {
  ...
  onSave = () => {
    ...
    if (this._modifiedCode !== null && this._originalCode !== this._modifiedCode) {
      Modal.confirm({
        title: (
          <div>
            <span>{`Save changes to ${this.props.file.name}?`}</span>
  ...
  get isTabular() {
    return this.fileType === 'dsv' || this.fileType === 'tsv' || this.fileType === 'csv';
  }

  get isSampleSheet() {
    const {file} = this.props;
    const code = this._modifiedCode === null ? this._originalCode : this._modifiedCode;
    return (
      this.isTabular &&
      file &&
      file.name &&
      this._fileContents &&
      this._fileContents.loaded &&
      code &&
      utilities.isSampleSheetContent(code, file.name)
    );
  }

  get structuredTableData() {
    const result = {};
    const csvString = this._modifiedCode !== null ? this._modifiedCode : this._originalCode || '';
  ...
  get stringTableData() {
    ...
    return Papa.unparse(this.tableEditor.getData(), unparseConfig);
  }
  ...
  renderFileEditor() {
    ...
    if (this.isSampleSheet && !this.state.editTabularAsText) {
      return (
        <SampleSheet
          content={this._modifiedCode === null ? this._originalCode : this._modifiedCode}
      ...
      <div style={styles.titleContainer}>
        <Row type="flex" justify="space-between">
          ...
          <Col>
            {this.isEditable && this.isTabular && (
              <span>
                {this.state.editMode ? 'Edit' : 'View'} as text:{' '}
                <Switch
            ...
            <Button id="code-form-close-button" className={styles.button} onClick={this.onClose}>
              Close
            </Button>
          </Col>
        </Row>
        {this.errors && this.errors.length > 0
          ? this.errors.map((error) => (
              <Alert key={error} title={error} type="error" style={{marginTop: '5px'}} />
            ))
          : null}
      </div>
    ) : null;
