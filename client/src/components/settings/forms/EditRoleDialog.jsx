...
@inject((common, params) => ({
  ...
  credentialProfiles: params.role ? new LoadEntityCredentialProfiles(params.role.id, false) : null,
}))
@observer
class EditRoleDialog extends React.Component {
  ...
  get addedUsers() {
    ...
    return users.filter(
      (u) => usersInitial.filter((uI) => uI.userName === u.userName).length === 0,
    );
  }

  get removedUsers() {
    const {users, usersInitial} = this.state;
    return usersInitial.filter(
      (uI) => users.filter((u) => u.userName === uI.userName).length === 0,
    );
  }
  ...
  updateValues = () => {
    ...
    if (!profilesInitialized && credentialProfiles && credentialProfiles.loaded) {
    ...
    this.setState(
      ...
      async () => {
        ...
        if (this.props.roleInfo.loaded) {
          assignedUsers = (this.props.roleInfo.value.users || []).map((r) =>
  ...
  saveChanges = async () => {
    if (this.modified) {
      ...
      if (defaultStorageId !== defaultStorageIdInitial || userDefault !== userDefaultInitial) {
        const roleType = predefined ? 'role' : 'group';
        const userDefaultMessage =
          userDefault !== userDefaultInitial ? `default ${roleType}` : undefined;
        const defaultStorageMessage =
          defaultStorageId !== defaultStorageIdInitial ? 'default data storage' : undefined;
      ...
      if (this.addedUsers.length > 0) {
        ...
        const requests = this.addedUsers.map((user) => new RoleAssign(this.props.role.id, user.id));
      ...
      if (this.removedUsers.length > 0) {
        ...
        const requests = this.removedUsers.map(
          (user) => new RoleRemove(this.props.role.id, user.id),
        );
        await Promise.all(requests.map((request) => request.send({})));
        const [error] = requests.map((request) => request.error).filter(Boolean);
        hide();
        ...
        <Splitter.Panel defaultSize="60%" min="33%" resizable>
          ...
          >
            <Row type="flex" style={{marginBottom: 8}} align="middle">
              <Checkbox checked={this.state.userDefault} onChange={this.onChangeUserDefault}>
              ...
              <div style={{flex: 1}} id="find-user-autocomplete-container">
                ...
                >
                  {this.state.fetchedUsers.map((user) => {
                    return (
                      <AutoComplete.Option key={user.id} text={user.userName}>
  ...
  renderTabs = () => {
    ...
    const blocked = this.props.roleInfo.loaded ? this.props.roleInfo.value.blocked : false;
    const tabsConfig = [
      {
        key: 'group',
        tab: (
    ...
    if (tabsConfig.length === 1) {
      return (
        <div className={classNames('cp-divider bottom', 'cp-text', styles.tabPlaceholder)}>
          {tabsConfig[0].tab}
        </div>
      );
    }
