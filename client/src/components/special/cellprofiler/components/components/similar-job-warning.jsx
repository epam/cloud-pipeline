...
import styles from './similar-job-warning.module.css';

function SimilarJobWarning({className, visible, onCancel, onSubmit, onOpenSimilar, jobs = []}) {
  return (
    <Modal
      className={className}
      open={visible}
      ...
      footer={
        ...
        >
          <Button style={{marginRight: 5}} onClick={onCancel}>
            NO
          </Button>
    ...
    >
      ...
      >
        {}
        It is possible that the same or similar analysis job{jobs.length > 1 ? 's' : ''}{' '}
        {jobs.length > 1 ? 'were' : 'was'} launched recently:
      </div>
      {jobs.map((similarJob) => (
        <CellProfilerJob
