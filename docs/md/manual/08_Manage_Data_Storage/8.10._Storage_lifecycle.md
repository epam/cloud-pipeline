...
Users have the ability to manage the lifecycle of data in storages - configure the automatical data transition from standard storage to different types of archival storages by occurance of a certain event and restore that data back as well if needed.  
...
- to select an archive class as the data destination for each specific rule. **_Note_**: archive classes depend on on the Cloud Provider
...
> **_Note_**: data archiving in the storage is available for admins and storage **_OWNER_** only
...
7. Here, the following fields shall be filled in:  
    ...
    - **Condition** - additional condition that shall be met to initiate the transition procedure. There are 2 ossible variants:  
