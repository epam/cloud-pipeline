...
function QuotaAction({action, className, style}) {
  if (!action) {
    return null;
  }
  const {threshold = 0, actions = [], activeAction} = action;
  if (actions.length === 0) {
    return null;
  }
  const actionsDescription = actions
    .map((action) => (actionNames[action] || action).toLowerCase())
    .join(', ');
  return (
    <span
...
const actionsByGroup = {
  ...
  [quotaGroups.storages]: [actions.notify, actions.readMode, actions.block],
};
