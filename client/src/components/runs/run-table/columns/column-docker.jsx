...
function DockersFilterComponent({
  ...
  const {pending, value: dockersRegistriesValue = {}, loaded} = dockerRegistriesRequest;
  if (pending && !loaded) {
    return (
  ...
  const renderFilterField = (image) => {
    ...
    return (
      <Row key={image.value} style={{margin: 5}}>
        <Checkbox onChange={onChangeSelection(image.value)} checked={checked}>
  ...
  return (
    ...
    >
      <Row>
        <Input.Search
          value={search}
          placeholder="Filter docker images"
      ...
      <Row>
        <div style={{maxHeight: 400, overflowY: 'auto'}}>
          {filteredImages.length > 0 && (
            <Row style={{overflowY: 'auto'}}>{filteredImages.map(renderFilterField)}</Row>
          )}
        </div>
      </Row>
