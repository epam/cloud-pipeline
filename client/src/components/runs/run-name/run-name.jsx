...
import styles from './run-name.module.css';
...
class RunName extends React.PureComponent {
  ...
  onChangeAlias = (event) => {
    ...
    if (newAlias.length <= ALIAS_MAX_LENGTH && /^[\da-zA-Z_-]*$/.test(newAlias)) {
  ...
  render() {
    const {children, className, style, editable, ignoreOffset, disabled} = this.props;
    const {alias, editMode} = this.state;
    ...
    return (
      <span
        className={classNames(className, styles.runName, {
      ...
      >
        {alias && !editMode && (
          <span
            className={classNames(styles.alias, 'cp-run-name', {
