...
export default class Preferences extends React.Component {
  ...
  get filteredPreferencesGroups() {
    ...
    return search ? [{name: `Search '${search}'`}] : this.preferencesGroups.map((g) => ({name: g}));
  }
  ...
  getPreferencesForGroup = (group) => {
    ...
    return this.preferences.filter((p) => p.preferenceGroup === group);
  };
  ...
  updatePreferences = async (preferences) => {
    ...
    if (request.error) {
      ...
      if (this.preferenceGroupFormRef.current) {
        this.preferenceGroupFormRef.current.resetFormFields();
      }
      hide();
    }
  };
  ...
  render() {
    ...
    const {search, operationInProgress} = this.state;
    const {routing} = this.props;
    ...
    return (
      ...
      >
        ...
        <SubSettings
          sections={this.filteredPreferencesGroups.map((group) => ({
            key: group.name,
            title: group.name,
        ...
        >
          {({section: group}) => {
            const groupPreferences = search
              ? allPreferences.filter(
                  (p) => p.name.toLowerCase().indexOf(search.toLowerCase()) >= 0,
                )
              : allPreferences.filter((p) => p.preferenceGroup === group.key);
            return (
              <PreferenceGroup
