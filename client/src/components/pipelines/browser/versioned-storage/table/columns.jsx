...
const renderActions = (item) => {
  ...
  if (item.type === DOCUMENT_TYPES.blob) {
    actions.push(
      <Button key="download" className={styles.action} data-action="download" size="small">
        <DownloadOutlined />
      </Button>,
    );
    if (roleModel.writeAllowed(item)) {
      actions.push(
        <Button key="edit" className={styles.action} data-action="edit" size="small">
          <EditOutlined />
        </Button>,
      );
      actions.push(
        <Button key="delete" danger className={styles.action} data-action="delete" size="small">
          <DeleteOutlined />
        </Button>,
      );
    }
  ...
  return <span className={styles.rowActions}>{actions}</span>;
};
...
const COLUMNS = [
  ...
  },
  {
    ...
    render: (size, item) =>
      item.type === DOCUMENT_TYPES.tree ? undefined : displaySize(size, false),
  },
  {
    title: 'Revision',
    dataIndex: 'commit',
