...
class App extends Component {
  ...
  openSearchDialog = () => {
    if (this.searchDialog) {
      if (this.searchDialog.openDialog) {
        this.searchDialog.openDialog();
      }
    }
  };
  ...
  render() {
    const {preferences, authenticatedUserInfo, uiNavigation, themes} = this.props;
    const antdTheme =
    ...
    const isBillingPrivilegedUser =
      authenticatedUserInfo.loaded && roleModel.isManager.billing(this);
    const activeTabPath = uiNavigation.getActivePage(this.props.router);
    const isExternalApp = [Pages.miew, Pages.wsi, Pages.hcs].indexOf(activeTabPath) >= 0;
    ...
    if (isExternalApp) {
      ...
      content = (
        <Layout id="root-layout">
          <Layout.Sider collapsible collapsed={this.state.navigationCollapsed} trigger={null}>
            <Navigation
