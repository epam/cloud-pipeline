...
import {Button, Checkbox, Dropdown, Input, message, Modal, Pagination, Row, Space} from 'antd';
import {
  AppstoreOutlined,
  CaretDownOutlined,
  CaretUpOutlined,
...
@inject(
  (
    {folders, pipelinesLibrary, authenticatedUserInfo, preferences, dataStorages, routing},
    params,
  ) => {
    ...
    const hasExplicitParameters =
      params.id !== undefined || params.metadataClass !== undefined || params.class !== undefined;
...
class Metadata extends React.Component {
  ...
  get currentClassEntityFields() {
    ...
    const [metadata] = this.entityTypes.filter(
      (e) => e.metadataClass.name.toLowerCase() === metadataClass,
    );
    if (metadata) {
      return (metadata.fields || []).filter((f) => ownKeys.indexOf(f.name) >= 0).map((f) => f);
    }
    return [];
  }
  ...
  get predefinedFilters() {
    ...
    return predefinedFilters.filter((predefined) =>
      [metadataClass, '*'].includes(predefined.metadataClass),
    );
  }
  ...
  get predefinedConditions() {
    ...
    const {metadataClass} = this.props;
    return predefinedConditions.filter((predefined) =>
      [metadataClass, '*'].includes(predefined.metadataClass),
    );
  }
  ...
  getColumnType = (key) => {
    ...
    const entityField = this.currentClassEntityFields.find((field) => field.name === key);
    if (entityField) {
  ...
  onDateRangeChanged = (range, key) => {
    ...
    const {from, to, emptyValue} = range || {};
    if (key === 'createdDate') {
    ...
    const filtered = this.state.filterModel.filters.filter((filter) => filter.key !== key);
    let periodFilters;
    if (emptyValue) {
      periodFilters = [
        {
    ...
    return this.setState({filterModel}, () => {
      this.clearSelection();
      this.loadData();
    });
  };
  ...
  handleFilterApplied = (key, fieldFilter) => {
    ...
    if (key && !!fieldFilter) {
      ...
      const currentFilterIndex = filterModel.filters.findIndex(
        (filter) => filter.key === unmapColumnName(key),
      );
  ...
  loadData = async () => {
    ...
    const conditions = currentMetadata.map((item) =>
      getPredefinedFilterForItem(item, this.predefinedConditions),
    );
    this.setState(
      {
        loading: false,
  ...
  filterApplied = (key) => {
    ...
    if (key !== 'createdDate') {
      return filters.filter((filterObj) => filterObj.key === unmapColumnName(key)).length;
    } else {
  ...
  renderFilterButton = (key) => {
    ...
    const {filters = [], startDateFrom, endDateTo} = filterModel;
    const filter = filters.find((filter) => filter.key === unmapColumnName(key));
    const values = filter ? filter.values : undefined;
    const button = (
      <Button
        shape="circle"
        onClick={(e) => e.stopPropagation()}
        className={classNames(
          this.filterApplied(key) ? 'cp-primary' : 'cp-text-not-important',
    ...
    if (/^date$/i.test(this.getColumnType(key))) {
      ...
      if (key === 'createdDate') {
        ...
        const currentFilters = filters.filter((filter) => filter.key === unmapColumnName(key));
        dateInfo.emptyValue =
          currentFilters.length === 1 &&
          currentFilters[0].values &&
          currentFilters[0].values.length === 0;
        const filterFrom = currentFilters.find(
          (filter) => filter.operator === FILTER_OPERATORS.greater,
        );
        const filterTo = currentFilters.find((filter) => filter.operator === FILTER_OPERATORS.less);
  ...
  resetColumns = (columns) => {
    return new Promise((resolve) => {
      const {defaultColumnsNames} = this.state;
      if ((defaultColumnsNames || []).length > 0) {
        const defaultColumns = defaultColumnsNames.map((name) => ({
          key: mapColumnName({name}),
        }));
        const newColumns = columns.sort(makeCurrentOrderSort(defaultColumns)).map((column) => ({
          ...column,
  ...
  loadColumns = (options) => {
    ...
    const metadataEntityKeysRequest = new MetadataEntityKeys(folderId, metadataClass);
    return new Promise((resolve) => {
      const {columns: currentColumns = []} = this.state;
      const columnIsSelected = (columnKey) =>
        ...
        metadataEntityKeysRequest
          ...
          .then((columns) => {
            if (Array.isArray(columns) && append) {
              columns.sort(makeCurrentOrderSort(currentColumns)).forEach((column) => {
                column.selected =
                  column.selected || currentColumns.findIndex((o) => o.key === column.key) === -1;
              });
            }
            return Promise.resolve(columns);
          })
  ...
  onArrayReferencesClick = (event, key, data, referenceType, item) => {
    const {ngsProjectInfo, ngsProjectMachineRuns, ngsProjectSamples} = this.props;
    ...
    if (
      ...
      ngsProjectInfo
        .fetchPreferences()
        .then(() => {
          if (ngsProjectInfo.isSampleClassName(referenceType)) {
        ...
        .then((machineRunFieldInfo) => {
          if (machineRunFieldInfo) {
            const {className, fieldName} = machineRunFieldInfo;
            const {router, folderId} = this.props;
            const query = metadataFilterUtilities.build([
              {key: fieldName, values: [item.ID.value]},
            ]);
            if (query) {
  ...
  onReferenceTypesClick = async (event, data) => {
    event.stopPropagation();
    const metadataEntityLoadExternalRequest = new MetadataEntityLoadExternal(
      data.value,
      data.type.split(':ID')[0],
      this.props.folderId,
    );
  ...
  itemIsSelected = (item) => {
    return (
      this.state.selectedItems.filter((s) => s.rowKey.value === item.rowKey.value).length === 1
    );
  };
  ...
  onItemSelect = (item) => {
    ...
    const [selectedItem] = this.state.selectedItems.filter(
      (s) => s.rowKey.value === item.rowKey.value,
    );
    if (selectedItem) {
      const index = selectedItems.indexOf(selectedItem);
      selectedItems.splice(index, 1);
      if (selectedItems.length === 0) {
        this.setState({selectedItemsAreShowing: false}, () => this.paginationOnChange(FIRST_PAGE));
      }
    } else {
  ...
  onColumnSelect = (item) => {
    ...
    if (column) {
      ...
      this.setState({columns: [...currentColumns]}, this.resetColumnsFiltersAndSorting);
    }
  };

  resetColumnsFiltersAndSorting = () => {
    const {columns, filterModel: oldFilters = {}} = this.state;
    const {orderBy, filters, startDateFrom, endDateTo, ...filterModelRest} = oldFilters;
    const selectedColumnsKeys = new Set(
      columns.filter((column) => column.selected).map((column) => column.key),
    );
    const filterModel = {
  ...
  onResetColumns = () => {
    this.resetColumns(this.state.columns).then((columns) =>
      this.setState({columns}, this.resetColumnsFiltersAndSorting),
    );
  };
  ...
  onRowClick = (item) => {
    const selectedItem =
      this.state.currentMetadata.find((row) => row.rowKey?.value === item?.rowKey?.value) ?? item;
    if (
      this.state.selectedItem &&
      this.state.selectedItem.rowKey?.value === selectedItem?.rowKey?.value
    ) {
  ...
  onSelectConfigurationConfirm = async (selectedConfiguration, expression) => {
    ...
    const {id, entries = []} = selectedConfiguration || {};
  ...
  cellIsSpreading = (row, column) => {
    ...
    if (spreadSelection) {
      ...
      return start.column <= column && start.row <= row && end.column >= column && end.row >= row;
    }
  };
  ...
  getCurrentSelection = () => {
    ...
    if (cellsSelection) {
      const {start, end, ...rest} = cellsSelection;
      return {
        start: {
  ...
  getSpreadSelection = () => {
    ...
    if (selection && selection.spread) {
      const {column: spreadColumn, row: spreadRow} = selection.spread;
      const start = {
  ...
  handleStartSelection = (opts) => {
    ...
    if (autoFillEntities.isAutoFillInteractionTarget(e.target)) {
      return;
    }
  ...
  applySelectionAction = (action, revert = true) => {
    ...
    const revertToInitialValues = revert && revertFn ? revertFn : () => Promise.resolve();
      ...
      .then((result) => {
        ...
        result.forEach((item) => {
          const index = (currentMetadata || []).findIndex(
            (o) => o.rowKey && item.rowKey && o.rowKey.value === item.rowKey.value,
          );
          if (index >= 0) {
        ...
        this.setState({
          currentMetadata,
          currentMetadataConditions: currentMetadata.map((item) =>
            getPredefinedFilterForItem(item, this.predefinedConditions),
          ),
        });
        return Promise.resolve();
  ...
  suppressRowClick = () => {
    this._ignoreRowClickUntil = Date.now() + 300;
  };
  ...
  handleFinishSelection = (e) => {
    ...
    if (
      ...
      const dataItemFrom = Math.min(spreadSelection.start.row, selection.start.row);
      const dataItemTo = Math.max(spreadSelection.end.row, selection.end.row);
      const {currentMetadata, columns: tableColumns} = this.state;
      const backup = (currentMetadata || []).slice(dataItemFrom, dataItemTo + 1);
      ...
      if (actions && actions.length > 0) {
        this.applySelectionAction(actions[0], false);
      }
      this.setState({
      ...
      const isSimpleClick =
        ...
        cellsSelection.start.row === cellsSelection.end.row &&
        cellsSelection.start.column === cellsSelection.end.column;
      this._selectionDragged = false;
      if (isSimpleClick) {
        if (
          Date.now() < this._ignoreRowClickUntil ||
          (e && autoFillEntities.isAutoFillInteractionTarget(e.target))
        ) {
          this.clearSelection();
          return;
        }
        const item = this.state.currentMetadata[cellsSelection.start.row];
        this.clearSelection();
        if (item) {
  ...
  handleCellSelection = (opts) => {
    ...
    if (!selection) {
      if (!selectable) {
        ...
        column !== undefined &&
        (!hoveredCell || hoveredCell.column !== column || hoveredCell.row !== row)
      ) {
        this.setState({
    ...
    if (selection.selecting && (selection.end.row !== row || selection.end.column !== column)) {
  ...
  clearHovering = (e) => {
    if (this.state.cellsActions?.length) {
      return;
    }
    const related = e?.relatedTarget;
    if (related instanceof Node) {
      if (e?.currentTarget instanceof Node && e.currentTarget.contains(related)) {
        return;
      }
      if (related.closest?.('.ant-dropdown, .ant-dropdown-menu, .ant-popover')) {
        return;
      }
    }
    this.setState({hoveredCell: undefined});
  ...
  renderContent = () => {
    ...
    const renderTable = () => {
      return [
        <MetadataTable
          ...
          onTableMouseLeave={this.clearHovering}
          ...
          noDataComponent={() => (
            <div className={classNames(styles.noData, 'cp-library-metadata-panel-placeholder')}>
              No rows found
            </div>
          )}
        />,
    ...
    let [currentItem] =
      this.state.selectedItem && this.state.selectedItem.rowKey
        ...
        {this.state.metadata && (
          <MetadataPanel
            ...
            readOnly={
              this.props.readOnly ||
              !(
                roleModel.writeAllowed(this.props.folder.value) && this.props.folderId !== undefined
              )
            }
            ...
            onUpdateMetadata={async () => {
              ...
              await this.loadColumns({append: true});
              await this.loadData();
              const [selectedItem] = this.state.currentMetadata.filter(
                (metadata) => metadata.rowKey.value === currentItem.id,
              );
  ...
  deleteMetadataClassConfirm = () => {
    const onDeleteMetadataClass = async () => {
      ...
      const request = new DeleteFromProject(this.props.folderId, this.props.metadataClass);
      await request.fetch();
      if (request.error) {
        hide();
        message.error(request.error, 5);
  ...
  renderActions = () => {
    ...
    if (metadataManager && !this.props.hideUploadMetadataBtn && !this.props.readOnly) {
      const Actions = {
      ...
      return (
        <Dropdown menu={{items: menuItems, onClick: triggerMenuItem}} trigger={['click']}>
          <Button
  ...
  isSampleSheetColumn = (key) => {
    const {ngsProjectInfo, ngsProjectMachineRuns} = this.props;
    return (
      ngsProjectInfo.isNGSProject &&
      ngsProjectMachineRuns.isMachineRunsMetadataClass &&
  ...
  get tableColumns() {
    ...
    const renderTitle = (key) => {
      ...
      return (
        <span onClick={(e) => onHeaderClicked(e, key)} className={styles.metadataColumnHeader}>
          {icon}
          {orderNumber}
          {getColumnTitle(key)}
    ...
    const isActionsCell = (column, row) =>
      this.state.cellsActions &&
    ...
    const cellWrapper = (props, reactElementFn) => {
      ...
      const {style, className: filterClassName} = getConditionStyles(condition, selected);
      const className = classNames(cellClassName, filterClassName, {
    ...
    const toCellProps = (info) => ({
      value: info.getValue(),
      original: info.row.original,
      index: info.row.index,
      column: {index: info.column.columnDef.meta?.index},
    });
    ...
    return [
      {
        ...
        cell: (info) =>
          cellWrapper(toCellProps(info), () => {
            const item = info.getValue();
            return (
              <Checkbox style={{borderCollapse: 'separate'}} checked={this.itemIsSelected(item)} />
            );
          }),
      },
      ...this.state.columns
        .filter((c) => c.selected)
        .map(({key}, index) => {
          const sampleSheetColumn = this.isSampleSheetColumn(key);
          return {
            id: key,
            accessorKey: key,
            header: () => renderTitle(key),
            size: sampleSheetColumn ? 280 : 150,
            enableResizing: !sampleSheetColumn,
            meta: {
              index,
              selectable: !sampleSheetColumn,
              style: {
                cursor: this.props.readOnly || sampleSheetColumn ? 'default' : 'cell',
                padding: 0,
              },
            ...
            cell: (info) =>
              cellWrapper(toCellProps(info), () => {
                ...
                const condition = currentMetadataConditions[info.row.index];
                const icon =
                  index === 0 && condition && condition.scheme && condition.scheme.icon ? (
                    <OldAntIconsResolver type={condition.scheme.icon} style={{marginRight: 5}} />
                  ) : undefined;
                if (sampleSheetColumn) {
                  return (
                ...
                if (data) {
                  if (data.type.toLowerCase().startsWith('array')) {
                    ...
                    const value = `${count} ${referenceType}(s)`;
                    return (
                      <a
                        title={value}
                        className={styles.actionLink}
                        onClick={(e) =>
                          this.onArrayReferencesClick(e, key, data, referenceType, item)
                        }
                      >
                        {icon}
                        {value}
                  ...
                  if (data.type.toLowerCase() === 'path') {
                    return (
                      <AttributeValue value={data.value} showFileNameOnly>
                        {icon}
                      </AttributeValue>
                    );
                  }
                  ...
                  if (isRunsValue(data.value)) {
                    return <RunsAttribute value={data.value} />;
                  }
                  return (
                    <span title={data.value}>
                      {icon}
  ...
  renderTableActions = () => {
    ...
    const selectedItemsString = `${selectedItems.length} selected item${selectedItems.length === 1 ? '' : 's'}`;
    const renderClearFiltersButton = () => {
      const {filters = [], startDateFrom, endDateTo, searchQueries = []} = filterModel;
      const filtersEnabled =
        filters.length > 0 || !!startDateFrom || !!endDateTo || searchQueries.length > 0;
    ...
    const renderSelectionInfo = () => {
      ...
      if (selectedItemsAreShowing && selectedItems.length > 0) {
        return (
          <span style={{margin: '0 5px'}} key="info">
            {' '}
    ...
    const renderSelectionControl = () => {
      ...
      return (
        <Space.Compact style={{margin: '0 5px'}}>
          <Button size="small" onClick={this.handleClickShowSelectedItems}>
            {selectedItemsAreShowing ? 'Show all metadata items' : `Show ${selectedItemsString}`}
          </Button>
          <Dropdown
  ...
  render() {
    return (
      <div style={{display: 'flex', flexDirection: 'column', height: '100%'}}>
        ...
        >
          <div className={styles.itemHeader} style={{flex: 'initial', marginRight: 'auto'}}>
            <Breadcrumbs
        ...
        <UploadToDatastorageForm
          ...
          fields={this.currentClassEntityFields
            .filter((f) => f.type.toLowerCase() !== 'path')
            .map((f) => f.name)}
  ...
  componentDidMount() {
    const {authenticatedUserInfo, route, router} = this.props;
    if (route && router) {
      router.setRouteLeaveHook(route, this.leavePageWithSelectedItems.bind(this));
    }
    this.onFolderChanged();
    authenticatedUserInfo.fetchIfNeededOrWait().then(() => this.fetchDefaultMetadataProperties());
  ...
  componentDidUpdate(prevProps, prevState, snapshot) {
    ...
    const filtersChanged = metadataFilterUtilities.filtersChanged(
      prevProps.filters,
      this.props.filters,
    );
    if (metadataClassChanged || folderChanged || filtersChanged) {
    ...
    this.setState({
      selectedItems: (this.props.initialSelection || [])
        .map((item) => {
          ...
          return (this.state.currentMetadata || []).find((entry) => entry.rowKey.value === item);
        })
        .filter(Boolean),
    });
  };
        ...
        this.setState(
          (prevState) => ({
            filterModel: {
              ...
              orderBy: getDefaultSorting(defaultOrderBy, columns),
            },
          }),
          async () => {
            await this.loadData();
  ...
  fetchDefaultMetadataProperties = (folderChanged = false) => {
    const {defaultMetadataPropertiesFetched, defaultMetadataPropertiesFetching} = this.state;
    const {authenticatedUserInfo, folderId} = this.props;
    return new Promise((resolve) => {
      if (
        authenticatedUserInfo.loaded &&
        ((!defaultMetadataPropertiesFetched && !defaultMetadataPropertiesFetching) || folderChanged)
      ) {
        this.setState(
          {
          ...
          () => {
            getDefaultMetadataProperties(folderId, authenticatedUserInfo.value).then((metadata) => {
              ...
              this.setState(
                {
                  ...
                  predefinedFilters: predefined.filter((fc) => fc.isFilter),
                  predefinedConditions: predefined.filter((fc) => fc.isCondition),
                  defaultOrderBy: metadata[columnsSorting],
                },
                () => {
                  this.loadColumns({reset: true}).then(() => {
                    const {columns: metadataColumns} = this.state;
                    this.setState(
                      (prevState) => ({
                        filterModel: {
                          ...prevState.filterModel,
                          orderBy: getDefaultSorting(metadata[columnsSorting], metadataColumns),
                        },
                      }),
                      () => resolve(),
                    );
  ...
  componentWillUnmount() {
    if (this.resetSelectedItemsTimeout) {
      clearTimeout(this.resetSelectedItemsTimeout);
    }
