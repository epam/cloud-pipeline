...
import styles from './run-parameters.module.css';
...
class RunParametersSection extends React.Component {
  ...
  renderRunParameter = (runParameter) => {
    ...
    if (runParameter.name === 'parent-id' && parseInt(valueSelector()) !== 0) {
      return (
        <tr key={runParameter.name}>
          ...
          <td className={styles.parameterValue}>
            <AdaptedLink to={`/run/${valueSelector()}`}>{valueSelector()}</AdaptedLink>
          </td>
        </tr>
      );
    }
    if (runParameter.name === CP_CAP_LIMIT_MOUNTS) {
      ...
      return (
        <tr key={runParameter.name}>
          <th className={styles.parameterName} style={{verticalAlign: 'middle'}}>
            {runParameter.name}:
          </th>
          <td className={styles.parameterValue}>
  ...
  renderInstanceParameter = (parameter) => {
    ...
    return (
      <tr key={parameter.key}>
        <th className={styles.parameterName}>{parameter.title || parameter.key}</th>
        <td className={styles.parameterValue}>{parameter.render(run)}</td>
      </tr>
    );
  };
  ...
  render() {
    ...
    const instanceParametersFiltered = instanceParameters.filter(
      (p) =>
        (p.available && typeof p.available === 'function' && p.available(run)) ||
    ...
    return (
      <div className={classNames(className, styles.runParametersSection)} style={style}>
        <table className={styles.runParametersTable}>
          <tbody>
            ...
            {canShare ? (
              <tr>
                <th className={styles.parameterName}>Share with:</th>
                <td className={styles.parameterValue}>
            ...
            {types.map((type, index) => {
              const parameters = filteredRunParameters.filter((p) => getParameterType(p) === type);
