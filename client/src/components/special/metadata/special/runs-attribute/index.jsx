...
import styles from './runs-attribute.module.css';
import {toUtcDayjs} from '../../../../../utils/dayjs';

const MAX_NESTED_RUNS_TO_DISPLAY = 1;

function sortByStartDate(runA, runB) {
  const {startDate: aStartDate} = runA;
  const {startDate: bStartDate} = runB;
  if (!bStartDate) {
    return -1;
  }
  if (!aStartDate) {
  ...
  const a = toUtcDayjs(aStartDate);
  const b = toUtcDayjs(bStartDate);
  if (a.isBefore(b)) {
    return 1;
  }
  if (b.isBefore(a)) {
...
function RunsAttribute({className, value}) {
  ...
  const renderSingleRun = function (run, index) {
    const {runId, status, startDate, endDate} = run;
    ...
    return (
      <Link
        key={index}
        className={classNames(className, styles.nestedRun, 'cp-run-nested-run-link')}
        to={`/run/${runId}`}
      >
  ...
  const renderTooltip = (nestedRuns) => {
    return <div className={styles.nestedRunsTooltip}>{nestedRuns.map(renderSingleRun, true)}</div>;
  };
    ...
    <div className={styles.nestedRuns}>
      ...
      {total > MAX_NESTED_RUNS_TO_DISPLAY && (
        <Tooltip title={renderTooltip(runs)} placement="left">
          <Link className={styles.allNestedRuns}>+{total - MAX_NESTED_RUNS_TO_DISPLAY} more</Link>
        </Tooltip>
      )}
    </div>
  );
