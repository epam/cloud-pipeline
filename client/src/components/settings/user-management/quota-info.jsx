...
import styles from './quota-info.module.css';
...
function QuotaInfo({className, style, quota}) {
  ...
  return (
    <div className={classNames(className, styles.quotaInfo)} style={style}>
      {description}
      {actions && actions.length && <span className={styles.quotaActionTitle}>Actions:</span>}
      {actions.map((action) => {
        return (
          <span
            key={action.id}
            className={classNames(styles.quotaAction, {
          ...
          >
            ...
            {(action.actions || []).map((quotaAction) => (
              <span key={quotaAction} className={styles.actionName}>
...
function QuotasInfo({className, style, quotaStyle, quotaClassName, quotas, onlyTriggered}) {
  if (!quotas || !quotas.length) {
    return null;
  }
  const filteredQuotas = (quotas || []).filter(
    (quota) => !onlyTriggered || quotaHasTriggeredActions(quota),
  );
  ...
  return (
    <div className={className} style={style}>
      <div>
        {overall.map((quota) => (
          <QuotaInfo className={quotaClassName} style={quotaStyle} key={quota.id} quota={quota} />
        ))}
      </div>
      <div>
        {user.map((quota) => (
...
function QuotasDisclaimerComponent({className, style, quotas}) {
  if (!quotas || !quotas.length) {
    return null;
  }
  return (
    <Popover content={<QuotasInfo onlyTriggered quotas={quotas} />}>
      <div className={classNames('cp-warning', className)} style={style}>
...
export {QuotaInfo, QuotasDisclaimerComponent};
