...
public class DataStorageApiService {
    ...
    public List<AbstractDataStorage> getWritableStorages() {
        return dataStorageManager.getDataStoragesWithRegions();
    }
    ...
    public AbstractTemporaryCredentials generateCredentials(List<DataStorageAction> operations) {
        return dataStorageManager.generateTemporaryCredentials(operations);
    }
    ...
    public void validateOperation(List<DataStorageAction> operations) {
        operations.stream().forEach(operation -> {
            Assert.isTrue(operation.getRead() || operation.getWrite(), messageHelper.getMessage(
                    MessageConstants.ERROR_INVALID_CREDENTIALS_REQUEST));
        });
    }
