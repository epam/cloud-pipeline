...
import {LaunchClusterTooltip, renderTooltip} from './launch-cluster-tooltips';
...
export function hybridAutoScaledClusterEnabled(parameters) {
  return (
    autoScaledClusterEnabled(parameters) &&
...
export function autoScaledClusterEnabled(parameters) {
  return (
    ...
    (gridEngineEnabled(parameters) || slurmEnabled(parameters) || kubeEnabled(parameters))
  );
}
...
export function getAutoScaledPriceTypeValue(parameters) {
  if (!parameters || !Object.hasOwn(parameters, CP_CAP_AUTOSCALE_PRICE_TYPE)) {
    return undefined;
  }
  return [AUTO_SCALE_PRICE_TYPES.spot, AUTO_SCALE_PRICE_TYPES.onDemand].includes(
    `${parameters[CP_CAP_AUTOSCALE_PRICE_TYPE].value}`,
  )
    ? `${parameters[CP_CAP_AUTOSCALE_PRICE_TYPE].value}`
    : undefined;
}
...
export function getSkippedSystemParametersList(controller) {
  if (
    controller &&
    controller.state &&
    controller.state.launchCluster &&
...
export function setClusterParameterValue(form, sectionName, configuration) {
  ...
  if (!formValue || !Object.hasOwn(formValue, 'params')) {
    return;
  }
...
export function parseChildNodeInstanceConfiguration(options) {
  const {autoScaled = false, gpuScaling = false, hybrid = false, parameters = {}} = options || {};
  if (autoScaled && !gpuScaling) {
    const readParameterValue = (parameter) =>
      parameters && parameters[parameter] ? parameters[parameter].value : undefined;
    if (hybrid) {
...
class ConfigureClusterDialog extends React.Component {
  static getClusterName = (ctrl, lowerCased) => {
    ...
    if (ctrl.state.fsConfig) {
      const {fsType = ShareFsType.lfs} = ctrl.state.fsConfig;
  ...
  static getClusterDescription = (ctrl, lowerCased = false) => {
    return (
      ConfigureClusterDialog.getClusterName(ctrl, lowerCased) ||
  ...
  static getDisplayConfig = (
    ...
    return {
      ...
      staticVisible: initialSelectedClusterType === CLUSTER_TYPE.fixedCluster || config.static,
      autoScaledEnabled: config.autoScaled,
      autoScaledVisible:
        initialSelectedClusterType === CLUSTER_TYPE.autoScaledCluster || config.autoScaled,
    };
  };
  ...
  get initialSelectedClusterType() {
    if (this.props.launchCluster) {
      return this.props.autoScaledCluster && !this.props.sparkEnabled
        ...
        this.setState(
          {
            ...
            nodesCount: Math.max(
              1,
              !isNaN(this.state.maxNodesCount) ? this.state.maxNodesCount : 1,
            ),
          },
          this.validate,
        );
        ...
        this.setState(
          {
            ...
            gpuScalingConfiguration: undefined,
            childNodeInstanceConfiguration: undefined,
            maxNodesCount: Math.max(1, !isNaN(this.state.nodesCount) ? this.state.nodesCount : 1),
          },
          this.validate,
        );
        break;
  ...
  onChangeEnableHybridAutoScaledCluster = (e) => {
    const {cloudRegionProvider: provider, preferences} = this.props;
  ...
  onChangeEnableGPUScaling = (e) => {
    const {cloudRegionProvider, preferences} = this.props;
  ...
  renderGPUScalingParameter = (value, onChange, gpu = false) => {
    ...
    const {cloudRegionProvider} = this.props;
    return (
      <Row
        ...
        align="middle"
      >
        <div style={PARAMETER_TITLE_STYLE}>{label}</div>
        {hybrid && (
          <InstanceFamilySelector
            value={value}
            onChange={onChange}
  ...
  renderChildNodeInstanceParameter = () => {
    ...
    const {instanceName, cloudRegionProvider} = this.props;
    const label = (
      <span style={PARAMETER_TITLE_STYLE}>Workers {hybrid ? 'family' : 'instance'}:</span>
    );
    ...
    const {name: emptyName, tooltip: emptyTooltip} = (() => {
      if (hybrid) {
  ...
  renderAutoScaledClusterConfiguration = () => {
    ...
    const onSetUpChildNodesClicked = () => {
      this.setState({
        ...
        maxNodesCount: !isNaN(this.state.maxNodesCount) ? Math.max(2, this.state.maxNodesCount) : 2,
        nodesCount: !isNaN(this.state.nodesCount) ? Math.max(1, this.state.nodesCount) : 1,
      });
    };
        ...
        return [
          <Row key="nodes count" type="flex" align="middle" style={{marginTop: 5}}>
            ...
            <InputNumber
              ...
              onChange={this.onChangeNodeCount}
            />
            {renderTooltip(LaunchClusterTooltip.autoScaledCluster.defaultNodesCount, {
              marginLeft: 5,
            })}
            {!this.props.disabled && autoScaledEnabled ? (
              <a onClick={onUnsetChildNodes} className="cp-text underline" style={{marginLeft: 5}}>
                <CloseOutlined /> Reset
              </a>
            ) : null}
          </Row>,
          ...
          <Row key="nodes count" type="flex" align="middle" style={{marginTop: 5}}>
            {!this.props.disabled && autoScaledEnabled ? (
              <span style={{marginLeft: LEFT_MARGIN, flex: 1}}>
                <a onClick={onSetUpChildNodesClicked} className="cp-text underline">
    ...
    const renderAutoScaledPriceTypeSelect = () => (
      <Row key="autoScalePriceType" type="flex" align="middle" style={{marginTop: 5}}>
        ...
        {renderTooltip(LaunchClusterTooltip.autoScaledCluster.autoScalePriceType, {marginLeft: 5})}
      </Row>
    );
    const renderGPUScalingConfigurationToggle = () => {
      ...
      const {gpuScalingConfiguration} = this.state;
      return (
        <Row key="gpu-scaling-checkbox" type="flex" align="middle" style={{marginTop: 5}}>
          <Checkbox
            style={{marginLeft: LEFT_MARGIN}}
    ...
    return [
      ...
      >
        <AllowedInstancesCountWarning
          payload={{
            nodeCount: this.state.setDefaultNodesCount ? this.state.nodesCount : 0,
        ...
        <div>
          <Row type="flex">
            <div style={{marginLeft: LEFT_MARGIN}}>
              <Radio.Group onChange={this.onChange} value={this.selectedClusterType}>
                ...
                {autoScaledVisible ? (
                  <Radio.Button disabled={sparkEnabled} value={CLUSTER_TYPE.autoScaledCluster}>
                    Auto-scaled cluster
                  </Radio.Button>
                ) : null}
              </Radio.Group>
          ...
          {this.selectedClusterType === CLUSTER_TYPE.singleNode && (
            <Row type="flex" justify="center">
              <AllowedInstancesCountWarning singleNode style={{width: '100%', marginTop: '5px'}} />
            </Row>
          )}
          {this.state.launchCluster &&
  ...
  get launchMaxAutoScaledNumber() {
    ...
    if (this.props.preferences && this.props.preferences.loaded) {
      const autoScalingMaxPreferenceValue = this.props.preferences.getPreferenceValue(
    ...
    this.setState(
      {
        ...
        nodesCount:
          this.props.nodesCount && !isNaN(this.props.nodesCount) ? this.props.nodesCount : 0,
