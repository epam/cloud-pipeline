...
async function downloadStorageItems(items) {
  const storageIds = [...new Set(items.map((item) => Number(item.storageId)))].filter(
    (id) => !Number.isNaN(id),
  );
  const results = await Promise.all(
    storageIds.map((storageId) =>
      downloadSingleStorageItems(items.filter((item) => Number(item.storageId) === storageId)),
    ),
  );
...
export default async function handleDownloadItems(preferences, items = []) {
  ...
  try {
    ...
    const {maximum} = preferences ? preferences.facetedFilterDownload : undefined;
