...
import styles from './vsi-preview.module.css';
import previewStyles from '../../search/preview/preview.module.css';
...
class VSIPreviewPage extends React.Component {
  onCameraChanged = (opts) => {
    ...
    const {zoom, roll, x, y} = opts || {};

    router.push(
      `/wsi?storage=${storageId}&file=${encodeURIComponent(file)}&roll=${roll}&zoom=${zoom}&x=${x}&y=${y}`,
    );
  };
  ...
  render() {
    const {storageId, file, zoom, roll, x, y} = this.props;
    if (storageId && file) {
      return (
