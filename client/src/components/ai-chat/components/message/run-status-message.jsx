...
const RUNNING_STATUSES = [RunStatuses.running, RunStatuses.paused];
...
export default class RunStatusMessage extends React.Component {
  ...
  render() {
    const {runId, run} = this;
    if (!runId) {
      return <Spin spinning />;
    }
    return (
      <div style={{display: 'flex', alignItems: 'center', gap: '5px'}}>
        {run && <Observer>{() => <StatusIcon run={run} small />}</Observer>}
        <Link to={`/run/${runId}`}>{run?.podId || `pipeline-${runId}`}</Link>
