...
import styles from './sub-settings.module.css';
...
class SubSettings extends React.Component {
  ...
  updateFromProps = () => {
    const {activeSectionKey} = this.props;
    this.setState(
      {
  ...
  selectDefaultSection = () => {
    const {sections = [], router} = this.props;
    const {section} = this.state;
    const keys = sections.map((o) => o.key);
    if (!keys.includes(section) && keys.length) {
      const sectionFromRoute =
        router && router.params && router.params.section
          ? sections.find(
              (o) => (o.key || '').toString().toLowerCase() === router.params.section.toLowerCase(),
            )
          : undefined;
  ...
  reportSectionSelection = () => {
    const {section} = this.state;
    const {onSectionChange, router, root, sections = []} = this.props;
    if (onSectionChange) {
  ...
  onSelectSection = (section) => {
    ...
    const {canNavigate, sections = []} = this.props;
    const currentSection = sections.find((o) => o.key === section);
    if (section === current || (currentSection && currentSection.disabled)) {
      return;
    }
    const onNavigate = () => {
      this.setState(
    ...
    canNavigatePromise().then((navigate) => (navigate ? onNavigate() : undefined));
  };
  ...
  renderSectionContent() {
    ...
    const {sections = [], children, router} = this.props;
    const currentSection = sections.find((o) => o.key === section);
    if (!currentSection) {
      return null;
    }
  ...
  render() {
    ...
    if (sections.length === 0) {
      return <div className={classNames(className, styles.container)}>{emptyDataPlaceholder}</div>;
    }
    const {section} = this.state;
    if (sections.length === 1 && hideListForSingleSection && section) {
      return (
        <div className={classNames(className, styles.container)}>{this.renderSectionContent()}</div>
      );
    }
    return (
