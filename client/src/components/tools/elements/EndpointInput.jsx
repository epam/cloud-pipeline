...
export function getValidationError(endpoint) {
  const {port, name, additional} = getValidationResult(endpoint);
...
export default class EndpointInput extends React.Component {
  ...
  get additionalConfigurationEditable() {
    ...
    if (authenticatedUserInfo.loaded) {
      const {admin, roles = []} = authenticatedUserInfo.value;
  ...
  composeValue = (options = {}) => {
    let {name, port, additional, isDefault, sslBackend, customDNS, sameTab} = options;
    ...
    const overlayMenuItems = [
      {
        ...
        label: (
          <>
            {this.isDefault ? <CheckOutlined /> : null}
      ...
      },
      {
        ...
        label: (
          <>
            {this.sslBackend ? <CheckOutlined /> : null}
      ...
      },
      {
        ...
        label: (
          <>
            {this.customDNS ? <CheckOutlined /> : null}
      ...
      },
      {
        ...
        label: (
          <>
            {this.sameTab ? <CheckOutlined /> : null}
    ...
    return (
      ...
      >
        <Row type="flex" align="top">
          <Col>
            ...
            {this.state.validation.port && (
              ...
              >
                <span>{this.state.validation.port}</span>
              </Row>
            )}
          </Col>
          <Col style={{width: 190}}>
          ...
          <Col style={{paddingLeft: 5, flex: 1, textAlign: 'right'}}>
            <Dropdown menu={{items: overlayMenuItems, onClick: onChange}} trigger={['click']}>
              <a>
                {options.join(', ')}
        ...
        {this.state.validation.additional && (
          ...
          >
            <span>{this.state.validation.additional}</span>
          </Row>
        )}
      </div>
    );
