...
import {RepositoryTypes, normalizeRepositoryType} from '../../special/git-repository-control';
...
class Folder extends localization.LocalizedReactComponent {
  ...
  renderMetadataKeyValue = (key, metadata) => {
    ...
    const renderSpecialMetadataComponent = () => {
      ...
      return (
        <div key={`${key}_value`} className="cp-text">
          <SpecialMetadataComponent key={key} metadata={metadata} showOnlySummary />
        </div>
      );
    };
    ...
    if (isSecret) {
      metadataValue = '*****';
    } else if (Object.hasOwn(SpecialTags, key)) {
    ...
    const metadataComponent = (
      <div key={key} className={styles.metadataItemContainer}>
        <Row className={classNames(styles.metadataItemKey, 'cp-library-metadata-item-key')}>
          {key}
        </Row>
        <Row
    ...
    return (
      <Tooltip
        ...
        overlay={
          <Row>
            <Row>{key}:</Row>
            <Row style={{wordBreak: 'break-word'}}>{value}</Row>
          </Row>
        }
      >
        {metadataComponent}
  ...
  renderMetadata = (metadata) => {
    ...
    for (const key in metadata) {
      if (Object.hasOwn(metadata, key)) {
        items.push(this.renderMetadataKeyValue(key, metadata[key]));
      }
    }
    if (items.length > MAX_INLINE_METADATA_KEYS) {
      ...
      return <Row className="object-metadata">{items}</Row>;
    }
  };
    ...
    },
    {
      ...
      render: (name, item) => {
        ...
        if (item.locked) {
          nameComponent = (
            <span>
              <LockOutlined /> <span className={`object-name ${boldClassName}`}>{name}</span>
            </span>
          );
        } else {
          nameComponent = (
            <span>
              <span className={`object-name ${boldClassName}`}>{name}</span>
            </span>
          );
        }
  ...
  renderTreeItemActions = (item) => {
    ...
    const issuesButton = (
      ...
      >
        <MessageOutlined />
        {item.issuesCount > 0 ? ` ${item.issuesCount}` : undefined}
      </Button>
    );
    switch (item.type) {
      case ItemTypes.folder:
        if (!item.isParent) {
          actions.push(issuesButton);
        }
      ...
      case ItemTypes.versionedStorage:
        ...
        break;
      case ItemTypes.storage:
        if (roleModel.isManager.storageAdmin(this) || roleModel.writeAllowed(item)) {
          actions.push(
            <Button
              key="edit"
              id={`folder-item-${item.key}-edit-button`}
  ...
  deleteStorage = async (cloud) => {
    ...
    const hide = message.loading(
      `${cloud ? 'Deleting' : 'Unregistering'} storage ${this.state.editableStorage.name}...`,
      0,
    );
    await request.fetch();
    hide();
  ...
  get deletingFolderIsEmpty() {
    ...
    return !(
      ...
      (this._folderToDeleteInfo.value.storages && this._folderToDeleteInfo.value.storages.length)
    );
  }
  ...
  addFolder = async ({name}) => {
    const request = new FolderRegister(
      this.state.folderTemplate ? this.state.folderTemplate.id : undefined,
    );
    const hide = message.loading(
  ...
  createStorage = async (storage) => {
    ...
    if (this.state.createOmicsStoreFlag) {
      payload = {
        ...
        regionId:
          (storage.serviceType === ServiceTypes.omicsRef ||
      ...
      payload = {
        ...
        regionId:
          storage.serviceType === ServiceTypes.objectStorage && storage.regionId
            ? storage.regionId
            : undefined,
  ...
  createVersionedStorage = async (opts = {}) => {
    const {pipelines, folder, onReloadTree} = this.props;
    const {name, description, foldersStructure} = opts;
  ...
  editPipeline = async (values) => {
    ...
    const objectName = /^versioned_storage$/i.test(pipelineType) ? 'versioned storage' : 'pipeline';
  ...
  deletePipeline = async (keepRepository) => {
    ...
    const hide = message.loading(
      `Deleting ${this.localizedString(objectName)} ${this.state.editablePipeline.name}...`,
      0,
    );
    await request.fetch();
    hide();
  ...
  editConfiguration = async (opts) => {
    const hide = message.loading(
      `Updating configuration '${this.state.editableConfiguration.name}'...`,
      0,
    );
    const payload = {
  ...
  deleteConfiguration = async () => {
    const hide = message.loading(
      `Removing configuration '${this.state.editableConfiguration.name}'...`,
      0,
    );
  ...
  renderContent = () => {
    const getRowClassName = (row) => {
      if (
        this.highlightedItem &&
        this.props.highlightByClick &&
        this.highlightedItem.id === row.id
      ) {
    ...
    return (
      <ContentIssuesMetadataPanel style={{flex: 1, overflow: 'auto'}} onPanelClose={onPanelClose}>
        <Table
          ...
          dataSource={formatTreeItems(this._currentFolder.data, {
        ...
        {this.showIssues && (
          <Issues
            ...
            entityDisplayName={
              this.state.issuesItem ? this.state.issuesItem.name : this._currentFolder.folder.name
            }
            entityId={this.state.issuesItem ? this.state.issuesItem.entityId : this.props.folderId}
            entityClass={this.state.issuesItem ? this.state.issuesItem.entityClass : 'FOLDER'}
            entity={this.state.issuesItem ? this.state.issuesItem : this.props.folder.value}
          />
        )}
        {this.showMetadata && this.props.folderId !== undefined && (
          <Metadata
            ...
            readOnly={
              !(
                roleModel.isOwner(this.props.folder.value) || roleModel.isManager.storageAdmin(this)
              )
            }
            entityName={this.props.folder.value.name}
            entityId={this.props.folderId}
            entityClass="FOLDER"
          />
        )}
        {this.showConfigurationPreview && (
          <PreviewConfiguration key={PREVIEW_PANEL_KEY} configurationId={this.highlightedItem.id} />
        )}
      </ContentIssuesMetadataPanel>
    );
  };
        ...
        () => {
          switch (type) {
            ...
            case folderKey:
              ...
              if (identifier) {
                const [template] = this.props.folderTemplates.value.filter(
                  (t) => t.id === identifier,
                );
    ...
    if (
      ...
      if (
        ...
        if (!this.props.templates.error && (this.props.templates.value || []).length > 0) {
          const templatesList = (this.props.templates.value || []).filter(
      ...
      if (createMenuItems.length > 0) {
        actions.push(
          <DropDownWrapper key="create actions" visible={this.state.createDropDownVisible}>
            <Dropdown
              trigger={['click']}
    ...
    if (displayOptionsMenuItems.length > 0) {
      actions.push(
        ...
        >
          ...
          >
            <Button id="display-attributes" size="small" className={styles.dropDownTrigger}>
    ...
    if (!this.props.isRoot && !this.props.listingMode) {
        ...
        editActions.push({
          ...
          label: (
            <span>
              ...
              {roleModel.writeAllowed(this.props.folder.value) ? 'Edit folder' : 'Permissions'}
            </span>
          ),
        });
      }
      ...
      if (
        ...
        (roleModel.isManager.storageAdmin(this) ||
          (roleModel.writeAllowed(this.props.folder.value) && roleModel.isManager.folder(this)))
      ) {
      ...
      if (editActions.length > 0) {
        ...
        actions.push(
          <DropDownWrapper key="edit" visible={this.state.editDropDownVisible}>
            <Dropdown
              placement="bottomRight"
              trigger={['click']}
              menu={{
  ...
  lockUnLockFolder = async (lock) => {
    ...
    const request = lock
      ? new FolderLock(this.props.folderId)
      : new FolderUnLock(this.props.folderId);
    await request.send({});
    if (request.error) {
      hide();
      message.error(request.error, 5);
  ...
  get showConfigurationPreview() {
    return !!(
      this.props.showConfigurationPreview &&
      this.state.showConfigurationPreview &&
      this.highlightedItem &&
  ...
  render() {
    ...
    if (this.props.folder.loaded) {
      if (
        !this.props.isRoot &&
        roleModel.readDenied(this.props.folder.value) &&
      ...
      let data = generateTreeData(this.props.folder.value, {
        ignoreChildren: true,
      ...
      if (this.props.isRoot) {
        ...
        if (!this.props.treatAsRootId || this.props.treatAsRootId !== this.props.folder.value.id) {
          if (this.props.folder.value.parentId) {
    ...
    const isProject = !!(
      ...
      this.props.folder.value.objectMetadata &&
    ...
    return (
      <div style={{display: 'flex', flexDirection: 'column', height: '100%'}}>
        ...
        <EditFolderForm
          ...
          title={
            this.state.folderTemplate
              ? `Create ${this.state.folderTemplate.id.toLowerCase()} folder`
              : 'Create folder'
          }
        ...
        >
          <Row>
            ...
            <Row className={styles.configurationConfirmBtn}>
              <Button key="cancel" size="large" onClick={this.closeDeleteFolderDialog}>
                Cancel
              </Button>
              <Button
                key="ok"
                ...
                size="large"
                disabled={
                  !this._folderToDeleteInfo ||
                  this._folderToDeleteInfo.pending ||
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (
      (this.state.folderToDelete && !prevState.folderToDelete) ||
