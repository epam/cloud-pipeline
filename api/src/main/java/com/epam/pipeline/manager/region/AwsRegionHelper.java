...
public class AwsRegionHelper implements CloudRegionHelper<AwsRegion, AbstractCloudRegionCredentials> {
    ...
    public AwsRegion mergeRegions(final AwsRegion originalRegion, final AwsRegion updatedRegion) {
        ...
        Optional.ofNullable(updatedRegion.getS3SendQueryAsSigned())
                .ifPresent(originalRegion::setS3SendQueryAsSigned);
        originalRegion.setOmicsServiceRole(updatedRegion.getOmicsServiceRole());
        originalRegion.setOmicsEcrUrl(updatedRegion.getOmicsEcrUrl());
