...
import {Period, getPeriod, getCurrentDate} from '../../../../special/periods';
...
import {COLUMNS, TABLE_CATEGORIES, TABLE_MODES, GROUPING} from './columns';
...
export default class UserInfoSummary extends React.Component {
  ...
  get statistics() {
    ...
    if (this.requests && this.requests.pipelines.loaded && this.requests.tools.loaded) {
  ...
  get tableData() {
    ...
    if (
      ...
      return [this.requests.pipelines, this.requests.tools, this.requests.storages]
        .reduce((acc, billing) => {
          const billingGroup = Object.values(billing._value || {}).map((billingEntry) => ({
            ...billingEntry,
  ...
  get filteredTableData() {
    const {tableMode} = this.state;
    if (tableMode) {
      return this.tableData.filter(({type}) => TABLE_CATEGORIES[tableMode].includes(type));
    }
  ...
  loadData = () => {
    ...
    this.setState({pending: true}, async () => {
      ...
      const range = ensureDayjs(period).format('YYYY-MM');
      ...
      const promises = Object.values(this.requests).map(
        (request) =>
          new Promise((resolve) => {
            request
              .fetch()
  ...
  changePeriod = (shift) => () => {
    this.setState(
      (prevState) => ({
        period: ensureDayjs(prevState.period).add(shift, 'month'),
      }),
      () => {
        this.loadData();
      },
  ...
  exportToCSV = () => {
    ...
    request.onreadystatechange = function () {
      ...
      if (request.status !== 200) {
        const error = request.statusText ? `Export error: ${request.statusText}` : 'Export error';
  ...
  renderUserStatistics = () => {
    ...
    const getLink = (item, type) => {
      if (type === 'tool') {
        ...
        return toolId ? <Link to={url}>{name}</Link> : name;
      }
      return item.fullName || item.name;
    };
    return (
      <div className={styles.statisticsContainer}>
        <p className={classNames(styles.statisticsBlock, 'cp-divider', 'right')}>
          <b>Most used tools (top {TOP_RUNS_AMOUNT})</b>
          {this.statistics.tools.map((tool) => (
            <span key={`${tool.name}_${tool.runsCount}`} style={{marginLeft: 5}}>
              {getLink(tool, 'tool')}
            </span>
          ))}
        </p>
        <p className={classNames(styles.statisticsBlock, 'cp-divider', 'right')}>
          ...
          {this.statistics.pipelines.map((pipeline) => (
            <span key={`${pipeline.name}_${pipeline.runsCount}`} style={{marginLeft: 5}}>
              {getLink(pipeline, 'pipeline')}
            </span>
          ))}
        </p>
        <p className={classNames(styles.statisticsBlock, 'cp-divider', 'right')}>
          <b>Last connection date</b>
          <span style={{marginLeft: 5}}>
            {displayDate(user.lastLoginDate, 'MMM D, YYYY, HH:mm')}
          </span>
        </p>
        <div
          className={classNames(styles.statisticsBlock, 'cp-divider', 'right')}
  ...
  renderTable = () => {
    ...
    return (
      <div className={styles.details}>
        <p className={styles.title}>Detailed usage data</p>
  ...
  renderTitle = () => {
    ...
    const nexNavigationDisabled = ensureDayjs(period)
      .add(1, 'month')
      .isSameOrAfter(getCurrentDate());
    return (
      <div className={styles.title}>
        ...
        <span>{`User statistics for ${currentPeriod}`}</span>
        <Button
          onClick={this.changePeriod(1)}
          size="small"
          style={{marginLeft: 10}}
  ...
  render() {
    ...
    return (
      <div className={classNames(styles.container, className)} style={style}>
        ...
        {this.renderTable()}
        <Button className={styles.exportBtn} type="primary" size="small" onClick={this.exportToCSV}>
          Export runs to CSV
        </Button>
      </div>
    );
