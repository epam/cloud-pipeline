...
get_tools() {
    ...
    if [ $? -eq 0 ]; then
        for registry in $(echo "$registries_tree" | jq -r '.payload | .registries[].path'); do
            ...
            echoerr "REGISTRY: $registry | Tools metadata will be stored in ${registry_output_dir}"
...
function process_tool() {
  ...
  for tag in ${tool_versions}; do
      ...
      if [ $? -ne 0 ]; then
          echoerr "    TOOL: ${tool} | Problem with preparing tool tag metadata: ${tag}'!"
...
function process_tool_version() {
    ...
    echoerr "      TOOL VERSION: ${tool}:${tag} | Loading configuration. Metadata will be stored in ${tool_versions_output_dir}"
