...
const Name = {
  ...
  renderFn: (value, document, onClick, renderCheckBox) => (
    ...
    >
      ...
      {renderIcon(document)}
      <span className={classNames('cp-ellipsis-text', 'cp-search-result-item-main')}>
        <b>{getDocumentName(document)}</b>
      </span>
    </div>
  ),
  width: '25%',
...
const Changed = {
  ...
  width: '15%',
  renderFn: (value) => (
    <span className="cp-ellipsis-text">{displayDate(value, 'MMM D, YYYY, HH:mm')}</span>
  ),
  types: new Set([
...
const Size = {
  ...
  name: 'Size',
  width: '15%',
  renderFn: (value) => <span className="cp-ellipsis-text">{displaySize(value, false)}</span>,
  types: new Set([
    SearchItemTypes.s3File,
    SearchItemTypes.gsFile,
    SearchItemTypes.azFile,
...
const otherColumns = DocumentColumns.filter(
  (aColumn) => !mainColumns.includes(aColumn) && !detailsColumns.includes(aColumn),
);
...
function getColumnsFromGroup(group, options = {}) {
  ...
  const {extraColumns = [], facetsColumns = []} = options || {};
...
function getColumns(config, options = {}) {
  ...
  if (typeof config === 'string') {
    const findColumnByProperty = (propertyName) =>
      DocumentColumns.find(
        (aColumn) => (aColumn[propertyName] || '').toLowerCase() === config.toLowerCase(),
      );
 ...
 * @returns {ColumnConfig[]}
 */
function getDefaultColumns(options = {}) {
  const {searchColumnsOrder = DEFAULT_ORDER} = options || {};
  const order =
    searchColumnsOrder && searchColumnsOrder.length ? searchColumnsOrder : DEFAULT_ORDER;
  const result = [];
  ...
  const columnAdded = (aColumn) => result.some((item) => item.key === aColumn.key);
  for (let p = 0; p < parsed.length; p++) {
    const {columns = [], group = false} = parsed[p];
    const items = group ? columns.filter((aColumn) => !columnDefinedExplicitly(aColumn)) : columns;
...
function filterDisplayedColumns(columns = [], documentTypes = []) {
  ...
  if (documentTypes.length > 0) {
    shouldExcludePathColumn = !documentTypes.some(
      (type) => !SearchGroupTypes.storage.types.includes(type),
    );
  }
  return columns.filter((column) => !shouldExcludePathColumn || column !== Path);
}
