...
> On connect, the extension adds the settings below to the **Remote [SSH]** settings of the instance (`~/.vscode-server/data/Machine/settings.json`, or `~/.cursor-server/data/Machine/settings.json` in Cursor).  
> They keep search, file watching and chat assistants (e.g. GitHub Copilot Chat) out of the mounted data storages under `~/cloud-data`. Without them a chat request may hang for a long time.  
>
> ```json
> {
...
> ```
>
> - only missing settings are added, values already set on the instance are kept
> - the settings file shall be plain JSON. If it contains comments or trailing commas - nothing is added
> - `~/cloud-data` is hidden from the VSCode explorer, the storages stay available in the terminal
> - to change the list - edit `cloudPipeline.remoteMachineSettings.values` in the VSCode settings. Set a key to `null` to skip it
> - to disable this behavior - set `cloudPipeline.remoteMachineSettings.enabled` to `false`
