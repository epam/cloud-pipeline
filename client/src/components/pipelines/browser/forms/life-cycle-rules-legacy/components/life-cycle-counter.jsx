...
class LifeCycleCounter extends React.Component {
  ...
  componentDidUpdate(prevProps) {
    ...
    if (visible && (storage !== prevProps.storage || path !== prevProps.path)) {
      this.fetchRulesInfo();
    }
  }
  ...
  get isS3Storage() {
    ...
    return storage && storage.id && /^s3$/i.test(storage.storageType || storage.type);
  }
    ...
    if (status === STATUS.SUCCEEDED) {
      return (
        <div>
          ...
          {restoredTill ? <span>Folder is restored till {displayDate(restoredTill)}.</span> : null}
        </div>
      );
    }
  ...
  render() {
    ...
    const {restoreEnabled, visible} = this.props;
    ...
    return (
      <div className={styles.container}>
        ...
        <div>{restoreEnabled ? this.renderRestoreActions() : null}</div>
      </div>
    );
  }
}
