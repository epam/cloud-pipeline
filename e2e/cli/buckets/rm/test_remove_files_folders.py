...
class TestRmFileFolder(object):
    ...
    def test_remove_file(self, path):
        try:
            ...
            assert not s3_object_exists(self.bucket_name, path), "Failed to delete s3 file object {}".format(path)
    ...
    def test_remove_folder(self, path):
        try:
            ...
            assert not s3_folder_exists(self.bucket_name, path), "Failed to delete s3 folder {}".format(path)
        except AssertionError as e:
            pytest.fail("Test case {} failed. {}".format(self.epam_test_case_rm_folder, e.message))
    ...
    def test_remove_with_confirmation(self):
        try:
            ...
            aws_output = get_aws_listing("{}/{}".format(self.bucket_name,
                                                        self.epam_test_case_rm_with_confirmation + ".txt"))
    ...
    def test_remove_folder_with_same_name_as_file(self):
        ...
        try:
            ...
            assert s3_object_exists(self.bucket_name, "%s/%s" % (case, self.file_in_root))
            pipe_storage_cp(self.root_file_path, "cp://%s/%s/%s/" % (self.bucket_name, case, self.file_in_root))
            assert s3_object_exists(self.bucket_name, "%s/%s/%s" % (case, self.file_in_root, self.file_in_root))

            pipe_storage_rm("cp://%s/%s/%s/" % (self.bucket_name, case, self.file_in_root), recursive=True)
            assert s3_object_exists(self.bucket_name, "%s/%s" % (case, self.file_in_root))
            assert not s3_object_exists(self.bucket_name, "%s/%s/%s" % (case, self.file_in_root, self.file_in_root))
    ...
    def test_remove_file_with_same_name_as_folder(self):
        ...
        try:
            ...
            pipe_storage_rm("cp://%s/%s/%s" % (self.bucket_name, case, self.file_in_root), recursive=True)
            assert not s3_object_exists(self.bucket_name, "%s/%s" % (case, self.file_in_root))
            assert s3_object_exists(self.bucket_name, "%s/%s/%s" % (case, self.file_in_root, self.file_in_root))
        except BaseException as e:
            pytest.fail("Test case {} failed. {}".format(case, e.message))
