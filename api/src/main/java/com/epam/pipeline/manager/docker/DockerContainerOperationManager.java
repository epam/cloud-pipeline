...
import com.epam.pipeline.manager.cluster.ClusterManager;
...
public class DockerContainerOperationManager {
    ...
    public void resumeRun(PipelineRun run, List<String> endpoints) {
        try {
            ...
            String awsRegion = run.getInstance().getAwsRegionId();
            clusterManager.startInstance(run.getInstance().getNodeId(), awsRegion);

            kubernetesManager.waitForNodeReady(run.getInstance().getNodeName(), run.getId().toString(), awsRegion);
