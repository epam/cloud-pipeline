...
- [Pool utilization](#pool-utilization)
    - [Navigation from the Hot node pool tab](#navigation-from-the-hot-node-pool-tab)
    - [Managing preferences](#managing-preferences)

> To view and manage node pools you need to have **ROLE\_ADMIN** role.
...
For the viewing of the hot node pool utilization, there is a separate tab **Pools usage** in the **Cluster state** section:  
    ![CP_HotNodePoolsUtilization](attachments/PoolUtilization_01.png)

At this tab, there are:

- panel with a chart showing the utilization of all existing hot node pools (_left_ chart):
    ...
    - user can hover any point of the desired hot node pool - the following info will appear:
        ...
        - hot node pool name
        - maximum utilization (in _percent_) of that pool on that time range - i.e. the ratio of the maximum number of busy nodes to the total possible number of nodes on that time range  
        ![CP_HotNodePoolsUtilization](attachments/PoolUtilization_02.png)
- panel with a chart showing the utilization of a certain selected hot node pool (_right_ chart):
    ...
    - user can hover any point of the selected hot node pool - the following info will appear:
        ...
        ![CP_HotNodePoolsUtilization](attachments/PoolUtilization_03.png)
    - user can hover any point of the hot node pool "limit" line - the following info will appear:
        ...
        ![CP_HotNodePoolsUtilization](attachments/PoolUtilization_04.png)
...
By default, charts are shown for the today. Via the **Calendar** control above the charts, there is the ability to select another day. Charts will be redrawn:  
    ![CP_HotNodePoolsUtilization](attachments/PoolUtilization_05.png)

Also, period of the report can be change from the "_Day_" to "_Month_" - by the corresponding control above the charts. In this case, charts will be redrawn for the current month, e.g.:  
    ![CP_HotNodePoolsUtilization](attachments/PoolUtilization_06.png)

Via the **Calendar** control above the charts, there is the ability to select another month:  
    ![CP_HotNodePoolsUtilization](attachments/PoolUtilization_07.png)

To view details of the another hot node pool utilization - select it in the dropdown above the right chart, e.g.:  
    ![CP_HotNodePoolsUtilization](attachments/PoolUtilization_08.png)
...
For that, click the "chart" button at the pool's tile:  
    ![CP_HotNodePoolsUtilization](attachments/PoolUtilization_09.png)
...
|---|---|
| **`monitoring.node.pool.usage.enable`** | Allows to enable monitoring of the hot node pools usage |
| **`monitoring.node.pool.usage.delay`** | Determines the frequency of pools load measurements (in _milliseconds_). Default - 5 minutes |
| **`monitoring.node.pool.usage.store.days`** | Determines the storing period for the monitoring data (in _days_) - all data older that period will be cleaned if `monitoring.node.pool.usage.clean.enable` set to `true`.<br>Default value - 365 days |
| **`monitoring.node.pool.usage.clean.enable`** | Allows to enable cleaning of the hot node pools usage monitoring data |
| **`monitoring.node.pool.usage.clean.delay`** | Determines the frequency of cleaning of pools usage monitoring data (in _milliseconds_).<br>Data will be cleaned every `monitoring.node.pool.usage.clean.delay` period, if `monitoring.node.pool.usage.clean.enable` is set to `true`.<br>Default value - 24 hours |
