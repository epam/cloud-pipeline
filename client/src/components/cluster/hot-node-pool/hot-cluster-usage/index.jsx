...
import colors, {getColor, backgroundColor, lineColor, textColor} from './charts/utils/colors';
import styles from './hot-cluster-usage.module.css';
...
@inject((stores, params) => {
  ...
  return {
    currentPoolId: Number.isNaN(Number(poolString)) ? undefined : Number(poolString),
  };
})
@observer
class HotClusterUsage extends React.Component {
  ...
  get backgroundColor() {
    const {themes} = this.props;
    if (themes && themes.currentThemeConfiguration) {
      return themes.currentThemeConfiguration['--cp-color-bg-elevated'] || backgroundColor;
    }
  ...
  toggleHiddenPools = (event, {datasetIndex}) => {
    const {hiddenPools, pools = []} = this.state;
  ...
  render() {
    ...
    const currentPoolData = (data || []).find(({poolId}) => poolId === currentPoolId);
    ...
    return (
      <div>
        ...
        {pending && !data && <LoadingView />}
        {data && (
          <div
            className={classNames(styles.chartsContainer, {
          ...
          >
            ...
            {resourceSharingPool && (
              <PoolsHardwareChart
                ...
                mappings={[HARDWARE_MAPPING.runs, HARDWARE_MAPPING.runsPending]}
                title={
