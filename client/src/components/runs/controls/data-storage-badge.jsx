...
import styles from './data-storage-badge.module.css';
...
class DataStorageBadge extends React.Component {
  ...
  render() {
    ...
    return (
      <Link className={styles.storageItem} to={`/storage/${this.storageInfo.id}`}>
        <AWSRegionTag regionId={this.storageInfo.regionId} />
        <span
          className={classNames(styles.storageName, {
            [styles.sensitiveStorageName]: this.storageInfo.sensitive,
          })}
        >
          {this.storageInfo.name}
        </span>
      </Link>
    );
  }
