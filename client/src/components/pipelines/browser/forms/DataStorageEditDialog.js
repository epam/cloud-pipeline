...
@inject('awsRegions')
@Form.create()
export default class DataStorageEditDialog extends React.Component {
  ...
  handleSubmit = (e) => {
    ...
    this.props.form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        values.type = this.isNfsMount ? 'NFS' : 'S3';
        if (!this.isNfsMount && this.state.versioningEnabled) {
  ...
  validateNfsStoragePath = (value, callback) => {
    if (value) {
      ...
      <Modal
        ...
        title={
          this.props.dataStorage
            ? (this.isNfsMount ? 'Edit NFS mount' : 'Edit object storage')
            : (this.isNfsMount
              ? 'Create NFS mount'
          ...
          <Tabs
            ...
            <Tabs.TabPane key="info" tab="Info">
              <Form id="edit-storage-form">
                <Form.Item
                  ...
                  {getFieldDecorator('path', {
                    rules: [{
                      ...
                      validator: (rule, value, callback) => this.validateNfsStoragePath(value, callback)
                    }],
                    initialValue: this.props.dataStorage ? this.props.dataStorage.path : undefined
                  })(
                    <DataStoragePathInput
                      awsRegions={this.awsRegions}
                      onValidation={this.onNfsPathValidation}
                      onPressEnter={this.handleSubmit}
                      visible={this.props.visible}
                      isNfs={this.isNfsMount}
                ...
                {
                  !this.isNfsMount && this.state.versioningEnabled &&
                    <Form.Item
                      className={styles.dataStorageFormItem}
                      {...this.formItemLayout}
