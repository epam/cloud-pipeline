...
import {BarchartDataLabelPlugin, ChartClickPlugin, HighlightTicksPlugin} from './extensions';
...
function getItemValue(item, ...keys) {
  const [key, ...rest] = keys;
  if (!item || !key || !Object.hasOwn(item, key)) {
    return undefined;
  }
...
function BarChart({
  ...
  const currentDataset =
    datasets.find((o) => o.main) || datasets.find((o) => !o.isPrevious) || datasets[0];
  ...
  const processedDatasets = datasets
    .map((dataset) => {
      const {isPrevious = false, sample, ...options} = dataset;
    ...
    tooltips: {
      ...
      callbacks: {
        label: function (tooltipItem, chart) {
          ...
          const {label, showTooltip = true, tooltipValue = (o) => o} = dataset || {};
