...
import styles from './DetachedConfiguration.module.css';
import Schedule from './schedule';
import browserStyles from '../browser/Browser.module.css';
...
class DetachedConfiguration extends localization.LocalizedReactComponent {
  ...
  get selectedConfiguration() {
    if (this.props.configurations.loaded && this.props.configurations.value.entries) {
      const [configuration] = this.props.configurations.value.entries.filter(
        (c) => c.name === this.selectedConfigurationName,
      );
  ...
  get selectedConfigurationIsDefault() {
    ...
    if (this.props.configurations.loaded && this.props.configurations.value.entries.length > 0) {
      const [configuration] = this.props.configurations.value.entries.filter(
        (c) => c.name.toLowerCase() === this.props.currentConfiguration.toLowerCase(),
      );
      if (configuration) {
  ...
  onRemoveConfigurationClicked = (configuration) => (e) => {
    ...
    const removeConfiguration = async () => {
      ...
      const [removableConfiguration] = entries.filter(
        (c) => c.name.toLowerCase() === configuration.name.toLowerCase(),
      );
      ...
      if (index >= 0) {
        ...
        const payload = {
          ...
          parentId: this.props.configurations.value.parent
  ...
  createConfigurationForm = async ({name, description, template}) => {
    ...
    const [configuration] = entries.filter(
      (c) => template && c.name.toLowerCase() === template.toLowerCase(),
    );
    if (configuration) {
  ...
  onSetAsDefault = async () => {
    if (
      ...
      const entries = this.props.configurations.value.entries;
      const [configuration] = entries.filter(
        (c) => c.name.toLowerCase() === this.selectedConfigurationName.toLowerCase(),
      );
      if (configuration) {
        entries.forEach((c) => {
          c.default = false;
        ...
        const hide = message.loading(
          `Updating '${this.selectedConfigurationName}' configuration ...`,
          0,
        );
        const request = new ConfigurationUpdate();
        const payload = {
  ...
  onSaveConfiguration = async (opts) => {
    if (
      ...
      if (
        entries.filter(
          (c) =>
            c.name.toLowerCase() !== this.selectedConfigurationName.toLowerCase() &&
            c.name === opts.configuration.name,
        ).length > 0
      ) {
      ...
      if (configuration) {
        ...
        if (configuration.executionEnvironment === DTS_ENVIRONMENT) {
          for (const key in opts) {
            if (Object.hasOwn(opts, key) && opts[key] !== undefined) {
          ...
          this.setState(
            ...
            () => {
              if (this.selectedConfigurationName !== configuration.name) {
                this.allowedNavigation = `/configuration/${this.props.configurationId}/${configuration.name}`;
  ...
  get selectedFireCloudMethod() {
    if (this.selectedConfiguration) {
      ...
      if (configuration && configuration.methodName && configuration.methodSnapshot) {
  ...
  getParameters = () => {
    ...
    for (const parameter in parameters) {
      if (
        this.state.selectedPipelineParameters &&
        Object.hasOwn(this.state.selectedPipelineParameters, parameter)
      ) {
        parameters[parameter].readOnly = !!this.state.selectedPipelineParameters[parameter].value;
      } else {
  ...
  getSelectedPipeline = () => {
    ...
    if (configuration && configuration.pipelineId && configuration.pipelineVersion) {
      const pipeline = this.getPipelines().find((p) => `${p.id}` === `${configuration.pipelineId}`);
      if (pipeline) {
        return pipeline;
      } else {
        return {
    ...
    if (selectedPipline) {
      ...
      if (!request.error) {
        const [config] = selectedPiplineVersionConfiguration
          ? request.value
              .map((c) => c)
              .filter((c) => c.name === selectedPiplineVersionConfiguration)
  ...
  runCluster = (opts, entitiesIds, metadataClass, expansionExpression, folderId) => {
    const launchFn = async () => {
      ...
      if (configuration) {
        ...
        configuration.executionEnvironment = opts.executionEnvironment;
        configuration.rootEntityId = this.selectedConfigurationIsDefault
          ? opts.rootEntityId
          : undefined;
        opts.pipelineId = undefined;
        opts.pipelineVersion = undefined;
  ...
  editConfiguration = async (opts) => {
    const hide = message.loading(
      `Updating configuration '${this.props.configurations.value.name}'...`,
      0,
    );
    const request = new ConfigurationUpdate();
    const payload = {
    ...
    if (request.error) {
      ...
      this.props.router.push(
        `/configuration/${this.props.configurationId}/${this.selectedConfigurationName}`,
      );
    }
  };
  ...
  renameConfiguration = async (name) => {
    const hide = message.loading(
      `Renaming configuration '${this.props.configurations.value.name}'...`,
      0,
    );
    const request = new ConfigurationUpdate();
    const payload = {
  ...
  deleteConfiguration = async () => {
    const hide = message.loading(
      `Deleting configuration '${this.props.configurations.value.name}'...`,
      0,
    );
  ...
  render() {
    ...
    const configurationTitleClassName = this.props.configurations.value.locked
      ? browserStyles.readonly
      : undefined;
    return (
      <div className={styles.fullHeightContainer}>
        ...
        >
          ...
          <Col className={styles.actionButtons}>
            <Schedule configurationId={this.props.configurationId} />
  ...
  componentDidUpdate(prevProps) {
    ...
    if (
      prevProps.currentConfiguration !== this.props.currentConfiguration ||
