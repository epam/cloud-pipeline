...
function TasksGroup(props) {
  const {className, style, tasksGroup, active, onClick} = props;
  ...
  const stats = getSortedTaskStatuses(tasksGroup ? tasksGroup.stats : [], false).map((st) => (
  ...
  return (
    <div
      className={classNames(className, 'cp-run-engine-tasks-group', styles.tasksGroup, {
    ...
    >
      <div className={styles.tasksGroupInfo}>
        <div className={styles.tasksGroupHeader}>
          ...
          <div className={styles.tasksGroupStats}>{stats}</div>
        </div>
